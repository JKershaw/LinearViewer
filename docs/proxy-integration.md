...
**`maxTasks` / `repo` (LIN-2975).** `maxTasks` is the scope bound this dispatch declared (or inherited via `sessionId`-stamping), `null` when unbounded — see the task-budget-exhausted response under [Enqueue a Dispatch](#enqueue-a-dispatch). `repo` is the validated/normalized repo basename this item was queued against, `null` for the workspace's default folder. Both are also on `GET /api/proxy/dispatch` (list) below, so verifying budget/repo stamping across a run's whole worker set doesn't require a per-row detail read — **verify stamping with this endpoint (or the list), never assume a field is `null` because you didn't check it.**
