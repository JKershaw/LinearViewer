...
**Verification pass (2026-06-10, HEAD `8b603e7`):** every code-level claim below was re-checked
against main after the notes were first written — three prompt-system changes landed in between
(LIN-356 provider-routed dashboard reads, LIN-364 terminal-parent steering, LIN-366
bug-investigation loop break) and none invalidate a finding; they extend the prompt-machinery
mapping in the addendum. Two previously-open questions are now closed; see the
"Verification at HEAD" section at the end.
...
1. **Stage-0-lite (deterministic backstops; the real gap).** Enable branch protection on `main`
   requiring `ci-success` (confirmed absent — see Verification at HEAD) and no self-merge for
   agent identities on control-plane paths; add a
...
must precede anything else is item 1's token fix and enabling branch protection, both small.

## Verification at HEAD (2026-06-10, `8b603e7`)

A second pass re-grounded every claim above against main at HEAD, after LIN-356, LIN-364, and
LIN-366 merged. Results:

**Closed questions (previously flagged for owner confirmation):**

- **Branch protection is NOT enabled.** The GitHub API reports `protected: false` for `main`.
  The `ci-success` aggregate job exists in `.github/workflows/test.yml` but nothing requires it;
  direct pushes and self-merges are platform-permitted. This converts the Stage-0-lite
  recommendation's first action from "confirm" to "enable" — and it strengthens the research's
  case, since the write-mode kickoff's merge authorization currently rests on prose plus an
  unprotected branch.
- **LIN-289 / LIN-291 / LIN-292 are confirmed Canceled in Linear** (the coded drift-defense
  subsystem), and **LIN-325 (operating manual) is Done** — the "handbook instead of coded gates"
  decision is real and current, not stale ticket prose. LIN-45 (fuzzy-test recommendations)
  remains Todo.

**Re-verified, unchanged:**

- `lib/periodicals.js`: five templates (documentation-review, test-coverage-gap, security-review,
  api-quality, code-quality); `cadence`/`lastRunAt` declared "carried, not yet consumed" in the
  JSDoc and grep confirms zero consumers outside the module. No scheduler; `autopilot.md` still
  lists periodical-cadence state as deferred (§8.C territory).
- The standing readWrite token debt is self-documented at `routes/proxy.js`
  (`buildProxyContextPreamble`): "SECURITY DEBT — revisit (do not ship to broad use as-is) …
  Planned hardening: mint a per-dispatch, short-TTL token bound to this item with a narrow scope,
  mirroring the Harbour per-item feedback token. For now (by explicit choice): standing
  readWrite." `lib/harbour-feedback-tokens.js` exists as the precedent.
- Write-mode kickoff still authorizes self-merge: "You may drive a task all the way to a merge
  *once CI is green and the diff matches what was approved* — the merge is yours."
- Foreman 3-strikes counters, halt-on-non-2xx, and the `kind` field on dispatch items all
  present; no CODEOWNERS file; no test-modified-with-code trigger anywhere in
  `lib/prompt-template-defs.js`, `lib/periodicals.js`, or `lib/prompts/`.
- `lib/report-history-store.js` and `scripts/eval/` (plus `scripts/eval-completeness-check.mjs`)
  exist as claimed — the trend-instrument substrate and the recommender-layer eval harness.

**New since the notes were written (extends, does not contradict):** LIN-364 (open parent with
all-complete subtasks steered to review/close) and LIN-366 (bug-already-investigated escape
hatch) added two more both-paths prompt branches — further instances of the pattern the addendum
documents: task-level controls landing as prompt machinery, each one a prose proxy by the
research's definition.
