...
| 2 (assistant/copilot use) | 54.77% have used GitHub Copilot in the past year | n=22,078 (of 40,307 total respondents answering this section) | Stack Overflow 2023 Developer Survey | mid-Jun 2023 | May 2023 (Wayback root capture 20230523133126, mid-fielding — evidences the survey was live, not the later publication) |
...
**Frontier gate.** DORA's own 2025 language: for an individual, friction "doesn't vanish so much as move: it shifts from manual grind to deciding and verifying." Trust fell as use rose on one instrument in this window: Stack Overflow's trust-in-accuracy figure worsened year over year, while DORA's "little or no trust in AI-generated code" figure moved the opposite way (see the disconfirming section). Budget is not named as an individual-level gate by any source eligible here.
...
**Rung 4 has no same-instrument series across cutoffs B and C.** Stack Overflow's 31% (mid-2025, n=49,009) and The Pragmatic Engineer's 55% (early 2026, n=906) are different instruments, different populations, and an order of magnitude apart in sample size. Presented as 31% → 55% movement this would repeat exactly the fault a review of the first paper found (`docs/papers/harbour/developer-adoption-ladder.md@7f1da76f`, citing the LIN-2925 review of 10:28 UTC 19 Sep) — the two figures sit in the cross-instrument table above, labelled, each with its own `n`, and are not read as a series anywhere in this paper.
...
**Trust falls as use rises, on one instrument — not two.** Stack Overflow's trust-in-accuracy figure worsened from a combined 43.0% trusting (2.7% highly, 40.3% somewhat) in 2024 to 33% trusting (3.1% highly) against 46% distrusting in 2025 — the wrong way for a ladder whose gate is supposed to accumulate with use. DORA's "little or no trust in AI-generated code" figure moves the other direction: it *fell* from 39% (2024 report) to 30% (2025 report), meaning fewer developers reported little or no trust as adoption approached 90% — trust rising, not falling. DORA does not support this pattern; only Stack Overflow does.
...
**Mapping rule.** Each source's own categories map onto the six rungs one way only, from source to rung, never the reverse. Where the same instrument recurs across sections — Stack Overflow's "use AI agents" question, JetBrains' broad "assistant, agent, or code editor" wording, Anthropic's auto-approve series — this paper reuses the first paper's placement of it exactly (`docs/papers/harbour/developer-adoption-ladder.md@7f1da76f`, Findings), rather than re-deriving a mapping.
