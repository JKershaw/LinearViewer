...
| [cheap-implementer](harbour/cheap-implementer.md) | Can a cheap model implement Harbour tickets to Opus's review standard, and what does it cost? | Yes, on small tickets: thirteen PRs from three models, eight first-pass, all within one round, no logic defect found. The Opus review is the cost, $2 to $5 each and flat with size. Close-out costs the same again, so the subscription pays about $8.80 of Opus per cheap ticket. A cheap shadow reviewer agreed with Opus on every clean PR, matched one second-order finding cold and missed the hardest one. The floor is the older DeepSeek Flash; gpt-oss-120b and an 8B model produced no work at all. An evening of one-ticket experiments then found Flash handles an hour-plus ticket, a thin ticket and a stepped run to Opus's standard, closes out reviewed tickets after a human reads the ledger, and writes research plan-review does not fault; its plans went round twice. |
