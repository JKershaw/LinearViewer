...
model: claude-opus-5 and claude-sonnet-5, six sessions, effort null, harness null (LIN-2364)
grounded_at: bbb60dd3
cites: [docs/reviews/recent-headwinds-review-2026-08-29.md@46c155cf, docs/reviews/recent-headwinds-review-2026-08-23.md@46c155cf, LIN-2364 2026-08-29, LIN-2254 2026-08-29]
---

> **A copy, and what the header could not hold.** The original is
> `docs/reviews/recent-headwinds-review-2026-08-29.md`, untouched; this is a copy, made by
> `archive-check.md` to test version 3 of the header against the hardest fit in `docs/reviews/`.
> The body below is unchanged. Five fields strain, and the fifth is the rule I just added:
>
> - **`grounded_at` is one commit; this review has two.** It is grounded at `LinearViewer` @
>   `bbb60dd3` and `simple-dispatcher` @ `972f95cb`, reported side by side and "never summed or
>   averaged together". The field carries the first and silently drops the second.
> - **`version` means a document's revision; this is edition seven of a periodical.** It reads
>   its predecessor in full, supersedes named metrics across editions (`external-injection-break`
>   superseded → `credential-defect-cluster`), and corrects itself in place ("Disposition: fixed
>   in place"). `version: 7` above is a guess at which of those the field counts.
> - **`model` is one value; the run was six sessions on two models.** LIN-2364's lineage puts
>   autopilot, research, review and close-out on `claude-opus-5` and plan and implementation on
>   `claude-sonnet-5`, $34.40 total, with `effort` and `harness` null on all six. The field
>   cannot say which model wrote which paragraph.
> - **A locator needs something that stays still.** Several figures here are live instrument
>   readings taken at write time — `/api/proxy/north-star` reported "stale", 19.3 days old; a
>   92-ticket `/cost` sweep — which have no path, no sha and no comment date. `cites` can name
>   the endpoint; it cannot make the reading reproducible.
> - **Rule 3 asks for a Next; this document's policy is "no fix-tasks minted".** A required
>   Next section would contradict the advisory stance the review states in its second line.
