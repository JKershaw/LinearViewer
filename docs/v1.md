...
4. **Start one task.** *"There's just a great big button that basically says Go."* The button does not literally say Go: it produces the next step, with the reasoning for choosing it and a prompt tailored to the task, and a prominent copy. Beside it sit the rungs of the same move with more handed over: run this step for me, then run the whole task, which runs one task, bounded, and hands back. It is not the standing loop. Rungs not yet set up are shown, never hidden, and pressing one says what it needs. Which way the person took the task is recorded, including a press on a rung not yet set up.
...
7. **Close out.** A user's run stops after review with the PR ready; close-out is theirs to send. It re-checks CI on the exact commit, settles the ledger, merges, and marks the task done. That press, or their own merge on GitHub, is the only path to main.
...
- **A run is one page per task,** in the shape of a CI pipeline page, at its own URL. The live view is that page unfinished; once finished it is a record. It is the operator's session page re-scoped to one task and translated out of Harbour's own vocabulary, not a page built from nothing. It reads in layers: a glance, a check, and a deep read where every claim is one click from its evidence.
- **While it runs:** progress through the steps is the one number; money shows as information when it is known and is left out, never guessed, when it is not; active running time is kept apart from wall-clock time; there is no cap on the page, though the bound still guards the run. "Nothing merged" is said as a promise and, once there is a PR, as its live state.
...
- **The share link is the same page seen as a guest,** at an unguessable key in the URL (the mechanism of LIN-3073): no actions, and nothing behind the clicks, meaning no prompts, transcripts, comments, diffs or cost; secret-scanned and previewed by its owner before it is sent.
- **Observation is not a rung.** It observes the runs at every rung, the way an operations platform grows from logs into a health view (ruling `lin2930-observation-rung`: rung-agnostic).
...
- **Execution.** Runs execute wherever a consumer polls the workspace (open by design, LIN-2938), with cheap models through OpenRouter on the work-shaped legs and the frontier only where judgement is bought. John: *"even if right now you just plug in OpenRouter and everything runs through the cloud, that's still a really good product."*
- **Pricing.** The free tier is unlimited copyable prompts plus a small, published number of cloud runs. The paid tier is more runs on a small monthly subscription, or the person's own key. John: *"potentially the cloud is the $5 subscription and you get copyable prompts, or something like that; we need to figure out the detail."* The number is decided after the dress rehearsal, not before. One tension to settle with it (found at the LIN-2947 sitting): the first delight is now the AI-tailored prompt, which today counts against the free tier's daily prompt quota (`lib/free-tier-store.js`), so "unlimited copyable prompts" is true only of the handwritten templates. Whether the tailored prompt is unlimited, generously capped, or part of the paid line is the likeliest thing here to change, and is decided with the free number.
- **Entry.** Most arrivals start with the tailored prompt and climb to the handover once they trust it. The draft of 19 September held the opposite; the first screen now bets this way, and the mode record, counting presses on rungs not yet set up, is how v1 finds out.
...
| See the work | The stack (`lib/task-stack.js`), the home list and Swipe | Home and Swipe stay. The opened task becomes one component shared by both: the next step with its reasoning and copy, the rungs, the proxy on and labelled plainly, the rest as a quiet row (LIN-2944). |
...
| Watch the run | The Observation view and session pages, built for the operator | The session page re-scoped to one task under a run id, translated for a person who did not build Harbour, with its questions pinned to it (LIN-2948). |
...
| Close out | Close-out owns the merge today | For a user, the run stops after review and close-out is theirs to send (LIN-2949). |
| Share the link | Nothing; LIN-3073 builds the key-in-a-URL share for task collections | The run page in guest mode, on the same mechanism (LIN-2950). |
...
| Hardening gate | The security review periodical; the north star's rule that secure defaults assume hostile input | Per-user token scoping, repo content treated as untrusted, the user's close-out press as the only write to main, private by design written down with a privacy page that tells the truth and a retention policy for every store (LIN-2954), reviewed before the first invitation. |
...
1. Go as a bounded one-task run, with the three reporting fixes. The design track opens here.
2. The run experience, the PR with its evidence, the close-out click, the share link.
3. The funnel and the mode record.
4. The free tier for runs.
5. The hardening gate.
6. **The dress rehearsal.** A fresh account, a repo Harbour has never seen, all eight steps, a result. v1 is done here.
7. The invited period: specific people, measured by counts and by what they choose to share. Billing, then the paper on what they did.
