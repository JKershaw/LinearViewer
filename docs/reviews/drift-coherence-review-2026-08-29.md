...
**Headline: a re-baseline, not a small delta.** ~973 commits landed in `LinearViewer/` and ~414 in `simple-dispatcher/` since 2026-06-25 — the provider family grew from 2 to 5 (linear, github, github-projects, jira, local), and this is the first edition to substantively audit `simple-dispatcher/` at all. **Static import cycles are confirmed at HEAD in two distinct clusters** — a new one in `LinearViewer/lib/`'s KPI/budget modules (three elementary cycles over three modules), never seen in any prior edition, and a second at the provider seam, corrected first by this run's required adversarial second-read and again by its independent review, closing through the provider identity barrel (`lib/providers/index.js`) and `lib/render-pages.js`/`lib/components/navbar.js` rather than the `registry.js` path every prior edition checked. All are latent, not live. The **fourth consecutive worsening** of the provider auth-router dependency inversion (edge count 2→3→4) is the same seam as the second cycle; its standing fix (LIN-675) has sat in Backlog, untouched, across all four editions. A helper-signature mismatch has let a renderer-local duplication pattern escape into `lib/` itself, now at 16 sites. Against that, the two client-side duplication rows this series has tracked since its first edition remain fully resolved, and `simple-dispatcher`'s state-write discipline — its one prior open question — is confirmed clean.
...
A full ESM import-graph walk of `lib/`+`routes/`+`public/`+`server.js` at HEAD finds a static cycle cluster among three `lib/` modules — **three elementary cycles over five edges** — verified by reading the source lines directly:
...
All **five** edges are static top-level `import`s (confirmed by direct read, not a heuristic grep): `kpi-stats.js:28`, `kpi-stats.js:29`, `terminal-marked-task-cost.js:50`, `weekly-budget.js:30`, `weekly-budget.js:31`. Corrected at close-out from an earlier “six” (independent review defect D2).

**Prose and ledger reconciled (independent review defect D3).** Those five edges close **three elementary cycles**, and both figures this report previously used were partial views of the same cluster: two 2-node cycles (`kpi-stats ↔ terminal-marked-task-cost`, `kpi-stats ↔ weekly-budget`) plus one 3-node cycle (`kpi-stats → weekly-budget → terminal-marked-task-cost → kpi-stats`). The ledger row and this finding now both state three.
...
**Correction from the adversarial second-read (accepted, verified independently — see Adversarial Second-Read below): the "no static cycle" claim in the initial draft of this finding was wrong.** The initial check only inspected `lib/providers/registry.js` (zero static imports, still true) but missed the **provider identity barrel**, `lib/providers/index.js` (LIN-2010, landed `0c0f5fc8`, 2026-08-20 — after every prior edition of this series). That barrel imports each provider's `index.js` at its own top level (`:31-34`), and **all four** auth routes import back into it indirectly — three via `lib/render-pages.js`, which imports `getProvider` from `lib/providers/index.js` (`render-pages.js:14`), and the linear one via two further paths corrected at close-out (below). Directly verified, three 4-node static cycles:
...
**Correction at close-out — the linear edge is *not* exonerated (independent review defect D1, verified here by direct read).** An earlier version of this finding claimed the linear edge closes no cycle, on the grounds that `routes/auth.js` imports `lib/render.js` rather than `lib/render-pages.js`, and that `render.js` reaches only `providers/registry.js`/`state-map.js`/`models.js`. That sub-claim is false on two counts: `lib/render.js:23` **also** imports `lib/components/navbar.js`, which imports the barrel at `navbar.js:11`; and `routes/auth.js:18` imports `lib/account-conflict.js`, whose `:2` import reaches `lib/render-pages.js:14`. Two further 5-node static cycles, each verified line by line:

- `lib/providers/index.js:31` → `lib/providers/linear/index.js:34` → `routes/auth.js:12` → `lib/render.js:23` → `lib/components/navbar.js:11` → `lib/providers/index.js`
- `lib/providers/index.js:31` → `lib/providers/linear/index.js:34` → `routes/auth.js:18` → `lib/account-conflict.js:2` → `lib/render-pages.js:14` → `lib/providers/index.js`

Both are latent on exactly the test this report applies elsewhere: neither `navbar.js` nor `account-conflict.js` carries a single module-scope statement (each declares only functions; every peer use — `getProvider` at `navbar.js:74-78`, `renderErrorPage`/`renderMerge*` at `account-conflict.js:59,72,90` — sits inside a function body). So **all four provider edges sit inside static cycles, not three**, and five cycles are directly verified rather than three. Severity is unchanged at **Medium** and the finding's direction is strengthened, not altered. The durable point is methodological, and it is the sibling of Finding 1's: hand-verifying named paths systematically **under**-enumerates, and `lib/components/navbar.js` / `lib/account-conflict.js` were this finding's unhandled instances. The second reader's broader tool-assisted walk (madge + independent graph traversal) reports **9 provider-seam cycles / 11-12 total** across the wider set of `lib/providers/index.js` importers (`render-landing.js`, `render-settings.js`, `components/navbar.js`, `components/landing-hero.js` also import the barrel) — this review directly verified the 3 core cycles above by hand but did not independently reproduce the full enumerated count.
...
**LIN-675 is the standing, unlanded fix for this row and is updated by this review** rather than duplicated (see Follow-ups) — it has sat in Backlog with zero comments across all four editions this row has now appeared in, and its scope (move the auth-router factories under `lib/`) does not by itself address the `render-pages.js` ↔ `providers/index.js` leg of the now-confirmed cycle; the update adds this correction. **Corrected again at close-out:** LIN-675's scope note no longer exonerates the linear leg either (D1 above), so an implementer picking it up does not read that leg as already clean.

### 3. `provider-resolution-incantation` — Low → **worsened sharply (4 → 16 sites)**

The hand-rolled `w => w.urlKey === urlKey` lookup — 4 renderer sites at both the 06-25 and 07-12 editions — has escaped the renderers. At HEAD there are **16** sites across 8 files (confirmed by direct grep and read; corrected at close-out from an earlier “17 across 11”, whose file count was wrong and whose site count both over-counted the total and under-counted the densest file — independent review defect D2):

- `lib/workspace-token-resolver.js` — **8** sites (`:44, :146, :206, :276, :316, :358, :413, :537`) — `:537` (`w?.urlKey === urlKey`) was missing from the earlier list
...
- `routes/live-console.js:57`, `routes/dashboard.js:2011` — `.some()` membership checks on a different data shape; a related but distinct concern, not counted in the 16 (18 across 10 files if they were counted)

**Root cause confirmed: a signature mismatch, not independent duplication.** The canonical `getWorkspaceByUrlKey(session, urlKey)` takes a `session`; the resolvers hold `data.workspaces`; the renderers hold a bare `workspaces` array. Neither caller shape can call the canonical helper without first unwrapping to a session object that doesn't exist at their call sites, so each site re-derives the lookup instead. **The fix is the helper's own shape (accept a workspaces array), not 16 call-site edits.**
...
`lib/errors.js` is canon. Adoption breadth **improved** across the series: importers went 5 (06-25) → 7 (07-12) → **14 at HEAD** (confirmed by direct grep: 10 `routes/` files, 3 `lib/` files — `linear-fetch.js`, `proxy-events.js`, `render-pages.js` — plus `server.js`). Corrected at close-out from an earlier “15 / 4 `lib/` files”, which counted `lib/workspace-token-resolver.js` as an importer; that file only *mentions* `lib/errors.js` in two comments (`:400`, `:508`) and imports nothing from it (independent review defect D2). The improving trend direction is unaffected. Canonical call-site density: `jsonError(` 257 combined-total call sites across `jsonError`/`badRequest` idioms, `badRequest.json`-family 222, confirming the canon is dominant, not marginal.
...
3. **LIN-2389** minted for `provider-resolution-incantation` (Finding 3) — fix the canonical `getWorkspaceByUrlKey` helper's signature (accept a workspaces array, not only a session) so the 16 call sites can converge on it; explicitly scoped as a helper-shape change, not a 16-site call-site sweep. (Title and body corrected at close-out from “17”.)
...
**What the quote above is, precisely (independent review defect D4 — recorded, not repaired).** The block quoted above is the runner's **736-character DONE summary** of the second reader's session, not the reader's analysis. The reader's actual working output ran to roughly 39.6k output tokens and survives in **neither** this report, the Linear comment, nor the dispatch feedback stream, all of which captured only the summary. §7 of this series' method asks for "the reader's answer in full", so this is a genuine gap in the evidence chain: the *conclusion* that forced Finding 2's correction is preserved and was independently re-derived below, but the reader's *detailed basis* — including how it arrived at 9 provider-seam cycles / 11–12 total — is now unrecoverable from the artifact. It is not reconstructed here, and nothing in this section should be read as a verbatim record of reasoning that was not preserved. **Process observation for the series, not a finding about the code:** a Tier 1 read's evidentiary value is capped by what the dispatch feedback stream retains, and today that is a summary field. Next run: capture the reader's full answer into the Linear comment at the time of the read, while it still exists.
...
**Disposition: fixed in place.** Finding 2 above has been rewritten with the corrected mechanism, the 3 directly-verified cycle paths, the corrected severity (medium-low → medium, following the same latent-not-live test as Finding 1), and the LIN-2010 provenance; the compact trend ledger and headline are updated to match; LIN-675 (Follow-ups) is updated to note the `render-pages.js` leg is outside its original scope. *(This paragraph and the two above it record the second-read exchange as it happened. The cycle count they cite — 3 — was superseded at close-out: the independent review found the linear edge had been wrongly exonerated, and the standing figure is now **five directly-verified cycles covering all four provider edges**. See Finding 2's close-out correction and the Independent review section below.)*
...
## Independent review and close-out corrections

This report was independently reviewed at the same two HEADs (PR #1291 head `bfa03ced`) and **approved conditional on discharging a correction ledger** before merge. The reviewer re-derived Findings 1–5, every `simple-dispatcher` row and every provenance SHA from source rather than from this prose, and recorded that every discrepancy it found ran in the same direction — **this report understates**. Four defects were raised; three were required corrections and are discharged in the text above:

| # | Defect | Disposition |
|---|---|---|
| D1 | Finding 2 wrongly exonerated the linear provider edge; the error was reproduced in LIN-675's scope note | **Fixed.** All four provider edges are shown inside static cycles; the two additional linear cycles (via `lib/components/navbar.js:11` and `lib/account-conflict.js:2`) are cited line by line, and LIN-675's scope note is corrected in place. Severity unchanged (Medium) — the finding is strengthened, not reversed. |
| D2 | Three count inaccuracies: Finding 1's "six edges" (5), Finding 3's "17 sites across 11 files" (16 across 8, with `workspace-token-resolver.js:537` missing), Finding 5's "15 importers / 4 `lib/`" (14 / 3) | **Fixed** in the findings, the trend ledger and the follow-up tickets (LIN-2388 body, LIN-2389 title and body). No conclusion inverts; no trend direction changes. |
| D3 | The `lib-import-cycles` ledger row ("2 static cycles") contradicted Finding 1's prose ("a genuine 3-node static cycle") | **Fixed.** Both now state the same thing: three elementary cycles over five edges among three modules — two 2-node and one 3-node. |
| D4 | The second reader's full answer was never preserved; only the runner's 736-character summary survives | **Recorded, not repaired** — see the note in Adversarial Second-Read above. The missing analysis is not reconstructed and is not represented as recovered. |

The reviewer's own independent import-graph walk found **8** provider-seam cycles, above this report's directly-verified count and broadly corroborating the second reader's 9 — so the enumeration in Finding 2 should still be read as a floor, not a total. That under-enumeration is itself the sibling-class point D1 makes.
...
|---|---|---|---|---|---|
| `lib-import-cycles` | medium | clean | clean | **3 elementary cycles / 3 modules / 5 edges (latent)** | **new regression** |
| `provider-auth-router-upward-imports` | medium (revised up post-second-read) | 2 edges, no cycle | 3 edges, no cycle | **4 edges, all four inside static cycles; 5 verified via `providers/index.js` ↔ `render-pages.js`/`components/navbar.js` (latent)** | **worsened (4th run); severity corrected by adversarial second-read, enumeration corrected at close-out** |
| `provider-resolution-incantation` | low | 4 sites | 4 sites | **16 sites** | **worsened sharply** |
| `production-to-test-fixture-imports` | medium-low | — | — | **7 sites** | **new** |
| `routes-error-envelope-fragmentation` | medium | 5 importers, ~53 residue | 7 importers, ~66 residue | 14 importers, 68 residue; `dashboard.js` now a half-adopter | unchanged severity, shape worsened |
