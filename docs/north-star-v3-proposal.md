...
## Proposed v3, in full
    ...
    # North star — v3, the self-funding loop, climbed one rung at a time
    ...
    **Meet a developer at rung three; make the next rung safe before it is visible.** People climb from copied answers to hand-driven sessions to saved prompts, then to one delegated task, a ratified passage, a standing loop — each step gated by trust and budget. Harbour's first job is the grounded next prompt: log in, connect a tracker and a repo, click a task, copy, go. Every feature names its rung; a person sees their rung plus one; rung four is one task, one bounded run, a human click before merge. Login → connected → first evidence-verified merge, without talking to the operator, is the rung-four bar. Secure defaults assume hostile input; payments land only behind the hardening gate; the free tier is a published, ring-fenced line item. The measure is people per rung and where they stop.
...
+++ /tmp/claude-0/-home-user/b26aec7c-ba1b-5cc0-abc0-a8f1a6bad478/scratchpad/v3.md	2026-09-19 07:51:24.255152077 +0000
@@ -1,19 +1,19 @@
...
| Rung clause: the ladder, and rung-3 entry as Harbour's first job | *"First people will ask Claude a question, copy the answer, carry on with the day. Next people open Claude Code and essentially walk through their work, meandering left and right … this is where they learn to trust Claude. Then people start bundling up tasks … they save their prompts … this is where the majority of developers today are."* And: *"Harbour's first parlour trick is essentially giving them those prompts. They log in, they get their prompt, off they go. That is the core of Harbour."* And: *"a user just logs in with an email address, adds a git repo with a couple of clicks, and then sees all of their tasks, potentially in Jira, and then can just click a task, get the next prompt, copy-paste it into Claude and off they go."* |
| Rung clause: "each step gated by trust and budget"; "the measure is people per rung and where they stop" | *"we have to factor in how people will actually use it and how far along they'll go, and that's based on trust and budget and other factors like that."* |
| Rung clause: "make the next rung safe before it is visible"; "every feature names its rung; a person sees their rung plus one; rung four is one task, one bounded run, a human click before merge" | **No independent sentence of John's.** These are the agent's proposals from the ladder table, which John agreed to (*"Fantastic, we're on the same page"*; *"I love it, yes"*). Flagged for the second read. Keeping, striking or rewording them is John's call. |
| Rung clause: the rung-four bar, secure defaults, hardening gate, free tier | Carried over from v2's stranger clause, unchanged in substance. The v2 head sentence "A stranger can trust it in one sitting" is replaced, not dropped: its content is the rung-four bar. |
| Header note | Depends on LIN-2926. John's words there: *"it's still my intent … even though the risk is that it brings in some bias that the model brings in a way that I don't necessarily notice."* |
...
1. Rule on LIN-2926. If adopted, the header note stands; if not, delete that line from the diff.
...
5. Then plan the rung-3 passage: the planner's Step-1 read will quote v3, and the rung-3 chain under LIN-614 is now inside its stack read.
