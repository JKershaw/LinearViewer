...
**Date:** 2026-09-22
**Duration:** onset ~19:15Z, recovering by ~19:47–19:52Z (LIN-2993 comments `825a549b`,
`9e91955a`), structurally resolved by 20:16:41Z after MongoDB-harbour moved to the app's
own EU region (LIN-3000 comment `4a531223`, `createdAt`; the comment's own text
self-labels this "~20:35Z" — see the clock-provenance note below)
...
> text that runs 2–19 minutes ahead of that comment's actual Linear `createdAt` (e.g.
> `9459e3d2` says "~19:40Z", posted 19:37:07Z; `1df2ffb0` says "~19:55Z", posted
> 19:42:53Z; `cfc1d6bd` says "~20:10Z", posted 20:07:58Z; `4a531223` says "~20:35Z",
> posted 20:16:41Z). Rows below cite the underlying clock, not the self-label.
...
|---|---|---|
| ~19:00–19:23 | system (`railway logs`) | Old container `85891c1c` serving; the two server-side merges (#1546 LIN-2975, #1543 LIN-2981) later ruled out as cause — neither adds a query (LIN-2993 comment `7905184c`) |
| **~19:15–19:20** | con self-label (LIN-2993 description) | **First failures** seen by the con's own watch |
| 19:23:00–19:25:45 | system (`railway logs`) | `#1546` container `2e03587f` serving. Railway's graph showed its deploy *created* at 20:14 GMT+1 — read at the time as go-live, ~8 min too early (`7905184c`) |
| ~19:23 | system (`railway metrics --raw`, per-minute) | Memory graph peaks at 1.28 GB — two containers (`85891c1c`→`2e03587f` handover) overlapping, not a leak (LIN-2993 comment `1fbe8f73`) |
| 19:25:38 | system (`railway logs`) | `#1547` (`a5f4383`) container `68d764ab` goes live; no crash or restart loop (`7905184c`) |
| ~19:30 | con self-label (LIN-2993 description) | LIN-2974 autopilot tree **cascade-aborted** on John's instruction as a precaution; the con's own polling stopped at the same time |
...
| 19:40:51 | system (`railway logs --http`) | Two observation `sessions` requests, held 546–573 s, released together, at the same moment the app logged a Linear `AbortError timedOut` (LIN-2993 comment `825a549b`) |
| 19:41–19:47 | system (`railway logs`) | Recovering — proxy calls down to 0.3–1.8 s (`825a549b`) |
...
| 19:51:24 | Linear comment (`createdAt`) | **Discriminating test:** `mongosh` shows Mongo healthy (no long ops, no queued locks); the feed's history read executes in 29 ms on the server but returns a 32 MB payload (LIN-2993 comment `8b2bcfa4`) |
| 19:51:54 | Linear comment (`createdAt`) | Authoritative root cause posted (LIN-2993 comment `9e91955a`), superseding the earlier, wrong-lead findings (`ba819bcd`) |
| 19:52–~20:04 | probe | A 12-minute probe's worst read is 2 s (LIN-2993 comment `cfc1d6bd`) |
| 20:07:58 | Linear comment (`createdAt`; self-labelled "~20:10Z") | Incident **effectively resolved**; follow-ups filed (`cfc1d6bd`) |
