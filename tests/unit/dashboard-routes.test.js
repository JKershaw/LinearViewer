...
// ─── Rulings feed: the basis-comparison join (LIN-2241 tier 1) ────────────────
//
// Covers `collectTaskBasis` — the only part of tier 1 that is neither a pure
// function nor a render branch: the point-read join from a pending task
// decision onto the freshest task snapshot Harbour already holds. Its failure
// modes are all "flag the wrong thing", which is the one outcome that destroys
// trust in the panel (docs/escalation-philosophy.md §4), so they are pinned
// here rather than left to the e2e.

describe('GET /api/dashboard/rulings — basisChanged join (LIN-2241 tier 1)', () => {
  const SCAN_ISSUE_ID = '11111111-2222-3333-4444-555555555555';
  const RAISED_AT = new Date('2026-09-01T09:00:00.000Z');
  const OBSERVED_AFTER = new Date('2026-09-03T09:00:00.000Z');
  const OBSERVED_BEFORE = new Date('2026-08-25T09:00:00.000Z');

  // A snapshot whose basis digest is whatever the projection makes of it —
  // the tests below vary `description`, which the basis DOES carry.
  function snapshotDoc({ description, capturedAt, canonicalId = SCAN_ISSUE_ID }) {
    return {
      id: 'snap-1',
      taskIdentifier: 'LIN-2241',
      canonicalId,
  ...
  function taskDecisionRow(overrides = {}) {
    return {
      id: 'scan_11111111_abcdef123456',
      urlKey: 'ws-a',
      issueId: SCAN_ISSUE_ID,
      issueIdentifier: 'LIN-2241',
      inputHash: 'abcdef123456',
      basisHash: null,
      decision: { decision_id: 'scan-d-1', question: 'Proceed?' },
      scannedAt: RAISED_AT.toISOString(),
      outcome: null,
      outcomeAt: null,
      ...overrides
    };
  ...
  async function rulingsWith({ taskDecisions, snapshots, taskSnapshotStore }) {
    const { dispatchQueueStore, agentStatusStore } = makeStores({ 'ws-a': { live: [], history: [], agentStatus: [] } });
    const reads = [];
    const router = createDashboardRoutes({
      workspaceFromUrl: (req, res, next) => next(),
      dispatchQueueStore,
      ...
      taskSnapshotStore: taskSnapshotStore || {
        async latest(urlKey, taskIdentifier) {
          reads.push(`${urlKey}::${taskIdentifier}`);
          return snapshots?.[taskIdentifier] ?? null;
        }
      }
    });
    const handler = getHandler(router, 'get', '/workspace/:urlKey/api/dashboard/rulings');
    const { req, res } = makeReqRes({ session: { workspaces: [{ urlKey: 'ws-a', name: 'Alpha' }] } });
    await handler(req, res);
    return { body: res.jsonBody, status: res.statusCode, reads };
  }

  test('a snapshot newer than the scan, with different content, flags the row', async () => {
    // The raise-time basis is recorded as a digest of the ORIGINAL description;
    // the snapshot carries an edited one, so the basis has demonstrably moved.
    const { scanBasisHashFromSnapshot } = await import('../../lib/scan-fingerprint.js');
    const raisedBasis = scanBasisHashFromSnapshot(snapshotDoc({ description: 'original', capturedAt: RAISED_AT }).snapshot);
    const { body, status } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: raisedBasis })],
      snapshots: { 'LIN-2241': snapshotDoc({ description: 'John answered it in the ticket', capturedAt: OBSERVED_AFTER }) }
    });
    assert.equal(status, 200);
    assert.equal(body.rulings.length, 1);
    assert.equal(body.rulings[0].basisChanged, true);
  });

  test('a snapshot newer than the scan with the SAME content reports false', async () => {
    const { scanBasisHashFromSnapshot } = await import('../../lib/scan-fingerprint.js');
    const snap = snapshotDoc({ description: 'unchanged', capturedAt: OBSERVED_AFTER });
    const { body } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: scanBasisHashFromSnapshot(snap.snapshot) })],
      snapshots: { 'LIN-2241': snap }
    });
    assert.equal(body.rulings[0].basisChanged, false);
  });

  test('a snapshot OLDER than the scan never flags — it describes content the scan already saw', async () => {
    const { body } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: 'raised-basis' })],
      snapshots: { 'LIN-2241': snapshotDoc({ description: 'stale observation', capturedAt: OBSERVED_BEFORE }) }
    });
    assert.equal(body.rulings[0].basisChanged, null);
  });

  test('a row with no recorded basis is never read for at all — and reports unknown', async () => {
    // Every ruling raised before this feature landed. Skipping the point-read
    // keeps the ambient poll from paying for a comparison it cannot make.
    const { body, reads } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: null })],
      snapshots: { 'LIN-2241': snapshotDoc({ description: 'anything', capturedAt: OBSERVED_AFTER }) }
    });
    assert.equal(body.rulings[0].basisChanged, null);
    assert.deepEqual(reads, [], 'a basis-less row must cost no snapshot read');
  });

  test('a snapshot whose canonicalId disagrees is declined, not compared across tasks', async () => {
    // The snapshot store is keyed by HUMAN identifier; a reused/re-pointed
    // identifier would otherwise compare two different tasks' content.
    const { body } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: 'raised-basis' })],
      snapshots: {
        'LIN-2241': snapshotDoc({
          description: 'a different task entirely',
          capturedAt: OBSERVED_AFTER,
  ...
  test('no snapshot for the task reports unknown', async () => {
    const { body } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: 'raised-basis' })],
      snapshots: {}
    });
  ...
  test('a throwing snapshot store degrades to unknown — the feed still renders', async () => {
    // An advisory flag must never take the rulings feed down with it.
    const { body, status } = await rulingsWith({
      taskDecisions: [taskDecisionRow({ basisHash: 'raised-basis' })],
      taskSnapshotStore: { async latest() { throw new Error('store down'); } }
    });
    assert.equal(status, 200);
    assert.equal(body.rulings.length, 1);
    assert.equal(body.rulings[0].basisChanged, null);
  ...
  test('an unwired snapshot store leaves every row unknown and changes nothing else', async () => {
    const { dispatchQueueStore, agentStatusStore } = makeStores({ 'ws-a': { live: [], history: [], agentStatus: [] } });
    const router = createDashboardRoutes({
      workspaceFromUrl: (req, res, next) => next(),
      dispatchQueueStore,
      ...
      getOpenRouterSource: () => 'env',
      getDeployInfo: () => ({}),
      taskDecisionsStore: { async listUnansweredForWorkspaces() { return [taskDecisionRow({ basisHash: 'raised-basis' })]; } }
      // taskSnapshotStore deliberately omitted
    });
    const handler = getHandler(router, 'get', '/workspace/:urlKey/api/dashboard/rulings');
    const { req, res } = makeReqRes({ session: { workspaces: [{ urlKey: 'ws-a', name: 'Alpha' }] } });
    await handler(req, res);
    ...
    assert.equal(res.jsonBody.rulings[0].basisChanged, null);
  });

  test('several rulings on ONE task cost a single snapshot read', async () => {
    const { reads } = await rulingsWith({
      taskDecisions: [
        taskDecisionRow({ basisHash: 'raised-basis', id: 'row-1', scannedAt: RAISED_AT.toISOString() }),
        taskDecisionRow({ basisHash: 'raised-basis', id: 'row-2', scannedAt: RAISED_AT.toISOString() })
      ],
      snapshots: { 'LIN-2241': snapshotDoc({ description: 'x', capturedAt: OBSERVED_AFTER }) }
    });
    assert.deepEqual(reads, ['ws-a::LIN-2241']);
  });
});
