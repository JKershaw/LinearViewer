/**
 * Census guard for docs/architecture/source-map.md's scheduler.js map entry
 * (LIN-2572, P0.4 of the LIN-2569 operating-model path).
 *
 ...
 *   - the doc side reads docs/architecture/source-map.md and derives, like
 *     the current-state-docs count guard in tests/unit/prompt-templates.test.js.
 *
...
const claudeSrc = readFileSync(fileURLToPath(new URL('../../docs/architecture/source-map.md', import.meta.url)), 'utf8');
...
describe('docs/architecture/source-map.md scheduler entry matches the scheduler.register sites in server.js (LIN-2572)', () => {
  ...
  test('the scheduler.js map entry carries a "N jobs registered today" roster sentence', () => {
    assert.ok(rosterMatch,
      'docs/architecture/source-map.md\'s scheduler.js entry must still carry its "<N> jobs registered today: ..." roster sentence');
  });
  ...
  test('every job registered in server.js is named in the documented roster', () => {
    ...
    assert.deepEqual(missing, [],
      `docs/architecture/source-map.md's scheduler entry omits registered job(s): ${missing.join(', ')} — server.js registers ${registeredNames.length}: ${registeredNames.join(', ')}`);
  });
  ...
  test('the documented roster names no job that server.js does not register', () => {
    ...
    assert.deepEqual(ghosts, [],
      `docs/architecture/source-map.md's scheduler entry names unregistered job(s): ${ghosts.join(', ')} — server.js registers: ${registeredNames.join(', ')}`);
  });
  ...
  test('the count word matches the actual number of registered jobs', () => {
    ...
    assert.strictEqual(claimed, registeredNames.length,
      `docs/architecture/source-map.md says ${rosterMatch[1]} jobs are registered; server.js registers ${registeredNames.length}: ${registeredNames.join(', ')}`);
  });
});
