...
describe('periodicals registry', () => {
  test('seeds the LIN-354 review set plus Drift & Coherence, Comprehension-Debt, Stability, Dependency & Supply-Chain, Recent Headwinds, Design & Interface, and Performance / Scale (12 templates)', () => {
  ...
  test('contains the ten corrective reviews plus the advisory Stability Review and Recent Headwinds report', () => {
    ...
    // performance reviews are 'corrective' (they mint fix-tasks); the Stability
    // Review (LIN-453) and the Recent Headwinds report (LIN-542) are the two
    // 'advisory' entries — trajectory governors that report for a human to act
    // on. The Design & Interface Review (LIN-520) is corrective with an advisory
    // tail: it mints fix-tasks for objective breakage only. The Performance /
    // Scale Review (LIN-1038) is corrective at the measured-symptom altitude.
    const expected = {
