 ...
 *
 *   ADVISORY ONLY, NOT AUTHORITATIVE (LIN-679 PR-0, plan-review F4): this "2+ regions
 *   => routes/proxy-context.js" rule is the FLAT-registration shape the LIN-2360 spike
 *   proposed and LIN-679's accepted plan REJECTED — the accepted shape is router.use()
 *   sub-routers with shared deps injected from the composer's existing param bag, and
 *   explicitly creates no routes/proxy-context.js. The REGIONS table below also predates
 *   LIN-2245 (its B/C destinations still say 'proxy-instructions.js', and its per-region
 *   `stage` numbers do not match the accepted plan's stage order — e.g. G is listed here
 *   at the same stage as F/H, not first). Nothing downstream reads Table A's `destination`
 *   or `stage` columns; do not treat them as the sequencing plan. Consult the Linear
 *   ticket's own per-stage triage (and Table B below, which IS load-bearing) instead.
 *
 ...
 *
 *   LOAD-BEARING PART vs advisory part: the pin INVENTORY itself — which test file/line
 *   binds against routes/proxy.js, by what pattern, with what match count, and which
 *   routes/proxy.js line(s) it currently matches — is shape-independent of REGIONS and IS
 *   the guard this ticket relies on. The `breaks`/`goesVacuousAt` STAGE NUMBER and the
 *   `destinations` column, however, are derived from the same stale/advisory REGIONS
 *   table as Table A above, so they inherit the same caveat: use them as a rough locator,
 *   not as the authoritative stage — cross-check against the ticket's own per-stage pin
 *   triage before re-pointing anything.
 *
...
// ADVISORY ONLY (see the file header) — the destination/stage columns here encode the
// rejected flat-registration shape and predate LIN-2245; only the route-index ranges
// (which region a given registration falls in) are still accurate against HEAD.
const REGIONS = [
...
// LIN-2535 (Stage 2 / PR-2c): group C (1 route) moved to
// routes/proxy-token-exchange.js — 48 - 1 = 47.
// LIN-2536 (Stage 3a / PR-3a): group D (13 routes) moved to
// routes/proxy-reads.js — 47 - 13 = 34.
// LIN-2537 (Stage 3b / PR-3b): group E (12 routes) moved to
// routes/proxy-writes.js — 34 - 12 = 22.
// LIN-2538 (Stage 4 / PR-4): group F (12 routes) moved to
// routes/proxy-compute.js — 22 - 12 = 10.
const EXPECTED_ROUTES_IN_FILE = 10;
...
if (want === 'a' || want === 'both') {
  ...
  console.log('ADVISORY ONLY: destination/stage below encode the REJECTED flat-registration');
  console.log('shape (routes/proxy-context.js) and predate LIN-2245. Not authoritative — see');
  console.log('the file header and the ticket\'s own per-stage pin triage.');
  console.log('='.repeat(100));
...
if (want === 'b' || want === 'both') {
  ...
  console.log('The pin inventory (file/line/pattern/match-count) is load-bearing. The `breaks` /');
  console.log('`GOES VACUOUS AT STAGE` numbers and `destinations` are ADVISORY (same stale REGIONS');
  console.log('table as Table A) — cross-check against the ticket\'s own per-stage pin triage.');
  console.log('='.repeat(100));
