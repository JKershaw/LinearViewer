...
//
// `isSuspended` (optional, 2026-08-09 incident follow-up 1) is a predicate over a
// row's provider call scope: return true and the row is skipped as if expired.
// It exists because recorded expiry is this selector's ONLY liveness test, so a
// credential revoked UPSTREAM stays selectable forever. Omitted (the default) the
// selection is byte-identical to before — every existing caller and test is
// unaffected. A workspace whose only rows are suspended therefore reports
// `session_expired`, not `not_connected`: a session row genuinely does exist, and
// that reason is what routes the caller into refresh-on-resolve rather than a
// dead end.
export function selectOwnerWorkspaceToken(sessions, urlKey, ownerAccountId = UNSCOPED, isSuspended = null) {
  ...
  for (const row of sessions) {
    ...
    if (!ws) continue;
    sawMatch = true;
    // A credential the provider has just refused repeatedly is treated as NOT
    // live, even though its recorded expiry is in the future (2026-08-09
    // incident, follow-up 1). Recorded expiry is the ONLY liveness test this
    // selector has, so a credential revoked upstream reads as perfectly healthy
    // here and is re-selected forever. Skipping it makes the caller's
    // `selected.token` falsy, which is exactly the condition refresh-on-resolve
    // is already gated on — so the dead credential gets replaced instead of
    // re-served. See lib/rejected-credentials.js for why suspension requires
    // CONSECUTIVE refusals rather than one.
    if (ws.accessToken && isSuspended?.(getWorkspaceCallScope(ws))) continue;
