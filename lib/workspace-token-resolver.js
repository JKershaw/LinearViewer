...
// selectOwnerSessionRows(sessions, urlKey, ownerAccountId): a pure sibling of
// selectOwnerSessionRow, added for LIN-2235 (L4.2, mirror retirement on
// rotation). Same no-filter semantics (every matching row, live or expired),
// but returns ALL of the owner's rows for `urlKey` instead of picking one —
// lib/workspace-token-refresh.js's mirror step loops this to write a
// successful rotation into every live session row for the owner, not just the
// single latest-expiring one `selectOwnerSessionRow` picks (which left every
// OTHER row's mirrored copy stale until its own next refresh, the shape
// behind the transient-401 burst on an otherwise-healthy credential, comment
// `18f2f69d`). Sorted latest-expiring first, so a caller that only wants
// `selectOwnerSessionRow`'s old single-row pick can take element 0 — same
// tie-break convention (ties keep first-encountered order; `Array#sort` is
// stable), so that first element is byte-identical to what
// `selectOwnerSessionRow` alone would return. Returns `[]` (never null) when
// the owner has no session row for `urlKey` at all.
export function selectOwnerSessionRows(sessions, urlKey, ownerAccountId) {
  ...
  for (const row of sessions) {
    ...
    const ws = data.workspaces[workspaceIndex];
    rows.push({ sid: row._id, session: data, workspaceIndex, expiresAt: ws.tokenExpiresAt || 0 });
  }
