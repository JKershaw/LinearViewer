...
// but which still carries a refreshToken to refresh from. Among the owner's
// matching rows, picks the latest-expiring one (mirroring the live selector's
// own tie-break) so a long-abandoned session doesn't win over a more
// recently-active one. Returns null when there is nothing refreshable: no
// owner, no session for this workspace, only still-live tokens, or an expired
// token with no refreshToken.
//
...
export function selectExpiredOwnerRow(sessions, urlKey, ownerAccountId) {
  ...
  for (const row of sessions) {
    ...
    if (isLive || !ws.refreshToken) continue;

    const expiry = ws.tokenExpiresAt || 0;
    if (expiry > bestExpiry) {
      bestExpiry = expiry;
      best = { sid: row._id, refreshToken: ws.refreshToken, session: data, workspaceIndex };
    }
  }
...
// of selectOwnerWorkspaceToken, added for LIN-1413. Answers a narrower question
// than the selector: is the owner's failure to resolve explained by their OWN
// token merely lapsing (LIN-1373's case, re-auth/refresh can fix it), or does a
// DIFFERENT account hold a live token for this workspace (re-auth cannot fix
// it — the owner account no longer holds the workspace)? Returns a verdict
// only — never a token, never the other account's id — so this changes how a
// resolution failure is *described*, never who gets a credential (LIN-1366
