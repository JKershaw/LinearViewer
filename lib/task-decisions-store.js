...
export class TaskDecisionsStore {
   ...
   *
   * Invariant: the current live row is never pruned. `_docsFor` sorts
   * newest-first and this keeps only `docs.slice(0, maxPerTask)`, so the
   * single most-recently-scanned row — which is always what `getStatus`
   * treats as current, outcome-stamped or not — survives any prune as long
   * as `maxPerTask >= 1` (`MAX_SCANS_PER_TASK` above is 50; a caller cannot
   * construct a `0` cap through the public constructor, since
   * `options.maxPerTask || MAX_SCANS_PER_TASK` treats `0` as unset). Do not
   * change this to an oldest-first slice, a different sort key, or a cap
   * that can reach `0` without re-checking this invariant.
   */
