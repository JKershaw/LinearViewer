 ...
 *                                  // its OWN `dueBasisVersion` (LIN-2665 L1 — see below) and
 *                                  // `lib/scan-fingerprint.js`'s `dueChanged`.
 *                                  // `null` on any row written before LIN-2649 WS2, or on a
 *                                  // terminal row until the [F-2] patch below back-fills it.
 *   dueBasisVersion: number|null,  // LIN-2665 L1: `BASIS_VERSION` at the time `dueBasisHash` was
 *                                  // computed — a SEPARATE field from `basisVersion`, deliberately.
 *                                  // The two hashes must never share one version field: the [F-2]
 *                                  // terminal-row patch below freezes `basisHash` while refreshing
 *                                  // `dueBasisHash`, and a shared version field meant that refresh
 *                                  // stamped a fresh version beside the frozen (and now
 *                                  // stale-version) `basisHash`, making tier-1's `basisChanged`
 *                                  // misread a v1 digest as current-version-comparable. `null` on
 *                                  // any row written before this split.
 *   decision:        Object|null,  // parseDecision's shape, or null (zero-finding)
 *   scannedAt:       Date,
 *   outcome:         string|null,  // 'answered' | 'dismissed' | 'self-resolved' | null
 *   outcomeAt:       Date|null,
 *   outcomeReason:   string|null,  // LIN-2650: required non-empty for 'self-resolved'; null for
 *                                  // 'answered'/'dismissed' and for a reversed (un-retired) row
 *   outcomeBasisHash: string|null, // LIN-2650: hashContext digest of the content the self-resolve
 *                                  // check ran against — "what was checked when this cleared",
 *                                  // narrower than `basisHash`/`dueBasisHash`. null unless self-resolved
 *   optionId:        string|null  // LIN-2754 Track B: the standing suggestion's already-validated
 *                                  // option id, written only on an 'answered' stamp when supplied.
 *                                  // null for 'dismissed'/'self-resolved' and for any pre-LIN-2754 row
 * }
 *
 * Outcome model (LIN-2650): terminal + one reversible machine-written outcome.
 * `'answered'`/`'dismissed'` are human-stamped and stay write-once forever —
 * `markOutcome`'s first-stamp-wins never re-fires on them, and there is no
 * path back off either value. `'self-resolved'` is machine-stamped (the
 * retire route) and is the ONLY outcome `reverseOutcome` (below) will ever
 * clear — reversing it restores the row to unanswered, exactly as if it had
 * never been stamped. This asymmetry is deliberate, not an oversight: a
 * human's terminal judgement must never silently un-happen, but a
 * mechanism's own inference about "did this resolve itself" must be
 * correctable when it turns out to be wrong.
 *
...
function toRecord(doc) {
  ...
  return {
    ...
    dueBasisVersion: doc.dueBasisVersion ?? null,
    ...
    outcomeBasisHash: doc.outcomeBasisHash ?? null,
    optionId: doc.optionId ?? null
  };
}
...
export class TaskDecisionsStore {
   ...
   * @param {number|null} [entry.basisVersion] - BASIS_VERSION that produced `basisHash`
   ...
   * @param {number|null} [entry.dueBasisVersion] - BASIS_VERSION that produced `dueBasisHash` — its
   *   OWN field (LIN-2665 L1), never `basisVersion`
   ...
   */
  async recordScan({ urlKey, issueId, issueIdentifier, inputHash, basisHash = null, basisVersion = null, dueBasisHash = null, dueBasisVersion = null, decision = null } = {}) {
    ...
    try {
      ...
      if (existing && existing.outcome) {
        ...
        // ONLY dueBasisHash/dueBasisVersion — outcome/outcomeAt/decision/basisHash
        // AND tier-1's own basisVersion are NEVER in this $set, so the terminal-row
        // invariant (never silently un-dismissing/un-answering a prior ruling, and
        // never touching tier-1's frozen basisHash/basisVersion pairing) holds by
        // field list, by construction, not by convention. dueBasisVersion is
        // patched alongside dueBasisHash — its OWN field, distinct from tier-1's
        // basisVersion (LIN-2665 L1) — because the two are compared together in
        // dueChanged; stamping tier-1's basisVersion here instead (the pre-fix
        // behaviour) corrupted `basisChanged`'s comparability by making a frozen
        // stale-version basisHash look current-version-comparable.
        ...
        if (freshDueBasisHash && freshDueBasisHash !== storedDueBasisHash) {
          const freshDueBasisVersion = Number.isFinite(dueBasisVersion) ? dueBasisVersion : null;
          await this.collection.updateOne({ _id, urlKey }, { $set: { dueBasisHash: freshDueBasisHash, dueBasisVersion: freshDueBasisVersion } });
          return toRecord({ ...existing, dueBasisHash: freshDueBasisHash, dueBasisVersion: freshDueBasisVersion });
        }
      ...
      const doc = {
        ...
        dueBasisVersion: Number.isFinite(dueBasisVersion) ? dueBasisVersion : null,
  ...
  /**
   * Stamp a specific scan row with a terminal outcome ('answered',
   * 'dismissed', or the reversible 'self-resolved' — LIN-2650). Idempotent:
   * a row that already carries an outcome is returned unchanged rather than
   * re-stamped — first stamp wins, mirroring `recordScan`'s own
   * terminal-row-never-overwritten discipline, so a double-submitted
   * dismiss/answer/retire can never flip an already-recorded one. This is
   * deliberate for 'self-resolved' too: re-marking is not how retirement is
   * corrected — `reverseOutcome` (below) is the only path off a
   * self-resolved row.
   *
   * @param {Object} entry
   * @param {string} entry.urlKey
   ...
   * @param {string} entry.outcome - 'answered' | 'dismissed' | 'self-resolved' — closed set, never an open string
   * @param {string} [entry.outcomeReason] - required, non-empty (after `.trim()`) when `outcome === 'self-resolved'`;
   *   ignored for 'answered'/'dismissed'
   * @param {string} [entry.outcomeBasisHash] - required, non-empty when `outcome === 'self-resolved'`;
   *   ignored for 'answered'/'dismissed'
   * @param {string} [entry.optionId] - LIN-2754 Track B: an answered-conditional durable side
   *   field — the standing suggestion's already-validated option id, written only when
   *   `outcome === 'answered'` and a value is supplied (never re-validated against
   *   `decision.options[].id` here; that happened once at propose time). Absent/ignored
   *   for 'dismissed'/'self-resolved'. Projected `?? null` by `toRecord`.
   * @returns {Promise<Object|null>} the (possibly already-terminal) record, with an additional
   *   `firstStampWins` discriminant (this call's own conditional write, not the earlier read,
   *   decided the outcome — LIN-2889/LIN-2724), or null if not found/bad input.
   *   `firstStampWins` is returned-only: never persisted, never part of `toRecord`'s projection.
   */
  async markOutcome({ urlKey, issueId, id, outcome, outcomeReason, outcomeBasisHash, optionId } = {}) {
    if (!this.collection || !urlKey || !issueId || !id) return null;
    if (outcome !== 'answered' && outcome !== 'dismissed' && outcome !== 'self-resolved') return null;
    if (!UUID_REGEX.test(issueId)) return null; // canonical-UUID guard — see class docstring

    let reason = null;
    if (outcome === 'self-resolved') {
      reason = typeof outcomeReason === 'string' ? outcomeReason.trim() : '';
      if (!reason) return null; // required, non-empty — same discipline as lib/dismissal-suggestions-store.js's `reason`
      if (typeof outcomeBasisHash !== 'string' || !outcomeBasisHash) return null;
    }

    try {
      const existing = await this.collection.findOne({ _id: id, urlKey, issueId });
      if (!existing) return null;
      // LIN-2889 plan-review C1: keep the terminal-row fast path AHEAD of the
      // zero-finding guard below. Dropping it would turn a zero-finding
      // terminal row's retire-route check from 409 ALREADY_TERMINAL into
      // null -> 404 ROW_GONE, silently widening LIN-2650 F7's
      // null-means-two-things ambiguity, which stays out of scope here.
      if (existing.outcome) return { ...toRecord(existing), firstStampWins: false }; // first stamp wins
      // LIN-2650 plan-review C3: a zero-finding row (no `decision`) has
      // nothing to retire and must stay evictable by `_pruneToCapacity`'s
      // zero-finding bucket — self-resolving one would move it into the
      // unconditionally-exempt bucket §3 adds below and pin it there forever.
      if (outcome === 'self-resolved' && !existing.decision) return null;

      const outcomeAt = new Date();
      const set = { outcome, outcomeAt };
      if (outcome === 'self-resolved') {
        set.outcomeReason = reason;
        set.outcomeBasisHash = outcomeBasisHash;
      }
      if (outcome === 'answered' && optionId != null) {
        set.optionId = optionId;
      }
      // LIN-2889/LIN-2724: the write itself must be conditional on the row
      // still being unstamped — an unconditional write here is what let an
      // interleaved answer-vs-retire race both believe they won. Only the
      // caller whose `updateOne` actually matches gets `firstStampWins: true`.
      const result = await this.collection.updateOne(
        { _id: id, urlKey, issueId, outcome: null },
        { $set: set }
      );
      if (result.matchedCount === 1) {
        return { ...toRecord({ ...existing, ...set }), firstStampWins: true };
      }
      // Lost the race: someone/something else wrote first between our
      // findOne and this updateOne. Re-read and report the true current state.
      const current = await this.collection.findOne({ _id: id, urlKey, issueId });
      if (!current) return null;
      return { ...toRecord(current), firstStampWins: false };
    } catch (err) {
  ...
  /**
   * Reverse a 'self-resolved' outcome (LIN-2650) — the ONLY outcome value
   * this store ever un-stamps. Restores the row to unanswered by clearing
   * `outcome`/`outcomeReason`/`outcomeBasisHash`/`outcomeAt` back to `null`
   * in one update, so the row re-enters `listUnansweredForWorkspaces` and is
   * once again writable by an ordinary `recordScan` (the terminal-row guard
   * stops firing on it).
   *
   * "Fails loud" is a thrown `Error`, not the class's usual returned `null`
   * — a deliberate divergence from every other method here. The `null`-on-miss
   * convention is safe everywhere else because callers already branch on
   * truthiness for an ORDINARY, expected miss (e.g. "no scan yet"). A
   * pruned-or-replaced row on an un-retire click is not an ordinary miss —
   * it is exactly the LIN-2211 orphaned-row class the ticket requires be
   * impossible to silently swallow. Do not "fix" this back to `null` by
   * pattern-matching the surrounding methods.
   *
   * @param {Object} entry
   * @param {string} entry.urlKey
   * @param {string} entry.issueId - canonical provider issue id (UUID); must match the row's own
   * @param {string} entry.id - the scan row's `_id`
   * @returns {Promise<Object>} the reversed record
   * @throws {Error} 'reverseOutcome: row not found (pruned or replaced)' — the row never existed,
   *   was evicted by `_pruneToCapacity`, or a newer scan superseded its `_id`
   * @throws {Error} 'reverseOutcome: row is not self-resolved' — never self-resolved, or already reversed
   */
  async reverseOutcome({ urlKey, issueId, id } = {}) {
    try {
      // Same "not found" message for an unconfigured store or a malformed
      // issueId as for a genuinely missing row — indistinguishable from the
      // caller's perspective, and this method has no `null`-on-bad-input
      // escape hatch to fall back to (see the fail-loud rationale above).
      if (!this.collection || !urlKey || !issueId || !id || !UUID_REGEX.test(issueId)) {
        throw new Error('reverseOutcome: row not found (pruned or replaced)');
      }
      const existing = await this.collection.findOne({ _id: id, urlKey, issueId });
      if (!existing) throw new Error('reverseOutcome: row not found (pruned or replaced)');
      if (existing.outcome !== 'self-resolved') throw new Error('reverseOutcome: row is not self-resolved');

      const cleared = { outcome: null, outcomeReason: null, outcomeBasisHash: null, outcomeAt: null };
      // LIN-2889: same conditional-write discipline as markOutcome — the
      // filter re-checks `outcome: 'self-resolved'` at write time, not just
      // at the earlier read, so a second reverse racing in loses cleanly.
      const result = await this.collection.updateOne(
        { _id: id, urlKey, issueId, outcome: 'self-resolved' },
        { $set: cleared }
      );
      if (result.matchedCount === 0) throw new Error('reverseOutcome: row is not self-resolved');
      return toRecord({ ...existing, ...cleared });
    } catch (err) {
      console.error('Error reversing task decision outcome:', err);
      throw err;
    }
  }

  /**
   * Shared answer operation for both human-answer paths (LIN-2889) — the
   * comment route's task-lineage half (`stampDecisionAnswers`) and
   * `/rulings/answer`. Un-retires a `self-resolved` row before stamping it
   * `answered`, so a human answer always outranks the model's earlier
   * self-resolve guess, then reports the same `firstStampWins` discriminant
   * `markOutcome` returns so both callers classify success identically.
   *
   * @param {Object} entry
   * @param {string} entry.urlKey
   ...
   * @returns {Promise<{record: Object|null, firstStampWins: boolean, unretried: boolean}>}
   */
  async answer({ urlKey, issueId, id, optionId } = {}) {
    let record = await this.markOutcome({ urlKey, issueId, id, outcome: 'answered', optionId });
    let unretried = false;
    if (record?.outcome === 'self-resolved') {
      try {
        await this.reverseOutcome({ urlKey, issueId, id });
        unretried = true;
      } catch (err) {
        // Swallow ONLY the one expected, named lost race — something else
        // moved the row off self-resolved between our markOutcome read and
        // this call. Anything else (a pruned/replaced row, a transient
        // store error) propagates unchanged, matching reverseOutcome's own
        // documented fail-loud contract (see its docstring above).
        if (err.message !== 'reverseOutcome: row is not self-resolved') throw err;
      }
      record = await this.markOutcome({ urlKey, issueId, id, outcome: 'answered', optionId });
    }
    return { record, firstStampWins: !!record?.firstStampWins, unretried };
  }
  ...
  /**
   * Newest scan row per task `(urlKey, issueId)`, for EVERY row regardless
   * of outcome (LIN-2729 / LIN-2893 Step 5) — the second input the
   * newest-then-filter reduction in `lib/unanswered-decisions.js` needs.
   *
   * `listUnansweredForWorkspaces` above filters `outcome:null` in the
   * QUERY, so a task's true newest row is invisible to that method's
   * caller whenever it is outcome-bearing — an older, still-unanswered row
   * for the same task then wins any "most recent" reduction built only from
   * that candidate set (LIN-2729). This method is a deliberately SEPARATE
   * query, not a relaxation of that one's filter (LIN-2227's cost control
   * on the candidate read stays untouched): it fetches every row for the
   * given workspaces with no outcome filter, but narrow by FIELD COUNT
   * instead — only `{urlKey, issueId, outcome, scannedAt}`, no `decision`
   * payload, no behavior history — so the newest-row lookup this buys stays
   * cheap even though nothing is excluded by row count.
   *
   * @param {Array<string>} urlKeys
   * @returns {Promise<Object<string, {urlKey: string, issueId: string, outcome: string|null, scannedAt: *}>>}
   *   Keyed `${urlKey}::${issueId}` — the SAME key format
   *   `lib/unanswered-decisions.js`'s `latestByTask` reduction already uses,
   *   one entry per task (the newest row only).
   */
  async listNewestScanPerTask(urlKeys) {
    if (!this.collection || !Array.isArray(urlKeys) || urlKeys.length === 0) return {};
    try {
      const docs = await this.collection
        .find({ urlKey: { $in: urlKeys } }, { projection: { urlKey: 1, issueId: 1, outcome: 1, scannedAt: 1, _id: 0 } })
        .toArray();
      const newestByTask = {};
      for (const doc of docs) {
        const key = `${doc.urlKey || ''}::${doc.issueId || ''}`;
        const existing = newestByTask[key];
        if (!existing || toMillis(doc.scannedAt) > toMillis(existing.scannedAt)) {
          newestByTask[key] = {
            urlKey: doc.urlKey,
            issueId: doc.issueId,
            outcome: doc.outcome ?? null,
      ...
      console.error('Error listing newest scan per task:', err);
      return {};
    }
  }
   ...
   *
   * A `'self-resolved'` row (LIN-2650) is EXEMPT the same way, for a
   * different reason: it is reversible via `reverseOutcome`, and an
   * eviction between retire and un-retire would leave nothing to reverse.
   * This exemption does NOT widen to every terminal outcome —
   * `'answered'`/`'dismissed'` rows stay in bucket 2 (terminal, evictable),
   * unchanged.
   *
  ...
  async _pruneToCapacity(urlKey, issueId) {
    try {
      ...
      for (const doc of candidates) {
        ...
        else if (doc.outcome === 'self-resolved') { /* LIN-2650: exempt, never evicted — reversibility must not expire at the cap */ }
