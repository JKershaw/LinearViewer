...
/**
 * Assemble the full roadmap model from raw Linear projects + issues.
 *
 * This is the deterministic pre-LLM layer: velocity, per-project milestones
 * (with timing), critical paths, risks, and the analysis summary. Extracted so
 * both the roadmap page route and the server-orchestrated generate endpoint
 * build an identical model from a single Linear fetch (LIN-317).
 *
 * @param {Array} projects - Raw Linear projects
 * @param {Array} issues - Raw Linear issues
 * @param {Object} [options]
 * @param {number} [options.daysBack=90] - Velocity lookback window
 * @param {Date} [options.now=new Date()] - Reference time; snapshotted once so
 *   velocity, timeline, and analysis all derive from a single consistent clock
 *   (avoids sub-millisecond drift between the internal time-dependent helpers).
 * @returns {{velocity, milestones, criticalPaths, risks, analysis, executionQueue}}
 */
export function buildRoadmapModel(projects, issues, { daysBack = 90, now = new Date() } = {}) {
  const velocity = calculateVelocity(issues, daysBack, now);
  ...
  // Project timeline
  const timedMilestones = projectTimeline(milestones, velocity, now);
  ...
  const analysis = analyzeRoadmap({ milestones: timedMilestones, velocity, criticalPaths, now });
