  ...
  if (normalizeProvider(workspace) === 'jira') {
    ...
    if (scope?.authType === 'oauth') {
      return {
        ...
        // `mode=add-source` is REQUIRED again as of LIN-1890 E1, and is no
        // longer inert. The route now defaults to `mode: 'new'` (the landing
        // "Continue with Jira" entry), so a re-link that omitted it would mint a
        // NEW workspace container instead of re-binding the one whose token just
        // failed — the opposite of what this notice promises. Same URL the
        // Settings chooser redirects to (`server.js`).
