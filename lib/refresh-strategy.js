  ...
  if (normalizeProvider(workspace) === 'jira') {
    ...
    if (scope?.authType === 'oauth') {
      return {
        ...
        // `?workspace=` only. `GET /auth/jira/oauth` reads no `mode` — it mints
        // its own `oauthIntent` — so a `mode=add-source` here was inert but read
        // as though the route consumed it (LIN-1887 close-out nit). This is the
        // same URL the Settings chooser redirects to (`server.js`).
        actionUrl: `/auth/jira/oauth?workspace=${encodeURIComponent(urlKey)}`,
      };
    }
    return {
      title: 'Access Token Invalid',
