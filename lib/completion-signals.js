...
export const COMPLETION_SIGNALS = {
  ...
  'plan-review': {
    coreOutcome: 'Plan\'s grounding claims independently re-run and an explicit verdict (Approve / Request Changes / Needs Discussion) issued that hands the plan on to implementation — or names what it must fix',
    signals: [
    ...
    readinessCheck: 'Plan-review verifies, it does not redesign — were all seven of the plan\'s grounding claims re-run independently, and is an explicit verdict recorded that either hands off to implementation or names what the plan must fix?'
  },
  'implementation': {
  ...
  'review': {
    ...
    readinessCheck: 'Review does not merge or mark Done (close-out does that) — is an explicit verdict issued with CI green on the PR, or, when CI is genuinely absent, with the two-branch substitute independently re-run and recorded (either discharges this precondition and authorizes Approve, conditional when the ledger is non-empty — CI absence is not itself a closure-blocker route), and is the `### What CI Did Not Prove` ledger written into the summary comment — or, if not, is a closure blocker filed and routed to?'
  },
  'close-out': {
    coreOutcome: 'Approved work landed: the Not-Proven-by-CI ledger is discharged, the PR merged, the task Done, the summary posted, the description archived and pruned of superseded stage artifacts (or the skipped prune recorded), and remaining follow-ups filed',
    signals: [
      ...
      'Any change close-out authored was a trivial, review-named edit (exact content review quoted; no new or changed test case, function, branch, condition, or control flow — an update to an existing review-quoted literal/expected-string pin is allowed; writing a new witness test is never trivial); commit and review sentence cited verbatim; CI re-established and green on that new head',
    ...
    readinessCheck: 'Did the review record an Approve, is every named ledger item discharged or accepted — proportional to risk class, so a low-risk claim routed to post-merge observation discharges via that routing and a claim review discharged by naming a monitor or a rollback is accepted by citing that name, while an unnamed one, or a risky claim neither lane covers, still needs cited evidence or a named human precondition (an absent or empty ledger under an Approve is a cheap no-op) — and is the work then merged, verified on the landed commit, set Done, summarized, archived-and-pruned (or the skip recorded), and followed-up?'
  },
  'retrospective-audit': {
    coreOutcome: 'Claims the deliverable rests on, and the integrity of the tests asserting them, independently checked against the landed code, with findings reported',
    signals: [
    ...
    readinessCheck: 'Were the deliverable\'s claims and its tests\' integrity independently checked against the landed code (not the overall correctness CI already covers), with findings reported and no state changed?'
  },
  'retro': {
