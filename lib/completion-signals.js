...
export const COMPLETION_SIGNALS = {
  ...
  'review': {
    ...
    readinessCheck: 'Review does not merge or mark Done (close-out does that post-approval) — is an explicit verdict issued with CI green on the PR (Approve authorizes the close) and a "What CI Did Not Prove" ledger written (items, or an explicit empty line), or, if not, is a closure blocker filed and routed to?'
  },
  'close-out': {
    coreOutcome: 'Approved work made final — review ledger discharged (or confirmed explicitly empty), PR merged, task set Done, summary posted, and remaining follow-ups filed and routed',
    signals: [
      'Latest review summary comment read and its "What CI Did Not Prove" ledger located',
      'Ledger gate satisfied: an explicitly-empty ledger passed through, or every item discharged with cited evidence or a precondition-naming human acceptance (green CI alone never discharges an item)',
      'Missing or unparseable ledger treated as a BLOCK, not as empty — routed back to review rather than merged',
      'CI re-confirmed green on a fresh read of the exact commit',
      'PR merged and task set to Done',
      'Closing summary comment posted (merge + ledger discharge + final state)',
      'Remaining review findings / non-blocking class siblings filed as routed, linked follow-up tasks'
    ],
    readinessCheck: 'Is the review ledger discharged (or explicitly empty), CI green on the exact commit, the PR merged, the task Done, the summary posted, and every remaining follow-up filed and routed — with a missing ledger having blocked the close rather than passed through?'
  },
  'retro': {
