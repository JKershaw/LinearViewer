...
export const COMPLETION_SIGNALS = {
  ...
  'close-out': {
    ...
    readinessCheck: 'Did the review record an Approve, is every named ledger item discharged or accepted — proportional to risk class, so a low-risk claim routed to post-merge observation discharges via that routing while a risky one still needs cited evidence or a named human precondition (an absent or empty ledger under an Approve is a cheap no-op) — and is the work then merged, set Done, summarized, and followed-up?'
  },
  'retro': {
