...
9. **Update Linear**: Add a summary comment in Linear with the PR link`,
        ...
        '',
        formatPlanFidelityCheck(context),
        '',
  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        '**Proportional to risk class.** An inherently-unprovable-before-merge claim — model/behavioural compliance with a new directive, real-world recurrence of an incident — becomes a hard close-out *gate item* (one needing cited evidence or explicit human acceptance before merge) only when the change carries real risk: it touches runtime logic, a data path, security, or an external contract. For a **low-risk, reversible** change — prompt-text, docs, or comment-only, with no runtime-logic, data-path, security, or external-contract surface — such a claim is NOT a hard gate item: record it as a **post-merge observation** (a note to monitor, optionally a lightweight monitoring follow-up ticket) and let it discharge through normal post-merge observation, the way a human would simply approve and merge a low-risk change. Key the lane on the *risk surface*, never on lines-of-code, t-shirt size, or literal file type — a docs-only change qualifies, and a prompt change that also edits routing logic does NOT. This never lowers the floor for risky claims: anything guarding a runtime, data, security, or external-contract surface stays a hard gate item, discharged only by cited evidence or a human naming the exact precondition (a reviewer\'s own "no action needed" self-assessment is never that sign-off).',
        '',
...
5. **Complete**: Set ${issue.identifier} status to "Done".
...
7. **File follow-ups**: Create any remaining follow-up tickets the review named, routed to the right project and linked to the source task.`,
        ...
        '',
        '**Proportional to risk class — low-risk post-merge-observation discharge.** When review recorded an item as a **post-merge observation** on a low-risk, reversible change — an inherently-unprovable behavioural-compliance or recurrence claim on a prompt-text / docs / comment-only change with no runtime-logic, data-path, security, or external-contract surface — that routing to normal post-merge observation (optionally a lightweight monitoring follow-up) IS its discharge under (a): you cite the note / follow-up and proceed, without a pre-merge human sign-off ceremony. This lane is narrow and keyed on the risk surface, not on size or file type. It does NOT touch the two hard floors: green CI still never discharges any item, and you still do NOT accept the reviewer\'s own "no action needed" self-assessment as the human sign-off for a *risky* item (one guarding runtime, data, security, or external-contract surface) — such an item stays a hard gate needing cited evidence or a human naming the exact precondition. The proportional lane fixes over-strict *authoring* (a low-risk claim should never have been a hard gate item); it never waves a genuine gate item through here.',
        '',
        '**Gate on the review verdict.** When the latest review records an **Approve** (or **Approve — conditional**), proceed: discharge any gaps it flagged as unproven by CI, and treat the absence of such gaps as an empty ledger — note that in your summary so it stays on the record. Only a task with no review verdict at all is unauthorized to close: leave it open and name `review` as the next action.',
        '',
        '### Cheap When Empty',
        '',
