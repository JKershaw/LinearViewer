  ...
  'plan-review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '### Completion',
        '',
        'Whatever the verdict, record it: add a comment containing the per-check findings and the verdict with its one-line justification. Leave the plan itself untouched — on an Approve the implementer acts on what you found; otherwise the planner does.'
      ].filter(Boolean)
