...
4. **Update Linear**: Add exploration notes as comment, then update description with key findings`,
        ...
        '',
        // Horizontal Obligations — generative meta-instruction (one class, four axes as
        // SEED examples not a fixed checklist). Mirrors the meta-prompt Research-prompts
        // rule. Sits under formatScaleToTask() above so small/single-surface tasks pay no
        // obligation tax. Axis-1 (reuse/don't-duplicate) shares ground with the plan
        // template's Completeness check but lives at a different step — cross-reference,
        // not a merge (LIN-697).
        '### Horizontal Obligations',
        '',
        'Before recommending an approach, characterise not just *what you are building* but *what it must hold true against* in the system it lands in. Enumerate this change\'s obligations to the existing system; for each, cite the specific existing thing and state how your approach honours it:',
        '',
        ...
        '',
        '### Attack Your Own Research',
        '',
        'Before writing your recommendation, turn on your own findings: what existing structure, sibling surface, sync obligation, failure path, or prior behaviour did you NOT check? What did you assert without verifying? Spend one pass actively hunting the obligation you missed — this is the cheap, single-agent version of an adversarial review, and it is where missed checks surface. Resolve or explicitly record anything it turns up before recommending.',
        '',
        ...
        // something already modelled" — the Horizontal Obligations axis-1 blind spot. This
  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '### Verdict',
        '',
        'Conclude with an explicit verdict: **Approve** / **Request Changes** / **Needs Discussion**, with a one-line justification. The verdict is review\'s deliverable — it is what *authorizes* the merge. Review does NOT merge and does NOT mark the task Done itself.',
        '',
        '### Close-Out Gate (pre-merge)',
        '',
        'Review is the gate that AUTHORIZES the close, not the step that performs it. The actual merge, the Done transition, and reconciling related Linear tasks happen AFTER the merge and belong to whoever merges — the orchestrator (Autopilot) on a green merge, or a human. Do NOT mark this task Done, and do NOT try to confirm a merge that has not happened yet (your Approve is its precondition). Before issuing **Approve**, confirm:',
        '',
        ...
        '',
        'On **Approve**, hand off: the merger (Autopilot or a human) re-checks CI on a fresh read of the exact commit, merges, and then closes the task. Your job ends at the verdict plus a summary comment of what you verified.',
        '',
        ...
        '- Leave the current task open with a **Request Changes** / **Needs Discussion** verdict. It closes only once the blocker is resolved and CI is green — review re-runs then, Approves, and the merger performs the final close (review → file blocker → resolve blocker → re-review → merge → close).',
        '',
        ...
        '### Completion',
        '',
        'Whatever the verdict, record it: add a summary comment noting what was verified, the verdict, and the CI state. Do NOT mark the task Done — the merge and the Done transition are the merger\'s, post-merge. On **Approve** the task is left ready-to-merge; on **Request Changes** / **Needs Discussion** it stays open with the next action named (per the cannot-close branch when a blocker surfaced).'
      ].filter(Boolean)
