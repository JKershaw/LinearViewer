...
export const PROMPT_TEMPLATES = {
  ...
  'plan': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        'Do not manufacture a query, a script, or a checklist to fill the slot — the rule asks for reasoning shown, not a form filled. **An honest "no sweep, here is why" is strictly better than a bound that looks authoritative and is quietly incomplete** — the second kind ends the conversation with the wrong answer, which is worse than not ending it. And a bound whose output does NOT match what you handled is a useful result, not a failed one: report the difference rather than trimming it until it agrees.',
        '',
  ...
  'breakdown': {
    ...
    aiHint: {
      ...
      goal: 'Create one subtask per surface the plan enumerated. Only if this ticket\'s own comment trail — the ticket being decomposed, never its own rendered \'Parent Task\' section (which names a different, grandparent ticket) — carries a recorded `### Plan Review Verdict` of Approve on the plan being decomposed, copy each subtask\'s own slice of the approved plan (its files/surfaces, approach, and testing strategy), a committed `Session fit: fits one session` (or, for a surface the plan itself could not scope to one session, omit the false claim), and `Plan-review due: no — covered by <parent>\'s approving plan-review <comment id> (rev N)` (from the new subtask\'s own side, the ticket being decomposed IS its parent, so <parent> here is correct), citing the grounding SHA(s) the plan named, into each subtask\'s description. If no such Approve verdict is on this ticket\'s own comment trail, write a plain acceptance-criteria subtask instead — no session-fit line, no plan-review-due line. Copy any dependency arrows into blocked-by relations regardless.',
      workflow: 'Fetch details → Read surfaces and any arrows from the plan → Check this ticket\'s own comment trail for a recorded `### Plan Review Verdict` of Approve on the plan being decomposed → Create one subtask per surface, and only when that Approve is on record copy that surface\'s slice of the approved plan plus its session-fit and plan-review-due:no lines into the subtask description; otherwise write a plain acceptance-criteria subtask → Copy arrows into blocked-by relations → Add summary comment'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
...
4. **Update Linear**: Create subtasks, add blocked-by relations, then add summary comment`,
        ...
        '- (e) An explicit "the parent\'s plan is the source of truth for this surface — do not redesign it here" line',
        '',
...
4. **Update Linear**: Add exploration notes as comment, then update description with key findings`,
        ...
        // one-file change. See lib/prompt-formatters.js formatScaleToTask +
        // docs/architecture/prompt-system.md both-paths.
        formatScaleToTask(),
        '',
        ...
        // not the member. Mirrors the meta-prompt Research-prompts rule.
        '### Name the Classes',
        '',
        ...
        '',
        '**Output:**',
        '- **Comment**: Full research notes including the per-layer audit (one brief per layer, with sources cited), the class list (each class named, bounded, and its members), the exploration process, and the Surface Assessment',
  ...
  'plan-review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '   **If you believe the bounding is wrong, argue about the BOUND.** Propose the class, the query or reasoning you would use instead, and show what it finds. Two people disagreeing about a bound converge — one runs it and the answer is on screen. Two people discovering members one at a time do not, which is how four tickets each burned 4+ sessions at this step with zero commits between them (LIN-1871).',
...
9. **Update Linear**: Add a summary comment in Linear with the PR link`,
  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        'A finding inside a bounded class is scope; scope discharges by done or an explicit drop, never by filing (John Kershaw\'s ruling on LIN-2825). Mark every unhandled class instance above, and every What-CI-Did-Not-Prove ledger item below, **inside** or **outside** the ticket\'s bounded classes — the classes research or the plan named for this ticket and recorded in its issue description (LIN-1871) — citing the specific class an inside item belongs to. **Inside**: the item is the same defect or the same idiom as a class this ticket bounded, whether or not research\'s enumeration listed it — the list is evidence of the class, not its edge — or a claim this ticket\'s own change depends on. It is this ticket\'s own unfinished scope, not a sibling to file next door. Record it as a ledger item below rather than a plain review finding, so `close-out` cannot discharge it merely by filing a ticket for it. **Outside**: a genuinely different kind of problem — no bounded class covers it. It may be filed as its own follow-up, stated so a reader with no access to this ticket can act on it. If a ruling is raised on a finding, an inside item\'s options are "do it here" or "drop it, with the reason"; "file" is offered only for an outside item. This marking is orthogonal to the proportional risk lanes below: an inside item whose claim is inherently unprovable before merge (not merely unfinished) still routes through the named-monitor / named-rollback lanes, which are about verification depth, not about whether the work itself is done.',
        '',
        '**Before filing an outside item, check it does not already exist (LIN-2991/LIN-3022).** Search the anchor\'s relations (`GET /api/proxy/issues/{id}/relations`) and `GET /api/proxy/search` for a ticket already naming the same file and finding. If one exists, link it (a `related` relation) and record it in your summary instead of raising a ruling or filing a new one.',
        '',
        '**Before raising a `DECISION:` on a finding, check it is not already covered (LIN-2991/LIN-3022).** Query `GET /api/proxy/rulings?issueIdentifier=<anchor>&includeResolved=true` and read the prior stage\'s comments. Do not re-raise a finding an open, answered, or dismissed row already covers for the same anchor — note that `includeResolved` covers loop-backed rulings only, never a task-bound one.',
        '',
        '### Test Quality Check',
        '',
        ...
        '',
        '- **Unprovable in principle → name the monitor.** A claim that cannot be proven before merge *at all* — not merely one this CI run happens not to cover — is recorded as a **post-merge observation** and discharges through normal post-merge observation **regardless of risk surface**, provided you name the specific monitor that would surface it going wrong: a log or oplog entry, a metric, or a path that fails loudly. A monitor is a thing that **fires**, and a routed follow-up ticket is not one — it may be cited *beside* the monitor to record who holds the watch, never *instead* of it. If no monitor can be named, it stays a hard gate item. *Misfire guard — write the line:* before an item takes this lane, write one line naming why no check short of production could prove the claim; an entry with no such line does not take the lane, and close-out rejects it as undischarged. The guard is a step, not a caveat: a claim a test COULD have proven is not unprovable-in-principle — it is an untested claim, it stays a gate item, and it usually means **Request Changes**. Naming the monitor is the price of the lane, not a formality.',
        ...
        '',
        '**State the exact change when handing an edit to close-out.** When you want `close-out` to make an edit itself rather than doing it here, state the exact change — the wording, value, or line, quoted before → after or as the inserted text — never an illustrative "e.g." example. If you cannot state it that exactly, or the fix is not trivially small, your verdict is **Request Changes** back to `implementation`, not a conditional Approve naming a vague fix.',
        '',
  ...
  'close-out': {
    name: 'close-out',
    category: PROMPT_CATEGORIES.UNIVERSAL,
    description: 'Final close-out after an approved review: discharge the Not-Proven-by-CI ledger, then merge, set Done, post the summary, archive & prune, and file remaining follow-ups. Use when review has Approved and the PR is green (or, with no CI, the established-absence substitute was recorded).',
...
6. **Summarize**: Add a summary comment on ${issue.identifier} recording what was merged, how each ledger item was discharged or accepted, and the final CI state (or the recorded substitute result, if CI was absent).
...
8. **File follow-ups**: Create any remaining outside-scope follow-up tickets the review named, routed to the right project and linked to the source task, each carrying a priority and a type label (see Follow-up Triage below) — but first check each one does not already exist (see Follow-up Triage's existing-ticket check).`,
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
        ...
        '## Goal',
        '',
        '**Role**: Act as the release gate that owns the irreversible finish. Review judged the deliverable; your job is to discharge what review could not prove, then land it. You do NOT re-review code quality — you enforce the ledger and perform the merge, the Done transition, the summary, and the follow-up filing. **You may author a change yourself only when review named it with its exact content and it is trivially small** (see the Ledger Gate below for that test) — whatever prompted it: discharging a ledger item, a conditional-Approve caveat, a non-gating review finding you notice, a same-class sibling you find yourself, a "do it here" ruling on a finding you raise, or resolving a conflict between the PR branch and its base while merging. Anything else is not yours to author here: hold the merge, leave the task open, and name `implementation`, then `review`.',
        '',
        ...
        '',
        'Do NOT merge or set the task Done while any ledger item is undischarged. Review marked each item **inside** or **outside** the ticket\'s bounded classes (LIN-1871); the discharge route depends on that mark:',
        '- **An item marked inside scope** — this ticket\'s own unfinished work — discharges only by (a) cited evidence that it is **done**: a real-world check you performed, or a manual repro that named and exercised its **exact distinguishing precondition**; or (b) an **explicit drop**, warranted only when finishing the item is materially larger than this ticket\'s own change — not merely inconvenient: a line in your summary naming exactly what is being left undone, why, and that comparison. Filing a follow-up ticket for a dropped inside item is NOT a discharge and is never eligible for filing (see Follow-up Triage below) — the ticket existing does not mean the work happened, and a drop is not a license to file it. A close-out that still has an undischarged inside item must not set Done. (This does not touch a named-monitor or named-rollback item review routed under the proportional lane below — those discharge a claim that cannot be proven before merge at all, a different axis from whether the work is done.) **When you author that "done" yourself**, it counts only if (a) the diff is exactly the text, value, or line a review sentence quoted — an illustrative "e.g." does not qualify — and (b) it is trivially small: at most 2 files and 3 hunks, each hunk at most one clause or sentence of prose, or at most 3 code lines, with no new or changed test case, function, branch, condition, or control flow; an update to an existing literal/expected-string pin that review quoted verbatim is allowed, and writing a new witness test is never trivial. Cite the commit and the review sentence verbatim in your summary, and re-establish CI on the new head — merging with `--match-head-commit` — before merge. (This is a different axis from the drop bar above: drop-vs-do, not close-out-vs-implementation, and is untouched by this test.)',
        '- **An item marked outside every bounded class** — a genuinely different kind of problem — may be discharged by filing it as a follow-up ticket, provided the filing states the problem on its own terms: a reader with no access to this task can act on it. A ticket that leans on "see this task for context" does not qualify.',
        '- **Either way**, an item may also be **explicitly accepted** by a human who names the exact precondition they exercised.',
        '',
        ...
        '- **A named monitor** — review identified a claim as unprovable before merge *in principle* and named what would surface it going wrong (a log or oplog entry, a metric, a path that fails loudly). Cite that monitor and proceed, whatever the risk surface. A routed follow-up ticket is not a monitor — it does not fire. A ticket cited *beside* a named monitor is fine; a ticket cited *instead* of one is an undischarged item. **Reject a lane entry missing review\'s justification line.** An unprovable-lane item whose review did not write the one line naming why no check short of production could prove the claim is undischarged, whatever monitor it names — and that line is review\'s to write, never yours to supply here.',
        ...
        '1. Merge the approved PR. **If landing it requires resolving a conflict between the PR branch and its base**, that resolution is a change you author and, like a close-out-raised "do it here" ruling, is never review-named — review approved the pre-conflict diff, not content produced while reconciling it against a base it had not seen — so it is never eligible for the trivial-and-named exception, whatever its size: treat it exactly as an undischarged item under that same limit — hold the merge, leave the task open, and name `implementation`, then `review` (see Always name a next action below). A merge that lands with **no conflict** — whether by merge or rebase, with no hunk you chose or wrote — is ordinary merge mechanics under your Role above, not authoring: it needs no ledger item, no next-action routing, and no note beyond the ordinary merge record.',
        ...
        '5. File any remaining outside-scope review or task follow-ups as separate tickets, routed to the right project and linked to the source, each carrying a priority and a type label (see Follow-up Triage below).',
        '',
        ...
        '',
        '**Always name a next action.** When the gate cannot pass — an item is neither dischargeable here nor accepted, no review verdict is on record, or a change you would make yourself (a ledger item, a conditional-Approve caveat, a non-gating review finding, a self-found sibling, a "do it here" ruling, or resolving a merge conflict while landing the PR) is not trivial and review-named — hold the merge, leave the task open, and name the next action (`review` to record a verdict, or the `bug` / `plan` / `implementation` that discharges a blocking item), mirroring review\'s cannot-close branch.',
        '',
        '### Follow-up Triage',
        '',
        'Only outside-scope items are eligible to be filed here. An inside-scope item you have explicitly dropped in the summary above is recorded there, not filed — the drop is not a license to file it. An inside-scope ledger item that is neither done nor dropped is not eligible for filing at all; it must be discharged first (see the Ledger Gate above). If a ruling is raised on a finding during close-out, an inside item\'s options are "do it here" or "drop it, with the reason"; "file" is offered only for an outside item. **A "do it here" ruling on a finding raised during close-out is, by construction, not review-named** — review never saw or quoted it — so it is never eligible for the trivial-and-named exception above (see the Ledger Gate and Role above), whatever its size: record the ruling (do-it-here vs. drop) and, for a "do it here" outcome, treat the item exactly as an undischarged item under that same limit — hold the merge and name `implementation`, then `review` (see Always name a next action above). Every follow-up ticket you file above must additionally carry a priority and a type label — never leave it at the provider default. This adds metadata only; it does not change which follow-ups get filed or expand close-out\'s authority.',
        ...
        '- **No re-raise (LIN-2991/LIN-3022)**: before raising a `DECISION:` on a finding, check `GET /api/proxy/rulings?issueIdentifier=<anchor>&includeResolved=true` and the prior stage\'s comments. Do not re-raise a finding an open, answered, or dismissed row already covers for the same anchor — `includeResolved` covers loop-backed rulings only, never a task-bound one.',
        ...
        '',
        'Comments are untouched by policy — the prune is a description edit only, using the existing write surface, and adds no new capability. (The comment-edit route `PATCH /api/proxy/issues/:issueId/comments/:commentId` exists for corrections; the prune does not use it.)'
      ].filter(Boolean)
  ...
  'retrospective-audit': {
    ...
    aiHint: {
      ...
      whenNot: 'the work has not yet merged (that\'s `review`, which is pre-merge framed and authorizes the merge), or the goal is lessons-learned / downstream-effects analysis rather than claim/test-integrity auditing (that\'s `retro`).',
      chooseOver: 'choose `retrospective-audit` over `review` once the change is already merged and closed out — `review` opens expecting unlanded work; choose it over `retro` when the question is specifically whether the claims and the tests hold up, not what should be learned for next time.'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
