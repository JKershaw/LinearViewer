  ...
  'plan-review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        '1. **Completeness check — re-run the plan\'s own sweep first.** Where the plan cites a reproducible sweep for a class it claims to cover, run THAT query, at the sha it names, and diff its output against the members the plan handled. This is the cheap, mechanical half of the check: either the output matches what the plan handled or it does not, and the difference is the finding. **The expensive half is the fallback, and it is reached only where the plan cites no sweep** — there you search independently for the concept (the behavior, a shared identifier, what a caller or user observes) rather than only the symbol the ticket cites. Missed siblings are **Request Changes**: each must be folded in, or explicitly marked out-of-scope with a named ticket identifier.',
