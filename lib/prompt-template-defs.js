...
9. **Update Linear**: Add a summary comment in Linear with the PR link`,
  ...
  'review': {
    ...
    aiHint: {
      ...
      goal: 'Verify the implementation is complete and correct, write the "What CI Did Not Prove" ledger, then issue a verdict (Approve / Request Changes / Needs Discussion) that authorizes the close. Review does not merge or mark Done — a non-empty ledger makes the Approve conditional on close-out discharging it.',
      workflow: 'Fetch details → Verify requirements/tests/CI green on the PR → Write the What-CI-Did-Not-Prove ledger → Issue verdict (conditional if the ledger is non-empty) → Add findings + ledger + verdict as comment in Linear → hand off to close-out'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        'Before the verdict, write a **Not-Proven-by-CI ledger**: enumerate every claim the deliverable depends on that the green CI run does not actually exercise. Common kinds (illustrative, not a fixed checklist): an external contract or API behaviour CI cannot reach; a *producer* that must emit an input this change now *consumes*; a user-reachable entry path; a sibling on a parallel surface; or anything only a human can confirm. For each item write one `- ` bullet naming the claim and how it can be discharged — a real-world check, a manual repro with its **exact distinguishing precondition**, or a routed follow-up ticket.',
        '',
        'This ledger is the handoff artifact the `close-out` step consumes. Write it as a list under this exact heading, and carry it in the summary comment (below), so close-out can find and gate on it.',
        '',
        '**If CI genuinely covers the whole deliverable, say so explicitly with the single line `CI covers the deliverable; ledger empty.`** A self-contained change is expected to be empty — do not manufacture doubt about a clean refactor. An empty ledger is the cheap, common case; an absent or unwritten ledger is NOT the same as an empty one and will block close-out.',
        '',
        '### Verdict',
        '',
        'Conclude with an explicit verdict: **Approve** / **Request Changes** / **Needs Discussion**, with a one-line justification. The verdict is review\'s deliverable — it is what *authorizes* the close. Review does NOT merge and does NOT mark the task Done itself.',
        '',
        '**The verdict is conditional on the ledger.** A non-empty ledger means the verdict is **Approve — conditional on close-out discharging the ledger**, never a bare Approve: the code is internally sound, but it must not land until close-out has discharged every ledger item. Only an empty ledger (the explicit `CI covers the deliverable; ledger empty.` line) yields a plain **Approve**.',
        '',
        '### Close-Out Gate (pre-merge)',
        '',
        'Review AUTHORIZES the close; it does not perform it. The actual merge, the Done transition, posting the summary, and filing remaining follow-ups all happen AFTER approval and belong to the **`close-out`** step (run by the orchestrator or a human). Do NOT mark this task Done, do NOT merge, and do NOT try to confirm a merge that has not happened yet (your Approve is its precondition). Before issuing **Approve**, confirm:',
        '',
        '1. **CI is green on the PR.** The change is not approvable while CI is red, however complete the code looks. This is a check, not a fix loop — if CI is red because the work is unfinished, that is **Request Changes**, handed back to implementation.',
        '2. **The verdict is Approve** and the checklist, gap analysis, regression, and class checks above are satisfied.',
        '',
        'On **Approve**, hand off to the `close-out` step, carrying the ledger in your summary comment. Close-out re-checks CI on a fresh read of the exact commit, discharges every ledger item, then merges and closes. Your job ends at the verdict plus the summary comment (including the ledger) of what you verified.',
        '',
        '**Cannot-close branch — when the work has landed but CI is red for a real failure, or verifying it surfaced a hidden blocker (a second, larger bug the fix exposed; a prerequisite that must be fixed first):** do NOT loop back into another `review`. Instead:',
        ...
        '### Completion',
        '',
        'Whatever the verdict, record it: add a summary comment noting what was verified, the **What CI Did Not Prove** ledger (its items, or the explicit `CI covers the deliverable; ledger empty.` line), the verdict, and the CI state. Do NOT mark the task Done — the merge and the Done transition belong to `close-out`. On **Approve** the task is left ready for close-out; on **Request Changes** / **Needs Discussion** it stays open with the next action named (per the cannot-close branch when a blocker surfaced).'
      ].filter(Boolean)
  ...
  'close-out': {
    name: 'close-out',
    category: PROMPT_CATEGORIES.UNIVERSAL,
    description: 'Perform the irreversible close for an approved task: discharge the review ledger, then merge, set Done, post the summary, and file remaining follow-ups. Use after review has issued an Approve.',
    completionSignals: COMPLETION_SIGNALS['close-out'],
    aiHint: {
      situation: 'review approved; ledger pending discharge before the close',
      goal: 'Discharge the review ledger, then perform the irreversible close — merge, set Done, post the summary, file remaining follow-ups. A missing or unparseable ledger blocks the close; an empty one is a cheap no-op pass-through.',
      workflow: 'Fetch details → Read the latest review ledger → Gate (missing/unparseable blocks, empty passes, non-empty discharge each item) → Re-check CI on the exact commit → Merge → Set Done → Post summary → File follow-ups'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
...
1. **Fetch details**: Get full issue details and the comment thread for ${issue.identifier}
2. **Read the review handoff**: Find the latest review summary comment and locate its **What CI Did Not Prove** ledger (the list under that exact heading)
3. **Gate**: Do NOT take any irreversible action until the ledger gate below is satisfied
4. **Re-check CI**: On a fresh read of the exact commit the approved PR points at, confirm CI is green NOW — green earlier is not green now
5. **Merge**: Merge the approved PR
6. **Set Done**: Move ${issue.identifier} to Done
7. **Post the summary**: Add a closing comment recording the merge, how each ledger item was discharged, and the final state
8. **File follow-ups**: Create or link any remaining review findings and non-blocking class siblings as separate tracked tasks, routed to the right project and linked back to ${issue.identifier}`,
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
        ...
        '## Goal',
        '',
        '**Role**: Act as the release steward who performs the irreversible finishing actions for an approved task — merge, mark Done, post the summary, and file remaining follow-ups. You own exactly the actions review is forbidden from taking. You do not re-litigate code quality; you enforce the handoff gate, then close.',
        '',
        '### The Ledger Gate (read before any irreversible action)',
        '',
        'Read the latest review summary comment and find its **What CI Did Not Prove** ledger — the list under that exact heading. The ledger is the handoff review wrote for you; everything below turns on it.',
        '',
        '- **Missing or unparseable ledger BLOCKS the close.** If you cannot find the heading, or cannot read the list under it, do NOT merge and do NOT set Done. A missing ledger is NOT an empty one — route back to `review` to produce it. (Absence reading as "nothing left to prove" is exactly the collapse this step exists to prevent.)',
        '- **Empty ledger → cheap pass-through.** Only the explicit line `CI covers the deliverable; ledger empty.` authorizes proceeding with no per-item discharge. This is the common case for a self-contained change; do not manufacture doubt about it.',
        '- **Non-empty ledger → discharge every item first.** Each item may be cleared only once it is either (a) discharged with evidence you cite, or (b) explicitly accepted by a human. **A green CI run is never evidence for a ledger item — the item exists precisely because CI cannot reach it.** A human "validated" that does not name the exact precondition it exercised does NOT discharge the item. If any item is undischarged, do NOT merge or set Done: record what remains and route it (a real-world check, a manual repro against the named precondition, or a routed follow-up ticket).',
        '',
        '### On an all-clear gate',
        '',
        '1. Re-confirm CI is green on a fresh read of the exact commit (green earlier is not green now).',
        '2. Merge the approved PR.',
        '3. Move the task to Done.',
        '4. Post the closing summary comment: what merged, how each ledger item was discharged (or that the ledger was explicitly empty), and the final state.',
        '5. File the remaining follow-ups: any review findings or non-blocking class siblings become separate tracked tasks, routed to the right project and linked to the source task. A class sibling is a follow-up, not a ledger item — unless the deliverable\'s correctness actually depends on it.',
        '',
        '### Always name the next action',
        '',
        'If the gate blocks — a missing or unparseable ledger, an undischarged item, or red CI on the fresh read — do NOT merge: name the exact next action that unblocks the close (back to `review` for a missing ledger, or the `bug`/`plan`/`implementation` that discharges a failing item) so the work never stalls without an owner.'
      ].filter(Boolean)
