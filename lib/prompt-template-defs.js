...
6. **Update Linear**: Add summary comment in Linear`,
        ...
        '3. For new dependencies being integrated for the first time, verify all required setup (CSS, config, initialization) beyond just API calls',
  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '### Manual Verification',
        '',
        'For visual, layout, or behavioral changes — verify directly:',
        '- Take screenshots at key states and inspect them for correctness',
        '- Run the app and navigate through affected flows',
        '- Check responsive behavior at different viewport sizes if layout changed',
        '',
        'If something cannot be verified this way (external service integration, cross-browser behavior, subjective UX), flag it for human testing with specific instructions on what to check.',
        '',
        '### Completion',
        '',
        'When review passes:',
