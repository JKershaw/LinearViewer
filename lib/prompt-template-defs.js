  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '### Close-Out Gate',
        '',
        'Review is also the close-out step: a session ends cleanly only when the work is either merged-and-green or explicitly handed to a blocker. Before deciding the outcome, run this gate:',
        '',
        '1. **Verify it landed.** Confirm the work is actually merged, not just committed on a branch — merged state is the hard close gate. Check deploy state too, but only when it is observable; the agent cannot always see deploys, so a merged-but-not-yet-deployed task may still close.',
        '2. **Confirm CI is green.** The change is not closeable while CI is red, however complete the code looks.',
        '3. **Reconcile related Linear tasks.** Make sure the parent, siblings, and any follow-ups this work touched are current — sync their status and leave a short summary where one is needed. This is status/summary reconciliation, not a mandate to restructure the task tree.',
        '',
        '**Cannot-close branch — when the work has landed but CI is red, or verifying it surfaced a hidden blocker (a second, larger bug the fix exposed; a prerequisite that must be fixed first):** do NOT loop back into another `review`. Instead:',
        ...
        '- Leave the current task open. Close it only once the blocker is resolved and CI is green — review re-runs then and performs the final close (review → file blocker → resolve blocker → close original).',
        '',
        ...
        '### Completion',
        '',
        'When the Close-Out Gate passes (merged, CI green, related tasks reconciled):',
        '1. Add a summary comment noting what was verified and the merged/CI state',
        '2. Update task status to reflect completion',
        '',
        'When the gate cannot pass (CI red or a blocker surfaced), follow the cannot-close branch above instead: file or link the `blocks` ticket, record what blocks closure in a comment, and hand off to that blocker — leave this task open rather than forcing a terminal state or repeating review.'
      ].filter(Boolean)
