  ...
  [WORK_ISSUE_LABELS.BLOCKED]: {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        formatWorkflow(PROMPT_CATEGORIES.WORK_ISSUE, issue, { useMcp: featureFlags.linearMcp !== false }),
        '',
        '## Context',
        '',
  ...
  'plan': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        `4. **Update description**: Add the implementation plan to ${issue.identifier} via Linear MCP`,
        '5. **Assess scope**: Evaluate whether the task needs breakdown into subtasks and identify any pre-requisite refactors',
        ...
        '- Identify code that must be cleaned up to make the implementation maintainable',
        '- Explicitly state if the plan includes cleaning up existing code to make the implementation cleaner',
        '',
        'If breakdown or refactoring is needed, note this in the description alongside the plan.',
      return sections.join('\n')
    }
  },
  ...
  'look-into': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        formatInformOnlyWorkflow(issue, { useMcp: featureFlags.linearMcp !== false }),
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
...
4. **Update Linear**: Create subtasks via Linear MCP, add blocked-by relations, then add summary comment`,
        ...
        '### Creating Subtasks',
        '',
        'For each subtask, create it in Linear via Linear MCP with:',
  ...
  'research': {
    ...
    aiHint: {
      ...
      workflow: 'Fetch details → Research → Add exploration notes as comment, update description with key findings via Linear MCP'
        ...
        `2. **Fetch details**: Use Linear MCP to get full issue details for ${issue.identifier}`,
        `3. **Refactor Identification**: Evaluate if the target area is "refactor-ready" and identify codebase improvements that would improve the outcome of this research or subsequent implementation.`,
        '4. **Analyze**: Complete the goal below',
        `5. **Update Linear**: Add exploration notes as comment, then update description with key findings via Linear MCP`,
        `## Workflow

1. **Start**: Use Linear MCP to set ${issue.identifier} status to "In Progress" (if not already)
2. **Fetch details**: Use Linear MCP to get full issue details for ${issue.identifier}
3. **Refactor Identification**: Evaluate codebase health and identify refactorings that would simplify or improve the task outcome.
4. **Analyze**: Complete the goal below
5. **Update Linear**: Add exploration notes as comment, then update description with key findings via Linear MCP`,
        '',
        '## Context',
        ...
        '- Refactoring and cleanup recommendations to improve the outcome',
  ...
  'design': {
    ...
    aiHint: {
      ...
      workflow: 'Fetch details → Design → Add full analysis as comment, update description with chosen design via Linear MCP'
        `2. **Fetch details**: Use Linear MCP to get full issue details for ${issue.identifier}`,
        '3. **Analyze**: Complete the goal below',
        '4. **Integration & Cleanup**: Evaluate if the design necessitates a migration or deletion of legacy patterns',
        `5. **Update Linear**: Add full design analysis as comment, then update description with chosen approach via Linear MCP`,
        '',
        `## Workflow
...
4. **Update Linear**: Add full design analysis as comment, then update description with chosen approach via Linear MCP`,
        ...
        '### Integration & Cleanup',
        '',
        'Consider if the new design renders old code obsolete. Include a migration/deletion plan for old patterns if applicable.',
        '',
        '**Output:**',
  ...
  'spike': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        formatWorkflow(PROMPT_CATEGORIES.UNIVERSAL, issue, { useMcp: featureFlags.linearMcp !== false }),
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
  ...
  'context': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        formatReadOnlyWorkflow(issue, { useMcp: featureFlags.linearMcp !== false }),
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
