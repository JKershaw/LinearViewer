...
6. **Update Linear**: Add summary comment in Linear`,
  ...
  'retro': {
    name: 'retro',
    category: PROMPT_CATEGORIES.UNIVERSAL,
    description: 'Run a retrospective on a completed task: reconstruct what happened, identify downstream effects, and surface honest lessons. Use after a task is Done.',
    completionSignals: COMPLETION_SIGNALS['retro'],
    aiHint: {
      situation: 'task is Done, worth a backward look',
      goal: 'Reconstruct what happened from Linear and git history, identify downstream effects, and give an honest assessment with actionable lessons.',
      workflow: 'Fetch details → Reconstruct from Linear + git history → Assess downstream effects → Add retrospective as comment in Linear'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '## Goal',
        '',
        '**Role**: Act as a retrospective analyst reviewing completed work with the benefit of hindsight. Your role is to give an honest account and surface useful lessons, not to assign blame or re-litigate decisions.',
        '',
        'A retrospective is most meaningful once the task has actually shipped. If this task is not yet complete, say so and recommend revisiting after it is Done rather than forcing a premature retro.',
        '',
        '### Reconstruct what happened',
        '',
        'Build the timeline from evidence, not memory:',
        '- **Linear history**: read the description, comments, status changes, subtasks, and relations (follow-ups, duplicates, reopened links) above and in Linear.',
        `- **Git history**: find the commits tied to this task — \`git log --grep=${issue.identifier}\` — and the work done between the task's creation and completion dates (\`git log --since=<createdAt> --until=<completedAt> -- <files the task touched>\`). Use the task's actual dates from Linear.`,
        '',
        '### Identify downstream effects',
        '',
        'This is the part hindsight makes visible. Look for what happened *after* the task completed:',
        ...
        '### Scale to the task',
        '',
        'Match the depth to the work: a small leaf task gets a short, focused retro on its single change. For an epic with subtasks, aggregate across children — which shipped, which slipped or were dropped, and the cumulative downstream churn — rather than retro-ing each subtask in isolation.',
        '',
        'Document the retrospective as a comment:',
        ...
        '- **Downstream impact**: follow-up fixes, regressions, reopened work, or "no downstream issues found"',
