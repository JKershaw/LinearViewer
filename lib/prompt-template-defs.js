...
8. **File follow-ups**: Create any remaining follow-up tickets the review named, routed to the right project and linked to the source task.`,
        ...
        '3. **Prune**: rewrite the description via `PATCH /api/proxy/issues/:id`, removing the embedded stage-artifact sections the landed work has discharged — implementation plan(s) and any plan revisions, research-findings sections, and scoping/design sections — and replacing them with a short stub recording: what was implemented, the PR link, that the plan was implemented as approved (or naming the deviations), and that the full history is preserved in the task snapshots and the comment trail.',
        '4. **Never prune**: the original problem statement, acceptance criteria, reproduction steps, scope (the scoping template\'s own "single source of truth" instruction stands — do not contradict it here), or any section a follow-up ticket references. When in doubt, keep it.',
        '5. **Preserve the load-bearing markers verbatim**: the stub MUST retain, word for word, the committed session-fit phrase (`fits one session` / `needs multiple sessions`) and/or an `Implementation Plan` heading. Two other deterministic readers key on exactly these literals in the description — deleting them silently breaks both.',
        '',
