  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        'A finding inside a bounded class is scope; scope discharges by done or an explicit drop, never by filing (John Kershaw\'s ruling on LIN-2825). Mark every unhandled class instance above, and every What-CI-Did-Not-Prove ledger item below, **inside** or **outside** the ticket\'s bounded classes — the classes research or the plan named for this ticket and recorded in its issue description (LIN-1871) — citing the specific class an inside item belongs to. **Inside**: the item is a member of a class this ticket already committed to covering, or a claim this ticket\'s own change depends on — it is this ticket\'s own unfinished scope, not a sibling to file next door. Record it as a ledger item below rather than a plain review finding, so `close-out` cannot discharge it merely by filing a ticket for it. **Outside**: the item falls outside every class this ticket bounded — a genuinely separate matter. It may be filed as its own follow-up, stated so a reader with no access to this ticket can act on it. This marking is orthogonal to the proportional risk lanes below: an inside item whose claim is inherently unprovable before merge (not merely unfinished) still routes through the named-monitor / named-rollback lanes, which are about verification depth, not about whether the work itself is done.',
        '',
        '### Test Quality Check',
        '',
...
8. **File follow-ups**: Create any remaining follow-up tickets the review named, routed to the right project and linked to the source task, each carrying a priority and a type label (see Follow-up Triage below).`,
        ...
        '- **An item marked inside scope** — this ticket\'s own unfinished work — discharges only by (a) cited evidence that it is **done**: a real-world check you performed, or a manual repro that named and exercised its **exact distinguishing precondition**; or (b) an **explicit drop**: a line in your summary naming exactly what is being left undone and why. Filing a follow-up ticket for an inside item is NOT a discharge, however well the ticket is written — the ticket existing does not mean the work happened. A close-out that still has an undischarged inside item must not set Done. (This does not touch a named-monitor or named-rollback item review routed under the proportional lane below — those discharge a claim that cannot be proven before merge at all, a different axis from whether the work is done.)',
        '- **An item marked outside every bounded class** — a genuinely separate matter — may be discharged by filing it as a follow-up ticket, provided the filing states the problem on its own terms: a reader with no access to this task can act on it. A ticket that leans on "see this task for context" does not qualify.',
        ...
        '### Follow-up Triage',
        '',
        'Only outside-scope items, and inside-scope items you have explicitly dropped in the summary above, are eligible to be filed here — an inside-scope ledger item that is neither done nor dropped is not eligible for filing; it must be discharged first (see the Ledger Gate above). Every follow-up ticket you file above must additionally carry a priority and a type label — never leave it at the provider default. This adds metadata only; it does not change which follow-ups get filed or expand close-out\'s authority.',
