...
export const PROMPT_TEMPLATES = {
  ...
  'plan': {
    ...
    aiHint: {
      ...
      goal: 'Create a clear implementation plan, enumerate surfaces with their dependencies, and name the execution structure (single pass, sequential chain, foundation + parallel, or hybrid).',
      workflow: 'Set status to "In Progress" → Analyze requirements → Document plan in description → Enumerate surfaces and draw dependency arrows → Name the execution structure → Ready for implementation or breakdown'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '### Scope Assessment',
        '',
        'After drafting the plan, decide how many execution passes this work needs. One pass is one implementation run — the scope a single focused session can design, implement, and verify without losing track of edges.',
        '',
        '**History signal:** Run `git log --oneline -10 -- <files in your plan>`. If recent commits touch the same code paths, read them: what were they protecting against, and could your changes affect or depend on that? Document any constraints this surfaces in the plan.',
        '',
        ...
        '- Which other surfaces it reads from or writes to (draw the dependency arrows between them explicitly — these arrows are the shared-boundary information, so one list serves both the cross-cutting analysis and the scope decision)',
        '- Two or three edges that are easiest to miss',
        '',
        '**Choose the structure whose shape matches the arrows you drew:**',
        '- **Single pass** — one or two surfaces, few arrows, combined edges fit one focused session',
        '- **Sequential chain** — each surface needs the previous one merged before the next can be designed or tested',
        '- **Foundation + parallel** — one shared surface everyone depends on, then several independent surfaces that only touch the foundation, not each other',
        '- **Hybrid** — a foundation, then some parallel children, then integration; choose only when at least two of the other three shapes each appear among two or more surfaces',
        '',
        '**Pick the smallest structure whose catches are concrete.** For each candidate, name what a run of that shape would catch: what a single pass would see in one window, what a split would dwell on separately. Choose the structure whose catches you can point to specifically. When two candidates look equivalent, prefer single pass — a plan with many files but one coherent change usually fits one pass.',
        '',
        '**Document the chosen structure in the issue description alongside the plan.** Name it clearly as one of: single pass / sequential chain / foundation + parallel / hybrid, and list the surfaces with their dependency arrows beneath it.',
        formatIfBlocked()
      ].filter(Boolean)
  ...
  'breakdown': {
    ...
    aiHint: {
      situation: 'plan names a multi-pass execution structure (sequential chain, foundation + parallel, or hybrid)',
      goal: 'Create one subtask per surface the plan enumerated, and copy the plan\'s dependency arrows directly into blocked-by relations.',
      workflow: 'Fetch details → Read surfaces and arrows from the plan → Create one subtask per surface → Copy arrows into blocked-by relations → Add summary comment with structure name'
    },
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
...
4. **Update Linear**: Create subtasks, add blocked-by relations, then add summary comment`,
        ...
        '',
        'Read the surfaces the plan enumerated and the dependency arrows it drew between them. Create one subtask per surface (unless the plan groups several small surfaces together). Each subtask\'s `blocked-by` relations are exactly the surfaces the plan shows it depending on — copy the arrows directly. The resulting subtask graph will match the plan\'s named structure automatically, because the arrows *are* the structure.',
        ...
        '2. Add a summary comment to the parent listing the subtasks with the structure name from the plan'
      ].filter(Boolean)
...
6. **Update Linear**: Add summary comment in Linear`,
        ...
        '2. Before modifying a file, run `git log --oneline -15 -- <file>`. If a file has 3+ recent commits, read those first — repeated changes signal hidden coupling or non-obvious constraints not visible in the code',
        '3. Each step should be self-contained: specify both the behavior **and** its cleanup/teardown contract (not just the happy path)',
        '4. For new dependencies being integrated for the first time, verify all required setup beyond just API calls',
        '5. Write tests for new/changed behavior',
        ...
        '',
        '**CSS and event side effects:** Modifying `touch-action`, `pointer-events`, `overflow`, `z-index`, or event listeners can break siblings or children. Before changing any of these, verify:',
        '- Will child elements inherit the new value unexpectedly?',
        '- Will the event bubble to parent handlers that rely on the current behaviour?',
        '',
        '### Testing',
        '',
