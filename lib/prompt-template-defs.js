  ...
  'review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '### Verdict',
        '',
        'Conclude with an explicit verdict: **Approve** / **Request Changes** / **Needs Discussion**, with a one-line justification. The verdict is review\'s deliverable — it *authorizes* the close, it does not perform it. **Make the approval conditional on the ledger:** when the `### What CI Did Not Prove` ledger above is non-empty, the verdict must be `Approve — conditional on close-out discharging the ledger`, never a bare Approve; only an explicitly empty ledger may carry a plain **Approve**. Reserve that conditional form for a ledger every item of which close-out can actually discharge within its own trivial, review-named-edit bound (LIN-3033) — a real-world check or manual repro, a named monitor or named rollback, a routed outside follow-up, or an exactly-stated trivial edit; when the ledger instead holds an **inside** item whose discharge requires authoring beyond that bound — a new or changed test, or a code change — the verdict is **Request Changes** back to `implementation`, not a conditional Approve. Review does NOT merge, does NOT mark the task Done, and does NOT file the close-out follow-ups itself.',
        '',
