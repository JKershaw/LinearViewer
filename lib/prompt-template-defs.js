...
8. **File follow-ups**: Create any remaining follow-up tickets the review named, routed to the right project and linked to the source task, each carrying a priority and a type label (see Follow-up Triage below).`,
        ...
        '- **Priority**: derive it from the finding\'s own risk (how bad if unaddressed, how likely to recur) and state that reasoning in one line on the ticket. Set it via the provider-neutral `priorityLevel` (ascending, 4 = highest) or the native `priority` field — do not invent a numeric scale of your own.',
