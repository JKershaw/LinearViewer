...
2. **Read the ledger**: From that review comment, read the \`### What CI Did Not Prove\` ledger and the verdict.
...
7. **File follow-ups**: Create any remaining follow-up tickets the review named, routed to the right project and linked to the source task.`,
        ...
        '',
        'Review is write-only and has already recorded its verdict plus a `### What CI Did Not Prove` ledger in its summary comment. You consume that ledger and decide whether the work may become irreversible.',
        '',
        ...
        '',
        '**Missing or unparseable ledger BLOCKS — absent is not empty.** If the latest review comment has no `### What CI Did Not Prove` section, or it cannot be parsed, STOP: do not merge and do not set Done. Treat it as an undischarged gate, route back to `review` to (re)write the ledger, and name that as the next action. A missing ledger must never be read as "nothing to prove" — that is the exact collapse this step exists to prevent.',
        '',
        '### Cheap When Empty',
        '',
        ...
        '',
        '**Always name a next action.** If the gate cannot pass — an item is neither dischargeable here nor accepted, or the ledger is missing/unparseable — do NOT merge; leave the task open and name the next action (`review` to (re)write the ledger, or the `bug` / `plan` / `implementation` that discharges a blocking item), mirroring review\'s cannot-close branch.'
      ].filter(Boolean)
