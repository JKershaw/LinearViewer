  ...
  'plan': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        '**Cite the sweep, not the conclusion.** Wherever the plan claims it has covered a *class* — every call site, every lane, every consumer, every shape — cite the **reproducible query whose output IS that enumeration**: an `rg`/`grep` pattern, an AST query, or a test that fails on an unhandled member. Paste the output and record the commit sha it was run at. Then map each member to in- or out-of-scope. A hand-written list with no query behind it is a claim; the query is the evidence, and it is what the reviewer re-runs instead of hunting members one at a time.',
        '',
        '**Some classes genuinely have no sweep, and saying so is a first-class answer.** Two shapes recur: (a) the *destinations* of new or moved code, which cannot be swept out of code that does not exist yet; (b) classes whose population is production data rather than source, where the enumeration needs a corpus query, not a repo query. For these, state plainly that the class has no sweep, which shape it is (or name a third), and what you did instead to bound it.',
        '',
        'Do not manufacture a query to fill the slot. **An honest "no sweep, here is why" is strictly better than a query that looks authoritative and is quietly incomplete** — the second kind ends the conversation with the wrong answer, which is worse than not ending it. And a sweep whose output does NOT match what you handled is a useful result, not a failed one: report the difference rather than trimming the query until it agrees.',
        '',
  ...
  'plan-review': {
    ...
    generate: (issue, context, featureFlags = {}) => {
      const sections = [
        ...
        '',
        '1. **Completeness check — re-run the plan\'s own sweep first.** Where the plan cites a reproducible sweep for a class it claims to cover, run THAT query, at the sha it names, and diff its output against the members the plan handled. This is the cheap, mechanical half: either the output matches what the plan handled or it does not, and the difference is the finding. Only where the plan cites no sweep do you fall back to searching independently for the concept (the behavior, a shared identifier, what a caller or user observes) rather than only the symbol the ticket cites. Missed siblings are **Request Changes**: each must be folded in, or explicitly marked out-of-scope with a named ticket identifier.',
        '   **If you believe the enumeration is wrong, argue about the SWEEP.** Propose the query you would run instead and show its output. Two people disagreeing about a query converge — one runs it and the answer is on screen. Two people discovering members one at a time do not, which is how four tickets each burned 4+ sessions at this step with zero commits between them (LIN-1871).',
