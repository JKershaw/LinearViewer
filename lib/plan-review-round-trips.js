 ...
 * the headline.
 *
 * **The one-revision-cycle assumption this header used to state here is
 * WRONG and is not restated (LIN-1871).** An earlier version of this comment
 * claimed the observable shape was "0-vs-1, not 0-vs-2", reasoning that the
 * templates' own "one revision cycle is the bound" language
 * (`lib/prompt-template-defs.js`, `lib/prompts/meta-prompt-template.js`,
 * `lib/prompts/autopilot-kickoff.js`) censored the endpoint there in
 * practice. `docs/papers/harbour/review-loops.md` (LIN-2800) measured the real
 * record and refuted it: 33 of 94 tickets went round plan-review TWO OR
 * MORE times. Both `countRoundTrips` (below) and `walkR0`'s `subWindows`
 * walk past R0 already count and resolve every round, not just the first —
 * that was never capped — so this fix is to the CLAIM, not the count: do not
 * read `roundTrips.distribution` as though anything past `1` were noise, and
 * do not build a downstream summary that collapses it back to a boolean.
 *
 ...
 *     cross-anchor conflicts during plan-review). Stray `verdict` anchors are
 *     common in this workspace's real text, not a theoretical edge case — a
 *     54-issue sample measured 34 comments carrying a `/verdict/i` anchor, 27
 *     of them resolving, and roughly a third of *those* quoting or restating
 *     an earlier verdict rather than declaring one (LIN-2043 N3). Extraction
 *     precision holds in practice because the `[windowStart, windowEnd)`
 *     bound scopes each row to its own dispatch cycle, not because stray
 *     anchors are rare — and that bound has its own known gap on an
 *     in-flight, last-pipeline-row R0 (`windowEnd` runs unbounded to `asOf`),
 *     tracked as the open wide-window escalation on LIN-1883.
 *     `diagnostics.verdictTier` and `crossTierDisagreements` remain the audit
 *     trail if a cross-anchor conflict surfaces in a live read.
 ...
 *     rows following at least one earlier `plan-review`-kind row, UNBOUNDED —
 *     a third or fourth round counts, exactly as the first does; LIN-1871
 *     dropped the earlier, refuted "0-vs-1" assumption from this header, see
 *     the PRIMARY METRIC section above) never feeds the primary numerator/
 *     denominator; it is reported beside the primary rate. The AGGREGATE `roundTrips` series
 ...
 *  7. `options.windowDays` (LIN-1871) is OFF by default — an un-windowed call
 *     reads every issue passed in, exactly as before this option existed.
 *     When set, the filter is issue-level (at least one qualifying pipeline
 *     row in the window), not row-level: an in-window `plan` row belonging to
 *     an issue whose gate row falls outside it still keeps that issue in (and
 *     vice versa) — the window decides which ISSUES are read, not which of an
 *     included issue's rows contribute to its own round-trip count.
 */
...
// LIN-2641: promoted to public exports (were module-private, reachable only via
// `__internal`) so `lib/effort-readout.js`'s population filter (step 3 of its
// join) can reuse these EXACT sets rather than a hand-copy — the two-status-
// vocabulary class of bug this module's own header warns about. `__internal`
// keeps its own reference to each (not a re-declaration) so `scripts/plan-
// review-round-trips.mjs` and every existing test stay byte-compatible.
export const NO_ATTEMPT_STATUSES = new Set(['cancelled', 'canceled', 'expired', 'aborted', 'failed']);
...
export const IN_FLIGHT_STATUSES = new Set(['queued', 'taken', 'blocked']);
...
function requireAsOf(asOf, fnName) {
  requireInstant(asOf, fnName, 'asOf');
}

/**
 * The shared body of the check above, so a second instant option validates with
 * byte-identical wording rather than a near-copy (LIN-2358). `requireAsOf` is
 * kept as its own named entry point because `asOf` is REQUIRED, while every
 * other instant on this surface is optional and must be guarded by the caller.
 *
 * @param {*} value
 * @param {string} fnName  the calling function's name, folded into the message
 * @param {string} paramName  the option's name, folded into the message
 */
function requireInstant(value, fnName, paramName) {
  // Accepts exactly what `toMs` accepts — a string or a Date — because
  // anything narrower rejects values that genuinely parse. An earlier revision
  // of LIN-2358 checked `typeof value !== 'string'`, which threw on a VALID
  // Date instance with the self-contradicting message "must be a parseable ISO
  // instant (got "2026-08-02T00:00:00.000Z")". Widening `asOf` to match is a
  // relaxation, never a new rejection, and is aligned with LIN-2037's intent:
  // fail loud on UNPARSEABLE input, not on a parseable one in another shape.
  const isInstantShape = typeof value === 'string' || value instanceof Date;
  if (!isInstantShape || !Number.isFinite(toMs(value))) {
    throw new Error(`plan-review-round-trips: ${fnName}: ${paramName} must be a parseable ISO instant (got ${JSON.stringify(value)})`);
  }
}
...
/** Structural round-trip count: `rePassKind` rows following an earlier `gateKind` row. */
function countRoundTrips(pipelineRows, gateKind, rePassKind) {
  let seenGate = false;
  let count = 0;
  for (const row of pipelineRows) {
    if (row.kind === gateKind) { seenGate = true; continue; }
    if (row.kind === rePassKind && seenGate) count++;
  }
  return count;
}
 ...
 */
function resolveAllTiers(row, comments, windowStart, windowEnd, nextRow, rePassKind) {
  ...
  // Tier C — structural: what kind of row followed this one. A `rePassKind`
  // row means a revision was requested; anything else means the pipeline
  // moved on. No next row ⇒ no structural signal either.
  const c = nextRow ? (nextRow.kind === rePassKind ? 'request changes' : 'approve') : null;
 ...
 */
function walkR0(pipelineRows, comments, asOf, gateKind, rePassKind) {
  const gateRows = pipelineRows.filter((r) => r.kind === gateKind);
  ...
  for (const row of gateRows) {
    ...
    const tiers = resolveAllTiers(row, comments, windowStart, windowEnd, nextRow, rePassKind);
 ...
 * @param {{asOf: string, gateKind?: string, rePassKind?: string}} options
 *   `asOf` — REQUIRED ISO instant, used as the fallback `windowEnd` for a
 *   `done` row with no next pipeline row. `gateKind`/`rePassKind` — the gate
 *   pair the walk is computed over, defaulting to `'plan-review'`/`'plan'`
 *   (LIN-2592) — every other input-contract caller (e.g. a future
 *   `{gateKind: 'review', rePassKind: 'implementation'}` walk) reuses this
 *   same canonical walk rather than a second implementation.
 * @returns {Object}
 */
export function computeIssueRoundTrips(issue, options = {}) {
  requireAsOf(options.asOf, 'computeIssueRoundTrips');
  const asOf = options.asOf;
  const gateKind = options.gateKind ?? 'plan-review';
  const rePassKind = options.rePassKind ?? 'plan';
  const pipelineRows = pipelineRowsOf(issue);
  const comments = Array.isArray(issue.comments) ? issue.comments : [];
  const walk = walkR0(pipelineRows, comments, asOf, gateKind, rePassKind);

  const hasRePassDispatch = pipelineRows.some((r) => r.kind === rePassKind);
  const description = typeof issue.description === 'string' ? issue.description : '';
  const gateDue = hasRePassDispatch && GATE_DUE_MARKER.test(description);
  const reachedPlanReviewAny = walk.R0 !== null;
  const gateHonoured = gateDue && reachedPlanReviewAny;

  const roundTrips = countRoundTrips(pipelineRows, gateKind, rePassKind);
 ...
 * @param {string} [options.gateKind]  the gate-pair's gate kind (LIN-2592),
 *   forwarded to `computeIssueRoundTrips` per issue; defaults to `'plan-review'`
 * @param {string} [options.rePassKind]  the gate-pair's re-pass kind (LIN-2592),
 *   forwarded to `computeIssueRoundTrips` per issue; defaults to `'plan'`
 * @param {string|Date} [options.rulerChangeAt]  optional ISO instant of a ruler
 ...
 *   Optional but NOT unvalidated (LIN-2358): a supplied value that does not
 *   parse throws, rather than emitting a fabricated `rulerContamination: 0`.
 *   Absent — or any falsy value — simply omits the field.
 ...
 * @param {number} [options.windowDays]  restrict the read to issues with at
 *   least one `gateKind` or `rePassKind` pipeline row dispatched within the
 *   last N days of `asOf` (LIN-1871). A CALENDAR window, not a row-count cap
 *   — deliberately unlike `routes/dashboard.js`'s `EFFORT_READOUT_HISTORY_LIMIT`
 *   (a flat 200-row slice of `/effort-readout`'s unrelated per-kind cost read),
 *   which gives no comparable boundary across two reads taken at different
 *   points in a workspace's activity: the same 200 rows can span an hour on a
 *   busy day or a month on a quiet one, so two such reads are not apples-to-
 *   apples. A calendar window is: `docs/papers/harbour/review-loops.md`'s own 30-day
 *   split is exactly this shape. Omitted (the default) reads every issue
 *   passed in, unchanged from before this option existed — existing callers
 *   and tests see no behavior change. An issue excluded by the window is
 *   excluded from every series this function returns, not just `roundTrips`.
 * @returns {Object}
 */
export function computePlanReviewRoundTrips(issues = [], options = {}) {
  ...
  if (windowDays != null && !(typeof windowDays === 'number' && Number.isFinite(windowDays) && windowDays > 0)) {
    throw new Error(`plan-review-round-trips: computePlanReviewRoundTrips: windowDays must be a positive finite number (got ${JSON.stringify(windowDays)})`);
  }
  // LIN-2358: validate, do not just parse. `toMs` returns NaN on unparseable
  // input, and because `NaN != null` is true the contamination branch below
  // still RAN and still emitted `rulerContamination` — but every `x < NaN`
  // comparison is false, so it always reported 0. A confident, well-formed
  // `rulerContamination: 0` that means "nothing measured" rather than "nothing
  // found", with the garbage echoed back verbatim in `window.rulerChangeAt`
  // and no signal anywhere that the input was never parsed.
  //
  // THROWS rather than degrading to absent, and the choice is a real one since
  // `rulerChangeAt` is optional where `asOf` is not. Throwing wins here because
  // there is no caller that passes this through unchecked: the only production
  // caller is scripts/plan-review-round-trips.mjs, which sends
  // `args.rulerChangeAt || undefined`, so a value reaches this line ONLY when
  // an operator explicitly typed `--ruler-change-at`. An unparseable one is
  // therefore unambiguously operator error, on an instrument whose whole job is
  // to be trusted — and `rulerContamination` is the one field where a false 0
  // most misleads, since it exists precisely to say whether a ruler change
  // polluted the window. Silently omitting the field would be honest about the
  // number but silent about the typo that caused it.
  if (options.rulerChangeAt) {
    requireInstant(options.rulerChangeAt, 'computePlanReviewRoundTrips', 'rulerChangeAt');
  }
  const rulerChangeAtMs = options.rulerChangeAt ? toMs(options.rulerChangeAt) : null;

  const asOfMs = toMs(asOf);
  const windowStartMs = windowDays != null ? asOfMs - windowDays * 24 * 60 * 60 * 1000 : null;
  const windowedIssues = windowStartMs == null
    ? (Array.isArray(issues) ? issues : [])
    : (Array.isArray(issues) ? issues : []).filter((issue) =>
        pipelineRowsOf(issue).some((r) =>
          (r.kind === gateKind || r.kind === rePassKind) &&
          Number.isFinite(toMs(r.dispatchedAt)) &&
          toMs(r.dispatchedAt) >= windowStartMs));
  ...
  for (const issue of windowedIssues) {
    const result = computeIssueRoundTrips(issue, { asOf, gateKind, rePassKind });
  ...
  return {
    ...
    window: { asOf, rulerChangeAt: options.rulerChangeAt || null, windowDays: windowDays ?? null },
    scale: {
      issuesRead: perIssue.length,
      reachedPlanReviewAny: reachedPlanReviewCount,
      issuesExcludedByWindow: windowStartMs == null ? 0 : (Array.isArray(issues) ? issues.length : 0) - windowedIssues.length,
    },
...
export const __internal = {
  ...
  // LIN-2358: exported so scripts/plan-review-round-trips.mjs can validate
  // `--ruler-change-at` at the argv boundary using THIS predicate rather than a
  // hand-copy. A copy already disagreed with it on non-string input; unreachable
  // from argv, but the point of a boundary check is that it cannot drift.
  requireInstant,
};
