 ...
 *     parameterised against a doubling-class relative effect (the plan's
 *     worked example: 19%→38%), α=0.05, power=0.80, mirroring `lib/follow-
 *     on-ratio.js`'s discipline of deriving the floor from the effect
 *     actually being measured, not that module's own count-based formula
 *     (`follow-on-ratio.js:133`, tuned to ITS pre-registered 50% rate-ratio
 *     effect — a floor for a COUNT, not this instrument's PROPORTION
 *     primary metric; reusing it verbatim was LIN-2035's B1 defect).
...
// doubling of the observed rate (19%→38%). p2 = min(p1·RATIO, 1-ε).
 ...
 * worked doubling-class effect (p2 = p1·`SUFFICIENCY_EFFECT_RATIO`) at
 ...
 *
 * The comparison proportion p2 is doubling-class (p1·RATIO) below p1=0.5 and
 * its mirror — halving the gap to 1 (`1-(1-p1)/RATIO`) — at or above it, so
 * the effect stays meaningful (non-zero, on the correct side of p1) all the
 * way to the p1=1 boundary. Only the doubling branch needs an upper clamp
 * (it touches exactly 1 at p1=0.5); the complement branch is algebraically
 * always in `(p1c, 1)` once p1c itself is bounded away from 1, so clamping
 * it too would collapse p2 back onto p1c right where the effect matters
 * most — exactly the divide-by-zero this function exists to avoid.
 *
...
export function derivePrimaryFloor(p1) {
  ...
  const p2 = p1c <= 0.5
    ? Math.min(p1c * SUFFICIENCY_EFFECT_RATIO, 1 - SUFFICIENCY_MIN_P)
    : 1 - (1 - p1c) / SUFFICIENCY_EFFECT_RATIO;
  const pBar = (p1c + p2) / 2;
  ...
  return {
    ...
    effectRatio: SUFFICIENCY_EFFECT_RATIO, zAlpha: SUFFICIENCY_Z_ALPHA, zBeta: SUFFICIENCY_Z_BETA,
  };
}
  ...
  return {
    ...
    definition: {
      ...
      sufficiencyFormula: floor
        ? `two-proportion power calc: N ≥ [z(α/2)·√(2p̄(1-p̄)) + z(β)·√(p1(1-p1)+p2(1-p2))]² / (p2-p1)² ≈ ${floor.requiredN}, numerator ≥ N·p1 ≈ ${floor.requiredNumerator} (p1 = ${p1.toFixed(4)} measured at read time, p2 = ${floor.p2.toFixed(4)} at a ${floor.effectRatio}× relative effect, α = 0.05, power = 0.80)`
