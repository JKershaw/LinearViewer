 ...
 * `cacheWrite1h` additions; 2026-08-30 for `openai/gpt-5.6-sol`, LIN-2384 — the
 * LIN-1763 figures were a copy-down transcription error, character-identical to
 * the `openai/gpt-5.5` row above it, that the same-commit "verified" claim did
 * not catch). OpenRouter prices change;
...
/**
 * @typedef {Object} ModelRateOverride
 * @property {number} minPromptTokens - Applies when total prompt tokens (inputTokens +
 *   cacheReadInputTokens + cacheCreationInputTokens) are STRICTLY GREATER than this
 *   (OpenRouter's own `min_prompt_tokens` gate — exclusive, not `>=`).
 * @property {number} [prompt] - USD per 1M uncached input tokens at this tier
 * @property {number} [completion] - USD per 1M output tokens at this tier
 * @property {number} [cacheRead] - USD per 1M cache-read input tokens at this tier
 * @property {number} [cacheWrite] - USD per 1M cache-write (5m) input tokens at this tier
 * @property {number} [cacheWrite1h] - USD per 1M cache-write (1h) input tokens at this tier
 *
 ...
 * @property {ReadonlyArray<ModelRateOverride>} [overrides] - Long-context tiers (LIN-2403),
 *   mirroring OpenRouter's `pricing.overrides` 1:1 in this table's own vocabulary
 *   (camelCase, USD/1M). Absent tier keys inherit the base row; when multiple entries
 *   apply, later entries win per key (OpenRouter's own merge rule — see
 *   {@link selectRateForPrompt}). A time-window-only entry (no `minPromptTokens`) is
 *   published upstream but deliberately unmodelled here — see selectRateForPrompt.
 */

/** Freeze the table and every rate row inside it, including a nested `overrides` array. */
function deepFreezeRates(table) {
  for (const rate of Object.values(table)) {
    if (Array.isArray(rate.overrides)) {
      for (const entry of rate.overrides) Object.freeze(entry);
      Object.freeze(rate.overrides);
    }
    Object.freeze(rate);
  }
...
export const MODEL_PRICING = deepFreezeRates({
  ...
  // openai/gpt-5.5, openai/gpt-5.5-pro and openai/gpt-5.6-sol carry a long-context
  // override tier (LIN-2403), verified against https://openrouter.ai/api/v1/models
  // on 2026-09-02: total prompt tokens strictly greater than 272,000 price at the
  // override row below instead of the base row above (merge over base — a key this
  // table's override omits inherits the base price; none do today).
  'openai/gpt-5.5': {
    prompt: 5.00, completion: 30.00, cacheRead: 0.50,
    overrides: [{ minPromptTokens: 272000, prompt: 10.00, completion: 45.00, cacheRead: 1.00 }],
  },
  'openai/gpt-5.5-pro': {
    prompt: 30.00, completion: 180.00,
    overrides: [{ minPromptTokens: 272000, prompt: 60.00, completion: 270.00 }],
  },
  ...
  'openai/gpt-5.6-sol': {
    prompt: 2.00, completion: 10.00, cacheRead: 0.20, cacheWrite: 2.50,
...
/**
 * Fold a rate row's applicable long-context override entries (LIN-2403) over the
 * base row, per OpenRouter's own documented rule: an entry applies when `promptTokens`
 * is STRICTLY GREATER than its `minPromptTokens`; applicable entries are folded in
 * array order, so a later entry's keys win over an earlier one's; a key an applicable
 * entry omits keeps inheriting the base row (or an earlier entry's value) rather than
 * being replaced. An entry with no finite `minPromptTokens` is a time-window-only
 * override (OpenRouter gates it on `utc_days`/`utc_start`/`utc_end` instead) and is
 * skipped here by construction — this table does not model time-of-day pricing.
 *
 * Pure and allocation-free on the common path: a rate with no `overrides`, or none of
 * whose entries apply, returns the SAME object reference (`===`), never a copy — so a
 * caller that doesn't need the merged row can't tell one was even considered.
 *
 * Exported test-facing (the established idiom here, see openrouter-catalog.js) because
 * `MODEL_PRICING` has no multi-entry row today, so "later entries win" and the
 * strict-`>` boundary can't otherwise be exercised without a synthetic rate.
 *
 * @param {ModelRate} rate
 * @param {number} promptTokens - inputTokens + cacheReadInputTokens + cacheCreationInputTokens
 * @returns {ModelRate} `rate` itself, or a merged plain object when an override applies
 */
export function selectRateForPrompt(rate, promptTokens) {
  if (!rate || !Array.isArray(rate.overrides) || rate.overrides.length === 0) return rate;
  let merged = rate;
  for (const entry of rate.overrides) {
    if (!entry || !Number.isFinite(entry.minPromptTokens)) continue; // time-window-only — skipped
    if (!(promptTokens > entry.minPromptTokens)) continue;
    if (merged === rate) merged = { ...rate };
    for (const [key, value] of Object.entries(entry)) {
      if (key === 'minPromptTokens') continue;
      merged[key] = value;
    }
  }
  return merged;
 ...
 *
 * **Long-context overrides (LIN-2403).** `promptTokens = inputTokens +
 * cacheReadInputTokens + cacheCreationInputTokens` (the WHOLE prompt, cached tokens
 * included, per OpenRouter's own `min_prompt_tokens` semantics). `min_prompt_tokens`
 * gates a single REQUEST's prompt size — meaningful for a per-request usage row
 * (`harness === 'opencode'`), but unrecoverable from a session-CUMULATIVE row
 * (`harness === 'claude-code'`, or absent/unknown — the same pairing
 * {@link reduceLineageCost} uses): summing many small requests can cross a threshold no
 * single request came near. So: an `opencode` row selects the applicable override rate
 * via {@link selectRateForPrompt} normally; a cumulative/unknown-harness row on an
 * override-bearing model whose `promptTokens` clears ANY of its thresholds returns
 * `null` rather than guess under- or over-priced — honest under this function's own
 * null contract, and a no-op today (no cumulative-harness row's resolved model carries
 * `overrides` yet).
 *
 * Never throws — it sits on the tolerant telemetry path.
 *
 * @param {{model?: string, harness?: string, inputTokens?: number, outputTokens?: number, cacheCreationInputTokens?: number, cacheCreation1hInputTokens?: number, cacheReadInputTokens?: number}} usage
...
export function computeUsageCostUsd(usage) {
  ...
  // The three fields that make up a request's total prompt size must be validated
  // up front — computeUsageCostUsd's own null contract already requires this of
  // each individually, but override selection (below) needs their sum before the
  // per-tier pricing loop would otherwise reach them.
  ...
  const promptTokens = (usage.inputTokens || 0) + (usage.cacheReadInputTokens || 0) + (usage.cacheCreationInputTokens || 0);

  let effectiveRate = rate;
  if (Array.isArray(rate.overrides) && rate.overrides.length > 0) {
    const crossesThreshold = rate.overrides.some(
      o => Number.isFinite(o.minPromptTokens) && promptTokens > o.minPromptTokens
    );
    if (crossesThreshold && usage.harness !== 'opencode') return null;
    effectiveRate = selectRateForPrompt(rate, promptTokens);
  }
  ...
  for (const [field, tier] of TOKEN_TIERS) {
    ...
    if (!Number.isFinite(effectiveRate[tier])) return null; // priced tier missing → unknown, not free
    perMillion += tokens * effectiveRate[tier];
    tokenTotal += tokens;
  }
  ...
  if (total === undefined || total === null) {
    ...
    if (total > 0) {
      ...
      if (capped1h > 0) {
        if (!Number.isFinite(effectiveRate.cacheWrite1h)) return null; // priced tier missing → unknown, not free
        perMillion += capped1h * effectiveRate.cacheWrite1h;
      }
      if (remainder5m > 0) {
        if (!Number.isFinite(effectiveRate.cacheWrite)) return null; // priced tier missing → unknown, not free
        perMillion += remainder5m * effectiveRate.cacheWrite;
      }
      tokenTotal += total;
    }
  }
