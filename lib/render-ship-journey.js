...
/**
 * The coverage figure is the primary on-page claim, and it must lead with
 * what it counts — completed tasks — not the run window it was read from
 * (LIN-2069: the waypoints already **are** tasks, so task-first is the
 * honest framing; the retained-run window is real provenance for that count,
 * not the headline). `listFull()` retains at most 20 runs per workspace
 * (ReportHistoryStore's `MAX_REPORTS_PER_WORKSPACE`), so that provenance
 * clause is scoped to "the last N retained runs", never presented as the
 * whole journey (LIN-1683 close-out ledger item 1, routed to this ticket).
 *
...
function formatCoverageFigure(coverage, capDropped, waypointCount) {
  ...
  if (completions === 0) {
    return `${waypointCount} task${waypointCount === 1 ? '' : 's'} charted; no completions recorded in the retained window, ${windowPhrase}.`;
  }
  const pct = Math.round((waypointCount / completions) * 100);
  return `${waypointCount} of ${completions} completed task${completions === 1 ? '' : 's'} charted — ${pct}% coverage, ${windowPhrase}.`;
}

/**
 * @param {Object} journey - deriveJourney() output: { waypoints, coverage, capDropped, starChanges, bearingHistogram }.
 *   `bearingHistogram` has no client consumer and is deliberately not embedded (LIN-2069).
