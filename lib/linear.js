 ...
 * In-progress items first, then todo, then terminal states (completed/canceled/duplicate share rank 3).
 */
const STATE_ORDER = { started: 0, unstarted: 1, backlog: 2, completed: 3, canceled: 4, duplicate: 4 }
...
export function selectFocusSubtask(children) {
  ...
  // 3. Fall back to first non-terminal
  return children.find(c => !isTerminalState(c.state?.type))
}
 ...
 */
export async function fetchRecommendationContext(apiKey, issueId, { signal } = {}) {
