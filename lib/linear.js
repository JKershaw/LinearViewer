 ...
 * Returns the issue details plus parent and sibling information.
 *
 * @param {string} apiKey - OAuth access token
 * @param {string} issueId - The issue ID to fetch
 * @returns {Promise<{issue: Object, parent: Object|null, siblings: Array}>}
 *   - issue: The issue with id, identifier, title, description, state
...
export async function fetchIssueContext(apiKey, issueId) {
  ...
  if (parent?.children?.nodes) {
    ...
    allSiblings.sort((a, b) => {
      const aOrder = stateOrder[a.state?.type] ?? 2
      const bOrder = stateOrder[b.state?.type] ?? 2
