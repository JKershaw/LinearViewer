...
export class AgentStatusStore {
   ...
   */
  async recordStatus({ urlKey, taskIdentifier, action, status, summary, tokenId, tokenLabel, dispatchId, timestamp }) {
    // `timestamp` is optional: honour a caller-supplied instant (e.g. backfills
    // and deterministic tests) when it parses, else stamp now. expiresAt is
    // always derived from the effective timestamp so the TTL stays consistent.
    const parsed = timestamp != null ? new Date(timestamp) : null;
   ...
   * @param {string} [options.dispatchId] - Filter to entries decorating this exact
   *   dispatch-history item (LIN-2934 D1). Independent of `taskIdentifier` — an
   *   identifier-less GENERAL autopilot anchor's own status rows report under
   *   whatever free-form `taskIdentifier` the worker chose (if any), never the
   *   anchor's own id, so `taskIdentifier` can't find them; `dispatchId` can.
   ...
   * @param {Date} [options.until] - Exclusive upper bound on `timestamp`
   *   (`$lt`, combinable with `since`'s `$gte` in one predicate). The
   *   live-console history cursor (LIN-1494): paging past a capped read only
   *   advances when the "older than" bound is pushed into the query — a JS
   *   post-filter over the same newest-N rows would re-read the same page
   *   forever. `total` counts the windowed set pre-slice, so callers can
   *   report honest truncation within the window.
   * @returns {Promise<{items: Array, total: number}>}
   */
  async listStatus(urlKey, { limit, offset = 0, tokenId, taskIdentifier, dispatchId, since, until } = {}) {
    if (!urlKey) {
    ...
    try {
      ...
      // since/until compose into ONE timestamp predicate ({ $gte, $lt }) —
      // supported by both real Mongo and MangoDB's query matcher.
      if (since || until) {
        query.timestamp = {};
        if (since) query.timestamp.$gte = since;
        if (until) query.timestamp.$lt = until;
      }
