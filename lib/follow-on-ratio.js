 ...
 * `partitionByDispatchTime`, `lib/transcript-spend.js:319-321`), as does a
 * window that cannot contain anything (`windowEnd <= windowStart`). Freeze-list
 ...
 *  7. **The plan marker is textual, not a schema, and is scoped to the
 *     DESCRIPTION ONLY.** `planScoped` keys off the `plan` template's own
 *     mandated wording (`lib/prompt-template-defs.js:242` — "fits one session"
 *     / "needs multiple sessions") plus an Implementation Plan heading, matched
 *     against `issue.description` and nothing else. The scoping is load-bearing,
 *     not tidiness: those same phrases appear in the INSTRUCTION TEXT of
 *     `plan-review` (`:782`) and `breakdown` (`:364`), and a `plan-review`
 *     verdict lands as a COMMENT on the issue. `plan-review` is the very gate
 *     LIN-1600 ships, so baseline-window issues never met it and re-read-window
 *     issues will — reading comments would let the marker appear in the after
 *     period for a reason unrelated to follow-on rework, inflating the
 *     `planScoped`/`primary` denominator and biasing the ratio DOWN, i.e. in the
 *     direction of the pre-registered hypothesis, on the designated primary
 *     instrument. That is differential misclassification aligned with the
 *     treatment: the one bias a falsifiable close cannot absorb. Recording the
 *     ruler (`definition.planMarker`) is necessary but not sufficient — a ruler
 *     that changes meaning between the two measurements still breaks
 *     comparability. It remains a heuristic: override the pattern via
 *     `options.planMarker` (any `/g` flag is stripped — see `nonGlobal`) and
 *     record which pattern a baseline used. Do NOT widen the surface back to
 *     comments without re-deriving this argument.
 ...
 *
 * MATCHED AGAINST THE DESCRIPTION ONLY, never comment bodies — see limit 7 and
 * `descriptionOf`. That scoping is part of the ruler, not an implementation
 * detail: widening it back to comments reintroduces the differential
 * misclassification the marker exists to avoid.
 */
 ...
 *
 * Matched across description AND comments, deliberately: `review` posts its
 * ledger as a COMMENT, so a description-only read here would measure nothing.
 * The asymmetry with the plan marker is the point — each marker reads the
 * surface its own template writes to.
 */
...
/**
 * The issue's OWN description — the narrow surface the plan marker reads.
 * `lib/prompt-template-defs.js:242` mandates the session-fit answer be written
 * into the description, so this is where a true positive lives; every other
 * surface is somebody else's prose. See limit 7.
 */
function descriptionOf(issue) {
  return issue && typeof issue.description === 'string' ? issue.description : '';
}

/**
 * A stateless copy of a caller-supplied pattern.
 *
 * `RegExp.prototype.test` is STATEFUL on a `/g` (or `/y`) regex: it advances
 * `lastIndex` on a hit and resumes from there on the next call, so testing the
 * same pattern against a list of sources alternates hit/miss and silently
 * UNDER-COUNTS. Both marker options are documented override points — the person
 * settling the plan-marker heuristic is exactly the person likely to hand in a
 * fresh `/gi` regex — so the flags are stripped rather than trusted. A copy,
 * never a `lastIndex` reset, because mutating the caller's own object is a side
 * effect a pure module has no business having. A pattern that is already
 * stateless is returned as-is, so the common path allocates nothing.
 */
function nonGlobal(pattern) {
  if (!(pattern instanceof RegExp)) return pattern;
  if (!pattern.global && !pattern.sticky) return pattern;
  return new RegExp(pattern.source, pattern.flags.replace(/[gy]/g, ''));
}

const hasPlanMarker = (issue, pattern = PLAN_MARKER) => nonGlobal(pattern).test(descriptionOf(issue));
const hasReviewLedger = (issue, pattern = REVIEW_LEDGER_MARKER) => nonGlobal(pattern).test(textOf(issue));
...
/**
 * Drop repeated issue rows on entry, first occurrence winning.
 *
 * `buildPeerIndex` already collapses duplicates (a Map keyed by id), but the
 * denominator loop walks the input list, so the same completion arriving twice
 * counted twice — one real task, `denominator: 2`. The plausible route is the
 * script's cursor paging over a collection that mutates mid-read, which is
 * precisely when the operator is least likely to notice. Rows with no usable id
 * cannot be compared, so they pass through untouched and are tolerated
 * downstream exactly as before.
 *
 * @returns {{issues: Array<Object>, duplicates: number}} `duplicates` is
 *   reported in `diagnostics.duplicateInputs` — a silent dedupe would hide a
 *   truncated or double-read population, which is a reason to distrust the run.
 */
function dedupeById(issues) {
  const seen = new Set();
  const out = [];
  let duplicates = 0;
  for (const issue of Array.isArray(issues) ? issues : []) {
    const id = issue && typeof issue.id === 'string' && issue.id ? issue.id : null;
    if (id) {
      if (seen.has(id)) {
        duplicates += 1;
        continue;
...
export function computeFollowOnRatio(issues = [], options = {}) {
  ...
  // A window that cannot contain anything is a caller error, not a measurement.
  // Both degenerate forms are refused together: inverted bounds and an empty
  // half-open window would each return `denominator: 0, ratio: null` — "no
  // data" — which reads as a finding rather than the typo it is.
  if (windowEndMs <= windowStartMs) {
    throw new Error(`follow-on-ratio: windowEnd must be after windowStart (got ${JSON.stringify(options.windowStart)} → ${JSON.stringify(options.windowEnd)})`);
  }

  const relationTypes = (options.relationTypes || COUNTED_RELATION_TYPES).map((t) => String(t).trim().toLowerCase());
  const maturityDays = Number.isFinite(options.maturityDays) ? options.maturityDays : MATURITY_DAYS;
  // Normalised ONCE, here, so `definition.*Marker` records the ruler actually
  // applied rather than the one handed in (see `nonGlobal`).
  const planMarker = nonGlobal(options.planMarker || PLAN_MARKER);
  const reviewLedgerMarker = nonGlobal(options.reviewLedgerMarker || REVIEW_LEDGER_MARKER);
  const floors = {
  ...
  const { issues: all, duplicates } = dedupeById(issues);
  ...
  for (const issue of all) {
    ...
    sources.push({
      ...
      unresolvedPeers,
      relationsSeen,
      // Through the shared predicates, so each marker's text surface is defined
      // in exactly one place (description-only vs description+comments).
      planned: hasPlanMarker(issue, planMarker),
      reviewed: hasReviewLedger(issue, reviewLedgerMarker),
  ...
  return {
    ...
    definition: {
      ...
      planMarkerScope: 'description',
      reviewLedgerMarker: String(reviewLedgerMarker),
      reviewLedgerMarkerScope: 'description+comments',
    },
    ...
    //    publication gate, just something a reader no longer has to infer from
    //    `diagnostics.skipped` alone.
    completeness: {
      attempted: issues.length + skippedCount,
      read: issues.length,
      skipped: skippedCount,
...
export const __internal = {
  ...
  toMs, requireInstant, rate, textOf, descriptionOf, nonGlobal, hasPlanMarker, hasReviewLedger,
  peerOf, buildPeerIndex, dedupeById, typeCounted, reading,
};
