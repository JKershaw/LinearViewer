...
export function sessionSpend(events, meta = {}) {
  ...
  // `totals.cacheCreation` reads raw JSONL `cache_creation_input_tokens` directly and is
  // tier-blind (LIN-2113): it sums 5m and 1h cache-write tokens together the same way the
  // pre-LIN-2113 `walkUsage` did, and this module never imports pricing (no
  // computeUsageCostUsd/MODEL_PRICING/getModelRate). Do not wire it into pricing without
  // also widening it to carry `cacheCreation1hInputTokens` the way session-telemetry.js does.
...
// ─── LIN-1241: before/after cohorts (the H3 falsifiable close) ────────────────

/**
 * Partition per-session results into before/after cohorts by dispatch time, for
 * the LIN-1241 before/after measurement (re-measure orientation-by-bytes once the
 * LIN-1240 grounding-suppression is live and compare against the LIN-1235
 * baseline). Pure so the segmentation is unit-testable independent of the proxy.
 *
 * A session with no parseable `dispatchedAt` is UNDATED and excluded from BOTH
 * cohorts (never silently folded into "before") — the delta is computed only over
 * datable sessions, and the undated count is surfaced so a large undated set is
 * visible rather than hidden.
 *
 * @param {Array<{dispatchedAt?: string|null}>} results  per-session results
 * @param {string} boundaryIso  ISO instant; dispatched AT or AFTER → after cohort
 * @returns {{before: Array, after: Array, undated: Array, boundaryMs: number}}
 */
export function partitionByDispatchTime(results, boundaryIso) {
  const boundaryMs = new Date(boundaryIso).getTime();
  if (!Number.isFinite(boundaryMs)) throw new Error(`invalid boundary: ${boundaryIso}`);
  const before = [], after = [], undated = [];
  for (const r of results || []) {
    const t = r && r.dispatchedAt ? new Date(r.dispatchedAt).getTime() : NaN;
    if (!Number.isFinite(t)) undated.push(r);
    else if (t >= boundaryMs) after.push(r);
    else before.push(r);
  }
  return { before, after, undated, boundaryMs };
}
