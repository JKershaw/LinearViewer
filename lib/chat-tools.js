...
export function createChatToolCatalog({
  ...
  const executors = {
    ...
    send_follow_up: async ({ sessionId, prompt } = {}) => {
      ...
      // kind defaults to 'custom'; this path carries no proxy context, so it passes
      // a plain prompt and no finalizePrompt (no bootstrapToken field).
      ...
      // this follow-up path's prior behavior.
