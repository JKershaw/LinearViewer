...
// lib/flight-companion-turn.js's COMPANION_INSTANCE_PREFIX).
 ...
 * the turn's terminal frame (see `lib/flight-companion-turn.js`'s
 ...
 * DATA ONLY: this changes no tool's behavior, and `createChatToolCatalog`
 * still resolves every tool against the same single `provider`/`scope` pair
 * it always has — making the tier legible here is what lets LIN-2967 turn
 * that single pair into a scope per tier as a reviewable, isolated diff.
 *
 ...
 *     `search_tasks` calls `provider.search` with the SAME captured `scope`
 *     rather than a workspace-wide read. None of the eight accepts a
 *     workspace or credential argument from the model.
 ...
 *     before ever reaching GitHub. Per LIN-2085/LIN-2967 this tier needs the
...
export function createChatToolCatalog({
  ...
  const executors = {
    ...
    get_pr_status: async ({ repo, number, sha } = {}) => {
      ...
      const allowlist = await resolveRepoAllowlist(provider, scope);
