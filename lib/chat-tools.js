...
//     break is opt-in per call site, not an accident of this surface.
//
...
export function createChatToolCatalog({
  ...
  const executors = {
    ...
    // directly, no new transport. `followUpTo`/`force` are derived from the TAIL
    // of the anchor loop's own lineage (`anchorLineageTail`), never the session's
    // aggregate/root; `target` still comes straight from the anchor loop itself —
