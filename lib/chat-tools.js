 ...
 * @param {string} [args.dispatchedBy] - The requesting user's id (e.g.
 *   `req.session.linearUserId`), forwarded to `dispatchQueueStore.addItem` as
...
export function createChatToolCatalog({
  ...
  const executors = {
    ...
    send_follow_up: async ({ sessionId, prompt } = {}) => {
      ...
      const force = sessionIsTerminal(session);
      // Reuse the shared dispatch factory (LIN-1139) so a tool-driven follow-up
      // resolves model/harness like every other dispatch path — inheriting the
      // workspace dispatch defaults and the default harness (LIN-1159). kind
      // defaults to 'custom'; this path carries no proxy context, so it passes a
      // plain prompt and no finalizePrompt (no bootstrapToken field).
