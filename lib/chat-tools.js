...
// (`lib/observer-sweep.js:150`): the superseded set over the whole workspace
  ...
  },
  {
    ...
    function: {
      ...
      description:
        ...
        "right now?\", \"what is stalled?\" or \"what is waiting on me?\" with actual session " +
        "ids and task identifiers rather than census counts. Terminal sessions are excluded " +
        "unless you ask for lane 'all'. Then drill into one with get_session.",
      parameters: {
        type: 'object',
        properties: {
          limit: {
          ...
          lane: {
            type: 'string',
            description:
              "Filter to one lifecycle lane. 'working' is actively moving, 'silent' has not " +
              "reported for over an hour, 'blocked' is parked waiting on a human (alive, not " +
              "dead), 'queued' has not started. 'all' also includes finished sessions, which " +
              "are otherwise omitted. Omit for every non-terminal session.",
            enum: ['working', 'silent', 'blocked', 'queued', 'all'],
          },
        },
        required: [],
      },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        ...
        "offered, the recommended one, which session and task raised it, how long it has been " +
        "parked, and whether it can still be replied to. Use this to answer \"what needs me?\" " +
        "or \"what is parked?\". Read-only: this never answers or dismisses a decision.",
      parameters: {
        type: 'object',
        properties: {
          limit: {
...
export const CHAT_TOOL_RESULT_BUDGETS = {
  ...
  list_active_sessions: 12000,
  list_pending_decisions: 12000,
};
...
const LIST_ACTIVE_LANES = new Set(['working', 'silent', 'blocked', 'queued', 'all']);
...
// dead (lib/observer-sweep.js's own header, lib/dispatch-terminal.js:103-106).
 ...
 * build-time derivations `_buildLoops` always sets (lib/pipeline-loops.js:741),
...
export function projectActiveSession(session, { superseded, now, staleMs } = {}) {
  ...
  // The session that SPAWNED this one, when a loop names an orchestrator other
  // than this session's own id (a lane child dispatched by a coordinator).
...
export function createChatToolCatalog({
  ...
  // (routes/dashboard.js:1436-1445, routes/proxy-rulings.js:119-123); without
  ...
  const executors = {
    ...
    list_active_sessions: async ({ limit, lane } = {}) => {
      ...
      // sweep does (lib/observer-sweep.js:150) — a per-session set would miss a
      ...
      for (const session of sessions) {
        const loops = session.loops || [];
        wakeLoopsFolded += loops.filter(l => l?.kind === 'wake').length;
        ...
        // becomes its own row. Wake loops inside a real session are already
        // folded by construction — that session emits one row regardless.
        if (loops.length && loops.every(l => l?.kind === 'wake')) continue;
        live.push(projectActiveSession(session, { superseded, now, staleMs: DEFAULT_LANE_STALE_MS }));
      }

      const terminalOmitted = live.filter(r => FINISHED_LANES.has(r.lifecycle)).length;
      const selected = lane && lane !== 'all'
        ? live.filter(r => r.lifecycle === lane)
        : lane === 'all' ? live : live.filter(r => !FINISHED_LANES.has(r.lifecycle));

      selected.sort((a, b) => String(b.lastActivityAt || '').localeCompare(String(a.lastActivityAt || '')));
      return {
        ...
        // So the model can say "37 wake rows are re-wakes, not work" instead of
        // silently under-reporting the fleet.
    ...
    list_pending_decisions: async ({ limit } = {}) => {
      ...
      // excludes `dash`/`local` targets (lib/pipeline-loops.js:1233), so
      ...
      // (urlKey, decisionId), so an untagged loop's shelved decision would
      // resurface (routes/proxy-rulings.js:105).
      ...
      const ordered = collected
        ...
        .sort((a, b) => (a.sinceMs ?? Infinity) - (b.sinceMs ?? Infinity));
      return {
        count: ordered.length,
        truncated: ordered.length > rows,
        decisions: ordered.slice(0, rows).map(({ row, sinceMs }) => projectPendingDecision(row, sinceMs)),
      };
    },
