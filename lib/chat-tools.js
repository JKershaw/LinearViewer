 ...
 *     (`:1624` above); `get_pr_status` calls it indirectly through
 *     `resolveRepoAllowlist` (`:1100-1102`) to build its repo allowlist
 ...
 * executor (`:1990` above) touches no store and no provider call at all — it
 * only forwards the validated playbook string to the injected `onRemember`
 * callback. There is no scope here for a future per-tier factory to resolve.
