...
export const CHAT_TOOL_RESULT_BUDGETS = {
  ...
  // Derived from the row caps rather than guessed, and pinned by a test that
  // serializes a full-cap payload: 20 rows x (300-char question + 4 x 60-char
  // option labels + ~250 of ids, stamps and flags) ~= 17k worst case.
...
/**
 * Map every loop to the session it belongs to, using the SAME precedence
 * `_buildSessions` groups by (`lib/pipeline-loops.js`'s pass 1/2/2.5/3 and
 * dispatch-store.js's own schema comment): the durable `sessionGroupId` when
 * stamped (LIN-1341), else the orchestrator id stamped as `sessionId`
 * (LIN-591), else the loop's own id — which is what a standalone or anchor
 * loop's session is keyed by.
 *
 * The one case this degrades on, named rather than hidden: a PRE-LIN-1341
 * follow-up loop carries neither field, so it resolves to its own id while
 * `_buildSessions` would walk its `followUpTo` chain to the root. That yields a
 * `sessionId` `get_session` cannot resolve for a historical follow-up decision.
 * The honest alternative is a second full session reconstruction per call,
 * which is the cost this whole tool is built to avoid.
 *
...
function buildSessionIdByLoopId(loops) {
  ...
  for (const loop of loops) {
    ...
    map.set(loop.loopId, loop.sessionGroupId || loop.sessionId || loop.loopId);
  }
  return map;
}
