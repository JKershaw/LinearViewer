...
// LIN-1486: loop-local mirror of routes/dashboard.js's `loopIsTerminal`
// (isTerminalLoop + the marker fallback in enrichLoop/effectiveAgentState).
// Those are unexported, and this module must NOT import from routes/
// (send_follow_up stays inside lib/chat-tools.js — see the ticket's binding
// constraint against touching routes/dashboard.js), so the two-clause rule is
// replicated here instead. BOTH clauses matter: `agentState` alone misses a
// `[aborted]`/`[skipped]` marker on an otherwise still-"taken" loop, and
// `terminalStatus` alone misses an expired/cancelled loop, which is terminal
// via `agentState` with no feedback marker at all.
...
export function createChatToolCatalog({
  ...
  const executors = {
    ...
    // per-run transcript. Sessions key by sessionId (a UUID), not issueId, so
    ...
    get_session: async ({ sessionId } = {}) => {
      ...
      return {
        ...
        runs: (session.loops || []).map(loop => ({
          ...
          transcript: (loop.feedback || []).map(f => ({
