...
export const CHAT_TOOL_RESULT_BUDGETS = {
  ...
  //              neither field can be trimmed to close it. `issueId` is
  //              mandatory on EVERY row, not just task-bound ones — since
  //              LIN-2704 it is also the primary reply-comment target
  //              (`decision.issueId || decision.issueIdentifier`, replacing
  //              the old issueIdentifier-only target), and real ids are
  //              already short (a UUID is ~36 chars, well under
  //              DECISION_ID_MAX's 200), so the added bytes are unavoidable
  //              key+value overhead, not truncatable content. Omitting the
  //              `taskDecisionId` key when null (the common loop-anchored
  //              case, where it structurally carries no information) would
  //              recover only ~440 of the 783 bytes — still over. 20000
  //              preserves roughly the original headroom-to-total ratio
  //              (18783 x ~1.065 ~= 20000, vs. the original's ~1.055) rather
  //              than being padded arbitrarily. No other tool result shares
  //              this budget key — `projectPendingDecision` has exactly one
