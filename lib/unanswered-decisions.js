 ...
 * @returns {Array<{decision: Object, decisionCase: Array<string>, anchor: Object, stampLoopId: string|null, disposition: string, canReply: boolean, shelvedLapseCount: number, effect: string|null, declaredEffect: string|null, alternate: string|null}>}
 */
export function collectUnansweredDecisions(
  ...
  for (const { lineageId, decisionId, members } of groups.values()) {
    ...
    if (answeredByLineage.get(lineageId)?.has(decisionId)) continue;
    ...
    if (superseded.has(contentLoop.loopId)) continue;
