...
export function collectUnansweredDecisions({ loops = [], taskDecisions = [], shelvedRulings = [] } = {}, { now } = {}) {
  ...
  const shelfByDecisionId = new Map();
  for (const shelf of shelvedRulings) {
    if (shelf?.decisionId) shelfByDecisionId.set(shelf.decisionId, shelf);
  }
  // Returns null when actively shelved (caller must exclude the row), else
  // the lapse count to attach (0 when never shelved).
  function shelfGate(decisionId) {
    const shelf = shelfByDecisionId.get(decisionId);
    if (!shelf) return 0;
  ...
  for (const loop of loops) {
    ...
    const shelvedLapseCount = shelfGate(loop.decision.decision_id);
  ...
  for (const entry of latestByTask.values()) {
    ...
    const shelvedLapseCount = shelfGate(entry.decision.decision_id);
    if (shelvedLapseCount === null) continue; // actively shelved
