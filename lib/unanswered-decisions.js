...
/**
 * Item-scoped withdrawal predicate (LIN-2891/LIN-3036, Choice C). Pure equality
 * against the loop's OWN `withdrawal` field (derived by `lib/digest-feedback.js`
 * and carried on the loop shape by `lib/pipeline-loops.js`) — deliberately NOT
 * a lineage-wide map, unlike `answeredDecisionIdsByLineage` above. A withdrawal
 * stamp on loop X can only ever discharge X's own row; it can never make a
 * sibling loop raising the same decision read as withdrawn. The predicate
 * requires a real `decision.decision_id` and a matching `withdrawal.decisionId`,
 * so a malformed/mismatched withdrawal is never a discharge.
 *
 * @param {{decision?: Object|null, withdrawal?: {decisionId: string}|null}|null|undefined} loop
 * @returns {boolean}
 */
export function isDecisionWithdrawn(loop) {
  return Boolean(loop?.decision?.decision_id) &&
    loop?.withdrawal?.decisionId === loop.decision.decision_id;
}
 ...
 * LOOP-BACKED group that is resolved (discharged by the lineage-wide
 * answered map, or withdrawn on its own content loop, LIN-2891) is not
 ...
 * An answered group is also exempted from two gates meant for open rulings
 * (LIN-2991 corrective fixes) — the content-loop supersession skip and the
 * shelf gate below. A root-raised decision answered on the root and later
 * superseded by an unrelated reply (the live LIN-2985 shape), or one that is
 * still under an active shelf when it gets answered, would otherwise vanish
 * from `includeResolved` too, hiding the very row an agent needs to see to
 * avoid re-raising it. The default (unanswered) read is untouched by either
 * exemption: each only ever fires for a group `includeResolved` already
 * chose to keep.
 *
 * `resolution.outcome` is `'answered'`, `'dismissed'`, or `'withdrawn'`
 * (Step 3, LIN-2891), or `null` for a row synthesized from a
 ...
 * assume `resolution.outcome` is always a non-null string — treat `null` as
 * "resolved (answered or dismissed), outcome unknown", not as a
 * missing/invalid row.
 *
 * A `'withdrawn'` row is the one outcome that also carries `reason` — the
 * withdrawing agent's stated reason, read off the content loop's own
 * `withdrawal` (the write route fails closed on an empty reason, and
 * `_findDecisionWithdrawal` skips a reason-less stamp, so it is never `null`
 * on such a row). Its `resolvedAt` is the withdrawal's own timestamp and its
 * `raisedAt` is `null`, since a withdrawal stamp records no raise. The
 * `'answered'`/`'dismissed'` rows from `answeredDecisions` never carry a
 * `reason`. When a decision is both answered (lineage-wide) and withdrawn
 * (on its content loop), `'answered'` wins — the row reports the answer.
 *
 ...
 * @returns {Array<{decision: Object, decisionCase: Array<string>, anchor: Object, stampLoopId: string|null, disposition: string, canReply: boolean, shelvedLapseCount: number|null, effect: string|null, declaredEffect: string|null, alternate: string|null, resolution?: {decisionId: string, raisedAt: string|null, resolvedAt: string|null, outcome: 'answered'|'dismissed'|'withdrawn'|null, reason?: string|null}|null}>}
 */
export function collectUnansweredDecisions(
  ...
  for (const { lineageId, decisionId, members } of groups.values()) {
    ...
    // withdrawn: a second, ITEM-SCOPED resolution state (LIN-2891/LIN-3036,
    // Choice C) consulted ALONGSIDE the lineage-wide `answered` map above,
    // never merged into it. `isDecisionWithdrawn` reads the content loop's own
    // `withdrawal` field only, so a withdrawal on a sibling member cannot
    // discharge this group. Tie-break: when a decision is both answered
    // (anywhere in the lineage) and withdrawn (on the content loop), `answered`
    // wins — `resolution` below reports the answer, never the withdrawal.
    const withdrawn = isDecisionWithdrawn(contentLoop);
    const resolved = answered || withdrawn;
    ...
    if (!resolved && superseded.has(contentLoop.loopId)) continue;
    ...
    //
    // LIN-2991 F2: same idiom as the supersession skip above — this gate
    // exists to hide an OPEN ruling that an operator has parked, not to hide
    // an already-answered one from `includeResolved`. An answered group only
    // reaches this line when `includeResolved` kept it past the discharge
    // check, so exempting it here narrows nothing for the default caller,
    // which never sees an answered group at all (`:506`). `shelvedLapseCount`
    // reads `null` on such a row — the row carries its own `resolution`, so a
    // caller reading `includeResolved` output already knows it's answered;
    // `null` here means "currently under an active shelf" rather than "N
    // lapses so far", the same distinction shelfGate always drew.
    const shelvedLapseCount = shelfGate(anchorLoop.workspaceUrlKey, anchorLoop.loopId, decisionId);
    if (shelvedLapseCount === null && !resolved) continue; // actively shelved, and still open
    ...
    rows.push({
      ...
      ...(resolved ? {
        resolution: answered
          ...
          : { decisionId, raisedAt: null, resolvedAt: contentLoop.withdrawal?.timestamp ?? null, outcome: 'withdrawn', reason: contentLoop.withdrawal?.reason ?? null }
      } : {})
    });
  }
