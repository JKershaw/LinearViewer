...
import { basisChanged as computeBasisChanged } from './scan-fingerprint.js';
 ...
 *
 * A FOURTH input, `taskBasis` (LIN-2241 tier 1) — `{urlKey, issueId, basisHash,
 * observedAt}` entries describing the freshest content the caller could observe
 * for a task WITHOUT spending a provider or model call (in practice the latest
 * `lib/task-snapshot-store.js` snapshot, captured opportunistically at the proxy
 * read seam). Each task-bound row gains a tri-state `basisChanged`: `true` when
 * the ruling's recorded basis and the observed content disagree, `false` when
 * they agree, and `null` when the question cannot be answered — no recorded
 * basis (a row raised before LIN-2241), no observation, or an observation not
 * strictly newer than the scan. `null` is never rendered as either answer; see
 * `basisChanged` in lib/scan-fingerprint.js for why unknown stays unknown.
 * Loop-backed rows always carry `basisChanged: null` — they are not raised from
 * a task scan and have no basis to compare — so the row shape stays uniform.
 *
 * @param {{loops?: Array<Object>, taskDecisions?: Array<Object>, shelvedRulings?: Array<Object>, taskBasis?: Array<Object>}} input
 * @param {{now: Date}} opts
 * @returns {Array<{decision: Object, decisionCase: Array<string>, anchor: Object, disposition: string, canReply: boolean, shelvedLapseCount: number, basisChanged: boolean|null}>}
 */
export function collectUnansweredDecisions({ loops = [], taskDecisions = [], shelvedRulings = [], taskBasis = [] } = {}, { now } = {}) {
  const effectiveNow = now instanceof Date ? now : new Date();
  const superseded = computeSupersededLoopIds(loops);

  // LIN-2241 tier 1: freshest KNOWN content per task, keyed the same way the
  // task-decision rows are ((urlKey, canonical issueId) — never the human
  // identifier, which a rename moves). Entries the caller could not resolve
  // are simply absent, which resolves to `basisChanged: null` (unknown) below.
  const basisByTask = new Map();
  for (const entry of taskBasis) {
    if (!entry?.urlKey || !entry?.issueId || !entry?.basisHash) continue;
    basisByTask.set(`${entry.urlKey}::${entry.issueId}`, entry);
  }
  ...
  for (const loop of loops) {
    ...
    rows.push({
      ...
      shelvedLapseCount,
      // Uniform row shape: a loop-backed decision is not raised from a task
      // scan and carries no basis to compare, so it is UNKNOWN rather than
      // absent — the client never has to distinguish `undefined` from `false`.
      basisChanged: null
    });
  }
  ...
  for (const entry of latestByTask.values()) {
    ...
    const observed = basisByTask.get(`${entry.urlKey || ''}::${entry.issueId || ''}`);
    rows.push({
      ...
      shelvedLapseCount,
      basisChanged: computeBasisChanged({
        raisedBasisHash: entry.basisHash,
        currentBasisHash: observed?.basisHash,
        // 0 is taskDecisionScannedAtMs's unparseable sentinel, not a real
        // epoch — passing it through would make every observation look newer.
        raisedAtMs: taskDecisionScannedAtMs(entry) || null,
        observedAtMs: observed ? new Date(observed.observedAt).getTime() : null
      })
    });
  }
