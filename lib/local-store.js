...
export class LocalStore {
  ...
  /** List all issues in `scope`. */
  async listIssues(scope) {
    return this.collection.find({ scope, kind: 'issue' }).toArray();
  }
  ...
  async addComment(scope, id, body) {
    ...
    await this.collection.updateOne({ _id: doc._id }, { $set: { comments } });
    return comment;
  }
  ...
  async addLabel(scope, id, label) {
    ...
    await this.collection.updateOne({ _id: doc._id }, { $set: { labels: [...labels, label] } });
    return true;
  }
  ...
  async removeLabel(scope, id, label) {
    ...
    await this.collection.updateOne({ _id: doc._id }, { $set: { labels } });
    return true;
  }
  ...
  async addRelation(scope, id, { type, relatedIssueId } = {}) {
    ...
    await this.collection.updateOne({ _id: doc._id }, { $set: { relations } });
    return relation;
  }
