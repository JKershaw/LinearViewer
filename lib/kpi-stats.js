 ...
 *
 * LIN-1957 adds two further aggregate-only output categories, both subject
 * to the same boundary: nullable USD dollar figures (the terminal-marked
 * task cost and its cash/unknown-lane split — never a per-workspace or
 * per-issue breakdown, always an instance-wide sum) and ratio bias/coverage
 * shares (0-1, rounded to 3dp). `issueIdentifier` is an internal grouping
 * key used only to attribute a lineage's cost to its issue for the
 * issue-level denominator — it is never itself emitted.
 *
...
import { computeTerminalMarkedTaskCost } from './terminal-marked-task-cost.js';
import { computeWeeklyBudgetGauge } from './weekly-budget.js';
import { parseTicketMarkers, __internal as SESSION_TELEMETRY_INTERNAL } from './session-telemetry.js';
import { MAX_REPORTS_PER_WORKSPACE } from './report-history-store.js';
...
/**
 * Count of distinct string values of a field, ignoring blank/non-string
 * values — the same population `topCounts` ranks, just sized rather than
 * sliced.
 */
function countDistinctValues(docs, field) {
...
/**
 * How many distinct values a top-`limit` cut silently drops — the shared
 * "excluded distinct values" math for `dispatchKinds`/`topEndpoints`
 * (mirroring `dispatchByDay`'s existing `other` bucket), factored once so
 * both call sites can't drift.
 */
function excludedDistinctCount(distinctCount, limit = TOP_LIST_LIMIT) {
  return Math.max(0, distinctCount - limit);
}
...
async function loadProxyBins(proxyEvents) {
  if (typeof proxyEvents.aggregate === 'function') {
    try {
      ...
      // LIN-3002: propagate, never fall through to the unbounded find path —
      // that fallback was the ~13.3 MB unbounded read this aggregation exists
      // to avoid, and it hid the failure from /kpis' own warm-stale-serve/
      // cold-500 handling (server.js), which already does the right thing
      // once it actually sees the rejection.
      console.error('kpi-stats: proxy-events aggregation failed:', err?.message || err);
      throw err;
    }
  }
...
// LIN-2996 Phase 4 (LIN-3012): true when a row's `feedbackDigest` is fresh —
// present, an object, and its `.version` matches the row's own
// `feedbackVersion` — so loadDispatchHistory can read the four precomputed
// `kpi*` fields instead of re-deriving them from `feedback` on every refresh.
// Both sides go through `$ifNull` so "missing" and "null" normalize to the
// SAME sentinel and can never accidentally compare equal to each other
// (missing digest.version -> -1, missing feedbackVersion -> 0; a legacy row
// with neither field is therefore never mistaken for version-0 fresh).
// Must stay semantically identical to Phase 3's JS `isFreshDigest`
// (LIN-3011, `v(doc) = feedbackVersion ?? 0`) — a Mongo `$cond` can't
// literally share code with a JS predicate, so this is a second, independent
// expression of the same semantics. Reconciliation (ledger L3, carried from
// the LIN-3012 review to LIN-3011 as whichever of Phase 3/Phase 4 landed
// second) is discharged: `tests/unit/mongo-smoke.test.js`'s
// "LIN-3011 ledger L3" parity test runs this exact expression (via
// `__internal.FRESH_DIGEST` below) and `isFreshDigest` side by side, on real
// `mongod`, over 7 row shapes (fresh/legacy/seeded-empty/stale/digest-
// without-version/null-feedbackVersion/non-object digest) and asserts
 ...
 *
 * LIN-2996 Phase 4 (LIN-3012): each of these five derived fields is wrapped
 * in `$cond: [FRESH_DIGEST, <digest field>, <this same per-row expression>]`
 * — a fresh row reads the precomputed `feedbackDigest.kpi*` field the Phase 1
 * writers maintain; every other row (legacy, seeded-empty, stale, or a
 * digest missing its own `.version`) falls through to exactly the expression
 * below, unchanged. This never re-derives feedback in the DB for a row whose
 * digest is already known-good, without ever reintroducing a whole-collection
 * fallback for the rows that still need re-deriving.
 *
...
export async function loadDispatchHistory(dispatchHistory) {
  if (typeof dispatchHistory.aggregate === 'function') {
    try {
      return await dispatchHistory.aggregate([
        {
          $project: {
            ...
            harness: 1,
            feedbackCount: {
              $cond: [FRESH_DIGEST, '$feedbackDigest.count', { $size: { $ifNull: ['$feedback', []] } }]
            },
            terminalEntry: {
              $cond: [
                FRESH_DIGEST,
                '$feedbackDigest.kpiTerminalEntry',
                {
                  $last: {
                    $filter: {
                      input: {
            ...
            evidenceCount: {
              $cond: [
                FRESH_DIGEST,
                '$feedbackDigest.kpiEvidenceCount',
                {
                  $size: {
                    $filter: {
                      input: {
            ...
            // LIN-2253: every `[ticket] LIN-XXXX <state>[ — <outcome>]`
            // feedback entry (LIN-2242/LIN-2243's worker-lane convention),
            // kept as raw {message,timestamp} — same shape terminalEntry/
            // usageEntry already carry — for parseTicketMarkers to walk
            // downstream. Unlike those two, this can be MORE than one entry
            // per lineage (a lane marks several tickets), so it is a
            // $filter, never narrowed with $last.
            ticketMarkerEntries: {
              $cond: [
                FRESH_DIGEST,
                '$feedbackDigest.kpiTicketMarkerEntries',
                {
                  $filter: {
                    input: {
                      $map: {
      ...
      // LIN-3002: propagate, never fall through to the unbounded find path —
      // that fallback was the ~67 MB unbounded read the projection above
      // exists to prevent, and it hid the failure from /kpis' own
      // warm-stale-serve/cold-500 handling (server.js), which already does
      // the right thing once it actually sees the rejection.
      console.warn('kpi-stats: dispatch-history aggregation failed:', err?.message || err);
      throw err;
    }
  }
...
/**
 * A dispatch's raw `[ticket] ...` marker feedback entries, in either shape
 * (LIN-2253) — the exact sibling of `evidenceCountOf`/`usageOf` above. The
 * find path (mock, and the aggregation fallback) hands back the WHOLE raw
 * feedback array — safe because `parseTicketMarkers` already ignores every
 * non-matching entry itself, so passing the unfiltered array is equivalent
 * to pre-filtering it. The aggregation path carries the pre-filtered
 * `ticketMarkerEntries` the $project above computes server-side.
 */
export function ticketMarkerEntriesOf(doc) {
  if (Array.isArray(doc.feedback)) return doc.feedback;
  return Array.isArray(doc.ticketMarkerEntries) ? doc.ticketMarkerEntries : [];
}
 ...
 *   sorted by the contributing row's OWN `dispatchedAt` (LIN-1957 review F3 —
 *   the input is an unsorted concatenation of an unsorted aggregate and an
 *   unsorted find, so array/iteration order is not a fact about the data and
 *   must not change the result), rows with none omitted. Beat 4's
 *   harness-conditional reduce (opencode sums, claude-code/unknown last-wins
 *   over the LAST entry) and cost parsing (`parseUsage([entry])`, decision
 *   (b)) consume this sorted form — parsing deliberately stays out of this
 *   extraction, which moves existing behavior rather than adding new
 *   business logic.
 * - `issueIdentifier` — LIN-1957 review F2: decoupled from `harness`, NOT the
 *   same earliest row. Captured from the EARLIEST row that actually CARRIES
 *   one (by `dispatchedAt`, same non-order-dependent comparison `earliest`
 *   and `harness` use — a null-identifier earliest row must not blank out a
 *   later row's identifier). For beat 4's issue-level (not lineage-level)
 *   denominator. Internal grouping key only; must never reach
 *   `computeDispatchOutcomes`'s or `collectKpiStats`'s output. LIN-2121 L1:
 *   rows with `kind: 'wake'` are EXCLUDED from this capture — a wake may
 *   inherit a lineage's attribution but never found it.
 * - `ticketMarkers` (LIN-2253) — every `[ticket] LIN-XXXX <state>[ —
 *   <outcome>]` marker posted anywhere across the lineage's rows, parsed via
 *   `parseTicketMarkers` (one entry per identifier, latest state wins,
 *   first-seen order preserved — same rules that module documents). A
 *   worker-lane session's own `issueIdentifier` covers only the dispatch's
 *   anchor ticket; this is what lets a cost/coverage consumer see the OTHER
 *   tickets the same lane landed, which have no lineage of their own at all.
 *   `parseTicketMarkers` is last-ARRAY-POSITION-wins, not timestamp-aware —
 *   correctness depends on its input already being chronological. Raw marker
 *   entries accumulate per row during the main pass below, GROUPED by each
 *   contributing row's own `dispatchedAt`, then sorted by that same key at
 *   finalize time before being flattened and parsed — the exact F3 discipline
 *   `rowUsage` already follows, applied here for the same reason (`rows` is
 *   an unsorted concatenation of an unsorted aggregate and an unsorted find,
 *   so push/iteration order is not a fact about the data).
 *
 * @param {Array<Object>} rows - history + queue dispatch rows, either shape
 * @returns {Map<string, {earliest: number|null, status: string|null, statusMs: number|null, harness: string|null, rowUsage: Array<Object>, issueIdentifier: string|null, ticketMarkers: Array<Object>}>}
 */
export function groupDispatchLineages(rows) {
  ...
  for (const { row, feedback } of normalized) {
    ...
    if (!lineage) {
      lineage = {
        ...
        harness: harnessOf(row), rowUsage: [], issueIdentifier: null, _issueIdentifierAt: null,
        _ticketMarkerRaw: []
      };
      ...
      // The row that becomes the new earliest re-captures harness too, so
      // "earliest row only" holds regardless of iteration order. issueIdentifier
      // is decoupled below (LIN-1957 review F2) — it does NOT move with harness.
      lineage.harness = harnessOf(row);
    }
    // F2: issueIdentifier — captured from the EARLIEST row that actually
    // CARRIES one (may differ from the row that is `earliest` overall), same
    // non-order-dependent dispatchedAt comparison as `earliest`/`harness`
    // above. A null-identifier earliest row must not blank out a later row's
    // identifier — the approved plan's "set from any row carrying one".
    //
    // LIN-2121 L1: a wake row may INHERIT a lineage's attribution, never FOUND
    // it. Since LIN-2121 wake rows carry the triggering dispatch's
    // `issueIdentifier`, and LIN-1468's self-heal can reconcile a wake row onto
    // an identifier-LESS parent lineage (an autopilot/general run whose root has
    // no issue). Without this exclusion that wake would found the whole
    // lineage's attribution, moving its cost from the overhead bucket into the
    // wake's issue. Skip wake rows here so an identifier-less root lineage stays
    // identifier-less (in overhead) exactly as before the wake stamp, while a
    // lineage whose root carries an identifier is unchanged.
    if (row.kind !== 'wake' && row.issueIdentifier && (
      lineage.issueIdentifier === null ||
      (dispatched !== null && (lineage._issueIdentifierAt === null || dispatched < lineage._issueIdentifierAt))
    )) {
      lineage.issueIdentifier = row.issueIdentifier;
      lineage._issueIdentifierAt = dispatched;
    }
    const usage = usageOf(row);
    // F3: pushed with the row's own dispatchedAt attached and sorted once
    // below — `rows` is an unsorted concatenation of an unsorted aggregate
    // and an unsorted find, so push/iteration order is not a fact about the
    // data and must not leak into the claude-code/unknown last-wins reduce.
    if (usage) lineage.rowUsage.push({ dispatched, usage });
    // LIN-2253: raw ticket-marker entries accumulate per row, each tagged
    // with the CONTRIBUTING ROW's own dispatchedAt — same F3 discipline as
    // rowUsage above, and for the same reason: parseTicketMarkers is a
    // last-ARRAY-POSITION-wins fold (a later marker for the same identifier
    // overwrites the earlier one in place), so its input must already be in
    // chronological order for "last" to mean "latest state" rather than
    // "whichever row the unsorted input concatenation happened to visit
    // last." Sorted once at finalize time below, mirroring rowUsage's F3 sort
    // exactly.
    const ticketEntries = ticketMarkerEntriesOf(row);
    if (ticketEntries.length) lineage._ticketMarkerRaw.push({ dispatched, entries: ticketEntries });
  ...
  // F3: finalize rowUsage — sort each lineage's contributions by the
  // contributing row's own dispatchedAt (unknown-time entries sort last,
  // since an unknown position is the least authoritative choice for
  // last-wins), then strip back down to the plain usage-entry shape
  // consumers already expect. issueIdentifier's internal bookkeeping field
  // is also stripped here — never part of the public accumulator shape.
  for (const lineage of lineages.values()) {
    lineage.rowUsage = lineage.rowUsage
      .sort((a, b) => (a.dispatched ?? Infinity) - (b.dispatched ?? Infinity))
      .map(({ usage }) => usage);
    delete lineage._issueIdentifierAt;
    // LIN-2253 review: parseTicketMarkers is last-ARRAY-POSITION-wins, not
    // timestamp-aware, so its input must be sorted first — same F3 rule as
    // rowUsage just above, applied per-CONTRIBUTING-ROW group (`entries`)
    // rather than per-individual-marker, since a marker's own timestamp is
    // author-supplied prose-adjacent metadata, not a reliable sort key on
    // its own, while the contributing row's dispatchedAt already IS.
    lineage.ticketMarkers = parseTicketMarkers(
      lineage._ticketMarkerRaw
        .sort((a, b) => (a.dispatched ?? Infinity) - (b.dispatched ?? Infinity))
        .flatMap(({ entries }) => entries)
    );
    delete lineage._ticketMarkerRaw;
  }
...
export async function collectKpiStats(collections, { now = new Date(), dbBackend = null } = {}) {
  ...
  // Terminal-marked-task cost (LIN-1957, Session 1 of LIN-1625): the
  // API-equivalent dollar cost of `[done]`-marked work, plus its bias/
  // coverage disclosures. Runs over the SAME dispatchDocs as the outcome
  // rate above, sharing `groupDispatchLineages` so the two can never
  // disagree about which issues resolved to `done`. Aggregate-only: no
  // issueIdentifier crosses into this block (see privacy assertion in
  // tests/unit/kpi-stats.test.js).
  const terminalMarkedTaskCost = computeTerminalMarkedTaskCost(dispatchDocs, now);

  // Weekly-budget burn gauge (LIN-2118): an ESTIMATE of the current
  // subscription-window's consumption, calibrated from the same windowed
  // `[usage]` telemetry, scoped to the Thursday-06:00Z reset window rather
  // than the 30-day outcome window above. Aggregate-only, same privacy
  // boundary — see lib/weekly-budget.js.
  const weeklyBudgetGauge = computeWeeklyBudgetGauge(dispatchDocs, now);
  ...
  const funnel = {
    ...
    // A non-reporting agent is indistinguishable from an incomplete task, so
    // `reported`/`completed` can only ever undercount the true totals — they
    // are lower bounds, never exact counts (LIN-2325 F4).
    reportedIsLowerBound: true,
    completedIsLowerBound: true
  };
  ...
  // Distinct endpoints the top-8 cut silently drops (endpointCounts is
  // already keyed by distinct endpoint, so its size IS the distinct count).
  const topEndpointsOtherCount = excludedDistinctCount(endpointCounts.size);
  // The 24h sibling of topEndpointsOtherCount (review LIN-2325 F1): the 30d
  // truncation count does not describe the 24h view the range toggle can
  // switch to, so the caption needs its own count for that window or it
  // can claim a truncation that didn't happen in the narrower view.
  const topEndpointsHourlyOtherCount = excludedDistinctCount(endpointHourlyCounts.size);
  ...
  // Distinct dispatch kinds the top-8 cut silently drops, mirroring
  // dispatchByDay's own 'other' bucket for this flat top-list (LIN-2325 F4).
  const dispatchKindsOtherCount = excludedDistinctCount(countDistinctValues(windowedDispatchDocs, 'kind'));

  return {
    generatedAt: now.toISOString(),
    totals: {
      workspaces: workspaceKeys.size,
      // M is genuinely unknowable here: this is a union over ≤30d-TTL
      // activity collections plus durable preferences, never a workspace
      // registry read, so an older workspace with no recent activity and no
      // stored preferences silently isn't counted (LIN-2325 F4).
      workspacesBasis: 'workspaces with retained activity or stored preferences (older, quiet workspaces may be undercounted)',
      users: usersCount,
      activeSessions,
      ...
      // M is knowable but capped here — the store's own per-workspace
      // retention limit, named rather than hand-restated so it can't drift.
      roadmapReportsBasis: `newest ${MAX_REPORTS_PER_WORKSPACE} reports per workspace`,
    ...
    topEndpoints,
    topEndpointsHourly,
    topEndpointsOtherCount,
    topEndpointsHourlyOtherCount,
    hourOfDay,
    freeTier: {
      days: freeTierDays,
...
// LIN-3011 (LIN-2996 Phase 3) ledger L3: exposes the real `FRESH_DIGEST`
// expression (not a hand-copied approximation) so the real-`mongod` parity
// test in tests/unit/mongo-smoke.test.js can run it side by side with the JS
// `isFreshDigest` (lib/digest-feedback.js) over the exact row shapes this
// file's own `loadDispatchHistory digest read` suite already exercises —
// same `__internal` convention `dispatch-terminal.js`/`session-telemetry.js`
// use to share an implementation detail with tests without exporting it as
// public API.
export const __internal = { FRESH_DIGEST };
