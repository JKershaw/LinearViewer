 ...
 * labels (parameterized endpoint templates, action names) — never workspace
 * urlKeys, prompt text, summaries, tokens, or issue content.
 *
...
export async function collectKpiStats(collections, { now = new Date(), dbBackend = null } = {}) {
  ...
  const activity = {
    ...
    foreman: bucketByDay(foremanDocs, 'timestamp', activityDays),
  ...
  activityDays.forEach((day, i) => {
    const total = activity.proxy[i] + activity.foreman[i] + activity.dispatch[i];
  ...
  for (const doc of historyDocs) {
    if (doc.status in dispatchOutcomes) dispatchOutcomes[doc.status]++;
  }
  ...
  return {
    ...
    foremanActions: topCounts(foremanDocs, 'action'),
    freeTier: {
      days: freeTierDays,
