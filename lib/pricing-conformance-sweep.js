 ...
 * @returns {{verified: boolean, violations: Array<{id: string, tier: string, tableValue: number, catalogValue: number}>, checked: number}}
 */
export function findPricingConformanceViolations(catalogModels) {
  if (!Array.isArray(catalogModels) || catalogModels.length === 0) {
    ...
    return { verified: false, violations: [], checked: 0 };
  }
  ...
  return { verified: true, violations, checked };
}
...
export function createPricingConformanceSweepRun({ getCatalog = getModelCatalog, logger = console } = {}) {
  return async () => {
    try {
      ...
      const { verified, violations } = findPricingConformanceViolations(catalogModels);
      ...
      // verified === true, violations.length === 0: steady state, no log.
    } catch (err) {
