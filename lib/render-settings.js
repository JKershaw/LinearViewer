 ...
 * the full OpenRouter-derived `DISPATCH_MODEL_SUGGESTIONS` list above (plus the
 * live catalog) — Claude Code only offers Haiku / Sonnet / Opus. These are the
 * Claude Code `--model` aliases, so they stay stable across model generations
...
function renderDispatchDefaultRow({ label, namePrefix, testid, model = '', harness = '', preselectDefault = false }) {
  ...
  // three presets, OpenCode the full list. Start on the datalist matching the
...
function renderDispatchDefaultsSection(dispatchDefaults = {}, urlKey, errorCode = null, modelCatalog = []) {
  ...
  const modelSuggestionsHtml = [...DISPATCH_MODEL_SUGGESTIONS, ...catalogModelIds]
    ...
    .join('');
  // Claude Code datalist (LIN-1282): only the three presets, catalog never merged.
...
/**
 * Renders one saved preset's row: an editable name, its config row (reusing
...
function renderDispatchPresetRow(preset) {
  ...
  return `
          <div class="node dispatch-preset-item" data-testid="dispatch-preset-item-${escapeHtml(id)}" data-preset-id="${escapeHtml(id)}">
            ...
            <div class="children">${configRowHtml}</div>
          </div>`;
}
 ...
 * `byKind` authoring is out of scope (top-level model/harness only); the
 * shared datalists (`#dispatch-model-suggestions[-claude]`) are the SAME
 * page-global ones the Dispatch defaults section renders, so this section
 * only needs to be present on a page that also renders that section.
 *
  ...
  return `<p class="settings-subtitle">Save a named model/harness combination to select at dispatch time. Presets are workspace-wide; editing or deleting one never changes an already-dispatched item.</p>
      ...
      <div class="tree dispatch-preset-create" data-testid="dispatch-preset-create-form" data-url-key="${escapeHtml(urlKey)}">
        <div class="node">
          ...
          <div class="children">${newRowHtml}</div>
        </div>
      </div>
...
export function renderSettingsPage(workspaceName = 'Workspace', options = {}) {
  ...
  // part of the "AI" section alongside the global default selector.
