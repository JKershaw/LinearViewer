...
/**
 * Theme options for the Appearance control (LIN-756). Keys match VALID_THEMES in
 * lib/user-preferences.js; the client toggle and the bootstrap consume the same keys.
 */
const THEME_OPTIONS = [
...
/**
 * Render the Appearance / theme control (LIN-756).
 *
 * Each option is a real POST→redirect form (works without JS — the server persists
 * to preferences.theme and seeds the cookie). On the client, `initThemeToggle`
 * (public/app.js) intercepts the click to apply the theme instantly and write
 * `localStorage.theme`, so the change is flash-free and survives navigation.
 *
 * @param {string} formAction - The theme save route
 * @param {string} current - The active theme key
 * @returns {string} HTML for the section body
 */
function renderThemeControl(formAction, current) {
  const buttons = THEME_OPTIONS.map(({ key, label }) => {
    const isActive = key === current;
    const stateClass = isActive ? 'toggle-on' : 'toggle-off';
    const marker = isActive ? '● ' : '○ ';
    return `
              <form action="${formAction}" method="POST" class="settings-form theme-form">
                <input type="hidden" name="theme" value="${escapeHtml(key)}">
                <button type="submit" class="toggle-btn ${stateClass} theme-option" data-theme-option="${escapeHtml(key)}"${isActive ? ' aria-pressed="true"' : ''}><span class="toggle-state">${marker}${escapeHtml(label)}</span></button>
              </form>`;
  }).join('');

  return `<p class="settings-subtitle">Applies on every page; persists across devices</p>
      <div class="tree">
        <div class="node">
          <div class="line theme-control" data-testid="settings-theme-control">
            <span class="field-label">theme:</span>${buttons}
          </div>
        </div>
      </div>`;
}
...
export function renderSettingsPage(workspaceName = 'Workspace', options = {}) {
  const { openRouterSource = null, deployInfo = {}, currentModel = '', availableModels = [], modelError = null, urlKey = null, workspaces = [], featureFlags = FEATURE_DEFAULTS, workspaceFeatures = WORKSPACE_FEATURE_DEFAULTS, llmStats = null, providerBindings = [], providerNotice = null, theme = 'light' } = options
  ...
  const themeFormAction = `/workspace/${encodeURIComponent(urlKey)}/settings/theme`
  ...
  <main>
    ${renderSection({ boxed: true, className: 'settings-section', attrs: 'data-testid="settings-section-appearance"', titleClass: 'section-header settings-header', title: 'Appearance', body: renderThemeControl(themeFormAction, theme) })}
