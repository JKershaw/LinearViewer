 ...
 *
 * Deliberately NOT `getAllProviders()`: GitHub self-registers only in test boot
 * (`routes/test.js`), never at production boot, so a runtime registry walk would
 * silently omit it. We want GitHub surfaced as a disabled, blocker-annotated row,
 * so the addable list is a static known set. `blockedBy` names the dependency
 * ticket that gates a provider's live add (GitHub → LIN-541); a null `blockedBy`
 * means the add scaffold can begin a real connection today.
 */
const KNOWN_ADD_PROVIDERS = [
  { name: 'linear', displayName: 'Linear', blockedBy: null },
  { name: 'github', displayName: 'GitHub', blockedBy: 'LIN-541' },
];
    ...
    return `
        <div class="node">
          <div class="line provider-binding" data-testid="settings-provider-binding" data-provider="${escapeHtml(b.provider)}" data-scope="${escapeHtml(b.scope)}">
            ...
            <span class="provider-token">${escapeHtml(maskToken(b.token, b.provider))}</span>${activeHtml}
            ...
            <form action="${removeAction}" method="POST" class="settings-form provider-form">
              <input type="hidden" name="provider" value="${escapeHtml(b.provider)}">
              <input type="hidden" name="scope" value="${escapeHtml(b.scope)}">
              <button type="submit" class="action-btn disconnect" data-testid="settings-provider-remove">remove</button>
            </form>
          </div>
        </div>`;
  }).join('');
        ...
        <div class="node">
          <div class="line provider-add" data-testid="settings-provider-add-${escapeHtml(p.name)}" data-provider="${escapeHtml(p.name)}">
            ...
            <form action="${addAction}" method="POST" class="settings-form provider-form">
              ...
              <button type="submit" class="action-btn connect" data-testid="settings-provider-add-btn">add</button>
            </form>
          </div>
        </div>`;
  }).join('');
