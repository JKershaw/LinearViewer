...
const AI_FEATURES = ['aiRecommendations', 'promptButtons'];

/** Features shown in the Workflow section */
const WORKFLOW_FEATURES = ['linearMcp', 'featureBranches', 'codeReview', 'dispatch'];
...
export function renderSettingsPage(workspaceName = 'Workspace', options = {}) {
  ...
  // --- Dispatch section (only when dispatch is enabled) ---
  const dispatchSectionHtml = (urlKey && featureFlags.dispatch === true) ? `
    <section class="settings-section">
      <h2 class="settings-header">Dispatch</h2>
      <div class="tree">
        <div class="node">
          <div class="line">
            <span class="settings-label">prompt:</span>
            <span class="settings-value" style="flex:1">
              <textarea class="dispatch-prompt-input" data-url-key="${escapeHtml(urlKey)}" placeholder="Type a custom prompt or /command..." rows="3"></textarea>
            </span>
          </div>
          <div class="children">
            <div class="node">
              <div class="line dispatch-prompt-actions">
                <button class="settings-action save dispatch-prompt-send" data-target="cli">dispatch</button>
                <button class="settings-action save dispatch-prompt-send" data-target="web">dispatch &rarr; web</button>
      ...
      <div class="dispatch-section-divider"></div>
      <div class="tree">
        <div class="node">
  ...
  // --- Dispatch history section (only when dispatch is enabled) ---
  const historySectionHtml = (urlKey && featureFlags.dispatch === true) ? `
    <section class="settings-section">
      <h2 class="settings-header">Dispatch History</h2>
      <div class="history-list" data-url-key="${escapeHtml(urlKey)}">
