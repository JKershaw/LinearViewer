...
const EXPERIMENTAL_FEATURES = ['collective', 'taskChat', 'ship', 'nextRun', 'flightCompanion', 'shipBiscuit'];
...
export function renderSettingsPage(workspaceName = 'Workspace', options = {}) {
  ...
  const flightCompanionLinkHtml = flightCompanionOn
    ...
    : '';
  // When The Ship's Biscuit is on, expose its page link here too (same Settings-only
  // discovery pattern as Collective/Task chat/Ship/Next run/Flight companion; LIN-818).
  const shipBiscuitOn = featureFlags.shipBiscuit ?? FEATURE_DEFAULTS.shipBiscuit;
  const shipBiscuitLinkHtml = shipBiscuitOn
    ? `
            <div class="node">
              <div class="line">
                <span class="field-label">shipBiscuit:</span>
                <a href="/workspace/${encodeURIComponent(urlKey)}/ship-biscuit" class="settings-action">open The Ship's Biscuit</a>
              </div>
            </div>`
    : '';
  ...
  const experimentalTogglesHtml = EXPERIMENTAL_FEATURES.map(key =>
    renderFeatureToggle(key, featureFlags, featureFormAction)
  ).join('') + collectiveLinkHtml + taskChatLinkHtml + shipLinkHtml + nextRunLinkHtml + flightCompanionLinkHtml + shipBiscuitLinkHtml;
