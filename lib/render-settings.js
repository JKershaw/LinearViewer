...
import { buildModelOptions, isFreeModel } from './openrouter-catalog.js';
...
/**
 * Honest default for a provider's "add a source" row when it declares no
 * `addHint` of its own (LIN-2803). Applied at THIS read site, not as a
 * base-getter default on `interface.js` — every provider's `addProvider`
 * getter returns a fresh object literal, so a base-class default would
 * silently not be inherited. Linear keeps its own override text (its add
 * creates a separate workspace, not the default bind).
 */
const DEFAULT_ADD_HINT = 'adds a source onto this workspace';

/** Friendly labels for the AI_OPERATION_KINDS rendered in the per-operation override rows — one entry per kind, pinned by the AI_OPERATION_KINDS.every(k => k in AI_OPERATION_LABELS) test. */
 ...
 * too would silently convert all 18 inherited rows into explicit `claude-code`
 ...
 * prompts, not the one a dispatched agent executes with). Rendered as a real
 * `<select>` (LIN-2719) via `buildModelOptions` (lib/openrouter-catalog.js) —
 * curated ids plus the live/mock catalog, each option carrying a `data-free`
 * marker — with a blank first option (still means "inherit"/null) and an
 * `other…` free-text escape hatch (`DISPATCH_MODEL_OTHER_VALUE` below) so a
 * model outside both lists is still reachable.
 */
 ...
 * and need no versioned ids here. The per-row model select's option set swaps
 * between this list and the OpenCode one based on the selected harness,
 * client-side (`syncHarnessModelList`, public/common.js).
 *
 ...
 * repositories. Update both, or the select is offering a model that cannot run. (The
 * `other…` escape hatch, LIN-2719, covers the gap for models outside this curated
 * four — e.g. `default` or a version-pinned `claude-…` id — that SD's own
 * substring-matched acceptance list runs happily but this repo doesn't curate.)
 */
 ...
 * (LIN-2719, HARD RULE 3): a `<select>` can only offer curated + catalog ids,
 * but Simple Dispatcher's `CLAUDE_MAPPABLE_MODEL_FAMILIES` accepts strictly
 * more (substring match, plus any bare `claude-…` id) than Harbour's four
 * curated Claude presets — so an "other…" option is a correctness
 * requirement, not a convenience. Mirrored in public/common.js
 * (`DISPATCH_MODEL_OTHER_VALUE`, exposed there as `window.DISPATCH_MODEL_OTHER_VALUE`
 * for public/settings.js) — the two runtimes can't share a module, so the
 * literal string is the contract.
 */
const DISPATCH_MODEL_OTHER_VALUE = '__other__';

/**
 * The model `<select>`'s option set for a given (effective) harness: curated
 * ids for that harness plus — OpenCode only, never Claude Code — the live/mock
 * catalog, merged and free-marked via the one shared builder (LIN-2719 S0).
 *
 * @param {string} effectiveHarness
 * @param {Array<{id: string, name?: string, pricing?: Object|null}>} modelCatalog
 * @returns {Array<{id: string, label: string, free: boolean}>}
 */
function dispatchModelOptionSetFor(effectiveHarness, modelCatalog) {
  if (effectiveHarness === 'claude-code') {
    return buildModelOptions({ curatedIds: DISPATCH_CLAUDE_MODEL_SUGGESTIONS, catalog: [] });
  }
  return buildModelOptions({ curatedIds: DISPATCH_MODEL_SUGGESTIONS, catalog: modelCatalog });
}
...
/**
 * Renders one dispatch-defaults row: a label, the shared harness control, a
 * model `<select>` (LIN-2719) with an `other…` free-text escape hatch, and a
 * free-text effort input (LIN-2616). Used for both the workspace-wide default
 * row and every per-prompt-type override row (LIN-1095), and reused for
 * preset rows (`renderDispatchPresetRow`/`renderDispatchPresetKindOverrides`).
 *
 * `effort` is opaque like `model` — no registry/enum, just a blank-by-default
 * text input. Unlike the harness select, nothing pre-selects an effort value:
 * `preselectDefault` only ever forwards to `renderHarnessSelect` (LIN-1111,
 * workspace-wide row only) because harness is the one enumerated control here;
 * an empty effort input always means "inherit", on every row including the
 * workspace-wide one.
 *
 * HARD RULE 1 (LIN-2719): the model select's blank first option is ALWAYS the
 * selected one when `model` is unset — never pre-selected to a real value,
 * since blank means "inherit" here and this form re-derives its whole tree on
 * every save (a pre-selected row would silently become an explicit override
 * on the next save of ANY field). The blank option's label carries the
 * re-expressed "blank = inherit" hint HARD RULE 1's swap would otherwise drop:
 * "— none —" on the one row with nothing to inherit from (the workspace-wide
 * default itself, `preselectDefault: true`), "— inherit —" on every other row.
 *
 * A stored `model` outside the current harness's option set (a legacy id, or
 * one from OpenRouter's wider catalog than what's curated) degrades onto the
 * `other…` escape hatch rather than being silently dropped (HARD RULE 2/3):
 * the select lands on the sentinel and the stored value fills the paired
 * free-text input, so a round-trip save still posts the original id.
 *
 * @param {Object} opts
 ...
 * @param {string} [opts.effort] - Current effort value
 * @param {boolean} [opts.preselectDefault] - Forwarded to `renderHarnessSelect` (LIN-1111; workspace-wide row only)
 * @param {Array<{id: string, name?: string, pricing?: Object|null}>} [opts.modelCatalog] - Live/mock OpenRouter catalog (LIN-1111 Session 2), merged into the OpenCode option set only
 * @returns {string} HTML for the row
 */
function renderDispatchDefaultRow({ label, namePrefix, testid, model = '', harness = '', effort = '', preselectDefault = false, modelCatalog = [] }) {
  // The model option set is harness-aware (LIN-1282): Claude Code offers only
  // its presets, OpenCode the full curated+catalog list. The shared client
  // handler (syncHarnessModelList, public/common.js) rebuilds the option set
  // when the harness select changes.
  const effectiveHarness = harness || (preselectDefault ? DEFAULT_HARNESS : '');
  const optionSet = dispatchModelOptionSetFor(effectiveHarness, modelCatalog);
  const isKnownModel = model && optionSet.some(opt => opt.id === model);
  const selectValue = !model ? '' : (isKnownModel ? model : DISPATCH_MODEL_OTHER_VALUE);
  const otherValue = model && !isKnownModel ? model : '';
  const blankLabel = preselectDefault ? '— none —' : '— inherit —';
  const optionsHtml = optionSet.map(opt => {
    const selected = opt.id === selectValue ? ' selected' : '';
    const freeAttr = opt.free ? ' data-free="true"' : '';
    return `<option value="${escapeHtml(opt.id)}"${selected}${freeAttr}>${escapeHtml(opt.label)}</option>`;
  }).join('\n                  ');
  return `
          <div class="node">
            <div class="line dispatch-default-row" data-testid="${escapeHtml(testid)}">
              ...
              <select name="${escapeHtml(namePrefix)}Model" class="dispatch-model-input" aria-label="Model">
                <option value=""${selectValue === '' ? ' selected' : ''}>${escapeHtml(blankLabel)}</option>
                ${optionsHtml}
                <option value="${DISPATCH_MODEL_OTHER_VALUE}"${selectValue === DISPATCH_MODEL_OTHER_VALUE ? ' selected' : ''}>other&hellip;</option>
              </select>
              <input type="text" class="dispatch-model-input-other${selectValue === DISPATCH_MODEL_OTHER_VALUE ? '' : ' hidden'}" maxlength="200" placeholder="custom model id" aria-label="Custom model id" value="${escapeHtml(otherValue)}">
              <span class="dispatch-model-hint" data-model-hint></span>
              <input type="text" name="${escapeHtml(namePrefix)}Effort" class="dispatch-effort-input" maxlength="200" placeholder="effort (blank = inherit)" value="${escapeHtml(effort)}">
            </div>
          </div>`;
}
 ...
 * @param {string} urlKey - Workspace urlKey
 * @param {string} [errorCode] - Validation error code from redirect, if any
 * @param {Array<{id: string, name: string, pricing?: Object|null}>} [modelCatalog] - Live/mock
 *   OpenRouter catalog (LIN-1111 Session 2), merged into every row's OpenCode
 *   model option set alongside DISPATCH_MODEL_SUGGESTIONS (LIN-2719: via
 *   `dispatchModelOptionSetFor`, per row, not a shared page-level datalist).
 * @returns {string} HTML for the section body
 */
function renderDispatchDefaultsSection(dispatchDefaults = {}, urlKey, errorCode = null, modelCatalog = []) {
  ...
  const hasKindOverride = kindKeys.some(kind => byKind[kind]?.model || byKind[kind]?.harness || byKind[kind]?.effort);

  // Carried to the client (LIN-2719) as the SAME {id, name, free} wire shape
  // GET .../api/openrouter/models returns, so public/common.js's harness-switch
  // rebuild (rebuildDispatchModelOptions) can reuse the exact same merge/de-dupe
  // logic for a server-rendered Settings row as it does for a client-fetched
  // exec-controls one — without it, switching a row's harness client-side would
  // rebuild the OpenCode option set from an empty catalog (nothing on this page
  // ever calls window.fetchDispatchModelCatalog) and silently drop every
  // catalog entry the server just rendered.
  const modelCatalogWireHtml = escapeHtml(JSON.stringify(
    modelCatalog.map(m => ({ id: m.id, name: (typeof m.name === 'string' && m.name) || m.id, free: isFreeModel(m) }))
  ));
  ...
  return `<p class="settings-subtitle">The model/harness dispatched agents execute <em>with</em> &mdash; distinct from the Workspace AI Model above, which is used to <em>write</em> prompts. Per-type rows override the workspace default; leave a row blank to inherit it.</p>
      <form action="${formAction}" method="POST" class="settings-form dispatch-defaults-form" data-model-catalog="${modelCatalogWireHtml}">
      <div class="tree">${errorHtml}
        ${defaultRowHtml}
        <div class="node">
 ...
 * @param {Array<{id: string, name?: string, pricing?: Object|null}>} [opts.modelCatalog] - Forwarded to each row (LIN-2719)
 * @returns {string} HTML for the collapsible per-kind block
 */
function renderDispatchPresetKindOverrides({ namePrefixBase, testidBase, byKind = {}, modelCatalog = [] }) {
  ...
  const hasKindOverride = DISPATCH_DEFAULT_KINDS.some(kind => byKind[kind]?.model || byKind[kind]?.harness || byKind[kind]?.effort);
 ...
 *
 * A "use on Dispatch" link (LIN-2719 S3) rides `?preset=<id>`, which the
 * Dispatch page's picker (`public/dispatch.js`) reads on load and
 * pre-selects — the reverse direction of the empty-picker "create a preset"
 * link back here.
 *
 * @param {{id: string, name: string, config: Object}} preset
 * @param {Array<{id: string, name?: string, pricing?: Object|null}>} [modelCatalog] - Live/mock catalog (LIN-2719)
 * @param {string} [urlKey] - Workspace urlKey, for the "use on Dispatch" link
 * @returns {string} HTML for the preset row
 */
function renderDispatchPresetRow(preset, modelCatalog = [], urlKey = '') {
  ...
  const useOnDispatchHtml = urlKey
    ? `<a href="/workspace/${encodeURIComponent(urlKey)}/dispatch?preset=${encodeURIComponent(id)}" class="settings-action dispatch-preset-use-link" data-testid="dispatch-preset-use-${escapeHtml(id)}">use on Dispatch &rarr;</a>`
    : '';
  return `
 ...
 * harness-aware model select + LIN-2719 `other…` escape hatch, plus a
 * two-option harness select) with
 ...
 * tell the two apart. Each row builds its own model option set from the same
 * `modelCatalog` the Dispatch defaults section renders (LIN-2719: no shared
 * page-level datalist any more), so this section still only needs the live
 * catalog threaded in alongside it.
 *
 ...
 * @returns {string} HTML for the section body
 */
function renderDispatchPresetsSection(dispatchPresets = [], urlKey, modelCatalog = []) {
  const presetRowsHtml = dispatchPresets.length
    ? dispatchPresets.map(preset => renderDispatchPresetRow(preset, modelCatalog, urlKey)).join('')
    : `
          <div class="node">
            <div class="line">
 ...
 * @param {string} urlKey
 * @param {{type: 'ok'|'fail'|'blocked', text: string, activate?: {provider: string, scope: string}}|null} notice -
 *   `activate` (LIN-2803) is present only for a just-added binding whose
 *   provider's add genuinely BINDS onto this workspace (never for one whose
 *   add creates a separate workspace, e.g. Linear — offering "make active"
 *   there would point at nothing this workspace actually gained). Renders the
 *   same `switchAction` POST form every per-binding row already uses.
...
function renderProvidersSection(bindings, urlKey, notice, githubEnabled = true, jiraOAuthEnabled = true) {
  ...
  // A just-added binding's "make active" affordance (LIN-2803): reuses the
  // EXACT `switchAction` POST form every per-binding row already renders
  // (`activateHtml` below), never a link — the notice is a shortcut to that
  // existing mechanism, not a new one. Its own testid
  // (`settings-provider-notice-activate`) stays distinct from the per-binding
  // `settings-provider-activate` so both can coexist unambiguously when the
  // just-added binding is also the one inactive row on the page.
  const noticeActivateHtml = notice && notice.activate ? `
            <form action="${switchAction}" method="POST" class="settings-form provider-form">
              <input type="hidden" name="provider" value="${escapeHtml(notice.activate.provider)}">
              <input type="hidden" name="scope" value="${escapeHtml(notice.activate.scope)}">
              <button type="submit" class="action-btn" data-testid="settings-provider-notice-activate">make active</button>
            </form>` : '';
  const noticeHtml = notice ? `
        <div class="node">
          <div class="line provider-notice provider-notice-${escapeHtml(notice.type)}" data-testid="settings-provider-notice">
            <span class="field-value">${escapeHtml(notice.text)}</span>${noticeActivateHtml}
          </div>
        </div>` : '';
  ...
  const addRows = getAllProviders().filter(p => p.addProvider).map(p => {
    ...
    // Honest note on what an add does (LIN-2803). Every unblocked add row gets
    // one: a provider's own `addHint` when declared (Linear's add-source
    // connects a whole separate organization as its own workspace, LIN-1351),
    // else the shared `DEFAULT_ADD_HINT` — the default "bind a source onto
    // THIS workspace" behavior GitHub/GitHub Projects/Jira share but never
    // said out loud until now.
    const addHintHtml = `
            <span class="field-value provider-add-hint" data-testid="settings-provider-add-hint-${escapeHtml(p.name)}">${escapeHtml(p.addProvider.addHint || DEFAULT_ADD_HINT)}</span>`;
    // "As a new workspace" verb (LIN-2803): a second, honest door to a new
    // workspace from the SAME row the bind verb lives on, mirroring the
    // navbar's own `entryCta.href` anchor (`lib/components/navbar.js`) rather
    // than a second route table. Gated on two provider-declared facts, never
    // on `entryCta` alone (Linear's add already creates a workspace, so its
    // "bind" and "create" verbs would be the same lie) and never a
    // `<button>`/`<form>` (would move the pinned Linear/Jira button counts).
    // Testid uses `new-workspace-`, deliberately outside the
    // `settings-provider-add-` scrape prefix `lin-2010-provider-identity-
    // registry.test.js` pins the row sequence with.
    const newWorkspaceHtml = (p.entryCta && p.addProvider.createsWorkspace !== true && p.addProvider.newWorkspaceVerb === true)
      ? `
            <a href="${p.entryCta.href}" class="action-btn provider-add-new-workspace" data-testid="settings-provider-new-workspace-${escapeHtml(p.name)}">as a new workspace</a>`
      : '';
    ...
    return `
        <div class="node">
          <div class="line provider-add" data-testid="settings-provider-add-${escapeHtml(p.name)}" data-provider="${escapeHtml(p.name)}">
            <span class="field-label">+ ${escapeHtml(displayName)}:</span>${addHintHtml}${formsHtml}${newWorkspaceHtml}
          </div>
        </div>`;
  }).join('');
 ...
 * @property {Array<{id: string, name: string, pricing?: Object|null}>} [dispatchModelCatalog] - Live
 *   OpenRouter model catalog (LIN-1111 Session 2, lib/openrouter-catalog.js),
 *   resolved by the caller and merged into every dispatch-defaults/preset row's
 *   model select alongside DISPATCH_MODEL_SUGGESTIONS (LIN-2719). Defaults to
 *   `[]` so a caller that hasn't fetched it yet (or a degraded/empty catalog)
 *   renders the static suggestion list unchanged.
 */
...
export function renderSettingsPage(workspaceName = 'Workspace', options = {}) {
  ...
  // not another default tier. Shares the same live/mock model catalog
  // (LIN-2719: each row builds its own inline option set, no page-level
  // datalist any more).
  // `id="dispatch-presets"` (LIN-2719 S3): the Dispatch-page picker's empty
  // state links here (`#dispatch-presets`) when there is nowhere to send a
  // freshly-created preset without it.
  const dispatchPresetsSectionHtml = renderSection({ boxed: true, className: 'settings-section', attrs: 'id="dispatch-presets" data-testid="settings-section-dispatch-presets"', titleClass: 'section-header settings-header', title: 'Dispatch presets', body: renderDispatchPresetsSection(dispatchPresets, urlKey, dispatchModelCatalog) });
