...
const KNOWN_ADD_PROVIDERS = [
  ...
  // Jira's add is live as of LIN-1885 Phase 1 — the API-token Basic-auth link
  // form (POST .../providers/add redirects into GET /auth/jira?workspace=...,
  // not an OAuth mode:'add-source' — Jira has no redirect round-trip).
  // Jira's add is live as of LIN-1885 Phase 1 (Basic) and gains a second auth
  // shape in LIN-1887 (OAuth 3LO). `authShapes` is what makes the row render a
  // CHOICE rather than a single button — D5's answer. The row itself stays
  // unconditionally enabled: Basic needs no server config, so gating the ROW on
  // Jira OAuth config would disable an add path that works without it. The
  // config gate belongs on the OAuth OPTION.
  { name: 'jira', displayName: 'Jira', blockedBy: null, authShapes: [
...
function renderProvidersSection(bindings, urlKey, notice, githubEnabled = true, jiraOAuthEnabled = true) {
  ...
  const addRows = KNOWN_ADD_PROVIDERS.map(p => {
    ...
    const addHintHtml = p.addHint
      ...
      : '';
    const formsHtml = (p.authShapes || [{ value: null, label: 'add' }]).map(shape => {
    ...
    return `
        <div class="node">
          <div class="line provider-add" data-testid="settings-provider-add-${escapeHtml(p.name)}" data-provider="${escapeHtml(p.name)}">
            <span class="field-label">+ ${escapeHtml(p.displayName)}:</span>${addHintHtml}${formsHtml}
          </div>
        </div>`;
  }).join('');
