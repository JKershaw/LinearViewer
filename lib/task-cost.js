 ...
 *   noLineage: boolean,
 *   workerSessions: Array<{rootItemId: string, kind: string, dispatchedAt: string|null, harness: string|null, model: string|null, effort: string|null, costUsd: number|null, durationMs: number|null}>,
 *   appCalls: Object
 * }}
 */
export function buildTaskCost({ ownRows = [], siblingRowsByAnchor = new Map(), appSummary = EMPTY_APP_CALLS } = {}) {
  ...
  for (const [anchor, { base, extraOwnRows }] of lineages) {
    ...
    if (!usage) {
      noTelemetryCount += 1;
      workerSessions.push({ rootItemId: anchor, kind: base.kind || 'custom', dispatchedAt: base.dispatchedAt || null, harness: null, model: null, effort: null, costUsd: null, durationMs });
      continue;
    }
    ...
    // LIN-2830: `harness` is the REALISED harness the run itself stamped on its
    // [usage] telemetry (both harnesses do — hook.js 'claude-code',
    // opencode-runner.js 'opencode'), never the dispatch row's REQUESTED
    // `harness` field — the same realised-never-requested ruling the effort
    // column already carries (D10). Null when the run reported no telemetry
    // at all (backfill), matching `model`/`effort` beside it.
    workerSessions.push({
      rootItemId: anchor,
