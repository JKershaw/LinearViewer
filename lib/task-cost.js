...
import { mergeLineageFeedback } from './dispatch-terminal.js';
 ...
 *   merged) scoped to the task's issueIdentifier, in the `/dispatch` list
 *   route's formatted shape ({id, status, dispatchedAt, feedback, kind,
 *   rootItemId, ...}). Rows whose `status !== 'taken'` never ran and are
 *   filtered out here (mirrors the `/dispatch` list route's own allowlist).
 ...
 *   workerSessions: Array<{rootItemId: string, kind: string, dispatchedAt: string|null, model: string|null, costUsd: number|null}>,
 *   appCalls: Object
 * }}
 */
export function buildTaskCost({ ownRows = [], siblingRowsByAnchor = new Map(), appSummary = EMPTY_APP_CALLS } = {}) {
  ...
  for (const [anchor, { base, extraOwnRows }] of lineages) {
    ...
    if (!usage) {
      noTelemetryCount += 1;
      workerSessions.push({ rootItemId: anchor, kind: base.kind || 'custom', dispatchedAt: base.dispatchedAt || null, model: null, costUsd: null });
      continue;
    }
  ...
  const fullyPriced = unpriced.length === 0 && noTelemetryCount === 0 && (appCalls.unpricedCalls || 0) === 0;
