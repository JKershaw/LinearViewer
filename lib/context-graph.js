...
//
// A session (an autopilot run) touches a SET of tasks, not one root: the seed
// issue, the children an epic descent reaches, and the issues a `breakdown`
// spins off mid-run. buildSessionContextGraph composes the SAME single-root
// neighborhood primitive (buildContextGraph) once per touched task and tags each
// with provenance, so it shows the shape of everything the run touched without
// inventing new edge kinds (breakdown spawn is ordinary parent/children edges +
// a `spun-off` tag) and without disturbing the single-root path — that stays
// byte-identical. Deterministic and network-free: every edge and every
// provenance decision is read from the supplied issue set.

/** Resolve an id-or-identifier (LIN-123) to a canonical issue, case-insensitively. */
function _resolve(byId, byIdentifier, key) {
  if (!key) return null;
  return byId.get(key) || byIdentifier.get(String(key).toLowerCase()) || null;
}

/**
 * The seed's hierarchy set: every issue id that is an ancestor OR a descendant
 * of the seed via `parent.id` chains, within the supplied set. Cycle-safe.
 * Used to separate a pre-existing in-hierarchy touch (`descended`) from a
 * non-hierarchical one.
 */
function _seedHierarchy(byId, seedId) {
  const inHierarchy = new Set();
  if (!seedId || !byId.has(seedId)) return inHierarchy;

  // Ancestors: walk parent links upward.
  let cursor = byId.get(seedId);
  const seenUp = new Set([seedId]);
  while (cursor?.parent?.id && byId.has(cursor.parent.id) && !seenUp.has(cursor.parent.id)) {
    seenUp.add(cursor.parent.id);
    inHierarchy.add(cursor.parent.id);
    cursor = byId.get(cursor.parent.id);
  }

  // Descendants: BFS over the inverted parent map.
  const childrenOf = new Map();
  for (const issue of byId.values()) {
    const pid = issue.parent?.id;
    if (pid && byId.has(pid)) {
      if (!childrenOf.has(pid)) childrenOf.set(pid, []);
      childrenOf.get(pid).push(issue.id);
    }
  }
  const seenDown = new Set([seedId]);
  let frontier = [seedId];
  while (frontier.length) {
    const next = [];
...
/**
 * Provenance of a touched task relative to the session (deterministic):
 *   - `seed`     — the session's seed issue.
 *   - `spun-off` — created DURING the run window (the breakdown-spawn case;
 *                  `createdAt` is the clean disambiguator a pure hierarchy test
 *                  can't give, since breakdown children are also seed children).
 *                  Also the fallback for a non-hierarchical touch (a sibling or
 *                  related task neither pre-existing-in-hierarchy nor in-window).
 *   - `descended`— pre-existing AND inside the seed's hierarchy (ancestor or
 *                  descendant) — reached by an epic descent, not spawned.
 */
function _provenance(issue, { seedId, startMs, endMs, hierarchy }) {
  if (issue.id === seedId) return 'seed';
  const createdMs = _toMs(issue.createdAt);
  const inWindow = createdMs != null && startMs != null &&
    createdMs >= startMs && (endMs == null || createdMs <= endMs);
  if (inWindow) return 'spun-off';
  if (hierarchy.has(issue.id)) return 'descended';
  return 'spun-off';
}

/**
 * Build the session context: the relationship neighborhood of every touched
 * task, each tagged with provenance.
 *
 * @param {Array<Object>} issues - Canonical issues (same set buildContextGraph reads).
 * @param {Array<string>} touchedIds - Touched task ids/identifiers, seed-first
 *   (e.g. `session.tasksTouched`).
 * @param {Object} [opts]
 * @param {string} [opts.seedIssue] - Seed issue id or identifier (`session.seedIssue`).
 * @param {Object} [opts.window] - The run window `{ start, end }` (ISO or Date;
 *   `session.dispatchedAt`/`completedAt`). `end` null ⇒ still in flight (open).
 * @param {Object} [opts.graphOpts] - Forwarded to buildContextGraph (caps).
 * @returns {Object} `{ seedIssue, window:{start,end}, tasks:[…], touchedIdentifiers, missing:[…] }`
 */
export function buildSessionContextGraph(issues, touchedIds = [], opts = {}) {
  const list = issues || [];
  const byId = new Map();
  ...
  for (const issue of list) {
    ...
    if (issue.identifier) byIdentifier.set(String(issue.identifier).toLowerCase(), issue);
  }

  const seed = _resolve(byId, byIdentifier, opts.seedIssue);
  const seedId = seed ? seed.id : null;
  const startMs = _toMs(opts.window?.start);
  const endMs = _toMs(opts.window?.end);
  const hierarchy = _seedHierarchy(byId, seedId);

  // Resolve touched ids → issues, de-duped, preserving seed-first input order.
  const resolved = [];
  const seenTouched = new Set();
  const missing = [];
  for (const key of touchedIds || []) {
    const issue = _resolve(byId, byIdentifier, key);
    if (!issue) { missing.push(key); continue; }
    if (seenTouched.has(issue.id)) continue;
    seenTouched.add(issue.id);
    resolved.push(issue);
  }

  const touchedIdentifiers = resolved.map(i => i.identifier || i.id);

  const tasks = resolved.map((issue) => {
    const graph = buildContextGraph(list, issue.id, opts.graphOpts || {});
    return {
      ...graph,
      provenance: _provenance(issue, { seedId, startMs, endMs, hierarchy }),
    };
  });

  return {
    seedIssue: seed ? (seed.identifier || seed.id) : (opts.seedIssue || null),
    window: {
