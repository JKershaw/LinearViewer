 ...
 *                              // changed) AND by advance() on every genuine
 *                              // transition, so a caller that never re-calls
 *                              // ensureSeeded but keeps ticking advance() with
 *                              // real changes still counts as seen. This, not
 *                              // updatedAt, is what cleanup() and INDEX_SPECS
 *                              // key liveness on
 * }
 *
 ...
 *   entries via the same `$push`'s `$slice`, so the ledger's OWN
 *   transition-history growth cannot run unbounded across an instance's
 *   lifetime. This bounds the ledger array only — it says nothing about the
 *   `state` field's own size, which this store is agnostic to and does not
 *   cap; a caller writing an ever-larger payload can still grow a document
 *   without bound. Keeping that payload bounded is the caller's own
 *   discipline (P1-3's `buildSweepPayload`, e.g., caps `attention` at
 *   `ATTENTION_CAP`), not something this store enforces or should be read as
 *   guaranteeing.
 ...
 * separate liveness signal, and it is refreshed by *two* independent paths so
 * that either common caller shape keeps a live instance visible:
 * `ensureSeeded()` refreshes it on *every* call, whether or not the document
 * already existed — the shape for a sweep that calls `ensureSeeded()` every
 * tick regardless of whether the diagnosis changed (see `ensureSeeded`'s own
 * doc comment) — and `advance()` also refreshes it on every genuine
 * transition (a bare `$set` alongside `updatedAt`, not a separate write) —
 * the shape for a sweep that seeds an instance once and thereafter drives
 * every tick through `advance()` alone, without a repeat `ensureSeeded()`
 * call. `cleanup()` and the `INDEX_SPECS` entry both key on `lastSeenAt` for
 * this reason, leaving `updatedAt` free to keep meaning "last changed", which
 * is what a reader of a versioned document expects it to mean. (The
 * duplicate-no-op branch of `advance()` deliberately does NOT refresh
 * `lastSeenAt` itself — it makes no write at all, by design; a caller relying
 * on it alone to signal liveness for a diagnosis that has gone genuinely
 * stable must still call `ensureSeeded()` to do so, which is the intended
 * decommissioning path for an instance neither shape is touching, not a gap.)
 *
 ...
 * reachable for a **genuinely decommissioned** instance: one that has gone
 * idle past `RETENTION_IDLE_MS` on `lastSeenAt` under BOTH liveness paths
 * above (no `ensureSeeded()` call and no genuine `advance()` transition —
 * see "Liveness vs. change"), i.e. actually evicted. It is not reachable for
 * an instance kept alive by *either* path — a caller re-seeding every tick,
 * or one driving genuine `advance()` transitions alone — since either one
 * keeps `lastSeenAt` fresh and so keeps the instance out of `cleanup()`'s
 * reach in the first place. A consumer that needs a revision
 ...
 * properties, and its `String()` coercion. This closes the collision for a
 * class instance whose distinguishing state shows up in EITHER its
 * enumerable own properties OR a meaningfully-overridden `toString()`/
 * `String()` coercion — the common case. It provides NO protection, and two
 * differently-stated instances of the same class WILL still collapse onto
 * the same hash, when a class keeps its entire distinguishing state in
 * private fields or other non-enumerable internals AND relies on the
 * default `Object.prototype.toString` (`"[object Object]"` for every
 * instance) — that combination reduces `canonicalizeForHash`'s fallback to
 * the same blind `{}` two such instances would otherwise share, identically
 * to the pre-fix `Date`/`Map`/`Set` collision this guard exists to close.
 * This is a guard against the payload shapes actually exercised
 * (`Date`/`Map`/`Set`/an ordinary class instance with real own properties),
 * not a proof of uniqueness for arbitrary exotic objects — a docblock note
 * alone was rejected because it leaves the classification silently wrong for
 * the exact payload shapes named above; P1-3's payload contract should still
 * prefer plain JSON-safe values where practical, and MUST avoid a
 * private-fields-only class shape if it is ever passed through this hash.
 *
...
function canonicalizeForHash(value) {
  ...
  if (proto !== Object.prototype && proto !== null) {
    ...
    // slots) — the general case of the Date/Map/Set blind spot. Folding in
    // the constructor name and a String() coercion alongside whatever IS
    // enumerable closes the collision when the distinguishing state shows up
    // in either place. It does NOT when neither does: a class whose entire
    // distinguishing state lives in private fields/internal slots AND relies
    // on the default Object.prototype.toString ("[object Object]" for every
    // instance) still collapses two differently-stated instances onto the
    // same blind {__type, __own: {}, __str: "[object Object]"} — the same
    // failure mode `new Date()` had, just not closed by this fallback. See
    // this module's own header ("Hashing non-plain values") for the full
    // statement of what this guard does and does not cover.
...
export class ObserverStateStore {
   ...
   *
   * Also refreshes `lastSeenAt` on every genuine transition (LIN-2129 review,
   * item B), alongside `updatedAt` in the same `$set` — not a separate write.
   * This is what keeps a caller that seeds an instance once and thereafter
   * drives every tick through `advance()` alone (never re-calling
   * `ensureSeeded()`) out of `cleanup()`'s reach, as long as it keeps making
   * genuine transitions; see the module header's "Liveness vs. change"
   * section. The duplicate-no-op branch below makes no write at all, so it
   * does not refresh `lastSeenAt` — a caller whose diagnosis has gone
   * genuinely stable still needs `ensureSeeded()` for that.
   *
   * @param {string} instanceKey
  ...
  async advance(instanceKey, expectedRev, nextState, transitionMeta = null) {
    ...
    try {
      ...
      const { matchedCount } = await this.collection.updateOne(
        ...
        {
          $set: { state: nextState, stateHash: nextHash, rev: expectedRev + 1, updatedAt: now, lastSeenAt: now },
          $push: {
            ledger: {
   ...
   * stable. `lastSeenAt` is kept fresh by EITHER of two independent paths —
   * `ensureSeeded()` on every call, or `advance()` on every genuine
   * transition (not `ensureSeeded()` alone) — so either a caller that
   * re-seeds every tick or one that drives every tick through `advance()`
   * alone stays out of this sweep's reach; see the module header's
   * "Liveness vs. change" section for the full contract, including the one
   * case neither path covers (a diagnosis gone genuinely stable with no
   * repeat `ensureSeeded()` call).
