...
/** Anchors for the one-sentence Principle 0 test within the hand-back section. */
const PRINCIPLE_ZERO_TEST_START = '> Does this genuinely require the human';
const PRINCIPLE_ZERO_TEST_END = '\n\nBefore you park BLOCKED';

/**
 * Extract just the one-sentence Principle 0 acceptance test and its positive-half
 * tie-break from the hand-back section — for worker-facing prompt paths that strand
 * a worker with a bare pointer (LIN-2973). Deliberately narrower than
 * extractPrincipleZeroSection(): that full section also carries orchestrator-only
 * ruling/close-out mechanics (the DECISION: block shape, sibling-blocker merging,
 * the close-out dispatch step) that don't belong in a worker's If-Blocked footer.
 * Composed from extractPrincipleZeroSection()'s own output, so it cannot drift from
 * the manual — never a separately hand-copied sentence.
 * @returns {string|null} The test text, or null if the anchor isn't found (e.g. the
 *   manual failed to load and the fallback text has no such section).
 */
export function extractPrincipleZeroTest() {
  const section = extractPrincipleZeroSection();
  if (section === null) return null;

  const start = section.indexOf(PRINCIPLE_ZERO_TEST_START);
  if (start === -1) return null;

  const rest = section.slice(start);
  const end = rest.indexOf(PRINCIPLE_ZERO_TEST_END);
