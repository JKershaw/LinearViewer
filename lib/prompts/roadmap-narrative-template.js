...
/** Linear numeric priority → word. 0 (none) and unknown values yield ''. */
const PRIORITY_WORDS = { 1: 'urgent', 2: 'high', 3: 'medium', 4: 'low' };
function priorityPrefix(priority) {
  const word = PRIORITY_WORDS[priority];
  return word ? `[${word}] ` : '';
}

/** Identifier prefix (e.g. "LIN-204 ") so the model can cite real IDs. */
function idPrefix(task) {
  return task.identifier ? `${task.identifier} ` : '';
}

/** Label suffix (e.g. " {bug, infra}") — the maintenance/feature signal. */
function labelSuffix(task) {
 ...
 * Surfaces identifier, priority, and labels on the headline, the (already
 * truncated) description on an indented continuation line, and nests subtasks
 * with box-drawing characters. Parents also show rollup stats.
 *
 * Used for current/near-term work (in progress, next up) where this detail
 * helps the reader judge what matters; shipped work stays title-only.
 */
function renderTaskLine(lines, task, indent) {
  const subs = task.subtasks || [];
  const head = `${priorityPrefix(task.priority)}${idPrefix(task)}${task.title}${labelSuffix(task)}`;
  if (subs.length > 0) {
    const r = task.rollup || {};
    lines.push(`${indent}- ${head} (${r.subtaskDone || 0}/${r.subtaskTotal || subs.length} subtasks done)`);
  } else {
  ...
  for (let i = 0; i < subs.length; i++) {
    ...
    const status = subs[i].stateType === 'completed' ? 'done'
      ...
      : 'todo';
    lines.push(`${indent}  ${prefix} ${idPrefix(subs[i])}${subs[i].title} [${status}]`);
  }
}
    ...
    for (const m of projects) {
      ...
      if (recent.length > 0) {
        ...
        for (const t of recent) {
          ...
          lines.push(`      - ${idPrefix(t)}${t.title}${state}${when ? ` (${when})` : ''}`);
        }
      }

      // In-progress tasks (present state) — with subtask hierarchy.
      const queue = m.tasksInQueue || [];
      const inProgress = queue.filter(t =>
      ...
      // Next up — committed Todo work not yet started (state type 'unstarted').
      // Backlog ('backlog') is intentionally excluded: it is someday/maybe, not
      // part of the near-term plan a roadmap reading is about.
      const inProgressSet = new Set(inProgress);
      const nextUp = queue.filter(t => !inProgressSet.has(t) && t.stateType === 'unstarted');
      if (nextUp.length > 0) {
        lines.push('    Next up:');
 ...
 * @param {string|null} [providerDisplayName=null] - Declared provider display name
 *   (LIN-2354's contract, LIN-2371). The identifier scheme belongs to whichever
 *   provider backs this workspace, so naming Linear unconditionally was false on
 *   every other backend. Omit/`null` ⇒ the neutral "their tracker identifier",
 *   never a fallback to Linear.
 * @returns {Array<{role: string, content: string}>} Messages array for streamChat
 */
export function buildRoadmapNarrativeMessages(roadmapModel, providerDisplayName = null) {
  const summary = summarizeRoadmapModel(roadmapModel);
  // The "(e.g. LIN-204)" example that used to qualify this phrase is gone
  // deliberately (found by review): once the scheme is named from the real
  // provider, "their Jira identifier (e.g. LIN-204)" contradicts itself on
  // exactly the non-Linear workspaces this fix exists for. The instruction that
  // follows already says what to do with the identifier, so the example was
  // carrying the contradiction and nothing else.
  const identifierPhrase = providerDisplayName
    ? `their ${providerDisplayName} identifier`
    : 'their tracker identifier';
...
- Tasks carry ${identifierPhrase} where one exists. Cite the identifier alongside the title at least on first mention of a task, so the reader can trace it back to the tracker.
- A priority is shown in brackets ([urgent], [high], [medium], [low]) and labels in braces ({bug, infra}) where set. Use priority to convey what matters most among in-progress and next-up work, and use labels to distinguish feature work from maintenance/bug work. Tasks with no priority or labels simply omit them — that is not a signal.
- A task may carry a short description line beneath it. Treat it as a raw, possibly-stale seed: use it to understand what the task actually entails, but do not reproduce its wording verbatim and do not trust it over more recent signals. The title is the authoritative name.
...
What's next: The committed but not-yet-started work (the "Next up" items), by project. Lead with the higher-priority items. This is the near-term plan, not speculation — describe what is queued, not when it will land. Skip if there is no next-up work.
