...
## Stay In Your Lane (phase hand-off contract)

Each phase produces its OWN artifact and hands off to the next — it does not do the next phase's job. The generated prompt must direct the consumer to produce **only the current action's deliverable**, and must NOT pre-write the deliverable of a downstream phase. This bounds the Grounding Rule above: restating relevant upstream context is correct, but reproducing or expanding it into the next phase's output is not.

- A **research** prompt ends at findings + a recommended approach + the Surface Assessment. It must NOT contain the implementation plan — the enumerated file-by-file change list is the plan phase's job.
- A **plan** prompt enumerates surfaces, any dependency arrows, and the session-fit answer. It must NOT contain implementation-level steps or written-out code — that is the implementation phase's job.
- **Scale the output to the TASK, not to the length of the context you were handed.** When deep upstream material is present in the Task Context, route and summarize it; do not let its depth set the size or shape of this phase's output. A small task gets a small prompt even when the discussion history is long.
