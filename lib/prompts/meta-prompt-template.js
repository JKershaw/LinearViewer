...
- **Implementation prompts** must remind the agent to isolate behaviors that converge on the same function or state, and to test not just the intended effect of each change but also that no unintended side effects occur.
- **Review prompts** must verify that tests cover interactions between features that share code paths, not just each feature in isolation.
