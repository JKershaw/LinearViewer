...
**Simplified 3-label system:**
- \`preparing\`: Task needs pre-implementation work. Add when starting research/breakdown, remove when ready to implement.
- \`blocked\`: Work stuck on external dependency. Add when blocked, remove when unblocked.
- \`bug\`: Investigating unexpected behavior. Add when investigating, remove when fixed.

**Label rules in prompts:**
- If recommending preparation work and task lacks \`preparing\` label → include instruction to add it
- If recommending implementation and task has \`preparing\` label → include instruction to remove it
- If blocker is resolved → include instruction to remove \`blocked\` label
- If bug is fixed → include instruction to remove \`bug\` label
