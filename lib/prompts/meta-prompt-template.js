...
### Step 0: The substantive work here is already complete — recommend \`review\` (or \`close-out\` once review has approved)

There is no open (non-terminal) child to descend into, and the task's own work is already finished${isTerminal ? `: its state is already a terminal state (Done / Canceled / Duplicate)` : `: the task itself is still open, but all ${subtaskCount} of its subtasks are in a terminal state (Done / Canceled / Duplicate), so the only work left is this task's own close-out`}. Closing out a finished task is a TWO-step sequence — pick the step that has NOT happened yet:
...
- **Review has already recorded an Approve (or Approve — conditional) verdict and a ledger, but the work is still unmerged / not Done → recommend \`close-out\`, NOT another \`review\`.** This is the inverted stuck-review signal: an Approve on record with the code unchanged means the next action is the ledger-gated close — discharge or explicitly accept each ledger item, then merge, set Done, post the summary, file follow-ups. \`close-out\` owns the merge and the Done transition; \`review\` never performs them.

\`close-out\` requires positive evidence that a review actually ran — a comment where a review recorded a verdict. A rich, detailed, or complete-looking description is not that evidence, and neither is the work merely looking done. **And a \`bug\`'s own investigation commentary is NOT a review verdict:** a \`bug\`-labelled task posts its own rich analysis — a \`Root cause CONFIRMED\` / findings write-up, a class-check, a stepper run-summary — which can read review-ish, but it is the author's own diagnosis of the problem, not a fresh-eyes review that recorded a verdict on the deliverable; do NOT count root-cause, findings, class-check, or run-summary comments as review evidence. Only an actual \`review\` verdict on the trail — a comment recording an explicit Approve / Request Changes / Needs Discussion (typically with the \`### What CI Did Not Prove\` ledger) — authorizes \`close-out\`; if no such review comment is on the trail, the review has not happened — recommend \`review\`. When the evidence is ambiguous, default to \`review\`.

**Cannot-close branch:** \`review\` authorizes the close and \`close-out\` performs it; the close can only proceed when CI is green and the work is ready to merge. If the comments already show the work landed but CI is red, or that verifying it surfaced a blocker that must be fixed first, do NOT keep routing to \`review\` or \`close-out\` — route instead via Step 2 to the blocker (\`blocked\`/\`bug\`/\`plan\`/\`implementation\`) that resolves it; the generated review prompt files or links that blocker as \`blocks\`, and the original is Approved on a later \`review\` and closed by \`close-out\` once the blocker is Done and CI is green.
` : ''}
...
- Implementation landed AND \`review\` has already recorded an Approve (or Approve — conditional) verdict with its \`### What CI Did Not Prove\` ledger, but the work is still unmerged / not Done → Recommend \`close-out\` — the ledger-gated finish (discharge or explicitly accept each ledger item, then merge, set Done, post the summary, file follow-ups), NOT a repeated \`review\`. An Approve on record with the code unchanged is the signal that the next step is the close, not another review pass. (Counterpart to the Stuck-review signal above: requested-changes-and-unchanged → the fix; approved-and-unchanged → \`close-out\`.) \`close-out\` requires positive evidence that a review actually ran — a comment where a review recorded a verdict. A rich, detailed, or complete-looking description is not that evidence, and neither is the work merely looking done. **And a \`bug\`'s own investigation commentary is NOT a review verdict:** a \`bug\`-labelled task posts its own rich analysis — a \`Root cause CONFIRMED\` / findings write-up, a class-check, a stepper run-summary — which can read review-ish, but it is the author's own diagnosis of the problem, not a fresh-eyes review that recorded a verdict on the deliverable; do NOT count root-cause, findings, class-check, or run-summary comments as review evidence. Only an actual \`review\` verdict on the trail — a comment recording an explicit Approve / Request Changes / Needs Discussion (typically with the \`### What CI Did Not Prove\` ledger) — authorizes \`close-out\`; if no such review comment is on the trail, the review has not happened — recommend \`review\`. When the evidence is ambiguous, default to \`review\`.
