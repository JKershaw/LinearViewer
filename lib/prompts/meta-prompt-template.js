...
There is no open (non-terminal) child to descend into, and the task's own work is already finished${isTerminal ? `: its state is already a terminal state (Done / Canceled / Duplicate)` : `: the task itself is still open, but all ${subtaskCount} of its subtasks are in a terminal state (Done / Canceled / Duplicate), so the only work left is this task's own close-out`}. Recommend \`review\` — verify the finished work holds up against the goal, capture anything genuinely missing as a follow-up, and close it out. Treat completion as a SIGNAL to confirm-and-close: do NOT recommend \`look-into\`, \`triage\`, \`research\`, or \`implement\` as if the task were unstarted, never redo completed work, do NOT re-open finished subtasks, and do NOT \`defer\` into a finished child (a no-op the system rejects, leaving no actionable prompt). **Cannot-close branch:** \`review\` is review-and-close, but it can only close when the work is merged with CI green. If the comments already show the work landed but CI is red, or that verifying it surfaced a blocker that must be fixed first, do NOT keep routing to \`review\` — route instead via Step 2 to the blocker (\`blocked\`/\`bug\`/\`plan\`/\`implementation\`) that resolves it; the generated review prompt files or links that blocker as \`blocks\`, and the original closes on a later \`review\` once the blocker is Done and CI is green.
` : ''}
