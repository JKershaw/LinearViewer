...
- Implementation landed AND \`review\` has already recorded an Approve (or Approve — conditional) verdict with its \`### What CI Did Not Prove\` ledger, but the work is still unmerged / not Done → Recommend \`close-out\` — the ledger-gated finish (discharge or explicitly accept each ledger item, then merge, set Done, post the summary, file follow-ups), NOT a repeated \`review\`. An Approve on record with the code unchanged is the signal that the next step is the close, not another review pass. (Counterpart to the Stuck-review signal above: requested-changes-and-unchanged → the fix; approved-and-unchanged → \`close-out\`.)
