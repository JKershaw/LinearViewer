 ...
 * @param {boolean} [params.featureFlags.linearMcp=true] - Include Linear MCP references
...
export function buildMetaPromptTemplate({
  ...
  const useMcp = featureFlags.linearMcp !== false;
  const useGitWorkflow = featureFlags.featureBranches === true;

  const toolIntro = useMcp
    ? 'Generate a tailored prompt they can use with Claude Code that has Linear MCP integration.'
    : 'Generate a tailored prompt they can use with Claude Code.';
...
4. **Update Linear**: [How to update Linear when done - add comment, change status/labels, etc. via Linear MCP]
...
- Push branch and create a pull request when implementation is complete` : ''}
