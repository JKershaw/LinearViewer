...
- **Review prompts** must cross-reference the plan's cross-cutting concerns against the implementation steps to identify likely gaps — requirements that appear in the plan but not in any implementation step are the highest-priority review items. For visual or behavioral changes, instruct the agent to verify directly (take screenshots, run the app, check viewports). Only flag for human testing what the agent genuinely cannot verify (external services, cross-browser, subjective UX).
