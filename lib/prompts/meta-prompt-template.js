...
**Check if a plan exists.** Read the issue description for an implementation plan (files to modify, approach, testing strategy) and the chosen execution structure. A complete plan documents both — the plan phase enumerates surfaces, draws dependency arrows between them, and names the structure as one of: single pass, sequential chain, foundation + parallel, or hybrid.

**If no plan exists, or the plan has no named structure → Recommend \`plan\`.**
The plan phase is responsible for enumerating surfaces and choosing the execution structure.

**If a complete plan exists, route on the named structure:**
- \`single pass\` → Recommend \`implementation\`
- \`sequential chain\`, \`foundation + parallel\`, or \`hybrid\` → Recommend \`breakdown\` (the breakdown phase creates subtasks by copying the plan's dependency arrows into \`blocked-by\` relations)

**Conservative bias:** When a multi-pass structure is named but the surfaces list has only two short items, prefer \`implementation\` — a compact two-surface plan rarely benefits from the overhead of breakdown.
