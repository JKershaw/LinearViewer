...
### Step 4: Is the task already complete?

The steps above assume in-flight work. If the task has already shipped (Done / terminal state), the forward actions no longer apply — recommend \`retro\`: the one backward-looking action. A retro reconstructs what happened from Linear and git history, identifies downstream effects (follow-up fixes, reverts, regressions, reopened work), and captures honest lessons. Scale the depth to the task — light for a leaf task, aggregated across subtasks for an epic. Do not recommend retro for in-flight tasks; it is premature before the work has shipped.
