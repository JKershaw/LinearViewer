...
There is no open (non-terminal) child to descend into, and the task's own work is already finished${isTerminal ? `: its state is already a terminal state (Done / Canceled / Duplicate)` : `: the task itself is still open, but all ${subtaskCount} of its subtasks are in a terminal state (Done / Canceled / Duplicate), so the only work left is this task's own close-out`}. Recommend \`review\` — verify the finished work holds up against the goal, capture anything genuinely missing as a follow-up, and close it out. Treat completion as a SIGNAL to confirm-and-close: do NOT recommend \`look-into\`, \`triage\`, \`research\`, or \`implement\` as if the task were unstarted, never redo completed work, do NOT re-open finished subtasks, and do NOT \`defer\` into a finished child (a no-op the system rejects, leaving no actionable prompt). **Already-approved branch — recommend \`close-out\`, not another \`review\`:** if the comments already show a \`review\` on record with an **Approve** verdict (with its **What CI Did Not Prove** ledger) AND the work is still unmerged / not yet Done, the remaining work is the irreversible close, not re-reviewing. Recommend \`close-out\` — it discharges the ledger, then merges, sets Done, posts the summary, and files remaining follow-ups. Routing to \`review\` again here only repeats a verdict already given. **Cannot-close branch:** \`review\` is the pre-close gate that authorizes the close (the \`close-out\` step then performs the merge and the Done transition); it can only Approve when CI is green and the work is ready to merge. If the comments already show the work landed but CI is red, or that verifying it surfaced a blocker that must be fixed first, do NOT keep routing to \`review\` — route instead via Step 2 to the blocker (\`blocked\`/\`bug\`/\`plan\`/\`implementation\`) that resolves it; the generated review prompt files or links that blocker as \`blocks\`, and the original is Approved and closed on a later \`review\` → \`close-out\` once the blocker is Done and CI is green.
` : ''}
...
- **Review prompts** must: (1) cross-reference the plan's cross-cutting concerns against the implementation steps to identify likely gaps — requirements not addressed in any implementation step are the highest-priority review items, (2) for visual or behavioral changes, instruct the agent to verify directly (take screenshots, run the app, check viewports), (3) only flag for human testing what the agent genuinely cannot verify (external services, cross-browser, subjective UX), (4) assess whether tests cover the right *level* — for behavior crossing module boundaries, user-facing flows, or integration surfaces, instruct the agent to verify higher-level tests like e2e or integration are present *where appropriate*, not only unit tests with mocks; determine the right level from the change itself rather than enforcing a fixed rule, (5) include a class check before approving the close: ask whether the verified work is one instance of a class with unhandled siblings (the same bug or gap under a different name, in a parallel code path, or split across server and client); if so, name the class and record the instances as a review finding rather than expanding the task — a genuinely isolated change is a valid result, (6) write a **What CI Did Not Prove** ledger before the verdict — a list, under that exact heading, of every claim the deliverable depends on that the green CI run does not actually exercise (an external contract CI can't reach, a producer that must emit an input this change now consumes, a user-reachable entry path, a sibling on a parallel surface, anything only a human can confirm), each with how it can be discharged; a self-contained change is expected to be empty and must say so with the explicit single line \`CI covers the deliverable; ledger empty.\` (do not manufacture doubt about a clean refactor). The ledger is carried in the review summary comment and is the handoff the \`close-out\` step consumes, (7) end with an explicit verdict (Approve / Request Changes / Needs Discussion) that is conditional on the ledger: a non-empty ledger forces **Approve — conditional on close-out discharging the ledger**, never a bare Approve; only an empty ledger yields a plain Approve. Review AUTHORIZES the close, it does not perform it: the prompt must instruct the agent to confirm CI is green on the PR before approving, and must NOT instruct it to merge, to mark the task Done, or to verify a merge that has not happened yet (the Approve is the close's precondition) — the merge, the Done transition, posting the summary, and filing remaining follow-ups happen AFTER approval and belong to the \`close-out\` step (run by the orchestrator or a human). When the work has landed but CI is red for a real failure, or verifying it surfaces a hidden blocker that must be fixed first, the prompt must NOT loop back into another review — instruct the agent to create a new Linear ticket (or link an existing open one) as \`blocks\` the current task, name that blocker as the next action (\`bug\`/\`plan\`/\`implementation\`, since a \`blocks\` relation alone does not make the engine descend), and leave the original open until the blocker resolves and CI is green (review re-runs then, Approves, and \`close-out\` performs the final close); keep this closure blocker distinct from plan's prerequisite-refactor subtask.
- **Close-out prompts must** own the irreversible finish that review is forbidden from performing, and gate it on review's ledger. The prompt must instruct the agent to (1) read the latest review summary comment and locate its **What CI Did Not Prove** ledger; (2) treat a missing or unparseable ledger as a BLOCK — do NOT merge or set Done, route back to \`review\` to produce it (a missing ledger is never the same as an empty one); (3) treat only the explicit \`CI covers the deliverable; ledger empty.\` line as an empty ledger that passes through cheaply with no per-item discharge; (4) for a non-empty ledger, discharge every item before any irreversible action — each item cleared only by cited evidence or an explicit human acceptance that NAMES the exact precondition it exercised, with green CI never counting as evidence for a ledger item (the item exists because CI cannot reach it); (5) only on an all-clear gate, re-confirm CI green on a fresh read of the exact commit, then merge, set the task Done, post the closing summary (merge + how each item was discharged + final state), and file remaining review findings / non-blocking class siblings as routed, linked follow-up tasks; (6) always name the next action when the gate blocks (back to \`review\` for a missing ledger, or the \`bug\`/\`plan\`/\`implementation\` that discharges a failing item) so the close never stalls without an owner.
