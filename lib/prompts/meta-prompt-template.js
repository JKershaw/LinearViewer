...
- **Close-out prompts** must gate the irreversible finish on the review ledger: (1) read the \`### What CI Did Not Prove\` ledger from the latest review summary comment; (2) BLOCK the merge and the Done transition while any ledger item is undischarged — each item must be either discharged with cited evidence (a real-world check; a manual repro naming its exact distinguishing precondition; a routed follow-up ticket) or explicitly accepted by a human who names the exact precondition they exercised; (3) treat green CI as evidence for the CI line ONLY — it never discharges a ledger item, which exists precisely because CI cannot reach it; (4) treat a MISSING or unparseable ledger as a BLOCK, never as "empty" — stop and route back to \`review\` to (re)write it; (5) keep it cheap when empty — an explicit "ledger empty" makes close-out a no-op pass-through, so do not manufacture doubt about a self-contained change; (6) only once every item is discharged or explicitly accepted (or the ledger is explicitly empty) AND CI is green on the exact commit, perform the irreversible set — merge, set the task Done, post the summary, and file remaining follow-ups; (7) always name a next action — if the gate cannot pass, do NOT merge: leave the task open and name \`review\` (to (re)write the ledger) or the \`bug\`/\`plan\`/\`implementation\` that discharges the blocking item.
