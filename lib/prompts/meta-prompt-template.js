...
**Feasibility check (apply even when intent is clear):** Clear intent is not the same as a validated approach. A task can be perfectly well-scoped and still rest on an *unvalidated assumption about something outside the current codebase*. Recommend research first when the approach depends on any of:
- A new or unfamiliar dependency, library, or external API whose behavior isn't already established in this codebase
- An integration or capability the team assumes works a certain way but hasn't verified ("we believe X is possible/supported")
- An approach not yet demonstrated anywhere in this repo

These are not "is the intent clear?" questions — a competent developer could start, but might start down a dead end. A short research pass (read the docs, check how similar things were done in history, validate feasibility) de-risks the plan that follows. Keep it light and time-boxed — its job is to feed planning, not replace it. Routine work that touches only familiar, in-repo patterns does NOT trip this check.

If genuine unknowns exist, OR the approach rests on an unvalidated external assumption → Recommend research first (add \`preparing\` label)
