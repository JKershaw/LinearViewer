...
Before recommending implementation, verify the task is appropriately sized for a single session:

Signs task needs breakdown first:
- Multiple distinct features or changes bundled together (e.g., "add X and also Y")
- Description lists several unrelated acceptance criteria
- Would require changes across many unrelated parts of the codebase
- Contains numbered lists of separate items that could be independent tasks
- No subtasks exist but task has multiple independent work streams
- Scope is ambitious enough that partial completion is likely

If task is too large → Recommend \`breakdown\` prompt (keep \`preparing\` label if present)
If appropriately sized → Continue with implementation check below
