...
**DeferTo:** [REQUIRED if and only if the action is \`defer\` — the exact identifier of the child task to descend into, e.g. ${identifier.split('-')[0] || 'ABC'}-123. This line is parsed structurally to trigger the descent, so emit the bare identifier and nothing else. OMIT this line entirely for every other action.]

## Prompt
[The complete prompt text following the structure above. EXCEPTION: when the action is \`defer\`, output nothing here — leave the Prompt section empty. The prompt is generated once, later, at the terminal actionable node, never on a node you are deferring past.]`;
}
