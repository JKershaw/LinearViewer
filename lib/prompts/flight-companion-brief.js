 ...
 *
 * The model behaves differently on each and cannot tell them apart from the
 * message alone — an auto-wake turn's stand-in user text reads exactly like a
 * real question, and a boot's synthetic "Start" text reads exactly like a
 * quiet check-in unless told otherwise. Getting the auto-wake case wrong is
 * what produces a chatty bubble on a silent tick, which LIN-2443 AC1 forbids.
 * Getting the boot case wrong (LIN-2622) is the opposite failure: falling
 * through to the auto-wake wording below would tell a turn a human just
 * started, by hitting Start, to apply the SILENT speak-only-for-a-reason
 * rule — backwards for the one turn that must always give the full readout.
 *
 * @param {'user-initiated'|'auto-wake'|'boot'|string} turnKind
 * @returns {string}
 */
export function describeTurnKind(turnKind) {
  if (turnKind === 'user-initiated') {
    return 'THIS TURN: the human just asked you something. Answer it.';
  }
  if (turnKind === 'boot') {
    return 'THIS TURN: the human just hit Start — the first orient of this session. Always speak: a short ' +
      'hello, then orient using your tools, then the full readout in the shape below. The speak-only-for-a-' +
      'reason rule below is for a SILENT check-in tick, not this one.';
  }
  return 'THIS TURN: a check-in tick — nobody typed anything. Apply the speak-only-for-a-reason rule ' +
