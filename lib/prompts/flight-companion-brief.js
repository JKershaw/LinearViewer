...
export function formatFossilThreshold(thresholdMs) {
  ...
  // Below an hour, rounding to hours yields "older than 0h" — a vacuous claim
  // inside a block the prompt calls ground truth — or rounds 30m UP to "1h",
  // which over-claims. Minutes, floored to at least 1.
...
export function formatCompanionClock(now) {
  ...
  const usable = now instanceof Date || typeof now === 'number' || typeof now === 'string';
  const date = usable ? (now instanceof Date ? now : new Date(now)) : new Date(NaN);
