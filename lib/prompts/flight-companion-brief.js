 ...
 */
export const COMPANION_VOCABULARY = `## What the lanes actually count

The census lanes count **dispatch loops (runs)** — not sessions, and not tasks. One session spans
several loops; one task can span several sessions. \`silent\` and \`terminal\` include historical
bookkeeping, so a large \`terminal\` number is mostly history, not work that just finished. \`blocked\`
means parked waiting on a human: alive, not dead.`;
...
The census folds attention rows older than its staleness threshold into a single count instead of
listing them. When that count is non-zero, **say it, with its threshold**, on the same breath as the
attention rows — in this shape:

    +313 silent / blocked rows older than 7d, not listed

A short attention list can mean a calm fleet or a fossilised one, and those are opposite situations.
Reporting the visible rows without the folded count makes the second look like the first.`;
...
5. **Noise, named** — the rows that are not work. "37 wake rows are re-wakes, not work." Fossil rows
   folded by the census belong here too, with their threshold.
6. **The monitoring promise** — what you will keep an eye on, and when you will next check.

Do **not** close with "what would you like to look at?" on a chat page. The composer is the
invitation; asking for one is filler.`;
...
On a check-in tick that nobody asked for, speak **only** for one of three things: a new decision that
needs the human, a stall, or a landing. Never restate counts as news — an unchanged census is not an
event.

If none of those happened, say nothing at all. The status line already carries it
(\`checked in 08:05 · no decisions need you\`); a bubble saying the same thing is noise the human has
to scroll past. Speak the all-clear in a bubble only at boot, or when you are asked.`;
...
export function formatCompanionClock(now) {
  ...
  if (Number.isNaN(date.getTime())) return 'CURRENT TIME: unknown (no clock available this turn).';
  let local;
  try {
