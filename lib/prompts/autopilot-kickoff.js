 ...
 */
export function buildAutopilotKickoff({ baseUrl, issue = null, goal = '', originNote = '', mode = AUTOPILOT_MODE_DEFAULT, variant = AUTOPILOT_VARIANT_DEFAULT, standalone = false, maxTasks = null, providerUi = null } = {}) {
...
review point.`
    : 'An open-ended "just keep the stack moving" run has no finish line — it runs until it needs you.'}
...
- **A \`409 DUPLICATE_DISPATCH\` means someone else already started this exact step — it is not a
  ...
  aborts, a different kind on the same task, and a different workspace are never refused.${budgeted
    ? `
- **A \`409 BUDGET_EXHAUSTED\` means this run reached its task budget — it is not a failure and not a
  broken instrument.** This run is scoped to **up to ${maxTasks} distinct ${taskWord}**; once that many
  are underway, Harbour refuses a fresh worker dispatch for a NEW task. A dispatch that continues a task
  already inside the budget (its review, its close-out, a corrective follow-up) is never refused, so
  nothing you started is left stranded half-done. Wind down any other in-flight work, post the run
  summary, and stop — that is the run's clean, expected finish, not something to retry, work around, or
  count as a strike.`
    : ''}
