 ...
 * - `coordinator`: the head/coordinator disposition (LIN-813). Instead of driving
 *   a task's *steps* itself, Autopilot sits ABOVE individual tasks and dispatches
 *   one task-altitude CHILD autopilot per task, each driving its own steps/beats in
 *   its own session and reporting back up the chain via the subscribe wake. Serial
 *   in this first slice (one child at a time). See buildCoordinatorDisposition().
 *
 ...
 */
export const AUTOPILOT_VARIANTS = ['standard', 'stepper', 'coordinator'];
 ...
 * `subscribe: true` and the orchestrator STANDS BY for the up-chain wake instead
 ...
 * `AWAITING_FOLLOWUP` so the next beat lands in-session; `subscribe` wires only
 * the up-chain wake, not the hold, so both flags are required for the warm drip.
...
judging each piece. Drive **one warm session** through ordered **beats**:
...
3. **Beat 1 is fresh — capture its dispatch id as \`ROOT\`.** Send it with a plain
   \`POST ${proxyBase}/dispatch\` (your \`sessionId\`, **\`subscribe: true\`**, **\`waitForFollowUps: true\`**,
   ...
   (always beat-1's id — a stable anchor, never the previous beat's), **\`force: true\`**, **\`subscribe: true\`**,
   **\`waitForFollowUps: true\`**, the **same \`target\`** as beat 1, \`sessionId\` = your own id. **\`subscribe: true\` and
   \`waitForFollowUps: true\` are the two halves of the warm drip — set BOTH on every beat.** They are
   orthogonal: \`subscribe: true\` is the up-chain **wake** — it declares the up-chain edge so the beat's
...
4. **Stand by for the push — do NOT long-poll.** After you dispatch a beat, **stop and stand by.** Because
   the beat is \`subscribe: true\`, you are **woken automatically** the moment it reaches a stop boundary —
...
5. **Judge AND challenge every beat before advancing — do not rubber-stamp.** Interrogate the recap: real
   ...
   **corrective \`followUpTo: ROOT\`** (also labelled \`beat N/M\`, also \`subscribe: true\`) and re-judge —
...
- **\`followUpTo\` is always \`ROOT\`** (beat-1's id); **\`force: true\`, \`subscribe: true\`, and
  \`waitForFollowUps: true\` on every beat** (\`subscribe\` = the up-chain wake, \`waitForFollowUps\` = the
...
/**
 * The coordinator orchestrator disposition (LIN-813) — the HEAD autopilot that
 * sits at user altitude and dispatches ONE task-altitude CHILD autopilot per task,
 * instead of driving each task's steps itself. Each child runs its own
 * research→plan→impl→review→close-out (with beats) in ITS OWN session/context, and
 * reports back up the chain on the same push rails every dispatch uses: the head
 * dispatches the child `subscribe: true` with the head's own session id as the
 * child's `sessionId`, so when the child reaches a terminal/`[pending]` boundary
 * the head is WOKEN with the outcome (LIN-826) rather than polling.
 *
 * The win is context isolation: an epic with N tasks becomes a head that dispatches
 * one focused autopilot per task, each carrying only its own task's context, while
 * the head holds the cross-task altitude — no single session drowns in every step
 * of every task. This first slice is SERIAL (one child at a time); parallel
 * fan-out / fan-in (LIN-874), nested Observation rendering (LIN-875), and
 * cross-autopilot messaging (LIN-876) are deferred follow-ups.
 *
 * Composed into the kickoff only when `variant: 'coordinator'`; it is the
 * authoritative loop for a coordinator run and supersedes the standard per-step
 * "How a loop goes" worker-dispatch loop above it. Kept here (gated on variant)
 * rather than in the shared handbook so a standard run's kickoff stays
 * byte-identical and never carries coordinator prose it won't use.
 *
 * @param {string} proxyBase - `${baseUrl}/api/proxy`.
 * @returns {string} The coordinator disposition section (Markdown).
 */
function buildCoordinatorDisposition(proxyBase) {
  return `## You're running as the COORDINATOR

This run uses the **coordinator** variant. You sit at **user altitude, above individual tasks** —
you do **not** drive a task's own steps and you do **not** run the standard "How a loop goes"
recommend-and-dispatch loop above for the work itself (that section's *instruments, halt rules, and
the four lines still hold*). Instead you dispatch **one task-altitude CHILD autopilot per task** and
let it drive that task's whole arc — research→plan→implementation→review→close-out, with beats — in
**its own session and context**, reporting back up to you when it's done.

The point is **context isolation**: an epic with several tasks becomes a head that hands each task to
a focused autopilot carrying only *that* task's context, while you hold the cross-task altitude. No
single session grinds every step of every task through one window.

Drive it like this:

1. **Orient and pick the next task.** Same precedence policy as the standard loop — an explicit human
   goal, else the top of the stack. Say what you picked and why, in a line. You pick **one** task at a
   time (this slice is serial).
2. **Dispatch a CHILD autopilot for that task — not a worker step.** \`POST ${proxyBase}/autopilot/kickoff\`
   with \`{ issueIdentifier: "<the task>", target, sessionId: <YOUR own session id>, subscribe: true }\`.
   - \`sessionId\` is **your** session id (from the *Your autopilot session id* block at the very end of
     this prompt) — it makes you the child's up-chain **wake target**.
   - \`subscribe: true\` declares that up-chain edge, so the child's terminal report is pushed back to
     **you**.
   - The response is a header: note the child's returned \`id\` (that is the *child's* own session id,
     for the sub-workers **it** fans out — distinct from the \`sessionId\` you passed, which points at
     you). You never see or hold the child's prompt body — it runs its own scoped autopilot.
3. **Stand by for the up-chain wake — do NOT poll.** Because you dispatched the child \`subscribe: true\`,
   you are **woken automatically** the moment it reaches a stop boundary — \`done\` / \`failed\` /
   \`blocked\`, **and** \`[pending]\` ("my part's done, the task isn't"), injected back into this session
   within seconds. **Do not build a watch loop, do not long-poll, do not run a \`run_in_background\` wait.**
   The hold is automatic once you stop without an outstanding background wait. (Same push the standard
   loop uses to learn a worker finished — here it carries the child autopilot's outcome up to you.) The
   one judgment the push can't make: a child that goes **silent without ever reaching a boundary** emits
   no wake, so if **~30 min** pass with zero new activity, send a one-line liveness \`followUpTo\` nudge to
   the child's id (carry your \`sessionId\`), then re-dispatch fresh if it can't resume — but do **not**
   turn that exception into a standing poll.
4. **Judge the child's report on evidence, then advance.** When the wake arrives, read the child's
   closing lines and cross-check the deliverable the same way the standard loop does (step 4 above):
   fetch the \`[evidence]\`, confirm the task's real artifact exists (a merged PR, green CI, Done state).
   A clean **complete** → move to the next task (step 1). A \`[pending]\` / \`blocked\` / \`failed\`, or
   evidence that contradicts the claim → decide: re-dispatch the child for the same task, or **pause for
   the human** (a direction question, a blocker you can't clear, anything that's theirs). Keep your
   altitude — hold task headers, not task detail; the child holds its task's context.
5. **Serial only, this slice.** Dispatch the next child **after** the current one has reported and been
   judged. Running 2A/2B/2C concurrently (parallel fan-out and fan-in) is a deliberate follow-up
   (LIN-874), as are nested Observation branches/nodes (LIN-875) and cross-autopilot messaging (LIN-876)
   — do not build them here.

**Hard rules (each preserves the push substrate):**
- **One child autopilot per task; you stay above the tasks.** You dispatch autopilots, not worker
  steps — the child owns its own research→…→close-out arc.
- **Every child dispatch carries \`sessionId\` = your own id AND \`subscribe: true\`** — that pair is the
  up-chain report edge. Omit either and the child cannot wake you.
- **Stand by for the push — never hand-roll a long-poll, never idle, never one long foreground loop.**
  The child's wake (incl. \`[pending]\`) reaches you on the push rails; trust it.
- **Serial in this slice.** One child at a time; parallel is a filed follow-up, not a freelance
  extension.`;
}
 ...
 * @param {('standard'|'stepper'|'coordinator')} [params.variant='standard'] - Run
 *   variant, an axis orthogonal to `mode`. `standard` is the normal orchestrator
 *   (output unchanged); `stepper` swaps in the warm single-session beat-stepping
 *   disposition (LIN-791); `coordinator` swaps in the head-dispatches-one-child-
 *   autopilot-per-task disposition (LIN-813).
 * @returns {string} The kickoff prompt text.
 */
export function buildAutopilotKickoff({ baseUrl, issue = null, goal = '', mode = AUTOPILOT_MODE_DEFAULT, variant = AUTOPILOT_VARIANT_DEFAULT } = {}) {
  ...
  const coordinator = variant === 'coordinator';
...
${firstAct}`;
  ...
  // separators (byte-identical); `stepper` and `coordinator` each insert ONE
  // disposition section between the standard guide and the snapshot so the four
  // lines + instruments still read first, then the variant loop supersedes the
  // worker-dispatch loop for the run. The variants are mutually exclusive (one
  // `variant` value), so at most one section is added.
  const sections = [intro, manual, guide];
  if (stepper) sections.push(buildStepperDisposition(proxyBase));
  else if (coordinator) sections.push(buildCoordinatorDisposition(proxyBase));
