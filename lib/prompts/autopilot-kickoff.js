 ...
 * of hand-rolling a long-poll. Because `[pending]` now fires a wake (a beat's
...
3. **Beat 1 is fresh — capture its dispatch id as \`ROOT\`.** Send it with a plain
   \`POST ${proxyBase}/dispatch\` (\`target: "web"\`, your \`sessionId\`, **\`subscribe: true\`**, label
   ...
   \`target: "web"\`, \`sessionId\` = your own id. **\`subscribe: true\` on every beat is what puts you on
   push rails:** it declares the up-chain edge so the beat's stop boundary wakes you — and that includes
   the beat's holdable \`PENDING\` ("my part's done, the task isn't"), not only a clean \`done\`/\`failed\`.
4. **Stand by for the push — do NOT long-poll.** After you dispatch a beat, **stop and stand by.** Because
   ...
   its **own** side — the runner holds it \`AWAITING_FOLLOWUP\` at \`PENDING\` — so when the wake arrives and
...
- **\`followUpTo\` is always \`ROOT\`** (beat-1's id); **\`force: true\` and \`subscribe: true\` on every beat.**
