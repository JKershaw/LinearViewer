...
3. **Watch.** Long-poll \`GET /dispatch/{id}?wait=50\` in a plain loop — **no \`sleep\`, no backoff,
   ...
   Read the **\`status\` field** for the terminal signal — don't read prose for it. Each response also
   carries **\`reason\`** (\`change\` | \`timeout\` | \`terminal\`) and **\`waitedMs\`** so you can tell *why*
   it came back: \`reason: "timeout"\` with \`waitedMs\` near 50000 means the call **held the whole window
   and nothing changed** — the worker's just quiet, not a fast empty return to spin on. (During a long
   silent stretch the snapshot body is byte-identical call to call; \`reason\`/\`waitedMs\` are how you
   know the hold is doing its job rather than short-circuiting — so expect roughly one ~50s call per
   quiet minute and trust it. Owning the wait yourself with hand-rolled sleeps is the old failure mode:
   backoff oversleeps and you sit idle for minutes after a task has already landed. \`?wait\` removes any
   need for it.) (If you poll in a shell
...
Knowing your instruments up front is what keeps a hiccup from becoming a halt you didn't need. A few
behave in ways worth expecting, so you recognise them instead of debugging them: the watch poll holds
its connection open up to ~50s and returns the moment something changes (a long quiet call is the hold
working, not a hang); a shell loop avoids a variable named \`status\` (zsh reserves it — use
\`dispatch_status\` or run under \`bash\`); the proxy rate-limits at 60 requests/minute, so space your
polls; and \`/recommend\` can run past 25s behind whitespace keepalives that \`JSON.parse\` ignores, so
don't set a short client timeout on it. Recognising one of these costs a second. The halt is for the
breakage that *isn't* on a list you were handed — that's the moment to surface and wait, rather than
drop to the byte level taking it apart.

A compounded halt is worth catching too. Uncertainty stacks from three independent places: your
**instruments** (a verb misbehaving), the **world** (a genuinely ambiguous task signal — a red that
might be the new diff or might be pre-existing), and your **own read** (judgment dulled by a long
stretch in the detail). Any one alone the loop handles — retry the instrument, leave the ambiguous
signal for review, trust a later pass to re-examine a tired call. When all three land at once — a
tired read, a flaky instrument, an ambiguous signal — that stacking is itself the halt: hand back
with the three threads named, because the conditions for judging it *well* aren't present, even when
any single thread might be resolvable on its own.
