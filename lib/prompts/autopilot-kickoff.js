 ...
 * long-poll-keep-alive beat-stepping loop distilled from the LIN-788 experiment
 * and the LIN-793 reap diagnosis. Composed into the kickoff only when
 * `variant: 'stepper'`; it is the authoritative loop for a stepper run and
...
3. **Beat 1 is fresh — capture its dispatch id as \`ROOT\`.** Send it with a plain
   \`POST ${proxyBase}/dispatch\` (\`target: "web"\`, your \`sessionId\`, label \`beat 1/M: <label>\`).
   Every *later* beat resumes that same warm session: \`followUpTo: ROOT\` (always beat-1's id — a stable
   anchor, never the previous beat's) **plus \`force: true\`**, \`target: "web"\`, \`sessionId\` = your own id.
4. **Deliver each beat via long-poll inside a \`run_in_background\` task.** Wait with
   \`GET ${proxyBase}/dispatch/{id}?wait=Ns\` *in a backgrounded task on sub-20-min cycles* — never a long
   foreground loop, never idle-on-\`PENDING\`. This is doing two jobs at once: the hold returns within
   seconds of the beat going terminal so the next beat lands **inside the session's warm hold** (in-session,
   no \`--resume\`), **and** the returning background task re-invokes you — the async-wait defer that resets
   the runner's verify-backstop clock so a long, quiet orchestrator isn't reaped as "silent" (the failure
   that killed the run-2/run-3 orchestrators at 32–47 min). **The long-poll background wait IS your
   keep-warm mechanism.**
5. **Judge AND challenge every beat before advancing — do not rubber-stamp.** Interrogate the recap: real
   ...
   **corrective \`followUpTo: ROOT\`** (also labelled \`beat N/M\`) and re-judge — advance only when the beat
   genuinely holds.
6. **Mid-chain \`PENDING\` is a clean advance, not a wobble.** A beat that reports "my part's done but the
   overall task isn't" has completed normally — move to the next beat. Only challenge/hold when **this
   beat's own** work is incomplete or unproven.
...
- **\`followUpTo\` is always \`ROOT\`** (beat-1's id); **\`force: true\` on every resume.**
- **Keep-warm = the long-poll background wait** — never idle on \`PENDING\`, never one long foreground loop.`;
}
...
3. **Watch.** Long-poll \`GET /dispatch/{id}?wait=50\` in a plain loop — no \`sleep\`, no backoff. The
   server holds each request open and returns the *instant* \`status\` transitions or new feedback
   arrives, else at a ~50s cap, so you just call again:
   \`do { r = GET .../dispatch/{id}?wait=50 } while (!terminal(r))\`. Read the **\`status\` field** for the
   terminal signal — not prose. \`reason\` (\`change\` | \`timeout\` | \`terminal\`) and \`waitedMs\` tell you
   *why* it returned: a \`timeout\` with \`waitedMs\` near 50000 means the call held the whole window and
   nothing changed — the worker's just quiet. Quiet has a ceiling, though: track the time since the
   *last new activity* (new feedback or a tool event), not since the last poll — a string of quiet 50s
   holds is healthy, but once **~30 min** pass with zero new activity the hold has stopped meaning
   "working" and started meaning "wedged or dead." Don't poll a silent session forever: send it a
   one-line liveness follow-up (\`followUpTo\` the dispatch id, with your \`sessionId\` carried on it —
   *"still working? report where things stand"*), and if it can't resume (\`[failed] no live session to resume\`) or stays silent after the
   nudge, re-dispatch fresh or hand back. (The poll's known quirks, and the rest of your instruments,
   are under *Your instruments* below — including why \`done\` means "go look," not "finished.")
...
approved, is the human's call — pause and hand back.

The merge is yours, and so is the close that follows it. \`review\` only *authorizes* the close — it
issues the Approve verdict and stops; it never merges or marks the task done. So once you've merged on
green, you finish the job: mark the task done and reconcile the Linear tasks this work touched — sync
the parent and siblings and leave a short summary where one is needed (status/summary reconciliation,
not a re-org of the tree). That post-merge close is the orchestrator's, not the worker's.
...
- **The watch poll holds open ~50s.** \`GET /dispatch/{id}?wait=50\` returns the moment something changes,
  else at the cap — a long quiet call is the hold working, not a hang. Expect ~one 50s call per quiet
  minute and trust it; no \`sleep\`, no backoff. Trust the quiet only up to a ceiling, though: ~30 min
  with zero new activity is a wedged session, not a working one — your cue to nudge then re-dispatch
  (step 3), not to poll on forever.
