...
- The verbs you drive: orient/choose \`GET /stack?view=digest\` (compact one-line headlines —
...
3. **Watch.** Poll \`GET /dispatch/{id}\`. Read the **\`status\` field** for the terminal signal —
   don't read prose for it. (If you poll in a shell loop, don't name the variable \`status\`: zsh
   reserves it as a read-only alias for \`$?\` and the assignment aborts. Use \`dispatch_status\`, or
   run the loop under \`bash\`.) Heartbeats tell you it's alive. Two things have fooled this loop before,
...
(one-line headlines, no full task bodies) — then orient against it under the precedence policy,
