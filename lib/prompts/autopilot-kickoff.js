...
- The verbs you drive: orient/choose \`GET /stack\`, \`GET /recap/{id}\`, \`GET /brief/{id}\`,
  \`GET /recommend/{id}\`; dispatch \`POST /dispatch\` then watch \`GET /dispatch/{id}\`
  (\`GET /dispatch?…\` to list); verify \`GET /issues/{id}\`, \`GET /search\`, plus the artifact
  URLs in \`[evidence]\`.
...
2. **Get the prompt.** \`GET /recommend/{identifier}\` chooses the next *step* and tells you its
   **kind** (planning / research / implementation / review / retro / …). Use its prompt verbatim.
   If \`/recommend\` times out or errors, that's a **halt** (below), not a cue to hand-write your
   own — that's how you'd paper over a broken signal.

3. **Dispatch.** \`POST /dispatch\` with \`{ prompt, promptName, issueIdentifier, target }\`. Note
   the \`id\` and \`kind\`. Watch the kind sequence over a task — it's your cheapest read on health:
   research→plan→impl→review is a task **converging** (good, expected); the same kind repeating is
   **looping**; the kind widening run after run is **sprawling** (worth a flag).
...
5. **Cross-check — the step that earns its keep.** On \`done\`, take the \`[evidence]\` URLs and any
...
task and calling \`GET /recommend/${identifier}\`; loop until it's complete (verified) or you hit a
point that wants the human, then stop. Do not pull other tasks off the stack.`
    : goal && goal.trim()
  ...
  const firstAct = scoped
    ? `**Your first act:** read **${identifier}** (\`GET /issues/${identifier}\`), then \`GET /recommend/${identifier}\`
for the next step. Announce your choice in a line, dispatch, and go. The human is watching.`
