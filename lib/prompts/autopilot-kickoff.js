...
5. **Decide.** A short line for the human, then one of:
   ...
   - **complete** — evidence confirms this task/feature is genuinely done. If you're working a
     ...
     open-ended, move to the next item. Either way, as the parent, **close the spent session on-done**:
     once it's verified-complete with no follow-up expected of it, reap it right then on the existing
     \`abort:true\`/\`abortTo=<session id>\` wire — see *Closing a session, once it's truly spent* in the
     manual. (A **child autopilot** you dispatched is the same rule, not a special case: once you've
     judged its terminal report and advanced, close that spent child on the
     \`abort:true\`/\`abortTo=<child session id>\` wire — an instance of close-on-done. The one session you
     never close this way is one a human may still be in — leave it open.)
...
idempotent straggler sweep for any window still open at run's end, distinct from the close-on-done that
already reaped most of the tree as each session finished (see *Closing a session, once it's truly spent*
in the handbook). Because you close each session on-done, most windows are gone before you get here — the
cascade just catches the stragglers, and a session already closed is a no-op. It's safe because the cascade emits
**plain** aborts and the runner **skips any session a human continued** — a window someone's mid-reply
in comes back \`[skipped] human-continued\` and is left open, not \`[aborted]\` — so an end-of-run sweep
can never slam shut a window someone's still in. **Never add \`force\` to the cascade**: \`force\` is only
for a deliberate *single* abort that overrides that skip, and it would defeat the guard that makes the
sweep safe. Skip this only when you're **pausing for the human** rather than finishing — a hand-back
may want its windows left open.
