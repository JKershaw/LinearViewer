 ...
 * `AWAITING_FOLLOWUP` so the next beat lands in-session, not as a cold
 * `claude --resume`; `subscribe` wires only the up-chain wake, not the hold, so
 * both flags are required for the warm drip. Because `[pending]` now fires a wake (a beat's
...
3. **Beat 1 is fresh — capture its dispatch id as \`ROOT\`.** Send it with a plain
   \`POST ${proxyBase}/dispatch\` (\`target: "web"\`, your \`sessionId\`, **\`subscribe: true\`**,
   **\`waitForFollowUps: true\`**, label
   \`beat 1/M: <label>\`). Every *later* beat resumes that same warm session: \`followUpTo: ROOT\` (always
   beat-1's id — a stable anchor, never the previous beat's), **\`force: true\`**, **\`subscribe: true\`**,
   **\`waitForFollowUps: true\`**, \`target: "web"\`, \`sessionId\` = your own id. **\`subscribe: true\` and
   ...
   *inside* that warm session as an in-session signal. **Omit the hold and the worker finalizes after
   beat 1**, so beat 2 (\`followUpTo: ROOT\`, \`force: true\`) falls back to a cold \`claude --resume\` that
   re-onboards the worker and loses the intra-session memory this whole disposition exists to keep —
   \`force: true\` only suppresses the busy-rejection, it does not create a hold to signal into.
