...
function buildStepperDisposition(proxyBase, standalone = false) {
  const beat3 = standalone
    ? `3. **Beat 1 is fresh — capture its dispatch id as \`ROOT\`.** Send it with a plain
   \`POST ${proxyBase}/dispatch\` (your \`sessionId\`, **\`waitForFollowUps: true\`**,
   ...
   (always beat-1's id — a stable anchor, never the previous beat's), **\`force: true\`**,
   **\`waitForFollowUps: true\`**, the **same \`target\`** as beat 1, \`sessionId\` = your own id.
   \`waitForFollowUps: true\` is the worker-side **hold** — it
  ...
  const beat4 = standalone
    ? `4. **Poll for completion — one background loop, not one call per turn.** After you dispatch
   a beat, run a single background loop over \`GET ${proxyBase}/dispatch/{id}?wait=50\` rather than
   a visible call every turn — plain \`bash\` works anywhere; prefer a wake-on-change tool only if
   your harness has one. Skip \`reason:"timeout"\` iterations and stay silent until a real change; on
   a change, note \`status\` plus the newest \`feedback[].message\`. When feedback shows the beat is
   done (or holding at \`PENDING\`), print the final feedback tail, judge the beat, then advance to
   the next \`followUpTo: ROOT\` + \`force: true\`. The wedged-beat ceiling still applies: ~30 min
   silence → liveness nudge → re-dispatch, and keep calls spaced for the 60 req/min limit.`
  ...
  const hardRules = standalone
    ? `- **\`followUpTo\` is always \`ROOT\`** (beat-1's id); **\`force: true\` and
  \`waitForFollowUps: true\` on every beat** (\`waitForFollowUps\` = the
  worker-side hold — without it beat 2+ falls back to a cold \`--resume\`).
- **Poll for completion with one background loop — never a visible call every turn, never idle on
  \`PENDING\`.** Run a single background \`GET /dispatch/{id}?wait=50\` loop per beat cycle: skip
  \`reason:"timeout"\` iterations, stay silent until a real change or terminal, and print only the
  final feedback tail on terminal. Bounded by the ~30-min ceiling and the 60 req/min limit; a
  harness's wake-on-change tool is fine in place of the loop, never required.`
...
\`GET ${proxyBase}/autopilot/manual\` if you want to re-read a part mid-run.)`;
  ...
  const setupBullet = standalone
    ? `- **Stamp every dispatch with your session id.** Your session id is \`${standaloneSessionId}\`. Pass it as \`sessionId\` on **every** worker dispatch you issue: every \`POST /recommend-and-dispatch\`, every plain \`POST /dispatch\`, and any \`followUpTo\` liveness nudge. (If this prompt was dispatched through the queue, the *Your autopilot session id* block at the very end overrides this value with the true dispatch id.) It's how the work you spawn this run groups into one session — set it once and carry it on all of them.`
  ...
  **session id** — given in the *Your autopilot session id* block at the very end of this prompt.
  ...
  const step3 = standalone
    ? `3. **Poll for completion — one background loop, not one call per turn.** Run a single
   background loop over \`GET ${proxyBase}/dispatch/{id}?wait=50\` after each dispatch — a plain
   \`bash\` loop works on any harness; if yours offers a wake-on-change tool, prefer that instead,
   but don't require one. Each call holds open for up to ~50s and returns on a status change or a
   timeout. Skip \`reason:"timeout"\` iterations — they carry no new heartbeat — and stay silent
   until a real change or a terminal state: on a real change, note only \`status\` and the newest
   \`feedback[].message\`; on terminal (\`done\` / \`failed\` / \`blocked\`), print the final feedback
   tail, stop the loop, then cross-check (step 4) and decide (step 5). Keep the loop bounded: space
   calls for the 60 req/min rate limit, and hold the ~30-min silence ceiling — a worker that goes
   silent without terminating gets a liveness \`followUpTo\` nudge, then a fresh re-dispatch if it
   can't resume. (Name the shell variable \`dispatch_status\`, or run it under \`bash\` — zsh reserves
   \`status\`.)`
