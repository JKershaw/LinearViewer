...
function buildStepperDisposition(proxyBase, standalone = false) {
  ...
  const beat4 = standalone
   ...
   One exception: a beat already woken to you as \`blocked\` is expected to stay silent afterward — that
   silence is a human parked on it, not a wedge; don't nudge or re-dispatch it on this rule, surface it to
   the human so they can unblock it. Otherwise, the wedged-beat ceiling still applies: ~30 min silence →
   ...
   woken to you as \`blocked\` is expected to stay silent afterward — that silence is a human parked on it,
   not a wedge; don't nudge or re-dispatch it on this rule, surface it to the human so they can unblock it.
...
\`GET ${proxyBase}/autopilot/manual\` if you want to re-read a part mid-run.)`;
  ...
  **session id** — given in the *Your autopilot session id* block at the very end of this prompt.
  ...
  const step3 = standalone
   ...
   yours to hold. One exception: a step already woken to you as \`blocked\` is expected to stay silent
   afterward — that silence is a human parked on it, not a wedge; don't nudge or re-dispatch it on this
   rule, surface it to the human so they can unblock it. If **~30 min** pass with zero new activity from a step, stop trusting the silence:
   ...
   **woken automatically** — that outcome is injected back into this session as a follow-up within
   ...
   as \`blocked\` is expected to stay silent afterward — that silence is a human parked on it, not a
   wedge; don't nudge or re-dispatch it on this rule, surface it to the human so they can unblock it. If **~30 min** pass with zero new activity
