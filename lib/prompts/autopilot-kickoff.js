...
emits the aborts, so you never close them one by one. This is the run's **final cleanup**, distinct
from the leave-open default that governs windows *during* the run (see *Closing a session, once it's
truly spent* in the handbook): mid-run a finished window is cheap optionality, but once the run is over
that value has expired, so you sweep the tree closed in one move. It's safe because the cascade emits
