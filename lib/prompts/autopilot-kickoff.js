...
4. **Deliver each beat via long-poll inside a \`run_in_background\` task.** Wait with
   ...
   keep-warm mechanism.** This is *intra-session*: you are long-polling your **own** warm session's
   beats (\`followUpTo: ROOT\` within one session), which is a different edge from the standard loop's
   step-3 child subscription — there the orchestrator dispatches a *separate* child and is **pushed**
   its terminal outcome, so it stands by without polling. Here you are stepping your own warm session,
   so you do wait on its beats; that keep-warm stays, the up-chain poll does not.
