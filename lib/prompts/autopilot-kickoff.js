...
function buildStepperDisposition(proxyBase, standalone = false) {
  ...
  const beat4 = standalone
    ? `4. **Poll for completion.** After you dispatch a beat, poll
   \`GET ${proxyBase}/dispatch/{id}?wait=50\` to watch for its status. When feedback
   shows the beat is done (or holding at \`PENDING\`), read the feedback, judge the
   beat, then advance to the next \`followUpTo: ROOT\` + \`force: true\`. The wedged-beat
   ceiling still applies: ~30 min silence → liveness nudge → re-dispatch.`
...
- **Poll for completion — never idle on \`PENDING\`, never one long foreground
  loop.** Use a single \`GET /dispatch/{id}?wait=50\` call per beat cycle.`
...
wake arrives, and you close each child the moment *its own* report is judged terminal. One stalled child
...
\`GET ${proxyBase}/autopilot/manual\` if you want to re-read a part mid-run.)`;
  ...
  **session id** — given in the *Your autopilot session id* block at the very end of this prompt.
  ...
  const step3 = standalone
    ? `3. **Poll for completion.** Poll \`GET ${proxyBase}/dispatch/{id}?wait=50\` after each dispatch. It holds open for up to ~50s and returns on any status change. When the status is terminal (\`done\` / \`failed\` / \`blocked\`), read the feedback, cross-check (step 4), and decide (step 5). One ceiling: a worker that goes silent without terminating — if ~30 min pass with zero new activity, send a liveness \`followUpTo\` nudge, then re-dispatch fresh if it can't resume.`
...
5. **Decide.** A short line for the human, then one of:
   ...
   - **complete** — evidence confirms this task/feature is genuinely done. If you're working a
     ...
     open-ended, move to the next item. Close the spent session on the existing
     \`abort:true\`/\`abortTo\` wire — see *Closing a session, once it's truly spent* in the manual;
     that's now the default for any session, not a special case. (If what just completed was a
     **child autopilot** you dispatched, once you've judged its terminal report and advanced, close
     that spent child on the same \`abort:true\`/\`abortTo=<child session id>\` wire — one instance of
     the general close-on-done default.)
...
**One last act when the run itself ends.** When the top-level task is *verified* complete and this run
is concluding — close-out landed, the task is Done, and you're about to stop — reap your own session
tree in a single deterministic call: \`POST /dispatch { abort: true, cascade: true, abortTo: <your own
session id> }\` (your id is the **session id** in Setup). That one **\`close --cascade\`** closes your
spent warm session plus every worker and child autopilot beneath it — Harbour walks the lineage and
emits the aborts, so you never close them one by one. This is the run's **final cleanup**, a cheap,
idempotent sweep for any stragglers still open (see *Closing a session, once it's truly spent* in the
handbook): most windows should already be closed by now under the per-session close-on-done default, so
this mostly costs nothing — but a straggler can still be sitting open, so the cascade catches it. It's
safe because the cascade emits **plain** aborts and the runner **skips any session a human continued** —
a window someone's mid-reply in comes back \`[skipped] human-continued\` and is left open, not
\`[aborted]\` — so an end-of-run sweep can never slam shut a window someone's still in.
**Never add \`force\` to the cascade**: \`force\` is only for a deliberate *single* abort that
overrides that skip, and it would defeat the guard that makes the sweep safe. Skip this only when
you're **pausing for the human** rather than finishing — a hand-back may want its windows left open.
