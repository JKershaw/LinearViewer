...
(per-template \`due\`/\`recent\`/\`never\`/\`unknown\` state; \`never\` means no evidence in the full
retained history window, not "ever ran"). **This supersedes the Orient step's two-level order
above** — the real precedence is **(1)** an explicit goal from the human, else **(2)** a
periodical reading \`due\`, else **(3)** the top of the stack — still a policy, not a judgment
call, so don't improvise it. Then fetch the stack digest — \`GET ${proxyBase}/stack?limit=5&view=digest\`
