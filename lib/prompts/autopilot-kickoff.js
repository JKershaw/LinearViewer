...
function buildStepperDisposition(proxyBase, standalone = false) {
  ...
  const beat4 = standalone
   ...
   not a wedge — don't nudge or re-dispatch it on this rule,
   surface it to the human so they can unblock it. You may be woken again on it — a later terminal on
   that row does now reach you — but you are not guaranteed to be, so hand it back to the human rather
   than waiting on a wake that may never arrive. Otherwise, the wedged-beat ceiling still applies: ~30 min silence →
   ...
   re-dispatch it on this rule,
   surface it to the human so they can unblock it. You may be woken again on it — a later terminal on
   that row does now reach you — but you are not guaranteed to be, so hand it back to the human rather
   than waiting on a wake that may never arrive.
...
\`GET ${proxyBase}/autopilot/manual\` if you want to re-read a part mid-run.)`;
  ...
  **session id** — given in the *Your autopilot session id* block at the very end of this prompt.
  ...
  const step3 = standalone
   ...
   that silence is not a wedge — don't nudge or re-dispatch it on this rule,
   surface it to the human so they can unblock it. You may be woken again on it — a later terminal on
   that row does now reach you — but you are not guaranteed to be, so hand it back to the human rather
   than waiting on a wake that may never arrive. If **~30 min** pass with zero new activity from a step, stop trusting the silence:
   ...
   **woken automatically** — that outcome is injected back into this session as a follow-up within
   ...
   as \`blocked\` has a human parked on it, so that silence is not a wedge — don't nudge or re-dispatch it
   on this rule,
   surface it to the human so they can unblock it. You may be woken again on it — a later terminal on
   that row does now reach you — but you are not guaranteed to be, so hand it back to the human rather
   than waiting on a wake that may never arrive. If **~30 min** pass with zero new activity
