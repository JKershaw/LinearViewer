...
3. **Watch.** Long-poll \`GET /dispatch/{id}?wait=50\` in a plain loop — no \`sleep\`, no backoff. The
   ...
   *why* it returned: a \`timeout\` with \`waitedMs\` near 50000 means the call held the whole window and
   nothing changed — the worker's just quiet. (The poll's known quirks, and the rest of your
   instruments, are under *Your instruments* below — including why \`done\` means "go look," not
   "finished.")
...
- **\`done\` is a session boundary, not proof.** A terminal \`done\` means *the session ended*, not that
  ...
  lands (or never does). Treat \`done\` as "go look" (step 4), never "it's finished."
...
- **The watch poll holds open ~50s.** \`GET /dispatch/{id}?wait=50\` returns the moment something changes,
  ...
  minute and trust it; no \`sleep\`, no backoff.
