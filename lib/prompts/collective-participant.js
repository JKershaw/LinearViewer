 ...
 * lib/prompts/autopilot-kickoff.js / foreman-playbook.js — NOT a per-issue
...
/**
 * Linear-access block appended when the participant is given a proxy token, so it
 * can pull this workspace's Linear context into the discussion and — only once
 * John approves in-channel — act on it. Mirrors the proxy-dispatch preamble's
 * standing-readWrite-token shape (the same security debt applies: a leaked prompt
 * leaks workspace write); kept separate so the prompt body has no Linear access
 * mention when no token is supplied.
 *
 * @param {Object} params
...
function buildLinearAccessBlock({ proxyBaseUrl, proxyToken, channel }) {
  return [
    ...
    '',
    `You have a Linear API proxy for this workspace. Base: ${proxyBaseUrl}/api/proxy`,
