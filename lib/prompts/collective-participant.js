 ...
 * docs/architecture/prompt-system.md does not apply here; there is no
 * AI-generated counterpart to keep in sync.
 *
 ...
 *   flag needed.
 * @param {string|null} [params.providerDisplayName=null] - The bound workspace's
 *   declared provider display name (LIN-2354), e.g. "Linear"/"GitHub Issues".
 *   Threaded into the appended access block's "currently backed by X" clause;
 *   omit/`null` when unresolved — the clause is then dropped, not guessed.
...
**The topic is: ${topic}**`;
  ...
  const linearBlock = (proxyBaseUrl && proxyToken)
    ? buildLinearAccessBlock({ proxyBaseUrl: String(proxyBaseUrl).replace(/\/+$/, ''), proxyToken, channel, providerDisplayName })
    : '';
 ...
 * @param {string|null} [params.providerDisplayName=null] - The bound workspace's
 *   declared provider display name (LIN-2354); see the participant builder's
 *   matching param for the omit-when-unresolved contract.
...
You may have been handed a workspace API token for this workspace (see below). Treat
the discussion as **read-and-talk by default**: do not file tickets, change tracker
 ...
 * once John approves in-channel — act on it. The wording is source-neutral (one
 * proxy contract across providers); the "currently backed by X" clause names the
 * resolved provider when known (LIN-2354) and is omitted, not guessed, otherwise.
 ...
 * @param {string|null} [params.providerDisplayName=null] - The bound workspace's
 *   declared provider display name (LIN-2354). `null`/omitted ⇒ the "currently
 *   backed by X" clause is dropped, not hedged.
 * @returns {string}
 */
function buildLinearAccessBlock({ proxyBaseUrl, proxyToken, channel, providerDisplayName = null }) {
  return [
    '',
    '',
    '---',
    ...
    '',
    `You have a workspace API proxy for this workspace (source-neutral${providerDisplayName ? `; currently backed by ${providerDisplayName}` : ''}). Base: ${proxyBaseUrl}/api/proxy`,
    '',
    ...
    `You have this bootstrap because a real dispatch just happened (John started this Collective session); the exchange response is your first call against this workspace's own Harbour control-plane, not a third-party service.`,
    ...
    `e.g. GET ${proxyBaseUrl}/api/proxy/stack, /search?q=…, /issues/{id}.`,
