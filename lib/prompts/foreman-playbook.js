...
The stack is pre-sorted (bugs → started → unstarted → backlog, then most-unblocking → longest-critical-path → priority; blockers before blocked; subtasks clustered with parents). Each line carries the deterministic features behind its rank (\`downstreamUnblocks\`, \`criticalPathLen\`, optional \`heldBy\`, and a compact \`why\`). Pick the top task. **The top may be a parent** — parents with incomplete subtasks are structure, not work. Descend to the first incomplete subtask; a parent doesn't have its own work unit. Skip completed/canceled.`;
