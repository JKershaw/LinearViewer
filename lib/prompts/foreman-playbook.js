...
When you stop, post a final status update with a clear summary of the recap and what you need.`;
  ...
  const linearWritesNote = useMcp
    ? `**Linear writes**: the generated prompt may reference specific Linear updates (state changes, comments, subtasks). Make those writes using the Linear MCP tools — see "Updating Linear" below. Do not use the proxy's \`/api/proxy/issue/*\` write endpoints while MCP is available.`
...
For an aged task whose description has grown, drifted, or pivoted, fetch the brief — a distilled, present-tense version of the task (Current / Constraints / Open questions / Changelog) that supersedes stale wording and folds in comments and subtask state:

\`\`\`bash
curl -s -H "Authorization: Bearer YOUR_TOKEN" ${baseUrl}/api/proxy/brief/{identifier}
\`\`\`

Returns \`{ status, brief }\` where \`brief\` is fixed-section Markdown. Read it before trusting the raw description.
...
2. If the worker claimed any Linear write this step (comment posted, state changed, label applied, subtask created), re-fetch \`GET ${baseUrl}/api/proxy/issue/{identifier}\` and confirm the write shows up. If it doesn't, the write didn't land — drop to 4.d (help) rather than continuing on a hallucinated success.
