...
**Linear writes**: the generated prompt assumes you can write to Linear (via MCP or the proxy itself). If you can't, treat its instructions as advisory and post the changes yourself via \`/api/proxy/issue/{identifier}/comments\`, \`PATCH /api/proxy/issue/{identifier}\`, etc.
