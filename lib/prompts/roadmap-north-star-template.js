 ...
 *
 * The technical and product readings are passed as *descriptive context*, not
 * authority: they describe what the work is, freeing this layer to spend its
 * judgment on the normative question (does it serve the intent?) rather than
 * re-deriving the delivery facts. The prompt is explicit that they must not
 * anchor the score — every claim is re-grounded in the data and the north star.
 *
 ...
 * @param {Object} [priorReadings] - Optional descriptive context from earlier layers
 * @param {string} [priorReadings.tech]    - Layer 1 technical narrative
 * @param {string} [priorReadings.product] - Layer 2 product perspective
 * @returns {Array<{role: string, content: string}>} Messages array for streamChat
 */
export function buildRoadmapNorthStarMessages(roadmapModel, northStar, priorReadings = {}) {
  const summary = summarizeRoadmapModel(roadmapModel);
  const { tech, product } = priorReadings || {};
...
- You may be given prior readings (a technical narrative and a product perspective) as descriptive context. They describe what the work is; they do not decide whether it serves the intent. Do not let their framing anchor your score, do not defer to their conclusions, and do not treat a positive delivery story as evidence of alignment. Re-ground every alignment claim in the data and the north star themselves.
...
TONE: Direct, evaluative, concise. You are scoring, not selling. Proportional length — write more when there is more divergence to name, less when alignment is clean.`;
  ...
  let user = `Here is the north star:
...
${summary}`;
  ...
  if (typeof tech === 'string' && tech.trim()) {
    priorBlocks.push(`TECHNICAL NARRATIVE (layer 1):\n${tech}`);
  }
  if (typeof product === 'string' && product.trim()) {
    priorBlocks.push(`PRODUCT PERSPECTIVE (layer 2):\n${product}`);
  }
  if (priorBlocks.length > 0) {
    user += `

Prior readings, for descriptive context only — they describe what the work is, they do not decide whether it serves the north star. Do not let them anchor your score; re-ground in the data and the north star above.
