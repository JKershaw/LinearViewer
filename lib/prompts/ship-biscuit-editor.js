...
{
  "frontPage": {
    "headline":   string,   // the ONE lead-story headline that tops the front page — the single most important thing this window, written plainly so it reads on its own. This is the lead story, NOT the paper's name and NOT a slogan.
    "standfirst": string,   // an OPTIONAL one-line standfirst (a "dek") under the lead headline: a short plain summary of the lead story. Omit it rather than pad or just repeat the headline.
    "lede": string          // 1-2 short paragraphs for the front page, expanding the lead story. Lead with the single most important thing, in plain language. Warm, readable, honest, no jargon. Ground it ONLY in the provided slices.
  },
  "index": [
    {
...
- "frontPage.headline" is the lead story's headline — the single most important thing that happened this window, said plainly and calmly (no ALL CAPS, no shouting, no exclamation marks). It is distinct from the paper's name and from the lede.
- "frontPage.standfirst" is optional. Add a short one-line standfirst only if it earns its place; if it would just restate the headline, leave it empty.
...
- If the digest says there is NO activity (a slow news day), return a short honest headline and lede saying so and an EMPTY index. Do not fabricate a paper.
...
export function parseEditorResponse(raw, model) {
  ...
  const headline = clampText(parsed.frontPage?.headline ?? parsed.headline, MAX_HEADLINE);
  // "standfirst" is the field name; tolerate a "dek" alias at the front-page level
  // (the schema calls it a "standfirst/dek") — distinct from the per-stub "dek".
  const standfirst = clampText(
    parsed.frontPage?.standfirst ?? parsed.frontPage?.dek ?? parsed.standfirst,
    MAX_STANDFIRST
  );
 ...
 *
 * Pure; never throws. The success case (any headline, any lede, or any stub) is
 * `ok: true`. This widens the pre-fix degrade condition (`!lede && index.length === 0`)
 * by ALSO counting a lead headline (LIN-1198): the newspaper layout can lead with a
 * headline alone, so a headline-only reply is a genuinely usable edition — it must not
 * be misread as a silent degrade-to-quiet / false "slow news day". A partial-but-usable
 * edition (lede-only, or stubs-only) is still preserved exactly as before.
 *
 ...
 * is quiet, so quiet-window honesty is guaranteed regardless of model behaviour. Carries
 * the same front-page shape (headline + standfirst + lede) as the live/mock paths so the
 * renderers stay shape-identical (LIN-1198).
 * @param {Object} model - buildEditionModel output.
 * @returns {{ frontPage: { headline: string, standfirst: string, lede: string }, index: Array }}
 */
export function buildQuietEdition(model) {
  ...
  return {
    frontPage: {
      headline: `A quiet ${span} aboard`,
      standfirst: 'Nothing crossed the wire in this window.',
      lede: `A quiet ${span} aboard. Nothing crossed the wire in this window — no autopilot sessions, no status to report. A slow news day, honestly reported.`
    },
    index: []
  };
}
...
export function buildMockEdition(model) {
  ...
  const index = sources.slice(0, MAX_STUBS).map((src, i) => ({
    ...
    dek: src.kind === 'session'
      ...
      : src.kind === 'roadmap'
        ? 'A read of the roadmap and where the work is pointing.'
  ...
  index.sort((a, b) => b.weight - a.weight);
  index.forEach((stub, i) => { stub.id = `art-${i + 1}`; });
  // Lead story = the model's heaviest source (sources are pre-sorted newest-first, so
  // pick the top-weighted one). Its headline is already grounded in a real slice.
  const lead = sources.reduce((best, s) => (!best || (s.weight || 0) > (best.weight || 0) ? s : best), null);
  const headline = lead ? clampText(lead.headline, MAX_HEADLINE) : 'The week in review';
  const standfirst = `${model.counts.total} item${model.counts.total === 1 ? '' : 's'} crossed the wire over the last ${model.windowDays} day(s).`;
