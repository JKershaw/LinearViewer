 ...
 * task*. Unlike recap (structured JSON) and brief (a fixed-section
 ...
 * the exact same issue context (description, comments, subtasks, relations,
 ...
 *
 * LIN-2371: the persona sentence named Linear unconditionally, so on a GitHub-,
 * Jira- or Local-backed workspace the sentence the whole conversation is built
 * on was false. `providerDisplayName` is the DECLARED provider identity —
 * LIN-2354's contract — so it names the real backend when one was declared and
 * degrades to the neutral "a single task" when unresolved. Never a fallback to
 * Linear: `.displayName` read through `getProviderForWorkspace` always names
 * some provider, which is exactly the defect.
 *
 ...
 * @param {string|null} [providerDisplayName=null] - Declared provider display name, or null.
 * @returns {string}
 */
function buildSystemPrompt(issue, context, providerDisplayName = null) {
  const issueContext = formatIssueContext(issue, context);
  const label = `${issue.identifier || 'this task'}${issue.title ? ` — ${issue.title}` : ''}`;
  const taskNoun = providerDisplayName ? `a single ${providerDisplayName} task` : 'a single task';

  return `You ARE ${taskNoun}, speaking for yourself in a conversation with the person who owns you. You are ${label}. Answer their questions about yourself from your own history — what you are, where you stand, why decisions were made, and what it would take to finish you.
...
RECENCY (exact times): the comment dates in your context are day-resolution only (the time-of-day is dropped). For the exact time a comment was posted, call get_comments — it returns each comment's ISO createdAt with the time. For when this task (or another) last changed state — moved to In Progress, was marked Done, etc. — call get_history, which returns state transitions with exact ISO timestamps, newest first. Do not estimate a time-of-day or a transition date the inline day-resolution context does not give you; fetch it.
 ...
 *   (LIN-2354's contract, LIN-2371). Omit/`null` ⇒ the persona degrades to the
 *   neutral "a single task" rather than naming a provider that was never declared.
 * @returns {Array<{role: string, content: string}>} Messages array for streamChat.
 */
export function buildTaskChatMessages(issue, context, question, history = [], providerDisplayName = null) {
  const messages = [{ role: 'system', content: buildSystemPrompt(issue, context, providerDisplayName) }];
