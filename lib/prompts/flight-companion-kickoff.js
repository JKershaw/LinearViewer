 ...
 * says go.
 *
 * Why this exists (the point of the prototype): at the time LIN-922 landed,
 * the LIN-751 companion needed model tool-calling, then blocked on LIN-489
 * (`streamChat` had no `tools` param — since closed by LIN-988, and
 * `streamChatWithTools` is now in production, e.g. `routes/task-chat.js`).
 * LIN-922's shortcut was to use a real Claude Code session AS the LLM — it
 * already has tools — so "act in place of the LLM, use the API as if it had
 * tools" = hand it this prompt + a `readWrite` proxy token and it curls the
 * proxy endpoints. That prototype shortcut is kept as-is here; it is no
 * longer the only way to reach tool-calling in this tree.
 *
 ...
 *      `GET /dispatch?status=taken` (the live queue) + its companion
 *      `GET /dispatch?status=blocked` (runs parked waiting on a human, which since
 *      LIN-2079 are NO LONGER returned by `status=taken`) + `GET /dispatch/{id}`
 *      (the `feedback[]` array with `[working]`/`[evidence]`/`[done]` markers). This
 ...
 *
 * LIN-2618: the persona, disposition, readout shape, fossil line, surfacing
 * policy, vocabulary and propose-then-wait gate are NO LONGER written here. They
 * are the shared brief (`lib/prompts/flight-companion-brief.js`), rendered
 * identically into this prompt and into the in-page chat's system turn. What
 * stays here is only what is true of THIS transport: the proxy setup, the curl
 * catalog, the boot sequence, and the two hard rules above — none of which mean
 * anything to a surface that has real model tool-calling and no token.
 */
import {
  COMPANION_PERSONA, COMPANION_BRIEF_SECTIONS,
} from './flight-companion-brief.js';
...
- \`GET ${proxyBase}/dispatch?status=blocked\` — runs parked **waiting on a human**. These are alive,
  not stuck: the session posted a \`[blocked]\` marker and is holding for a decision. They are NOT in
  the \`status=taken\` list, so make this a second call — surface them to the human as "waiting on
  you", never report them as stalled or dead.
...
2. **Orient.** Pull \`GET ${proxyBase}/stack?view=digest\` for the headlines, then
   \`GET ${proxyBase}/dispatch?status=taken\` to see what's actually in flight right now, and
   \`GET ${proxyBase}/dispatch?status=blocked\` for anything parked waiting on a human (a separate
   call — blocked rows are not in the taken list). Dig into
   anything interesting with \`GET /brief/{id}\` or \`GET /dispatch/{id}\`.
3. **Give the human the readout**, in the shape set out below — the headline block first, then the
   body.
...
5. **Keep monitoring, thinking out loud.** As you talk, keep checking the dispatch feedback stream
  ...
  // The shared sections render here byte-identically to the way the in-page
  // chat renders them (LIN-2618). They sit AFTER this transport's own setup and
  // catalog because the boot sequence above refers forward to the readout shape.
  // The shared gate is transport-neutral ("anything that changes state"), because
  // the in-page chat has no verbs to name. THIS surface does, and the old
  // inline gate named them — so the concretisation is restored here rather than
  // lost in the extraction. Without it a curl-driven session has to infer which
  // of its own calls the rule covers.
  const gateVerbs = `### What that means for the calls above

- Every \`GET\` in the catalog above — stack, brief, recap, issues, search, and the whole dispatch
  monitoring feed — is reading. No approval needed, ever. That is your job; do it freely.
- Every \`POST\` — \`/dispatch\`, \`/recommend-and-dispatch\`, and any follow-up that sends new work —
  changes state and needs an explicit **yes** from the human first.`;

  const closing = `That's the whole brief. Boot, orient, be a good companion, and keep the human in the driver's seat.`;

  return [intro, setup, tools, boot, ...COMPANION_BRIEF_SECTIONS, gateVerbs, closing].join('\n\n---\n\n') + '\n';
}
