 ...
 *   tokenExpiresAt,
 *   pendingSpend:   {refreshToken, attemptedAt} | null,  // LIN-2235 L4.1: spend-intent journal
 *   createdAt, updatedAt
 * }
 *
...
export class OwnerCredentialStore {
      ...
      await this.collection.updateOne(
        { _id: this._id(accountId, urlKey, provider) },
        {
          $set: {
            ...
            refreshToken,
            tokenExpiresAt,
            // LIN-2235 (L4.1): a full write of the credential's own fields is
            // by definition a resolved outcome for any spend-intent marker —
            // see `markPendingSpend` below.
            pendingSpend: null,
            updatedAt: now
          },
          $setOnInsert: {
      ...
      const { matchedCount } = await this.collection.updateOne(
        { _id: this._id(accountId, urlKey, provider), refreshToken: expectedRefreshToken },
        {
          $set: {
            ...
            refreshToken,
            tokenExpiresAt,
            // LIN-2235 (L4.1): a CAS win means the rotation this spend-intent
            // marker was tracking has now been durably recorded — the pending
            // outcome is resolved, so clear it. See `markPendingSpend` below.
            pendingSpend: null,
            updatedAt: now
          }
        }
  ...
  /**
   * Marks a spend-intent journal entry (LIN-2235, L4.1) — written durably
   * BEFORE a refresh token is spent at the provider, so a crash between that
   * exchange and {@link OwnerCredentialStore#putIfRefreshToken}'s durable
   * write leaves evidence of what was attempted. `lib/workspace-token-refresh.js`'s
   * `doOwnerRefresh` reads it back via the normal {@link OwnerCredentialStore#get}
   * (it is just a field on the same record) and, finding one that still names
   * the current `refreshToken`, knows a prior attempt's outcome was never
   * recorded.
   *
   * A cheap, field-only `$set` — never touches the sibling credential fields
   * (`scope`/`token`/`refreshToken`/`tokenExpiresAt`), so this write and a
   * concurrent {@link OwnerCredentialStore#putIfRefreshToken} CAS can never
   * race each other's fields. No upsert: marking intent to spend a credential
   * that does not exist would create a bogus record; the caller only ever
   * calls this after reading a real one.
   *
   * Cleared as a side effect of the next full write to this record — see the
   * `pendingSpend: null` in {@link OwnerCredentialStore#put} and
   * {@link OwnerCredentialStore#putIfRefreshToken} above — never by this
   * method itself, which only ever sets.
   *
   * @param {string} accountId
   * @param {string} urlKey - the workspace urlKey
   * @param {string} [provider] - the credential's provider; legacy-normalized to `'linear'`
   * @param {string} refreshToken - the token about to be spent (the marker's witness)
   * @param {number} attemptedAt - epoch ms the spend was attempted
   * @returns {Promise<boolean>} true if the write completed (whether or not a record existed to mark)
   */
  async markPendingSpend(accountId, urlKey, provider, refreshToken, attemptedAt) {
    if (!accountId || !urlKey || !refreshToken) {
      console.warn('OwnerCredentialStore.markPendingSpend called without accountId/urlKey/refreshToken');
      return false;
    }
    ...
    try {
      await this.collection.updateOne(
        { _id: this._id(accountId, urlKey, provider) },
        { $set: { pendingSpend: { refreshToken, attemptedAt } } }
      );
      return true;
    } catch (err) {
