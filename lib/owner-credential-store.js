...
export class OwnerCredentialStore {
      ...
      const { matchedCount } = await this.collection.updateOne(
        { _id: this._id(accountId, urlKey, provider), refreshToken: expectedRefreshToken },
        {
          $set: {
            ...
            tokenExpiresAt,
            updatedAt: now,
            // LIN-2235 (L4.1): a landed rotation always resolves whatever
            // spend-intent marker `markSpendIntent` left for the token this
            // write just replaced — see the module doc on `pendingSpend`.
            pendingSpend: null
          }
        }
      );
  ...
  /**
   * Records that `spentRefreshToken` is about to be spent against the
   * provider's OAuth exchange, BEFORE the exchange happens (LIN-2235, L4.1 —
   * the "spend-intent journal" of the LIN-2231 design, amendment A3). Written
   * as a `pendingSpend` field on the SAME record the exchange is about to
   * rotate, not a separate collection: it needs to be read alongside the
   * record on every refresh attempt (no extra round trip for the common
   * case), and it is meaningless without that record.
   *
   * Why this matters: `putIfRefreshToken`'s durable compare-and-set already
   * makes concurrent refreshes race-safe — but a refresh whose PROCESS dies
   * between a successful Linear exchange and the matching CAS write leaves no
   * code path to record what happened. The stored `refreshToken` is now
   * spent-but-unrotated: unusable for a normal refresh, yet indistinguishable
   * from an untouched record without this marker. `pendingSpend` is what
   * makes that gap detectable on the next resolve (see
   * `lib/workspace-token-refresh.js`'s `doOwnerRefresh`), instead of the spent
   * token being retried blindly forever once Linear's reuse grace lapses.
   *
   * Resolved (cleared) two ways, both representing a completed, IN-PROCESS
   * outcome — never a crash: `putIfRefreshToken`'s `$set` above always clears
   * it as part of a successful rotation; `clearSpendIntent` clears it after a
   * refresh attempt fails cleanly (the exchange itself rejected the token, or
   * any other outcome this same process fully observed). A marker that is
   * still set on a LATER read is therefore never an ordinary failure — it is
   * specifically "nobody in this process ever got the chance to resolve it".
   *
   * @param {string} accountId
   * @param {string} urlKey - the workspace urlKey
   * @param {string} provider - the credential's provider; selects the partition
   * @param {string} spentRefreshToken - the refreshToken about to be spent (the same value the caller is about to hand to the provider's exchange)
   * @returns {Promise<boolean>} true if the write completed (the record's own existence is not re-verified here — a miss is harmless, see the module's no-throw convention)
   */
  async markSpendIntent(accountId, urlKey, provider, spentRefreshToken) {
    if (!accountId || !urlKey || !spentRefreshToken) {
      console.warn('OwnerCredentialStore.markSpendIntent called without accountId/urlKey/spentRefreshToken');
      return false;
    }
    try {
      await this.collection.updateOne(
        { _id: this._id(accountId, urlKey, provider) },
        { $set: { pendingSpend: { refreshToken: spentRefreshToken, attemptedAt: new Date() } } }
      );
      return true;
    } catch (err) {
      console.error('Error marking owner credential spend intent:', err);
      return false;
    }
  }

  /**
   * Resolves (clears) a `pendingSpend` marker after a refresh attempt fails
   * cleanly IN THIS PROCESS — see `markSpendIntent`'s docstring for why this
   * is distinct from the successful-rotation clear already folded into
   * `putIfRefreshToken`.
   *
   * @param {string} accountId
   * @param {string} urlKey - the workspace urlKey
   * @param {string} provider - the credential's provider; selects the partition
   * @returns {Promise<boolean>} true if the write completed
   */
  async clearSpendIntent(accountId, urlKey, provider) {
    if (!accountId || !urlKey) {
      console.warn('OwnerCredentialStore.clearSpendIntent called without accountId/urlKey');
      return false;
    }
    try {
      await this.collection.updateOne(
      ...
      console.error('Error clearing owner credential spend intent:', err);
      return false;
    }
  }
