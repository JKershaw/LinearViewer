...
export function computeTerminalMarkedTaskCost(rows, now) {
  ...
  for (const [key, lineage] of lineages) {
    ...
    anchoredLineageRecords.push({ ticketIds: lineageTicketIds, fullyPriced: reduced.fullyPriced });
  ...
  for (const record of anchoredLineageRecords) {
    ...
    if (!component) {
      component = { ticketIds: new Set(), fullyPriced: true, lineageCount: 0 };
  ...
  // LIN-2418: `laneLandedCount` and `lineageBearingCount` are derived from
  // the SAME filter so they can never drift apart — `lineageBearingCount` is
  // the denominator `opencodeSummedShare`/`unknownHarnessShare` need below
  // (T minus every issue this system has no lineage for at all) and is
  // never itself published as a new wire field. LIN-2253 (narrowed):
  // `laneLandedCount` is the LIN-2253 rename of `noLineageCount` — the
  // filter itself is UNCHANGED (the coupling above is what keeps LIN-2418's
  // dilution guard intact through the rename); `noLineageCount` stays on the
  // wire for one deprecation cycle since `__KPI_DATA__` is public wire.
  const laneLandedCount = issueList.filter(i => i.noLineage).length;
  const lineageBearingCount = T - laneLandedCount;
  return {
