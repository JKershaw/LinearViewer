...
export function computeTerminalMarkedTaskCost(rows, now) {
  ...
  // is never double-counted here. Checked against BOTH `issues` (the DONE,
  // fully-resolved anchors) AND `anchoredIdentifiers` (review fix: every
  // in-window anchor regardless of status) — a ticket whose own real
  // lineage is still in-flight/failed/aborted has a lineage, just not a
  // finished one, and must not be misclassified as `noLineage`; that is a
  // DIFFERENT, already-disclosed mechanism (`inFlightUsd`), not this one.
  // Every identifier surviving both checks genuinely has no lineage of its
  ...
  for (const identifier of new Set(laneTicketDoneIdentifiers)) {
    if (issues.has(identifier) || anchoredIdentifiers.has(identifier)) continue;
  ...
  return {
    ...
    noLineageCount: issueList.filter(i => i.noLineage).length,
