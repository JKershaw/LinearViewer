 ...
 * `payload.attention`, which `buildSweepPayload` sorts by recency of
 * `since` (most-recently-transitioned first, `loopId` tie-break) since
 * LIN-2619 — any deterministic order suffices here; none is assumed.
 *
 * Fossil rows are INTENTIONALLY absent (LIN-2619 beat 2, recorded as
 * intended by LIN-2647): `buildSweepPayload` folds any attention-eligible
 * row older than `FOSSIL_AGE_MS` into `staleAttentionCount` instead of
 * enumerating it in `attention`, so a fossil `blocked` row produces no
 * would-be action here — they are exactly the rows LIN-2619 set out to
 * stop enumerating. Pinned by the LIN-2647 tests in
 * tests/unit/observer-sweep.test.js.
 *
 * CONSUMER CONSEQUENCE of that ruling (also LIN-2647): the fold caps this
 * store's own per-loop time series at the fossil boundary.
 * `lib/observer-efficacy-signal.js`'s `computeNewHarnessSignal` reads
 * `listByWorkspace` output and derives `stillBlockedObservedMs =
 * lastLoggedAt - firstDetectedAt` and `relayCount = entries.length`, so
 * for any loop blocked longer than `FOSSIL_AGE_MS` both silently freeze
 * at ≈ the boundary — the loop is still blocked, but no further entries
 * ever arrive for it. That consumer's `truncated` flag does NOT surface
 * this bias: it is keyed on the retention cap (`entries.length >=
 * capacity`, LIN-2263), an unrelated bound. The consequence is documented
 * beside that caveat in `computeNewHarnessSignal`'s own docblock and
 * pinned by the LIN-2647 cross-module test in
