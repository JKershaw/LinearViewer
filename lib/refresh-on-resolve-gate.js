 ...
 */
// M1: sweep stale entries every this-many `shouldAttempt` calls, rather than on
// every call (an O(map size) scan per request would be its own cost) or never
// (the pre-existing defect — `lastAttempt` grew one entry per distinct
// (scopeKey, fingerprint) pair ever seen, forever, since a settled cooldown
// window left its entry behind with nothing to evict it).
...
export function createRefreshOnResolveGate({
  ...
  // Drops every entry whose cooldown window has already elapsed — safe to
  // remove unconditionally, since `shouldAttempt` only ever consults an
  // entry's age relative to `cooldownMs`; a stale one is indistinguishable
  // from no entry at all.
  function sweepStale(currentTime) {
    for (const [key, attemptedAt] of lastAttempt) {
      if (currentTime - attemptedAt >= cooldownMs) lastAttempt.delete(key);
    }
  }
  ...
  // Test-only: exposes the map's current size so M1's sweep can be pinned
  // behaviourally rather than only by inference from shouldAttempt's boolean
  // return (mirrors _resetInflightForTests's precedent elsewhere in this
  // codebase — a narrow, clearly-named test seam beats reaching into closure
  // state).
  return { shouldAttempt, _sizeForTests: () => lastAttempt.size };
}
