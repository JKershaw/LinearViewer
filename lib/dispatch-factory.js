...
import { isLaunchTimeFailure } from './dispatch-terminal.js';
 ...
 *   (a launch-time failure that never started work) does NOT refuse — see
 *   `isLaunchTimeFailure` (LIN-2872).
...
export async function createDispatchItem({
  ...
  if (guardApplies) {
    ...
    // LIN-2872 — the launch-time-failure exemption. A prior that reached a
    // terminal `[failed]` BEFORE any `[working]` heartbeat died at launch (an
    // opencode HTTP 500 on the first message, "serve never became ready"); it
    // never started work, so it is NOT a duplicate risk and the retry must not
    // be taxed with the window. The exemption reads the prior's OWN feedback
    // (returned by `findRecentFreshDispatch` since LIN-2872) and is
    // deliberately FAIL-CLOSED: feedback absent/unreadable, a `[working]`
    // heartbeat present (the dispatch actually ran), or any terminal other
    // than `[failed]` all keep the window exactly as before. This is the
    // precise carve-out the bake-off measured — never a widening of the guard
    // to "any failed prior", and never a removal of the window for a prior
    // that is still running or that ran.
    if (existing && priorDispatchedAt && !Number.isNaN(priorDispatchedAt.getTime()) && !isLaunchTimeFailure(existing.feedback)) {
