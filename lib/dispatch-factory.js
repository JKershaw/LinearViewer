...
import { resolveRoutingFromConfig } from './workspace-preferences.js';
...
export async function createDispatchItem({
  ...
  if (budgetGuardApplies && fields.sessionId) {
    ...
    // `maxTasks == null` (absent, or the run row couldn't be resolved/read) ⇒
    // skip entirely — zero extra reads beyond the one above, zero behavior
    // change. This is what makes "a kickoff without maxTasks behaves exactly as
    // today" true on cost, not just semantics. A budget that was never
    // confirmed to exist can't be enforced.
    if (typeof maxTasks === 'number' && maxTasks >= 1) {
      ...
      if (!result.alreadyCounted && result.count >= maxTasks) {
  ...
  // 4. Resolve harness FIRST, model SECOND (LIN-1694) — amending the prior
  //    "sources combined field-by-field, independent fall-through" design,
  //    which is what let a resolved model survive from a row scoped to a
  //    harness this dispatch does NOT end up running on (the crossed-pair
  //    class this ticket closes). Both fields still share the same five-source
  //    precedence — explicit incoming > selected preset (byKind) > inherited
  //    anchor presetConfig (byKind) > anchor's own harness (harness only, 4.5)
  //    > workspace dispatchDefaults (byKind → workspace-wide) > null — but
  //    model resolution now runs AFTER harness has fully settled, passing it
  //    to `resolveRoutingFromConfig` as `harnessInForce` so a row whose own
  //    `harness` field disagrees with the harness actually in force donates no
  //    model (it falls through to the next source, exactly like a genuinely
  //    absent value — the null-omitted-⇒-everything-eligible contract
  //    `resolveRoutingFromConfig` documents). A row whose harness is BLANK
  //    stays eligible regardless — blank still means inherit, so an
  //    all-blank-harness workspace (LIN-1111's escape hatch) is unaffected.

  // Pass A — settle harness only.
  ...
  //   No model twin by design — an anchor's own resolved model does not ride
  //   along with its harness (LIN-1431), so Pass B below has no equivalent step.
  if (!resolvedHarness && anchor?.harness) resolvedHarness = anchor.harness;

  // Fetched at most once, lazily, and shared with Pass B below — the same
  // single workspace-preferences read a dispatch with no explicit/preset/
  // anchor harness or model needed BEFORE this split, preserved here even
  // though the two fields are no longer resolved from one combined call.
  let workspaceDefaultsConfig;
  const getWorkspaceDefaultsConfig = async () => {
    if (workspaceDefaultsConfig === undefined) {
      const prefs = workspacePreferencesStore ? await workspacePreferencesStore.getWorkspacePreferences(urlKey) : null;
      workspaceDefaultsConfig = prefs?.dispatchDefaults || null;
    }
    return workspaceDefaultsConfig;
  };

  if (!resolvedHarness && workspacePreferencesStore) {
    resolvedHarness = resolveRoutingFromConfig(await getWorkspaceDefaultsConfig(), effectiveKind).harness;
  }

  // Pass B — resolve model, using the now-settled (pre-interpose) harness as
  // `harnessInForce` on every `resolveRoutingFromConfig` call. Deliberately
  // BEFORE step 5's default-harness interpose below: keying eligibility off
  // the interposed default (rather than the harness genuinely in force from a
  // real source) would treat an all-blank-harness workspace's own model rows
  // as scoped to `claude-code` and wrongly reject them — the LIN-1111 escape
  // hatch this preserves.
  let resolvedModel = model || null;

  if (!resolvedModel && selectedPreset) {
    resolvedModel = resolveRoutingFromConfig(selectedPreset.config, effectiveKind, resolvedHarness).model;
  }

  if (!resolvedModel && anchor?.presetConfig) {
    resolvedModel = resolveRoutingFromConfig(anchor.presetConfig, effectiveKind, resolvedHarness).model;
  }

  if (!resolvedModel && workspacePreferencesStore) {
    resolvedModel = resolveRoutingFromConfig(await getWorkspaceDefaultsConfig(), effectiveKind, resolvedHarness).model;
  }

  // 5. Interpose the default harness (LIN-1159) unless the caller opts out.
  //    Runs AFTER both passes above — the interpose stays outside eligibility
  //    keying, so it can never retroactively invalidate the model Pass B just
  //    resolved against the pre-interpose harness.
