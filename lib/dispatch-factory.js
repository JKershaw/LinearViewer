...
export async function createDispatchItem({
  ...
  if ((!resolvedModel || !resolvedHarness) && workspacePreferencesStore) {
    // One read serves both fields. When `resolvedHarness` is still null, `harnessInForce` is null
    // too — the eligibility check is off, so this read's `model` and `harness` can still come from
    // different rows of the same config (`byKind` vs. top-level; LIN-1694 review F2), not
    // necessarily one self-consistent row. When a harness is already in force, that row must clear
    // eligibility to donate a model.
