...
export async function createDispatchItem({
  ...
  if ((!resolvedModel || !resolvedHarness) && workspacePreferencesStore) {
    // One read serves both fields. When `resolvedHarness` is still null the workspace row IS the
    // harness source, so passing null is correct — the row answers both fields and cannot cross
    // itself. When a harness is already in force, that row must clear eligibility to donate a model.
