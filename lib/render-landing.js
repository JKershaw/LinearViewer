...
const PROVIDERS = [
  { name: 'Linear', note: 'the original backend' },
  { name: 'GitHub Issues', note: 'first foreign backend' },
  { name: 'GitHub Projects', note: 'boards, read-only V1' },
  { name: 'Local', note: 'writable, no tracker' },
];
...
function renderTrySection({ freeTierEnabled }) {
  ...
  const pricingLine = pricingHint
    ? `The default model runs at ${escapeHtml(pricingHint)} — a single generated prompt costs a fraction of a cent, so dozens fit under $1.`
    : 'A single generated prompt costs a fraction of a cent, so dozens fit under $1.';
  ...
  return `<section class="lx-section lx-try" data-testid="landing-try" aria-labelledby="lx-try-h">
      <div class="lx-try__inner">
        ...
        <p class="lx-try__scope">This covers trying AI Generated Prompts — running full autopilot dispatch uses a separate, pricier model and can run long.</p>
      </div>
    </section>`;
}
 ...
 *
 * @param {Object} [opts]
 * @param {Object} [opts.deployInfo] - Heroku deploy info for the footer.
...
export function renderLandingPage({
  ...
  const navHtml = renderNavBar({ isLanding: true, featureFlags });
  const heroHtml = renderLandingHero({ githubEnabled });
