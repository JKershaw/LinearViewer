...
/**
 * LIN-2830: one row per REALISED (harness, model, effort) group of this
 * kind's sessions — the bake-off granularity. Null cells render as the page's
 * established "—" figure placeholder (same convention as formatUsd/
 * formatDuration): a missing value is a missing datum, never a dropped row —
 * opencode posts no effort field by design, so its rows read "—" there.
 * Per-row partial cost/duration coverage is disclosed inline (R2 discipline,
 * row-scoped) rather than presented as if it covered the row's sessions.
 */
function renderGroupRows(card) {
  if (!Array.isArray(card.rows) || card.rows.length === 0) return '';
  const rowsHtml = card.rows.map((row) => {
    const coverage = [];
    if (row.sessionCount > row.costPricedCount) {
      coverage.push(`${row.costPricedCount} of ${row.sessionCount} priced`);
    }
    if (row.sessionCount > row.durationCoveredCount) {
      coverage.push(`${row.durationCoveredCount} of ${row.sessionCount} timed`);
    }
    return `<div class="effort-group-row">
      <span class="effort-group-cell effort-group-harness">${row.harness == null ? '—' : escapeHtml(row.harness)}</span>
      <span class="effort-group-cell effort-group-model">${row.model == null ? '—' : escapeHtml(row.model)}</span>
      <span class="effort-group-cell effort-group-effort">${row.effort == null ? '—' : escapeHtml(row.effort)}</span>
      <span class="effort-group-cell effort-group-sessions">${row.sessionCount} lineage${row.sessionCount === 1 ? '' : 's'}</span>
      <span class="effort-group-cell effort-group-cost">${formatUsd(row.costUsd)}</span>
      <span class="effort-group-cell effort-group-duration">${formatDuration(row.durationMs)}</span>
      ${coverage.length ? `<span class="effort-group-coverage">${escapeHtml(coverage.join(' · '))}</span>` : ''}
    </div>`;
  }).join('\n');
  return `<div class="effort-group-rows" data-testid="effort-card-rows-${escapeHtml(card.kind)}">
    <div class="effort-group-rows-head"><span>harness</span><span>model</span><span>effort</span><span>lineages</span><span>cost</span><span>duration</span></div>
    ${rowsHtml}
  </div>`;
}
  ...
  const notesHtml = `<div class="effort-notes" data-testid="effort-notes">
    ...
    <details class="effort-notes-details">
      ...
      <ul>
        ...
        <li>${escapeHtml(readout.notes.harnessModelRows)}</li>
