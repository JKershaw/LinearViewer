...
export function renderEffortReadoutPage(workspaceName = 'Workspace', options = {}) {
  ...
  const populationCaption = `Live queue: ${population.liveCount} rows (TTL-scoped, not row-limited) · History: showing ${population.historyCount} of ${population.historyTotal} most-recently-resolved rows${population.historyTruncated ? ' (truncated to the 200-row bound)' : ''}.`;
