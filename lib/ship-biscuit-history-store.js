 ...
 *   frontPage:    { headline: string, standfirst: string, lede: string },  // lead story (LIN-1198)
...
const MAX_STANDFIRST_CHARS = 600;
...
function toRecord(doc) {
  ...
  return {
    ...
    // Tolerate older docs that stored only `lede` — headline/standfirst default to ''.
    frontPage: {
      headline: doc.frontPage?.headline || '',
      standfirst: doc.frontPage?.standfirst || '',
      lede: doc.frontPage?.lede || ''
    },
...
export class ShipBiscuitHistoryStore {
   ...
   * @param {{headline?: string, standfirst?: string, lede: string}} [data.frontPage]
  ...
  async save(urlKey, { model, window, since, workspaceName, isQuiet, frontPage, index, weather } = {}) {
    ...
    const doc = {
      ...
      frontPage: {
        headline: String(frontPage?.headline || '').slice(0, MAX_HEADLINE_CHARS),
        standfirst: String(frontPage?.standfirst || '').slice(0, MAX_STANDFIRST_CHARS),
        lede: String(frontPage?.lede || '').slice(0, MAX_LEDE_CHARS)
      },
