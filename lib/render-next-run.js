...
export function renderNextRunPage(data = {}, options = {}) {
  ...
  // Task-budget dial (LIN-1737 Beat 1, D3): a small numeric input with preset
  // chips, sitting BESIDE the per-option S/M/L size chip (D2) — this sizes the
  // RUN, S/M/L sizes an OPTION, two axes that don't merge. Blank ⇒ unbounded,
  // byte-identical to pre-LIN-1737 dispatch (D3/SC#4). No persistence in this
  // first cut (D4) — public/next-run.js holds the value in memory only.
  //
  // The dial dispatches through the SAME proxy-gated kickoff every option's
  // `Dispatch ▾` uses (window.fetchAutopilotKickoff), so when the proxy feature
  // is off it would be visibly present but silently inert (LIN-1737 review B2).
  // DISABLED, not hidden, with an inline explanation in that case — mirroring
  // how `data.proxyEnabled` already gates the dispatch affordance itself in
  // public/next-run.js's `renderOptionCards`.
  const proxyEnabled = nextRunData.proxyEnabled;
  const budgetDisabledAttr = proxyEnabled ? '' : ' disabled';
  const budgetDescribedByAttr = proxyEnabled ? '' : ' aria-describedby="next-run-budget-hint"';
  const budgetPresets = [5, 10, 25, 50, 100];
  const budgetPresetsHtml = budgetPresets.map((n) =>
    `<button type="button" class="next-run-budget-preset" data-value="${n}"${budgetDisabledAttr}${budgetDescribedByAttr}>${n}</button>`
  ).join('');
  const budgetDial = `<div class="next-run-budget" id="next-run-budget">
          <label class="next-run-budget-label" for="next-run-budget-input">task budget</label>
          <div class="next-run-budget-row" role="group" aria-label="task budget presets"${budgetDescribedByAttr}>
            ${budgetPresetsHtml}
            <input type="number" min="1" step="1" inputmode="numeric" class="next-run-budget-input" id="next-run-budget-input" placeholder="unbounded"${budgetDisabledAttr}${budgetDescribedByAttr} />
          </div>
          ${proxyEnabled
            ? ''
            : '<p class="next-run-budget-hint" id="next-run-budget-hint">the task budget requires the proxy feature — enable +proxy to use it</p>'}
        </div>`;
  ...
  //
  // Above both sits the degradation notice (LIN-1665), filled from the response's
  // `degraded` flag: when a reply could not be parsed the cards below are entirely
  // deterministic fills, which used to be indistinguishable from a real generation.
  // Reuses .next-run-warning (the AI-unconfigured amber) — same register, one style.
  //
  // Between the preamble and the cards sits the direction chooser (LIN-1566): an
  // empty mount point the client fills from the response's resolved `directions`,
  // so the user picks a direction before a goal. It stays hidden when a generation
  // carries no usable grouping, leaving the flat card list untouched.
  const optionsBody = `<p class="next-run-warning" id="next-run-degraded" role="status" hidden></p>
      <p class="next-run-summary" id="next-run-summary" hidden></p>
      ...
      <div class="next-run-directions" id="next-run-directions" hidden></div>
  ...
  return renderPage({
    ...
    ${renderPageHeader({ title: 'Suggested Next Run', subtitle: 'Let the project propose where autopilot should go next.' })}
