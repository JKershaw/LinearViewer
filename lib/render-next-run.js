...
export function renderNextRunPage(data = {}, options = {}) {
  ...
  const budgetPresetsHtml = budgetPresets.map((n) =>
    `<button type="button" class="next-run-budget-preset" data-value="${n}"${budgetDisabledAttr}>${n}</button>`
  ).join('');
  const budgetDial = `<div class="next-run-budget" id="next-run-budget">
          ...
          <div class="next-run-budget-row" role="group" aria-label="task budget presets">
            ${budgetPresetsHtml}
            <input type="number" min="1" step="1" inputmode="numeric" class="next-run-budget-input" id="next-run-budget-input" placeholder="unbounded"${budgetDisabledAttr} />
          </div>
          ${proxyEnabled
            ? ''
