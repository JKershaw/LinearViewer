...
/**
 * The LAUNCH ANNOUNCEMENT — `[working] Session launched …` — posted by Simple
 * Dispatcher's executor the INSTANT the terminal window opens
 * (simple-dispatcher/executors.js:711), BEFORE the harness has run a single
 * message. It must NOT count as "work started": every real launch-time failure
 * (the opencode HTTP 500 / "serve never became ready" class of LIN-2787/LIN-2121)
 * already carries this beat by the time it posts terminal `[failed]`, so a
 * predicate that treated it as work would never fire on the very rows this
 * exemption exists for (LIN-2872 review F1).
 *
 * Consumed by `isLaunchTimeFailure` (LIN-2872).
 */
const LAUNCH_ANNOUNCEMENT_REGEX = /^\s*\[working\]\s*Session launched/i;

/**
 * A real during-work beat — the runner's heartbeat/tool-call markers. This
 * covers BOTH the plain `[working] …` lead (tool beats, "no tool calls in …",
 * "Session resumed. …", the opencode reaper's "(opencode — Ns so far; …)") AND
 * the CATEGORIZED lead `[working · <category>]` that heartbeat.js:282 emits
 * (`[working · editing] 4 tools/20s`, `[working · verifying] …`,
 * `[working · running] …`) — the latter is the LIN-2872 review F2 gap: a regex
 * keyed on `[working]` alone would silently exempt a dispatch that genuinely
 * ran and failed after only categorized beats. Presence of ANY such marker is
 * the evidence that a taken dispatch actually started work.
 *
 * Consumed by `isLaunchTimeFailure` (LIN-2872).
 */
const WORK_STARTED_FEEDBACK_REGEX = /^\s*\[working(\s*·\s*[^\]]+)?\]/i;
 ...
 * feedback ends terminal `[failed]` AND contains no real work-started marker at
 * all. A dispatch that never started work is not a duplicate risk — the retry
 * of a launch-time failure is exactly the case the five-minute duplicate
 * window wrongly taxed — while a prior that reached any real during-work beat
 * (or any other terminal) genuinely ran and must keep blocking.
 *
 * "Work started" is NOT "any `[working]` marker": the executor's
 * `[working] Session launched …` announcement (executors.js:711) fires the
 * instant the terminal window opens, before any tool runs, so it is ignored.
 * Every other `[working]`/`[working · <category>]` heartbeat counts as work
 * (LIN-2872 review F1/F2).
 *
 ...
 *   real work-started marker (a `[working]`/`[working · cat]` beat other than
 *   the launch announcement) ever appeared
 */
export function isLaunchTimeFailure(feedback) {
  ...
  return !feedback.some(entry => {
    ...
    return WORK_STARTED_FEEDBACK_REGEX.test(message) && !LAUNCH_ANNOUNCEMENT_REGEX.test(message);
  });
}
...
export const __internal = { TERMINAL_FEEDBACK_REGEX, TERMINAL_MARKER_TO_STATUS, WAKE_FEEDBACK_REGEX, RESUME_MARKER_REGEX, LAUNCH_ANNOUNCEMENT_REGEX, WORK_STARTED_FEEDBACK_REGEX };
