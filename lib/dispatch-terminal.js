...
/**
 * A `[working]` marker — the runner's during-work heartbeat (tool-call beats,
 * "Session launched", "Session resumed", …). Presence of ANY such marker is the
 * evidence that a taken dispatch actually started work: a launch-time failure
 * that died before any tool ran — the opencode HTTP 500 / "never became ready"
 * class of LIN-2787/LIN-2121 — posts terminal `[failed]` with no `[working]`
 * ever appearing. Consumed by `isLaunchTimeFailure` (LIN-2872).
 */
const WORKING_FEEDBACK_REGEX = /^\s*\[working\]/i;
 ...
 * feedback ends terminal `[failed]` AND contains no `[working]` heartbeat at
 ...
 * window wrongly taxed — while a prior that reached any `[working]` marker
...
export function isLaunchTimeFailure(feedback) {
  ...
  return !feedback.some(entry => WORKING_FEEDBACK_REGEX.test(entry?.message || ''));
}
...
export const __internal = { TERMINAL_FEEDBACK_REGEX, TERMINAL_MARKER_TO_STATUS, WAKE_FEEDBACK_REGEX, RESUME_MARKER_REGEX, WORKING_FEEDBACK_REGEX };
