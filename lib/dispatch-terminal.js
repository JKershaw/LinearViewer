...
/**
 * Wake events (LIN-826) — the markers that, when a *subscribed* child reaches
 * them, wake its parent with a follow-up. This is a deliberate SUPERSET of the
 * terminal markers: it additionally counts `[blocked]`, because a blocked child
 * must wake its parent so it can react — not only a clean done.
 *
 ...
 * dashboard Loop feed, where counting `[blocked]` as a *completion* would
 * corrupt those semantics. The split keeps the blast radius of the new
 * `[blocked]` recognition at zero on existing consumers — this predicate is
 * consumed ONLY by the up-chain wake auto-enqueue. `[blocked]` recognition is
 * forward-compatible: it only bites once the runner actually emits that marker.
 */
const WAKE_FEEDBACK_REGEX = /^\s*\[(done|complete|failed|aborted|blocked)\]/i;
...
/**
 * Whether a single feedback message is a wake event (LIN-826) — a `[done]`,
 * `[complete]`, `[failed]`, `[aborted]`, or `[blocked]` prefix. Pure; the marker
 * must be a leading prefix (a mid-sentence mention does not count), matching the
 ...
 * The wake superset includes `[blocked]`; unlike findTerminalFeedback there is
 * no status mapping — a wake event is an event, not a completion verdict.
 *
 * @param {Array<{message?: string, timestamp?: string}>} feedback
