...
/**
 * LIN-2123: Simple Dispatcher already posts this exact, one-time marker when
 * a session is resumed (`dispatcher.js`'s resume path, in place — same
 * dispatch row, no follow-up) — distinct from an ordinary `[working]`
 * heartbeat, which carries no such text and must NOT clear a derived
 * `blocked` (the ticket's own pinned case: `[blocked]` then plain `[working]`
 * heartbeats stays `blocked`). No Simple Dispatcher change is needed; this is
 * purely a read-side gap in `deriveLifecycleStatus` below.
 *
 ...
 */
const RESUME_MARKER_REGEX = /^\s*\[working\]\s*Resumed session\b/i;
 ...
 *
 * LIN-2123: a session unblocked and resumed IN PLACE posts no wake event at
 * all (`[working]` heartbeats, including the runner's own resume beat, are
 * not wake events — see WAKE_FEEDBACK_REGEX's docstring), so the last wake
 * event stays the earlier `[blocked]` forever while the session runs. Fixed
 * by scanning for BOTH the last wake event and `RESUME_MARKER_REGEX`
 * together, backward, in one pass: whichever occurs LATER in the array wins
 * (array position only — no timestamp comparison, no "any later activity"
 * heuristic; a plain `[working]` heartbeat still does not count, exactly as
 * the ticket requires). This keeps the derivation marker-pure — it is a new,
 * more specific marker recognized in a NEW place, not a widening of what
 * counts as "later activity".
 *
 * @param {Array<{message?: string}>} feedback
