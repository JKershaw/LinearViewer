...
/**
 * LIN-2872 — the ALLOW-LIST of "evidence of work" that a prior dispatch must
 * show before the duplicate-dispatch guard treats it as having actually run
 * (and so keeps blocking a retry). A launch-time failure is one that reached
 * terminal `[failed]` with NONE of these in its feedback.
 *
 * This is deliberately an ALLOW-LIST, NOT a deny-list of liveness strings. The
 * runner posts several `[working]`-prefixed liveness beats that must NOT count
 * as work: the executor's `[working] Session launched …` announcement
 * (simple-dispatcher/executors.js:711, posted the instant the terminal window
 * opens, before any tool runs), the opencode reaper's post-failure liveness
 * poll `[working] (opencode — Ns so far; next check in …)`, and `[working]
 * Session resumed. …` (LIN-2123's shared prefix). A deny-list that excludes each
 * known liveness line is defeated the moment a NEW liveness line appears; an
 * allow-list of what is unambiguously work survives it. LIN-2872 went through
 * three passes on this — the original "no `[working]` beat", the "ignore
 * Session launched" rework, and this allow-list — and the first two both missed
 * the incident rows on the reaper poll.
 *
 * The four evidence classes:
 *  - a CATEGORIZED heartbeat `[working · <category>]` — heartbeat.js:282's lead
 *    for a window with completed tools (`[working · editing]`,
 *    `[working · verifying]`, `[working · running]`, …). The category is what
 *    makes it work evidence; a bare `[working]` lead (liveness, "no tool calls
 *    in …", "Session launched/resumed", the reaper poll) is not.
 *  - an `[evidence]` row — a produced artifact (PR, CI run, link).
 *  - a `[usage]` row whose parsed payload has `outputTokens > 0` — the runner
 *    posts one `[usage] {…json…}` per turn (opencode-runner.js / hook.js); token
 *    output is work. A `[usage]` with `outputTokens` absent or 0 is not (a
 *    launch-time failure may still relay a zero-output usage row).
 *  - a `[ticket]` marker — the lane-marker relay's `[ticket] LIN-#### <state>`
 *    lines (LIN-2423).
 *
 * Consumed by `isLaunchTimeFailure` (LIN-2872).
 */
const CATEGORIZED_WORK_FEEDBACK_REGEX = /^\s*\[working\s*·\s*[^\]]+\]/i;
const EVIDENCE_FEEDBACK_REGEX = /^\s*\[evidence\]/i;
const USAGE_FEEDBACK_REGEX = /^\s*\[usage\]/i;
const TICKET_FEEDBACK_REGEX = /^\s*\[ticket\]/i;

/**
 * Parse a `[usage] {…json…}` feedback row's payload and report whether it has
 * genuine token output (`outputTokens` a finite number > 0). Tolerant, mirrors
 * `lib/session-telemetry.js`'s `parseUsagePayload` never-throws posture but kept
 * LOCAL — session-telemetry imports FROM this module, so importing its parser
 * here would create a cycle. Returns false on any non-string/malformed payload.
 *
 * @param {string} message
 * @returns {boolean}
 */
function usageRowHasOutputTokens(message) {
  ...
  return Number.isFinite(payload.outputTokens) && payload.outputTokens > 0;
}

/**
 * Whether a single feedback entry is evidence that the dispatch actually
 * started work (LIN-2872) — the allow-list consumed by `isLaunchTimeFailure`.
 * Pure; reads only the entry's `message`. Everything the harness posts that is
 * NOT one of the four evidence classes is liveness and is deliberately ignored.
 *
 * @param {{message?: string}|null|undefined} entry
 * @returns {boolean}
 */
function isWorkEvidence(entry) {
  const message = entry?.message || '';
  if (CATEGORIZED_WORK_FEEDBACK_REGEX.test(message)) return true;
  if (EVIDENCE_FEEDBACK_REGEX.test(message)) return true;
  if (TICKET_FEEDBACK_REGEX.test(message)) return true;
  if (USAGE_FEEDBACK_REGEX.test(message)) return usageRowHasOutputTokens(message);
  return false;
}
...
/**
 * Whether a dispatch died at launch before it started any work (LIN-2872): its
 * feedback ends terminal `[failed]` AND contains no evidence of work at all
 * (see the allow-list above). A dispatch that never started work is not a
 * duplicate risk — the retry of a launch-time failure is exactly the case the
 * five-minute duplicate window wrongly taxed — while a prior that reached any
 * work evidence (or any other terminal) genuinely ran and must keep blocking.
 *
 ...
 * executor's `[working] Session launched …` announcement, the opencode reaper's
 * `[working] (opencode — Ns so far; next check in …)` poll, `[working] Session
 * resumed. …`, `[started] …` — is LIVENESS, not work, and never fires the
 * predicate (LIN-2872 review F1/F2 and the LIN-2875 run's verification).
 *
 * Fails CLOSED by design: a missing/unparseable feedback list, or one whose
 * terminal marker is anything other than `[failed]`, returns false, so the
 * exemption never silently widens — the caller (the duplicate-dispatch guard)
 * keeps refusing whenever it cannot establish the launch-time failure.
 *
 * @param {Array<{message?: string}>} feedback
 * @returns {boolean} true only when the terminal status is `failed` and no
 *   work-evidence marker ever appeared
 */
export function isLaunchTimeFailure(feedback) {
  if (!Array.isArray(feedback)) return false;
  if (deriveTerminalStatus(feedback) !== 'failed') return false;
  return !feedback.some(isWorkEvidence);
}
...
export const __internal = { TERMINAL_FEEDBACK_REGEX, TERMINAL_MARKER_TO_STATUS, WAKE_FEEDBACK_REGEX, RESUME_MARKER_REGEX, CATEGORIZED_WORK_FEEDBACK_REGEX, EVIDENCE_FEEDBACK_REGEX, USAGE_FEEDBACK_REGEX, TICKET_FEEDBACK_REGEX };
