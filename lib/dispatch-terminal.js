...
// Duplicated from lib/dispatch-store.js's `feedbackTimestampMs` (LIN-1470) —
// deliberately NOT shared, to avoid enlarging the diff on a seam LIN-1461/
// LIN-1468 both churned within the last day. Name LIN-1465 here: when that
// ticket lands an explicit `.sort({_id: 1})` tiebreak, converge the two.
 ...
 * @returns {Array}  own feedback + verified sibling feedback, timestamp-ascending
 *   (NaN-safe: entries with a missing/unparseable timestamp sort last, ties broken
 *   by Array#sort's guaranteed stability — own feedback first, then each sibling's
 *   own entries in query order)
 */
export function mergeLineageFeedback(ownFeedback, siblingRows, anchor) {
