...
import { isDecisionAnsweredInLineage, answeredDecisionIdsByLineage } from './unanswered-decisions.js';
...
export function classifyLoop(loop, { superseded, now, staleMs, answeredByLineage } = {}) {
  ...
  if (lifecycle === 'blocked' && !supersededIds.has(loopId)) {
    ...
    if (isDecisionAnsweredInLineage(loop, answeredByLineage)) return 'resolved';
    return 'blocked';
  }
