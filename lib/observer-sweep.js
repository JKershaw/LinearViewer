...
import { isDecisionAnsweredInLineage, answeredDecisionIdsByLineage, isDecisionWithdrawn } from './unanswered-decisions.js';
 ...
 *
 * Two independent facts clear the blocked lifecycle, inside the same
 * `!superseded` guard: a follow-up loop resumed the row (supersession), or
 * the human answered its decision out of band (`isDecisionAnsweredInLineage`,
 * LIN-2671/LIN-2991 — the same predicate the rulings feed reads, so a row the
 * census calls `blocked` can never be one `list_pending_decisions` has
 * nothing to show for). Both land `resolved`, never `silent` — `silent` is
 * itself an attention lane and would keep a done-with row on the
 * waiting-on-human list.
 *
 ...
 * computed, once per tick, over the whole workspace read. `answeredByLineage`
 * (LIN-2991 design step 4) is `answeredDecisionIdsByLineage(loops)`, computed
 * once at the SAME call site, over the SAME `loops` array — never a second,
 * forked derivation.
 *
 ...
 * @param {Map<string, Set<string>>} [opts.answeredByLineage] - from `answeredDecisionIdsByLineage(loops)`; absent (not merely lineage-less) falls back to this loop's own answered set — see `isDecisionAnsweredInLineage`
 * @returns {'working'|'silent'|'blocked'|'terminal'|'queued'|'resolved'|'unknown'}
 */
export function classifyLoop(loop, { superseded, now, staleMs, answeredByLineage } = {}) {
  ...
  // import — this module's static-import allowlist is pinned (LIN-2671 lifted
  // it from four to five for `./unanswered-decisions.js`).
  //
  ...
  // `silent`. `status` is untouched, so this function is the only place that
  // computes a different value for a stamped row. This function now has
  // THREE call sites (LIN-2991 A2-corrected wording — see LIN-3022 plan-review
  // `18a547e2`): two thread a workspace-wide `answeredByLineage` map (the
  // census here, and `lib/chat-tools.js`'s `projectActiveSession`, which folds
  // lanes into `lifecycle`/`waitingOnHuman`, see docs/fossil-bookkeeping-pass.md)
  // and both also observe this stamped-row output. The third,
  // `scripts/fossil-pass-lin2633.js`'s `selectFossilRows`, deliberately passes
  // no map and relies on `isDecisionAnsweredInLineage`'s own-loop fallback
  // instead — NOT because it "only ever sees one loop at a time" (it reads a
  // workspace's full non-lean loop array and already computes `superseded`
  // over all of it, same as this function's other two callers); it simply
  // never needed lineage-wide grouping before this plan and isn't wired to
  // it now. That fallback safely NARROWS fossil eligibility relative to
  // today's scalar check (set membership accepts a superset of what scalar
  // equality accepted) — it does not reproduce today's exact behaviour.
  if (bookkeeping) {
    return 'resolved';
  }
  if (lifecycle === 'blocked' && !supersededIds.has(loopId)) {
    // LIN-2671: a blocked row whose decision has already been answered out of
    // band (John's ruling on LIN-2632's dispatch `67b7b85a`: answered in the
    // review, merged, but never resumed by a follow-up dispatch) is NOT waiting
    // on a human. The marker/agent-status channels above only ever say the row
    // once asked; `isDecisionAnsweredInLineage` (LIN-2991) is the independent
    // evidence that the human's obligation is discharged, unioned across the
    // whole lineage rather than this one loop's own last stamp (the C1/D1
    // last-stamp-wins collision this plan removes) — the SAME predicate the
    // rulings feed uses (`lib/unanswered-decisions.js`), imported by reference
    // rather than re-derived so the census and `list_pending_decisions` cannot
    // disagree again (the runner's 2026-09-05 observation: "blocked" in the
    // census, nothing to show in the queue).
    //
    // `resolved`, never `silent`: both `silent` and `blocked` are the
    // waiting-on-a-human lanes (`attention` below admits only those two), so a
    // `silent` lane would keep this row exactly where it must leave. The
    // `resolved` lane is the same operator-done-with meaning the
    // cancelled/expired and bookkeeping rows carry.
    //
    // Supersession is preserved, not replaced: this check is INSIDE the same
    // `!supersededIds.has(loopId)` guard, so a follow-up loop that resumed the
    // row still wins the classification path exactly as before — an answered
    // decision only discharges the blocked state when nothing supersedes it.
    // A row that is both answered and superseded keeps today's fall-through
    // (queued/active/unknown), never a newly-minted `resolved`.
    //
    // LIN-2891/LIN-3036: a WITHDRAWN decision discharges the blocked lane the
    // same way an answered one does. `isDecisionWithdrawn` is consulted ALONGSIDE
    // the answered check (never merged into it) and reads the loop's OWN
    // `withdrawal` field — item-scoped (Choice C), so a withdrawal on a sibling
    // loop never clears this one. `answered` and `withdrawn` both land
    // `resolved`, so the two signals agree with no special ordering.
    if (isDecisionAnsweredInLineage(loop, answeredByLineage) || isDecisionWithdrawn(loop)) return 'resolved';
    return 'blocked';
  }
 ...
 *
 * Duplicate-tick dedup (`advance()`'s `stateHash` gate) and the clock: the
 * hashed payload is NOT fully clock-invariant, and never has been — TWO
 * boundaries move it on a pure clock advance (identical fleet, only `now`
 * moved), each pinned by its own LIN-2647 test in
 * tests/unit/observer-sweep.test.js:
 *
 *  1. The lane census's `staleMs` working→silent boundary (pre-LIN-2619;
 *     `classifyLoop`'s `now - loopLastActivityMs(loop) > staleMs`). A
 *     `working` row crossing `DEFAULT_LANE_STALE_MS` moves `lanes`
 *     (working−1, silent+1) AND adds a member to `attentionKeysFull` — a
 *     genuine set-membership change, which the companion gate's no-delta
 *     fold (`lib/flight-companion-gate.js`) can and CORRECTLY does spend
 *     on: a row that stopped being worked and started waiting is a new
 *     attention fact, not clock noise.
 *  2. `staleAttentionCount`'s `FOSSIL_AGE_MS` boundary (LIN-2619): a pure
 *     clock advance that moves an attention row past `FOSSIL_AGE_MS`
 *     shifts it from enumerated `attention` into `staleAttentionCount`
 *     and changes the hash.
 *
 * What LIN-2619 guarantees — and the load-bearing half for the companion
 * gate — is narrower: `attentionKeysFull` is clock-independent ACROSS A
 * FOSSIL-BOUNDARY CROSSING specifically. The fossil filter never touches
 * it (loopId-sorted identity tuples over every attention-eligible row), so
 * a row merely ageing from enumerated into counted is not a set-membership
 * change the gate can see. That claim is scoped, not general: boundary (1)
 * above DOES change this key, by design. There is still no
 * `sweptAt`/age-in-ms field (the store stamps `updatedAt`/`lastSeenAt`
 * outside the hashed payload). The remaining trap stands unchanged:
 * `attention`/`attentionKeysFull` must be sorted here by the sweep itself,
 * since `stableStringify` (`lib/recap-cache.js:30-32`) sorts object keys
 * but preserves array order and `canonicalizeForHash`
 * (`lib/observer-state-store.js:215-217`) maps arrays without sorting
 * either.
...
export function buildSweepPayload(loops, { now, staleMs } = {}) {
  ...
  // LIN-2991 design step 4: computed once, over the same full workspace
  // `loops` array, right alongside `superseded` — never a second, forked
  // derivation. Threaded into every `classifyLoop` call in this tick's loop.
  ...
  for (const loop of loops) {
    const lane = classifyLoop(loop, { superseded, now, staleMs, answeredByLineage });
    lanes[lane] += 1;
