...
import { isDecisionAnswered } from './unanswered-decisions.js';
 ...
 * the human answered its decision out of band (`isDecisionAnswered`,
 * LIN-2671 — the same predicate the rulings feed reads, so a row the census
 * calls `blocked` can never be one `list_pending_decisions` has nothing to
 * show for). Both land `resolved`, never `silent` — `silent` is itself an
 * attention lane and would keep a done-with row on the waiting-on-human list.
 *
...
export function classifyLoop(loop, { superseded, now, staleMs } = {}) {
  ...
  // computes a different value for a stamped row — but it has two call sites
  // whose own output then differs too: the census here, and
  // `lib/chat-tools.js`'s `projectActiveSession`, which folds lanes into
  // `lifecycle`/`waitingOnHuman` (see docs/fossil-bookkeeping-pass.md).
  if (bookkeeping) {
    return 'resolved';
  }
  if (lifecycle === 'blocked' && !supersededIds.has(loopId)) {
    ...
    // once asked; `isDecisionAnswered` is the independent evidence that the
    // human's obligation is discharged — the SAME predicate the rulings feed
    // uses (`lib/unanswered-decisions.js`), imported by reference rather than
    // re-derived so the census and `list_pending_decisions` cannot disagree
    // again (the runner's 2026-09-05 observation: "blocked" in the census,
    // nothing to show in the queue).
    //
    ...
    if (isDecisionAnswered(loop)) return 'resolved';
    return 'blocked';
  }
