 ...
 *
 * Duplicate-tick dedup (`advance()`'s `stateHash` gate) and the clock, since
 * LIN-2619: `staleAttentionCount` is derived from `now`, so the hashed
 * payload is NO LONGER fully clock-invariant — a pure clock advance that
 * moves a row past `FOSSIL_AGE_MS` shifts it from enumerated `attention`
 * into `staleAttentionCount` and changes the hash. That is the one
 * deliberate exception; there is still no `sweptAt`/age-in-ms field (the
 * store stamps `updatedAt`/`lastSeenAt` outside the hashed payload), and
 * `attentionKeysFull` is deliberately clock-INDEPENDENT (loopId-sorted
 * identity tuples, untouched by the fossil filter) — which is exactly what
 * keeps the companion gate's no-delta fold (`lib/flight-companion-gate.js`)
 * correct across a fossil-boundary crossing. The remaining trap stands
 * unchanged: `attention`/`attentionKeysFull` must be sorted here by the
 ...
 * The boundary-crossing behaviour itself is pinned by the LIN-2647 test in
 * tests/unit/observer-sweep.test.js (hash moves, `attentionKeysFull` does
 * not).
