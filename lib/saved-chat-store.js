 ...
 * by `{ urlKey, linearUserId }` and are PRIVATE to that user within the
 ...
 * owns the identity gate: `req.session.linearUserId` is the only accepted
 ...
 *   linearUserId:   string,   // owning user (indexed)
...
/**
 * MongoDB/MangoDB-backed durable saved-chat store, private per {urlKey, linearUserId}.
 */
export class SavedChatStore {
   ...
   */
  async create(urlKey, linearUserId, { taskIdentifier, transcript } = {}) {
    if (!urlKey) throw new Error('urlKey is required');
    if (!linearUserId) throw new Error('linearUserId is required');
    ...
    await this._pruneToCapacity(urlKey, linearUserId);
    return toRecord(doc);
  }
   ...
   */
  async list(urlKey, linearUserId) {
    if (!urlKey || !linearUserId) return [];
    try {
      const docs = await this._docsFor(urlKey, linearUserId);
   ...
   * `{urlKey, linearUserId}` so a user can never read another user's chat.
   */
  async get(urlKey, linearUserId, id) {
    if (!urlKey || !linearUserId || !id) return null;
    try {
      const doc = await this.collection.findOne({ _id: id, urlKey, linearUserId });
      return toRecord(doc);
    } catch (err) {
   ...
   * `{urlKey, linearUserId}` so a user can only delete their own.
   *
   * @returns {Promise<boolean>} true if a document was removed
   */
  async delete(urlKey, linearUserId, id) {
    if (!urlKey || !linearUserId || !id) return false;
    try {
      const result = await this.collection.deleteOne({ _id: id, urlKey, linearUserId });
  ...
  async _docsFor(urlKey, linearUserId) {
    const docs = await this.collection.find({ urlKey, linearUserId }).toArray();
  ...
  async _pruneToCapacity(urlKey, linearUserId) {
    try {
      const docs = await this._docsFor(urlKey, linearUserId);
      for (const doc of docs.slice(this.maxPerUser)) {
        await this.collection.deleteOne({ _id: doc._id, urlKey, linearUserId });
      }
    } catch (err) {
