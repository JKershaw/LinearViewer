 ...
 *     content-bearing, so this store is session-auth only, with ONE narrow,
 *     deliberate exception (LIN-2634): a creator-scoped, read-only query
 *     (`listByTask`) reachable over the proxy/token-auth surface
 *     (`GET /api/proxy/flight-companion/transcripts`, routes/proxy-flight-
 *     companion.js) — never a blanket wiring, never a write, and still never
 *     reachable from /kpis (lib/kpi-stats.js) or the workspace API.
 *
...
export class SavedChatStore {
  ...
  /**
   * Full saved chats for a user, filtered to one taskIdentifier, newest-first,
   * transcripts included. Scoped by {urlKey, accountId, taskIdentifier} in the
   * query itself — never post-filtered. Deliberately does NOT swallow errors to
   * [] the way list() does: a store failure must stay distinguishable from
   * "genuinely no matches".
   */
  async listByTask(urlKey, accountId, taskIdentifier) {
    if (!urlKey || !accountId || !taskIdentifier) return [];
    const docs = await this.collection.find({ urlKey, accountId, taskIdentifier }).toArray();
