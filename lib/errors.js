...
/**
 * The provider named in `classifyUpstreamError`'s `detail` when a caller supplies
 * none (LIN-2363). Linear is the historical single provider, so defaulting to it
 * keeps every existing call site byte-identical; a caller that KNOWS which backend
 * it called passes that name, and a caller that resolved nothing passes `null` for
 * neutral wording. This is a back-compat default for callers, deliberately not the
 * same thing as `getProviderForWorkspace`'s LEGACY_DEFAULT_PROVIDER, which resolves
 * a workspace's actual provider.
 */
const LEGACY_DETAIL_PROVIDER = 'Linear';
 ...
 *
 * PROVIDER ATTRIBUTION (LIN-2363). `providerDisplayName` names the backend in the
 * human-readable `detail`. It DEFAULTS to `'Linear'`, so every existing caller
 * that passes nothing keeps byte-identical output — the callers this ticket does
 * not own (`lib/render-pages.js`'s session-authed HTML dashboard lane, explicitly
 * recorded as unowned by both LIN-2351's review and LIN-2363) are unchanged.
 * Pass an explicit `null` to get provider-neutral wording instead: that is what a
 * caller does when nothing resolved, so an unattributable failure is never
 * attributed to Linear by default.
 *
 * The `LINEAR_*` `code`s are deliberately NOT parameterised. They are a published,
 * machine-matchable contract (`docs/proxy-integration.md`), renaming them is a
 * breaking change, and retiring the vocabulary needs a deprecation path — the same
 * boundary LIN-2351 held and LIN-2363 holds again. Only the human string moves.
 *
 * @param {*} error - the caught error
 * @param {string|null} [providerDisplayName='Linear'] - backend to name in `detail`;
 *   `null` ⇒ provider-neutral wording.
 * @returns {{code: string, category: string, retryable: boolean, detail: string}}
 */
export function classifyUpstreamError(error, providerDisplayName = LEGACY_DETAIL_PROVIDER) {
  const status = error?.response?.status ?? error?.status;

  if (status === 401 || status === 403) {
    return { code: 'LINEAR_AUTH', category: 'auth', retryable: false,
      detail: providerDisplayName
        ? `${providerDisplayName} rejected the request as unauthenticated.`
        : 'The upstream provider rejected the request as unauthenticated.' };
  }
  if (status === 429) {
    return { code: 'LINEAR_RATE_LIMITED', category: 'upstream', retryable: true,
      detail: providerDisplayName
        ? `${providerDisplayName} rate-limited the request; it should recover shortly.`
        : 'The upstream provider rate-limited the request; it should recover shortly.' };
  }
  if (typeof status === 'number' && status >= 500) {
    return { code: 'LINEAR_UPSTREAM_5XX', category: 'upstream', retryable: true,
      detail: providerDisplayName
        ? `${providerDisplayName} returned a ${status} server error.`
        : `The upstream provider returned a ${status} server error.` };
  }
  ...
  if (isNetworkError(error)) {
    return { code: 'LINEAR_UNREACHABLE', category: 'upstream', retryable: true,
      detail: providerDisplayName
        ? `The connection to ${providerDisplayName} closed before a response arrived — usually transient.`
        : 'The connection to the upstream provider closed before a response arrived — usually transient.' };
  }
 ...
 * deliberately NOT `classifyUpstreamError` — that one builds its message from the
 * status alone, while this path surfaces GitHub's own human error text, which is
 * strictly more informative when it exists. (Until LIN-2363 the stated reason was
 * that `classifyUpstreamError`'s wording was Linear-hardcoded; it is now
 * provider-parameterised, so that justification no longer holds and has been
 * replaced with the one that still does.) The status→category mapping mirrors it
 * so the two paths classify auth/upstream/network/internal the same way.
 *
...
const WORKSPACE_UNAVAILABLE_BY_REASON = {
  store_unreachable: {
    ...
    category: 'upstream',
    retryable: true,
    // LIN-1746: a transient infra blip, not a credential-death signal —
    // excluded from NON_OWNERLESS_WORKSPACE_DEATH_REASONS below.
    nonOwnerlessDeath: false,
  ...
  token_ownerless: {
    code: 'TOKEN_HAS_NO_OWNER',
    category: 'config',
    retryable: false,
    // LIN-1746: a property of the CALLING TOKEN, not the workspace credential
    // — its own distinct LIN-1448 remedy (re-issue the token, not re-dispatch)
    // — excluded from NON_OWNERLESS_WORKSPACE_DEATH_REASONS below.
    nonOwnerlessDeath: false,
    // LIN-1448. Split out of not_connected, whose copy sends the reader to the one
    // remedy that provably cannot work here. This failure is a property of the
    // CALLING TOKEN (minted with createdBy: null, so it can never resolve an
    // owner), not of the workspace — which is usually healthy and serving other
    // tokens 200s in the same second (LIN-1576's 09:35–11:30 census). The copy
    // therefore has one job: stop the reader reconnecting the workspace, and point
    // at re-issuing the token. `category: 'config'` and non-retryable match
    // not_connected — retrying cannot help, and neither can signing in, because a
    // token's owner stamp is fixed at mint time and nothing repairs it in place
    // (LIN-1413 considered and rejected rebinding `createdBy`).
    detail: 'This token was minted without an owner, so it can never resolve a workspace credential. The workspace itself may be healthy and serving other tokens — signing in again will not change this token. Re-issue the token from an account that has the workspace connected.'
  },
  owner_mismatch: {
  ...
  owner_signed_out: {
    code: 'WORKSPACE_OWNER_SIGNED_OUT',
    category: 'auth',
    retryable: false,
    nonOwnerlessDeath: true,
    // Hedged like owner_mismatch, for a symmetric reason: detectOwnerSignedOut
    // (lib/workspace-token-resolver.js) proves only that the owner has no
    // session row anywhere right now — not that they permanently lost the
    // workspace. Unlike owner_mismatch, category is 'auth' here (not
    // 'config'): re-authenticating is exactly the fix when it's simply that
    // the owner is logged out (LIN-1506).
    detail: 'No active session exists for this account. Sign in again, or issue a fresh token, to restore access.'
  }
};

// LIN-1746: the exact non-ownerless credential-death class lib/proxy-events.js's
// `recentFailureReasons` tallies — derived from the `nonOwnerlessDeath` tag
// above (not a second, hand-duplicated reason list) so a future reason added
// to WORKSPACE_UNAVAILABLE_BY_REASON without an explicit `nonOwnerlessDeath`
// tag fails loudly here (`undefined` is falsy, so it defaults OUT of the set
// — the safe default, since a wrongly-INCLUDED reason pollutes a worker's
// dead-credential verdict, while a wrongly-excluded one only delays it by one
// ticket) rather than silently drifting the two lists apart.
export const NON_OWNERLESS_WORKSPACE_DEATH_REASONS = Object.freeze(
  new Set(Object.entries(WORKSPACE_UNAVAILABLE_BY_REASON).filter(([, v]) => v.nonOwnerlessDeath).map(([k]) => k))
);
 ...
 *
 * @param {string} reason - one of store_unreachable | session_expired | not_connected | token_ownerless | owner_mismatch | owner_signed_out
