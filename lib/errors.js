...
export function classifyUpstreamError(error, providerDisplayName = LEGACY_DETAIL_PROVIDER) {
  ...
  if (status === 401 || status === 403) {
    return { code: 'LINEAR_AUTH', category: 'auth', retryable: false,
      detail: name
        ? `${name} rejected the request as unauthenticated.`
  ...
  if (status === 429) {
    return { code: 'LINEAR_RATE_LIMITED', category: 'upstream', retryable: true,
      detail: name
        ? `${name} rate-limited the request; it should recover shortly.`
  ...
  if (typeof status === 'number' && status >= 500) {
    return { code: 'LINEAR_UPSTREAM_5XX', category: 'upstream', retryable: true,
      detail: name
        ? `${name} returned a ${status} server error.`
  ...
  if (isNetworkError(error)) {
    return { code: 'LINEAR_UNREACHABLE', category: 'upstream', retryable: true,
      detail: name
        ? `The connection to ${name} closed before a response arrived — usually transient.`
