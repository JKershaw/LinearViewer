 ...
 * agentSummary/feedback), NOT the Linear issue context.
 *
...
import { isDecisionLifecycleStampEntry } from './pipeline-loops.js';
...
export function formatRunContext(loop) {
  ...
  if (loop.agentAction) lines.push(`Agent action: ${loop.agentAction}`);
  if (loop.agentStatus) lines.push(`Agent status: ${loop.agentStatus}`);
  ...
  if (loop.agentSummary) {
    ...
    lines.push(String(loop.agentSummary).slice(0, 2000));
  }

  // LIN-1728/LIN-3037: exclude decision-lifecycle stamps before taking the
  // last-8 tail — a stamp is answer/withdrawal metadata, not a fact about the
  // run, and must not consume a slot or inject a bare JSON line (or a
  // withdrawal's free-text reason) into the summary prompt.
  const feedback = (Array.isArray(loop.feedback) ? loop.feedback : []).filter(fb => !isDecisionLifecycleStampEntry(fb));
