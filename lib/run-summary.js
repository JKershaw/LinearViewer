...
export function formatRunContext(loop) {
  ...
  // LIN-1728: exclude `decision-answer` stamps before taking the last-8 tail —
  // the stamp is answer metadata, not a fact about the run, and must not
  // consume a slot or inject a bare JSON line into the summary prompt.
  const feedback = (Array.isArray(loop.feedback) ? loop.feedback : []).filter(fb => !isDecisionAnswerEntry(fb));
