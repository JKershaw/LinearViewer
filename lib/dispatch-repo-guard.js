 ...
 * `fetchProjects` -> `knownWorkspaceRepos` computation `validateDispatchRepo`
 ...
 */
export async function fetchKnownRepos({ provider, scope, timeoutMs = FETCH_PROJECTS_TIMEOUT_MS }) {
  if (!provider) return { ok: false, reason: 'no-provider' };
  if (typeof provider.supports !== 'function' || !provider.supports('fetchProjects')) {
