 ...
 * BEFORE enqueueing, so an unrecognized-by-the-tracker value never reaches
 * the runner at all.
 *
...
/**
 * Bound on the extra `fetchProjectsList` round-trip this validation adds to a
 ...
 *
 * `fetchProjectsList` (not `fetchProjects`, LIN-2974): the inventory this
 * guard needs is project metadata only (`content`, for the `repo=` line) —
 * `fetchProjects` additionally pages through every issue in the workspace,
 * which routinely pushed this round-trip toward (and past) this very bound.
 * `fetchProjectsList` runs the same projects query without that issue walk.
 */
const FETCH_PROJECTS_LIST_TIMEOUT_MS = 8_000;

function withTimeout(promise, ms) {
  let timer;
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(() => reject(new Error('dispatch-repo-guard: fetchProjectsList timed out')), ms);
  });
...
/**
 * Fetches the workspace's current known-repos inventory: the same
 * `fetchProjectsList` -> `knownWorkspaceRepos` computation `validateDispatchRepo`
 * runs internally, but on its own — no `repo` value to check against it.
 * This is the seam the read-only known-repos endpoint (LIN-2974) uses, so
 * that endpoint and the dispatch-time guard can never silently diverge on
 * what "the inventory" means.
 *
 * Uses `fetchProjectsList`, not `fetchProjects` (LIN-2974): both providers'
 * implementations run the identical project-metadata query (same `content`
 * field `knownWorkspaceRepos` reads the `repo=` line from), but
 * `fetchProjectsList` skips `fetchProjects`'s per-workspace issue walk, so it
 * answers well inside `timeoutMs` where `fetchProjects` routinely didn't —
 * every built-in provider (linear/github/github-projects/jira/local) was
 * checked to confirm its `fetchProjectsList` carries `content` before this
 * switch (none silently degrade to an empty inventory).
 *
 * Distinguishes "the provider was reachable and returned zero non-null
 * repos" from "the provider could not be asked at all" — `validateDispatchRepo`
 * folds the latter into its own fail-OPEN pass-through (a capability gap must
 * never block a dispatch), but a bare inventory read has no dispatch to
 * protect, so it reports the gap as `ok: false` rather than papering over it
 * with an empty list a caller could mistake for "this workspace has no
 * repos".
 *
 * @param {Object} params
 ...
 * @param {number} [params.timeoutMs] - Override for `FETCH_PROJECTS_LIST_TIMEOUT_MS`.
 *   Test-injection seam only — production callers never pass this.
 * @returns {Promise<{ok: true, knownRepos: string[]} | {ok: false, reason: 'no-provider'|'unsupported'|'fetch-failed'}>}
 */
export async function fetchKnownRepos({ provider, scope, timeoutMs = FETCH_PROJECTS_LIST_TIMEOUT_MS }) {
  if (!provider) return { ok: false, reason: 'no-provider' };
  if (typeof provider.supports !== 'function' || !provider.supports('fetchProjectsList')) {
    return { ok: false, reason: 'unsupported' };
  }

  let projects;
  try {
    projects = await withTimeout(provider.fetchProjectsList(scope), timeoutMs);
  } catch {
    return { ok: false, reason: 'fetch-failed' };
  }

  const knownRepos = knownWorkspaceRepos(projects || []).map(row => row.repo).filter(Boolean);
  return { ok: true, knownRepos };
}
 ...
 * capability isn't there to check with: no provider, no `fetchProjectsList`
 * support (`provider.supports('fetchProjectsList')`), or the fetch itself
 ...
 */
export async function validateDispatchRepo({ repo, provider, scope, timeoutMs = FETCH_PROJECTS_LIST_TIMEOUT_MS }) {
  if (!repo) return { ok: true, repo: repo || null, validated: false };

  const inventory = await fetchKnownRepos({ provider, scope, timeoutMs });
  if (!inventory.ok) return { ok: true, repo, validated: false };

  const resolved = resolveKnownRepo(repo, inventory.knownRepos);
  if (resolved.ok) return { ok: true, repo: resolved.repo, validated: true };
  return { ok: false, knownRepos: inventory.knownRepos };
}
