 ...
 * @returns {string} Full HTML document
 */
export function renderMergeConfirmPage() {
  ...
  <div class="login-container" data-testid="merge-confirm-page">
    ...
    <p class="error-message">This Linear account is already connected to a different Harbour account. Since you're signed in as your other account right now, this looks like the same person — merge them?</p>
    ${body}
  </div>`
  })
}
 ...
 * @returns {string} Full HTML document
 */
export function renderMergeReauthRequiredPage() {
  return renderPage({
  ...
  <div class="error-container" data-testid="merge-reauth-required-page">
    ...
    <p class="error-message">This Linear account is already connected to a different Harbour account. To merge them, we need to confirm you're recently signed in on BOTH — please sign in again on your current account, then reconnect this one.</p>
    <a href="/auth/linear" class="login-button">Sign in again</a>
