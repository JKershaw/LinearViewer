 ...
 *
 * This form is add-source only, and the REASON matters because an earlier
 * version of this comment gave a wrong one. It said "there is no top-level
 * 'Continue with Jira' entry point" — false since LIN-1890, which added
 * exactly that (`lib/components/landing-hero.js`'s `landing-cta-jira` and the
 * nav's `nav-login-jira`, both routed to `/auth/jira/oauth` with the default
 * `mode: 'new'`). The real reason is unchanged by that: an API token
 * authenticates a WORKSPACE BINDING, not a human, so it cannot establish a
 * login. The OAuth 3LO path is what carries the top-level entry.
 *
...
export function renderJiraLinkForm({ workspaceUrlKey, error } = {}) {
  ...
  <div class="login-container" data-testid="jira-link-page">
    ...
    <p>Enter your Jira Cloud site and an API token. Harbour signs in as you with it, so it can do anything your own Jira account can — including changing issues: updating them, commenting, and adding or removing labels.</p>
    ${errorHtml}
    ${body}
  </div>`
  })
