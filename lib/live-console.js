 ...
 * `credential` come from the identity source too, UNLESS it is itself a stale
 * terminal loop — a terminal loop's `agentAction`/`agentTokenId` are its last
 * values from BEFORE it went terminal, so substituting them would render a
 * stale action/credential as live; in that case they come from the liveness
 * source's own (honest, typically absent) fields instead. An unresolvable
 * target (aged out / never present) falls back to the loop's own fields.
 *
...
export function deriveLoopLanes(loops, { credentialByToken = {} } = {}) {
  ...
  for (const members of groups.values()) {
    ...
    // A stale terminal identity source (the ticket's own primary case: a
    // terminal target resumed by a wake) must never donate action/credential
    // — its agentAction/agentTokenId are snapshots from BEFORE it went
    // terminal, so substituting them would render a stale value as live
    // (plan-review finding 10(d)).
    const identityIsStaleTerminal = identitySource !== liveMember && !!identitySource.terminalStatus;
