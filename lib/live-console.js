...
import { STALE_AFTER_MS, loopLastActivityMs } from './staleness.js';
 ...
 * only shows what's genuinely moving. Sourced from the shared, system-wide
 * staleness definition (LIN-1445) so the Live Console and the Observation feed
 * agree on the threshold; re-exported under this name for back-compat.
 */
export const DEFAULT_LANE_STALE_MS = STALE_AFTER_MS;
 ...
 * @returns {Array<Object>}
 */
export function deriveLoopLanes(loops) {
  if (!Array.isArray(loops)) return [];
  const lanes = [];
  for (const lp of loops) {
    ...
    lanes.push({
      ...
      lastActivityMs: loopLastActivityMs(lp), // shared, heartbeat-aware (LIN-1445)
...
export function buildConsoleFeed(input, opts = {}) {
  ...
  let lanes = mergeLanes(deriveLoopLanes(loops), deriveLanes(statusEvents));
  ...
  const summary = { active: lanes.length, done: 0, failed: 0, blocked: 0, total: allEvents.length };
  ...
  return { events: page, lanes, tempo, pulse, summary, hasMore, oldestTs, serverNow: now };
}
