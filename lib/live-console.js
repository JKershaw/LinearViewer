...
/**
 * LIN-1505 Phase C: strip-only zoom. Bucket COUNT stays fixed at every span
 * (payload always 72 numbers: buckets[] + load[]) — only `bucketMs` scales
 * with the requested window. Spans snap to one of these four rungs so the
 * wire carries at most four shapes; the strip never pans and never requests
 * an arbitrary span.
 */
export const PULSE_BUCKET_COUNT = 36; // fixed at every span — payload stays 72 numbers
export const PULSE_SPAN_RUNGS_MS = [
  DEFAULT_PULSE_WINDOW_MS,      // 3 min — floor, unchanged default
  15 * 60 * 1000,                // 15 min
  60 * 60 * 1000,                // 1 h
  6 * 60 * 60 * 1000,            // 6 h — ceiling
];
export const PULSE_MAX_WINDOW_MS = PULSE_SPAN_RUNGS_MS[PULSE_SPAN_RUNGS_MS.length - 1];

/**
 * Snap an arbitrary requested span to the nearest rung >= it. Never trust a
 * free-form client value as a store-facing window — the wire carries at most
 * four shapes, always.
 */
export function snapPulseWindowMs(ms) {
  const n = Number(ms);
  if (!Number.isFinite(n) || n <= 0) return DEFAULT_PULSE_WINDOW_MS;
  for (const rung of PULSE_SPAN_RUNGS_MS) if (n <= rung) return rung;
  return PULSE_MAX_WINDOW_MS;
}
 ...
 *
 * LIN-2905 (requirement iv): a wake resuming a session is a SECOND loop for
 * the same session, carrying the triggering child's own `issueIdentifier`
 * (LIN-2121) rather than the target's — left unfolded, it would surface as
 * its own mislabeled "Working now" lane. This resolves each loop's IDENTITY
 * SOURCE — a `kind:'wake'` loop with a `followUpTo` resolvable in the full
 * loop set resolves to that target loop; anything else is its own identity
 * source — then groups ACTIVE loops by identity-source `loopId`, so a wake
 * and the target it resumes always land in the SAME group and emit exactly
 * ONE lane (this is what makes a `mergeLanes` key collision structurally
 * impossible). Within a group, the FRESHEST member (by `loopLastActivityMs`)
 * is the liveness source — a partial substitution, never a full loop swap, so
 * a wake's live heartbeat/`lastActivityMs`/`sinceMs`/ticketWalk/parkedWait are
 * never lost. `task` always comes from the identity source (a plain stored
 * field, always resolvable when the identity source is present). `action`/
 * `credential` come from the identity source too, UNLESS it is itself NOT
 * ACTIVE (`isLoopActive`, covering terminal AND waiting/blocked/parked/
 * complete/error/queued alike) — an inactive loop's `agentAction`/
 * `agentTokenId` are its last values from BEFORE it stopped being active, so
 * substituting them would render a stale action/credential as live; in that
 * case they come from the liveness source's own (honest, typically absent)
 * fields instead. An unresolvable target (aged out / never present) falls
 * back to the loop's own fields.
 *
...
export function deriveLoopLanes(loops, { credentialByToken = {} } = {}) {
  ...
  for (const lp of loops) {
    if (lp && lp.loopId != null) byLoopId.set(lp.loopId, lp);
  }

  const identitySourceOf = (lp) => {
    if (lp.kind === 'wake' && lp.followUpTo && byLoopId.has(lp.followUpTo)) {
      return byLoopId.get(lp.followUpTo);
    }
    return lp;
  };
  ...
  for (const lp of loops) {
    ...
    const key = identitySourceOf(lp).loopId ?? lp.loopId;
  ...
  for (const members of groups.values()) {
    // Liveness source: the freshest member supplies heartbeat/lastActivityMs/
    // sinceMs/ticketWalk/parkedWait (LIN-2243/LIN-2244 ownership) — a partial
    // substitution, never a full loop swap.
    let liveMember = members[0];
    let liveMs = loopLastActivityMs(liveMember);
    for (let i = 1; i < members.length; i++) {
      const ms = loopLastActivityMs(members[i]);
      if (ms > liveMs) { liveMember = members[i]; liveMs = ms; }
    }

    const identitySource = identitySourceOf(liveMember);
    // A stale (non-active) identity source (the ticket's own primary case: a
    // terminal OR waiting/blocked/parked target resumed by a wake) must never
    // donate action/credential — its agentAction/agentTokenId are snapshots
    // from BEFORE it stopped being active, so substituting them would render
    // a stale value as live (plan-review finding 10(d)). Keying on
    // `terminalStatus` alone missed every inactive-but-not-terminal case
    // (e.g. `waiting`/blocked on a human — the canonical case a wake
    // resumes, LIN-826) and let a parked target's last pre-park action leak
    // into a live lane (LIN-2905 review finding B1) — `isLoopActive` covers
    // the full inactive surface, not just the terminal-marker slice of it.
    const identityIsStaleTerminal = identitySource !== liveMember && !isLoopActive(identitySource);
    const fieldSource = identityIsStaleTerminal ? liveMember : identitySource;

    const dispatchedMs = _epoch(liveMember.dispatchedAt);
    const agentMs = _epoch(liveMember.agentTimestamp);

    lanes.push({
      workspaceUrlKey: liveMember.workspaceUrlKey || null,
      workspaceName: liveMember.workspaceName || liveMember.workspaceUrlKey || null,
      task: identitySource.issueIdentifier,
      action: fieldSource.agentAction || fieldSource.stage || 'working',
      summary: String(liveMember.agentSummary || '').slice(0, SUMMARY_MAX),
      sinceMs: dispatchedMs ?? agentMs ?? 0,
      lastActivityMs: liveMs,
      heartbeat: latestHeartbeat(liveMember),
      // LIN-2243: a worker-lane's per-ticket walk (lib/session-telemetry.js),
      // so a lane's chip can show "ticket N of M so far" instead of one
      // identifier for the lane's whole life. null for a non-lane loop.
      ticketWalk: liveMember.telemetry?.ticketWalk || null,
      // LIN-2244: currently parked on an async wait (e.g. a ScheduleWakeup CI
      // poll) — a THIRD lane state, distinct from both actively working and
      // blocked on a human. null when not currently parked.
      parkedWait: liveMember.telemetry?.parkedWait || null,
      ...
      credential: {
        state: resolveCredentialState(fieldSource.agentTokenId, credentialByToken),
        label: fieldSource.agentTokenLabel || null,
      },
    });
  }
...
export function buildTimeline(loops, { now, staleMs = DEFAULT_LANE_STALE_MS } = {}) {
  ...
  for (const lp of loops) {
    ...
    runs.push({
      ...
      workspaceUrlKey: lp.workspaceUrlKey || null,
      // LIN-2243: carried so the timeline bar can annotate its label/tooltip
      // with ticket-transition progress instead of one static identifier for
      // the run's whole life. null for a non-lane run.
      ticketWalk: lp.telemetry?.ticketWalk || null,
    });
  }
 ...
 * pointing outside the list (the predecessor aged out of the 24h window, or
 * resolves to a folded `kind:'wake'` run with no bar of its own — LIN-2905)
 ...
 * the edge from.
 *
 * LIN-2905: a `kind:'wake'` run never gets a row/bar of its own — it is
 * folded into the target run it resumes (`followUpTo`, exact and
 * nesting-independent — a wake's `followUpTo` always equals its target's own
 * `id`) as a `wakeMarkers[]` entry, so a chatty coordinator's hundred wake
 * dispatches don't flood the timeline with their own bars. A wake whose
 * target isn't present in `runs` (aged out of the 24h window, or dropped by
 * `TIMELINE_RUN_CAP`) is dropped outright — no marker, no bar, no orphan
 * `connectorTruncated` row.
 *
...
export function packTimelineRows(runs, { groupKeyOf = (r) => r.groupKey } = {}) {
  ...
  // LIN-2905 (i)/(ii): accumulate wake markers into a plain id-keyed map
  // BEFORE the decorate pass, keyed by TARGET id — never a write through
  // byId's references to buildTimeline's own run objects (that would mutate
  // the caller's input). A wake run itself never enters `nonWakeRuns`/
  // `groups`/the pack pipeline, so it produces no row of its own; an
  // unresolvable target (aged out of the window or truncated by
  // TIMELINE_RUN_CAP — byId.has() is false identically for both) drops the
  // wake outright.
  //
  // L4 (implementation review, LIN-2905): a THIRD, undocumented-until-now
  // drop path exists — a wake whose `followUpTo` resolves to ANOTHER wake
  // (itself already folded away here) passes `byId.has()` (byId is built
  // from the full, unfiltered `runs` list, wakes included) and accumulates a
  // marker keyed to that other wake's id. But only `nonWakeRuns` ever get
  // decorated with `wakeMarkers` below, so a marker keyed to a wake's own id
  // is never attached to anything and is silently dropped. This is
  // intentional, not a bug to chase: `dispatch-wake.js:4d45e0ec` (LIN-901)
  // already guarantees a wake can never itself beget a wake, so this path is
  // unreachable in practice — but "convention says it cannot happen" is the
  // same reachability argument F3 rejected for its own sibling case, so it
  // is pinned by a regression test (not silently relied upon) rather than
  // folded onto the root target.
  const wakeMarkersByTargetId = new Map();
  const nonWakeRuns = [];
  for (const r of runs) {
    if (r.kind !== 'wake') { nonWakeRuns.push(r); continue; }
    if (r.followUpTo && byId.has(r.followUpTo)) {
      if (!wakeMarkersByTargetId.has(r.followUpTo)) wakeMarkersByTargetId.set(r.followUpTo, []);
      wakeMarkersByTargetId.get(r.followUpTo).push({
        id: r.id,
        at: r.start,
        issueIdentifier: r.issueIdentifier,
        // F1 (plan-review pass 2): carry the folded wake's own outcome/
        // liveness so a wake that's still running (or ended non-done) can
        // render distinctly from a quiet, successful one instead of
        // disappearing into an indistinguishable tick.
  ...
  // (never leaving it absent-when-false) AND its own `wakeMarkers`/
  // `wakeCount`, without mutating the input runs. Only non-wake runs are
  // decorated/grouped/packed — unaffected for every non-wake run, satisfying
  // (iii) by construction, since no non-wake run carries `kind: 'wake'`.
  const decorated = nonWakeRuns.map(r => {
    const wakeMarkers = wakeMarkersByTargetId.get(r.id) || [];
    const base = { ...r, wakeMarkers, wakeCount: wakeMarkers.length };
    // F3 (plan-review pass 2): a non-wake run's followUpTo can resolve to a
    // run that WAS a wake — that wake was just folded away above and has no
    // bar of its own, so there's nothing to connect to. Treat it the same as
    // an out-of-window/truncated target: no connector, marked truncated.
    const target = r.followUpTo ? byId.get(r.followUpTo) : null;
    if (target && target.kind !== 'wake') {
      connectors.push({ fromId: r.followUpTo, toId: r.id });
      return { ...base, connectorTruncated: false };
    }
    return { ...base, connectorTruncated: !!r.followUpTo };
  });
 ...
 * data-rate signal), `load[]` accumulates each beat's own magnitude — `toolCount`
 * falling back to `total` — so the client can render how much work a beat
...
export function buildPulse(loops, { now, windowMs = DEFAULT_PULSE_WINDOW_MS, bucketMs = DEFAULT_PULSE_BUCKET_MS } = {}) {
  ...
  // Anchor the grid to an absolute, bucket-aligned instant rather than the raw
  // response time `now` — otherwise latency variance between polls shifts the
  // grid's phase and the SAME fixed heartbeat re-quantises into a different
  // bucket every poll (LIN-1505: the dominant cause of the strip's jerk,
  // measured up to 9.67 css-px in one frame). The admission window below still
  // uses `now` itself, so a beat only becomes visible once actually observed.
  const gridEnd = Math.floor(now / bucketMs) * bucketMs;
  const buckets = new Array(count).fill(0);
  const load = new Array(count).fill(0);
  const start = gridEnd - count * bucketMs;
  for (const lp of Array.isArray(loops) ? loops : []) {
    ...
    for (const m of ms) {
      ...
      let idx = count - 1 - Math.floor((gridEnd - t) / bucketMs);
      if (idx < 0) idx = 0;
  ...
  // `endTs` must switch to the quantised anchor together with the binning
  // above — otherwise the client's tsCenter math still positions buckets
  // against the old jittery raw-`now` anchor, silently reintroducing the jerk
  // visually even though the bins themselves are now stable.
  return { bucketMs, endTs: gridEnd, buckets, load };
}
 ...
 * @param {number} [opts.pulseWindowMs=DEFAULT_PULSE_WINDOW_MS] - LIN-1505
 *   Phase C: the flowing strip's span. Bucket COUNT stays fixed
 *   (PULSE_BUCKET_COUNT) at every span — only `bucketMs` is derived from it —
 *   so the payload size never changes and no extra store read is needed (the
 *   loops read already carries up to 24h). Callers should snap this to
 *   `PULSE_SPAN_RUNGS_MS` via `snapPulseWindowMs` first; passed through as-is
 *   otherwise.
...
export function buildConsoleFeed(input, opts = {}) {
  ...
  const pulseBucketMs = Math.round(pulseWindowMs / PULSE_BUCKET_COUNT);
  const pulse = buildPulse(loops, { now, windowMs: pulseWindowMs, bucketMs: pulseBucketMs });
