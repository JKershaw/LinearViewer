 ...
 * @returns {string}
 */
export function stateOptionValue(state) {
 ...
 *
 * Exported (alongside `stateOptionValue` above) for the dedicated task-create page
 * (LIN-1973), which renders the same state `<select>` contract — one resolution
 * order, two surfaces, so it can't drift the way a second copy would.
 */
export function byBoardPosition(a, b) {
 ...
 * @param {{priority?: boolean}} [options.ui] - The provider's `ui` capability
 *   surface (LIN-1886). Only `priority` is read: `false` hides the priority
 *   control entirely (Jira, whose priority is unmapped) rather than rendering
 *   one whose value can never actually apply; every other provider defaults to
 *   `true` and keeps the control.
...
export function renderTaskEditPage({ issue, states = [], urlKey, issueId = '', source = '' }, options = {}) {
  ...
  // JS (the `.inline-issue-trigger` reveal handler in public/app.js — since
  // removed by LIN-1973, which moved CREATE to its own drill-down page too);
  // the attribute is the server-rendered equivalent — no script, and unlike a
  // focus() call it is already in effect during the pre-arm window above.
  const formBody = `<form class="task-edit-form" data-task-edit data-issue-id="${issueIdAttr}" data-url-key="${urlKeyAttr}" data-source="${sourceAttr}" data-testid="task-edit-form">
      ...
      <div class="task-edit-row">
        ...
        ${ui.priority !== false ? `<div class="task-edit-field">
