...
import { formatFossilThreshold } from './prompts/flight-companion-brief.js';
...
function renderObserverReportPanel(observerReportDoc) {
  ...
  // LIN-2645 (LIN-2619 release companion): the fossil summary line, rendered
  // from `report.*` (the panel never reads the census doc directly). Since
  // LIN-2619 the census folds rows past the staleness threshold into
  // `staleAttentionCount` and scopes `truncated` to the fresh population, so a
  // report can show a short attention list with `truncated: false` and a large
  // fossil tail. Without this line that reads as a clean fleet. The count is
  // the load-bearing agreement with the companion seed's own line
  // (`buildCensusSeedText`, routes/flight-companion.js); the threshold is
  // formatted through the SAME shared helper so the wording cannot drift.
  const staleCount = typeof report.staleAttentionCount === 'number' && report.staleAttentionCount > 0
    ? report.staleAttentionCount
    : null;
  const staleHtml = staleCount
    ? `<p class="fc-obs-stale">+${escapeHtml(String(staleCount))} silent / blocked rows older than ${escapeHtml(formatFossilThreshold(report.staleAttentionThresholdMs))}, not listed above</p>`
    : '';
 ...
 *
 * `#flight-companion-strip-tab-total` (LIN-2621, "N check-ins · $x this
 * tab" — a format sketch, not byte-pinned like the mode line; beat 4
 * pluralises the noun properly, "1 check-in" vs "2 check-ins") is rendered
 * with its TRUE initial value, "0 check-ins · $0.00 this tab" — unlike
 * next-check-in, a fresh tab's spend really is zero at render time, so this
 * is a fact, not a guess. Updated in place client-side on every `done`
 * frame the tab observes (silent or visible, any turn kind) — the whole
 * point being that a silent tick's cost otherwise has nowhere to appear.
 *
...
function renderStatusStrip(strip) {
  ...
  const lastCheckInText = s.lastCheckInAt
    ...
    : 'never';
  // LIN-2623 beat 3: the tools-off warning. Exactly one trigger — the
  // resolved workspace DEFAULT (`s.toolsOn`, already computed by
  // `buildFlightCompanionStripData` off the SAME model this whole strip
  // renders) is not tool-capable. Never re-derived from the picker's own
  // (curated-only, therefore always tool-capable) selection below — visible
  // before the user sends anything, a NEW sibling span rather than a
  // rewrite of the pre-existing `.fc-strip-tools` line above (which stays
  // byte-identical, LIN-2621's own pinned contract).
  const toolsWarningHtml = s.toolsOn
    ? ''
    : '<span class="fc-strip-tools-warning" id="flight-companion-tools-warning" role="status">⚠ tools are off for this model — no status checks or follow-up proposals</span>\n        ';
  // LIN-2623 beat 3: the per-turn model picker + its rate card. The
  // selectable set is deliberately EXACTLY `s.modelOptions` (built server-
  // side off the SAME curated allow-list `resolveTurnModelOverride`, routes/
  // flight-companion.js, accepts) — never widened, so every option here is
  // guaranteed to be honored, never 400. The leading, always-present
  // "current default" option carries an EMPTY value: leaving it selected
  // sends no `model` field at all, so an untouched picker falls through to
  // beat 1's `resolveAiOperationModel` exactly as before — it never silently
  // substitutes some other curated id for a workspace default that happens
  // not to be curated itself (the coherence hazard a non-empty default
  // value would create).
  const modelOptions = Array.isArray(s.modelOptions) ? s.modelOptions : [];
  const defaultPricingAttr = s.currentPricing ? ` data-pricing="${escapeHtml(s.currentPricing)}"` : '';
  const optionsHtml = modelOptions.map((m) => {
    const priceAttr = m.pricing ? ` data-pricing="${escapeHtml(m.pricing)}"` : '';
    return `<option value="${escapeHtml(m.id)}"${priceAttr}>${escapeHtml(m.name)}</option>`;
  }).join('\n            ');
  const priceText = escapeHtml(s.currentPricing || '—');
  // LIN-2623 beat 3, req 4: free tier must not IMPLY a pick will be honored
  // — the picker is disabled outright (nothing to silently override) and a
  // plain note explains why, rather than accepting a choice the server-side
  // `forceDefault` clamp is about to discard.
  const pickerDisabledAttr = s.isFreeTier ? ' disabled' : '';
  const freeTierNoteHtml = s.isFreeTier
    ? '\n        <span class="fc-strip-freetier" id="flight-companion-freetier-note">free tier: model selection is not honored — every request runs the clamped default</span>'
    : '';
  // LIN-2623 beat 4 finding: the leading option's label is deliberately
  // SHORT ("— current default —", no embedded model id, matching Settings'
  // own "— inherit" convention) — the id is already shown plainly in
  // .fc-strip-model just above, and a native <select> auto-sizes to its
  // widest option's rendered text; embedding the id here (previously
  // "current default (<id>)") was measured pushing this row onto its own
  // wrapped line and overflowing #flight-companion-start off a 1024x812
  // viewport (tests/e2e/flight-companion-prestart-overflow.spec.js). The
  // visible "override:" label stays for sighted users; an aria-label covers
  // the control for everyone else, since it carries no other accessible name
  // once the id was dropped from the option text.
  const modelPickerHtml = `<span class="fc-strip-model-picker">override: <select id="flight-companion-model-select" class="fc-model-select" aria-label="Per-turn model override"${pickerDisabledAttr}><option value=""${defaultPricingAttr}>— current default —</option>${optionsHtml}</select> <span class="fc-strip-price" id="flight-companion-model-price">${priceText}</span></span>${freeTierNoteHtml}`;
  let sweepText;
  ...
  return `<div class="fc-status-strip" id="flight-companion-strip">
        ...
        ${toolsWarningHtml}<span class="fc-strip-checkin">last check-in: ${lastCheckInText}</span>
...
export function renderFlightCompanionPage(data = {}, options = {}) {
  ...
  const chatBody = `${renderStatusStrip(strip)}
      ...
      <div class="fc-chat-composer chat-composer chat-composer--inline">
        <textarea id="flight-companion-question" class="fc-composer-input chat-composer__input" rows="1" placeholder="ask the companion…" maxlength="2000" autocomplete="off" aria-label="Ask the flight companion"></textarea>
  ...
  return renderPage({
    ...
    // observation.js load-order comment). purify.min.js before marked.min.js
    // before flight-companion.js (LIN-2670, session page's order,
    // lib/render-session.js): the done-frame Markdown swap needs both loaded
    // before flight-companion.js's own script runs.
    scripts: ['/common.js', '/chat.js', '/purify.min.js', '/marked.min.js', '/flight-companion.js'],
    content: `<main class="flight-companion-page" data-url-key="${encodedUrlKey}" data-fc-ai-configured="${aiConfigured ? 'true' : 'false'}">
