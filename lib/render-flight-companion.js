...
export function renderFlightCompanionPage(data = {}, options = {}) {
  ...
  // to one another, keyed off the empty-state's own visibility) — never a
  // status STRIP, which is LIN-2621's (still Backlog at the time of writing)
  // to build. `#flight-companion-reorient` sits right beside the existing
  // check-in line — the closest landed "status" surface today — rather than
  // inventing a container for a strip this ticket has no business building.
  // Both are direct siblings of `#flight-companion-thread`/the empty state/
  // the composer (never wrapped in a new element), which is load-bearing:
  ...
  const chatBody = `<ul class="chat-thread" id="flight-companion-thread" hidden></ul>
      ...
      <button type="button" id="flight-companion-reorient" class="action-btn fc-reorient-btn hidden" title="Ask the companion to re-orient now">reorient</button>
