...
function renderStatusStrip(strip) {
  ...
  const freeTierNoteHtml = s.isFreeTier
    ...
    : '';
  const modelPickerHtml = `<span class="fc-strip-model-picker">
          override:
          <select id="flight-companion-model-select" class="fc-model-select"${pickerDisabledAttr}>
            <option value=""${defaultPricingAttr}>current default (${modelText})</option>
            ${optionsHtml}
          </select>
          <span class="fc-strip-price" id="flight-companion-model-price">${priceText}</span>
        </span>${freeTierNoteHtml}`;
  let sweepText;
