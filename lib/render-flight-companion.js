...
export function renderFlightCompanionPage(data = {}, options = {}) {
  ...
  const introBody = `<div class="tree">
        ...
        <ol class="flight-companion-steps">
          <li>Enable the workspace API proxy and mint a <code>readWrite</code> token (Settings → Linear API proxy).</li>
          <li>Copy the kickoff prompt below.</li>
          <li>Paste it into a fresh Claude Code session, supplying the token in the <code>+proxy</code> block.</li>
  ...
  const promptBody = `<div class="flight-companion-actions">
        <button type="button" id="flight-companion-copy" class="action-btn save">copy prompt</button>
  ...
  return renderPage({
    ...
    <header class="flight-companion-header">
      <h1>Flight Companion</h1>
      <p class="flight-companion-subtitle">Chat with work in flight — via a real Claude Code session standing in for the model.</p>
    </header>
