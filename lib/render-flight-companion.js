...
export function renderFlightCompanionPage(data = {}, options = {}) {
  ...
  const introBody = `<div class="tree">
        <p class="flight-companion-experimental">⚗ Experimental — <strong>realtime chat with work in flight</strong> (LIN-751). Chat with the companion below: it watches the dispatch feed and checks in with you on its own, or answers directly whenever you send it something. When it proposes a follow-up on one of its own check-ins, approve or dismiss it — only your approval can queue that one. A follow-up from something you asked it directly can go out right away, the same as any other request you make of it.</p>
        <ol class="flight-companion-steps">
          <li>Say something in the chat below, or just leave the tab open — the companion checks in periodically on its own.</li>
