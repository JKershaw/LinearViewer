 ...
 * - `openRouterDurableConsentAt` — **account-owned, sibling to
 *   `openRouterApiKey`** (LIN-2412). The opt-in bit that gates *unattended*
 *   use of the durable key (`lib/openrouter-key-resolver.js`'s
 *   `getUnattendedOpenRouterKey`) — the key itself is not the opt-in; a
 *   pre-existing connected user is not eligible for unattended use until this
 *   is set. Written by `routes/openrouter-auth.js`'s connect-flow consent beat
 *   and its `POST /auth/openrouter/consent` route for already-connected
 *   accounts; cleared alongside the key on disconnect (`clearOpenRouterApiKey`
 *   below clears both in the same read-merge write). Read fresh from this
 *   store on every use — by the unattended resolver and by the settings route
 *   — and deliberately **never** mirrored into `req.session` (the interactive
 *   `OAuth > env > free` chain must stay byte-identical for non-opt-in users).
 ...
 * - `northStarDocVersionByWorkspace` — account-owned, sibling map to
 *   `northStarByWorkspace` (LIN-2254). `null` for every workspace whose pasted
 *   value doesn't match `docs/north-star.md`'s normalised whitespace form at
 *   paste time (CRLF folded to LF, trailing whitespace/blank lines stripped —
 *   LIN-2838); only a genuinely doc-sourced paste carries a stamp.
 */
...
export class UserPreferencesStore {
  ...
  /**
   * Removes a user's stored OpenRouter API key AND its durable consent flag
   * (read-merge — leaves all other preferences intact), in the same write.
   * Mirrors the session disconnect. Consent has no meaning without a key, so
   * disconnect clears both together rather than leaving a stale consent
   * timestamp a later re-connect would silently inherit.
   *
   * @param {string} accountId - The account ID
   * @returns {Promise<boolean>} True if save succeeded
   */
  async clearOpenRouterApiKey(accountId) {
    ...
    const existing = await this.getUserPreferences(accountId);
    if (!('openRouterApiKey' in existing) && !('openRouterDurableConsentAt' in existing)) return true;
    const { openRouterApiKey, openRouterDurableConsentAt, ...rest } = existing;
    return this.saveUserPreferences(accountId, rest);
  }

  /**
   * Reads the durable unattended-use consent timestamp for an account
   * (LIN-2412). `null` means the account has never granted it — read fresh
   * from here on every use, never from `req.session`.
   *
   * @param {string} accountId - The account ID
   * @returns {Promise<string|null>} ISO-8601 timestamp of consent, or null
   */
  async getOpenRouterConsent(accountId) {
    const prefs = await this.getUserPreferences(accountId);
    return prefs.openRouterDurableConsentAt || null;
  }

  /**
   * Records durable unattended-use consent for an account (read-merge —
   * preserves the rest of the preferences object). Idempotent: re-granting
  ...
  async setOpenRouterConsent(accountId) {
    if (!accountId) {
      console.warn('setOpenRouterConsent called without accountId');
      return false;
    }
    const existing = await this.getUserPreferences(accountId);
    return this.saveUserPreferences(accountId, { ...existing, openRouterDurableConsentAt: new Date().toISOString() });
  }
 ...
 *
 * Deliberately does NOT rehydrate `openRouterDurableConsentAt` (LIN-2412):
 * consent is read fresh from the store by its two consumers (the unattended
 * resolver and the settings route) on every use, never mirrored into
 * `req.session` — this is part of what keeps the interactive chain
 * byte-identical for non-opt-in users.
 *
