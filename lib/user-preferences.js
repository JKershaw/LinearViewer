...
export class UserPreferencesStore {
  ...
  /**
   * Retrieves the tile order for a user.
   * Returns an empty array if no tile order has been set.
   *
   * @param {string} linearUserId - The Linear user ID
   * @returns {Promise<string[]>} Ordered array of tile keys (empty if not set)
   */
  async getTileOrder(linearUserId) {
    const prefs = await this.getUserPreferences(linearUserId);
    return Array.isArray(prefs.tileOrder) ? prefs.tileOrder : [];
  }

  /**
   * Sets the tile order for a user.
   * Persists the ordered array of tile keys.
   *
   * @param {string} linearUserId - The Linear user ID
   * @param {string[]} order - Ordered array of tile keys
   * @returns {Promise<boolean>} True if save succeeded, false otherwise
   */
  async setTileOrder(linearUserId, order) {
    const prefs = await this.getUserPreferences(linearUserId);
    return this.saveUserPreferences(linearUserId, {
      ...prefs,
      tileOrder: order
    });
  }
