...
/**
 * Folds projected proxy-event rows into a per-token credential-health verdict.
 *
 * Pure and exported so the predicate is testable without a collection
 * (tests/unit/credential-health-predicate.test.js).
 *
 ...
 * @param {number} options.since - Epoch ms; rows at or before this are ignored.
 * @returns {Array<{tokenId, tokenLabel, ownerlessCount, okCount, verdict}>}
 */
export function foldCredentialHealth(rows, { since }) {
  const byToken = new Map();

  for (const row of rows || []) {
    if (!row || !row.tokenId) continue;

    // Re-apply the window in JS. The query is already time-bounded, but this
    // keeps the predicate honest on its own (and on stores whose find() does
    // not implement range operators).
    ...
    // Newest label wins for a token that was relabelled mid-window; a row that
    // carries no label never blanks one already seen.
    if (row.tokenLabel != null) entry.tokenLabel = row.tokenLabel;

    if (row.note === OWNERLESS_NOTE) entry.ownerlessCount++;
    if (typeof row.status === 'number' && row.status < 400) entry.okCount++;
  }

  return [...byToken.values()].map(entry => ({
    ...entry,
    verdict: entry.ownerlessCount > 0 && entry.okCount > 0 ? 'credential_dead' : 'ok'
  }));
}
...
export class ProxyEventStore {
  ...
  async listCredentialHealth(urlKey, { windowMs = CREDENTIAL_HEALTH_WINDOW_MS } = {}) {
    const effectiveWindow = Number.isFinite(windowMs) && windowMs > 0
      ? windowMs
    ...
    try {
      ...
      return {
        ...
        tokens: foldCredentialHealth(rows, { since: since.getTime() })
      };
    } catch (err) {
