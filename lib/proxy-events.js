  ...
  async listSelfCredentialHealth(urlKey, tokenId, { windowMs } = {}) {
    ...
    try {
      ...
      const docs = await this.collection.find(
        ...
        // LIN-2473: `credentialFingerprint` joins the projection because
        // `providerLaneOccupancy` now reads it — a provider-lane 503 carrying
        // one is a real credential rejection (LIN-2216's transient
        // reclassification), and dropping the field here made that rule dead
        // on arrival: the endpoint answered `ok` through the exact fault it
        // was extended to expose. The four fields below were the complete set
        // only while 401 was the sole fault signal.
        { projection: { status: 1, stage: 1, note: 1, timestamp: 1, credentialFingerprint: 1 } }
      ).toArray();
