...
export async function runFlightCompanionTurn({
  ...
  // gate does, above) — read it fresh here, purely for orientation. LIN-2622:
  // a boot turn already read it above (its reservation needs it too), so this
  // is a no-op there whenever that read found something; it only re-fires for
  // a boot on a workspace with no sweep yet, which re-reads the same still-null
  // result — harmless, and kept for symmetry with the general "any turn that
  // hasn't read the census yet, reads it now for orientation" shape.
  // Optional-guarded: an absent store degrades to the honest "not available"
  // seed text rather than throwing.
  if (currentCensusDoc === null && (turnKind === 'user-initiated' || turnKind === 'boot') && observerStateStore) {
