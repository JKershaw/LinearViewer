...
function renderNode(node, parentId, options = {}) {
  ...
  // Escape each field individually before concatenation to prevent XSS via crafted field content
  const searchParts = [
    escapeHtml(issue.title),
    escapeHtml(issue.description || ''),
    escapeHtml(issue.assignee?.name || ''),
    escapeHtml(issue.identifier || ''),
    ...(issue.labels?.nodes || []).map(l => escapeHtml(l.name))
  ]
  ...
  const line = `<div class="${lineClasses}" data-id="${issue.id}" data-parent="${parentId}" data-depth="${depth}" data-section="${section}" data-search-text="${escapeHtml(searchText)}" style="--depth: ${depth}"><span class="state ${stateClass}" data-status="${stateClass}" aria-label="Status: ${stateLabel}">${stateChar}</span><span class="${titleClass}">${escapeHtml(issue.title)}</span>${timeBadge}${projectBadge}${toggle}</div>`
