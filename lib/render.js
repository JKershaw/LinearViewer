...
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '' } = {}) {
  ...
  return `<div class="prompt-container${hiddenClass}"${issueAttr}${urlKeyAttr}><div class="prompt-header"><span class="prompt-name">${escapeHtml(name)}</span><button class="prompt-copy">copy</button></div><div class="prompt-text">${content}</div></div>`
}
...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null) {
  ...
  if (issue.description) {
    ...
    if (promptBlockMatch) {
      ...
      if (promptContent.trim()) {
        lines.push(renderPromptContainer({ name: 'Setup Prompt', content: escapeHtml(promptContent.trim()) }))
      }
  ...
  if (!isLanding) {
    lines.push(renderPromptContainer({ issueId: issue.id, urlKey }))
