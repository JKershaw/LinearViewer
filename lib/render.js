...
const DEFAULT_PROMPTS = [
  { key: 'look-into', label: 'look into' },
  { key: 'research', label: 'research' },
  { key: 'plan', label: 'plan' },
  { key: 'implementation', label: 'implement' },
]
const DEFAULT_PROMPT_KEYS = new Set(DEFAULT_PROMPTS.map(p => p.key))
...
export function renderPromptButtons(issue, isLanding = false, openRouterSource = null) {
  ...
  for (const { key, label } of DEFAULT_PROMPTS) {
    parts.push(`<a href="#" class="label-prompt" data-issue-id="${issueId}" data-label="${key}">${label}</a>`)
  }

  // Everything else behind "more"
  const hiddenKeys = getPromptLabels().filter(k => !DEFAULT_PROMPT_KEYS.has(k))
  if (hiddenKeys.length > 0) {
    const hiddenLinks = hiddenKeys.map(key => {
      const label = key.replace(/-/g, ' ')
      return `<a href="#" class="label-prompt" data-issue-id="${issueId}" data-label="${escapeHtml(key)}">${escapeHtml(label)}</a>`
    }).join(' ')
