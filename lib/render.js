...
export { renderLoginPage, renderErrorPage, renderUpstreamAwareErrorPage, renderWorkspaceNotFoundPage, renderOpenRouterConsentPage } from './render-pages.js'
...
// LIN-1553 (Session B) shipped the in-app issue CREATE form inline, per-project.
// LIN-1973 replaced it with a dedicated drill-down page (lib/render-task-create.js,
// `GET /workspace/:urlKey/task/new`) — the "+ Add task" affordance below now
// LINKS there instead of rendering a form inline. Render still gates on the
// `ui.inlineCreate` flag exclusively — never on `supports()` — per the LIN-177
// capability-surface convention; only the affordance's shape changed.
//
// The EDIT half moved out first, in LIN-1565: an inline form hidden inside a
// collapsed Details panel was the UX defect, so editing is its own drill-down
// page (lib/render-task-edit.js) that the Details panel links to. The
// `ui.inlineEdit` gate did not change — it just moved to the link and the
// page's route. CREATE now follows the same shape.
// Exported for the dedicated task-edit and task-create pages (LIN-1565/LIN-1973),
// and the feedback route's priority text-fold (LIN-1557) — all three render or
// quote the same 0–4 priority vocabulary. One list, several surfaces — a
// forked copy would drift the moment Linear's priority scale changed.
export const PRIORITY_OPTION_LABELS = ['No priority', 'Urgent', 'High', 'Medium', 'Low']

export function priorityOptionsHtml(selected) {
  const sel = Number(selected)
  return PRIORITY_OPTION_LABELS
 ...
 * @param {string} [options.source] - Resolved provider `.name` (LIN-1904) stamped as
 *   `data-source`, so the client fetch can forward it as `?source=`. Optional — the
 *   landing/periodical call sites pass no `source` and get no attribute (no-op).
 * @returns {string} HTML string
 */
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false, isLocalhost = false, kind = '', periodicalId = '', optionsId = '', proxyForce = false, source = '' } = {}) {
  ...
  const sourceAttr = source ? ` data-source="${escapeHtml(source)}"` : ''
  ...
  // data-periodical-id (LIN-1825): the periodicals-registry template id this
  // container was minted from — the shared dispatch handler forwards it so the
  // dispatched item can be joined back to its template, replacing the fragile
  // promptName join (the Mint + Autopilot variant sends "<title> + Autopilot").
  const periodicalIdAttr = periodicalId ? ` data-periodical-id="${escapeHtml(periodicalId)}"` : ''
  ...
  return `<div class="prompt-container${hiddenClass}"${issueAttr}${urlKeyAttr}${kindAttr}${periodicalIdAttr}${proxyForceAttr}${sourceAttr}><div class="prompt-header"><span class="prompt-name">${escapeHtml(name)}</span><div class="prompt-actions">${actionButtons}</div></div><div class="prompt-text">${content}</div></div>`
}
 ...
 * @property {string[]} [availableAssignees] - Array of assignee display names for the assignee selector (dashboard-only, LIN-2529)
 * @property {string|null} [selectedAssignee] - Currently selected assignee: 'all', 'me', or a literal name
...
export function renderPage(projectTrees, inProgressTrees = [], recentActivityTrees = [], organizationName = 'Projects', options = {}) {
  const { isLanding = false, teams = [], selectedTeamId = null, availableAssignees = [], selectedAssignee = null, canFilterByMe = false, workspaces = [], openRouterSource = null, urlKey = null, deployInfo = {}, featureFlags = {}, customPrompts = [], setupNotice = null, isLocalhost = false, showSource = false, heroHtml = null, truncated = false } = options
  ...
  // Navigation bar with workspace/team/assignee selectors (authenticated users
  // only). Uses urlKey from URL to determine active workspace (enables
  // multi-tab support). availableAssignees/selectedAssignee/canFilterByMe
  // (LIN-2529) are dashboard-only in practice — renderNavBar itself gates the
  // assignee selector on currentPage === 'projects', which this page always is.
  const navBarHtml = !isLanding ? renderNavBar({ workspaces, teams, selectedTeamId, assignees: availableAssignees, selectedAssignee, canFilterByMe, urlKey, currentPage: 'projects', featureFlags }) : ''
  ...
  return renderPageShell({
    ...
    nav: navBarHtml,
    scripts: ['/common.js', '/purify.min.js', '/marked.min.js', '/brief.js', '/recap.js', '/scan.js', '/context.js', '/sessions.js', '/app.js'],
  ...
  ${truncated ? `<div class="truncation-notice">
    <p>\u26a0 Some issues could not be loaded \u2014 the read hit its results cap. The board below is incomplete.</p>
  </div>` : ''}
  <main>
    ${inProgressHtml}
    ${recentActivityHtml}
...
function renderProject({ project, incomplete, completed, completedCount }, options = {}) {
  ...
  const isRealProject = !isLanding && urlKey && project.id && !project.id.startsWith('__')
  // LIN-1553: providers with session-auth createIssue support (ui.inlineCreate)
  // link to the dedicated create page (LIN-1973); the external deep-link is
  // REPLACED for them. Providers with only an external create affordance
  // (ui.write, no inlineCreate) keep the original linear.app-style
  // target="_blank" deep-link unchanged.
  let addTaskLink = ''
  if (isRealProject && ui.inlineCreate) {
    const createHref = `/workspace/${encodeURIComponent(urlKey)}/task/new?projectId=${encodeURIComponent(project.id)}`
    addTaskLink = `<div class="add-task-link" data-testid="create-task"><a href="${createHref}" class="detail-link" data-testid="create-task-trigger">+ Add task</a></div>`
  } else if (isRealProject && ui.write) {
...
function renderNode(node, parentId, options = {}) {
  ...
  if (hasDetails) {
    if (isLanding) {
      ...
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      // LIN-1903: stamp the issue's own provenance on the lazy wrapper too
      // (unconditional, unlike sourceBadge's showSource gate) so the client can
      // forward it as `/api/detail`'s `source` param — the join key the server
      // needs to resolve THIS issue's own binding in a merged multi-binding
      // workspace, instead of always resolving the workspace's active provider.
      details = `<div class="details hidden" data-details-for="${issue.id}" data-section="${section}" data-source="${escapeHtml(source)}" data-lazy="1"${urlKeyAttr} style="--depth: ${depth}"></div>`
    }
  }
...
export function renderDetailsContent(issue, { isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false, provider = null, section = '' } = {}) {
  ...
  // ==========================================================================
  // Edit affordance (LIN-1553; moved to its own page in LIN-1565) — a one-click
  // link to the dedicated task-edit page (routes/task-edit.js). Gated on
  // ui.inlineEdit (session-auth updateIssue support) ONLY, byte-identically to
  // the inline form it replaces — the route re-reads the SAME flag, so the two
  // can never disagree. An <a> in the LIN-1007 Chat-deep-link idiom below
  // (navigation, not an in-place section), so app.js leaves it as a plain link.
  //
  // The href carries the issue's canonical `id`, the value already in hand here;
  // the page accepts an identifier too, but linking the id costs nothing extra.
  // ==========================================================================
  if (!isLanding && urlKey && ui.inlineEdit) {
    // LIN-1904: carry the resolved provider's name forward as `?source=` so
    // the task-edit page (routes/task-edit.js) — and its PATCH submit —
    // resolve THIS issue's own binding rather than the workspace's active
    // provider. `provider` here is always the binding this detail fragment
    // was itself resolved from (/api/detail, LIN-1903's pattern).
    const sourceQuery = provider?.name ? `?source=${encodeURIComponent(provider.name)}` : ''
    const editHref = `/workspace/${encodeURIComponent(urlKey)}/task/${encodeURIComponent(issue.id)}/edit${sourceQuery}`
    detailsContent.push(`<a class="detail-toggle nested-toggle" href="${escapeHtml(editHref)}" data-testid="issue-edit-link">Edit ↗</a>`)
  }
  ...
  if (!isLanding && urlKey && ui.comments) {
    ...
    // LIN-1904: stamp the resolved provider so the client comments fetch
    // (public/app.js) can forward it as `?source=`.
    const sourceAttr = provider?.name ? ` data-source="${escapeHtml(provider.name)}"` : ''
    detailsContent.push(`<div class="detail-toggle nested-toggle" data-toggle="comments" data-issue-id="${escapeHtml(issue.id)}"${urlKeyAttr}${sourceAttr}>Comments ▶</div>`)
  ...
  if (!isLanding && urlKey) {
    ...
    // LIN-1910: stamp the resolved provider so the client Brief/Recap fetches
    // (public/app.js) can forward it as `?source=`, resolving THIS issue's own
    // binding instead of the workspace's active provider.
    const sectionSourceAttr = provider?.name ? ` data-source="${escapeHtml(provider.name)}"` : ''
    const sectionAttrs = ` data-issue-identifier="${escapeHtml(identifier)}" data-url-key="${escapeHtml(urlKey)}"${sectionSourceAttr}`
    ...
    // Scan (LIN-2197 Phase 5) — a human-triggered "scan for blockers" read,
    // the third producer into the operator decision queue (LIN-1721). Same
    // lazy-toggle/init-on-first-expand shape as Brief/Recap above, but the
    // scan itself is never auto-triggered on expand (public/app.js) — only
    // the cheap status check is.
    detailsContent.push(`<div class="detail-toggle nested-toggle" data-toggle="scan"${sectionAttrs}>Scan ▶</div>`)
    detailsContent.push(`<div class="detail-content hidden" data-content="scan"><div class="scan-section" data-scan-placeholder="1"></div></div>`)
    ...
    if (featureFlags.taskChat === true) {
      // LIN-1910: carry the resolved provider as a `source` hint so the task-chat
      // page can pre-scope the chat turn to THIS issue's own binding.
      const chatSourceQuery = provider?.name ? `&source=${encodeURIComponent(provider.name)}` : ''
      const chatHref = `/workspace/${encodeURIComponent(urlKey)}/task-chat?task=${encodeURIComponent(identifier)}${chatSourceQuery}`
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    // LIN-1904: `source` threads the resolved provider forward so the client
    // prompt fetch (public/app.js) can forward it as `?source=`.
    promptsContent.push(renderPromptContainer({ issueId: issue.id, urlKey, dispatchEnabled: featureFlags.dispatch === true, proxyEnabled: featureFlags.proxy === true, isLocalhost, optionsId: `prompt-options-${instanceKey}`, source: provider?.name || '' }))
    ...
    if (featureFlags.aiRecommendations !== false) {
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      // LIN-1910: stamp the resolved provider so the recommend stream fetch
      // (public/app.js) can forward it as `?source=`.
      const recommendSourceAttr = provider?.name ? ` data-source="${escapeHtml(provider.name)}"` : ''
    ...
    if (featureFlags.proxy === true) {
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      // LIN-1904: stamp the resolved provider so fetchAutopilotKickoff
      // (public/common.js) can forward it as `?source=`.
      const sourceAttr = provider?.name ? ` data-source="${escapeHtml(provider.name)}"` : ''
      ...
      promptsContent.push(`<div class="autopilot-container prompt-container hidden" data-autopilot-for="${issue.id}" data-kind="autopilot"${urlKeyAttr}${sourceAttr}><div class="prompt-header"><span class="prompt-name">Autopilot</span><div class="prompt-actions">${autopilotActions}</div></div><div class="prompt-text"></div></div>`)
    }
  }
