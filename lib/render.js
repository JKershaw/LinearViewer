...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null) {
  ...
  // ==========================================================================
  if (!isLanding && urlKey) {
    const commentsUrlKeyAttr = ` data-url-key="${escapeHtml(urlKey)}"`
    detailsContent.push(`<div class="detail-toggle nested-toggle" data-toggle="comments" data-issue-id="${escapeHtml(issue.id)}"${commentsUrlKeyAttr}>Comments ▶</div>`)
