...
import { getPromptLabels, isEligibleForPlan, getAllPromptsByCategory, CATEGORY_DISPLAY_NAMES, PROMPT_CATEGORIES } from './prompt-templates.js'
...
export function renderPage(projectTrees, inProgressTrees = [], organizationName = 'Projects', options = {}) {
  ...
  const inProgressHtml = renderInProgressSection(inProgressTrees, isLanding)
  const projectsHtml = projectTrees.map(tree => renderProject(tree, isLanding)).join('\n')
...
function renderNavBar({ workspaces = [], activeWorkspaceId = null, teams = [], selectedTeamId = null }) {
  ...
  <nav class="nav-bar" aria-label="Main navigation">
    ...
    <div class="nav-actions">
      <a href="/fancy" class="nav-action">audit</a>
 ...
 */
function renderInProgressSection(inProgressTrees, isLanding = false) {
 ...
 */
function renderProject({ project, incomplete, completed, completedCount }, isLanding = false) {
  ...
  const incompleteHtml = incomplete
    .map(node => renderNode(node, project.id, { isLanding }))
    .join('\n')
  ...
  const completedHtml = completed.length > 0
    ? `<div class="tree hidden" data-completed-for="${project.id}">${completed.map(node => renderNode(node, project.id, { isLanding })).join('\n')}</div>`
    : ''
...
function renderNode(node, parentId, options = {}) {
  const { section = 'project', projectName = null, isLanding = false } = options
  ...
  const details = hasDetails ? renderDetails(issue, depth, section, isLanding) : ''
...
/**
 * Render the "more prompts" dropdown for an issue
 * Shows all available prompt templates organized by category
 * @param {string} issueId - The issue ID for data attributes
 * @returns {string} HTML for the dropdown trigger and panel
 */
function renderMorePromptsDropdown(issueId) {
  const promptsByCategory = getAllPromptsByCategory()
  const categoryOrder = [PROMPT_CATEGORIES.PRE_WORK, PROMPT_CATEGORIES.WORK_ISSUE, PROMPT_CATEGORIES.READY]
  ...
  categoryOrder.forEach((category, catIdx) => {
    ...
    const isLastCategory = catIdx === categoryOrder.length - 1
    const catPrefix = isLastCategory ? '└─' : '├─'

    // Category header
    optionsHtml += `<div class="more-prompts-category">${catPrefix} ${categoryName}</div>`

    // Prompts in this category
    prompts.forEach((prompt, idx) => {
      const isLastPrompt = idx === prompts.length - 1
      const linePrefix = isLastCategory ? '   ' : '│  '
      const itemPrefix = isLastPrompt ? '└─' : '├─'

      optionsHtml += `<div class="more-prompts-row"><span class="option-prefix">${linePrefix}${itemPrefix}</span><button class="more-prompts-option" data-issue-id="${issueId}" data-prompt-key="${escapeHtml(prompt.key)}">${escapeHtml(prompt.name)}</button></div>`
    })
  })

  return `<div class="more-prompts-wrapper"><button class="more-prompts-toggle" data-issue-id="${issueId}" aria-expanded="false">more prompts</button><div class="more-prompts-panel hidden" data-more-prompts-for="${issueId}">${optionsHtml}</div></div>`
}
 ...
 */
export function renderLabels(issue, isLanding = false) {
  ...
  if (!isLanding && isEligibleForPlan(issue)) {
    ...
    if (!hasExistingPlanLabel) {
      parts.push(`<a href="#" class="label-prompt state-prompt" data-issue-id="${issueId}" data-label="plan">plan</a>`)
    }
    const hasExistingCodeReviewLabel = labels.some(l => l.name === 'code-review')
    if (!hasExistingCodeReviewLabel) {
      parts.push(`<a href="#" class="label-prompt state-prompt" data-issue-id="${issueId}" data-label="code-review">code-review</a>`)
    }
  }
 ...
 */
function renderDetails(issue, depth, section = 'project', isLanding = false) {
  const lines = []
  ...
  const labelHtml = renderLabels(issue, isLanding)
  if (labelHtml) {
    meta.push(labelHtml)
  }
  ...
  // Add prompt container for issues with promptable labels OR in Ready queue
  // Skip on landing page - prompt UI requires authentication
  if (!isLanding) {
    const promptableLabels = (issue.labels?.nodes || []).filter(l => PROMPT_LABELS.has(l.name))
    const needsPromptContainer = promptableLabels.length > 0 || isEligibleForPlan(issue)
    if (needsPromptContainer) {
      // Add "more prompts" dropdown to access all prompt templates
      lines.push(`<div class="detail-line">${renderMorePromptsDropdown(issue.id)}</div>`)
      lines.push(`<div class="prompt-container hidden" data-prompt-for="${issue.id}"><div class="prompt-header"><span class="prompt-name"></span><button class="prompt-copy">copy</button></div><pre class="prompt-text"></pre></div>`)
    }
  }
