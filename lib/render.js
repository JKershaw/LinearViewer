...
export function renderPage(projectTrees, inProgressTrees = [], organizationName = 'Projects', options = {}) {
  const { isLanding = false, teams = [], selectedTeamId = null, workspaces = [], activeWorkspaceId = null, openRouterSource = null, urlKey = null, deployInfo = {} } = options
  ...
  const navBarHtml = !isLanding ? renderNavBar({ workspaces, activeWorkspaceId, teams, selectedTeamId, openRouterSource, urlKey }) : ''
 ...
 * @param {string|null} options.urlKey - Current workspace URL key for generating links
 * @returns {string} HTML for navigation bar
 */
function renderNavBar({ workspaces = [], activeWorkspaceId = null, teams = [], selectedTeamId = null, openRouterSource = null, urlKey = null }) {
...
function renderOpenRouterNavItem(source, urlKey = null) {
  ...
  const settingsUrl = urlKey ? `/workspace/${urlKey}/settings` : '/settings'
...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null) {
  ...
  if (!isLanding) {
    const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
    lines.push(`<div class="prompt-container hidden" data-prompt-for="${issue.id}"${urlKeyAttr}><div class="prompt-header"><span class="prompt-name"></span><button class="prompt-copy">copy</button></div><div class="prompt-text"></div></div>`)
...
export function renderWorkspaceNotFoundPage(urlKey, workspaces = []) {
  const workspaceListHtml = workspaces.length > 0
    ? `<div class="workspace-list">
        <p>Your workspaces:</p>
        <ul>
          ${workspaces.map(ws => `<li><a href="/workspace/${escapeHtml(ws.urlKey)}/">${escapeHtml(ws.name)} (${escapeHtml(ws.urlKey)})</a></li>`).join('')}
        </ul>
      </div>`
    : '';
