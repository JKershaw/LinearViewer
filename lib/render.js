...
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false, isLocalhost = false } = {}) {
  ...
  const localButton = dispatchEnabled && isLocalhost ? '<button class="prompt-dispatch dispatch-btn" data-target="local">local</button>' : ''
...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false) {
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    if (featureFlags.aiRecommendations !== false) {
      ...
      const recommendLocalButton = featureFlags.dispatch === true && isLocalhost ? '<button class="prompt-dispatch dispatch-btn" data-target="local">local</button>' : ''
