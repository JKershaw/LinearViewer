...
function renderProject({ project, incomplete, completed, completedCount }, options = {}) {
  ...
  // Create task link (authenticated users only)
  const createTaskLink = !isLanding && urlKey && project.id
    ? `<a href="https://linear.app/${encodeURIComponent(urlKey)}/new?project=${encodeURIComponent(project.id)}" target="_blank" class="detail-link" data-action="create-task">+ Create task</a>`
    : ''

  const projectLink = project.url || createTaskLink
    ? `<div class="project-meta${hasDescription ? ' hidden' : ''}">${project.url ? `<a href="${project.url}"${projectTarget} class="detail-link">${projectLinkText}</a>` : ''}${project.url && createTaskLink ? ' ' : ''}${createTaskLink}</div>`
    : ''
