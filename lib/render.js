 ...
 */
export function renderPage(projectTrees, inProgressIssues = [], organizationName = 'Roadmap', options = {}) {
  ...
  const authLink = isLanding
    ? '<a href="/auth/linear" class="login">login</a>'
...
function renderInProgressSection(issues) {
  ...
  const itemsHtml = issues.map(issue => {
    ...
    return `<div class="in-progress-item${hasDetails ? ' expandable' : ''}" data-id="${issue.id}">
...
function renderInProgressDetails(issue) {
  ...
  if (issue.url) {
    lines.push(`<div class="detail-line"><a href="${issue.url}" target="_blank" class="detail-link">View in Linear →</a></div>`)
  }

  if (lines.length === 0) return ''

  return `<div class="details hidden" data-details-for="${issue.id}">${lines.join('')}</div>`
}
...
function renderProject({ project, incomplete, completed, completedCount }) {
  ...
  const projectLink = project.url
    ? `<div class="project-meta${hasDescription ? ' hidden' : ''}"><a href="${project.url}" target="_blank" class="detail-link">View in Linear →</a></div>`
    : ''
...
function renderDetails(issue, prefix, depth) {
  ...
  if (issue.url) {
    lines.push(`<div class="detail-line"><span class="prefix">${prefix}  </span><a href="${issue.url}" target="_blank" class="detail-link">View in Linear →</a></div>`)
  }
