...
function renderWorkspaceOptions(workspaces, urlKey) {
  ...
  const optionsHtml = workspaces.map(ws => {
    ...
    return `<div class="nav-options-row">
      ...
      <a href="/workspace/${encodeURIComponent(ws.urlKey)}/" class="nav-option${selectedClass}" role="option" aria-selected="${ariaSelected}">
...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null) {
  ...
  // ==========================================================================
  // Comments section (authenticated users only)
  // LIN-156: Collapsible comments fetched on-demand
  // ==========================================================================
  if (!isLanding && urlKey) {
    const urlKeyAttr = ` data-url-key="${escapeHtml(urlKey)}"`
    lines.push(`<div class="detail-toggle" data-toggle="comments" data-issue-id="${escapeHtml(issue.id)}"${urlKeyAttr}>Comments ▶</div>`)
    lines.push(`<div class="detail-content hidden" data-content="comments">
