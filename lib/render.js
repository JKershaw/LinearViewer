 ...
 */
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false } = {}) {
  ...
  const actionButtons = dispatchEnabled
    ? '<span class="prompt-actions-label">Dispatch:</span><button class="prompt-dispatch dispatch-btn" data-target="cli">cli</button><button class="prompt-dispatch dispatch-btn" data-target="web">web</button><button class="prompt-dispatch dispatch-btn" data-target="dash">dash</button><button class="prompt-copy">copy</button>' + proxyToggle
...
export function renderPage(projectTrees, inProgressTrees = [], recentActivityTrees = [], organizationName = 'Projects', options = {}) {
  const { isLanding = false, teams = [], selectedTeamId = null, workspaces = [], openRouterSource = null, urlKey = null, deployInfo = {}, featureFlags = {}, customPrompts = [] } = options
  const inProgressHtml = renderInProgressSection(inProgressTrees, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts })
  const recentActivityHtml = renderRecentActivitySection(recentActivityTrees, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts })
  const projectsHtml = projectTrees.map(tree => renderProject(tree, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts })).join('\n')
...
function renderInProgressSection(inProgressTrees, options = {}) {
  const { isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [] } = options
...
function renderNode(node, parentId, options = {}) {
  const { section = 'project', projectName = null, isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [] } = options
  ...
  const details = hasDetails ? renderDetails(issue, depth, section, isLanding, openRouterSource, urlKey, featureFlags, customPrompts) : ''
 ...
 */
export function renderPromptButtons(issue, isLanding = false, openRouterSource = null, customPrompts = []) {
  if (isLanding) return ''
  ...
  // Everything else behind "more" (built-in + custom)
  const hiddenKeys = getPromptLabels().filter(k => !DEFAULT_PROMPT_KEY_SET.has(k))
  const hasHidden = hiddenKeys.length > 0 || customPrompts.length > 0
  if (hasHidden) {
    let hiddenLinks = hiddenKeys.map(key =>
    ...
    if (customPrompts.length > 0) {
      const customLinks = customPrompts.map(cp =>
        `<a href="#" class="label-prompt custom-prompt-btn" data-issue-id="${issueId}" data-label="custom:${escapeHtml(cp.id)}">${escapeHtml(cp.name)}</a>`
      ).join(' ')
      hiddenLinks = hiddenLinks ? `${hiddenLinks} ${customLinks}` : customLinks
    }
 ...
 */
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = []) {
  const lines = []
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    const promptButtonsHtml = renderPromptButtons(issue, isLanding, effectiveOpenRouterSource, customPrompts)
    ...
    promptsContent.push(renderPromptContainer({ issueId: issue.id, urlKey, dispatchEnabled: featureFlags.dispatch === true, proxyEnabled: featureFlags.proxy === true }))
    ...
    if (featureFlags.aiRecommendations !== false) {
      ...
      const recommendActions = featureFlags.dispatch === true
        ? '<span class="prompt-actions-label">Dispatch:</span><button class="prompt-dispatch dispatch-btn" data-target="cli">cli</button><button class="prompt-dispatch dispatch-btn" data-target="web">web</button><button class="prompt-dispatch dispatch-btn" data-target="dash">dash</button><button class="prompt-copy">copy</button>' + recommendProxyToggle
