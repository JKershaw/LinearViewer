...
// Prompts shown by default (before "more" toggle)
const DEFAULT_VISIBLE_PROMPTS = new Set(['look-into', 'research', 'implementation'])
...
export function renderPage(projectTrees, inProgressTrees = [], recentActivityTrees = [], organizationName = 'Projects', options = {}) {
  ...
  const navBarHtml = !isLanding ? renderNavBar({ workspaces, teams, selectedTeamId, urlKey, currentPage: 'projects' }) : ''
...
export function renderPromptButtons(issue, isLanding = false, openRouterSource = null) {
  ...
  // Skip prompts for completed/canceled issues
  const stateType = issue.state?.type?.toLowerCase() || ''
  const isActionable = !['completed', 'canceled'].includes(stateType)
  if (!isActionable) return parts.join(' ')

  // Collect all applicable prompts with their HTML
  const allApplicable = []

  // Label-based prompts (blocked, bug) - only if label exists on the issue
  for (const label of labels) {
    if (PROMPT_LABELS.has(label.name) && !seenKeys.has(label.name)) {
      const name = escapeHtml(label.name)
      allApplicable.push({ key: label.name, html: `<a href="#" class="label-prompt" data-issue-id="${issueId}" data-label="${escapeHtml(label.name)}">${name}</a>` })
      seenKeys.add(label.name)
    }
  }

  // State-based prompts (plan, code-review) for Ready queue issues
  if (isEligibleForPlan(issue)) {
    if (!labels.some(l => l.name === 'plan') && !seenKeys.has('plan')) {
      allApplicable.push({ key: 'plan', html: `<a href="#" class="label-prompt state-prompt" data-issue-id="${issueId}" data-label="plan">${getPromptDisplayName('plan')}</a>` })
      seenKeys.add('plan')
    }
    if (!labels.some(l => l.name === 'code-review') && !seenKeys.has('code-review')) {
      allApplicable.push({ key: 'code-review', html: `<a href="#" class="label-prompt state-prompt" data-issue-id="${issueId}" data-label="code-review">${getPromptDisplayName('code-review')}</a>` })
      seenKeys.add('code-review')
    }
  }

  // Universal prompts (look-into, triage, breakdown, research, etc.)
  for (const label of getUniversalLabels()) {
    if (!seenKeys.has(label)) {
      allApplicable.push({ key: label, html: `<a href="#" class="label-prompt state-prompt" data-issue-id="${issueId}" data-label="${escapeHtml(label)}">${escapeHtml(getPromptDisplayName(label))}</a>` })
      seenKeys.add(label)
    }
  }

  // Split into visible (default) vs hidden (behind "more")
  const visible = allApplicable.filter(p => DEFAULT_VISIBLE_PROMPTS.has(p.key))
  const hidden = allApplicable.filter(p => !DEFAULT_VISIBLE_PROMPTS.has(p.key))

  // Also include any remaining prompt templates not yet collected
  const remainingKeys = getPromptLabels().filter(k => !seenKeys.has(k))
  for (const key of remainingKeys) {
    hidden.push({ key, html: `<a href="#" class="label-prompt" data-issue-id="${issueId}" data-label="${escapeHtml(key)}">${escapeHtml(getPromptDisplayName(key))}</a>` })
  }
  ...
  if (hidden.length > 0) {
    const hiddenLinks = hidden.map(p => p.html).join(' ')
    parts.push(
