...
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false, isLocalhost = false, kind = '', optionsId = '' } = {}) {
  ...
  const actionButtons = dispatchEnabled
    ? renderDispatchDisclosure(disclosureId, isLocalhost) + '<button class="prompt-copy">copy</button>' + proxyToggle
...
function renderProject({ project, incomplete, completed, completedCount }, options = {}) {
  ...
  // '__no_project__') are not Linear projects, so building a create-task URL for
  // them yields a broken Linear link. Requiring !startsWith('__') suppresses the
  // link for all synthetic groups (LIN-341; also fixes the latent __no_project__ bug).
  const addTaskLink = !isLanding && urlKey && project.id && !project.id.startsWith('__')
    ? `<div class="add-task-link"><a href="${getProvider('linear').getCreateTaskUrl(urlKey, project.id)}" target="_blank" class="detail-link" data-action="create-task">+ Add task</a></div>`
    : ''
 ...
 * issues: no "View in Linear", no AI-suggest/foreman prompt buttons, no add-task
...
function renderPeriodicalNode(node, options = {}) {
  ...
  // issue fields, which the endpoint accepts (issueId is null-safe end-to-end).
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    if (featureFlags.aiRecommendations !== false) {
      ...
      const recommendActions = featureFlags.dispatch === true
        ? renderDispatchDisclosure(`recommend-options-${issue.id}`, isLocalhost) + '<button class="prompt-copy">copy</button>' + recommendProxyToggle
    ...
    if (featureFlags.proxy === true) {
      ...
      // Foreman and mini-foreman are two distinct containers, so each needs its
      // own panel id — a shared id would collide and break disclosure resolution.
      const foremanActions = featureFlags.dispatch === true
        ? renderDispatchDisclosure(`foreman-options-${issue.id}`, isLocalhost) + '<button class="prompt-copy">copy</button>' + foremanProxyToggle
        : '<button class="prompt-copy">copy</button>' + foremanProxyToggle
      const miniForemanActions = featureFlags.dispatch === true
        ? renderDispatchDisclosure(`mini-foreman-options-${issue.id}`, isLocalhost) + '<button class="prompt-copy">copy</button>' + foremanProxyToggle
        : '<button class="prompt-copy">copy</button>' + foremanProxyToggle
      const autopilotActions = featureFlags.dispatch === true
        ? renderDispatchDisclosure(`autopilot-options-${issue.id}`, isLocalhost) + '<button class="prompt-copy">copy</button>' + foremanProxyToggle
        : '<button class="prompt-copy">copy</button>' + foremanProxyToggle
      ...
      // Autopilot container: parallel to Foreman, but data-kind is baked in so
      // the shared dispatch handler tags the item as the 'autopilot' meta-loop.
