...
function renderWorkspaceOptions(workspaces, activeWorkspaceId) {
  ...
  const optionsHtml = workspaces.map(ws => {
    ...
    const removeBtn = isActive
      ? `<form action="/workspace/${ws.id}/remove" method="POST" class="nav-option-form" onsubmit="return confirm('Remove this workspace?')">
...
function renderInProgressSection(issues) {
  ...
  const itemsHtml = issues.map(issue => {
    ...
    return `<div class="in-progress-item${hasDetails ? ' expandable' : ''}" data-id="${issue.id}" data-section="in-progress">
      <span class="state in-progress" data-status="in-progress" aria-label="Status: In Progress">◐</span>
...
function renderNode(node, isLast, ancestors, parentId) {
  ...
  const line = `<div class="${lineClasses}" data-id="${issue.id}" data-parent="${parentId}" data-depth="${depth}" data-section="project" style="--depth: ${depth}"><span class="prefix">${prefix}</span><span class="state ${stateClass}" data-status="${stateClass}" aria-label="Status: ${stateLabel}">${stateChar}</span><span class="${titleClass}">${escapeHtml(issue.title)}</span>${toggle}</div>`
