...
import { getPromptLabels, isInReadyQueue } from './prompt-templates.js'
...
function renderLabels(issue) {
  ...
  if (!hasExistingPlanLabel && isInReadyQueue(issue)) {
...
function renderDetails(issue, depth, section = 'project') {
  ...
  const needsPromptContainer = promptableLabels.length > 0 || isInReadyQueue(issue)
