 ...
 */
// LIN-295: dispatch options live behind a single disclosure trigger. The same
// issue can render in multiple sections (In Progress + project tree), so panel
// IDs are drawn from a monotonic counter (reset per renderPage) to stay unique
// within the document — a plain issue-id-based ID would collide.
let dispatchPanelSeq = 0
function nextDispatchPanelId() {
  dispatchPanelSeq += 1
  return `dispatch-options-${dispatchPanelSeq}`
}

/**
 * Build the prompt action cluster. When dispatch is enabled the target buttons
 * (cli/web/dash/harbour) are collapsed behind a single `Dispatch ▾` disclosure
 * trigger that expands a `.hidden` `.dispatch-options` panel; copy and the
 * +proxy toggle stay visible. Mirrors the dispatch page pattern
 * (lib/render-dispatch.js): state owned by the trigger via aria-expanded.
 */
function renderDispatchActions({ dispatchEnabled = false, proxyEnabled = false, isLocalhost = false } = {}) {
  ...
  if (!dispatchEnabled) {
    return '<button class="prompt-copy">copy</button>' + proxyToggle
  }
  const panelId = nextDispatchPanelId()
  const localButton = isLocalhost ? '<button class="prompt-dispatch dispatch-btn" data-target="local">harbour</button>' : ''
  return `<button type="button" class="prompt-dispatch-toggle dispatch-toggle" aria-expanded="false" aria-haspopup="true" aria-controls="${panelId}">Dispatch &#9662;</button>`
    + `<div class="dispatch-options hidden" id="${panelId}">`
...
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false, isLocalhost = false } = {}) {
  ...
  const actionButtons = renderDispatchActions({ dispatchEnabled, proxyEnabled, isLocalhost })
...
export function renderPage(projectTrees, inProgressTrees = [], recentActivityTrees = [], organizationName = 'Projects', options = {}) {
  ...
  dispatchPanelSeq = 0 // LIN-295: deterministic, unique dispatch panel IDs per page render
...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false) {
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    if (featureFlags.aiRecommendations !== false) {
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      const recommendActions = renderDispatchActions({ dispatchEnabled: featureFlags.dispatch === true, proxyEnabled: featureFlags.proxy === true, isLocalhost })
    ...
    if (featureFlags.proxy === true) {
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      // Foreman + mini-foreman each get their own disclosure (distinct panel IDs).
      const foremanActions = renderDispatchActions({ dispatchEnabled: featureFlags.dispatch === true, proxyEnabled: true, isLocalhost })
      const miniForemanActions = renderDispatchActions({ dispatchEnabled: featureFlags.dispatch === true, proxyEnabled: true, isLocalhost })
      ...
      promptsContent.push(`<div class="mini-foreman-container prompt-container hidden" data-mini-foreman-for="${issue.id}"${urlKeyAttr}><div class="prompt-header"><span class="prompt-name">Mini-foreman</span><div class="prompt-actions">${miniForemanActions}</div></div><div class="prompt-text"></div></div>`)
    }
  }
