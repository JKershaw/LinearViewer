...
import { getPromptLabels } from './prompt-templates.js'
 ...
 */
function renderLabels(labels, issueId) {
  ...
  const labelHtml = labels.map(label => {
    ...
    if (PROMPT_LABELS.has(label.name)) {
      ...
      return `<a href="#" class="label-prompt" data-issue-id="${issueId}" data-label="${escapeHtml(label.name)}">${name}</a>`
    }
...
function renderDetails(issue, depth, section = 'project') {
  ...
  const labelHtml = renderLabels(issue.labels?.nodes, issue.id)
  if (labelHtml) {
    meta.push(labelHtml)
  }
  ...
  // Add prompt container for issues with promptable labels
  const promptableLabels = (issue.labels?.nodes || []).filter(l => PROMPT_LABELS.has(l.name))
  if (promptableLabels.length > 0) {
