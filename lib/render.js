 ...
 */
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false, isLocalhost = false, kind = '', periodicalId = '', optionsId = '', proxyForce = false } = {}) {
  ...
  return `<div class="prompt-container${hiddenClass}"${issueAttr}${urlKeyAttr}${kindAttr}${periodicalIdAttr}${proxyForceAttr}><div class="prompt-header"><span class="prompt-name">${escapeHtml(name)}</span><div class="prompt-actions">${actionButtons}</div></div><div class="prompt-text">${content}</div></div>`
}
...
export function renderDetailsContent(issue, { isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false, provider = null, section = '' } = {}) {
  ...
  if (!isLanding && urlKey && ui.inlineEdit) {
    const editHref = `/workspace/${encodeURIComponent(urlKey)}/task/${encodeURIComponent(issue.id)}/edit`
