...
// LIN-1553 (Session B): the in-app issue CREATE form. Rendered ONLY when the
// active provider's derived `ui.inlineCreate` flag is true (the session-auth
// createIssue capability derived in lib/providers/interface.js). Render gates on
// that `ui.*` flag exclusively — never on `supports()` — per the LIN-177
// capability-surface convention. This is markup only: the submit wiring (fetch →
// POST /workspace/:urlKey/api/issues → refresh the detail view) lives in
// public/app.js, keyed off the `data-testid` / `name` contract below. Styling
// uses semantic token classes only (no hard-coded colors), so light and
// `.theme-dark` both work.
//
// The EDIT half moved out in LIN-1565: an inline form hidden inside a collapsed
// Details panel was the UX defect, so editing is now its own drill-down page
// (lib/render-task-edit.js) that the Details panel links to. The `ui.inlineEdit`
// gate did not change — it just moved to the link and the page's route.
const PRIORITY_OPTION_LABELS = ['No priority', 'Urgent', 'High', 'Medium', 'Low']

// Exported for the dedicated task-edit page (LIN-1565), which renders the same
// 0–4 priority vocabulary the create form does. One list, two surfaces — a
// second copy would drift the moment Linear's priority scale changed.
...
// Create form (project-scoped). v1 fields ONLY: title (required), description,
// teamId, projectId, priority, stateId — mirroring POST /workspace/:urlKey/api/issues.
function renderInlineCreateForm({ urlKey, projectId }) {
  const urlKeyAttr = escapeHtml(urlKey || '')
  const projectIdAttr = escapeHtml(projectId || '')
  return `<div class="add-task-link inline-issue" data-testid="create-task">` +
    `<button type="button" class="inline-issue-trigger" data-action="create-task-toggle" data-testid="create-task-trigger">+ Add task</button>` +
    `<form class="inline-issue-form hidden" data-inline-create data-url-key="${urlKeyAttr}" data-project-id="${projectIdAttr}" data-testid="create-task-form">` +
      `<label class="inline-field"><span class="inline-field-label">Title</span>` +
        `<input type="text" class="inline-field-input" name="title" required data-testid="create-task-title"></label>` +
      `<label class="inline-field"><span class="inline-field-label">Description</span>` +
        `<textarea class="inline-field-input inline-field-textarea" name="description" data-testid="create-task-description"></textarea></label>` +
      `<label class="inline-field"><span class="inline-field-label">Team ID</span>` +
        `<input type="text" class="inline-field-input" name="teamId" data-testid="create-task-teamId"></label>` +
      `<label class="inline-field"><span class="inline-field-label">Project ID</span>` +
        `<input type="text" class="inline-field-input" name="projectId" value="${projectIdAttr}" data-testid="create-task-projectId"></label>` +
      `<label class="inline-field"><span class="inline-field-label">Priority</span>` +
        `<select class="inline-field-input" name="priority" data-testid="create-task-priority">${priorityOptionsHtml(0)}</select></label>` +
      `<label class="inline-field"><span class="inline-field-label">State ID</span>` +
        `<input type="text" class="inline-field-input" name="stateId" data-testid="create-task-stateId"></label>` +
      `<div class="inline-field-actions">` +
        `<button type="submit" class="inline-issue-submit" data-testid="create-task-submit">Create task</button>` +
        `<span class="inline-issue-status" data-inline-status aria-live="polite"></span>` +
      `</div>` +
    `</form>` +
  `</div>`
}
...
function renderProject({ project, incomplete, completed, completedCount }, options = {}) {
  ...
  // render an in-app create form; the external deep-link is REPLACED for them.
  // Providers with only an external create affordance (ui.write, no inlineCreate)
  // keep the original linear.app-style target="_blank" deep-link unchanged.
  let addTaskLink = ''
  if (isRealProject && ui.inlineCreate) {
    addTaskLink = renderInlineCreateForm({ urlKey, projectId: project.id })
