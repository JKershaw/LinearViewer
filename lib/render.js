...
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false } = {}) {
  ...
  const dispatchBtns = dispatchEnabled
    ? '<button class="prompt-dispatch dispatch-btn" data-target="cli">dispatch</button><button class="prompt-dispatch dispatch-btn" data-target="web">dispatch &rarr; web</button>'
    : ''

  return `<div class="prompt-container${hiddenClass}"${issueAttr}${urlKeyAttr}><div class="prompt-header"><span class="prompt-name">${escapeHtml(name)}</span><div class="prompt-actions">${dispatchBtns}<button class="prompt-copy">copy</button></div></div><div class="prompt-text">${content}</div></div>`
}
...
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}) {
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    if (featureFlags.aiRecommendations !== false) {
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      const recommendDispatchBtn = featureFlags.dispatch === true ? '<button class="prompt-dispatch dispatch-btn" data-target="cli">dispatch</button><button class="prompt-dispatch dispatch-btn" data-target="web">dispatch &rarr; web</button>' : ''
