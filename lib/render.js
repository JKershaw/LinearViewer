...
// LIN-1553 (Session B): in-app issue create/edit forms. Rendered ONLY when the
// active provider's derived `ui.inlineCreate` / `ui.inlineEdit` flags are true
// (the session-auth createIssue/updateIssue capabilities derived in
// lib/providers/interface.js). Render gates on those `ui.*` flags exclusively —
// never on `supports()` — per the LIN-177 capability-surface convention. This is
// markup only: the submit wiring (fetch → POST/PATCH /workspace/:urlKey/api/
// issues[...] → refresh the detail view) lands in public/app.js (beat 3), keyed
// off the `data-testid` / `name` contract below. Styling uses semantic token
// classes only (no hard-coded colors), so light and `.theme-dark` both work.
...
// Edit form (issue-scoped). v1 fields ONLY: title, description (full-body
// textarea — the PATCH sends the whole body), state (stateId), priority —
// mirroring PATCH /workspace/:urlKey/api/issues/:id.
function renderInlineEditForm({ issue, urlKey }) {
  const urlKeyAttr = escapeHtml(urlKey || '')
  const issueIdAttr = escapeHtml(issue.id || '')
  const titleVal = escapeHtml(issue.title || '')
  const descVal = escapeHtml(issue.description || '')
  const stateVal = escapeHtml(issue.state?.name || '')
  return `<div class="inline-issue inline-issue-edit" data-testid="edit-issue">` +
    `<button type="button" class="inline-issue-trigger" data-action="edit-issue-toggle" data-testid="edit-issue-trigger">Edit</button>` +
    `<form class="inline-issue-form hidden" data-inline-edit data-issue-id="${issueIdAttr}" data-url-key="${urlKeyAttr}" data-testid="edit-issue-form">` +
      `<label class="inline-field"><span class="inline-field-label">Title</span>` +
        `<input type="text" class="inline-field-input" name="title" value="${titleVal}" data-testid="edit-issue-title"></label>` +
      `<label class="inline-field"><span class="inline-field-label">Description</span>` +
        `<textarea class="inline-field-input inline-field-textarea" name="description" data-testid="edit-issue-description">${descVal}</textarea></label>` +
      `<label class="inline-field"><span class="inline-field-label">State</span>` +
        `<input type="text" class="inline-field-input" name="stateId" value="${stateVal}" data-testid="edit-issue-stateId"></label>` +
      `<label class="inline-field"><span class="inline-field-label">Priority</span>` +
        `<select class="inline-field-input" name="priority" data-testid="edit-issue-priority">${priorityOptionsHtml(issue.priority)}</select></label>` +
      `<div class="inline-field-actions">` +
        `<button type="submit" class="inline-issue-submit" data-testid="edit-issue-submit">Save changes</button>` +
        `<span class="inline-issue-status" data-inline-status aria-live="polite"></span>` +
      `</div>` +
    `</form>` +
  `</div>`
...
export function renderDetailsContent(issue, { isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false, provider = null, section = '' } = {}) {
  ...
  // ==========================================================================
  // Edit affordance (LIN-1553) — in-app edit form for the read-only detail.
  // Gated on ui.inlineEdit (session-auth updateIssue support) ONLY. Trigger +
  // hidden form prefilled with the issue's current values; client wiring (beat
  // 3) PATCHes /workspace/:urlKey/api/issues/:id and refreshes the detail view.
  // ==========================================================================
  if (!isLanding && urlKey && ui.inlineEdit) {
    detailsContent.push(renderInlineEditForm({ issue, urlKey }))
  }
