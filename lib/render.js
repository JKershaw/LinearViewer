...
export function renderLabels(issue, isLanding = false) {
  ...
  if (!isLanding && visiblePromptKeys.size > 0) {
  ...
  // Add "suggest" button for AI recommendation (only for authenticated users)
  if (!isLanding) {
    parts.push(`<button class="suggest-btn" data-issue-id="${issueId}" title="Get AI recommendation">suggest</button>`)
  }
...
function renderDetails(issue, depth, section = 'project', isLanding = false) {
  ...
  if (!isLanding) {
    ...
    // Add recommendation container for AI suggestions
    lines.push(`<div class="recommend-container hidden" data-recommend-for="${issue.id}"><div class="recommend-header"><span class="recommend-title">Suggested approach</span><button class="recommend-close">dismiss</button></div><p class="recommend-reasoning"></p><div class="recommend-actions"><button class="recommend-use-prompt hidden" data-issue-id="${issue.id}"></button><div class="recommend-custom-prompt hidden"><div class="prompt-header"><span class="prompt-name">Custom suggestion</span><button class="prompt-copy">copy</button></div><pre class="prompt-text"></pre></div></div></div>`)
  }
