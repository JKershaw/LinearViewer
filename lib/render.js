...
function renderNode(node, isLast, ancestors, parentId) {
  ...
  const line = `<div class="line${hasDetails ? ' has-details' : ''}" data-id="${issue.id}" data-parent="${parentId}" data-depth="${depth}" style="--depth: ${depth}"><span class="prefix">${prefix}</span><span class="state ${stateClass}">${stateChar}</span><span class="${titleClass}">${escapeHtml(issue.title)}</span>${toggle}</div>`
