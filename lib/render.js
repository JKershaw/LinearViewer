...
export { renderLoginPage, renderErrorPage, renderWorkspaceNotFoundPage } from './render-pages.js'
 ...
 */
function renderPromptContainer({ name = '', content = '', issueId = '', urlKey = '', dispatchEnabled = false, proxyEnabled = false, isLocalhost = false } = {}) {
  ...
  const localButton = dispatchEnabled && isLocalhost ? '<button class="prompt-dispatch dispatch-btn" data-target="local">harbour</button>' : ''
  const actionButtons = dispatchEnabled
    ? '<span class="prompt-actions-label">Dispatch:</span><button class="prompt-dispatch dispatch-btn" data-target="cli">cli</button><button class="prompt-dispatch dispatch-btn" data-target="web">web</button><button class="prompt-dispatch dispatch-btn" data-target="dash">dash</button>' + localButton + '<button class="prompt-copy">copy</button>' + proxyToggle
    : '<button class="prompt-copy">copy</button>' + proxyToggle

  return `<div class="prompt-container${hiddenClass}"${issueAttr}${urlKeyAttr}><div class="prompt-header"><span class="prompt-name">${escapeHtml(name)}</span><div class="prompt-actions">${actionButtons}</div></div><div class="prompt-text">${content}</div></div>`
}
...
export function renderPage(projectTrees, inProgressTrees = [], recentActivityTrees = [], organizationName = 'Projects', options = {}) {
  ...
  const inProgressHtml = renderInProgressSection(inProgressTrees, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts, isLocalhost })
  const recentActivityHtml = renderRecentActivitySection(recentActivityTrees, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts, isLocalhost })
  const projectsHtml = projectTrees.map(tree => renderProject(tree, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts, isLocalhost })).join('\n')
...
function renderInProgressSection(inProgressTrees, options = {}) {
  const { isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false } = options
...
function renderProject({ project, incomplete, completed, completedCount }, options = {}) {
  ...
  const incompleteHtml = incomplete
    .map(node => renderNode(node, project.id, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts }))
    .join('\n')
  ...
  const completedHtml = completed.length > 0
    ? `<div class="tree hidden" data-completed-for="${project.id}">${completed.map(node => renderNode(node, project.id, { isLanding, openRouterSource, urlKey, featureFlags, customPrompts })).join('\n')}</div>`
    : ''
...
function renderNode(node, parentId, options = {}) {
  const { section = 'project', projectName = null, isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false } = options
  ...
  const line = `<div class="${lineClasses}" data-id="${issue.id}" data-identifier="${escapeHtml(issue.identifier || '')}" data-parent="${parentId}" data-depth="${depth}" data-section="${section}" data-search-text="${searchText}" style="--depth: ${depth}"><span class="state ${stateClass}" data-status="${stateClass}" aria-label="Status: ${stateLabel}">${stateChar}</span><span class="${titleClass}">${escapeHtml(issue.title)}</span>${timeBadge}${projectBadge}${toggle}</div>`

  // Render details section
  const details = hasDetails ? renderDetails(issue, depth, section, isLanding, openRouterSource, urlKey, featureFlags, customPrompts, isLocalhost) : ''
...
export function renderPromptButtons(issue, isLanding = false, openRouterSource = null, customPrompts = [], featureFlags = {}) {
  ...
  // Foreman button (proxy feature) — sibling to AI suggest: launches an
  // autonomous-run playbook pinned to this task, ready for copy or dispatch.
  if (featureFlags.proxy === true) {
    parts.push(`<a href="#" class="label-prompt foreman-btn" data-issue-id="${issueId}" title="Foreman playbook pinned to this task">Foreman</a>`)
    parts.push(`<a href="#" class="label-prompt mini-foreman-btn" data-issue-id="${issueId}" title="One-step API fetch: agent pulls a fresh prompt from the proxy and runs it once">Mini-foreman</a>`)
  }

  // No prompts for terminal-state issues (completed/canceled/duplicate)
  const stateType = issue.state?.type?.toLowerCase() || ''
  if (isTerminalState(stateType)) return parts.join(' ')
 ...
 */
function renderDetails(issue, depth, section = 'project', isLanding = false, openRouterSource = null, urlKey = null, featureFlags = {}, customPrompts = [], isLocalhost = false) {
  const lines = []
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    if (featureFlags.aiRecommendations !== false) {
      ...
      const recommendLocalButton = featureFlags.dispatch === true && isLocalhost ? '<button class="prompt-dispatch dispatch-btn" data-target="local">harbour</button>' : ''
      const recommendActions = featureFlags.dispatch === true
        ? '<span class="prompt-actions-label">Dispatch:</span><button class="prompt-dispatch dispatch-btn" data-target="cli">cli</button><button class="prompt-dispatch dispatch-btn" data-target="web">web</button><button class="prompt-dispatch dispatch-btn" data-target="dash">dash</button>' + recommendLocalButton + '<button class="prompt-copy">copy</button>' + recommendProxyToggle
        : '<button class="prompt-copy">copy</button>' + recommendProxyToggle
    ...
    // Foreman container: parallel to the AI recommendation container but
    // non-streaming. Rendered when the proxy feature is on.
    if (featureFlags.proxy === true) {
      const urlKeyAttr = urlKey ? ` data-url-key="${escapeHtml(urlKey)}"` : ''
      const foremanLocalButton = featureFlags.dispatch === true && isLocalhost ? '<button class="prompt-dispatch dispatch-btn" data-target="local">harbour</button>' : ''
      const foremanProxyToggle = '<button class="prompt-proxy-toggle" title="Append proxy API instructions to prompt">+proxy</button>'
      const foremanActions = featureFlags.dispatch === true
        ? '<span class="prompt-actions-label">Dispatch:</span><button class="prompt-dispatch dispatch-btn" data-target="cli">cli</button><button class="prompt-dispatch dispatch-btn" data-target="web">web</button><button class="prompt-dispatch dispatch-btn" data-target="dash">dash</button>' + foremanLocalButton + '<button class="prompt-copy">copy</button>' + foremanProxyToggle
        : '<button class="prompt-copy">copy</button>' + foremanProxyToggle
      promptsContent.push(`<div class="foreman-container prompt-container hidden" data-foreman-for="${issue.id}"${urlKeyAttr}><div class="prompt-header"><span class="prompt-name">Foreman</span><div class="prompt-actions">${foremanActions}</div></div><div class="prompt-text"></div></div>`)
      promptsContent.push(`<div class="mini-foreman-container prompt-container hidden" data-mini-foreman-for="${issue.id}"${urlKeyAttr}><div class="prompt-header"><span class="prompt-name">Mini-foreman</span><div class="prompt-actions">${foremanActions}</div></div><div class="prompt-text"></div></div>`)
    }
  }
