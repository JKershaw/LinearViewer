...
function renderProject({ project, incomplete, completed, completedCount }, options = {}) {
  ...
  const viewLink = project.url
    ? `<a href="${project.url}"${projectTarget} class="detail-link">${project.linkText || 'View →'}</a>`
    : ''
  const addLink = !isLanding && urlKey && project.id
    ? `<a href="https://linear.app/${encodeURIComponent(urlKey)}/new?project=${encodeURIComponent(project.id)}" target="_blank" class="detail-link" data-action="create-task">Add +</a>`
    : ''

  const projectLink = viewLink || addLink
    ? `<div class="project-meta${hasDescription ? ' hidden' : ''}">Linear: ${viewLink}${viewLink && addLink ? ' ' : ''}${addLink}</div>`
    : ''
