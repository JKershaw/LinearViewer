...
function renderNode(node, parentId, options = {}) {
  ...
  const line = `<div class="${lineClasses}"${expandableAttrs} data-id="${issue.id}" data-testid="issue-line" data-identifier="${escapeHtml(issue.identifier || '')}" data-parent="${parentId}" data-depth="${depth}" data-section="${section}" data-search-text="${searchText}" style="--depth: ${depth}"><span class="state ${stateClass}" data-status="${stateClass}" aria-label="Status: ${stateLabel}">${stateChar}</span><span class="${titleClass}">${escapeHtml(issue.title)}</span>${timeBadge}${projectBadge}${sourceBadge}${toggle}</div>`
  ...
  if (!isLanding && featureFlags.promptButtons !== false) {
    ...
    if (featureFlags.aiRecommendations !== false) {
      ...
      const recommendActions = featureFlags.dispatch === true
        ? renderDispatchDisclosure(`recommend-options-${instanceKey}`, isLocalhost) + '<button class="prompt-copy">copy</button><button class="prompt-download" title="Download prompt as a .md file">download</button>' + recommendProxyToggle
    ...
    if (featureFlags.proxy === true) {
      ...
      const autopilotActions = featureFlags.dispatch === true
        ? renderDispatchDisclosure(`autopilot-options-${instanceKey}`, isLocalhost) + '<button class="prompt-copy">copy</button><button class="prompt-download" title="Download prompt as a .md file">download</button>' + autopilotProxyToggle
