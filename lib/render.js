...
export function renderPage(projectTrees, inProgressTrees = [], organizationName = 'Projects', options = {}) {
  const { isLanding = false, teams = [], selectedTeamId = null, workspaces = [], activeWorkspaceId = null, openRouterSource = null } = options
...
function renderOpenRouterNavItem(source) {
  ...
  return `<div class="nav-item" data-selector="openrouter">
      ...
      <a href="/fancy" class="nav-value nav-openrouter-status ${statusClass}" title="OpenRouter: ${statusText}">${statusIcon}</a>
    </div>`
}
...
function renderDetails(issue, depth, section = 'project', isLanding = false) {
  ...
  if (!isLanding) {
    ...
    lines.push(`<div class="recommend-container hidden" data-recommend-for="${issue.id}"><div class="recommend-header"><span class="recommend-title">AI Suggestion</span><button class="recommend-close">dismiss</button></div><p class="recommend-reasoning"></p><div class="recommend-prompt"><div class="prompt-header"><span class="prompt-name">Generated Prompt</span><button class="prompt-copy">copy</button></div><pre class="prompt-text"></pre></div></div>`)
  }
