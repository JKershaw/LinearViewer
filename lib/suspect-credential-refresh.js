 ...
 */
export function isBareTokenCredentialProvider(provider) {
  return normalizeProviderName(provider) === 'linear';
}

/**
 * Remedy 1 in isolation (LIN-2473), extracted (LIN-2933) so a second
 * consumer — the session-lane comment route's one-shot recovery — can call
 * it without pulling in Remedy 2's exchange/cooldown/session-load
 * machinery. `attemptSuspectCredentialRefresh` below calls this as its own
 * first remedy; behavior there is unchanged.
 *
 * Deliberately BEFORE `shouldAttemptRefresh` in the caller above: adopting
 * costs nothing, so it must not consume the one rate-limited exchange
 * attempt this window allows.
 *
 ...
 * @returns {Promise<{token: *, expiresAt: number, provider: string, credentialFingerprint: string}|null>}
 */
export async function adoptDurableCredentialIfDifferent({ fingerprint, urlKey, ownerAccountId, provider, store }) {
  if (!isBareTokenCredentialProvider(provider)) return null;
  try {
...
export async function attemptSuspectCredentialRefresh({ fingerprint, urlKey, ownerAccountId, provider, loadSessions, registry, store, lifecycleEventStore, refreshAccessToken, persistSession, resolveProvider, resolveExchange }) {
  ...
  const adopted = await adoptDurableCredentialIfDifferent({ fingerprint, urlKey, ownerAccountId, provider, store });
