 ...
 * the route gathers `sessions` / `agentStatusItems` / `llmStats` from the stores and
 * hands them in, so determinism and addressability are trivially testable.
 *
 * @param {Object} input
...
export function formatEditionContext(model) {
  ...
  lines.push(`Activity: ${model.counts.sessions} autopilot session(s), ${model.counts.status} status update(s) with prose.`);
