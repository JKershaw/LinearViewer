...
import { QUEUE_CONFIG, getQueueForLabel, getRequiredQueues } from './queue-config.js';
...
/**
 * Fetches custom fields (if any exist).
 * NOTE: This query may not exist in Linear's current public API.
 * It's wrapped in try/catch and will return empty array if unavailable.
 */
...
async function fetchAuditData(accessToken) {
  ...
  // Try to fetch custom fields (may not exist in all workspaces)
  let customFields = [];
  try {
    const customFieldsData = await client.request(CUSTOM_FIELDS_QUERY);
    customFields = customFieldsData.customAttributes?.nodes || [];
  } catch {
    // Custom attributes endpoint may not be available
    customFields = [];
  }
...
/**
 * Computes queue readiness analysis.
 *
 * @param {Object} data - Raw audit data
...
function computeQueueReadiness(data, labelAnalysis) {
  // Build a map of queue -> label info
  const queueStatus = {};
  for (const queueDef of QUEUE_CONFIG) {
    queueStatus[queueDef.name] = {
      ...
      labelPatterns: queueDef.labelPatterns,
      matchedLabel: null,
      taskCount: 0,
      exists: false
    };
  ...
  // Check which queues have matching labels
  for (const label of labelAnalysis.mapped) {
    if (label.queue && queueStatus[label.queue]) {
      queueStatus[label.queue].exists = true;
      queueStatus[label.queue].matchedLabel = label.name;
      queueStatus[label.queue].taskCount = label.issueCount;
    }
  }
...
function computeFieldUsage(data) {
  const { issues, customFields } = data;

  if (issues.length === 0) {
    return {
      estimatesUsage: 0,
      dueDatesUsage: 0,
      customFields: customFields.map(f => ({ name: f.name, type: f.type })),
...
export function computeAuditFromData(mockData) {
  const data = {
    ...
    customFields: mockData.customFields || []
  };
