...
import { getWorkIssueLabels } from './workflow-config.js';
import {
  getPromptLabels,
  PROMPT_TEMPLATES,
  PROMPT_CATEGORIES,
...
async function fetchAuditData(accessToken) {
  // Per-attempt timeout + bounded retry on transient Linear connection drops
  // (lib/linear-fetch.js). Audit fires several parallel paginated queries, so a
  // single dropped socket would otherwise fail the whole report; all queries
  // here are reads, so every request is safely retryable.
  const client = new GraphQLClient('https://api.linear.app/graphql', {
    headers: { Authorization: accessToken },
    fetch: createLinearFetch(globalThis.fetch)
  });
...
function computeLabelAnalysis(data) {
  ...
  const workflowLabelsExpected = [...getWorkIssueLabels()];
