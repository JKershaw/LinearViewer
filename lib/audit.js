...
import { getPhaseLabels, getWorkIssueLabels } from './workflow-config.js';
import {
  getPromptLabels,
  PROMPT_TEMPLATES,
  PROMPT_CATEGORIES,
...
function computeLabelAnalysis(data) {
  ...
  // Get all workflow-managed labels
  const phaseLabelsExpected = getPhaseLabels();
  const workIssueLabelsExpected = getWorkIssueLabels();
  const allWorkflowLabels = new Set([...phaseLabelsExpected, ...workIssueLabelsExpected]);
  ...
  const phaseLabels = buildWorkflowLabelStatus(phaseLabelsExpected);
  const workIssueLabels = buildWorkflowLabelStatus(workIssueLabelsExpected);

  const workflowPresentCount = [...phaseLabels, ...workIssueLabels].filter(l => l.exists).length;
  const workflowTotalCount = phaseLabelsExpected.length + workIssueLabelsExpected.length;
...
function computeTaskHealth(data) {
  ...
  // States that indicate a task is "done" and shouldn't be flagged for health issues
  const DONE_STATES = ['completed', 'canceled'];
  ...
  for (const issue of issues) {
    ...
    // Skip health checks for completed/canceled tasks - they're done
    const isDone = DONE_STATES.includes(stateType.toLowerCase());
    if (isDone) {
      continue;
    }
