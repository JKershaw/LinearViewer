...
import { DEFAULT_MODEL, streamChat, resolveReasoningBudget } from './openrouter.js';
...
/**
 * Visible-prose budget for the goal-suggestion reply (LIN-1665). This is the JSON
 * document itself — analysis + directions + up to MAX_GENERATED_OPTIONS options —
 * NOT the request's `max_tokens`: hidden reasoning is reserved on top of it by
 * resolveReasoningBudget in the generator. 1600 is the LIN-1566 figure (1400 + the
 * directions block), kept unchanged; what changed is that it is now a *prose* floor
 * rather than a total the reasoning run could eat.
 */
export const NEXT_RUN_PROSE_MAX_TOKENS = 1600;
 ...
 * @param {number} [options.maxTokens=NEXT_RUN_PROSE_MAX_TOKENS] - Visible-prose budget for the
 *   reply. Hidden reasoning is reserved on top of it (LIN-1665), so this is a floor on the JSON
 *   document, not the request's `max_tokens`.
...
export async function generateGoalSuggestions({ projects = [], issues = [], organizationName = '', roadmapReport = null, northStar = '' } = {}, options = {}) {
  ...
  // Reserve hidden-reasoning headroom ON TOP of the prose budget instead of sending
  // a bare cap the reasoning run can eat (LIN-1665, the residual LIN-1000 explicitly
  // left on this path). The prose figure is still 1600 — not 1400 (LIN-1566): the
  // directions block adds ~80-150 tokens and a truncated reply does NOT degrade
  // gracefully — extractJsonObject returns null and the whole generation collapses to
  // deterministic size fills. That collapse was reachable at 1600 too, because a
  // gpt-5.x model bills its hidden reasoning INSIDE the same cap (measured: 516
  // reasoning tokens against a pinned 1600 ⇒ finish_reason 'length' ⇒ JSON cut
  // mid-document ⇒ flat cards). No-op on a non-reasoning model by construction:
  // `reasoning` is undefined ⇒ streamChat omits the field ⇒ byte-identical body, and
  // maxTokens stays the prose budget unchanged.
  const { reasoning, maxTokens } = resolveReasoningBudget({
    model,
    proseTokens: options.maxTokens || NEXT_RUN_PROSE_MAX_TOKENS,
  });
  ...
  await streamChat(
    messages,
    { apiKey: options.apiKey, model, maxTokens, reasoning, temperature: 0.4,
  ...
  if (!outcome.ok) {
    console.error(
      ...
      `maxTokens=${maxTokens}, reasoning=${reasoning ? reasoning.max_tokens : 'none'}) — ` +
