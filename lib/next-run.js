 ...
 * @returns {Promise<{options: Array, model: string, context: string}>} options always
 *   ends with the continue-until-stopped option (empty goal); an LLM failure still
 *   yields that single guaranteed option rather than an error. `context` is the exact
 *   deterministic grounding blob the model was given, returned verbatim so the page
 *   can show the user what the suggestions were grounded in (LIN-633).
 */
export async function generateGoalSuggestions({ projects = [], issues = [], organizationName = '' } = {}, options = {}) {
  ...
  // lead.
  return { options: [...generated, { ...CONTINUE_UNTIL_STOPPED_OPTION }], model, context };
}
