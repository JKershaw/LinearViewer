...
- Provide AT LEAST ONE option for each size S, M, and L (three sizes, so three or more options). Make them genuinely different (e.g. finish in-progress work vs. clear a blocker vs. push a milestone), not rewordings of one idea.
...
export async function generateGoalSuggestions({ projects = [], issues = [], organizationName = '' } = {}, options = {}) {
  ...
  // lead.
  return { analysis, options: [...covered, { ...CONTINUE_UNTIL_STOPPED_OPTION }], model, summary, context };
}
