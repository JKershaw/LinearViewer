 ...
 *
 * Options are additionally grouped under a small set of named *directions*
 * (LIN-1566) so the page can ask "which direction?" before "which goal?". The
 * flat `options` array stays authoritative — a direction is a per-option string
 * tag plus a derived `directions: [{name, summary, optionIndexes}]` list, so an
 * ungrouped reply still renders as the flat list. "Direction" is deliberately
 * this module's word for the group: "theme" is already taken by page theming and
 * by the roadmap narrative layer.
 */
...
/**
 * `max_tokens` cap for the goal-suggestion reply (LIN-1665). This is a BARE
 * `max_tokens` cap — no `reasoning` field is sent (live evidence on gpt-5.4-mini
 * shows sending it switches reasoning ON in a model that otherwise spends zero, and
 * the field is advisory, not honoured as a cap by OpenRouter). 4000 is the
 * evidence-backed figure from the LIN-1665 investigation: it fixes the failing
 * model (gpt-5.5, 3/3 grouped vs. 0/2 at 1600) and is a no-op on the default
 * model (~750-1050 chars spent either way). A bare ceiling, not a guarantee.
 */
export const NEXT_RUN_PROSE_MAX_TOKENS = 4000;

/**
 * Max named directions retained from a reply (LIN-1566). The prompt asks for 2–3;
 * this is the tolerance cap, not the target. The catch-all below is added on top
 * of these, so the rendered chooser can hold one more row than this number.
 */
export const MAX_DIRECTIONS = 4;

/**
 * Name of the trailing catch-all direction that collects options the model left
 * untagged or tagged with a name it never declared — plus the deterministic
 * size fills, which are synthesized after the LLM reply and so carry no
 * direction of their own (LIN-1566). Lowercase to match this page's copy
 * register ("continue until stopped"), and deliberately not "theme"/"misc".
 */
export const CATCH_ALL_DIRECTION = 'other';
...
Schema:
{
  ...
  "directions": [
    {
      "name":    string,   // a short lowercase label naming what this direction is about, e.g. "finish the proxy migration" — 2-6 words
      "summary": string    // one line: what choosing this direction commits the next run to
    }
  ],
  "options": [
    {
      ...
      "direction":         string    // which direction this option sits under — copied VERBATIM from a "name" declared above
    }
  ]
}
...
- Group your options under 2-3 "directions": named strands of work the project could push on next. Infer them from the provided state — there is no fixed taxonomy — and order them most valuable first, by the same ranking you used for the options. A direction NAMES what its goals are about ("finish the proxy migration"), it is not a size, a status, or a project name.
- Give each direction 2-3 options, and set every option's "direction" to one of the declared names, copied verbatim. Never leave "direction" blank and never use a name you did not declare.
- Return AT MOST ${MAX_GENERATED_OPTIONS} options in total across all directions. Anything beyond that is discarded, which would silently empty out your last direction — so if you name 3 directions, keep them to 2 options each.
- Provide AT LEAST ONE option for each size S, M, and L (three sizes, so three or more options). Make them genuinely different (e.g. finish in-progress work vs. clear a blocker vs. push a milestone vs. bundle many small queued tasks into one sweep), not rewordings of one idea. This S/M/L spread is required across ALL your options TAKEN TOGETHER — it is NOT required within each direction. Do NOT add extra options just to balance sizes inside a direction.
...
/**
 * Shared age computation for a report's `generatedAt`, used by
 * resolveRoadmapNarrative, resolveNorthStarSignal, and classifyReportFreshness
 * so the three can never interpret "how old is this report" differently.
 *
 * Returns null when there is no usable age: missing/invalid `generatedAt`, or
 * future-dated (clock skew) — both treated as unusable rather than "0 days",
 * matching the original resolvers' behavior.
 *
 * @param {string|undefined} generatedAt - A report's ISO generatedAt.
 * @param {number} now - Injectable clock (ms since epoch).
 * @returns {number|null} Whole days old, or null when unusable.
 */
function computeAgeDays(generatedAt, now) {
...
export function resolveRoadmapNarrative(report, { maxAgeDays = ROADMAP_REPORT_MAX_AGE_DAYS, now = Date.now() } = {}) {
  ...
  const ageDays = computeAgeDays(report.generatedAt, now);
...
export function resolveNorthStarSignal(northStar, report, { maxAgeDays = ROADMAP_REPORT_MAX_AGE_DAYS, now = Date.now() } = {}) {
  ...
  if (report && report.generatedAt) {
    const age = computeAgeDays(report.generatedAt, now);
...
/**
 * Freshness classification of a durable roadmap report, independent of any
 * narrative content (LIN-1810). `resolveNorthStarSignal`'s `ageDays` is `null`
 * for four distinct causes (no report / stale report / future-dated report /
 * fresh report whose narrative never scored alignment) that a consumer cannot
 * tell apart from the resolver's output alone. This gives a proxy route (or any
 * caller) the report-level discriminator directly, sharing `computeAgeDays` with
 * both resolvers above so it cannot apply a second interpretation of the gate.
 *
 * @param {Object|null} report - A ReportRecord from reportHistoryStore.getLatest().
 * @param {Object} [opts]
 * @param {number} [opts.maxAgeDays=ROADMAP_REPORT_MAX_AGE_DAYS]
 * @param {number} [opts.now=Date.now()] - Injectable clock for deterministic tests.
 * @returns {'absent'|'stale'|'fresh'} 'absent' when there is no report, no
 *   `generatedAt`, or an unparseable `generatedAt` (no trustworthy timestamp to
 *   judge at all); 'stale' when the timestamp IS usable but fails the gate —
 *   older than `maxAgeDays`, or future-dated (clock skew); 'fresh' otherwise.
 *   Says nothing about whether the report's narrative is populated.
 */
export function classifyReportFreshness(report, { maxAgeDays = ROADMAP_REPORT_MAX_AGE_DAYS, now = Date.now() } = {}) {
  if (!report || !report.generatedAt) return 'absent';
  if (!Number.isFinite(new Date(report.generatedAt).getTime())) return 'absent';
  const ageDays = computeAgeDays(report.generatedAt, now);
  if (ageDays === null) return 'stale'; // future-dated (clock skew) — a report exists but is unusable
...
/**
 * Normalize the reply's declared direction list to `[{ name, summary }]` (LIN-1566):
 * trimmed, blank names dropped, deduped case-insensitively (first wins), capped at
 * MAX_DIRECTIONS. Order is preserved — the model is asked to rank directions the
 * same way it ranks options, so declaration order is the render order.
 *
 * @param {*} value
 * @returns {Array<{name: string, summary: string}>}
 */
function normalizeDeclaredDirections(value) {
...
/**
 * Parse a full next-run response into its global `analysis` preamble, the declared
 * `directions` (LIN-1566), plus the sanitized concrete options. Tolerates code
 * fences and prose around the JSON. The continue-until-stopped option is NOT added
 * here — that is the generator's job. Referenced-task ids are kept as the model
 * emitted them; validation against the live state happens in the generator (which
 * holds the roadmap model). A reply that declares no directions (or tags no option)
 * parses cleanly to `directions: []` / `direction: ''`, which the resolver turns
 * into today's ungrouped list — grouping is additive, never required.
 *
 * @param {string} raw - Raw model text.
 * @returns {{ analysis: string, directions: Array<{name: string, summary: string}>, options: Array<{title: string, goal: string, reasoning: string, size: string, referencedTaskIds: string[], direction: string}> }}
 */
export function parseNextRunResponse(raw) {
  const parsed = extractJsonObject(raw);
  if (!parsed) return { analysis: '', directions: [], options: [] };

  const analysis = typeof parsed.analysis === 'string' ? parsed.analysis.trim() : '';
  const directions = normalizeDeclaredDirections(parsed.directions);
  ...
  const options = rawOptions
    .map(o => {
      ...
      return {
        ...
        // The grouping carrier (LIN-1566). Kept as the model emitted it (trimmed);
        // matching it against the declared names is resolveDirections' job, so an
        // unknown or blank tag is a normal, handled case rather than a parse error.
        direction: typeof o?.direction === 'string' ? o.direction.trim().slice(0, 60) : ''
      };
    })
    .filter(Boolean)
    .slice(0, MAX_GENERATED_OPTIONS);

  return { analysis, directions, options };
}
...
/**
 * Resolve the reply's declared directions + each option's `direction` tag into the
 * grouping the page renders (LIN-1566). Pure. This is the ONE place grouping is
 * normalised: the client renders `optionIndexes` and never re-derives groups by
 * matching names itself, and the test-mode mock calls this same function rather
 * than re-implementing it (a mock that grouped differently would let every grouped
 * e2e test pass against a shape the live path never produces).
 *
 * Three invariants the client may rely on:
 *
 *   1. `[]` means there is no usable grouping — render the flat list exactly as
 *      before. That covers a reply with no `directions`, a reply whose tags match
 *      nothing declared, a grouping that collapses to a single direction (one chip
 *      filters nothing), and the LLM-failure path (assumption A5).
 *   2. When non-empty, `optionIndexes` PARTITIONS every concrete option: each
 *      appears under exactly one direction, none is orphaned. Options that are
 *      untagged or tagged with an undeclared name land in a trailing
 *      CATCH_ALL_DIRECTION, which is what makes the partition total.
 *   3. The continue-until-stopped option is in NO direction — it is always offered
 *      outside the grouping (assumption A4).
 *
 * Call this LAST, after ensureSizeCoverage: the deterministic size fills are
 * synthesized after the reply is parsed, so they carry no `direction` and only
 * exist at the end of the pipeline. Running it earlier silently drops them out of
 * every group. Relatedly, `direction` rides the pipeline only because every stage
 * after the parser copies options by spread — do NOT "tidy" ensureSizeCoverage /
 * attachReferencedTaskTitles / the id-grounding filter into explicit field lists,
 * which would strip the tag and collapse every option into the catch-all.
 *
 * @param {Array} options - The FINAL option list, continue-until-stopped included.
 * @param {Array<{name: string, summary: string}>} declared - The reply's declared directions.
 * @returns {Array<{name: string, summary: string, optionIndexes: number[]}>}
 */
export function resolveDirections(options, declared) {
  const list = Array.isArray(options) ? options : [];
  const names = normalizeDeclaredDirections(declared);
  if (!names.length) return [];

  const byKey = new Map();
  for (const d of names) {
    byKey.set(d.name.toLowerCase(), { name: d.name, summary: d.summary, optionIndexes: [] });
  }

  const catchAll = [];
  let placed = 0;
  list.forEach((opt, index) => {
    // Invariant 3: the open-ended option is never grouped.
    if (!opt || opt.continueUntilStopped) return;
    const tag = typeof opt.direction === 'string' ? opt.direction.trim().toLowerCase() : '';
    const bucket = tag ? byKey.get(tag) : undefined;
    if (bucket) {
      bucket.optionIndexes.push(index);
      placed++;
    } else {
      catchAll.push(index);
    }
  ...
  // Nothing matched a declared name — the model declared directions but tagged
  // nothing with them (or tagged everything with names it never declared). One
  // catch-all pile holding every option is strictly worse than the flat list, so
  // report "no usable grouping" (invariant 1) instead.
  if (placed === 0) return [];

  const resolved = [...byKey.values()].filter(d => d.optionIndexes.length > 0);
  if (catchAll.length) {
    resolved.push({
      name: CATCH_ALL_DIRECTION,
      summary: 'Goals that do not sit under one of the named directions above.',
      optionIndexes: catchAll
    });
  }

  // A single surviving direction holds every concrete option, so its chooser would
  // be one chip that filters nothing — chrome for a no-op control. That is the same
  // degradation the `placed === 0` branch above already rejects, reached from the
  // other side (there: everything fell into the catch-all; here: everything landed
  // under one declared name, or the empty-direction filter left only one). A chooser
  // is only worth its space when there is something to choose between, so fall back
  // to the flat list (invariant 1). LIN-1566 review F2.
...
/**
 * Classify what the LLM reply amounted to, so a collapsed generation is not
 * presented as a normal one (LIN-1665).
 *
 * The generator degrades rather than throws: a reply it cannot read still yields
 * the deterministic S/M/L size fills plus the continue-until-stopped option, so the
 * page always has something to offer. That fallback is *indistinguishable from
 * success* on the wire — same 200, same four cards, same "generated with <model>"
 * caption — which is the defect this reports. `extractJsonObject` parses the
 * document all-or-nothing, so one truncated tail discards every complete option
 * that preceded it and `parseNextRunResponse` returns zero options; the fills that
 * follow carry no `direction`, so `resolveDirections` returns `[]` and the chooser
 * disappears. Exactly the reported symptom: ~4 flat cards, no chips.
 *
 * Zero parsed options therefore means the reply was unusable, NOT that the model
 * had nothing to suggest — the prompt always asks for options and the context is
 * never empty. `finishReason: 'length'` distinguishes the overwhelmingly common
 * cause (the reply cut off against the token cap) from a reply that arrived whole
 * but unreadable. Pure; never throws. Mirrors assessEditorOutcome (LIN-1185), the
 * same seam on the Ship's Biscuit editor.
 *
 * @param {Array} parsedOptions - parseNextRunResponse(...).options, BEFORE the size fills.
 ...
 */
export function assessNextRunOutcome(parsedOptions, finishReason) {
  if (Array.isArray(parsedOptions) && parsedOptions.length > 0) {
    return { ok: true, truncated: false, reason: null };
  }
 ...
 * @param {number} [options.maxTokens=NEXT_RUN_PROSE_MAX_TOKENS] - Bare `max_tokens` cap for the
 *   reply (LIN-1665). No `reasoning` field is sent — live evidence shows it switches reasoning
 *   ON in gpt-5.4-mini (which otherwise spends zero) and the cap is not honoured.
 * @returns {Promise<{analysis: string, directions: Array, options: Array, model: string, summary: string, context: string, degraded: ({reason: string, truncated: boolean, finishReason: string|null}|null)}>}
 ...
 *   `degraded` is null when the reply parsed to at least one option and non-null when it did
 *   not (LIN-1665) — the options in that case are ENTIRELY deterministic fills, which the page
 *   must say out loud rather than caption as a normal generation; see assessNextRunOutcome.
 *   `directions` is the resolved grouping over those options (LIN-1566) — `[]` when the reply
 *   carried no usable grouping, in which case the page renders the flat list exactly as before;
 *   see resolveDirections for the partition/exclusion invariants the client relies on.
...
export async function generateGoalSuggestions({ projects = [], issues = [], organizationName = '', roadmapReport = null, northStar = '' } = {}, options = {}) {
  ...
  // Bare `max_tokens` cap — no `reasoning` field is sent. Live evidence (LIN-1665
  // re-run): sending `reasoning: {max_tokens: N}` to gpt-5.4-mini switches reasoning
  // ON in a model that otherwise spends zero, and OpenRouter does NOT honour N as a
  // cap — the run expands to consume the entire budget and returns zero characters.
  // Raising the bare cap instead fixes the failing model (gpt-5.5: 3/3 grouped at
  // 4000 vs 0/2 at 1600) and leaves the default path measurably unchanged (0
  // reasoning tokens spent, same request shape bar one integer).
  const maxTokens = options.maxTokens || NEXT_RUN_PROSE_MAX_TOKENS;

  let buffer = '';
  let finishReason = null;
  await streamChat(
    messages,
    { apiKey: options.apiKey, model, maxTokens, temperature: 0.4,
  ...
  const { analysis, directions: declaredDirections, options: parsed } = parseNextRunResponse(buffer);
  // Whether the reply was readable at all — assessed on the PARSED options, before
  // the deterministic fills below make the list look populated again (LIN-1665).
  const outcome = assessNextRunOutcome(parsed, finishReason);
  ...
  // Grouping is resolved LAST, over the final list (LIN-1566): the deterministic
  // size fills above carry no `direction` of their own, so they only become
  // groupable — into the catch-all — once they exist. `[]` here means "no usable
  // grouping", which the page renders as today's flat list.
  const directions = resolveDirections(finalOptions, declaredDirections);
  if (!outcome.ok) {
    console.error(
      `Next-run generation produced no usable options for ${options.urlKey || '(unknown workspace)'}: ` +
      `${outcome.reason} (model=${model}, finishReason=${finishReason || 'unknown'}, chars=${buffer.length}, ` +
      `maxTokens=${maxTokens}) — ` +
      'serving deterministic fills, flagged degraded.'
    );
  }
  // `degraded` is the wire signal for "these are fallback options, not generated
  // ones" (LIN-1665): null on the normal path, so a successful generation's shape is
  // unchanged and an older client that ignores the field behaves exactly as today.
  return {
    analysis,
    directions,
    ...
    summary,
    context,
    degraded: outcome.ok ? null : { reason: outcome.reason, truncated: outcome.truncated, finishReason },
  };
}
