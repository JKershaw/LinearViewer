...
- "size" must be exactly one of S, M, L — never XL. Size by ballpark scope:
  - S = an individual task or two.
  - M = a string of tasks, or a large task likely to become an epic, or an epic.
  - L = a more ambitious target spanning several tasks or epics.
 ...
 * @returns {Promise<{options: Array, model: string, summary: string, context: string}>} options
 *   always ends with the continue-until-stopped option (empty goal); an LLM failure still
 *   yields that single guaranteed option rather than an error. `summary` is a short
 *   deterministic intro paragraph (LIN-638). `context` is the exact deterministic grounding
 *   blob the model was given, returned verbatim so the page can show the user what the
 *   suggestions were grounded in (LIN-633).
 */
export async function generateGoalSuggestions({ projects = [], issues = [], organizationName = '' } = {}, options = {}) {
  ...
  const context = formatNextRunContext(roadmapModel, organizationName);
  ...
  // lead.
  return { options: [...generated, { ...CONTINUE_UNTIL_STOPPED_OPTION }], model, summary, context };
}
