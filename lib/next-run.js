 ...
 * @param {number} [options.maxTokens=1600]
 * @returns {Promise<{analysis: string, directions: Array, options: Array, model: string, summary: string, context: string}>}
...
export async function generateGoalSuggestions({ projects = [], issues = [], organizationName = '', roadmapReport = null, northStar = '' } = {}, options = {}) {
  ...
  await streamChat(
    messages,
    // 1600, not 1400 (LIN-1566): the directions block adds ~80-150 tokens and a
    // truncated reply does NOT degrade gracefully — extractJsonObject returns null
    // and the whole generation collapses to deterministic size fills.
    { apiKey: options.apiKey, model, maxTokens: options.maxTokens || 1600, temperature: 0.4,
  ...
  return { analysis, directions, options: finalOptions, model, summary, context };
}
