...
export class ProxyTokenStore {
  ...
  async cleanup() {
    try {
      ...
      let idleDeleted = 0;
      const idleCutoff = new Date(now.getTime() - this.idlePruneSeconds * 1000);
      const idleCandidates = await this.collection.find({
        createdAt: { $lt: idleCutoff }
      }).toArray();
      const idleIds = idleCandidates
        .filter(doc => {
          const last = doc.lastUsedAt ? new Date(doc.lastUsedAt).getTime() : null;
          return last === null || last < idleCutoff.getTime();
        })
        .map(doc => doc._id);
      if (idleIds.length) {
        const idleResult = await this.collection.deleteMany({
          _id: { $in: idleIds }
        });
        idleDeleted = idleResult.deletedCount || 0;
      }
