  ...
  async listTokens(urlKey) {
    ...
    try {
      ...
      return docs.map(doc => ({
        ...
        expiresAt: doc.expiresAt?.toISOString?.() || doc.expiresAt,
        // LIN-1586: a VERDICT, never the owning account id — mirrors
        // DispatchTokenStore.listTokens (lib/dispatch-tokens.js). An ownerless
        // proxy token cannot resolve a workspace, so "which of my tokens are
        // ownerless?" is the question an operator must be able to answer.
