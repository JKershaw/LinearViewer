...
export class ProxyTokenStore {
   ...
   * @throws {Error} on an invalid kind/scope, and — LIN-1582 — on a `kind: 'bootstrap'`
   *   mint with no `createdBy` while `DISPATCH_OWNERLESS_BROKER_COMPAT` is off. That
   *   refusal is the structural guarantee behind the switch: it applies to every
   *   bootstrap mint site, present and future. Nothing is inserted when it fires.
   */
   ...
   * @param {string} [options.label='exchanged'] - Label for the minted working token
