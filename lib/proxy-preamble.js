 ...
 *
 * PROVIDER IDENTITY (LIN-2354): `providerDisplayName` names the backing provider
 * in the "currently backed by X" clause. Omit it (or pass `null`) when the
 * caller has not resolved a DECLARED provider identity — the clause is then
 * dropped entirely rather than guessing. Callers must never pass a value that
 * merely fell back to a legacy default (`getProviderForWorkspace`'s Linear
 * fallback, or `req.resolvedProvider.displayName` read without checking
 * `.declared`) — that reproduces the exact defect this param exists to fix.
 *
 * @param {Object} params
 ...
 * @param {string|null} [params.providerDisplayName=null] - The resolved provider's
 *   declared display name (e.g. "Linear", "GitHub Issues"). `null`/omitted ⇒ the
 *   "currently backed by X" clause is dropped, not hedged.
 * @param {Object|null} [params.providerUi=null] - The resolved provider's `ui`
 *   capability summary (LIN-2804). Gates ONLY the endpoint-hint `contextLines` below
 *   on `providerUi.issueDetail`/`.relations`/`.search` — swapping the `/issues/{id}`/
 *   `/relations` hint for a brief+search alternative when unsupported, and further
 *   dropping the `/search` mention (review Finding 1) when the provider supports
 *   neither `issueDetail` nor `search` (e.g. GitHub Projects, Jira), leaving a
 *   brief-only fallback rather than trading one 422 hint for another.
 *   `exchangeLines` (the credential-delivery half) is never touched by this param —
 *   see the note above the `contextLines` construction. `providerUi?.issueDetail !==
 *   false` (not a truthy check) is what makes omitted/null default to
 *   full-capability, so every existing caller that passes no `providerUi` stays
 *   byte-identical.
 * @returns {string} Block to append to the prompt
 */
export function buildProxyContextPreamble({ baseUrl, token, issueIdentifier, tokenDelivery = 'prose', providerDisplayName = null, providerUi = null }) {
  ...
  // LIN-2804: gate the endpoint-hint text (only) on the resolved provider's real
  // read support, so a GitHub/GitHub-Projects/Jira workspace is never pointed at a
  // `/issues/{id}`/`/relations` read that 422s (`CAPABILITY_NOT_SUPPORTED`).
  // `!== false` (not a truthy check) is what makes omitted/null `providerUi` default
  // to full-capability, matching every other caller's byte-identical expectation.
  const issueDetailSupported = providerUi?.issueDetail !== false;
  const relationsSupported = providerUi?.relations !== false;
  ...
  // issueDetail-unsupported fallback must not assume `/search` is a safe landing —
  // GitHub Projects and Jira support neither. `!== false` for the same
  // omitted/null-defaults-to-full-capability reason as the other two flags.
    ...
    ? (issueDetailSupported
        ? [
            ...
            relationsSupported
              ...
              : `and update the workspace as you work (status, comments, labels).`
          ]
        : (searchSupported
            ? [
                `Start from the distilled brief: GET ${apiBase}/api/proxy/brief/${issueIdentifier}`,
                `(present-state — folds in comments, supersedes stale wording; this provider does not`,
                `support raw issue-detail reads, so the brief is the fullest detail available). Use GET`,
                `${apiBase}/api/proxy/search?q=… to find related tasks by text, and update the workspace`,
                `as you work (status, comments, labels).`
              ]
            : [
                `Start from the distilled brief: GET ${apiBase}/api/proxy/brief/${issueIdentifier}`,
                `(present-state — folds in comments, supersedes stale wording; this provider does not`,
                `support raw issue-detail reads, so the brief is the fullest detail available), and`,
                `update the workspace as you work (status, comments, labels).`
              ]))
    : (issueDetailSupported
        ? (searchSupported
            ? [
                ...
                `/issues/{id}) and to update the workspace as you work (status, comments, labels).`
              ]
            : [
                `Once you pick a task, start from its distilled brief (GET ${apiBase}/api/proxy/brief/{id}).`,
                `Use the proxy to pull context (e.g. GET ${apiBase}/api/proxy/stack, /issues/{id})`,
                `and to update the workspace as you work (status, comments, labels).`
              ])
        : (searchSupported
            ? [
                `Once you pick a task, start from its distilled brief (GET ${apiBase}/api/proxy/brief/{id}).`,
            ...
            : [
                `Once you pick a task, start from its distilled brief (GET ${apiBase}/api/proxy/brief/{id}).`,
                `Use the proxy to pull context (e.g. GET ${apiBase}/api/proxy/stack) and to update`,
                `the workspace as you work (status, comments, labels).`
              ]));
  ...
  const exchangeLines = tokenDelivery === 'mcp'
    ? [
        ...
        // Write-verb opt-in (LIN-1948, fix 1). MCP BRANCH ONLY — the header is
        // read by the local broker, which no prose-harness worker goes through,
        // so stating it in the prose branch would be actively misleading. A
        // guarded verb without it comes back 403 with the remedy in the body,
        // which is a live refusal and not a dead credential (LIN-1985).
        `Reads (GET) need nothing extra. For a WRITE (POST/PATCH/DELETE) add the header`,
        `\`X-Harbour-Intent: write\` — the local proxy refuses write verbs without it with a`,
        `403 naming this remedy, so a write is deliberate rather than accidentally armed.`,
  ...
  return [
    ...
    '',
    `You have a workspace API proxy for this workspace (source-neutral${providerDisplayName ? `; currently backed by ${providerDisplayName}` : ''}). Base: ${apiBase}/api/proxy`,
    '',
    ...exchangeLines,
...
export async function provisionBootstrapToken({
  ...
  //
  // The guarantee is now STRUCTURAL, not a per-site convention (LIN-1582). This
  // function is one of several bootstrap mint sites, and the ones that do not
  // route through it — `routes/collective.js`'s prose-harness branch, the
  // session-auth `POST .../api/proxy/tokens`, and the test-only
  // `/test/create-proxy-token` — used to call `createToken({ kind: 'bootstrap' })`
  // directly and escape the switch entirely. LIN-1582 closed that two ways:
  // `ProxyTokenStore.createToken` itself now refuses an ownerless bootstrap when
  // the lane is off (so a future direct mint cannot reopen the gap by forgetting
  // a gate), and the two production sites additionally refuse at the call site so
  // the policy surfaces as a 503 / a dropped best-effort block rather than a raw
  // store throw. With the lane off, ownerless inheritance is therefore impossible,
  // not merely unused.
  //
  ...
  // Compat off: refuse, so a dispatched session can no longer inherit an
  // ownerless token. Either way an OWNER-STAMPED mint is untouched and silent.
  //
  // This refusal stays here even though the store now backstops it (LIN-1582):
  // refusing before the call is what lets each site report the policy in its own
  // idiom — this one's `missing()` fails closed for claude-code and degrades to
  // null for a prose harness — instead of every caller having to catch and
  // reinterpret a store throw. The store's guard therefore only ever fires for a
  // site nobody has gated, so the two never double-log.
  if (!createdBy) {
 ...
 * @param {string|null} [params.providerDisplayName=null] - The resolved provider's declared
 *   display name (LIN-2354), threaded straight through to {@link buildProxyContextPreamble}.
 *   Omit/`null` when the caller has not resolved a declared identity — never pass a
 *   legacy-fallback name.
 * @param {Object|null} [params.providerUi=null] - The resolved provider's `ui` capability
 *   summary (LIN-2804), threaded straight through to {@link buildProxyContextPreamble}.
 *   Omit/`null` when the caller has not resolved one — the endpoint hints then default to
 *   full-capability, the historical behavior.
