 ...
 *   on `providerUi.issueDetail`/`.relations` — swapping the `/issues/{id}`/`/relations`
 *   hint for a brief+search alternative when unsupported. `exchangeLines` (the
 *   credential-delivery half) is never touched by this param — see the note above the
 *   `contextLines` construction. `providerUi?.issueDetail !== false` (not a truthy
 *   check) is what makes omitted/null default to full-capability, so every existing
 *   caller that passes no `providerUi` stays byte-identical.
