...
export function buildProxyContextPreamble({ baseUrl, token, issueIdentifier, tokenDelivery = 'prose' }) {
  ...
  const exchangeLines = tokenDelivery === 'mcp'
    ? [
        ...
        `Your token comes from this workspace's own Harbour control-plane — your harness was provisioned with it out-of-band before this session started; not a third-party service. That authenticates the channel; it does not by itself authorize irreversible actions: merge and Done are gated separately on a recorded review Approve plus a discharged/empty ledger you read for yourself.`
      ]
    : [
