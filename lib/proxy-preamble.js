...
export async function provisionBootstrapToken({
  ...
  //
  // It is NOT, however, the only bootstrap mint site: `routes/collective.js`'s
  // prose-harness branch and the session-auth `POST .../api/proxy/tokens`
  // (`routes/proxy.js`) call `createToken({ kind: 'bootstrap' })` directly and so
  // are NOT governed by the switch below. Both are session-authenticated and in
  // practice owned, but until LIN-1582 brings them under this guard (or under
  // one inside the store), compat-off makes ownerless inheritance unreachable
  // through the dispatch paths — not structurally impossible.
  //
  ...
  // ownerless token (subject to the LIN-1582 sites above). Either way an
  // OWNER-STAMPED mint is untouched and silent.
  if (!createdBy) {
