...
GET ${baseUrl}/api/proxy/rulings
  ...
  → Query params (both additive; the default read with neither is
    ...
    non-root carrier's "anchor" now names the root instead of itself):
      ...
      "includeResolved=true" — also return decisions this endpoint would
        ...
        and exactly the row an agent must be able to see to avoid re-raising
        an already-answered finding. Covers LOOP rulings only: an ANSWERED
        task-bound ruling (anchor.taskDecisionId set) never appears under
        this — there is no answered-task-decision read on this endpoint
        today; OPEN task-bound rulings are returned exactly as in the
        default read.
  ...
  → "resolution" appears ONLY when "includeResolved=true" surfaced an
    ...
    "answered, outcome unknown", not as a malformed row.
