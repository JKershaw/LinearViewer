...
GET ${baseUrl}/api/proxy/rulings
  ...
  → "effect" is a SEPARATE field from "disposition" — never merge the two
    ...
    no run). It is null only for the two read-only dispositions
    ("mid-turn"/"indeterminate") — a row that cannot act never claims an
    effect. "declaredEffect" is what the runner's own DECISION: block
    requested via an optional on_answer: { "effect": "..." } field, when
    present — null when nothing was declared. "effect" itself may differ
    from "declaredEffect": live evidence (an already-resumable disposition,
    or a live run already on the same anchor issue) always overrides a
    declaration. "alternate" names the untaken option ONLY when a genuine
    declared-vs-default choice existed and evidence did not force the
    outcome; otherwise null.
