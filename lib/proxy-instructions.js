...
GET ${baseUrl}/api/proxy/rulings
  ...
  → Query params (both additive; the default read with neither is
    ...
    non-root carrier's "anchor" now names the root instead of itself):
      ...
      "includeResolved=true" — also return decisions this endpoint would
        otherwise hide because they are already answered, each carrying a
        "resolution" field (see below), even when the decision's own
        ...
        to see to avoid re-raising an already-answered finding. An OPEN
  ...
  → "resolution" appears ONLY when "includeResolved=true" surfaced an
    already-answered row: { "decisionId": "...", "raisedAt": "..." | null,
    "resolvedAt": "..." | null, "outcome": "answered" | "dismissed" | null }.
    Absent (not null) on every row from the default read, and never present
    on a task-bound row (see the query-param note above). "outcome": null is
    legitimate, not a bug: a row synthesized from a pre-answered-set-redesign
    ("legacy") digest carries only the old scalar answer marker, which never
    recorded which of "answered"/"dismissed" it was — treat null as
    "resolved (answered or dismissed), outcome unknown", not as a malformed
    row.
