...
GET ${baseUrl}/api/proxy/known-repos (LIN-2974)
  → Read-only exposure of this workspace's own known-repos inventory — the
    exact same "repo=" project-line computation the REPO VALIDATION check
    below runs before enqueueing a dispatch, reachable without tripping its
    422. No "repo" input, no write, no dispatch item created, no interaction
    with the duplicate-dispatch guard. A plain read-scoped token is enough —
    this route needs no write scope.
  → { "knownRepos": ["LinearViewer", "simple-dispatcher"] }
  → UNAVAILABLE (the provider can't answer — no provider, unsupported, a
    throwing/timing-out fetch): a distinct non-200 503, never a fail-open
    "knownRepos": []. A silent empty list here would read as "this workspace
    has no repos" instead of "could not check" — this endpoint refuses that
    conflation on purpose (unlike the dispatch-time guard below, which fails
    OPEN because it has a real dispatch to protect and this endpoint does not).
    { "error": "Could not determine this workspace's known-repos inventory",
      "code": "REPO_INVENTORY_UNAVAILABLE", "reason": "no-provider"|"unsupported"|"fetch-failed" }
  → ACCEPTED HERE ≠ RESOLVABLE BY THE CONSUMER (LIN-2974): this is the
    TRACKER's namespace, the same one the "repo" dispatch field validates
    against — see the REPO VALIDATION note below for why a value here can
    still be refused by the consumer's own, separate admission check.
...
GET ${baseUrl}/api/proxy/labels?teamId={teamId}
  → Labels (id, name, color, team); optional team filter. A team-scoped request
    returns that team's labels PLUS workspace-level labels (team: null), because a
    workspace-level label applies to an issue on any team (LIN-2787) — a scoped
    catalog is never a silent subset that hides type labels like Bug/Feature/Improvement.
    Caveat: the \`team\` key and inclusive \`?teamId=\` are the Linear-backed shape; on
    GitHub/Jira/Local-backed workspaces the catalog is flat ({ id, name, color }, no
    \`team\`) and \`teamId\` is ignored.
  → { "labels": [{ "id": "...", "name": "bug", "color": "#f00", "team": { "id": "...", "name": "Engineering" } }, { "id": "...", "name": "Improvement", "color": "#0f0", "team": null }] }
...
POST ${baseUrl}/api/proxy/flight-companion/turn
  → Drive one Flight Companion turn — the same companion the browser's Flight
    Companion chat page runs, over the proxy: an agent can check in on how it's
    performing and improve it, closing the loop between what an agent proposes and
    what a human sees. Read scope is sufficient (a proposal is not a write); billed
    to the TOKEN CREATOR's own OpenRouter key (same credential rule as recommend/
    recap/brief), never the caller's own bearer token.
  → Body: { "message"?: "...", "history"?: [{ "role": "user"|"assistant", "content": "..." }],
      "stream"?: false }
  → Omit "message" (or send only whitespace) for a message-less, auto-wake-shaped
    check-in: it runs the SAME census gate the browser's silent tick does, but against
    its OWN reservation instance — it can never advance the browser's own instance, so
    polling this endpoint can never consume a report a human has not seen yet.
  → ALWAYS propose mode, regardless of "message" — a proxy caller is an agent, not a
    human, so a model-proposed follow-up (the "send_follow_up" tool) comes back in
    "proposals" for a human to act on later; it is never enqueued directly. Approving
    one is still done by a human, over the session-authed page.
  → Default (no "stream", or "stream": false) returns ONE JSON body once the turn
    completes:
      { "turnKind": "user-initiated" | "auto-wake", "spent": true, "reason"?: "...",
        "text": "...", "tools": [{ "name": "...", "arguments": {...}, "phase": "result" | "error" | "proposed", "result"?: ..., "error"?: "..." }],
        "proposals": [{ "proposed": true, "sessionId": "...", "prompt": "..." }],
        "usage": { "prompt_tokens": 147, "completion_tokens": 33, "total_tokens": 180, "cost": 0.0004 } | null,
        "model": "openai/gpt-5.4-mini" | null }
    "spent": false means the gate/quota refused before any model call — "text"/"tools"/
    "proposals" are empty, "usage"/"model" are null, and "reason" names why (e.g.
    "no-census", "hash-identical", "lost-race", "free-tier", "not-configured").
  → "stream": true instead yields the identical Server-Sent-Events frames the browser's
    chat gets (same "token"/"tool"/"done"/"error" event types, same "call"/"result"/
    "error"/"proposed"/"cap" tool phases) rather than one aggregated body — useful for
    watching a turn unfold rather than only its outcome.
  → Rate-limited independently of the general 60/min proxy limiter: 30 turns/hour per
    token, 200 turns per workspace per day. A 429 here means back off this endpoint
    specifically, not that the token itself is unhealthy.

GET ${baseUrl}/api/proxy/flight-companion/transcripts
  → The TOKEN CREATOR's own saved Flight Companion transcripts — the other half of the
    feedback loop /turn opens: read what the companion told a human, not just talk to
    it. Read scope is sufficient; no write path. Pure Harbour-local read, no Linear
    fetch, no LLM call.
  → { "chats": [{ "id": "...", "taskIdentifier": "flight-companion", "title": "...",
      "transcript": [{ "role": "user"|"assistant", "content": "..." }],
      "createdAt": "...", "updatedAt": "..." }] }, newest-first, full transcripts.
  → Identity is the token creator's account, never a session — a creator-less/
    ownerless token gets an empty list, always. 503 when the saved-chat store isn't
    configured on this deployment; 500 (never a bare empty list) on a read failure.
  → No shipped code emits the sentinel today, so this reads empty on a fresh
    deployment until a Save control exists on the Flight Companion page.
...
GET ${baseUrl}/api/proxy/north-star
  ...
  → "docVersion" (LIN-2254) makes the "northStar" value's freshness
    ...
    is only ever recorded when the paste matches that doc after normalising
    whitespace on both sides (CRLF folded to LF, trailing whitespace and
    trailing blank lines stripped — LIN-2838). "drift" is
...
GET ${baseUrl}/api/proxy/rulings
  → Every UNANSWERED operator decision in this workspace — the same set the
    Observation "Rulings" tab shows a human, scoped to your token's workspace
    (never a merged cross-workspace view).
  → Query params (both additive; the default read with neither is
    set-preserving relative to before these existed — no row added or
    removed, though every row now also carries "stampLoopId" and a
    non-root carrier's "anchor" now names the root instead of itself):
      "issueIdentifier" — filter the returned rows to one anchor issue,
        applied AFTER grouping (never a pre-filter): a decision raised on
        one issue but stamped-answered on a differently-tagged sibling loop
        in the same lineage still discharges correctly, and still counts
        toward "includeResolved" below, before this filter narrows the list.
      "includeResolved=true" — also return decisions this endpoint would
        otherwise hide because they are already resolved (answered or
        withdrawn), each carrying a "resolution" field (see below), even when
        the decision's own
        content loop has since been superseded by a later reply (e.g. a
        root-raised, root-answered decision that a resumed session then
        replied to) — the common shape a re-visited session leaves behind
        — or when the decision is still under an active shelf (an operator
        parked it, and it was then answered before the shelf's timer
        lapsed): either way, this is exactly the row an agent must be able
        to see to avoid re-raising an already-resolved finding. An OPEN
        (unanswered) shelved decision is still excluded, with or without
        this flag. Covers LOOP rulings only: an ANSWERED task-bound ruling
        (anchor.taskDecisionId set) never appears under this — there is no
        answered-task-decision read on this endpoint today; OPEN task-bound
        rulings are returned exactly as in the default read.
  → { "count": 2, "rulings": [{ "decision": { "decision_id": "...", "question": "...",
        "options": [{ "id": "a", "label": "..." }] },
        "anchor": { "issueId": "<raw uuid>", "issueIdentifier": "LIN-42",
          "workspaceUrlKey": "...", "loopId": "...", "taskDecisionId": "..." },
        "stampLoopId": "...",
        "disposition": "resumable|gone|mid-turn|indeterminate|task-bound",
        "effect": "resume|dispatch|record" | null, "declaredEffect": "resume|dispatch|record" | null,
        "alternate": "resume|dispatch|record" | null,
        "canReply": true, "shelvedLapseCount": 0 | null,
        "suggestedDismissal": null }], "generatedAt": "..." }
  → "stampLoopId" is the loop an answer stamp is written to; "anchor.loopId"
    is the loop a reply/comment is posted to. They differ whenever the
    decision was raised on a non-root turn of a session (most decisions
    after a session's first) — always use "stampLoopId ?? anchor.loopId"
    when addressing an answer, never "anchor.loopId" alone. A single row can
    also stand for MORE than one raise: duplicate re-asks of the same
    decision within one session/lineage (e.g. a repeated wake) collapse
    into this one row, anchored on the lineage's root.
  → "resolution" appears ONLY when "includeResolved=true" surfaced an
    already-resolved row: { "decisionId": "...", "raisedAt": "..." | null,
    "resolvedAt": "..." | null, "outcome": "answered" | "dismissed" | "withdrawn" | null,
    "reason": "..." | null }.
    Absent (not null) on every row from the default read, and never present
    on a task-bound row (see the query-param note above). "reason" carries
    the withdrawing agent's stated reason on a "withdrawn" row — the one
    outcome that populates it; it is absent (not null) on "answered"/"dismissed" rows and
    never null on a "withdrawn" row by construction, since the write path
    fails closed on an empty reason. A "withdrawn" row's "resolvedAt" is the
    withdrawal's own timestamp (its "raisedAt" is null: a withdrawal stamp
    records no raise). "outcome": null is legitimate, not a bug: a row
    synthesized from a pre-answered-set-redesign ("legacy") digest carries
    only the old scalar answer marker, which never recorded which of
    "answered"/"dismissed" it was — treat null as "resolved (answered or
    dismissed), outcome unknown", not as a malformed row.
  → "shelvedLapseCount" is a small integer (0 = never shelved) counting past
    shelve-then-lapse cycles, EXCEPT it reads null on a row carrying
    "resolution" whose decision is still under an active shelf right now —
    that specific combination (answered, actively shelved) only ever appears
    under "includeResolved=true"; every other row's shelf has either never
    been active or has already lapsed by the time it is returned.
  → "suggestedDismissal" is the standing proposal on that ruling, if any:
    { "reason": "...", "suggestedBy": "...", "suggestedAt": "...",
      "decisionLoopId": "..." | null, "proposedOutcome": "dismissed" | "answered",
      "optionId": "..." | null }. It is a SUGGESTION a human has not yet acted
    on — never evidence the ruling is resolved. A ruling listed here is
    unanswered, full stop. The field name is a legacy holdover — it now covers
    BOTH proposal kinds — read "proposedOutcome" to tell them apart:
    "dismissed" means propose that the ruling be dismissed, "answered" means
    propose that it be answered with "optionId". "optionId" is present only
    when "proposedOutcome" is "answered". A record from before this field
    existed reads with "proposedOutcome": "dismissed" and "optionId": null —
    byte-compatible with what it always meant. Check this before proposing
    over an existing standing suggestion: re-proposing overwrites it in place.
  → "effect" is a SEPARATE field from "disposition" — never merge the two
    namespaces. "disposition" governs whether/how a reply can be delivered;
    "effect" is what happens once it is: "resume" (continue the parked
    session), "dispatch" (start a fresh run), or "record" (log the answer,
    no run). "effect" is null only when no live run exists on the anchor
    issue. A read-only disposition ("mid-turn"/"indeterminate") never
    surfaces a DECLARED effect — but live evidence on the anchor, including
    the read-only row's own loop (non-terminal by definition of
    "mid-turn"/"indeterminate"), still resolves "effect" to "record". So a
    read-only row never surfaces its declared effect IN "effect" —
    "declaredEffect" is NOT guaranteed null on such a row; it still reports
    whatever the block declared, if anything. "declaredEffect" is what the
    runner's own DECISION: block requested via an optional on_answer:
    { "effect": "..." } field, when present — null when nothing was
    declared. "effect" itself may differ from "declaredEffect": live
    evidence (an already-resumable disposition, or a live run already on
    the same anchor issue) always overrides a declaration. "alternate"
    names the untaken option ONLY when a genuine declared-vs-default choice
    existed and evidence did not force the outcome; otherwise null.

POST ${baseUrl}/api/proxy/rulings/{decisionId}/suggest-dismissal   (readWrite)
  Body: { "reason": "why this ruling can go", "decisionLoopId": "..." }
    "reason"         — required, non-empty, <= 500 chars
    "decisionLoopId" — OPTIONAL but strongly preferred; when given, a non-empty
                       string. Send the target row's own
                       "anchor.loopId ?? anchor.taskDecisionId" from GET
                       /api/proxy/rulings.
  → PROPOSE that a ruling be dismissed. This does NOT dismiss it, and there is
    deliberately no endpoint that does: only a human discharges a ruling, by
    pressing "dismiss as proposed" on your suggestion in the UI. Returns 201:
  → { "success": true, "suggestion": { "reason": "...", "suggestedBy": "...",
        "suggestedAt": "...", "withdrawn": false, "decisionLoopId": "..." | null },
      "note": "Recorded as a SUGGESTION only. The ruling is still unanswered until a human dismisses or answers it." }
  → A reason is mandatory because the operator's whole interaction is pressing
    "dismiss as proposed" — an unjustified proposal turns that press into a
    rubber stamp. Attribution is taken from your token, not from the body.
  → LIN-2756: a decision_id is NOT unique — one session can emit the same
    DECISION: block from two loops, so two rows can share it. Omitting
    "decisionLoopId" writes the legacy, workspace-wide shape, which is
    deliberately back-compatible and applies your proposal to EVERY loop
    carrying that decision_id. Send the field to address exactly the one row
    you mean.
  → 404 if {decisionId} names no unanswered ruling in this workspace (a typo,
    or one already answered), and likewise if "decisionLoopId" is given but no
    unanswered row carries that decisionId AND that loop. 400 if
    "decisionLoopId" is present but not a non-empty string. 403 without a
    readWrite token.

POST ${baseUrl}/api/proxy/rulings/{decisionId}/suggest-answer   (readWrite)
  Body: { "optionId": "a", "reason": "why this is the right answer", "decisionLoopId": "..." }
    "optionId"       — required, non-empty string; must name one of the
                       decision's own "options[].id" (from GET /api/proxy/rulings)
    "reason"         — required, non-empty, <= 500 chars
    "decisionLoopId" — OPTIONAL but strongly preferred, same convention as
                       suggest-dismissal above.
  → PROPOSE that a ruling be ANSWERED with the given option. This does NOT
    answer it, and there is deliberately no endpoint that does: only a human
    discharges a ruling, by pressing "answer as proposed" on your suggestion
    in the UI. Returns 201:
  → { "success": true, "suggestion": { "reason": "...", "suggestedBy": "...",
        "suggestedAt": "...", "withdrawn": false, "decisionLoopId": "..." | null,
        "proposedOutcome": "answered", "optionId": "a" },
      "note": "Recorded as a SUGGESTION only. The ruling is still unanswered until a human dismisses or answers it." }
  → A reason is mandatory for the same reason it is on suggest-dismissal: the
    operator's whole interaction is pressing "answer as proposed", so an
    unjustified proposal turns that press into a rubber stamp. Attribution is
    taken from your token, not from the body.
  → 400 if "optionId" is missing, blank, or not a string. 400 if "reason" is
    missing/blank/over 500 chars, or if "decisionLoopId" is present but not a
    non-empty string. 422 if "optionId" does not name one of the decision's own
    "options[].id" — including when the decision is free-text and carries no
    "options[]" at all: this route REFUSES an unanswerable proposal rather than
    silently recording an answer to nothing. 404 under the same conditions as
    suggest-dismissal (unknown decisionId, or an unmatched decisionLoopId). 403
    without a readWrite token.
  → This route stays disposition-agnostic: it never checks the ruling's
    current disposition/canReply/effect, because those are resolved fresh at
    press time, not at propose time. Nothing here ever writes "agreed",
    "agreedAt", or "acceptedAt" — those are stamps only the human's own
    dismiss/answer press writes.
...
GET ${baseUrl}/api/proxy/dispatch/halt
  → Read the workspace's operator halt (LIN-2994 Decision 4 — the degraded-mode operator path: proxy-token auth, no Linear, no heavy render path in front of it). This stores a request; the runner does not yet honor it (pending LIN-2995) — never read a set halt as meaning the runner is actually paused or stopped.
  → { "halt": null } when nothing is requested, else { "halt": { "mode": "pause"|"stop", "setAt": "...", "setBy": "..." | null } }. "setBy" is the creator recorded on the token that made the request — null for a legacy token with no recorded creator.
...
POST ${baseUrl}/api/proxy/dispatch
  ...
  → "maxTasks" (optional integer >= 1; LIN-2975) declares THIS dispatch as a budgeted run; workers must carry this dispatch's "id" as their own "sessionId" for the bound to apply — see the BUDGET GUARD note on the kickoff verb below, which applies identically here. Rejected 400 "maxTasks must be an integer >= 1" on anything else (non-integer, < 1, or a numeric string). Omit or pass "null" for an unbounded run, byte-identical to today. Echoed on the 201 body below.
  ...
  → { "id": "...", "status": "queued", "promptName": "...", "kind": "implementation", "issueIdentifier": "...", "target": "cli", "abort": false, "abortTo": null, "cascade": false, "sessionId": null, "maxTasks": null, "dispatchedAt": "...", "consumerLastSeenAt": "..." } (a "cascade": true request instead returns { "success": true, "cascade": true, "closed": [...], "count": N })
  → CONSUMER POLL WARNING (LIN-2885) — "consumerLastSeenAt" is the most recent lastUsedAt across the workspace's non-revoked consumer tokens, stamped once at enqueue; null means no consumer token has EVER polled/taken here ("never", not merely "not recently"). When that stamp is null or older than the staleness threshold (default 1h, configurable via CONSUMER_POLL_WARNING_THRESHOLD_MS), the response ALSO carries a top-level "warning" string naming the last poll time or "never". This does NOT refuse the dispatch — it still enqueues; a runner may come up later. The same stamp (plus a "consumerPollWarning" re-derived against the current clock) is readable later via GET /dispatch/{id} and GET /dispatch (list) below.
  → REPO VALIDATION (LIN-2886) — "repo" (when supplied) is validated + normalized against the workspace's known repos (the repo= lines on its Linear projects, plus the default) BEFORE enqueueing. Accepted forms: a matching basename, a full URL, an SCP-like git remote, or "owner/name" — all normalized to the basename. Case-sensitive. An unrecognized value is refused 422: { "error": "Unknown repo \"...\"", "code": "UNKNOWN_REPO", "knownRepos": ["LinearViewer", "simple-dispatcher"] }. UNLIKE the poll-recency warning above, this ONE DOES refuse — it is not merely a warning. Fails OPEN (skips the check, forwards "repo" unvalidated) when the workspace's provider can't answer at all, so this is never a reason to retry against a provider outage. ACCEPTED HERE ≠ RESOLVABLE BY THE RUNNER (LIN-2974): "knownRepos" is the TRACKER's namespace (project repo= lines); the consumer resolves "repo" against its OWN namespace (simple-dispatcher: the folder basenames in its host's workspaces.json), and nothing keeps the two in sync — so a value this check accepts can still return 201 and then fail terminally at the consumer's admission with "[failed] Unknown repo ...". That reject is a separate check against a different list, not a narrower fallback behind this one. Omit "repo" (null) to run in the workspace's default folder.
...
POST ${baseUrl}/api/proxy/recommend-and-dispatch
  Body: { "issueIdentifier": "LIN-42", "target": "cli|web|dash", "repo": "...", "model": "anthropic/claude-opus-4.8", "harness": "opencode", "appendProxyContext": true, "noDescend": false, "kind": "review", "sessionId": "...", "periodicalId": "...", "waitForFollowUps": false, "followUpTo": "...", "force": false }
  ...
  → "followUpTo" (optional) resumes an existing session, same meaning and same rules as on POST /dispatch — pass the "id" of an earlier dispatch and the SERVER-WRITTEN body becomes a follow-up instruction to that same session (you still pick neither the words nor, unless you override "kind", the verb). Must be a well-formed UUID; cli/web targets only (a "dash"/other target is rejected 400 "followUpTo is only supported for cli/web targets", a malformed id 400 "Invalid followUpTo format"). Threaded onto whichever arm this verb resolves — the "kind"-override arm and the recommendation-derived arm both forward it. Exempt from the duplicate guard below (a follow-up IS the intended second dispatch), and — exactly as on POST /dispatch — the proxy-context prose block is NOT appended by default for a follow-up (pass "appendProxyContext": true to force it back on); a broker-dependent harness still receives a live credential unless you pass "appendProxyContext": false, which opts out of both. The runner owns session liveness: a dead session posts terminal "[failed] no live session to resume". Use sparingly, on the same terms as POST /dispatch. See LIN-2869.
  → "force" (optional boolean, default false) is the OPERATOR RESCUE HATCH, same meaning as on POST /dispatch: this verb requires "issueIdentifier", so its dispatches are always issue-scoped and "force": true bypasses the duplicate guard below for a human recovering a wedged task who has confirmed the colliding dispatch is not doing the work. Non-boolean is rejected 400 "force must be a boolean". This is NOT the answer to a 409 you were just handed — adopt the returned "id" and watch it, as that refusal says; reaching for "force" reflexively makes the guard advisory rather than a bound. It does NOT bypass the BUDGET guard (see the kickoff verb below), and unlike POST /dispatch there is no "cascade"/bare-"force" interaction here (neither shape exists on this verb). The runner reads it as "item.force" off the polled item. See LIN-1656/LIN-2872.
  ...
  → { "id": "...", "status": "queued", "kind": "plan", "promptName": "plan", "issueIdentifier": "...", "target": "cli", "sessionId": null, "dispatchedAt": "...", "consumerLastSeenAt": "..." }
  → Carries the same "consumerLastSeenAt" stamp, and the same top-level "warning" when stale/never, as POST /dispatch above (LIN-2885) — it does not refuse the dispatch.
  → REPO VALIDATION (LIN-2886) applies here too, same shape as POST /dispatch above, against whichever repo value this call resolves to after "repoInherited" precedence: { "error": "Unknown repo \"...\"", "code": "UNKNOWN_REPO", "knownRepos": [...] }. Same caveat: passing this check does not guarantee the consumer can resolve the value.
  → The duplicate guard documented under POST /dispatch applies here too, keyed on the kind this verb RESOLVES (the recommendation's own action, or your "kind" override). Same 409 "DUPLICATE_DISPATCH" body, same response: adopt the returned "id" and watch it. Its two documented exemptions are reachable on THIS verb as well as on POST /dispatch — a "followUpTo" beat is never refused, and "force": true is the operator hatch (both above). See LIN-1656/LIN-2872.

POST ${baseUrl}/api/proxy/autopilot/kickoff
  Body: { "goal": "...", "mode": "write|readonly", "variant": "standard|stepper", "issueIdentifier": "LIN-42", "target": "cli|web|dash", "repo": "...", "appendProxyContext": true, "sessionId": "...", "subscription": "terminal-only|everything", "maxTasks": 50, "maxSessionsPerTask": 10 }
  ...
  → "goal" (optional) is free text. For a GENERAL run ("issueIdentifier" absent) it focuses the stack walk; for a SCOPED run ("issueIdentifier" present) the goal is pinned to that task and a non-empty "goal" is ALSO appended as additional human context (LIN-2818). An opaque string, accepted multi-line: max 100000 characters (UTF-16 code units), no control characters — a violation is refused with an error naming the constraint, and the received length when the length cap is the cause; an over-large multibyte body may instead be refused earlier by the JSON body-size limit (also loud). NOTE the POST cap is 100000, distinct from the GET preview twin's 1000-char cap — that preview is URL-bound and cannot carry more. Omitted or explicit "null" are both accepted as absent — a general run walks the stack under the default precedence policy, a scoped run carries the pinned task alone. See LIN-2075/LIN-2818.
  → "repo" (optional) overrides the project's "repo=" inheritance for a SCOPED run. An opaque string: max 1000 characters (UTF-16 code units), no control characters, 400 on violation naming the constraint, and the received length when the length cap is the cause. Its cap is independent of "goal"'s — "goal" was raised to 100000 by LIN-2818, "repo" stays at 1000. Omitted or explicit "null" are both accepted as absent, so the project-derived "repo=" (or none) is used instead. See LIN-2075.
  ...
  → { "id": "...", "sessionId": "...", "status": "queued", "kind": "autopilot", "promptName": "Autopilot (stack walk)", "mode": "write", "variant": "standard", "issueIdentifier": null, "target": "cli", "dispatchedAt": "...", "consumerLastSeenAt": "..." }
  → Carries the same "consumerLastSeenAt" stamp, and the same top-level "warning" when stale/never, as POST /dispatch above (LIN-2885) — it does not refuse the kickoff.
  ...
  → "maxSessionsPerTask" (optional integer >= 1, LIN-2934) is the SIBLING bound: it caps fresh WORKER-SESSION dispatches to a single task, independent of "maxTasks" — the bound a scoped one-task run actually needs, since "maxTasks" cannot bind a run whose whole job is one task. Same seam, same "sessionId" enforcement key and cooperating-orchestrator caveat as "maxTasks". Either, both, or neither may be declared. Omit for an unbounded-per-task run (today's behavior, byte-identical).
  → BUDGET GUARD — once a budgeted run's worker dispatches have touched "maxTasks" distinct tasks (by "issueIdentifier"), the first fresh worker dispatch for a NEW (would-be 51st) task is refused 409: { "error": "...", "code": "BUDGET_EXHAUSTED", "bound": "tasks", "count": 50, "maxTasks": 50, "sessionId": "<the run's id>" }. Separately, once a "maxSessionsPerTask"-budgeted run has dispatched that many fresh worker sessions to ONE task, the next fresh dispatch to THAT SAME task is refused 409: { "error": "...", "code": "BUDGET_EXHAUSTED", "bound": "sessionsPerTask", "taskDispatches": 10, "maxSessionsPerTask": 10, "sessionId": "<the run's id>" } — unlike "maxTasks", there is no exemption for a task already inside the budget, since this bound counts dispatches to that task, not distinct tasks. Either refusal is an orderly, expected finish, not a failure or an instrument breakage — wind down any other in-flight work and, on EITHER a "tasks" or a "sessionsPerTask" refusal, comment on the ticket naming the bound that was hit and the ledger (sessions used, tasks touched, priced spend). A rarer third value, "bound": "unverified" (only possible when BOTH bounds are declared and the underlying count could not be read), fails CLOSED the same way — treat it as the same orderly, expected finish and the same hand-back; it names "unverified" rather than either bound specifically because the read failed before either could be evaluated, so neither can be singled out as "the one that fired". NEVER refused: a dispatch that continues a task already inside the "maxTasks" budget (its review, its close-out, a corrective followUpTo beat), a "followUpTo" beat, an "abort", or a dispatch carrying no "issueIdentifier". Unlike the duplicate guard, "force": true does NOT bypass EITHER bound — a budget any caller could wave through would be advisory, not a bound. Match on "code" AND "bound" — 409 alone is ambiguous, other refusals use it too, and both bounds share the same code. NOTE the enforcement key is "sessionId" itself: it is optional, caller-supplied, and format-validated only (not tied to any real dispatch), so either bound holds only for a cooperating orchestrator that follows the kickoff prose's instruction to stamp its own "sessionId" on every worker dispatch — a dispatch under a budgeted run with no "sessionId" is admitted, not refused, the same as an unresolvable run. Also note the concurrency caveat: there is no atomic reserve-then-insert, so either bound is "at most N, modulo in-flight concurrency," not a transactional cap. See LIN-1751/LIN-2934.
...
GET ${baseUrl}/api/proxy/dispatch?issueIdentifier={LIN-42}&status={queued|taken|done|failed|blocked|aborted}&limit={n}
  ...
  → { "items": [{ "id": "...", "status": "queued|taken|done|failed|blocked|aborted", "promptName": "...", "kind": "implementation", "issueIdentifier": "...", "issueUrl": "...", "target": "cli", "sessionId": null, "maxTasks": null, "maxSessionsPerTask": null, "dispatchedAt": "...", "resolvedAt": null, "completedAt": null, "feedbackCount": 1, "consumerLastSeenAt": "...", "consumerPollWarning": null }], "total": N }
  → EXPLICIT ALLOW-LIST (LIN-2975) — the fields above are the WHOLE list-item shape, not a sample; nothing else is on this row. "sessionId"/"maxTasks"/"maxSessionsPerTask" (LIN-2975/LIN-2934) mirror the same fields on GET /dispatch/{id} below, so a list reader can confirm budget stamping without a per-row detail GET — null means the row carries no session/budget, not "unread". The list stays deliberately leaner than the detail read: no "repo", "dispatchedBy", "model", "harness", "feedback", or "prompt" here — GET /dispatch/{id} (or /prompt) for those.
  → "consumerLastSeenAt"/"consumerPollWarning" (LIN-2885): same fields, same meaning as on GET /dispatch/{id} below — the enqueue-time poll-recency stamp, and a warning re-derived against the current clock on every read. Lets you spot a stale queued row directly from the list.
...
GET ${baseUrl}/api/proxy/dispatch/{id}
  ...
  → { "id": "...", "status": "queued|taken|done|failed|blocked|aborted", "kind": "implementation", "repo": null, "consumerLastSeenAt": "...", "consumerPollWarning": null, "feedback": [{ "message": "...", "url": "...", "timestamp": "..." }], ... }
  → "repo" (LIN-2975) is the validated/normalized repo basename this dispatch was queued against, or null when none was supplied (the workspace's default folder). Absent from the leaner GET /dispatch list above by design; read it here.
  → "consumerLastSeenAt" (LIN-2885) is the poll-recency stamp taken once when this item was enqueued — the most recent lastUsedAt across the workspace's consumer tokens at that moment, or null for "never". "consumerPollWarning" is re-derived from that SAME stamp against the CURRENT clock on every read, so a queued item that grows stale while sitting unpolled starts showing a warning here even if the workspace looked fine at dispatch time — and goes back to null once a consumer actually polls/takes. Same threshold as the enqueue-time "warning" (default 1h, CONSUMER_POLL_WARNING_THRESHOLD_MS).
...
POST ${baseUrl}/api/proxy/dispatch/halt
  Body: { "mode": "pause" | "stop" }
  → Requests an operator halt for this workspace (LIN-2994 Decision 4 — the degraded-mode operator path: proxy-token auth, no Linear, no heavy render path in front of it). This stores a request; the runner does not yet honor it (pending LIN-2995) — a successful call here does NOT mean a running session is actually paused or stopped.
  → Any "mode" other than "pause" or "stop" is rejected 400.
  → { "success": true, "halt": { "mode": "pause"|"stop", "setAt": "...", "setBy": "..." | null } }. "setBy" is your token's recorded creator — null for a legacy token with no recorded creator, which is still accepted (see GET /api/proxy/dispatch/halt above).

DELETE ${baseUrl}/api/proxy/dispatch/halt
  → Clears the workspace's operator halt request, same degraded-mode operator path as POST above. Harmless when nothing is set.
  → { "success": true }
