...
GET ${baseUrl}/api/proxy/rulings
  ...
  → Query params (both additive; the default read with neither is
    ...
    non-root carrier's "anchor" now names the root instead of itself):
      ...
      "includeResolved=true" — also return decisions this endpoint would
        ...
        "resolution" field (see below). Covers LOOP rulings only: an
        ANSWERED task-bound ruling (anchor.taskDecisionId set) never
        appears under this — there is no answered-task-decision read on
        this endpoint today; OPEN task-bound rulings are returned exactly
        as in the default read.
  → { "count": 2, "rulings": [{ "decision": { "decision_id": "...", "question": "...",
        ...
        "canReply": true, "shelvedLapseCount": 0,
  ...
  → "resolution" appears ONLY when "includeResolved=true" surfaced an
    ...
    "resolvedAt": "..." | null, "outcome": "answered" | "dismissed" }. Absent
    (not null) on every row from the default read, and never present on a
    task-bound row (see the query-param note above).
