...
POST ${baseUrl}/api/proxy/rulings/{decisionId}/suggest-dismissal   (readWrite)
  ...
  → PROPOSE that a ruling be dismissed. This does NOT dismiss it, and there is
    ...
    agreeing to your suggestion in the UI. Returns 201:
  ...
  → A reason is mandatory because the operator's whole interaction is a
    one-click Agree — an unjustified proposal turns that click into a rubber
    stamp. Attribution is taken from your token, not from the body.
...
POST ${baseUrl}/api/proxy/rulings/{decisionId}/suggest-answer   (readWrite)
  ...
  → PROPOSE that a ruling be ANSWERED with the given option. This does NOT
    ...
    discharges a ruling, by agreeing to your suggestion in the UI. Returns 201:
  → { "success": true, "suggestion": { "reason": "...", "suggestedBy": "...",
        "suggestedAt": "...", "withdrawn": false, "decisionLoopId": "..." | null,
        "proposedOutcome": "answered", "optionId": "a" },
      "note": "Recorded as a SUGGESTION only. The ruling is still unanswered until a human agrees to it." }
  → A reason is mandatory for the same reason it is on suggest-dismissal: the
    operator's whole interaction is a one-click Agree, so an unjustified
    proposal turns that click into a rubber stamp. Attribution is taken from
    your token, not from the body.
  ...
  → This route stays disposition-agnostic: it never checks the ruling's
    ...
    "agreedAt", or "acceptedAt" — those are the human's own Agree stamp.
