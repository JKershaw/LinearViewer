...
export function createFakeGitHubClient(seed = {}) {
  // Account-level seed for the OAuth/login surface (LIN-541): the authenticated
  // user and the repo list the picker shows. Read off reserved `_user`/`_repos`
  // keys so they don't collide with per-repo issue partitions below.
  const account = {
    user: seed._user || { id: 1, login: 'tester', name: 'Tester' },
    repos: seed._repos || [],
    // The user's App installations for the already-installed re-bind flow
    // (LIN-728). Reserved `_installations` seed key, shaped like GitHub's
    // `GET /user/installations` + `/user/installations/{id}/repositories`:
    //   [{ id, account: {login}, repositories: [{full_name, private}] }]
    installations: seed._installations || [],
  }
  ...
  for (const [slug, data] of Object.entries(seed)) {
    if (slug === '_user' || slug === '_repos' || slug === '_installations') continue
    repos[slug] = {
  ...
  return {
    ...
    async listRepos() {
      return account.repos.map(r => ({ ...r }))
    },
    async listUserInstallations() {
      return account.installations.map(i => ({ id: i.id, account: i.account ? { ...i.account } : null }))
    },
    async listUserInstallationRepos(installationId) {
      const inst = account.installations.find(i => String(i.id) === String(installationId))
      return (inst?.repositories || []).map(r => ({ ...r }))
    },
