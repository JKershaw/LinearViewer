...
export function createGitHubClient({ token, baseUrl = GITHUB_API_BASE, fetchImpl } = {}) {
  ...
  // (`out.length >= total_count`) or a short page (fewer than `per_page` rows —
  // the honest end-of-list signal when `total_count` is absent/wrong); the
  // safety cap is enforced by slicing afterward (page size is fixed at 100, so
  // the walk itself can only land on a page boundary, not the cap exactly).
  ...
  async function walkRepoPages(pathForPage, cap) {
    ...
    for (let page = 1; ; page++) {
      ...
      out.push(...batch)
      const totalCount = data?.total_count ?? out.length
