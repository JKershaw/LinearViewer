...
/**
 * Strip GitHub's own `#`-prefixed identifier rendering (LIN-2361) at the point it enters this
 * provider's issueId-taking methods. `ISSUE_ID_REGEX` (lib/workspace.js) now accepts a leading
 * `#` so every Harbour surface can render/echo `#55` as-is (a worker copying what it's shown must
 * have that string accepted end to end) — but a literal `#` reaching client.js's REST URL
 * interpolation (`/repos/{repo}/issues/{number}`) is a fragment delimiter, silently truncating to
 * a DIFFERENT valid endpoint (issue list on GET, issue create on POST), not a 404. So the widened
 * validator and this strip must ship together: widening alone is a silent-wrong-endpoint hazard,
 * not a visible failure.
 */
function stripHashPrefix(issueId) {
  return typeof issueId === 'string' && issueId.startsWith('#') ? issueId.slice(1) : issueId
}
...
export class GitHubProvider extends ProviderInterface {
  ...
  async fetchIssueFields(scope, issueId) {
    issueId = stripHashPrefix(issueId)
    const { client, repo } = this._clientFor(scope)
   ...
   */
  get ui() {
    // LIN-2361: GitHub's real workflow-state vocabulary (GITHUB_STATES, open/closed) is a fixed
    // static array, safe to surface SYNCHRONOUSLY — unlike Linear's per-team async states(), which
    // stays `fixedStates: null` (the base class default) precisely because it cannot be.
    return { ...super.ui, estimates: false, subtasks: false, attachments: true, displayName: 'GitHub Issues', fixedStates: GITHUB_STATES.map(s => ({ ...s })) }
  }
   ...
   *
   * `newWorkspaceVerb: true` (LIN-2803): the settings add row's "as a new
   * workspace" verb is honest here because `entryCta.href` (`/auth/github`,
   * `mode=new` by default) genuinely mints a fresh container for a
   * signed-in user (`supportsFreshContainer: true`, `routes/github-auth.js`
   * — `lib/github-install-flow.js`'s `intent.fresh` branch). This mirrors
   * that same underlying capability at the identity layer rather than
   * re-deriving it; if `supportsFreshContainer` is ever turned off, this
   * flag must go with it or the verb starts lying.
   */
  get addProvider() {
    return { blockedBy: null, configPredicate: isGitHubConfigured, newWorkspaceVerb: true }
  }
  ...
  async listRepos(token) {
    ...
    // `.map()` builds a fresh array and does not carry over the source array's
    // own `.truncated` property (LIN-2820) — re-attach explicitly so a capped
    // client-side read never silently reads as "that's every repo" downstream.
    // Only set when true (mirrors the client): an untruncated read stays a
    // plain array, so existing exact-array assertions are unaffected.
    if (repos.truncated) mapped.truncated = true
    return mapped
  }
  ...
  async listReboundableRepos(userToken) {
    ...
    const out = []
    // Aggregated across installations into a fresh array (LIN-2820) — a
    // per-installation `.truncated` from the client's page walk does not
    // survive into `out` on its own, so fold it in as we go: ANY installation
    // truncating means the aggregate is truncated too.
    let truncated = false
    for (const inst of installations) {
      const repos = await client.listUserInstallationRepos(inst.id)
      if (repos.truncated) truncated = true
    ...
    // Only set when true, mirroring listRepos above — an untruncated
    // aggregate stays a plain array, unaffected by exact-array assertions.
    if (truncated) out.truncated = true
    return out
  }

  /** LIN-2010 Phase 2: a connection is one App installation. */
  get connectionUnit() {
    return 'installation'
  }

  /** LIN-2010 Phase 2: a scope is one repository. */
  get scopeType() {
    return 'repository'
  }

  /**
   * LIN-2010 Phase 2 — thin delegation to {@link listReboundableRepos},
   * verbatim output, so `supports('listScopes')` and the generic
   * ProviderInterface surface work for GitHub without a call-site change.
   * @param {string} userToken - the user-to-server OAuth token from completeAuth.
   * @returns {Promise<Array<{slug: string, name: string, private: boolean, installationId: string}>>}
   */
  async listScopes(userToken) {
    return this.listReboundableRepos(userToken)
  }
