...
// How much of a stray/invalid suffix an error message may ever echo verbatim
// (LIN-2081 round-5 review, finding 1): content after the END footer can be
// an entire second, signable PEM if two keys were concatenated, so this must
// stay well under any threshold a leak-detector would treat as "a real run
// of key material" — 12 raw base64 chars is not that.
const PEM_LEAK_PREVIEW_MAX = 12

// How many individual offending characters a bad-alphabet-line message may
// name (LIN-2081 round-5 review, finding 3): uncapped, a single corrupt line
// of a few hundred characters produced a multi-KB error message into logs
// that have no such line reproduced anywhere else in the codebase.
const PEM_BAD_CHAR_PREVIEW_MAX = 5

/**
 * Describe a stray PEM suffix (the content after the END footer) for an
 * error message without ever echoing it verbatim past a small bound
 * (LIN-2081 round-5 review, finding 1). A non-global `String.match` for the
 * END footer binds to the FIRST `-----END ... KEY-----`, so for two
 * concatenated PEMs `trailing` is the entire second key — the reviewer
 * extracted it from the raw error text and signed with it. If the suffix
 * itself contains a second armor header, name that fact and its length
 * instead of any of its content; otherwise fall back to a short, explicitly
 * truncated preview. Never returns unbounded content.
 * @param {string} trailing
 * @returns {string} a message fragment already single-quoted where relevant
 */
function describeStraySuffix(trailing) {
  if (trailing.includes('-----BEGIN')) {
    return `a second '-----BEGIN' block (${trailing.length} chars)`
  }
  if (trailing.length <= PEM_LEAK_PREVIEW_MAX) {
    return `'${trailing}'`
  }
  return `'${trailing.slice(0, PEM_LEAK_PREVIEW_MAX)}' (+${trailing.length - PEM_LEAK_PREVIEW_MAX} more)`
}
...
function assertPemShape(privateKey) {
  ...
  if (trailing !== '' && trailing !== '\n') {
    throw new Error(`GitHub App auth: GITHUB_APP_PRIVATE_KEY ends with stray characters after the END line: ${describeStraySuffix(trailing)}`)
  }
  ...
  if (badLineIndex !== -1) {
    ...
    const allBadChars = [...bodyLines[badLineIndex]]
      .map((ch, col) => ({ ch, col: col + 1 }))
      .filter(({ ch }) => !PEM_ARMOR_LINE.test(ch))
    // Capped at PEM_BAD_CHAR_PREVIEW_MAX (LIN-2081 round-5 review, finding
    // 3): uncapped, a single long corrupt line inflated this message to
    // several KB. No key-material risk either way — only characters OUTSIDE
    // the base64 alphabet are ever named, so this is log hygiene, not redaction.
    const shown = allBadChars.slice(0, PEM_BAD_CHAR_PREVIEW_MAX).map(({ ch, col }) => `'${ch}' at column ${col}`).join(', ')
    const badChars = allBadChars.length > PEM_BAD_CHAR_PREVIEW_MAX
      ? `${shown} (+${allBadChars.length - PEM_BAD_CHAR_PREVIEW_MAX} more)`
      : shown
