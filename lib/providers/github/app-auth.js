 ...
 * onto one line), convert CRLF to LF, strip leading tab/space indentation
 * from every line, and collapse leading/trailing whitespace to at most a
 * single trailing newline — a CRLF round-trip (Windows/secret manager), a
 * stray leading blank line, an extra trailing blank line, a trailing space
 * after the END footer, and an indented body (a YAML block-scalar or heredoc
 * indentation leak) all sign fine in OpenSSL and must not be rejected
 * (review finding 2 + observation A — same "reject-what-signs" risk, and the
 * DECISION here, recorded rather than inherited per the review's ask, is to
 * resolve it the same way finding 2 did: normalize it away rather than stay
 * strict, since this file's whole point is to never hard-reject a key that
 * actually works). Targeted regexes rather than a blanket `.trim()` so a key
 * with NO stray whitespace round-trips byte-for-byte. Shared by
 * getAppConfig() and getGitHubConfigProblems() so the two never validate
 * different bytes.
