 ...
 * onto one line), convert CRLF to LF, and collapse leading/trailing
 * whitespace to at most a single trailing newline — a CRLF round-trip
 * (Windows/secret manager), a stray leading blank line, an extra trailing
 * blank line, and a trailing space after the END footer all sign fine in
 * OpenSSL and must not be rejected (review finding 2). Targeted regexes
 * rather than a blanket `.trim()` so a key with NO stray whitespace
 * round-trips byte-for-byte. Shared by getAppConfig() and
 * getGitHubConfigProblems() so the two never validate different bytes.
