...
/**
 * The legacy back-compat default provider (LIN-561). Resolving an absent
 * `workspace.provider` to Linear is a deliberate, documented fallback — NOT an
 * implicit "everything is Linear" assumption — kept so legacy workspaces (no
 * `provider` field) and the unauthenticated landing page (no workspace at all)
 * keep resolving to the historical single provider, byte-identically. Whether to
 * retire it once every workspace carries an explicit provider is an open question
 * tracked with the broader per-workspace provider-selection work (LIN-306).
 */
const LEGACY_DEFAULT_PROVIDER = 'linear'

/**
 * Resolve the provider for a workspace, falling back to the legacy default
 * provider ({@link LEGACY_DEFAULT_PROVIDER}).
 *
 * Centralizes the fallback used by every render surface (dashboard, swipe)
 * so they resolve provider display strings identically (LIN-177 S3).
 *
 * @param {{provider?: string}} [workspace]
 * @returns {object|undefined} Provider instance (legacy default fallback), or
 *   undefined only if even the default provider is unregistered (not possible at
 *   runtime).
 */
export function getProviderForWorkspace(workspace) {
  return providers.get(workspace?.provider) || providers.get(LEGACY_DEFAULT_PROVIDER)
}
