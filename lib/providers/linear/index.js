    ...
    issue(id: $id) {
      ...
      children {
        nodes {
          ...
          # Grandchildren (ids only) so the subtask overview can show a nested-
          # subtask count — a cheap "this child is itself a node" signal for the
          # recommender, without fetching the grandchildren's full detail.
          children {
            nodes {
              id
            }
...
export class LinearProvider extends ProviderInterface {
  ...
  fetchProjects(apiKey, teamId = null) { return fetchProjects(apiKey, teamId) }
