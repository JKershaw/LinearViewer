...
const ISSUE_DETAIL_QUERY = gql`
  query($id: String!) {
    issue(id: $id) {
      ...
      # fetchIssueContext's normalization below). Also cache-safe (not in the hashable
      # slice). first: 50 bounds payload; newest-first is Linear's default order.
      history(first: 50) {
        nodes {
          createdAt
          fromState { name }
...
export async function fetchIssueContext(apiKey, issueId, { signal } = {}) {
  ...
  // transition. Map to a compact provider-neutral shape and keep Linear's newest-first
  // order. Surfaced only via the task-chat get_history tool, so absence (no history
  // selected / other provider) simply yields [].
