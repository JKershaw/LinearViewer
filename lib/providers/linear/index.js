 ...
 *
 * The comments below carry no `id`/`commentId` (see the mapping a few lines
 * down) — `scanBasisFromContext` (lib/scan-fingerprint.js) never reads
 * `focusedChild.comments[]` and is safe by construction; LIN-2649's
 * `dueBasisFromContext` is the second confirmed-safe-because-unread consumer,
 * for the same reason. Do not read `.id`/`.commentId` off these comments.
 *
 * @param {string} apiKey - OAuth access token
...
// /`deleteComment`: their commentUpdate/commentDelete mutations had no proxy
// endpoint at the time this file was written and were added here to complete
// the write surface per LIN-307's scope expansion (the parent ticket's
// "comment edit/delete" note). The proxy route landed later, in LIN-1160
// (`routes/proxy-writes.js:641`).
//
...
// LIN-307 scope expansion: comment edit/delete had no proxy endpoint yet at
// the time, so these two mutations are the only NEW GraphQL in this step
// (everything else is relocated). Standard Linear commentUpdate/commentDelete.
// The proxy route landed later, in LIN-1160 (`routes/proxy-writes.js:641`).
...
// LIN-694: description + comments are additive fields on the SAME query (no
// extra round trip) — routes/proxy.js's PATCH /api/proxy/issues/:id handler
// uses them to apply the periodical report-persistence gate before a
// completed-state write, without paying a second network call on the
// overwhelmingly common (non-periodical) path. Both call sites of
// issueWriteGuard already ignore fields they don't need, so this is safe for
// every existing consumer. `comments(last: 20)` is deliberately bounded —
// unlike `description`, an issue's full comment history has no size cap, and
// this guard runs on EVERY write to EVERY issue in the workspace, not just
// periodical ones; the gate only ever needs to see a comment posted near a
// task's own conclusion, so the most recent 20 is ample headroom.
const TRASHED_GUARD_QUERY = gql`
  query($id: String!) {
    issue(id: $id) {
      ...
      description
      team { id }
      comments(last: 20) { nodes { id body } }
    }
  }
`
 ...
 * @param {string} apiKey
 * @param {string|null} [teamId] - When set, that team's labels PLUS the
 *   workspace-level labels (team: null), because a workspace-level label is
 *   applicable to an issue on any team. Deliberately inclusive (LIN-2787): the
 *   old team-scoped GraphQL filter hid workspace-level labels, so a workspace
 *   whose type labels (Bug/Feature/Improvement) live at workspace level looked
 *   like it had no type label at all. Filtering happens in-process over the
 *   org-wide catalog rather than via a second Linear filter, since Linear's
 *   team filter cannot express "team is this one OR null".
 * @returns {Promise<Array>} Labels ({ id, name, color, team })
 */
export async function labels(apiKey, teamId = null) {
  const client = createLinearClient(apiKey)
  const data = await client.request(LABELS_QUERY, {})
  const all = data.issueLabels?.nodes || []
  if (!teamId) return all
  return all.filter((label) => !label.team || label.team.id === teamId)
}
...
/**
 * Trashed/team/description/comments probe (`{ id, trashed, team, description,
 * comments }` or null) for write handlers that do not otherwise read the
 * issue. Null means Linear could not resolve the id. `description` and
 * `comments` (LIN-694) exist for the periodical report-persistence gate, not
 * as a general-purpose issue read — do not grow this further; a caller
 * needing more should use a dedicated read instead of overloading this guard.
 * @param {string} apiKey
 * @param {string} issueId
 * @returns {Promise<Object|null>}
 */
...
export class LinearProvider extends ProviderInterface {
  ...
  // LIN-1557: the headless proxy's accept-list is wider than the UI form —
  // assigneeId/parentId/cycleId round-trip through IssueCreateInput today
  // even though the create form (createFields() above) never renders them.
  apiWriteFields() {
    return [...this.createFields(), 'assigneeId', 'parentId', 'cycleId']
  }
  ...
  // API-surface issue reads relocated from the proxy route (LIN-308), now
  // declared on PROVIDER_SURFACE's `apiReads` group (LIN-2350) so the
  // capability descriptor sees them and `denyIfUnsupported` can gate them.
  ...
  // surface — unlike the api reads above (declared since LIN-2350), these stay
  // route-internal data-fetch a write depends on, not a capability a caller
  // asks for directly.
  ...
  get landingCatalogue() {
    return { blurb: 'the original backend', order: 0 }
  }

  /**
   * No `configPredicate`/`authShapes` — the row is live and unconditional
   * (adding a 2nd Linear org needs no server config). `addHint` is declared
   * literally here, byte-identical to lib/render-settings.js's prior
   * KNOWN_ADD_PROVIDERS entry (R2). `createsWorkspace: true` (LIN-2803) is
   * the one piece of provider-declared state behind that hint: Linear's add
   * genuinely `upsertWorkspace`s a second, separate workspace
   * (`routes/auth.js`'s add-source arm) rather than binding onto the viewed
   * one, so it drives the settings row's new-workspace-verb suppression
   * (both verbs would mean the same thing here) and the post-bind notice's
   * "make active" suppression (there is nothing on THIS workspace to
   * activate) from one fact instead of three prose copies.
   */
  get addProvider() {
    return {
      blockedBy: null,
      addHint: 'connects another Linear organization as its own workspace',
      createsWorkspace: true,
    }
  }

  /**
   * Deliberately ungated (R2): `/auth/linear` 503s without three LINEAR_*
   * vars, but `routes/auth.js` short-circuits under NODE_ENV=test and the
   * Playwright server sets no LINEAR_* — a real config predicate here would
   * break landing.spec.js/auth.spec.js/landing-swipe.spec.js.
   */
  get entryCta() {
    return { href: '/auth/linear', isConfigured: () => true }
  }
}
