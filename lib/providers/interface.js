...
export const PROVIDER_SURFACE = {
  ...
  // Consumer-API reads relocated from the proxy route by LIN-308 (`cf018538`),
  // one cohort from one commit. Declared together here by LIN-2350: `viewer`,
  // `projects`, `issues`, `issueDetail`. Distinct contracts from the dashboard
  // `fetchViewer`/`fetchProjects`/`fetchProjectsList` reads above — different
  // response shapes, different consumers (the agent-facing proxy, not the
  // dashboard render path) — proven by `local`, which implements `viewer` but
  // not `fetchViewer`. Declaring them (with throwing `NotImplementedError`
  // stubs below) lets `supports()`/`denyIfUnsupported` see and gate them; LIN-2350
  // itself only wires the gate onto `/api/proxy/me` (`viewer`) and
  // `/api/proxy/issues/:id` (`issueDetail`) — LIN-2355 gates the other two,
  // `/api/proxy/projects` and `/api/proxy/issues` (list); LIN-1573 is narrowed
  // to its distinct product question (should GitHub actually implement these
  // reads) and owns no route gating. This is the deliberate CONTRAST with the
  // route-internal reads just below
  // (`issueWriteGuard`/`issueDescription`/`issueLabels`/`updateIssueLabels`,
  // LIN-1559): those four stay off-surface because they are route-internal
  // data-fetch a write depends on, never a capability a caller asks for
  // directly, whereas these four ARE the capability a caller's GET asks for —
  // hence declared, not existence-gated.
  apiReads: ['viewer', 'projects', 'issues', 'issueDetail'],
  ...
  // Reboundable-scope enumeration (LIN-2010 Phase 2, additive groundwork for
  // LIN-2149's Account -> Connection -> Workspace picker). A single additive
  // method, thinly delegated to each provider's existing bespoke enumerator
  // (github.listReboundableRepos, github-projects.listReboundableBoards,
  // jira's fetchJiraAccessibleResources) — no call-site changes this ticket,
  // zero consumers yet. Listed here so `supports('listScopes')` and the 422
  // CAPABILITY_NOT_SUPPORTED gate work identically to every other declared
  // method; providers that don't implement it inherit the decline.
  scopes: ['listScopes'],
}
...
export class ProviderInterface {
  ...
  get ui() {
    ...
    return {
      ...
      // LIN-2804: derived from real method support, same idiom as inlineCreate/inlineEdit
      // above — a prompt-generation call site reads these to gate the `/issues/{id}` and
      // `/relations/{id}` read hints it advertises to a dispatched worker, so an
      // unsupported-capability workspace never gets pointed at a read that 422s.
      issueDetail: this.supports('issueDetail'),
      relations: this.supports('relations'),
      // LIN-2804 review Finding 1: the `/search` fallback hint is itself capability-gated
      // (`denyIfUnsupported(provider, 'search', ...)` in routes/proxy-reads.js) — GitHub
      // Projects and Jira implement neither `issueDetail` nor `search`, so a naive
      // brief+search fallback still 422s them. Derived the same way, so the prompt-hint
      // gate and the route's own denial can never drift apart.
      search: this.supports('search'),
      ...
      // LIN-2361: a SYNCHRONOUS-only workflow-state vocabulary a dispatched prompt can name
      // verbatim ("Set #55 status to X"), or `null` when the provider's real vocabulary needs an
      // async/per-team fetch (Linear's `states(apiKey, teamId)` is `async`, called live per team —
      // `ui` is read synchronously with no `await` at every call site, so it can never carry
      // Linear's real states). `null` is the safe default: no synchronous vocabulary exists, so
      // prompt-formatters.js leaves any "Set status" wording exactly as it is today — no
      // regression, no silent gap. A provider whose full vocabulary IS available synchronously
      // (GitHub: a fixed `open`/`closed`) overrides this with that array.
      fixedStates: null,
    }
  }
  ...
  //
  // All three default to `null` on the base class, meaning "declares nothing
  // for this surface" — the correct behavior for a provider that doesn't
  // participate in it (e.g. `local` has no Settings add row, and
  // `github-projects` has no entry CTA) rather than an oversight to paper over.
  // Settings and the hero/navbar CTAs genuinely skip a `null` provider
  // (`addProvider`: `getAllProviders().filter(p => p.addProvider)`; `entryCta`:
  // read per-provider via `getProvider(...)?.entryCta?.href`, so a `null` just
  // yields no CTA). The landing strip does NOT skip it: `render-landing.js`
  // maps every registered provider to an `<li>` regardless of
  // `landingCatalogue`, sorting a `null`-catalogue provider last via
  // `?? Infinity` but still rendering it with an empty blurb — so on the
  // landing surface specifically, `null` is "shown last with blank markup",
  // not "absent."

  /**
   * Landing-strip identity: `{ blurb, order }`, or `null` for "no blurb" —
   * the provider is still rendered (sorted last via `?? Infinity`) with an
   * empty note, not omitted from the strip.
   ...
   * configPredicate, createsWorkspace, newWorkspaceVerb }` or `null` for
   * "no row" (e.g. `local`, whose door is `POST /workspace/new`, not
   * `/providers/add`).
   ...
   * `addHint` is the row's honest note on what its add DOES; omitting it is not
   * an oversight — lib/render-settings.js's `DEFAULT_ADD_HINT` supplies the
   * majority "adds a source onto this workspace" wording at the read site
   * (never as a default on this getter: every override returns a fresh object
   * literal, so a base-class default would silently not be inherited).
   * `createsWorkspace` and `newWorkspaceVerb` (LIN-2803) are the two facts the
   * settings add row reads to keep its verbs truthful, both read as `=== true` /
   * `!== true` so declaring nothing means the honest majority reading:
   * - `createsWorkspace: true` — this provider's add mints a SEPARATE workspace
   *   rather than binding onto the viewed one (today: `linear`). It suppresses
   *   both the "as a new workspace" verb (both verbs would mean the same thing)
   *   and the post-bind notice's "make active" (this workspace gained nothing to
   *   activate). Default/`false` = binds onto the viewed workspace.
   * - `newWorkspaceVerb: true` — render the second "as a new workspace" verb on
   *   this row, pointing at `entryCta.href` (today: `github` only). Declare it
   *   ONLY when that route really mints a fresh container for an already
   *   signed-in user (GitHub's `supportsFreshContainer`); Jira deliberately
   *   withholds it because `?mode=new` still rejoins its existing container
   *   until LIN-2819. A provider with no `entryCta` renders bind-only whatever
   *   it declares here.
   * @returns {{authShapes?: object[], addHint?: string, blockedBy?: string|null, configPredicate?: () => boolean, createsWorkspace?: boolean, newWorkspaceVerb?: boolean}|null}
   */
  get addProvider() {
    return null
  }
  ...
  /**
   * Label naming what a "connection" is for this provider in the
   * Account -> Connection -> Workspace hierarchy LIN-2149 builds
   * (e.g. `'installation'` for the GitHub family's App installations,
   * `'site'` for Jira's Atlassian tenants). `null` on the base class — a
   * provider that declares nothing has no connection concept above its
   * workspace binding (Linear, Local). Display-only; no consumer this ticket.
   * @returns {string|null}
   */
  get connectionUnit() {
    return null
  ...
  /**
   * Label naming what one item returned by {@link listScopes} represents
   * (e.g. `'repository'` for github, `'board'` for github-projects, `'site'`
   * for jira). `null` on the base class. Display-only; no consumer this
   * ticket — see `listScopes`' own doc for why its return shape stays
   * provider-specific rather than canonical.
   * @returns {string|null}
   */
  get scopeType() {
    return null
  ...
  // Consumer-API reads (LIN-2350) — see the `apiReads` group comment above.
  viewer() { throw new NotImplementedError('viewer', this.name) }
  projects() { throw new NotImplementedError('projects', this.name) }
  issues() { throw new NotImplementedError('issues', this.name) }
  issueDetail() { throw new NotImplementedError('issueDetail', this.name) }
  ...
  // ---------------------------------------------------------------------------
  // Scope enumeration (LIN-2010 Phase 2) — additive groundwork for LIN-2149's
  // Account -> Connection -> Workspace picker. Declared headroom only; no
  // provider is wired into a picker this ticket, so nothing regresses.
  // ---------------------------------------------------------------------------
  //
  // listScopes(credential) lists the reboundable scopes (repos/boards/sites)
  // an already-acquired credential can reach, tagged per item with whatever
  // that provider needs to bind one (e.g. GitHub's per-item `installationId`).
  // Each implementer is a THIN delegation to its existing bespoke enumerator —
  // github.listScopes wraps `listReboundableRepos`, github-projects.listScopes
  // wraps `listReboundableBoards`, jira.listScopes wraps
  // `fetchJiraAccessibleResources` — verbatim output, no shape change.
  //
  // The return shape is DELIBERATELY provider-specific, not canonical: github
  // returns `{slug, name, private, installationId}`, github-projects returns
  // `{login, number, title, url, shortDescription, closed, installationId}`,
  // jira returns `{cloudId, url, name}`. Defining a canonical envelope over
  // these is LIN-2149's job, once a real picker exists to validate a shape
  // against — inventing one here, with zero consumers, would be unvalidated
  // interface-widening. `scopeType` (above) names what one item IS; it does
  // not imply the items share a shape across providers.
  listScopes() { throw new NotImplementedError('listScopes', this.name) }
}
