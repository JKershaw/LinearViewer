...
import { mintInstallationToken, fetchInstallation, buildAuthorizeUrl, buildInstallUrl, exchangeOAuthCode, listUserInstallations, isGitHubConfigured } from '../github/app-auth.js'
...
export class GitHubProjectsProvider extends ProviderInterface {
  ...
  _clientFor(scope) {
    if (scope && typeof scope === 'object') {
      // LIN-1887 F3.3 — see the Issues provider's `_clientFor` for why the
      // github family needs this explicitly: `board ?? null` would otherwise
      // swallow a dropped scope into a silent board-less call.
      if (scope.ambiguousCallScope) {
        throw new Error('GitHubProjectsProvider: this workspace has several GitHub Projects bindings and the active one could not be identified — refusing to guess which board to call')
      }
  ...
  /**
   * The authenticated human's GitHub identity for a user-to-server token (GET
   * /user). This provider's own client is GraphQL-only (board reads), so the
   * viewer lookup goes through the plain REST client GitHubProvider already
   * uses for the same call — hoisted here for LIN-1329 (Phase C): a GitHub
   * identity's scope is the human's GitHub user id, shared with the Issues
   * provider (Q3) so the same human reaches the same account through either
   * door.
   * @param {string} token - a GitHub user-to-server OAuth token.
   * @returns {Promise<{id: string, login: string, name: string|null}>}
   */
  async fetchViewer(token) {
    const user = await this._restClientForToken(token).getAuthenticatedUser()
    return { id: String(user.id), login: user.login, name: user.name || null }
  }

  /**
   * Wrap the plain GitHub REST client (LIN-1329). Overridable in tests
   * (mirrors `_listUserInstallations`) so `fetchViewer` can run offline
   * against a fake instead of a real HTTP call.
   * @param {string} token
   * @returns {object} a client exposing `getAuthenticatedUser()`.
   */
  _restClientForToken(token) {
  ...
  /** LIN-2010 Phase 2: a scope is one Projects v2 board. */
  get scopeType() {
    return 'board'
  }

  /**
   * LIN-2010 Phase 2 — thin delegation to {@link listReboundableBoards},
   * verbatim output, so `supports('listScopes')` and the generic
   * ProviderInterface surface work for GitHub Projects without a call-site
   * change.
   * @param {string} userToken - the user-to-server OAuth token from completeAuth.
   * @returns {Promise<Array<{login: string, number: number, title: string, url: string|null, shortDescription: string|null, closed: boolean, installationId: string}>>}
   */
  async listScopes(userToken) {
    return this.listReboundableBoards(userToken)
  }
  ...
  get landingCatalogue() {
    return { blurb: 'boards, read-only V1', order: 2 }
  }

  /**
   * `configPredicate` is the SAME `isGitHubConfigured` function reference the
   * `github` provider declares (F1, shared-App gate) — imported from the same
   * module, not re-derived, so both rows resolve one shared predicate.
   */
  get addProvider() {
    return { blockedBy: null, configPredicate: isGitHubConfigured }
  }

  // No entryCta: `/auth/github-projects` supports mode='new', but this is a
  // declared absence, not derived from capability — Projects is not a new-login
  // entry point (plan Design decision 3).
}
