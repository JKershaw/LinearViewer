...
/**
 * LIN-2001 — a test-only override for the two DIRECT `fetchImpl` call sites
 * below (`postJiraToken`'s POST, `fetchJiraAccessibleResources`'s GET). It
 * exists to give Playwright a real-HTTP seam for the OAuth callback, which has
 * no other stub point: `exchangeJiraCode`/`fetchJiraAccessibleResources` are
 * called with no injected `fetchImpl` from `routes/jira-auth.js`.
 *
 * Deliberately does NOT touch `buildJiraAuthorizeUrl` (still always
 * `JIRA_AUTH_HOST`, so the authorize redirect landing.spec.js/
 * settings-providers.spec.js assert stays untouched) or `jiraOAuthApiBase`
 * (still hard-pinned to `JIRA_API_HOST`, so its LIN-1887 F6 SSRF guard is
 * never in this call path).
 *
 * Read at CALL TIME, not frozen at module load — deliberately, so a unit test
 * can toggle both env vars per case in one process (the same reason
 * `getMissingJiraOAuthConfig` above reads `process.env` per-call rather than
 * caching it).
 *
 * `postJiraToken` is shared by `exchangeJiraCode` (the callback's code
 * exchange) AND `refreshJiraAccessToken` (the live durable-credential refresh,
 * registered at `server.js`), so this override reaches the refresh caller too.
 * That is inert in production and in this repo's e2e harness — the gate below
 * fails closed on `NODE_ENV !== 'test'`, and every OAuth binding this harness
 * seeds carries a ~1h expiry against the fake's own `expires_in: 3600`, so
 * nothing here triggers a real refresh — but it is a real consequence of one
 * override serving both callers, not a Jira-refresh-specific carve-out.
 *
 * @returns {string|null} the override base, or `null` when inactive.
 */
function oauthTestOverrideBase() {
  if (process.env.NODE_ENV !== 'test') return null;
  return process.env.JIRA_OAUTH_TEST_BASE || null;
}

/**
 * The requested scope set (D2 — John's decision; read+write since LIN-1968).
 *
 * `offline_access` is what makes a refresh token issued at all; without it the
 * whole `oauth-refresh` strategy has nothing to refresh from.
 *
 * `write:jira-work` was DELIBERATELY ABSENT for LIN-1887, and its addition is a
 * decision rather than an oversight being corrected. D2 originally fixed the set
 * at three read-only scopes because widening a consent screen re-prompts every
 * already-consented user — a one-way door. LIN-1968 found what that cost: the
 * capability gate is provider-level (`get ui()` / `supports()` auto-derive from
 * which methods are implemented), so `inlineEdit` flipped `true` the moment
 * LIN-1886 implemented `updateIssue` — for BASIC and OAuth bindings alike. An
 * OAuth binding therefore advertised edit affordances its read-only grant could
 * not honour, and because bindings key on `(provider, scope)` and MERGE
 * (`routes/jira-auth.js`), a Basic→OAuth "upgrade" was a silent capability
 * DOWNGRADE.
 *
 * John's ruling (2026-08-09, recorded on LIN-1968): request the write scope. The
 * cost of widening is a function of the consented population, and at the moment
 * of the decision that population was exactly one — the grant created during the
 * LIN-1947 gate. LIN-1890 makes OAuth the landing-page front door, after which
 * the same change would force a re-consent on every user permanently. Cheap only
 * inside that window; taken inside it.
 *
 * Still deliberately does NOT include `read:me`. The plan's Step 4 proposed
 * resolving identity via `GET https://api.atlassian.com/me`, which requires that
 * scope — but identity is instead resolved through `GET /rest/api/3/myself` on
 * the chosen site, covered by `read:jira-user`. That is the SAME endpoint and the
 * SAME `accountId` Phase 1 already keys identity on (`routes/jira-auth.js`),
 * which is load-bearing beyond scope economy: a human upgrading a Basic link to
 * OAuth must resolve to the same Harbour account, and two identity endpoints
 * could drift.
 *
 * Widening this set again is still a one-way door, and the population is no
 * longer one. Treat any further addition as the same class of decision.
 */
...
async function postJiraToken(body, { fetchImpl = fetch } = {}) {
  ...
  let data;
  try {
    response = await fetchImpl(`${oauthTestOverrideBase() || JIRA_AUTH_HOST}/oauth/token`, {
      method: 'POST',
...
export async function fetchJiraAccessibleResources(accessToken, { fetchImpl = fetch } = {}) {
  const res = await fetchImpl(`${oauthTestOverrideBase() || JIRA_API_ORIGIN}/oauth/token/accessible-resources`, {
