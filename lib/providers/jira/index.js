...
// Done tier's recency window (LIN-2155) — regression prevention for Recent
// Activity's `completed` rows (tree.js), not a feature: "done last, no
// window" would starve that feed on a large project.
const DONE_RECENT_WINDOW_DAYS = 7
...
export class JiraProvider extends ProviderInterface {
  ...
  async _epicsForProjects(client, projects) {
    ...
    const lowerTiers = [
      ...
      {
        name: 'done',
        jql: tierJql(keys, CANONICAL_TYPE_TO_JIRA_STATUS_CATEGORY[doneType], {
          extra: `resolutiondate >= -${DONE_RECENT_WINDOW_DAYS}d`,
          orderBy: 'key ASC',
        }),
        cap: DONE_RECENT_CAP,
      },
