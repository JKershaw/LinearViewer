...
//                      empty. ADDITIVE: a Jira project keeps being surfaced as a
//                      canonical PROJECT too (id = numeric project id, via
//                      `_toCanonicalProject`/`fetchProjects`) until LIN-2011
//                      re-points the project level at epics — the two ids never
//                      collide, so both mappings are safe to carry at once.
...
export class JiraProvider extends ProviderInterface {
  ...
  async fetchProjects(scope, teamId = null, _opts = {}) {
    ...
    // Read `.truncated` off the array BEFORE `.map()` below — `Array.prototype.map`
    // returns a fresh array that does not carry the custom property (LIN-2006).
   ...
   *
   * ADDITIVE, not a replacement: a Jira project keeps being surfaced as a
   * canonical PROJECT too (id = numeric project id, via
   * `_toCanonicalProject`/`fetchProjects` above) until LIN-2011 re-points the
   * project level at epics — removing the project mapping now would strand
   * every issue in "No Project" (LIN-2007's own sequencing warning). The two
   * ids never collide (a project key is never numeric-only in practice, and
   * even if it were, `<source>:<id>` merge keys on the SOURCE + id pair, not
   * a bare id), so both mappings coexist safely.
   */
