...
import { isJiraOAuthConfigured } from './oauth.js'
...
export class JiraProvider extends ProviderInterface {
   ...
   * parent in a different project) or a truncation-order mismatch (the
   * epic's key sorts past `DEFAULT_SEARCH_CAP` while a child sorts before
   * it). `_toCanonicalIssue` still stamps that issue's canonical `project`
   ...
   * forest.get(p.id) ...)` walk. Below, any epic referenced by an in-batch
