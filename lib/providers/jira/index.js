...
// off), comment edit/delete, relations, file upload, and story-point mapping
// (LIN-1888) — `supports()` is the live answer, this list is not exhaustive by
// construction. OAuth 3LO is NO LONGER out of scope: it landed as
// LIN-1887/LIN-1890 (routes at routes/jira-auth.js; `JIRA_OAUTH_SCOPES`, in
// ./oauth.js, includes `write:jira-work`) and sits alongside the API-token
// Basic shape. See LIN-275's
...
export class JiraProvider extends ProviderInterface {
   ...
   * this is an EXTERNAL link, not an in-app write. `createIssue` stays
   * unimplemented (LIN-1557); `updateIssue` does NOT — it landed with LIN-1886,
   * as the `ui` getter's own note below records. Uses the boot-configured `site`
   ...
   *
   * `createsWorkspace: false` is written explicitly (self-documenting, even
   * though falsy is already the read site's default) alongside a deliberate
   * absence of `newWorkspaceVerb` (LIN-2803): `/auth/jira/oauth?mode=new`
   * still REJOINS the existing `jira:<accountId>` container for a returning
   * signed-in user (`routes/jira-auth.js`'s `if (existing)` branch) instead
   * of minting a fresh one — unlike GitHub's `supportsFreshContainer`. Adding
   * the verb here today would render a working-looking button that lies.
   * Flip `newWorkspaceVerb: true` once LIN-2819 makes the route mint fresh.
   */
  get addProvider() {
    return {
      blockedBy: null,
