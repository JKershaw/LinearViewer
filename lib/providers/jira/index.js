...
//     whenever the CURRENT stored ADF contains a node or mark this module
//     cannot losslessly round-trip (`adfHasUnrenderableContent`) — never a
//     silent, corrupting overwrite.
...
// adfToMarkdown/renderAdfNode above. Deliberately covers only the SAME node/
// mark set the reader models (paragraphs, headings, bullet/ordered lists, code
// blocks, blockquotes, hr, hard breaks, and the strong/em/code/strike/link
// marks) — round-tripping THOSE constructs is the contract
// (`markdownToAdf(adfToMarkdown(doc))` must re-render to the same Markdown for
// every modeled node type). Anything else in the input Markdown degrades to
// plain text; this function never throws — refusal-on-unrenderable-content is
 ...
 * write is about to overwrite; this function only ever produces the modeled
 * node/mark set, so it can never itself manufacture something that function
 * would flag.
...
// over an ADF tree. `adfToMarkdown`'s node switch + this module's MARK_RENDERERS
// are the two sources of truth for "modeled"; this function derives its mark
// check from MARK_RENDERERS directly (never a second hand-copied list) and
// hard-codes the node-type set to MIRROR (not duplicate the enforcement of)
// `renderAdfNode`'s switch — a node/mark outside either set degrades silently
// on READ (falls through to its child content / is dropped) but must refuse
// loudly before a WRITE overwrites it, since a silent-degrade round-trip would
// destroy content the reader never even represented losslessly on the way in.
// -----------------------------------------------------------------------------
const MODELED_ADF_NODE_TYPES = new Set([
  'doc', 'text', 'paragraph', 'heading', 'bulletList', 'orderedList', 'listItem',
  'codeBlock', 'blockquote', 'rule', 'hardBreak', 'mention', 'inlineCard', 'blockCard', 'emoji',
])

function nodeHasUnrenderableContent(node) {
  if (!node || typeof node !== 'object') return false
  if (!MODELED_ADF_NODE_TYPES.has(node.type)) return true
...
/**
 * True if `doc` contains any node whose `type` is outside the set
 * `adfToMarkdown` models, OR any text node carrying a mark whose `type` is
 * outside `MARK_RENDERERS`'s keys — checked recursively through the WHOLE
 * tree (a table nested three levels inside an otherwise-modeled list is still
 * caught). Used to refuse (422) an in-app description overwrite that would
 * silently destroy Jira-native content this integration cannot round-trip
 * (LIN-1886 D1 policy) — never used to reject a read, only a write.
 *
...
export class JiraProvider extends ProviderInterface {
  ...
  /**
   * The issue's RAW ADF description (`{ id, description, trashed }` or null) —
   * NOT markdown. `updateIssue`'s D1 refusal check must inspect the CURRENT
   * stored ADF before allowing an overwrite, so this is deliberately the
   * unconverted wire shape, unlike every other description read in this file.
   * @returns {Promise<Object|null>}
   */
  async issueDescription(scope, issueId) {
    ...
    const jira = await client.getIssue(issueId)
    if (!jira) return null
    return { id: jira.id, description: jira.fields?.description ?? null, trashed: false }
  }
  ...
  async updateIssue(scope, issueId, patch = {}) {
    ...
    // D2: status transition — skip entirely (no getTransitions/doTransition
    // call at all) when the current canonical type already matches.
    if (patch.stateId != null) {
      ...
      if (targetType !== currentType) {
        ...
        await client.doTransition(issueId, match.id)
        didWrite = true
      }
    }
