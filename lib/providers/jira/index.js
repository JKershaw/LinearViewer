 ...
 * one, even the near-universal `orderedList {order: 1}` — cannot survive a
 * write. This is the rule that catches the review's `orderedList {order: 5}`
 * (attrs dropped, the list silently renumbering 5,6 → 1,2) and its
 * `heading {level: 7}` (clamped to 6).
 *
 * `order` is refused rather than modelled ON PURPOSE, per the human decision:
 * "refuse … e.g. `orderedList` with `order ≠ 1` UNTIL THE WRITER MODELS THE
 * ATTRIBUTE". Modelling it means the reader numbering from `attrs.order` and
 * the writer reading the first list number back, which changes what
 * hand-authored `5. foo` means — a behaviour change outside this cycle.
 */
 ...
 *   - `attrs` the writer cannot re-emit (`WRITER_EMITTED_ATTRS` — an
 *     `orderedList` `order`, a heading `level` outside 1-6, a padded
 *     `codeBlock` language: LIN-1886 review F1 sub-class (b));
