...
//
// Auth has TWO shapes, forked on `authType` (LIN-1887 Step 3):
//   * Basic (Phase 1, LIN-1885, unchanged) — `email:apiToken` base64, addressed
//     at the tenant's own base URL (`https://<tenant>.atlassian.net`).
//   * OAuth 2.0 3LO (Phase 3, LIN-1887) — a `Bearer` access token addressed at
//     `https://api.atlassian.com/ex/jira/{cloudId}`, the API gateway. The tenant
//     hostname is NOT the request base for this shape; `site` remains only the
//     human-facing URL the provider builds browse links from.
// `fetchImpl` is injectable (proxy-aware fetch / test stub), defaulting to global
// fetch.
//
...
import { jiraOAuthApiBase } from './oauth.js'
...
/**
 * @param {{ authType?: string, email?: string, apiToken?: string, accessToken?: string, cloudId?: string, site?: string, fetchImpl?: Function, sleepImpl?: Function, randomImpl?: Function }} [opts]
 *   authType       — `'oauth'` selects the LIN-1887 3LO shape; anything else (including
 *                    absent) is Phase 1's Basic shape, unchanged.
 *   email/apiToken — Basic-auth credential pair (Basic shape only).
 *   accessToken    — OAuth 2.0 3LO access token (OAuth shape only).
 *   cloudId        — the Atlassian site id the OAuth grant addresses (OAuth shape only).
 *   site           — tenant base URL, e.g. `https://mycompany.atlassian.net`. The
 *                    request base for the Basic shape; human-facing only for OAuth,
 *                    which is addressed by `cloudId` through the API gateway.
 ...
 */
export function createJiraClient({ authType, email, apiToken, accessToken, cloudId, site, fetchImpl, sleepImpl, randomImpl } = {}) {
  ...
  // LIN-1887 Step 3: the two auth shapes differ in BOTH halves — the request base
  // and the Authorization header — so they fork together, here, once.
  //
  // OAuth is addressed through the API gateway by cloudId, never the tenant
  // hostname. `jiraOAuthApiBase` hard-codes the host, encodes the cloudId, and
  // asserts the parsed hostname — the OAuth-shape equivalent of what
  // `normalizeJiraSite` is for the tenant shape, which does NOT apply here (its
  // `.atlassian.net`-only rule would reject `api.atlassian.com`).
  const baseUrl = isOAuth
    ? jiraOAuthApiBase(cloudId)
    : String(site || '').replace(/\/+$/, '')
  ...
  // reintroduce the vulnerability by skipping that validation. Kept for BOTH
  // shapes: it is cheap, and the OAuth base has its own stricter guard above
  // rather than instead.
  if (!/^https:\/\//i.test(baseUrl)) {
    throw new Error(`createJiraClient: site must be an https:// URL, got: ${baseUrl || '(empty)'}`)
  }

  // Branches on `authType`, NOT on the presence of `email` (LIN-1887 Step 3).
  // `linkProvider`'s credentials merge is `{...existing, ...credentials}`, which
  // can make a key falsy but never absent — so an upgraded binding that "cleared"
  // `email` yields `{email: undefined}`. That happens to fall into the
  // `!email || !apiToken` miss below and drop the header, which is a clean 401
  // rather than a leak, but by coincidence rather than design. An explicit
  // discriminator makes the outcome stop depending on a falsiness accident.
  function authHeader() {
    if (isOAuth) {
  ...
  return {
    ...
    /**
     * `GET /rest/api/3/project/{projectIdOrKey}/statuses` (LIN-2018) — the
     * project's REAL per-issue-type workflow statuses. A plain JSON array, NOT
     * paginated (`List<IssueTypeWithStatus>`, one element per issue type, each
     * carrying `statuses: [StatusDetails...]`) — there is no `startAt`/
     * `nextPageToken` to walk here, unlike every other `listAll*` method in this
     * file. Goes through the shared `request()` helper so the bounded 429 retry/
     * `Retry-After` ceiling/timeout apply unchanged — this adds no second retry
     * or backoff path. Requires Browse Projects on the target project.
     * Flattening across issue types, deduping by status id, and synthesizing a
     * `position` (this endpoint carries none) are the PROVIDER's job, not the
     * client's — mirrors the client/provider split every other read here uses.
     */
    async getProjectStatuses(projectIdOrKey) {
      return request('GET', `/rest/api/3/project/${enc(projectIdOrKey)}/statuses`)
    },
    ...
    /**
     * `POST /rest/api/3/search/approximate-count` (LIN-2155) — the
     * measurement oracle for per-tier coverage: how many issues a JQL query
     * matches, without walking and paging through them. Approximate and can
     * move mid-sync (Jira's own documented behavior) — callers tolerate
     * small churn, never treat it as an exact count. Goes through the shared
     * `request()` helper so the 429/`Retry-After`/timeout discipline applies
     * unchanged — no second retry path.
     */
    async searchApproximateCount(jql) {
      const result = await request('POST', '/rest/api/3/search/approximate-count', { body: { jql } })
    ...
    /**
     * `GET /rest/api/3/field` (LIN-2011) — every field defined on the site
     * (id, name, schema), used to discover a company-managed tenant's legacy
     * `Epic Link` custom field id (team-managed tenants carry the epic link
     * natively in `fields.parent` and never need this). A flat JSON array,
     * not paginated.
     */
    async listFields() {
