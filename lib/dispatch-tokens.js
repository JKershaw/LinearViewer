 ...
 * - Token validation uses crypto.timingSafeEqual for defense-in-depth against timing attacks
 */
...
export class DispatchTokenStore {
   ...
   */
  async createToken(urlKey, label = 'default') {
    if (!urlKey) {
   ...
   * @returns {Promise<string|null>} Workspace URL key if valid, null otherwise
   */
  async validateToken(token) {
    ...
    try {
      ...
      // Defense in depth: timing-safe comparison of hashes
      const storedHashBuffer = Buffer.from(doc.tokenHash, 'hex');
