...
/**
 * Walk the same blocker/parent reachability BFS that promotion (below) uses,
 * seeded from `shipCards` (in-progress), over the FULL unfiltered card set.
 *
 * Shared by computeProximityRings (ring promotion) and buildSegments (backlog
 * exemption, LIN-1208) so the two questions — "should this be promoted to the
 * innermost ring" and "should this backlog card be exempt from hiding" — use
 * IDENTICAL relation semantics and cannot drift apart. buildSegments calls
 * this over the pre-filter `cards` array specifically because a card the
 * card-level filter removes can never be rescued by computeProximityRings'
 * own BFS afterward (it never reaches the orbit at all).
 *
 * @param {Array} allCards   Every card in this render (orbit candidates + shipCards)
 * @param {Array} shipCards  Started cards — the BFS seed
 * @returns {Set<string>}  ids of non-ship cards reached via the transitive
 *   blocksIds chain or parentId walk
 */
export function computeShipReachableIds(allCards, shipCards = []) {
  const shipIds = new Set(shipCards.map(c => c.id));
  const candidateById = new Map();
  for (const card of allCards) {
    if (!shipIds.has(card.id)) candidateById.set(card.id, card);
  }
  ...
  while (queue.length > 0) {
    ...
    for (const blockerId of blockersOf.get(id) || []) {
      if (!visited.has(blockerId) && candidateById.has(blockerId)) {
        visited.add(blockerId);
        reachable.add(blockerId);
    ...
    if (card?.parentId && !visited.has(card.parentId)) {
      ...
      if (candidateById.has(card.parentId)) reachable.add(card.parentId);
...
export function computeProximityRings(orbitCards, shipCards = []) {
  ...
  //                  *outward* from Y.
  //
  // Any orbit card reached this way drops to the innermost ring. Delegated to
  // computeShipReachableIds (LIN-1208) so this walk and buildSegments' backlog
  // exemption walk can never drift apart — this call only ever sees `orbitCards`
  // as candidates, same as before the extraction.
  const reachable = computeShipReachableIds([...orbitCards, ...shipCards], shipCards);
  for (const id of reachable) {
    if (orbitById.has(id)) ring.set(id, INNERMOST_RING);
  }
...
export function buildSegments(cards, config = {}) {
  const bugLabels = (config.bugLabels || ['bug']).map(s => s.toLowerCase());
  const heading = config.heading || null;
  const showBacklog = config.showBacklog === true;
  ...
  // post-routing pass over every bucket — routing above is unaffected, so a
  // backlog card still lands in its heading/bug/project/drift bucket before
  // being dropped from it here. Exempts any backlog card that transitively
  // blocks, or is an ancestor of, in-progress work, via the SAME reachability
  // walk computeProximityRings uses for ring promotion (computed once, over
  // the full unfiltered `cards`, since a card dropped here can never be
  // rescued by that walk afterward — it would never reach the orbit at all).
  if (!showBacklog) {
    const reachable = computeShipReachableIds(cards, shipCards);
    const keepCard = (card) => card.stateType !== 'backlog' || reachable.has(card.id);
    headingCards = headingCards.filter(keepCard);
    bugCards = bugCards.filter(keepCard);
    driftCards = driftCards.filter(keepCard);
    for (const [name, group] of byProject) {
      byProject.set(name, group.filter(keepCard));
    }
    ...
    // emptiness, not by "every card is backlog" (a group can end up holding
    // only an exempt backlog card, which must survive per the exemption
    // above). skipBacklogProjects has its own opt-out, independent of
    // showBacklog, for this cleanup specifically.
    const skipBacklogProjects = config.skipBacklogProjects !== false;
    if (skipBacklogProjects) {
      for (const [name, group] of [...byProject]) {
        if (group.length === 0) byProject.delete(name);
      }
    }
  }
...
/**
 * First-paint fit zoom (LIN-1221 F1). The radial canvas is far larger than the
 * viewport, so centring on the ship at zoom=1 leaves most cards off-canvas —
 * badly on a 390px phone, where the graph reads as an empty void. Pick the
 * largest scale at which the whole content box fits the available viewport (with
 * padding), never zooming IN past `maxZoom` (default 1) and never below
 * `minZoom`. Pure: no DOM. Mirrored in public/common.js (window.computeFitZoom).
 *
 * @param {Object} opts
 * @param {number} opts.contentWidth   content bounding-box width (canvas px)
 * @param {number} opts.contentHeight  content bounding-box height (canvas px)
 * @param {number} opts.availWidth     usable viewport width (px)
 * @param {number} opts.availHeight    usable viewport height (px)
 * @param {number} [opts.pad=24]       inset kept clear on each edge (px)
 * @param {number} [opts.minZoom=0.3]  hard floor for the fit
 * @param {number} [opts.maxZoom=1]    never zoom in past this on fit
 * @returns {number} zoom scale in [minZoom, maxZoom]
 */
export function computeFitZoom({
  ...
  // Degenerate inputs (no content, zero viewport) → neutral 1.
  if (!(contentWidth > 0) || !(contentHeight > 0) ||
      !(availWidth > 0) || !(availHeight > 0)) {
    return Math.min(1, maxZoom);
  }
  const usableW = Math.max(1, availWidth - 2 * pad);
  const usableH = Math.max(1, availHeight - 2 * pad);
  const raw = Math.min(usableW / contentWidth, usableH / contentHeight);
