...
const COLOR_TOKENS = [
  ...
  // Doc-vocabulary aliases (LIN-756): map theme-design.md names onto the existing
  // palette via var(), so they resolve to whatever the active theme defines.
  { name: '--amber', value: 'var(--yellow)', kind: 'fill' },
  { name: '--slate', value: 'var(--fg-mid)', kind: 'fill' },
  { name: '--muted', value: 'var(--bg-muted)', kind: 'fill' },
  // Net-new surface/elevation ramp (LIN-756): inset < bg < card < raised. Richer
  // per-primitive demos land with the rest of the styleguide coverage in LIN-758.
  { name: '--inset', value: '#eceef1', kind: 'fill' },
  { name: '--card', value: '#f4f5f7', kind: 'fill' },
  { name: '--raised', value: '#ffffff', kind: 'fill' }
];
...
// Standardized transition tokens (LIN-757). Demoed as a hover-driven box so the
// timing is visible; richer per-primitive coverage lands in LIN-758.
const TRANSITION_TOKENS = [
  { name: '--transition-fast', value: '0.12s ease' },
  { name: '--transition-base', value: '0.15s ease' }
];
...
export function renderStyleguide() {
  ...
  const transitionBoxes = TRANSITION_TOKENS.map(({ name, value }) =>
    `<figure class="sg-swatch">
        <div class="sg-box" style="transition: transform var(${name})"></div>
        <figcaption><code class="sg-name">${name}</code><span class="sg-value">${value}</span></figcaption>
      </figure>`).join('\n        ');
  ...
  //
  // LIN-758: every new primitive from LIN-754 — the canonical StatusPill (C, the
  // full state union incl. done-with-warning + stale) plus the 7 surface-D
  // primitives (Surface, Tag, Chip, Button, AccentBar, SegmentBar, Disclosure) —
  // is demoed here, so each is exercised inside ALL THREE theme panels below.
  const themePanel = (themeClass, label) => {
    ...
    return `<div class="sg-theme-panel ${themeClass}">
        ...
        <p class="sg-prim-label">StatusPill — full state union</p>
        <div class="sg-theme-row">
          ${renderStatusPill({ state: 'done', label: 'done' })} ${renderStatusPill({ state: 'in-progress', label: 'in progress' })} ${renderStatusPill({ state: 'todo', label: 'todo' })} ${renderStatusPill({ state: 'backlog', label: 'backlog' })} ${renderStatusPill({ state: 'failed', label: 'failed' })} ${renderStatusPill({ state: 'done-with-warning', label: 'done · warning' })} ${renderStatusPill({ state: 'stale', label: 'stale' })}
        </div>
        <p class="sg-prim-label">Tag / Chip</p>
        <div class="sg-theme-row">
          <span class="tag">tag</span>
          <span class="chip">chip<button class="chip__remove" type="button" aria-label="remove">×</button></span>
        </div>
        <p class="sg-prim-label">Button</p>
        <div class="sg-theme-row">
          <button class="btn" type="button">button</button>
          <button class="btn btn--primary" type="button">primary</button>
          <button class="btn btn--danger" type="button">danger</button>
        </div>
        <p class="sg-prim-label">Disclosure</p>
        <div class="sg-theme-row">
          <button class="disclosure" type="button" aria-expanded="false"><span class="disclosure__caret">▸</span> disclosure</button>
        </div>
        <p class="sg-prim-label">AccentBar</p>
        <div class="accent-bar">accent bar — state-coloured left rule</div>
        <p class="sg-prim-label">SegmentBar — per-phase progress</p>
        <div class="segment-bar" role="img" aria-label="segment bar: complete, running, queued, error">
          <span class="segment" data-state="complete"></span>
          <span class="segment" data-state="running"></span>
          <span class="segment" data-state="queued"></span>
          <span class="segment" data-state="error"></span>
        </div>
        <p class="sg-prim-label">Surface — elevation ramp</p>
        <div class="surface surface--raised">surface · raised</div>
        <div class="surface surface--inset">surface · inset</div>
  ...
  <main class="legal-content styleguide-content">
    ...
    ${sgSection('Transitions', `<p class="sg-intro" style="margin-bottom: var(--space-3)">Standardized transition tokens (LIN-757). Hover a box to see its timing; the new design-system primitives share these durations and <code>prefers-reduced-motion</code> neutralises them globally.</p>
      <div class="sg-grid">
        ${transitionBoxes}
      </div>`)}
...
└─ sibling</pre>`)}
    ...
    ${sgSection('Themes', `<p class="sg-intro" style="margin-bottom: var(--space-3)">The same components rendered under each theme so the impact of different themes is visible at-a-glance. A theme is a pure override of the color tokens via the <code>.theme-*</code> hook (<code>public/style.css</code>); the structural tokens stay shared and the default (light) is <code>:root</code>, so every existing page renders unchanged.</p>
      <div class="sg-theme-grid">
        ...
        ${themePanel('theme-amber', 'amber (.theme-amber)')}
      </div>`)}
  </main>
  ${footerHtml}`
  });
