/**
 * lib/agent-turn.js — the Flight Companion turn, extracted from the
 ...
 *
 * LIN-2966: a fourth caller, `routes/task-chat.js`, drives this same core with
 * its own prompt (`deps.buildMessages`) and its own op kind (`opKind`), rather
 * than re-deriving a second turn loop. Task Chat is always `turnKind:
 * 'user-initiated'`, so it never touches the reservation/gate machinery below
 * (that stays auto-wake/boot-only) — it only needed the model-resolution,
 * credential-adjacent, tool-catalog and streaming seam, all of which were
 * already caller-neutral. Flight Companion's own call sites pass neither
 * param and get today's behaviour back unchanged.
 *
 ...
 * @param {'user-initiated'|'auto-wake'|'boot'} p.turnKind - derived by the CALLER (message presence, or the endpoint itself for a boot), never client-asserted
 ...
 * @param {string|null} [p.model] - LIN-2623 beat 2: an optional per-turn model
 *   override. The CALLER has already validated it against the curated
 *   AVAILABLE_MODELS allow-list (an uncurated id never reaches here — the
 *   route 400s before calling in). Ignored on free tier: the `forceDefault`
 *   clamp must keep winning over an explicit user choice (LIN-513).
 * @param {'execute'|'propose'} [p.followUpMode] - defaults from `turnKind`
 * @param {string} [p.opKind] - LIN-2966: the `resolveAiOperationModel` op kind
 *   AND the `callMeta.feature` attribution tag, both derived from this ONE
 *   value so a surface's per-operation Settings override and its cost/KPI
 *   attribution can never drift apart. Defaults to `'flight-companion'`,
 *   byte-identical to before this param existed — the browser and proxy
 *   routes never pass it. A second surface (Task Chat: `'task-chat'`) passes
 *   its own op kind here rather than the core re-deriving one.
 * @param {string|null} [p.issueIdentifier] - LIN-2966: additive attribution
 *   merged into `callMeta`, mirroring `via`. Absent → `callMeta` carries no
 *   observable field change (`lib/llm-call-log.js` already normalizes a
 *   missing key the same as an explicit `null`). Task Chat passes the row's
 *   identifier so `summarizeByIssue` keeps working for chat-attributed calls.
...
export async function runAgentTurn({
  ...
  const {
    ...
    // LIN-2967: passed straight through to `createToolCatalog` untouched —
    // this core has no opinion on tiers or subsets, it just forwards whatever
    // the caller supplied. Absent for every existing caller (Flight
    // Companion's routes never pass either), so `createChatToolCatalog`
    // receives `undefined` for both, exactly as before these existed.
    scopeByTier, includeTiers, includeTools,
  } = deps;
  ...
  if (turnKind === 'boot') {
    // LIN-2620 rebase: `instanceKeySuffix` threaded here too, matching the
    // auto-wake branch above — a no-op for today's only boot caller (the
    // browser route, which never passes one), but leaving it out here would
    // silently defeat LIN-2620's isolation guarantee for any future caller
    // that DID want a boot turn scoped to a non-browser instance.
    const companionInstanceKey = `${COMPANION_INSTANCE_PREFIX}${workspace.urlKey}${instanceKeySuffix}`;
    const sweepInstanceKey = `${SWEEP_INSTANCE_PREFIX}${workspace.urlKey}`;
    const companionDocEnvelope = await observerStateStore.ensureSeeded(companionInstanceKey, COMPANION_SEED_STATE);
    currentCensusDoc = await observerStateStore.readCurrent(sweepInstanceKey);
  ...
  // the free-tier clamp rather than re-deriving it. LIN-2623 beat 2 threads
  // the per-turn override into this SAME site rather than adding a second
  // resolver: a non-free-tier caller's explicit (already-validated) choice
  // wins outright, and a free-tier caller's choice is ignored altogether so
  // the forceDefault clamp — resolved via resolveAiOperationModel exactly as
  // before — always has the final say.
  const selectedModel = (model && !isFreeTier)
    ? model
    : await resolveAiOperationModel({
        urlKey: workspace.urlKey, workspacePreferencesStore, forceDefault: isFreeTier, opKind,
      });
  // LIN-2966: `deps.buildMessages` lets a second surface supply its own prompt
  // (Task Chat: `buildTaskChatMessages`) instead of the Flight Companion brief.
  // The FC branch is left byte-identical to before this param existed — not
  // routed through a shared default — so neither browser nor proxy route
  // needs to change, and Flight Companion's own turn stays provably unaltered.
  const messages = deps.buildMessages
    ? deps.buildMessages({
        history,
        message,
        censusSeedText: deps.buildCensusSeedText ? deps.buildCensusSeedText(currentCensusDoc) : null,
        now: now(),
        turnKind,
        playbook: currentPlaybook,
      })
    : buildFlightCompanionMessages({
        history,
        message,
  ...
  const callMeta = { urlKey: workspace.urlKey, feature: opKind, issueIdentifier, ...(via ? { via } : {}) };
  ...
  if (isToolCapableModel(selectedModel)) {
    const { tools, executeTool: catalogExecuteTool } = createToolCatalog({
      ...
      // LIN-2967: forwarded as given — `undefined` for every caller that
      // doesn't pass one (Flight Companion's routes), reproducing today's
      // single-pair catalog exactly.
      scopeByTier,
      includeTiers,
      includeTools,
      recapCacheStore,
