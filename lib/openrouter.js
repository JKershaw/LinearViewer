 ...
 * NOT the LLM call log (that is retrospective spend, not a rate) — it is co-located
 * here so the same human edit that adds a model adds its rate.
 *
 * POINT-IN-TIME RATES — verified against https://openrouter.ai/api/v1/models on
 * 2026-07-04. OpenRouter prices change; re-verify against that endpoint when editing
 * this list, and treat these as indicative, not billed. Omit `pricing` for a model
 * whose rate is unknown — the UI degrades to "pricing: —" rather than lying.
 */
export const AVAILABLE_MODELS = [
  { id: 'openai/gpt-5.4-mini', name: 'GPT-5.4 Mini', description: 'Default - validated, fast and cheap', pricing: { prompt: 0.75, completion: 4.50 } },
  { id: 'anthropic/claude-sonnet-4.6', name: 'Claude Sonnet 4.6', description: 'Safe higher tier (mid frontier)', pricing: { prompt: 3.00, completion: 15.00 } },
  { id: 'anthropic/claude-opus-4.8', name: 'Claude Opus 4.8', description: 'Safe frontier (most expensive)', pricing: { prompt: 5.00, completion: 25.00 } },
  { id: 'openai/gpt-5.5', name: 'GPT-5.5', description: 'Frontier - newer, higher cost', pricing: { prompt: 5.00, completion: 30.00 } },
  { id: 'openai/gpt-5.5-pro', name: 'GPT-5.5 Pro', description: 'Frontier Pro - highest tier, most expensive', pricing: { prompt: 30.00, completion: 180.00 } },
];
...
export async function streamChatWithTools(messages, options = {}, onEvent) {
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration++) {
    const { message } = await runToolHop(convo, {
    ...
    if (toolCalls.length === 0) {
      // Model is ready to answer — stop hopping and stream the final answer.
      break;
    }
    ...
    for (const call of toolCalls) {
      ...
      try {
        ...
        resultText = truncateToolResult(raw, resultMaxChars);
        ...
        resultText = truncateToolResult(`Error: ${errMessage}`, resultMaxChars);
