...
import { isTerminalState, compareByIdentifier, computeFrontierFacts } from './tree.js';
...
function buildMetaPrompt(issue, context, featureFlags = {}, providerUi = null) {
  ...
  // FRONTIER FACTS (LIN-433): deterministic child-derived facts surfaced into the
  // meta-prompt so the model stops re-deriving them at the defer-vs-breakdown fork.
  // Reuses the SAME selectFocusSubtask + isBlocked the descent uses (computeFrontierFacts),
  // plus a light session-fit extraction of the plan's canonical phrase (non-authoritative;
  // the model still reads the plan). Mirrored in the handwritten path via formatFrontierFacts.
  const frontierFacts = hasSubtasks ? computeFrontierFacts(children) : null;
  if (frontierFacts) frontierFacts.sessionFit = extractSessionFit(issue.description);

  // Completion branch (LIN-353, unified): when a task's substantive work is already
  // finished and there is no open child to descend into — its own state is terminal
  // (Done/Canceled/Duplicate) OR every subtask is terminal — Step 0 routes to
  // `review`/close, never a no-op `look-into`. A terminal task that still has open
  // children keeps descending to those children instead. `review` is no longer gated
  // to those terminal cases: Step 3 also routes an implemented-but-still-open leaf to
  // `review` from the implementation completion signals (so a merged-but-In-Progress
  // task stops looping `implementation`). Routing lives only in this meta-prompt path;
  // the handwritten path mirrors the terminal-completion *content* via
  // formatTerminalStateNote / formatChildrenCompleteNote.
  const isTerminal = isTerminalState(issue.state?.type);
  const hasOpenChildren = remainingCount > 0;
 ...
 */
function extractSessionFit(description) {
...
export async function getRecommendationStream(issue, context, options = {}, onEvent) {
  ...
  try {
    ...
    const structured = parseRecommendationResponse(rawContent, finishReason, completionTokens);
...
export async function getRecommendation(issue, context, options = {}) {
  ...
  try {
    ...
    return parseRecommendationResponse(content, finishReason, completionTokens);
  } catch (error) {
    if (error.name === 'AbortError') {
      throw new Error('OpenRouter request timed out');
    }
