...
function buildMetaPrompt(issue, context, featureFlags = {}) {
  ...
  // Terminal-state branch (LIN-353): a Done/Canceled/Duplicate task with no open
  // children gets a `review`/close recommendation, never a no-op `look-into`. A
  // terminal task that still has open children keeps descending to those children.
