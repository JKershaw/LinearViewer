...
import { fetchProjects as defaultFetchProjects } from './linear.js';
...
function isLeaf(stackTask) {
  ...
  return subtasks.every(s => s.stateType === 'completed' || s.stateType === 'canceled');
}
...
export async function buildPipelineSnapshot(urlKey, deps = {}) {
  ...
  // Partition: active = hasActiveLoop && (isLeaf || hasRecentOwnLoop).
  const activeEntries = [];
  const queueEntries = [];
  for (const entry of rolled) {
    const { task, stackTask } = entry;
    const active = hasActiveLoop(task);
    if (active && (isLeaf(stackTask) || hasRecentOwnLoop(task, nowMs))) {
      activeEntries.push(entry);
    } else if (!active) {
      // No open loop at all → candidate for the "ready to start" queue.
      // Parents and leaves alike; the filter is "no open loop".
      queueEntries.push(entry);
    }
    // The "else" case (has loop but not leaf and no recent own-loop) falls
    // out of both — a parent that is only decoratively associated with an
    // in-flight child. The child itself carries the cell.
  }
  ...
  const recent = allLoops
    .filter(l =>
      (l.agentState === 'complete' || l.agentState === 'error') &&
      l.resolvedAt &&
      (() => {
