 ...
 * `getWorkspaceAccessToken`, `dispatchStore`, `agentStatusStore`, and optionally
...
import {
  getLoopsForWorkspace as defaultGetLoopsForWorkspace,
  getLoopsForIssue as defaultGetLoopsForIssue
} from './pipeline-loops.js';
import {
  buildForest,
  partitionCompleted,
...
function _buildStack(projects, issues) {
  ...
  // The pipeline floor's "recent" lane means recently *finished* work, so it
  // keeps the legacy completion-only feed; the multi-kind expansion (LIN-490) is
  // scoped to the dashboard's Recent activity section.
  const recentActivityTrees = buildRecentActivityForest(issues, localProjects, 1, { kinds: ['completed'] });
...
export async function buildPipelineSnapshot(urlKey, deps = {}) {
  ...
  if (!dispatchStore || !agentStatusStore) {
    throw new Error('pipeline-state: dispatchStore and agentStatusStore must be injected');
  }
  ...
  const [projectsResult, allLoops] = await Promise.all([
    ...
    getLoopsForWorkspace(urlKey, { dispatchStore, agentStatusStore })
  ]);
 ...
 * @param {Object} [deps]    - like `buildPipelineSnapshot`, but reads loops via
 *                             the issue-scoped `getLoopsForIssue` (no whole-
 *                             workspace download)
...
export async function getTaskForIssue(urlKey, identifier, deps = {}) {
  const {
    ...
    agentStatusStore,
    fetchProjects = defaultFetchProjects,
    getLoopsForIssue = defaultGetLoopsForIssue,
    now = Date.now()
  } = deps;
  ...
  // Issue-scoped read: the loop reader pushes `issueIdentifier` down into the
  // store queries (LIN-613/LIN-615), so we never download the whole-workspace
  // 30-day log just to keep one issue's rows.
  const [projectsResult, loopsForIssue] = await Promise.all([
    fetchProjects(accessToken),
    getLoopsForIssue(urlKey, identifier, { dispatchStore, agentStatusStore })
  ]);
