...
export const INDEX_SPECS = [
  ...
  {
    collection: 'llm-call-log',
    keySpec: { urlKey: 1, issueIdentifier: 1, expiresAt: 1 },
    options: {},
    reason: 'per-issue call-log summary (summarizeByIssue, LIN-1775 task cost)'
  },
  {
  ...
  // --- Observer harness state (LIN-2129) ---
  {
    ...
    keySpec: { updatedAt: 1 },
    options: {},
    reason: 'cleanup() range scan for decommissioned observer instances (lib/observer-state-store.js, LIN-2129) — the _id (instance key) point read needs no index, per the deliberately-not-indexed precedent above'
  }
]
