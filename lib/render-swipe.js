 ...
 * Order: completed/canceled last, bugs first, then by state, then by priority.
 *
...
export function sortIssuesForSwipe(issues) {
  ...
  issues.sort((a, b) => {
    const aCompleted = a.stateType === 'completed' || a.stateType === 'canceled';
    const bCompleted = b.stateType === 'completed' || b.stateType === 'canceled';
    ...
    const aState = stateOrder[a.stateType] ?? 1;
    const bState = stateOrder[b.stateType] ?? 1;
...
export function applyBlockingOrder(issues) {
  ...
  // Skip edges from completed/canceled blockers (blocking is resolved).
  const adj = new Map();
...
export function clusterByParent(issues) {
  ...
  for (const issue of issues) {
    ...
    // Collect the full family tree from the root, depth-first
    // Subtasks before parent = reverse of parent-first traversal
    const cluster = [];
    function collectFamily(nodeId) {
      if (!issueById.has(nodeId) || placed.has(nodeId)) return;
      const children = childrenOf.get(nodeId) || [];
      // Collect children first (subtasks before parent = unblocking order)
      for (const childId of children) {
        collectFamily(childId);
      }
      cluster.push(issueById.get(nodeId));
    }
    collectFamily(rootId);
...
export function renderSwipePage(data, options = {}) {
  ...
  const { deployInfo = {}, urlKey = null, openRouterSource = null, workspaces = [], featureFlags = {}, customPrompts = [], initialIdentifier = null } = options;
  ...
  const swipeData = {
    ...
    customPrompts: customPrompts.map(p => ({ id: p.id, name: p.name })),
    urlKey: urlKey || '',
    initialIdentifier,
  ...
  <main class="swipe-page" data-url-key="${encodedUrlKey}">
    ...
    <div class="swipe-prompts" id="swipe-prompts">
      ...
      <div class="swipe-prompt-result hidden" id="swipe-prompt-result">
        ...
        <div class="swipe-reasoning-toggle hidden" id="swipe-reasoning-toggle"></div>
        <div class="swipe-reasoning-content hidden" id="swipe-reasoning-content"></div>
