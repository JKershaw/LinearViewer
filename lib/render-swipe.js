 ...
 * @returns {Array} Flat array of {issue, projectName, section}
 */
function flattenTrees(trees, type) {
  const items = [];

  function walkNode(node, projectName) {
  ...
  if (type === 'project') {
    for (const { project, incomplete } of trees) {
      for (const node of incomplete) {
        walkNode(node, project.name);
      }
    }
  } else if (type === 'in-progress') {
    for (const { projectName, roots } of trees) {
      for (const node of roots) {
        // Only include issues that are actually in-progress (not ancestor context)
        if (node.isInProgress) {
          walkNode(node, projectName);
        } else {
          // Walk children to find in-progress descendants
          for (const child of node.children || []) {
            walkInProgress(child, projectName, items);
          }
        }
      }
    }
    ...
    for (const { roots, projectName } of trees) {
      for (const node of roots) {
        items.push({
          ...
          priority: node.issue.priority || 0,
          url: node.issue.url || '',
          stateType: node.issue.state?.type || 'completed',
          stateName: node.issue.state?.name || '',
          labels: (node.issue.labels?.nodes || []).map(l => l.name),
          projectName: node.projectName || projectName || '',
          completedAt: node.issue.completedAt || null,
          dueDate: node.issue.dueDate || null,
...
/**
 * Walk tree nodes, only collecting in-progress issues
 */
function walkInProgress(node, projectName, items) {
  if (node.isInProgress) {
    items.push({
      id: node.issue.id,
      identifier: node.issue.identifier || '',
      ...
      priority: node.issue.priority || 0,
      url: node.issue.url || '',
      stateType: node.issue.state?.type || 'unstarted',
      stateName: node.issue.state?.name || '',
      labels: (node.issue.labels?.nodes || []).map(l => l.name),
      projectName: projectName,
      completedAt: node.issue.completedAt || null,
...
/**
 * Build filter groups from the project trees
 * @returns {Array} [{key, label, issueIds}]
 */
function buildFilterGroups(allIssues, projectTrees) {
  const groups = [];
...
export function renderSwipePage(data, options = {}) {
  ...
  const filterGroups = buildFilterGroups(allIssues, projectTrees);
...
<body>
  ...
  ${footerHtml}
  <script>window.__SWIPE_DATA__ = ${JSON.stringify(swipeData)};</script>
