...
export function renderTemplateCard(template) {
  ...
  // Migrated onto the canonical `card` component (LIN-468): the wrapper gains
  // `.card` chrome, `.prompt-card` survives as a no-style E2E/JS hook, and the
  // header converges onto `.card-header` while keeping `.prompt-header` as a
  // hook. Pill/meta children (`.prompt-label`, `.prompt-chars`) are untouched —
  // their styling is LIN-465's.
  return renderCard({
    className: 'prompt-card',
    body: `<div class="card-header prompt-header">
 ...
 */
export function renderTemplatesByCategory(templates) {
...
export function renderPromptsPage(workspaceName = 'Workspace', options = {}) {
  ...
  return renderPage({
    ...
    content: `${renderPageHeader({ title: 'Prompts', subtitle: 'Prompt templates for AI-assisted task workflows', subtitleClass: 'prompts-subtitle' })}
