 ...
 *   - two large primary CTAs: Linear (always) and GitHub. The GitHub CTA is
 *     gated on `githubEnabled` (process.env.GITHUB_CLIENT_ID), mirroring how
 *     `/auth/github` 503s and `renderLoginPage` hides the button when the GitHub
 *     App is unconfigured (LIN-541). When false the GitHub CTA is omitted
...
export function renderLandingHero({ githubEnabled = false } = {}) {
  ...
  return `<header class="landing-hero" data-testid="landing-hero">
    ...
    <h1 class="landing-wordmark">harbour<span class="landing-wordmark-accent" aria-hidden="true">.</span></h1>
