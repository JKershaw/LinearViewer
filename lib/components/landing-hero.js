 ...
 *     CTA is gated on `githubEnabled`, threaded in by the caller (LIN-2010:
 *     `getProvider('github').entryCta.isConfigured()`, the provider-owned
 *     `isGitHubConfigured()` predicate — not an inline `process.env` read),
 *     mirroring how `/auth/github` 503s and `renderLoginPage` hides the button
 *     when the GitHub App is unconfigured (LIN-541). When false the GitHub CTA
 *     is omitted entirely — never a dead link.
...
export function renderLandingHero({ githubEnabled = false, jiraEnabled = false } = {}) {
  // LIN-2010: the boolean gate stays a threaded param (zero signature change —
  // callers still decide whether the CTA is live), but the href is now sourced
  // from the registry rather than a literal, so it can't drift from entryCta.href.
  const githubCta = githubEnabled
    ? `<a href="${getProvider('github')?.entryCta?.href}" class="landing-cta landing-cta-github" data-testid="landing-cta-github">Continue with GitHub</a>`
    : '';
  const jiraCta = jiraEnabled
    ? `<a href="${getProvider('jira')?.entryCta?.href}" class="landing-cta landing-cta-jira" data-testid="landing-cta-jira">Continue with Jira</a>`
    : '';
