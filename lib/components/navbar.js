 ...
 * @property {'projects'|'settings'|'audit'|'prompts'|'dispatch'|'pipeline'|'proxy'|'foreman'|'roadmap'} [currentPage] - Current page identifier
...
export function renderNavBar({ workspaces = [], urlKey = null, currentPage = 'projects', teams = [], selectedTeamId = null, featureFlags = {}, isLanding = false }) {
  // Unauthenticated landing pages get a minimal nav with a sign-in CTA
  if (isLanding) {
    return `
 ...
 * @param {Workspace[]} workspaces - Array of connected workspaces
 * @param {string|null} urlKey - Current workspace URL key from URL
 * @param {'projects'|'settings'|'audit'|'prompts'|'dispatch'|'pipeline'|'proxy'|'foreman'|'roadmap'} currentPage - Current page to preserve on switch
...
function renderWorkspaceOptions(workspaces, urlKey, currentPage = 'projects') {
  ...
  // Feature-flagged pages (dispatch, proxy, foreman, roadmap, pipeline) may not be
  // enabled on the target workspace, so fall back to projects for those.
  const featureFlaggedPages = new Set(['dispatch', 'proxy', 'foreman', 'roadmap', 'pipeline'])
  ...
  const localAddRow = `
    <div class="nav-options-row">
      ...
      <form action="/workspace/new" method="POST" class="nav-option-form">
        <button type="submit" class="nav-option nav-option-add"><span class="option-marker-placeholder"></span>+local workspace</button>
      </form>
    </div>`
