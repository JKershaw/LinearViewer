...
export function renderNavBar({ workspaces = [], urlKey = null, currentPage = 'projects', teams = [], selectedTeamId = null, assignees = [], selectedAssignee = null, canFilterByMe = false, featureFlags = {}, isLanding = false, minimalNav = false }) {
  ...
  // Feedback trigger (LIN-2298). Lives in nav chrome, NOT as a fixed FAB.
  //
  // John's ruling on LIN-2298: the floating trigger was a `position: fixed`
  // element over full-width content, and LIN-2272's result is that NO reserve
  // clears such an element at every scroll offset once the content spans the
  // column (the Observation feed's `.obs-session` cards do, by design — the
  // card's `open ↗` control measured as covered at 26 consecutive offsets).
  // Moving the trigger into normal-flow chrome retires the defect CLASS rather
  // than reserving around its third instance. The panel it opens may still be
  // an overlay: a modal the user asked for is the user's own choice.
  //
  // The gate is `urlKey && featureFlags.feedbackWidget === true` — exactly the
  // condition under which the FAB used to render, so this is a relocation and
  // not a reach change. Both halves matter: the widget's own mount
  // (`renderFeedbackMount`, lib/components/footer.js) is gated on
  // `!isLanding && urlKey`, and the widget only BUILDS client-side when the
  // per-user flag is on. A trigger without a panel would be a dead control, and
  // a panel without a trigger would be unreachable — tests/unit/navbar-feedback-
  // trigger.test.js pins the two gates as one contract so they cannot drift.
  const feedbackTrigger = (urlKey && featureFlags.feedbackWidget === true) ? renderFeedbackTrigger() : ''
  ...
  <nav class="nav-bar" aria-label="Main navigation">
    <div class="nav-primary-row">
      ...
      <div class="nav-actions">
        ...
        ${rulingsBadge}
        ${feedbackTrigger}
        ${/* Nav-actions order (LIN-1149, extended LIN-1728, LIN-2298): search
             (page-specific, projects only) before the queue badge before the
             rulings badge (both feature-gated, all pages), with the feedback
             trigger (LIN-2298, gated on its own flag) trailing. On non-projects
             pages the badge(s)/trigger are the sole nav-action(s). Must not leak
             search onto non-projects pages and must not gate search on dispatch. */''}
      </div>
    </div>
    ${viewNav}
  </nav>
...
/**
 * Render the feedback trigger that opens the feedback panel (LIN-2298).
 *
 * Replaces the fixed `.feedback-fab` this ticket deleted. A plain `<button>` in
 * normal flow inside `.nav-actions`: it is a control, not a navigable link, so
 * it is NOT routed through `getViewNavLinks`/`renderViewNav` — those emit
 * anchors whose `text` is compared against `currentPage` for active-match and
 * active-hoist, and a non-navigating entry there would have to be special-cased
 * through both, for no gain. The `⋯ more` overflow the ruling offers as an
 * alternative is where flag-gated VIEWS collapse at narrow widths; the trigger
 * instead rides `.nav-actions`, which is already the home of the other two
 * flag-gated non-navigation controls (queue and rulings badges).
 *
 * Carries no `urlKey`: unlike the badges it polls nothing and navigates
 * nowhere. `public/feedback-widget.js` binds it by testid and reads the
 * workspace off the widget mount's own `data-url-key`, so the trigger stays a
 * pure affordance and the widget keeps a single source of workspace identity.
 *
 * `aria-expanded` starts `false` and is maintained by the widget as the panel
 * opens/minimizes (it was on the FAB before), which is what makes this a
 * disclosure control rather than an unlabelled button.
 *
 * SHIPS DISABLED, and public/feedback-widget.js enables it once it has bound
 * the click handler and built the panel. This is not defensive dressing — it
 * closes a race the FAB could not have: the FAB was CREATED by the widget
 * script, so its very existence proved the handler was attached, whereas this
 * button is server-rendered and is on screen from first paint while
 * `feedback-widget.js` is still a deferred script that has not run. A click
 * landing in that window would find no listener and be silently swallowed —
 * measured, not theorised: it is what made the LIN-2298 e2e run hang on a
 * click that opened nothing. `disabled` makes the gap honest (the control
 * genuinely cannot work yet) and observable (Playwright's actionability check
 * waits for enabled, so tests block on readiness instead of racing it).
 *
 * @returns {string} HTML for the feedback trigger
 */
function renderFeedbackTrigger() {
  return `<button type="button" class="nav-action nav-feedback-trigger" data-testid="nav-feedback-trigger" aria-expanded="false" aria-label="Give feedback" title="Give feedback" disabled>feedback</button>`
}
 ...
 */
export function renderWorkspaceOptions(workspaces, urlKey, currentPage = 'projects') {
  if (!workspaces?.length) return ''
  ...
  // Registry-driven add rows (LIN-2802): one row per provider that declares
  // an `entryCta`, gated by that provider's own `entryCta.isConfigured()` and
  // labelled from `ui.displayName` \u2014 so a newly-registered provider with an
  // entry CTA appears here without a second hand-edit to this file. Row order
  // follows registry (barrel-import) order: linear, github, jira today.
  //
  // Linear's `entryCta.isConfigured()` is unconditionally `true` by design
  // (see linear/index.js) \u2014 it does not encode the PAT gate, so the existing
  // `hasPAT` hide stays a separate, orthogonal filter applied only to the
  // Linear row, exactly as it was when hand-written.
  //
  // No `role="option"` on these rows (existing e2e selectors, e.g.
  // tests/e2e/jira-provider.spec.js, rely on add rows staying excluded from
  // `[role="option"]` queries) \u2014 matches the pre-existing Linear/Local rows.
  const providerAddRows = getAllProviders()
    .filter(p => p.entryCta)
    .filter(p => p.entryCta.isConfigured())
    .filter(p => !(p.name === 'linear' && hasPAT))
    .map(p => `
    <div class="nav-options-row">
      <span class="option-prefix">\u251c\u2500</span>
      <a href="${p.entryCta.href}" class="nav-option nav-option-add" data-testid="nav-workspace-add-${p.name}"><span class="option-marker-placeholder"></span>+ ${escapeHtml(p.ui.displayName)}</a>
    </div>`).join('\n    ')

  // Local-workspace create has no `entryCta` (it declares none by design \u2014
  // its door is `POST /workspace/new`, not a provider login route) and keeps
  // its bespoke, hand-written row. Shown REGARDLESS of PAT/OAuth config \u2014
  // local onboarding is independent of Linear auth (LIN-377). Always the last
  // row, so it alone gets the closing `\u2514\u2500` prefix.
  const localAddRow = `
    <div class="nav-options-row">
      <span class="option-prefix">\u2514\u2500</span>
