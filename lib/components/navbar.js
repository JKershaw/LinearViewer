 ...
 * @property {'projects'|'settings'|'audit'|'prompts'|'dispatch'|'proxy'} [currentPage] - Current page identifier
 ...
 */
export function renderNavBar({ workspaces = [], urlKey = null, currentPage = 'projects', teams = [], selectedTeamId = null, featureFlags = {} }) {
  ...
  // Proxy link when feature is enabled (not shown on the proxy page itself)
  const proxyLink = (urlKey && featureFlags.proxy === true && currentPage !== 'proxy')
    ? `<a href="/workspace/${encodeURIComponent(urlKey)}/proxy" class="nav-action">proxy</a>`
    : ''
 ...
 * @param {Workspace[]} workspaces - Array of connected workspaces
 * @param {string|null} urlKey - Current workspace URL key from URL
 * @param {'projects'|'settings'|'audit'|'prompts'|'dispatch'} currentPage - Current page to preserve on switch
...
function renderWorkspaceOptions(workspaces, urlKey, currentPage = 'projects') {
  ...
  // Dispatch/proxy pages may not be enabled on target workspace, so fall back to projects
  const pageSuffix = (currentPage === 'projects' || currentPage === 'dispatch' || currentPage === 'proxy') ? '/' : `/${currentPage}`
