 ...
 * @property {'projects'|'settings'|'audit'|'prompts'|'dispatch'} [currentPage] - Current page identifier
...
function renderWorkspaceOptions(workspaces, urlKey, currentPage = 'projects') {
  ...
  const pageSuffix = (currentPage === 'projects' || currentPage === 'dispatch' || currentPage === 'proxy' || currentPage === 'roadmap') ? '/' : `/${currentPage}`
  ...
  // Hide +add in PAT mode (OAuth may not be configured)
  const addRowHtml = hasPAT ? '' : `
