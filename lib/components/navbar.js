 ...
 * (public/app.js) against `GET .../api/dashboard/rulings`, not the queue
 * badge's 1s `QUEUE_POLL_INTERVAL_MS`.
 * @param {string} urlKey - Current workspace URL key
 * @returns {string} HTML for rulings badge
 */
function renderRulingsBadge(urlKey) {
  return `<button class="rulings-badge hidden" data-rulings-badge data-url-key="${escapeHtml(urlKey)}" aria-label="Waiting on you"><span class="rulings-count">0</span> waiting on you</button>`
}
