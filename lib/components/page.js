...
/**
 * Pre-paint theme bootstrap (LIN-785).
 *
 * Emitted as the first thing in `<head>` so it runs BEFORE first paint and the
 * page never flashes light-then-dark (FOUC-free). It reads the persisted `theme`
 * cookie (written by the global toggle / seeded from UserPreferencesStore at
 * login) and adds `theme-dark` to `<html>` when the user chose dark. Default —
 * no cookie, or any value other than `dark` — is light (no class). This is the
 * ONE seam that makes the toggle global without threading a `theme` param
 * through all ~30 renderPage call sites. Kept tiny, inline, and try/guarded so a
 * cookie-less or restricted environment degrades silently to light.
 */
const THEME_PREPAINT_SCRIPT =
  `<script>(function(){try{var m=/(?:^|;\\s*)theme=(dark|light)/.exec(document.cookie);if(m&&m[1]==='dark'){document.documentElement.classList.add('theme-dark');}}catch(e){}})();</script>`;
...
export function renderPage({
  ...
  const headLines = [
    '<head>',
    '  <meta charset="UTF-8">',
    // Theme bootstrap runs before paint, ahead of the title/stylesheets, so the
    // persisted dark preference is applied with no flash of the light default.
