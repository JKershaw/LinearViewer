...
/**
 * Pre-paint global theme bootstrap (LIN-756).
 *
 * Emitted as the FIRST thing in every `<head>` (before any stylesheet link) so it
 * runs synchronously before first paint — the no-FOUC guarantee across navigation.
 * It resolves the active theme from two inputs and applies the matching class to
 * `document.documentElement` (the `.theme-*` selectors are class-based + self-painting,
 * so they work on `<html>` and never clobber a per-page `<body class>`):
 *
 *   1. `localStorage.theme` — written instantly by the settings toggle, so the
 *      choice survives navigation on the same device with zero flash.
 *   2. a `theme` cookie — the server-seeded fallback for an authenticated load on a
 *      device whose localStorage is empty (e.g. fresh login). Set from durable
 *      `preferences.theme` at the save route and the auth callback.
 *
 * Values are theme KEYS ('light' | 'dark' | 'amber'), not class strings, so an
 * explicit 'light' choice (class '') is distinguishable from "unset" and correctly
 * overrides a stale dark cookie. This keeps `renderPage` a pure template with no
 * session/preference access — the contract gap is bridged on the client, not by
 * threading a resolved `bodyClass` through every renderer.
 */
const THEME_BOOTSTRAP = '<script>(function(){try{' +
  "var t=localStorage.getItem('theme');" +
  "if(!t){var m=document.cookie.match(/(?:^|; )theme=([^;]*)/);if(m){t=decodeURIComponent(m[1]);}}" +
  "var c={dark:'theme-dark',amber:'theme-amber'}[t];" +
...
export function renderPage({
  ...
  const headLines = [
    '<head>',
    '  <meta charset="UTF-8">',
    // Theme bootstrap runs before any stylesheet so first paint is themed (LIN-756).
    `  ${THEME_BOOTSTRAP}`,
