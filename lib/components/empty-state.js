 ...
 * across the app (`.custom-prompts-empty`, `.roadmap-empty`, `.foreman-empty` /
 * `.foreman-now-empty`, `.swipe-card-empty`, `.pipeline-{queue,grid,activity}-empty`).
 *
 * The five per-page looks genuinely DIVERGE — only `color: var(--fg-dim)` is
 * universal; padding, alignment, borders (foreman dashed), font-family (roadmap
 ...
 * deliberately NOT deleted in A.6 — three of them (`custom-prompts`, `pipeline`,
 * `swipe`) are load-bearing client-JS contracts, and convergence onto one look
 ...
 * any leading glyph such as pipeline's `○` is part of the text). `tag` is the
 * wrapper element (`div` default; roadmap + pipeline need `p`). `className` adds
 * the retained per-page variant class(es) (and e.g. pipeline's initial `hidden`).
 * `id` and `attrs` extend the wrapper (the client-contract / E2E-hook escape
 * hatch — pipeline toggles `.hidden` by id).
 *
 * @param {Object} opts
 * @param {string} opts.text - Placeholder copy (escaped). Required.
 * @param {string} [opts.tag='div'] - Wrapper tag (e.g. `p` for roadmap/pipeline).
 * @param {string} [opts.className] - Retained per-page variant class(es).
 * @param {string} [opts.id] - Wrapper id (client contract, e.g. pipeline toggles by id).
