 ...
 * (e.g. `tag` for a neutral, stateless label chip; `bare` for a box-less inline
 * glyph that reproduces the legacy `.state` tree-row look pixel-for-pixel — used
 * by the LIN-782-locked project tree, which forbids pill/card chrome on its
 * rows). `className`/`attrs` extend
 ...
 * @param {string} [opts.variant] - Extra modifier name, e.g. `tag` (neutral chip) or `bare` (box-less inline glyph).
