...
export function renderPageFooter(options = {}) {
  ...
  // Feedback widget (LIN-635). Authenticated, workspace-scoped pages get a
  // footer-resident toggle plus the widget mount + assets. The toggle persists
  // through the SAME per-user `feedbackWidget` flag and `/settings/features`
  // endpoint as the Settings toggle (one source of truth — no second store);
  // the floating widget itself only renders client-side when the flag is on.
  const feedbackHtml = (!isLanding && urlKey)
    ? renderFeedbackToggle(urlKey, featureFlags) + renderFeedbackMount(urlKey, featureFlags)
    : '';
...
/**
 * Renders the footer-resident feedback toggle control (LIN-635 S2).
 *
 * This is net-new UI, but NOT a new persistence mechanism: clicking it POSTs to
 * the existing `POST /workspace/:urlKey/settings/features` endpoint with the
 * `feedbackWidget` flag, exactly like the Settings-page toggle. The two controls
 * therefore read/write one source of truth (`userPreferencesStore`).
 *
 * @param {string} urlKey - Current workspace URL key
 * @param {Object} featureFlags - Current feature toggle states
 * @returns {string} HTML for the footer toggle
 */
function renderFeedbackToggle(urlKey, featureFlags = {}) {
  const on = featureFlags.feedbackWidget === true;
  const icon = on ? '●' : '○';
  return `
    <div class="footer-feedback-toggle-wrap">
      <a href="#" class="footer-action footer-feedback-toggle" role="switch"
         data-testid="footer-feedback-toggle"
         data-url-key="${escapeHtml(urlKey)}"
         data-enabled="${on ? 'true' : 'false'}"
         aria-checked="${on ? 'true' : 'false'}"
         title="Toggle the feedback widget">feedback: ${icon}</a>
    </div>`;
}
