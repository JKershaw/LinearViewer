...
function renderAiStatus(source, urlKey) {
  ...
  if (source === 'free') {
    return `<a href="${settingsUrl}" class="footer-ai-status ${statusClass}" data-ai-source="free" title="OpenRouter: ${statusText}">ai: ${statusIcon} free</a>${modelHtml}`;
  }

  return `<a href="${settingsUrl}" class="footer-ai-status ${statusClass}" title="OpenRouter: ${statusText}">ai: ${statusIcon}</a>${modelHtml}`;
}
...
function getFooterLinks(urlKey, featureFlags = {}) {
  ...
  if (featureFlags.proxy === true) {
    ...
    links.push({ href: `${prefix}/foreman`, text: 'foreman' });
  }
