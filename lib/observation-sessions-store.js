...
// `lane` present.
// v7 (LIN-1789): `telemetry.resources` is now parsed (peakRssBytes,
// hostMemAvailableBytes, hostMemTotalBytes, hostSwapUsedBytes, oomKillDelta,
// loadAvg1, cpuCount, activeSessionCount, cloneDiskBytes, cloneCount — all
// optional, numeric-only). Like v6, this genuinely ADDS A KEY rather than
// changing a value inside an existing field, so a v6 doc no longer matches the
// new shape — a real shape change, not another efficacy lever over a doc that
// was stale-but-not-wrong. Bump so those docs read-miss and rebuild with
// `resources` present.
// v8 (LIN-2113): `telemetry.usage.cacheCreation1hInputTokens` is now parsed
// (USAGE_NUMBER_FIELDS), and `telemetry.usage.costUsd` is now derived by
// splitting `cacheCreationInputTokens` across the 5m/1h cache-write tiers
// instead of pricing the whole total at the 5m rate. This trips BOTH prior
// bump triggers at once: it genuinely ADDS A KEY like v6/v7, and it changes
// the DERIVED VALUE of an existing field (`costUsd`) like v5. Bump so those
// docs read-miss and rebuild with the new field present and the repriced
// `costUsd`, rather than silently serving a stale-shape, understated cost for
// up to 30 days.
// v9 (LIN-2182): loops now carry pre-derived `decision`/`decisionCase` beside
// `wakeMarker`/`waitingMessage`. Like v6/v7, this genuinely ADDS A KEY rather
// than changing a value inside an existing field, so a v8 doc no longer
// matches the new shape — a real shape change, not another efficacy lever
// over a doc that was stale-but-not-wrong. Bump so those docs read-miss
// (`findByWorkspace`, `getSession`) and rebuild live (`upsertSession`) with
// `decision`/`decisionCase` present, rather than silently serving a doc with
// NEITHER key at all for up to `DEFAULT_HISTORY_TTL` (30 days).
// v10 (LIN-1728): loops now also carry pre-derived `answeredDecisionId`
// beside `decision`/`decisionCase`. Same v9 rationale — a genuinely new key,
// so a v9 doc's embedded loops silently lack it. Bump so materialized session
// docs (whose `renderInlineReplyBox`/transcript rendering reads this path)
// read-miss and rebuild with the field present, rather than serving stale
// docs that read every decision as permanently unanswered.
// v11 (LIN-2403): `computeUsageCostUsd` now models OpenRouter's long-context
// `pricing.overrides` tiers, so `telemetry.usage.costUsd` is DERIVED
// differently for a usage row on an override-bearing model once its prompt
// crosses a threshold — repriced (opencode, per-request) or now `null` rather
// than a silent base-rate guess (claude-code/unknown, cumulative). Same
// character as v5/v8: a value inside an existing field changes, not a new
// key, so a v10 doc still matches the shape and would otherwise keep serving
// a stale-and-now-wrong `costUsd` for up to 30 days. Bump so those docs
// read-miss and rebuild with the repriced (or honestly null) figure.
// v12 (LIN-3011, LIN-2996 Phase 3): the lean loop's terminal/wake/decision/
// decisionCase/answeredDecisionId/parkedWait/telemetry/toolPeak now derive
// from a self-healed `feedbackDigest` instead of a raw-feedback re-scan, and
// `telemetry.metrics`/`lineageMetrics` are retention-trimmed (R4) rather than
// the full heartbeat list. Same character as v9/v10: a v11 doc was built the
// old way, so without the bump it would keep serving pre-digest-backed values
// for up to 30 days.
// v13 (LIN-3022, LIN-2991 Surface 2): loops now also carry the set-derived
// `answeredDecisions` field beside the scalar `answeredDecisionId` — same v10
// rationale, a genuinely new key, so a v12 doc's embedded loops silently lack
// it. Bump so materialized session docs read-miss and rebuild with the field
// present. This bump covers only the materialized session docs this store
// writes — it does NOT protect the lean rulings/census/dashboard read path
// (`routes/proxy-rulings.js`'s `readRulings`, which never touches this
// store): that path's freshness is `isFreshDigest`/`feedbackVersion`
// (`lib/digest-feedback.js:290-294`), and a pre-v13 lean digest with no
// `answeredDecisions` key is protected by `_loopFactsFromDigest`'s own
// legacy fallback (`lib/pipeline-loops.js`), not by this version number.
// v14 (LIN-2934): the assembled session now carries `taskPosition`/
// `sessionPosition` ("n of N" against the run's `maxTasks`/`maxSessionsPerTask`
// bounds, derived read-time from the session's loops). Same v9/v10 character:
// a genuinely new key, so a v13 doc silently lacks it. Bump so materialized
// session docs (whose Observation feed chip reads this path) read-miss and
// rebuild with the position fields present, rather than serving a stale doc
// that never shows the running count for up to 30 days.
// v15 (LIN-2891 / LIN-3034): loops now also carry the item-scoped `withdrawal`
// field (`{decisionId, reason, timestamp}` or `null`, derived by
// `_findDecisionWithdrawal` in lib/digest-feedback.js) beside
// `answeredDecisions`. Same v10/v13/v14 rationale: a genuinely new key, so a
// v14 doc's embedded loops silently lack it. Bump so materialized session
// docs read-miss and rebuild with the field present, rather than serving a
// stale doc with no `withdrawal` key at all for up to 30 days. Same v13 scope
// caveat: this bump covers only the materialized session docs this store
// writes — it does NOT protect the lean rulings path (which never touches
// this store): that path's freshness is `isFreshDigest`/`feedbackVersion`
// (`lib/digest-feedback.js`), and a pre-v15 digest with no `withdrawal` key
// is already read as `null` by `_loopFactsFromDigest`'s `?? null`
// (`lib/pipeline-loops.js`), not by this version number.
// v16 (LIN-2891 F1): the `withdrawal` field's DERIVATION became decision-aware
// — `_findDecisionWithdrawal(feedback, decision.decision_id)` now returns only
// the live withdrawal for the loop's CUMULATIVE (last) `decision_id`, so a
// later withdrawal of a moot/older decision can no longer replace it and
// reopen a live withdrawn ruling. The field's SHAPE is unchanged (still
// `{decisionId, reason, timestamp}` or `null`), but a v15 doc's value was
// computed under the old "last live withdrawal on the row, whatever its
// decision" rule and can therefore be wrong for its own current decision.
// Bump so materialized session docs read-miss and rebuild under the new
// derivation. Same v13 scope caveat as v15: a fresh-by-feedbackVersion lean
// digest whose `withdrawal` predates this fix is NOT covered here; it
// self-heals on the row's next feedback write (which recomputes the digest),
// and until then it can only over-report an OPEN ruling, never a lost one.
