 ...
 * @param {boolean} [params.noDescend=false] - Deterministic leaf-target lever (LIN-365).
 *   When true the resolver never follows a `defer`: it resolves the start node in a
 *   single hop and returns whatever that node recommends. Paired with the context
 *   layer suppressing `focusedChild` (so the model recommends the node's OWN work),
 *   this lets a caller drive a parent's work even while it has an open child — instead
 *   of the descent routing into that child. This is the hard guarantee: even if the
 *   model still emits a `defer`, the child is never fetched or dispatched.
 * @param {() => number} [params.now=Date.now] - Injectable clock (for tests).
...
export async function resolveRecommendation({
  ...
  for (let depth = 0; ; depth++) {
    ...
    // noDescend (LIN-365): treat the start node as terminal regardless of what it
    // recommends — never follow a defer into a child. The single deferredVia entry
    // (the start node) signals "no descent happened" to callers exactly as a leaf does.
    const deferring = !noDescend && rec.recommendedAction === 'defer' && !!rec.deferTo;
