...
export function parseLandingPage(filePath) {
  ...
  try {
    ...
    console.error(`Failed to read landing page file: ${filePath}`, err.message);
    // Return minimal fallback content
    return {
      organizationName: 'Linear Projects',
      projects: [{
        id: 'error-project',
        name: 'Configuration Error',
        description: 'Landing page content could not be loaded.',
        state: 'started',
        sortOrder: 0
      }],
      issues: []
  ...
  for (const block of projectBlocks) {
    ...
    // Find description (blockquote) and project-level metadata
    let description = '';
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (line.startsWith('> ')) {
        ...
        // Project-level metadata (e.g., @collapsed: true)
        const match = line.match(/^@(\w+):\s*(.+)$/);
        projectMeta[match[1]] = match[2];
      } else if (line.trim() === '') {
        // Skip empty lines
        continue;
      } else if (line.startsWith('- ')) {
        // Found first issue, stop parsing project metadata
        issueStartIndex = i;
        break;
      } else {
        issueStartIndex = i;
    ...
    projects.push({
      ...
      collapsed: projectMeta.collapsed === 'true'
    });
  ...
  return {
    organizationName: frontmatter.title || 'Projects',
    projects,
    issues
  };
}
...
function parseIssues(lines, projectId) {
  ...
  while (i < lines.length) {
    ...
    // Parse blocking relationships (@blocks: comma-separated issue IDs)
    const relations = metadata.blocks
      ? { nodes: metadata.blocks.split(',').map(id => ({ type: 'blocks', relatedIssue: { id: id.trim() } })) }
      : { nodes: [] };
    ...
    issues.push({
      ...
      state,
      priority: metadata.priority ? parseInt(metadata.priority, 10) : 0,
