 ...
 *
 * LIN-2790: this store also carries ANSWER proposals now (`proposedOutcome:
 * 'answered'`, with an `optionId`), written by `routes/proxy-rulings.js`'s
 * `suggest-answer` route. The name `suggestedDismissal` on the wire is a
 * legacy holdover now covering both proposal kinds (LIN-2757 owns the
 * vocabulary rename). The boundary this store enforces is unchanged either
 * way: the proxy may never write `agreed`, `agreedAt`, or `acceptedAt` on its
 * own proposal row. `suggest()`'s accepted-field set is frozen at exactly
 * `{proposedOutcome, optionId, reason, decisionLoopId, suggestedBy,
 * suggestedAt, withdrawn}`.
 *
...
import { buildRulingRef } from './unanswered-decisions.js';
...
function toRecord(doc) {
  ...
  return {
    ...
    proposedOutcome: doc.proposedOutcome ?? 'dismissed',
...
export class DismissalSuggestionsStore {
   ...
   * @param {string} [entry.proposedOutcome] - LIN-2790: `'dismissed'` (default,
   *   when omitted — every pre-existing caller keeps this shape byte-for-byte)
   *   or `'answered'`. The store is a dumb writer: it does not validate that
   *   `optionId` is required/meaningful for `'answered'`, or that it names a
   *   real option — that belongs to the route (routes/proxy-rulings.js).
   * @param {string} [entry.optionId] - LIN-2790: meaningful only when
   *   `proposedOutcome === 'answered'`; defaults to `null`.
   ...
   */
  async suggest({ urlKey, decisionId, decisionLoopId, proposedOutcome, optionId, reason, suggestedBy, now } = {}) {
    ...
    try {
      ...
      const doc = {
        ...
        proposedOutcome: proposedOutcome ?? 'dismissed',
 ...
 * @returns {Array<Object>} rows, each additionally carrying `suggestedDismissal:
 *   {reason, suggestedBy, suggestedAt, proposedOutcome, optionId}|null`. LIN-2790:
 *   this is the ONE read path that defaults an absent `proposedOutcome` to
 *   `'dismissed'` — a legacy row (written before this field existed) reads as
 *   exactly the ordinary dismissal proposal it always was, byte-compatible.
 */
export function attachStandingSuggestions(rows, suggestions) {
  ...
  for (const s of suggestions || []) {
    ...
    if (s.decisionLoopId) {
      byKey.set(buildRulingRef({ urlKey: s.urlKey, loopId: s.decisionLoopId, decisionId: s.decisionId }), s);
    } else {
  ...
  return rows.map(row => {
    ...
    const scoped = byKey.get(buildRulingRef({ urlKey, loopId: loopKey, decisionId }));
    ...
    return {
      ...
      suggestedDismissal: suggestion
        ? {
            ...
            proposedOutcome: suggestion.proposedOutcome ?? 'dismissed',
            optionId: suggestion.optionId ?? null
          }
        : null
    };
  });
