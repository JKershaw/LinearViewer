...
export class DismissalSuggestionsStore {
   ...
   *   the standing suggestion happens to be stored — see the two-tier lookup
   *   below, which mirrors the read side (`attachStandingSuggestions`,
   *   `shelfGate`) exactly, since a Keep targets whatever suggestion is
   *   currently DISPLAYED on the row, not a guess at its storage shape.
  ...
  async withdraw({ urlKey, decisionId, decisionLoopId, now } = {}) {
    if (!this.collection || !urlKey || !decisionId) return null;
    try {
      // LIN-2766: the loop-scoped `_id` first (a suggestion proposed against
      // this exact row), falling back to the legacy/workspace-wide
      // two-segment `_id` — the SAME two-tier order `attachStandingSuggestions`
      // reads with. Without this fallback, a legacy suggestion that the read
      // side fans out to every loop's row (and whose banner + Keep button
      // therefore render there) could never be found by the write side a
      // press of that same Keep button addresses — a silent, permanent no-op
      ...
      const existing = (scopedId && await this.collection.findOne({ _id: scopedId }))
        || await this.collection.findOne({ _id: legacyId });
 ...
 * row. Mirrors `lib/unanswered-decisions.js`'s `shelfGate` two-tier match
 * exactly: a row's own loop-scoped suggestion (`${urlKey}::${loopKey}::
 * ${decisionId}`) wins when one exists; otherwise a workspace-wide,
 * `decisionLoopId`-less suggestion (`${urlKey}::${decisionId}`, legacy or
 * deliberately wide) applies — see the module docstring's back-compat note.
 * A withdrawn suggestion is never standing (a human already pressed Keep),
 * so it is excluded here rather than in the store, which returns raw rows so
 * exactly one place — this one — owns the predicate.
 *
...
export function attachStandingSuggestions(rows, suggestions) {
  ...
  for (const s of suggestions || []) {
    if (s.withdrawn || !s.urlKey || !s.decisionId) continue;
    if (s.decisionLoopId) {
  ...
  return rows.map(row => {
    ...
    const suggestion = (loopKey && byKey.get(`${urlKey}::${loopKey}::${decisionId}`))
      || byLegacyKey.get(`${urlKey}::${decisionId}`)
      || null;
    return {
      ...row,
