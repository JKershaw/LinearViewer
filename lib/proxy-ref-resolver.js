...
//      LIN-561 `source` vocabulary, and reject any namespace not in the input
//      allowlist. This allowlist is still Linear-only, so only `linear:` (or a
//      bare ref) is accepted; LIN-544 relaxes it when providers coexist and
//      inherits the collision-safe parser for free. (This is the input *namespace*
//      gate, NOT provider selection — proxy provider selection is per-workspace
//      as of LIN-581; the two are deliberately sequenced apart.)
...
import { SOURCE_LINEAR, SOURCE_GITHUB, SOURCE_LOCAL, SOURCE_JIRA } from './providers/models.js';
...
// NOTE this list is already incomplete before this addition — SOURCE_GITHUB_PROJECTS
// (models.js) has never been added here, live precedent that "registrable but
// not active" is an accepted, ordinary state, not a gap unique to Jira. Adding
// SOURCE_JIRA has exactly ONE effect: it widens SOURCE_PREFIX_RE below so a
// `jira:FOO-1` ref parses as a namespace and gets a clean 422 "not active"
// (rather than being misread as a bare local ref) — it does NOT turn `jira:`
// refs live. That is ACTIVE_SOURCES below, which deliberately stays
// Linear-only (LIN-1885 Phase 1 is read-only via the provider layer, not via
// this proxy-ref input namespace; turning it on is a separate, LIN-544-style
// decision).
const KNOWN_SOURCES = [SOURCE_LINEAR, SOURCE_GITHUB, SOURCE_LOCAL, SOURCE_JIRA];

// The `<source>:` namespace prefixes accepted on input refs. Still Linear-only
// (per LIN-306/LIN-544 sequencing): only `linear:` is live; the rest are reserved
// and rejected with a clean 422 until the input-namespace relaxation lands. This
// is independent of provider *selection*, which is per-workspace as of LIN-581 —
// a non-Linear workspace still selects its own provider for the write; what stays
// fixed here is only which namespace prefixes the input parser will honour.
 ...
 * exact raw id → symbolic type alias / literal name. The id check
 * short-circuits BEFORE the alias/name sweep (mirrors resolveProjectRef) so a
 * non-UUID native state id (e.g. a future Jira `stateId`) matches directly
 * without risking a spurious ambiguity if it happens to collide with another
 * state's alias or name.
...
export function resolveStateRef(states, rawRef) {
  ...
  const idMatch = (states || []).find(s => s.id === ref);
...
/**
 * Resolve a project reference. Order: UUID → exact raw id → case-insensitive
 * name. The id check short-circuits BEFORE the name sweep (never merged into
 * one filter) — Local's composite ids (`${urlKey}-proj-1`) and GitHub's
 * milestone-number-string ids are real non-UUID ids, and merging the two
 * checks would throw a spurious NOT_UNIQUE when one project's id equals
 * another's name.
...
export function resolveProjectRef(projects, rawRef) {
  ...
  const idMatch = (projects || []).find(p => p.id === ref);
  if (idMatch) return idMatch.id;
