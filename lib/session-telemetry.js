 ...
 *                        nullable `costUsd` (null for the `claude-code`
 *                        harness, which has no native cost field; pricing is
 *                        a deliberate follow-up, not derived here).
 */
...
function parseUsagePayload(message) {
  ...
  usage.costUsd = typeof payload.costUsd === 'number' ? payload.costUsd : null;
  return usage;
}
