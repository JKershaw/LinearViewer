...
import { ISSUE_ID_REGEX } from './workspace.js';

// LIN-3037 (Surface 11): the 3 decision-lifecycle stamp kinds
// (`lib/digest-feedback.js`'s `isDecisionLifecycleStampEntry`), inlined here
// rather than imported — `digest-feedback.js` already imports
// `buildRunTelemetry`/`deriveRuntime`/`parseDecision`/`__internal` FROM this
// module, so an import back would close that cycle. One module-level set,
// shared by both `parseHeartbeats` and `parseParkedWait` below.
const DECISION_LIFECYCLE_STAMP_KINDS = new Set([
  'decision-answer', 'decision-withdrawn', 'decision-withdrawal-reversed',
]);
const _isStampEntry = (entry) => DECISION_LIFECYCLE_STAMP_KINDS.has(entry?.kind);
...
export function parseHeartbeats(feedback) {
  ...
  for (const entry of feedback) {
    ...
    // LIN-3037: a decision-lifecycle stamp's message (e.g. a withdrawal's
    // free-text `reason`) can incidentally match HEARTBEAT_HINT the same way
    // decision prose can (LIN-2182 above) — excluded by kind for the same
    // reason.
    if (_isStampEntry(entry)) continue;
 ...
 *
 * LIN-3037: a decision-lifecycle stamp is excluded from deciding "currently" —
 * a stamp posted after the real latest entry must not mask a genuine parked
 * wait, and a stamp sitting inside a parked run must not break the backward
 * walk. The latest NON-STAMP entry decides `latest`; the backward walk passes
 * over stamps (without treating them as a match) so `since` still reaches the
 * earliest entry of the contiguous non-stamp parked run.
 *
 * @param {Array<{message?: string, timestamp?: string}>} feedback
...
export function parseParkedWait(feedback) {
  ...
  let lastIdx = feedback.length - 1;
  while (lastIdx >= 0 && _isStampEntry(feedback[lastIdx])) lastIdx--;
  if (lastIdx < 0) return null;
  const last = feedback[lastIdx];
  if (!PARKED_WAIT_HINT.test(last?.message || '')) return null;
  let sinceIdx = lastIdx;
  for (let i = lastIdx - 1; i >= 0; i--) {
    if (_isStampEntry(feedback[i])) continue;
...
//  - `on_answer` (LIN-2773 Area 1) is the opposite of `if_unanswered`: a
//    VALIDATING, not opaque, object guard. `effect` is checked against the
//    closed `{resume, dispatch, record}` set and `record_on` against the
//    shared `ISSUE_ID_REGEX` (`lib/workspace.js`) — each sub-field drops
//    individually on failure, same as `recommended`, and this parser never
//    throws or nulls the entry because of it. Additive: absent `on_answer`
//    produces a decision byte-identical to before this field existed.
//
...
const ON_ANSWER_EFFECTS = new Set(['resume', 'dispatch', 'record']);

// Validating guard for `decision.on_answer` (LIN-2773 Area 1). Unlike
// `_parseIfUnanswered`'s opaque object guard above, each field here is
// independently validated and FIELD-level dropped on invalid — `effect`
// against the closed vocabulary a rulings row may derive to, `record_on`
// against the same `ISSUE_ID_REGEX` every other issue-id-shaped field in
// this codebase validates against (imported from lib/workspace.js, never
// re-derived). Mirrors `recommended`'s cross-validation below: an invalid
// value drops only itself, never the whole `decision` entry, and this
// helper must never throw. Collapses to `undefined` (field omitted from
// `decision`) when the shape guard fails or no sub-field survives — the
// same "empty collapses to undefined" convention `_parseDecisionOptions`
// uses above, so a decision with no salvageable on_answer content is
// byte-identical to one that never carried the field at all.
function _parseOnAnswer(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const clean = {};
  if (typeof value.effect === 'string' && ON_ANSWER_EFFECTS.has(value.effect)) {
    clean.effect = value.effect;
  }
  if (typeof value.record_on === 'string' && ISSUE_ID_REGEX.test(value.record_on)) {
    clean.record_on = value.record_on;
  }
 ...
 *
 * @param {string} message
 * @returns {{decision_id: string, question?: string, options?: Array<{id: string, label: string, cost?: number}>, recommended?: string, free_text?: boolean, if_unanswered?: Object, on_answer?: {effect?: 'resume'|'dispatch'|'record', record_on?: string}}|null}
 */
export function parseDecision(message) {
  ...
  const onAnswer = _parseOnAnswer(payload.on_answer);
  if (onAnswer) decision.on_answer = onAnswer;
