 ...
 * @param {string} opts.placeholder - Label for the leading empty option that
 *   represents "nothing chosen". Required so the browser can never fall back
 *   to auto-selecting the first real option (F1, LIN-1973 review): the empty
 *   option ships `selected` whenever `selectedId` didn't match a real item,
 *   so "unset" is always representable and never silently coerced into a
 *   real value the user never picked.
 * @returns {string|null}
 */
function renderIdSelect({ id, name, testid, items, selectedId = '', labelFn = (item) => item?.name, placeholder }) {
  if (!Array.isArray(items) || items.length === 0) return null;
  const hasResolvedMatch = items.some(item => selectedId && String(item.id) === selectedId);
  ...
  const placeholderOption = `<option value=""${hasResolvedMatch ? '' : ' selected'}>${escapeHtml(placeholder)}</option>`;
  return `<select class="task-create-input" id="${id}" name="${name}" data-testid="${testid}">${placeholderOption}${options}</select>`;
}
...
function renderStateField(states) {
  ...
  if (Array.isArray(states) && states.length > 0) {
    ...
    // There is no "current" state for a new task, so the placeholder is
    // ALWAYS the selected option here — never a real state chosen implicitly
    // by browser default (F1, LIN-1973 review).
    control = `<select class="task-create-input" id="task-create-stateId" name="stateId" data-testid="task-create-stateId"><option value="" selected>No state</option>${options}</select>`;
  } else {
