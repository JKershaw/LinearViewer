 ...
 *
 * LIN-2361 (plan-review G2): `resolvePromptUi`'s output shape changed (added `fixedStates`), and
 * this function's whole-object `JSON.stringify` equality below is exactly the kind of comparison
 * a shape change can silently move. It does not move here: `fixedStates` is `null` on both sides
 * for every provider that doesn't declare it (the base-class default), and GitHub — the one
 * provider that does declare it — was already DIVERGENT on `displayName` alone, so its verdict is
 * unchanged too. Any future provider that declares `fixedStates` while otherwise matching the
 * Linear floor would newly read divergent (correctly — it IS a capability difference).
 *
