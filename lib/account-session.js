...
export async function establishAccount(session, accountStore, accountWorkspaceStore, provider, scope, credentials, workspaceId) {
  ...
  if (existingOwner) {
    ...
    if (session.accountId && session.accountId !== resolvedOwnerId) {
      ...
      // resolved owner IS the account already live in this session (its
      // owner was merged into it), no conflict is raised — falls straight
      // through to the idempotent re-link below. Deliberate, bounded
      // behavior change (LIN-2285): this suppresses only that same-account
      // no-op confirmation (`mergeAccounts` writes nothing for it either
      // way); a merge offer against any genuinely distinct canonical
      // account is still raised exactly as before.
