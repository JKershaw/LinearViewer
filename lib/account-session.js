 ...
 * candidate), plus the OAuth state/intent riding alongside it. Byte-identical
 * to `respondToAccountConflict`'s (`routes/auth.js`) own non-mergeable
 * branch: NOT a general session-reset helper, and deliberately NOT called on
 * a mergeable `conflict` return, where the id must survive for the
 * merge-confirm flow. `respondToAccountConflict` is left calling its own
 * inline copy rather than this helper — no behavior change intended there.
...
export async function establishAccount(session, accountStore, accountWorkspaceStore, provider, scope, credentials, workspaceId) {
  ...
  // conflict decision and the merge-confirm flow it builds (routes/auth.js's
  // `respondToAccountConflict`), which is what let a later merge attempt
  // offer — and confirm — a merge in the OPPOSITE direction, writing a
  // `mergedInto` CYCLE. Canonicalizing here, at the one seam every sign-in
  // path converges on, means every downstream reader of `session.accountId`
  // sees the true canonical id, not a merged one.
  //
  // Wrapped, exactly as LIN-2234 wraps its own call to the same method inside
  // `resolveWorkspaceAccess` (server.js): `resolveCanonicalAccountId` THROWS
  // on a corrupt `mergedInto` chain (a pre-existing cycle, or one deeper than
  // maxDepth) and on a store failure. Only data corrupted BEFORE the guard
  // above shipped can reach that — no new cycle can be created now — but for
  // such an account an unwrapped call would turn sign-in into a throw where
  // it previously proceeded, and on the entry paths that do not wrap their
  // `establishAccount` call (POST /auth/jira/link) that rejection is
  // unhandled rather than a clean error page. Degrade instead: keep the id
  // uncanonicalized, exactly as before LIN-2265. That is safe because the
  // write-path guard in `mergeAccounts` is independent of this one — a
  // cycle-forming merge built from a stale id still fails closed there.
  if (session.accountId) {
    try {
      session.accountId = await accountStore.resolveCanonicalAccountId(session.accountId);
    } catch (err) {
      console.error(`[account-session] canonical account resolution failed for ${session.accountId}; continuing with the uncanonicalized id:`, err);
    }
  }
