...
function renderInlineReplyBox(loop, urlKey) {
  ...
  return `<div class="sess-inline-reply" ${attrs}>
      <textarea class="sess-inline-reply-input" rows="2" placeholder="Reply to this run…" aria-label="Reply to this run"></textarea>
      <div class="sess-inline-reply-actions">
...
function renderRun(loop, options = {}) {
  const { urlKey = '', canReply = false } = options;
  ...
  const expandedBody = (() => {
    ...
    if (canReply && loop.loopId) {
      pieces.push(renderInlineReplyBox(loop, urlKey));
    }
  ...
  return `<li class="sess-run" data-testid="session-run" data-loop-id="${escapeHtml(String(loop.loopId || ''))}">
...
export function renderSessionPage(data = {}, options = {}) {
  ...
  const runsBody = loops.length
    ? `<ul class="sess-runs">${loops.map(l => renderRun(l, { urlKey, canReply })).join('')}</ul>`
  ...
  return renderPage({
    ...
    nav: navHtml,
    content,
    scripts: ['/common.js', '/purify.min.js', '/marked.min.js', '/brief.js', '/recap.js', '/session.js']
  });
}
