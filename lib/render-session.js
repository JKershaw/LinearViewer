...
export function renderSessionPage(data = {}, options = {}) {
  ...
  return renderPage({
    ...
    nav: navHtml,
    content,
    // The reply box (LIN-1004) is the page's only interactive element — load its
    // tiny scoped script ONLY when the box is present (a no-JS snapshot otherwise).
    scripts: canReply ? ['/session.js'] : []
  });
}
