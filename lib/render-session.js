 ...
 *   - Context   — a clearly-marked placeholder for the brief/recap panels
 *                 (beat 3 fills it; cache-only, never an LLM spend on load).
 *
...
// Brief/recap placeholder — beat 3 fills this with the cache-only join. Marked
// so it is obvious the panel is intentionally deferred, not missing.
function renderContextPlaceholder() {
  return `<div class="sp-context-placeholder" data-testid="session-context-placeholder">`
    + `<p class="sp-muted">Brief &amp; recap context will appear here (cached only — never auto-generated on load).</p>`
    + `</div>`;
}
...
export function renderSessionPage(data = {}, options = {}) {
  ...
  const content = `<main class="sp-page" data-testid="session-page" data-url-key="${encodedUrlKey}" data-session-id="${escapeHtml(sessionId)}">
    ...
    ${renderSection({ className: 'sp-section sp-context-section', title: 'Context', body: renderContextPlaceholder() })}
  </main>
  ${footerHtml}`;
