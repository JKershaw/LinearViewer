...
import { isDecisionAnswerEntry } from './pipeline-loops.js';
 ...
 *
 * Trailing `decision-answer` entries (LIN-1728) are skipped before the
 * last-entry read: the answer stamp is metadata about a decision, not a new
 * status, so a `[blocked]`-then-answered run must not read as un-waiting to
 * this predicate (a stamped answer is a Save/Save-and-continue reply, not a
 * resume — a still-parked run stays parked until the actual resume lands).
 */
function runIsWaiting(loop, supersededLoopIds = EMPTY_SET) {
  ...
  while (i >= 0 && isDecisionAnswerEntry(entries[i])) i--;
  if (i < 0) return false;
  return isBlockedEntry(entries[i]?.message);
}
 ...
 *
 * `data-decision-id` (LIN-1728 Phase 2): the loop's own unanswered `decision`,
 * when present — threaded client-side into `window.ReplyDelivery.deliverReply`
 * (Save-and-continue) and the standalone `postComment` call (Save) so the
 * comment route can best-effort stamp `markDecisionAnswered` alongside the
 * ruling response. Omitted (not emitted as an empty attribute) when the loop
 * carries no decision, so `session.js` can gate the threading on its presence.
 */
function renderInlineReplyBox(loop, urlKey, waiting = false) {
  ...
  const attrs = [
    ...
    loop.decision ? `data-decision-id="${escapeHtml(String(loop.decision.decision_id || ''))}"` : ''
  ].filter(Boolean).join(' ');
...
function renderRun(loop, options = {}) {
  ...
  // NOT guaranteed disjoint from waitingFlag by construction: `runIsWaiting`
  // matches on a `[blocked]`/`[pending]` PREFIX (`BLOCKED_MARKER_REGEX`),
  // while `PARKED_WAIT_HINT` is a substring match anywhere in the text — a
  // message shaped like `[pending] ... a scheduled wakeup ...` would satisfy
  // both. The explicit `!waitingFlag` gate is therefore real disambiguation,
  // not a redundant safety net, and it resolves in favor of blocked-on-a-
  // human, matching this ticket's "distinct from" acceptance criterion.
