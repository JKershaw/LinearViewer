...
export function renderSessionPage(data = {}, options = {}) {
  ...
  // Load all scripts whenever there is content that needs client-side enhancement
  // (feedback for markdown rendering, context panels for Brief/Recap widgets,
  // or reply capability), not just when canReply.
  const hasFeedback = loops.some(l => Array.isArray(l.feedback) && l.feedback.length > 0);
  const needsEnhance = hasFeedback || issueContext.length > 0 || canReply;
  const baseScripts = [
    '/marked.min.js',
    '/purify.min.js',
    '/common.js',
  ...
  const scripts = needsEnhance ? [...baseScripts, '/session.js'] : [];
