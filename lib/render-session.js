 ...
 * fold), or a `session-lineage` container wrapping each constituent run's
 * unchanged `renderRun()` output for a group of two-or-more — still N
 * addressable `session-run` segments with their own `data-loop-id`s, testids
 * and chips, just visually joined under one lineage.
 */
function renderLineageGroup(group, options) {
  ...
  return `<li class="sess-lineage" data-testid="session-lineage" data-lineage-id="${escapeHtml(String(lineageId || ''))}">
        <ul class="sess-lineage-runs">${group.map(l => renderRun(l, options)).join('')}</ul>
      </li>`;
}
...
export function renderSessionPage(data = {}, options = {}) {
  ...
  const runOptions = { urlKey, canReply, waiting, supersededLoopIds };
