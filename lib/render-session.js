...
/**
 * Loop ids superseded by a follow-up within the same session (LIN-1163
 * close-out): a reply to a blocked run creates a NEW loop with `followUpTo`
 * set to the original's id, but the original's own feedback is append-only and
 * never touched — so its last marker stays `[blocked]`/`[pending]` forever.
 * Mirrors `deriveSessionWaiting`'s `supersededLoopIds` construction in
 * `routes/dashboard.js` (same rule, different consumer: that one excludes a
 * superseded loop from the session-wide waiting rollup; this one excludes it
 * from the per-run collapsed-card flag).
 */
function computeSupersededLoopIds(loops) {
