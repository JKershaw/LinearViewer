...
export function createObservationMaterializer({ dispatchStore, agentStatusStore, observationSessionsStore, resolveWorkspaceTitles = null }) {
  ...
  async function _sessionsTouchingIssue(urlKey, issueIdentifier) {
    ...
    for (const row of rows) {
      ...
      // LIN-1307: a follow-up-only row (no sessionId, not autopilot) is invisible
      // to the two checks above — resolve its followUpTo chain to the root
      // anchor and target THAT session instead, mirroring pipeline-loops.js
      // `_buildSessions` pass 2.5's upward `resolveChainRoot`. A standalone/
      // manual root (its own dispatch, no sessionId) is skipped: those sessions
      // stay live-only by design, matching the write-trigger gap-1 fix and the
      // M1 test below.
      if (row.followUpTo) {
