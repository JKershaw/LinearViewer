 ...
 *     the {urlKey, sessionId} index, PLUS every followUpTo descendant of the seed/
 *     workers, discovered via a downward followUpTo BFS — LIN-1307) and build over
 *     that whole closure.
 *
 ...
 * @param {Function} [deps.resolveWorkspaceTitles] - LIN-962/1986: optional
 *   off-hot-path title source, `(urlKey, ownerAccountId) => Promise<Map|Object of
 *   {identifier → title}>`, owner-scoped. When wired, resolves a real task title
 *   onto loops whose dispatch data lacked one, so the Observation Level-2 card
 *   shows a title instead of a bare identifier. Absent (or failing) → no
 *   enrichment; behaviour is byte-identical to before.
...
export function createObservationMaterializer({ dispatchStore, agentStatusStore, observationSessionsStore, resolveWorkspaceTitles = null }) {
  ...
  /**
   * LIN-1986: distinct `dispatchedBy` values among loops actually missing a
   * title, in stable batch order — which is also most-recent-session-first,
   * since `_buildSessions` sorts `sessions` that way before `_enrichTitles` ever
   * sees them (lib/pipeline-loops.js). Title resolution is workspace-scoped, not
   * per-loop-owner-scoped (one `{identifier→title}` map applies to the whole
   * batch), so any candidate here is a legitimate credential owner for this
   * urlKey — "which one" only needs to land on one that actually RESOLVES, which
   * `_enrichTitles` below tries in this order. Returns [] when every candidate
   * loop's `dispatchedBy` is null (agent-status-only-derived loops, or
   * pre-LIN-1948 historical dispatches) — the bounded residual tracked by
   * LIN-2099.
   *
   * @param {Array<Object>} sessions
   * @returns {Array<string>}
   */
  function _deriveOwnerCandidates(sessions) {
    const seen = new Set();
    const candidates = [];
    for (const s of sessions) {
      for (const l of (s.loops || [])) {
        if (l && l.issueIdentifier && !l.issueTitle && l.dispatchedBy && !seen.has(l.dispatchedBy)) {
          seen.add(l.dispatchedBy);
          candidates.push(l.dispatchedBy);
        }
      }
    }
    return candidates;
   ...
   *
   * LIN-1986: `resolveWorkspaceTitles` is now owner-scoped, so the owner is
   * derived here from data already in scope (`_deriveOwnerCandidates`) rather
   * than threaded in from `onWrite`/`backfillWorkspace`. A batch can carry more
   * than one distinct dispatcher; candidates are equally LEGITIMATE (each really
   * did dispatch work against this urlKey) but not equally LIVE (a candidate's
   * own session/token may since have lapsed — the population LIN-1983 is about),
   * so each is tried in order until one actually resolves a non-empty map,
   * rather than stopping at the first non-null candidate.
   *
   * Best-effort and null-safe by construction: no resolver wired, nothing missing,
   * no derivable owner, or a resolver that throws all leave loops exactly as
   * built — degrading to today's identifier-only display, never worse. We only
   * ever populate a REAL, distinct title (never the identifier itself), so the
   * id-once guard can never re-print `LIN-701 LIN-701` (AC-2 held by
   * construction).
   *
   * @param {string} urlKey
  ...
  async function _enrichTitles(urlKey, sessions) {
    ...
    const candidates = _deriveOwnerCandidates(sessions);
    if (candidates.length === 0) return; // no derivable owner — LIN-2099 residual

    let titles = null;
    for (const ownerAccountId of candidates) {
      let result;
      try {
        result = await resolveWorkspaceTitles(urlKey, ownerAccountId);
      } catch (err) {
        console.error('observation materializer title resolve error:', err?.message || err);
        return; // degrade to identifier-only, never worse
      }
      // `Object.keys` doesn't see a Map's entries — resolveWorkspaceTitles may
      // return either shape (see `lookup` below), so check size the same way.
      const resultSize = result instanceof Map ? result.size : (result ? Object.keys(result).length : 0);
      if (resultSize > 0) {
        titles = result;
        break;
      }
  ...
  async function _sessionsTouchingIssue(urlKey, issueIdentifier) {
    ...
    for (const row of rows) {
      if (row.kind === 'autopilot') { targets.add(row.id); continue; }
      if (row.sessionId) { targets.add(row.sessionId); continue; }
      if (row.followUpTo) {
        // LIN-1341: a stamped row's durable group id IS the target session, O(1)
        // — no chain walk needed, and (unlike the legacy walk below) it also
        // covers a follow-up on a standalone/manual root, closing the old skip
        // for that case. The standalone root itself still never leaks into the
        // Autopilot feed: `isStandaloneSession` filters it out at read time
        // regardless of whether the materializer tracks its doc.
        if (row.sessionGroupId) { targets.add(row.sessionGroupId); continue; }
        // LIN-1307 fallback: a pre-LIN-1341 follow-up-only row (no sessionId,
        // not autopilot, no sessionGroupId) is invisible to the two checks
        // above — resolve its followUpTo chain to the root anchor and target
        // THAT session instead, mirroring pipeline-loops.js `_buildSessions`
        // pass 2.5b's upward `resolveChainRoot`. A standalone/manual root (its
        // own dispatch, no sessionId) is skipped here: those sessions stay
        // live-only by design, matching the write-trigger gap-1 fix and the
        // M1 test below.
        const root = await dispatchStore.resolveFollowUpRoot(urlKey, row.id);
        if (root) {
          if (root.kind === 'autopilot') targets.add(root.id);
          else if (root.sessionId) targets.add(root.sessionId);
        }
      }
    }
    return targets;
   ...
   * @param {Map<string, Object>} [anchorsOut] - LIN-2934 R1: when supplied,
   *   receives `sessionId -> anchor` for a GENERAL (goal-only) autopilot
   *   anchor — one whose own row carries no `issueIdentifier`, so it can never
   *   be added to `into` and is otherwise structurally invisible to
   *   `getSessionsForIssues`'s per-issue queries (see that function's own
   *   `extraItems` doc). A scoped run's anchor never lands here — its
   *   `issueIdentifier` already flows through `into` the normal way.
   */
  async function _collectSessionIssues(urlKey, sessionId, into, anchorsOut = null) {
    ...
    if (anchor && anchor.issueIdentifier) {
      into.add(anchor.issueIdentifier);
    } else if (anchor && anchor.kind === 'autopilot' && anchorsOut) {
      anchorsOut.set(sessionId, anchor);
    }
    const ownIds = new Set([sessionId]);
    for (const it of hist.items || []) { if (it.issueIdentifier) into.add(it.issueIdentifier); ownIds.add(it.id); }
    for (const it of live || []) { if (it.issueIdentifier) into.add(it.issueIdentifier); ownIds.add(it.id); }

    // LIN-1341: durable-group discovery. A single {urlKey, sessionGroupId} query
    // (indexed — lib/db-indexes.js) gathers every row stamped into this session's
    // group directly, no walk needed — this composes with (does not replace) the
    // followUpTo BFS below, which stays the only path to a genuinely pre-field
    // (unstamped) descendant. A stamped descendant of a stamped row is ALWAYS
    // stamped with this SAME group id (inheritance is unconditional at ingest),
    // so nothing the BFS would find is missed by skipping stamped rows twice.
    const [groupHist, groupLive] = await Promise.all([
      dispatchStore.listHistory(urlKey, { sessionGroupId: sessionId }),
      Promise.resolve(dispatchStore.listItems(urlKey, { sessionGroupId: sessionId }))
    ]);
    for (const it of [...(groupHist.items || []), ...(groupLive || [])]) {
      if (it.issueIdentifier) into.add(it.issueIdentifier);
      ownIds.add(it.id);
    }

    // LIN-1307: downward followUpTo BFS. A follow-up loop (the reply box)
    // carries no sessionId, so it's invisible to the {urlKey, sessionId} index
    // above — discover it (and any chained follow-up of it) by walking
    // followUpTo forward from every dispatch id this session directly owns (the
    // anchor + its workers), the opposite direction of `_sessionsTouchingIssue`'s
    // upward walk. Cycle-guarded via `visited`; a missing/aged-out edge just
    // yields no children for that branch (degraded-but-safe, never throws).
    const visited = new Set(ownIds);
    let frontier = [...ownIds];
    while (frontier.length) {
      const [histChildren, liveChildren] = await Promise.all([
        dispatchStore.listHistory(urlKey, { followUpTo: { $in: frontier } }),
        Promise.resolve(dispatchStore.listItems(urlKey, { followUpTo: { $in: frontier } }))
      ]);
      const children = [...(histChildren.items || []), ...(liveChildren || [])];
      const next = [];
      for (const child of children) {
        if (visited.has(child.id)) continue;
        visited.add(child.id);
        if (child.issueIdentifier) into.add(child.issueIdentifier);
        next.push(child.id);
      }
      frontier = next;
    }
  ...
  async function _doRebuild(urlKey, { sessionId, issueIdentifier }) {
    ...
    // LIN-2934 R1: sessionId -> a GENERAL (goal-only) autopilot anchor whose
    // own row carries no issueIdentifier — collected alongside the closure
    // since such an anchor can never contribute one, and is otherwise
    // invisible to step 3's per-issue queries. See `getSessionsForIssues`'s
    // `extraItems` doc and `_collectSessionIssues`'s `anchorsOut` doc.
    const anchorItems = new Map();
    if (issueIdentifier) issueClosure.add(issueIdentifier);
    for (const s of targets) await _collectSessionIssues(urlKey, s, issueClosure, anchorItems);
    if (issueClosure.size === 0 && anchorItems.size === 0) {
      // No discoverable issues, and no anchorless anchor either, for the
      // target(s) — their rows are gone; drop docs.
      for (const s of targets) await observationSessionsStore.removeSession(urlKey, s);
      return;
    }

    // 3. Rebuild over the closure (plus any anchorless anchors) using the SAME
    // builders the live feed runs.
    const built = await getSessionsForIssues(urlKey, loopDeps, [...issueClosure], { lean: true, extraItems: [...anchorItems.values()] });
