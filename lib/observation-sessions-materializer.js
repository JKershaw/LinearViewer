...
export function createObservationMaterializer({ dispatchStore, agentStatusStore, observationSessionsStore }) {
  ...
  async function _doRebuild(urlKey, { sessionId, issueIdentifier }) {
    ...
    for (const s of targets) {
      ...
      if (session) await observationSessionsStore.upsertSession(urlKey, session);
      else await observationSessionsStore.removeSession(urlKey, s);
    }
  }
  ...
  return { rebuildForWrite, backfillWorkspace };
}
