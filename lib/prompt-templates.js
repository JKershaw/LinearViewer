...
function formatSubtaskSummary(children) {
  ...
  const completed = children.filter(c => c.state?.type === 'completed' || c.state?.type === 'canceled').length
  const inProgress = children.filter(c => c.state?.type === 'started')
  const todo = children.filter(c => c.state?.type === 'unstarted' || c.state?.type === 'backlog')
  const total = children.length

  let line = `**Subtasks:** ${completed}/${total} done`
  if (inProgress.length > 0) {
    line += `, ${inProgress.length} in progress`
    line += ` → Continue: ${inProgress[0].identifier}`
  } else if (todo.length > 0) {
    line += ` → Next: ${todo[0].identifier}`
  }
  return line + '\n'
}
...
export const PROMPT_TEMPLATES = {
  ...
  'plan': {
    ...
    aiHint: {
      ...
      workflow: `Set status to "In Progress" → Plan → Update description with plan → Implement → Update description with summary → Add "${PHASE_LABELS.REVIEW}" label`
    },
    generate: (issue, context) => {
      const sections = [
        formatHeader('Implement', issue),
        ...
        `7. **Complete**: Update description with implementation summary, add "${PHASE_LABELS.REVIEW}" label to ${issue.identifier} to indicate ready for review`,
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
  ...
  'triage': {
    ...
    generate: (issue, context) => {
      ...
      const sections = [
        ...
        '',
        `- **Labels**: Should it have ${PHASE_LABELS.BREAKDOWN}, ${PHASE_LABELS.RESEARCH}, ${WORK_ISSUE_LABELS.BUG}, ${WORK_ISSUE_LABELS.BLOCKED}, or other workflow labels?`,
