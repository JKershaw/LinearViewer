 ...
 * @param {string} noParentText - Text to show when no parent exists
 * @returns {string} Formatted parent info
 */
function formatParent(parent, noParentText = 'None (this is a top-level task)') {
  if (!parent) return noParentText
  return `${parent.identifier}: "${parent.title}" (${parent.state?.name || 'Unknown'})`
}

/**
 * Format issue metadata (status, assignee, estimate)
 * @param {Object} issue - Issue object
 * @param {Object} options - Options for which fields to include
 * @returns {string} Formatted metadata lines
 */
function formatIssueMetadata(issue, options = {}) {
  const { includeStatus = true, includeAssignee = true, includeEstimate = false } = options
  const lines = []
  ...
  if (includeEstimate && issue.estimate) {
...
**Parent Task:** ${formatParent(context.parent)}
...
**Other Labels:** ${formatLabels(issue.labels, ['needs-design'])}
...
**Other Labels:** ${formatLabels(issue.labels, ['needs-spike'])}
...
**Other Labels:** ${formatLabels(issue.labels, ['needs-context'])}
...
**Other Labels:** ${formatLabels(issue.labels, ['bug'])}
...
- If fixed, link the PR and update status`
  ...
  'plan': {
    ...
    generate: (issue, context) => {
      return `Using the Linear MCP, help me create an implementation plan for ${issue.identifier}: "${issue.title}"

## Task Overview

**Status:** ${issue.state?.name || 'Unknown'}
**Assignee:** ${issue.assignee?.name || 'Unassigned'}
**Estimate:** ${issue.estimate ? `${issue.estimate} points` : 'Not estimated'}
**Project:** ${formatProject(context.project)}
**Issue URL:** ${issue.url}
...
**Other Labels:** ${formatLabels(issue.labels, ['plan'])}
...
1. Use \`mcp__linear__get_issue\` to read the full task details, comments, and any linked issues
2. Review the requirements and acceptance criteria
3. Use the codebase tools to research:
   - Relevant existing code patterns and modules
   - Related test files and testing patterns
   - Similar implementations to reference
4. Identify the specific files that need to be created or modified
5. Consider edge cases, error handling, and test coverage
6. Create a step-by-step implementation plan

## Output Format

Provide:
1. **Summary**: One paragraph describing what this task accomplishes
2. **Research Findings**:
   - Key files/modules involved
   - Existing patterns to follow
   - Test files to update or create
3. **Implementation Steps**: Numbered list with:
   - Specific file to modify/create
...
6. **Questions**: Any clarifications needed before starting

After I approve the plan, you can begin implementation. Use \`mcp__linear__update_issue\` to:
...
- Update status to "In Progress" when starting work`
  ...
  'code-review': {
    ...
    generate: (issue, context) => {
      ...
      const estimateInfo = issue.estimate
        ? `${issue.estimate} points`
        : 'Not estimated'

      return `Using the Linear MCP, help me review the code changes for ${issue.identifier}: "${issue.title}"
...
**Other Labels:** ${formatLabels(issue.labels, ['code-review'])}
...
1. Use \`mcp__linear__get_issue\` to read the full task details, comments, and any linked issues
2. Understand what this change was trying to accomplish
3. Identify which files were likely modified based on the requirements
4. Review the code changes systematically using the focus areas below
...
- Are there unnecessary loops, queries, or allocations?
- Could this impact response times or resource usage?

## Output Format

Provide:
1. **Summary**: What this change accomplishes and overall assessment
2. **Files Reviewed**: List of files examined
3. **Findings**: Organized by focus area:
   - ✅ **Correctness**: [findings]
   - ✅ **Tests**: [findings]
   - ✅ **Style**: [findings]
   - ✅ **Security**: [findings]
   - ✅ **Performance**: [findings]
4. **Issues Found**: Specific problems that need addressing
...
After the review is complete, use \`mcp__linear__update_issue\` to:
- Add a comment with the review summary
- Remove the \`code-review\` label if present
- Update status or add labels based on findings`
    }
  }
}
