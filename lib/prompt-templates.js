...
/**
 * Check if an issue is in the Ready queue (eligible for plan prompt)
 * Ready = backlog/unstarted state AND no pre-work labels
 * @param {Object} issue - The issue object with state and labels
 * @returns {boolean} True if the issue is in the Ready queue
 */
export function isInReadyQueue(issue) {
  const stateType = issue.state?.type?.toLowerCase() || ''
  const readyStates = ['backlog', 'unstarted']

  if (!readyStates.includes(stateType)) {
    return false
  }
...
export function getAvailablePrompts(issue) {
  ...
  // Add state-based prompts (plan for Ready queue)
  if (isInReadyQueue(issue) && !available.includes('plan')) {
    available.push('plan')
  }
