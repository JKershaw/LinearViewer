...
import { PROMPT_CATEGORIES, formatGitWorkflow, formatSelfReview, formatCicdCheck, formatPrReview, formatChildren, formatComments, formatStalenessCheck, formatTerminalStateNote, formatChildrenCompleteNote, formatBugInvestigatedNote, resolvePromptUi, applyPromptCapabilities } from './prompt-formatters.js';
...
export function generatePrompt(labelName, issue, context, featureFlags = {}, providerUi = null) {
  ...
  // Bug-already-investigated note: a `bug`-labelled task with prior investigation in
  // its comments should advance to the fix, not loop research (LIN-366). Mirrored in
  // the AI meta-prompt path (Step 2 "First check whether the bug has already been
  // investigated") per CLAUDE.md's both-paths rule.
  prompt += formatBugInvestigatedNote(issue, context)
