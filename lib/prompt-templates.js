...
import { PROMPT_CATEGORIES, formatGitWorkflow, formatSelfReview, formatCicdCheck, formatPrReview, formatChildren, formatComments, formatStalenessCheck, formatLaneBoundary } from './prompt-formatters.js';
 ...
 */
export function generateCustomPrompt(customPrompt, issue, context, featureFlags = {}) {
  ...
  // Apply feature flag transforms (same as built-in prompts)
  if (featureFlags.linearMcp === false) {
    prompt = prompt.replace(/ in Linear/g, '');
  }
...
export const DISPATCH_KINDS = [...Object.keys(PROMPT_TEMPLATES), ...DISPATCH_META_KINDS, DISPATCH_KIND_DEFAULT]
...
export function generatePrompt(labelName, issue, context, featureFlags = {}) {
  ...
  // Phase hand-off ("stay in your lane") contract for chained phases — keeps a phase
  // from pre-writing the next phase's deliverable (research writing the plan, the plan
  // writing the implementation). No-op for phases with no downstream. Mirrored in the
  // meta-prompt path. See lib/prompt-formatters.js formatLaneBoundary + CLAUDE.md.
  prompt += formatLaneBoundary(labelName)
 ...
 *
 * @returns {string[]} Display names (e.g. "plan", "implement", "code review")
 */
