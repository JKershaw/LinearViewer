 ...
 */
function formatWorkflow(category, issue, { useMcp = true } = {}) {
 ...
 */
function formatReadOnlyWorkflow(issue, { useMcp = true } = {}) {
  const identifier = issue.identifier
  const mcp = useMcp ? ' via Linear MCP' : ''
  const mcpTool = useMcp ? ' Use Linear MCP to' : ''
  return `## Workflow
 ...
 */
function formatGitWorkflow(issue) {
  const identifier = issue.identifier
  return `
 ...
 * @property {boolean} [linearMcp=true] - Include Linear MCP references in prompts
...
export function generatePrompt(labelName, issue, context, featureFlags = {}) {
  ...
  // Strip MCP references from inline template strings when disabled
  if (featureFlags.linearMcp === false) {
    prompt = prompt
      .replace(/\s*via Linear MCP/g, '')
      .replace(/Use Linear MCP to /g, '')
  }
