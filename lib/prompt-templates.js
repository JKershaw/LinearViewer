...
import { PHASE_LABELS, WORK_ISSUE_LABELS, VIRTUAL_PROMPTS } from './workflow-config.js';
 ...
 *
 * This section focuses on the CURRENT label only. Transitions to other
 * labels or status changes are handled in the workflow steps.
 *
 * @param {string} labelName - The phase label name (e.g., 'in-research')
 * @param {Object} issue - Issue object with identifier
 * @returns {string} Label lifecycle section
 */
function formatLabelLifecycle(labelName, issue) {
  if (!issue?.identifier) {
    return ''
  }
  const identifier = issue.identifier
  ...
  [PHASE_LABELS.IMPLEMENTATION]: {
    ...
    generate: (issue, context) => {
      const sections = [
        ...
        formatIfBlocked(),
        formatLabelLifecycle(PHASE_LABELS.IMPLEMENTATION, issue)
      ].filter(Boolean)
  ...
  [PHASE_LABELS.REVIEW]: {
    ...
    generate: (issue, context) => {
      const sections = [
        ...
        formatLabelLifecycle(PHASE_LABELS.REVIEW, issue)
      ].filter(Boolean)
  ...
  'plan': {
    ...
    generate: (issue, context) => {
      ...
      const sections = [
        ...
        `5. **Begin implementation**: Add "${PHASE_LABELS.IMPLEMENTATION}" label to ${issue.identifier} via Linear MCP, then execute the plan with test coverage`,
        '6. **Commit**: Push changes with commit message referencing ' + issue.identifier,
        `7. **Complete**: Update description with summary via Linear MCP, remove "${PHASE_LABELS.IMPLEMENTATION}" label, and add "${PHASE_LABELS.REVIEW}" label to indicate ready for review`,
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
