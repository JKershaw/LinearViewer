...
/**
 * Check if an issue is eligible for the plan prompt
 ...
 * @returns {boolean} True if the issue is eligible for plan prompt
 */
...
export function getAvailablePrompts(issue) {
  ...
  // Add state-based prompts (plan for eligible tasks)
  if (isEligibleForPlan(issue) && !available.includes('plan')) {
    available.push('plan')
  }
