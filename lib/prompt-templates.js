...
export const PROMPT_TEMPLATES = {
  ...
  'plan': {
    ...
    aiHint: {
      ...
      workflow: 'Set status to "In Progress" → Plan → Update description with plan → Implement → Update description with summary → Set status to "In Review"'
    },
    generate: (issue, context) => {
      ...
      const sections = [
        ...
        `7. **Complete**: Update description with implementation summary, set ${issue.identifier} status to "In Review" (or "Done" if unavailable)`,
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
