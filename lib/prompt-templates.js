...
import { PROMPT_CATEGORIES, formatGitWorkflow, formatSelfReview, formatCicdCheck, formatPrReview, formatChildren, formatComments, appendGroundingSections, formatAttachmentsSection, resolvePromptUi, applyPromptCapabilities } from './prompt-formatters.js';
...
export function generatePrompt(labelName, issue, context, featureFlags = {}, providerUi = null) {
  ...
  // Deterministic grounding sections — staleness check (LIN-353/re-grounding),
  // terminal-state (LIN-353), all-subtasks-complete (LIN-364), and
  // bug-already-investigated (LIN-366). Appended via the shared post-pass that the
  // AI meta-prompt path also runs (applyGroundingToRecommendation, lib/openrouter.js):
  // ONE source of truth for these rules per docs/architecture/prompt-system.md's
  // both-paths rule (LIN-435),
  // mirroring the capability-awareness post-pass below. Each section self-gates to ''
  // when not applicable.
  prompt = appendGroundingSections(prompt, issue, context)

  // Shared Attachments section (LIN-772) — the SAME formatAttachmentsSection seam
  // the AI meta-prompt path folds into its context block, so the worker sees an
  // identical attachment set on both paths. A post-pass mirroring appendGrounding-
  // Sections: it self-gates to '' when context.attachments is empty, so attachment-
  // less prompts stay byte-identical. No literal "Linear", so the capability post-
  // pass below leaves it invariant for Linear.
 ...
 *
 * @returns {string[]} Display names (e.g. "plan", "implement", "review", "defer")
 */
...
export function formatAIHintsForMetaPrompt() {
  ...
  for (const [key, template] of Object.entries(PROMPT_TEMPLATES)) {
    ...
    // whenNot / chooseOver (LIN-878): additive discriminators that steer the
    // recommender toward richer kinds (design/scoping/spike) — emitted only for the
    // kinds that carry them, back-compatible for kinds that do not. Meta-path only;
    // the handwritten generatePrompt() path never reads aiHint.
    const { situation, goal, workflow, whenNot, chooseOver } = template.aiHint
    lines.push(`**${template.name}** (${situation}):`)
    lines.push(`Goal: "${goal}"`)
    lines.push(`Workflow: ${workflow}`)
    if (whenNot) lines.push(`When NOT: ${whenNot}`)
    if (chooseOver) lines.push(`Choose over: ${chooseOver}`)
    lines.push('')
  }
