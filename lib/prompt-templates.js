...
function formatLabelLifecycle(labelName, issue) {
  ...
  // Only show lifecycle section if completion signals are defined for this label
  if (!signal) {
    return ''
  }
...
export const PROMPT_TEMPLATES = {
  ...
  'plan': {
    ...
    aiHint: {
      ...
      workflow: `Set status to "In Progress" → Plan → Update description → Implement → Commit → Add "${PHASE_LABELS.REVIEW}" label`
    },
    generate: (issue, context) => {
      ...
      const sections = [
        ...
        `7. **Complete**: Update description with summary via Linear MCP, add "${PHASE_LABELS.REVIEW}" label to indicate ready for review`,
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
