...
export const PROMPT_TEMPLATES = {
  'needs-breakdown': {
    ...
    description: 'Break a large or vague task into smaller, actionable subtasks (1-3 hours each). Use when task scope is unclear or too big to start.',
  ...
  'plan': {
    ...
    generate: (issue, context) => {
      const sections = [
        ...
        '',
        formatWorkflow(PROMPT_CATEGORIES.READY, issue),
        '',
        '## Context',
        '',
        formatSection('Project', formatProject(context.project)),
        ...
        '## Goal',
        '',
        'Research the codebase, identify files to modify, and implement the task with test coverage.',
        '',
        'Implementation approach:',
        '1. Read the full task description via Linear MCP',
        '2. Explore the codebase to understand the relevant code',
        '3. Plan the changes (files to modify, new files if needed)',
  ...
  'triage': {
    name: 'Task Triage',
    category: PROMPT_CATEGORIES.UNIVERSAL,
    description: 'Review and update task metadata: labels, priority, estimates, assignee. Use when a task needs organizational cleanup before work begins.',
    generate: (issue, context) => {
      ...
      const sections = [
        ...
        formatSection('Estimate', issue.estimate !== undefined ? `${issue.estimate} points` : 'Not set'),
        ...
        '- **Estimate**: Based on complexity, what point estimate would be reasonable?',
