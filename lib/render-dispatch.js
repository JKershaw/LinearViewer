...
export function renderDispatchPage(workspaceName = 'Workspace', options = {}) {
  const { deployInfo = {}, urlKey = null, openRouterSource = null, workspaces = [], featureFlags = {}, projectRepos = [] } = options;
...
<body>
  ...
  <main>
      ...
      <div class="tree">
        <div class="node">
          ...
          <div class="children">${projectRepos.length > 0 ? `
            <div class="node">
              <div class="line">
                <span class="dispatch-label">repo:</span>
                <span class="dispatch-value">
                  <select class="dispatch-repo-select">
            ...
            <div class="node">
              <div class="line dispatch-prompt-actions">
                <span class="dispatch-actions-label">Dispatch:</span>
                <button class="dispatch-action save dispatch-prompt-send" data-target="cli">cli</button>
                <button class="dispatch-action save dispatch-prompt-send" data-target="web">web</button>
                <button class="dispatch-action save dispatch-prompt-send" data-target="dash">dash</button>${featureFlags.proxy === true ? `
    ...
    <section class="dispatch-section">
      <h2 class="dispatch-section-header">History <button class="dispatch-action history-refresh">refresh</button></h2>
