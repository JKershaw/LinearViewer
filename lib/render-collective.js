...
export function renderCollectivePage(data, options = {}) {
  ...
  const workspaceRows = workspaces.length > 0
    ...
    : '<p class="collective-empty">No connected workspaces.</p>';
  ...
  const setupBody = `<div class="tree">
        ...
        <div class="collective-field">
          ...
          <input type="text" id="collective-topic" class="collective-input collective-input-wide" value="${escapeHtml(defaultTopic)}" maxlength="500">
        </div>
        <div class="collective-field">
  ...
  const transcriptBody = `<div class="collective-transcript-head">
      ...
      <p class="collective-transcript-empty" id="collective-transcript-empty">○ no messages yet — start a discussion above, then watch it unfold here</p>
