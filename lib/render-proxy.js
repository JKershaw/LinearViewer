...
export function renderProxyPage(workspaceName = 'Workspace', options = {}) {
  ...
  return renderPage({
    ...
    stylesheets: ['/style.css', '/common-actions.css', '/proxy.css', '/foreman.css'],
    nav: navBarHtml,
  ...
  <main>
    ...
    <section class="proxy-section">
      <h2 class="proxy-section-header">
        <a href="/workspace/${encodedUrlKey}/foreman" style="text-decoration: none; color: inherit;">Foreman <span class="foreman-experimental">experimental</span></a>
      </h2>
      <p class="proxy-hint">Autonomous task runner — copy a playbook prompt and let an AI agent work through your task stack.</p>
    </section>

    <section class="proxy-section">
      <h2 class="proxy-section-header">Event Log <button class="action-btn proxy-events-refresh">refresh</button></h2>
      <details class="proxy-collapsible" id="proxy-events-collapsible">
        <summary class="proxy-collapsible-summary">
