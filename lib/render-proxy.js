...
export function renderProxyPage(workspaceName = 'Workspace', options = {}) {
  ...
  <main>
    ...
    ${renderSection({ boxed: true, className: 'proxy-section proxy-health-section', titleClass: 'section-header proxy-section-header', title: 'Credential Health', body: `<p class="proxy-hint">A token that keeps succeeding on workspace-free calls while its workspace-scoped calls report <code>token_ownerless</code> is dead as a workspace credential — re-issue it.</p>
      <div class="proxy-health-panel" id="proxy-health-panel" data-url-key="${encodedUrlKey}">
        <div class="proxy-health-loading">Loading credential health...</div>
      </div>` })}
