 ...
 *
 * This is load-bearing diagnostically, not cosmetic. Selection in
 * `selectOwnerWorkspaceToken` is by MAXIMUM `tokenExpiresAt`, not by recency, so
 * a sentinel expiry mirrored onto a workspace's scalar fields wins selection
 * PERMANENTLY: no token refresh can outbid it, and re-connecting the workspace
 * cannot either (a fresh Linear token's now+24h always loses). It also makes the
 * credential permanently ineligible for refresh-on-resolve, which only fires
 * when no token resolved at all. "Sentinel expiry on a Linear-provider call" is
 * therefore a specific, self-sustaining failure this field names on sight.
 *
