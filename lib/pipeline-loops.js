 ...
 * answeredDecisionId, telemetry}` shape `deriveLoopFacingFacts` produces
 * (lib/digest-feedback.js), from a row's persisted `feedbackDigest` instead of
 * re-scanning feedback (LIN-3011 lean path). The digest's own field names and
 * always-present-but-nullable convention (`digestFeedback`'s own narrowing)
 * differ from `buildRunTelemetry`'s omit-when-absent convention — `evidence`
 ...
 *   decisionCase: Array, answeredDecisionId: string|null, telemetry: Object}}
 */
function _loopFactsFromDigest(digest) {
  if (!digest) {
    return {
      terminal: null, wake: null, decision: null, decisionCase: [], answeredDecisionId: null,
      telemetry: { metrics: [], producedArtifacts: [], runtime: { ms: null, dispatchedAt: null, completedAt: null, crossCheck: null } }
    };
  }
    ...
    for (let i = 0; i < loops.length; i++) {
      ...
      if (lean) {
        const harvestedDigest = item.feedbackDigest
          ...
          : item.feedbackDigest;
        facts = _loopFactsFromDigest(harvestedDigest);
