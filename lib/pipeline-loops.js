  ...
  for (const [issueIdentifier, loops] of byIssue) {
    ...
    for (let i = 0; i < loops.length; i++) {
      ...
      result.push({
        ...
        // derivations of this loop's feedback (LIN-594).
 ...
 * Omit `issueIdentifier` (workspace/session paths) to read everything unbounded.
 *
 ...
 */
async function _fetchWorkspaceData(urlKey, { dispatchStore, agentStatusStore }, { issueIdentifier } = {}) {
  // Build options only when scoping, so the unscoped (workspace) path keeps
  // calling the stores with no options — preserving the "no limit means
  // everything" contract its regression tests pin.
  const itemOpts = issueIdentifier ? { issueIdentifier } : undefined;
  const statusOpts = issueIdentifier ? { taskIdentifier: issueIdentifier } : undefined;
  const [liveItems, historyResult, agentStatusResult] = await Promise.all([
    dispatchStore.listItems(urlKey, itemOpts),
    dispatchStore.listHistory(urlKey, itemOpts),
    agentStatusStore.listStatus(urlKey, statusOpts)
  ]);
...
export async function getSessionsForWorkspace(urlKey, deps = {}) {
  ...
  const { live, history, agentStatus } = await _fetchWorkspaceData(urlKey, { dispatchStore, agentStatusStore });
  // `lean` omits the heavy, feed-unused `promptText` per loop (LIN-622); the
  // session feed (mergeSessions) opts in. Single-session on-demand paths
  // (session-summary/session-context) keep the default (full).
