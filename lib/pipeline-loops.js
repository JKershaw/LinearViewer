...
// the terminal wake markers (done/complete/failed/aborted). `[blocked]` is a
// pause/wait signal and `[pending]` a holdable boundary — neither is terminal
// (dispatch-terminal.js keeps them out of the terminal set). Rolling these up to
// a session-level "waiting on user" state is the LIN-1005 human-facing surface;
// the existing agent-to-agent wake path (lib/dispatch-wake.js) is untouched.
