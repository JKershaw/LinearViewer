...
// row of one of these kinds without an identifier is not malformed. `wake` is
// the only known case (a wake follow-up dispatched via dispatch-wake.js's
// `buildWakeFollowUp`). Since LIN-2121 a wake row normally DOES carry an
// `issueIdentifier` (inherited from its triggering dispatch) and is therefore
// built like any other loop; this set still covers the identifier-less remainder
// — a legacy pre-LIN-2121 row, or a wake minted from an identifier-less edge (a
// general run's worker), both of which remain identifier-less by design. Widen
// this set only for another kind with the same structural property, not for a
// one-off row shape.
 ...
 * answer-state stamp `markDecisionAnswered` (dispatch-store.js) writes. This
 * is the ONE shared check every consumer that reads `feedback[]` for a
 * purpose other than the store write, the tail-read predicates, or this
 * module's own baking must import and apply before doing its work: the
 * answer stamp is metadata about a decision, not a chat turn or a fact about
 * run progress, and must stay invisible to those readers.
 *
 ...
 * fails to parse. Mirrors `_findLastDecision`'s backward-scan discipline —
 * the most recent answer wins, so a loop can carry an older answered
 * decision superseded by a newer, still-unanswered one.
 *
 * @param {Array<{kind?: string, message?: string}>} feedback
 * @returns {string|null}
 */
function _findDecisionAnswer(feedback) {
  ...
  for (let i = feedback.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      // earlier valid one, mirroring `_findLastDecision`'s discipline; this
      // module never throws on stored data it did not itself validate at
      // write time.
    }
  }
  return null;
...
function _buildLoops({ liveItems = [], historyItems = [], agentStatusEntries = [], now = new Date(), lean = false } = {}) {
  ...
  for (const [issueIdentifier, loops] of byIssue) {
    ...
    for (let i = 0; i < loops.length; i++) {
      ...
      // Pre-derive the decision facts the same way (LIN-2182 / H3): baked here so
      // lean and non-lean loops carry the SAME `decision`/`decisionCase` without
      // re-scanning raw feedback.
      //
      // Read the post-harvest `feedback` local, not `rawFeedback` — for CONSISTENCY
      // with the neighbouring derivations above, so every fact on this loop
      // describes one and the same view of the feedback. Note the narrower truth
      // (LIN-2182 review, ledger 8): `feedbackWithHarvestedAbort` is APPEND-ONLY
      // (`[...base, abortEntry]`, lib/dispatch-terminal.js), so the indices of
      // pre-existing entries are identical in both arrays and a synthetic
      // `[aborted]` entry can only ever land PAST `decisionEntryIndex` — which the
      // backward scan never reaches. Reading `rawFeedback` here would not corrupt
      // the index today; it would simply be the odd one out. If the harvest ever
      // gains a non-append form, that stops being true and this becomes load-bearing.
      ...
      // Build-time answer fact (LIN-1728), same discipline as `decision` above:
      // always present (lean and non-lean), `null` when absent, never
      // `undefined`. Read from the same post-harvest `feedback` local as
      // `decision` for consistency — a synthetic `[aborted]` entry lands past
      // any `decision-answer` stamp too, by the same append-only argument.
