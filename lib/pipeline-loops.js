...
import { buildRunTelemetry, buildSessionTelemetry, parseHeartbeats } from './session-telemetry.js';
 ...
 * @param {boolean} [deps.lean=false] - omit heavy feed-unused fields (promptText); LIN-622
 * @param {Date} [deps.now] - injectable "now" for testing (the 30-day lookback
 *   window is derived from it; fixed-date fixtures age out of a real-clock window)
 * @returns {Promise<Array<Object>>} session records, most-recent first
 */
export async function getSessionsForWorkspace(urlKey, deps = {}) {
  if (!urlKey) return [];
  const { dispatchStore, agentStatusStore, issueGraph = null, lean = false, now = new Date() } = deps;
  if (!dispatchStore || !agentStatusStore) {
    throw new Error('pipeline-loops: dispatchStore and agentStatusStore must be injected');
  }
  ...
  const loops = _buildLoops({ liveItems: live, historyItems: history, agentStatusEntries: agentStatus, now, lean });
  const sessions = _buildSessions(loops, { issueGraph, now });
