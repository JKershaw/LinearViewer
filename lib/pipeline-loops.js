...
// Dispatch-item `kind`s that are identifier-less BY DESIGN (LIN-2232) — a row
// of one of these kinds legitimately carries no `issueIdentifier` and must not
// be classified as malformed. `wake` is the only known case today (a wake
// follow-up dispatched via dispatch-wake.js's `buildWakeFollowUp`, addressed
// to a sessionId rather than an issue); widen this set only for another kind
// with the same structural property, not for a one-off row shape.
