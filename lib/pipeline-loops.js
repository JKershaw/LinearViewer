...
function _buildLoops({ liveItems = [], historyItems = [], agentStatusEntries = [], now = new Date(), lean = false } = {}) {
  ...
  for (const [issueIdentifier, loops] of byIssue) {
    ...
    for (let i = 0; i < loops.length; i++) {
      ...
      // regardless of `lean`. The lean feed can then drop raw `feedback[]`
      // (the dominant per-row bytes on a long autopilot run — the heartbeat /
      // [evidence] log) and still answer those questions without re-scanning:
      ...
      const facts = deriveLoopFacingFacts(feedback, item.dispatchedAt || null);
      const { terminal, wake, decision, decisionCase, answeredDecisionId } = facts;
 ...
 * loop-facing facts from — the lean read excludes `feedback` entirely. Such
 * rows are re-read WHOLE by `_id` (prompt excluded, N3), each whole re-read
 * doc is digested (`digestFeedback`) and the digest is written back guarded on
 * the row's own `feedbackVersion` (best-effort — a write-back failure is
 * logged only and never fails the fill), and the re-read's formatted feedback
 * is restored onto the row for THIS fill so `_buildLoops`'s existing
 * derivation (unchanged this beat) still produces the correct facts. Reading
 * `feedbackDigest` directly instead of restoring `feedback` is beat 4's job
 * (LIN-2996 Phase 3, digest-backed loop building); this only makes the read
 * correct-and-complete again, not yet digest-sourced.
 *
...
async function _selfHealLeanHistory(historyArr, dispatchStore) {
  ...
  return Promise.all(historyArr.map(async (item) => {
    ...
    if (!raw) return item; // re-read found nothing for this id; leave the row as-is rather than inventing facts
    ...
    return {
      ...item,
      feedback: formatFeedbackEntries(Array.isArray(raw.feedback) ? raw.feedback : []),
      feedbackVersion: v,
      feedbackDigest
    };
  }));
