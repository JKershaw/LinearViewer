 ...
 * answeredDecisionId, answeredDecisions, telemetry}` shape
 ...
 *   decisionCase: Array, answeredDecisionId: string|null, answeredDecisions: Array, telemetry: Object}}
 */
...
function _buildLoops({ liveItems = [], historyItems = [], agentStatusEntries = [], now = new Date(), lean = false } = {}) {
  ...
  for (const [issueIdentifier, loops] of byIssue) {
    ...
    for (let i = 0; i < loops.length; i++) {
      ...
      const { terminal, wake, decision, decisionCase, answeredDecisionId, answeredDecisions } = facts;
...
function _assembleSession(sessionId, anchorLoop, loops) {
  ...
  if (anchorLoop) {
    ...
    if (typeof anchorLoop.maxSessionsPerTask === 'number' && anchorLoop.maxSessionsPerTask >= 1) {
      // What sessionPosition means for a multi-task run (N1): the bound is
      ...
      // RECENTLY DISPATCHED task — the last distinct issueIdentifier among
      // the seam-scoped, time-ordered rows, under the same first-seen
      // ordering `tasksTouched` above already builds — matching what an
      // operator watching the live feed cares about (the count for whichever
      // task is currently in flight), and reducing to the unambiguous single
      // case for a scoped one-task run.
      const seamTasksTouched = [];
      const seenSeamTasks = new Set();
      for (const l of seamScoped) {
        if (!seenSeamTasks.has(l.issueIdentifier)) {
          seenSeamTasks.add(l.issueIdentifier);
          seamTasksTouched.push(l.issueIdentifier);
        }
      }
      const currentTask = seamTasksTouched[seamTasksTouched.length - 1] || null;
      if (currentTask) {
