...
function extractHashableLoop(loop) {
  ...
  return {
    ...
    agentAction: loop.agentAction,
    agentStatus: loop.agentStatus,
    agentSummary: loop.agentSummary,
    // LIN-1728/LIN-3037: a decision-lifecycle stamp is answer/withdrawal
    // metadata, not a fact about the run — excluded before mapping so
    // appending one to a terminal loop's feedback (a Save/Save-and-continue
    // after the run is already cached) does not change `inputHash` and does
    // not invalidate the cached summary.
    feedback: (Array.isArray(loop.feedback) ? loop.feedback : [])
      .filter(fb => !isDecisionLifecycleStampEntry(fb))
      .map(fb => typeof fb === 'string' ? fb : (fb?.message || ''))
  };
}
