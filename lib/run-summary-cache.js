...
function extractHashableLoop(loop) {
  ...
  return {
    ...
    // LIN-1728: a `decision-answer` stamp is answer metadata, not a fact about
    // the run — excluded before mapping so appending one to a terminal loop's
    // feedback (a Save/Save-and-continue after the run is already cached)
    // does not change `inputHash` and does not invalidate the cached summary.
    feedback: (Array.isArray(loop.feedback) ? loop.feedback : [])
      .filter(fb => !isDecisionAnswerEntry(fb))
