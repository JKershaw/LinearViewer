...
export function createWorkspaceTokenCache({ ttlMs = DEFAULT_TTL_MS, blockWindowMs = DEFAULT_BLOCK_WINDOW_MS, now = () => Date.now() } = {}) {
   ...
   *
   * LIN-2216: bounded by the cached TOKEN's own remaining validity, not only
   * by `ttlMs`. Before this, an entry cached with a live token could outlive
   * that token's actual expiry (or a rotation elsewhere invalidating it
   * upstream) for up to the rest of the `ttlMs` window — every request
   * served from it during that gap presented a token Linear had already
   * stopped honouring, producing time-clustered 401 bursts that self-healed
   * only once the entry aged out on TTL. `value.expiresAt` is already the
   * shape every `set(key, {..., expiresAt})` call site in server.js
   * populates (LIN-1373's resolved-credential shape), so this reads data
   * already present rather than adding a new field. `Number.MAX_SAFE_INTEGER`
   * (a never-expiring local/PAT/Basic credential) and a missing/non-finite
   * `expiresAt` both fall through unaffected — there is nothing to bound.
   * @param {string} key
   * @returns {*}
   */
  function get(key) {
    ...
    const tokenExpiresAt = entry.value?.expiresAt;
    if (Number.isFinite(tokenExpiresAt) && currentTime >= tokenExpiresAt) {
      entries.delete(key);
      return undefined;
    }
    return entry.value;
