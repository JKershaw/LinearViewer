 ...
 * with `sessionId` = the coordinator's id (its dispatching parent) + `subscribe:
 * true` — MUST wake that coordinator when it finishes; that up-chain report is the
 * literal substrate for "each autopilot reports back up the chain." The
 * coordinator's own kickoff never falls through: a top-level kickoff carries no
 * `sessionId` (rejected above), and a run owner that stamps `sessionId === its own
 * id` is caught by the self-skip below. So the only `kind: 'autopilot'` item that
 * reaches the wake is exactly a subscribed child whose parent differs from itself —
 * which is the one we now want.
 *
...
export function buildWakeFollowUp(child, feedback) {
  ...
  // NOTE (LIN-813): there is intentionally no `if (child.kind === 'autopilot')
  // return null` here. An autopilot acting as a coordinator dispatches one
  // task-altitude CHILD autopilot per task with `sessionId` = the coordinator's id
  // + `subscribe: true`; the child must wake the coordinator up-chain when it
  // terminates, exactly like any other subscribed child. The coordinator's own
  // kickoff is already excluded (no sessionId, or the self-skip above), so lifting
  // this guard only admits genuine child APs.
