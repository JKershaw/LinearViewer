...
/**
 * Phase hand-off ("stay in your lane") contract.
 *
 * Each chained phase produces its OWN artifact and must not pre-write the next
 * phase's deliverable. This is the counter-force to depth propagating through the
 * Linear write-back (a deep research artifact dragging the plan into its style) —
 * see docs/lin-260-prompt-scaling-research.md. A universal floor plus a sharpened
 * line for the two worst bleeds (research→plan, plan→implementation).
 *
 * NOTE: Mirrored in the AI meta-prompt path (lib/prompts/meta-prompt-template.js,
 * "## Stay In Your Lane"). Per CLAUDE.md, prompt-behavior changes apply to BOTH paths.
 *
 * @param {string} labelName - The template/label name (e.g. 'plan', 'research')
 * @returns {string} Lane-boundary section, or '' for phases with no downstream
 */
export function formatLaneBoundary(labelName) {
  const sharpened = {
    research: 'You are researching — stop at findings, a recommended approach, and the Surface Assessment. Do **not** write the implementation plan (the file-by-file change list); that is the plan phase\'s job.',
    plan: 'You are planning — stop at the surfaces, any dependency arrows, and the session-fit answer. Do **not** write implementation-level steps or actual code; that is the implementation phase\'s job.',
    breakdown: 'You are breaking down — create subtasks from the plan\'s surfaces and arrows. Do **not** re-plan or implement; carry the existing plan\'s structure into subtasks.'
  }
  const line = sharpened[labelName]
  if (!line) return ''
  return `

## Stay In Your Lane

Produce only this phase's artifact; do not do the next phase's job. ${line}

Scale your output to the task itself, not to the length of the prior discussion you were handed — a small task gets a short result even when the discussion history is long.`
}
