 ...
 */
export function formatStalenessCheck(issue, context = {}) {
  // Grounding-freshness suppression (LIN-1240): when the proxy route has proven the
  // ticket slice AND the code HEAD are unchanged since the last snapshot, emit a short
  // "grounding is current" note in place of the full re-read. `context.groundingFreshness`
  // is precomputed async at the route (resolveGroundingFreshness); this formatter only
  // READS it, staying pure/synchronous. ABSENT freshness ⇒ the exact full section below,
  // so output is byte-identical when the field is not threaded (LIN-435/177 parity).
  const fresh = context?.groundingFreshness?.fresh === true ? context.groundingFreshness : null
  if (fresh) {
    const asOf = fresh.capturedAt ? ` as of the last snapshot (${fresh.capturedAt})` : ''
    return `

## Re-ground the Ticket (grounding current)

Grounding was verified current${asOf}: this task's slice (title/description/state/labels/comments/parent/children) and the code HEAD are both **unchanged** since that snapshot, so you may trust the ticket's description of the codebase without a full re-read. Spot-check only the specific files/symbols you are about to change, and re-ground fully if anything looks off.`
  }
 ...
 */
export function formatPlanFidelityCheck(context = {}) {
  // Grounding-freshness suppression (LIN-1240): the handwritten `implement` template
  // calls this INLINE and threads `context` in, so when the route has proven the plan's
  // inputs (ticket slice + code HEAD) are unchanged since the last snapshot we emit a
  // short note instead of asking for a full re-derivation from the research thread. Reads
  // only the precomputed `context.groundingFreshness` — stays pure. ABSENT the field it
  // returns the full section below byte-for-byte (LIN-698 tests stay green), and the
  // meta-path's LLM plan-fidelity prose is deliberately NOT touched (accepted V1 boundary:
  // it is only softly suppressible).
  if (context?.groundingFreshness?.fresh === true) {
    return `
### Re-ground the Plan (grounding current)

Grounding was verified current as of the last snapshot: the plan's inputs (the ticket slice and the code HEAD) are **unchanged**, so you may proceed on the plan's distillation without re-deriving it from the full research thread. Spot-check only the specific constraints for the code you are about to change, and re-ground fully if the plan and the code appear to disagree.
`;
  }
  return `
 ...
 * Review-prompts quality rules). Per CLAUDE.md, prompt-behavior changes must be
 * applied to BOTH the handwritten and AI-generated paths.
 *
...
This task carries ${items.length} attachment(s). Before making any grounding claim in your findings, plan, or verdict:

1. Fetch AND perceive EVERY attachment listed for this task — not only the ones that are trivially readable as text. An image or screenshot must actually be viewed, never skipped because it takes an extra step.
2. Enumerate the attachments you perceived explicitly, by title and id, in your output — never summarize them collectively as "the attachments".
3. If a required attachment is unreadable or otherwise unperceivable, **stop** — do not proceed to produce the normal findings, plan, or verdict on a partial read. Instead, add a comment naming the exact attachment (title and id) and why it could not be perceived, and recommend \`blocked\` as the next action.`;
}
...
export function appendGroundingSections(prompt, issue, context = {}) {
  let out = prompt;
  // Pass `context` so formatStalenessCheck can read a precomputed `groundingFreshness`
  // (LIN-1240) and emit the short "grounding current" note when fresh. Absent that field
  // it returns the full staleness section byte-for-byte, so this seam stays parity-pinned.
  out += formatStalenessCheck(issue, context);
