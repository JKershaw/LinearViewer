...
export function formatSuccessCriteria(issue, context) {
  ...
  // Add default health criterion for research or planning oriented tasks
  const isResearchOrPlan = issue.labels?.some(l => 
    l.name.toLowerCase().includes('research') || 
    l.name.toLowerCase().includes('plan')
  ) || issue.title.toLowerCase().includes('research') || issue.title.toLowerCase().includes('plan');

  if (isResearchOrPlan) {
    lines.push('- [ ] Maintain or improve overall code health by identifying brittle or redundant code in the path of the change')
  }
...
/**
 * Generate code health instructions for refactoring check
 * @returns {string} Code health section
 */
export function formatCodeHealth() {
  return `

## Code Health
Always look for opportunities to simplify existing logic or remove redundancy in the files you are researching. If a refactor would make the eventual implementation safer or cleaner, highlight it as a 'Refactoring Opportunity'.`
}