 ...
 *
 * NOTE: This directive is intentionally mirrored in the AI meta-prompt path
 * (lib/prompts/meta-prompt-template.js → Step 0). Per
 * docs/architecture/prompt-system.md, prompt-behavior changes must be applied
 * to BOTH the handwritten and AI-generated paths. A terminal task that still
 * has open children is NOT short-circuited here.
 *
 ...
 *
 * NOTE: Mirrored in the AI meta-prompt path (lib/prompts/meta-prompt-template.js →
 * Step 0, the `!isTerminal && hasSubtasks && !hasOpenChildren` branch) per
 * docs/architecture/prompt-system.md's both-paths rule. A parent with any
 * open child is NOT short-circuited here.
 *
 * @param {Object} issue - Issue object (uses state)
 * @param {Object} [context] - Context with children (to detect open remaining work)
 ...
 * docs/architecture/prompt-system.md's both-paths rule. This is a SOFT signal
 * by necessity: there is no deterministic "investigated" marker today
 * (findings live in free-form comments), so
 ...
 * meta-prompt — is the genuine kill of the two-paths maintenance tax
 * (docs/architecture/prompt-system.md's both-paths rule). It mirrors the
 * capability-awareness post-pass (applyPromptCapabilities)
