...
import { isTerminalState, selectFocusSubtask, computeFrontierFacts } from './tree.js';
 ...
 * the `bug` label — the bug template removes it only "when fixed" — so without a
 * counter-signal the next recommendation re-investigates the same bug. That is the
