...
export function selectFocusSubtask(children) {
  ...
  // 1. Continue in-progress work (lowest identifier among in-progress)
  const inProgress = children.filter(c => c.state?.type === 'started').sort(compareByIdentifier)
  if (inProgress.length) return inProgress[0]

  // 2. First non-blocked todo (lowest identifier; checks both labels and relations)
  ...
  // 3. Fall back to the lowest-identifier non-terminal subtask
