...
export function buildForest(issues) {
  ...
  // Group issues by project (use virtual project for issues without one)
  const byProject = new Map()
  for (const issue of issues) {
    const projectId = issue.project?.id || NO_PROJECT_ID
...
export function buildInProgressForest(issues, projects) {
  ...
  // Build nodes for relevant issues, grouped by project (use virtual project for issues without one)
  const byProject = new Map()
