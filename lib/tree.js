...
import { computeGraphFeatures, childrenToGraphNodes } from './graph-features.js'
 ...
 *
 * Within the chosen eligibility tier, candidates are ranked by live frontier
 * signals — `downstreamUnblocks` desc (clear the most successors), then
 * `criticalPathLen` desc (shorten the makespan), then `compareByIdentifier` asc as
 * the deterministic tie-break. The signals are the same ones the swipe/stack digest
 * computes, reconstructed from the in-set siblings' `inverseRelations` (canonical
 * children carry no forward `blocksIds`). An edge-free set leaves every feature at
 * its default, so ranking degrades cleanly to identifier order — today's behavior.
 *
 ...
 *
 * Out of scope (LIN-391 deferred Scope 3): transitive *external* cross-sibling-set
 * blocker edges — only in-set sibling edges are reconstructed here.
 *
...
export function selectFocusSubtask(children) {
  ...
  // 1. Continue non-blocked in-progress work, frontier-ranked (never a blocked child)
  const inProgress = children
    .filter(c => c.state?.type === 'started' && !isBlocked(c))
    .sort(byFrontier)
  if (inProgress.length) return inProgress[0]

  // 2. First non-blocked todo, frontier-ranked (checks both labels and relations)
  const nextTodo = children
    .filter(c => (c.state?.type === 'unstarted' || c.state?.type === 'backlog') && !isBlocked(c))
    .sort(byFrontier)
