...
function renderPromptCard(prompt) {
  ...
  // Migrated onto the canonical `card` component (LIN-468). The markup is kept
  // byte-identical to the client-side renderer in public/custom-prompts.js so
  // server-rendered and live-inserted cards converge. `.custom-prompt-card` /
  // `.custom-prompt-header` survive as no-style E2E/JS hooks alongside
  // `.card` / `.card-header`.
  return renderCard({
    className: 'custom-prompt-card',
    attrs: `data-prompt-id="${escapeHtml(prompt.id)}"`,
    body: `<div class="card-header custom-prompt-header">
...
export function renderCustomPromptsPage(workspaceName = 'Workspace', options = {}) {
  ...
  const promptCardsHtml = customPrompts.length > 0
    ...
    : renderEmptyState({ className: 'custom-prompts-empty', text: 'No custom prompts yet. Create one to get started.' });

  const editorHtml = renderEditor();

  return renderPage({
    title: `${escapeHtml(workspaceName)} - Custom Prompts`,
    stylesheets: ['/style.css', '/common-actions.css', '/custom-prompts.css'],
    nav: navBarHtml,
    scripts: ['/common.js', '/app.js', '/custom-prompts.js'],
    content: `${renderPageHeader({ title: 'Custom Prompts', subtitle: 'Create your own prompt templates with variable substitution' })}

  <main>
    ${renderSection({ body: `<div class="custom-prompts-toolbar">
