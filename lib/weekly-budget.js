...
//
// Recorded as a named record (LIN-2404), not just comment prose above, so
// the calibration date/window/sample stay a testable, surfaceable fact next
// to the scalar they derive rather than a claim that only lives in a
// comment nobody re-reads. `DEFAULT_USD_PER_POINT` is derived FROM this
// record (spendUsd / windowPoints), so the two can never drift apart from
// each other — only from reality, which is exactly the staleness this
// provenance makes visible instead of silent.
export const DEFAULT_USD_PER_POINT_PROVENANCE = {
  calibratedAt: '2026-08-14',
  windowPoints: 27,
  spendUsd: 1070.58,
  source: 'LIN-2087, docs/reviews/capacity-test-run-review-2026-08-14.md'
};

export const DEFAULT_USD_PER_POINT = DEFAULT_USD_PER_POINT_PROVENANCE.spendUsd / DEFAULT_USD_PER_POINT_PROVENANCE.windowPoints;
