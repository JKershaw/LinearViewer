...
// Restated here, and DELIBERATELY pinned against openrouter's own source rather
// than trusted. `REQUEST_TIMEOUT_MS` is module-private there, so it cannot be
// imported — which is exactly why LIN-2447's own test regexes it out of the
// source instead of asserting a number. That pin protects the DEFAULT budget
// through `RESERVATION_LEASE_MS`; without a matching pin here, every
// non-default budget (the boot turn this function exists for) would drift
// silently the next time the timeout moves. `tests/unit/flight-companion-gate.test.js`
// carries that pin.
 ...
 *
 * LIN-2661 introduces a SECOND, structurally identical deploy-boundary tick,
 * for the same reason: a `companionDoc.lastCensusSnapshot` persisted BEFORE
 * that fix can carry raw, unsanitized malformed tuples baked into its
 * `attentionKeysFull`/`attentionKeys` (a `null` row, or one missing
 * `loopId`/`lane`/`stage`, both of which `buildCompanionSnapshot` below used
 * to pass through uncleaned). The first post-deploy `currentSnapshot` will
 * have those stripped, so the two identity sets differ on that one tick ->
 * one delta -> one spend, self-healing the instant the next tick's PRIOR
 * snapshot is itself post-LIN-2661 too. Same acceptance rationale as the
 * fossil-cap case above: folding this boundary tick to `no-delta`
 * unconditionally would risk swallowing a genuinely new row landing on that
 * exact tick, which this fold must never do.
 *
...
/**
 * LIN-2661: a persisted attention row is well-formed only if it carries all
 * three identity fields. Everything else (`null`/`undefined`, a non-object,
 * or an object missing any one of `loopId`/`lane`/`stage`) is malformed and
 * must never reach `attentionKeys` — a `null` row throws on the very first
 * property read, and a partial row would otherwise silently enter the
 * `attentionKeySetsEqual` identity diff as `[undefined,undefined,undefined]`.
 *
 * @param {*} row
 * @returns {boolean}
 */
function isWellFormedAttentionRow(row) {
  return row != null && typeof row === 'object' && row.loopId != null && row.lane != null && row.stage != null;
}

/**
 * LIN-2661: a persisted `attentionKeysFull` tuple is well-formed only if it
 * is a 3-element array with every element present. `attentionKeysFull`'s
 * elements are already identity tuples (not row objects) sourced separately
 * from the census (`lib/observer-sweep.js`), and a present field is passed
 * through today with zero element-level validation — this is the array
 * `resolveAttentionIdentitySet` above PREFERS, so it is the one the no-delta
 * fold actually diffs; sanitizing only `attention`/`attentionKeys` would
 * clean the array nobody diffs and leave this one exposed.
 *
 * @param {*} tuple
 * @returns {boolean}
 */
function isWellFormedAttentionTuple(tuple) {
  return Array.isArray(tuple) && tuple.length === 3 && tuple[0] != null && tuple[1] != null && tuple[2] != null;
}
 ...
 *
 * `malformedAttentionCount` (LIN-2661) counts persisted `attention` rows
 * dropped for being malformed (a `null`/`undefined` row, or one missing
 * `loopId`/`lane`/`stage`) — a sibling scalar modelled on
 * `staleAttentionCount`/`staleAttentionThresholdMs`
 * (`lib/observer-sweep.js:303-305`), never folded into `attentionCount`
 * (which still drives the seed-turn `surface` decision below — an unusable
 * row must not surface to a human). `attentionKeysFull`, whose elements are
 * already identity tuples rather than row objects, is sanitized
 * independently through its own tuple predicate for the same reason.
 *
 ...
 * @returns {{lanes: Object<string, number>, attentionKeys: Array<[string,string,string]>, attentionKeysFull: Array<[string,string,string]>, attentionCount: number, malformedAttentionCount: number, truncated: boolean, censusRev: number}}
 */
export function buildCompanionSnapshot(currentCensusDoc) {
  ...
  const wellFormedRows = attention.filter(isWellFormedAttentionRow);
  const attentionKeys = wellFormedRows.map((row) => [row.loopId, row.lane, row.stage]);
  const malformedAttentionCount = attention.length - wellFormedRows.length;
  const attentionKeysFull = Array.isArray(state.attentionKeysFull)
    ? state.attentionKeysFull.filter(isWellFormedAttentionTuple)
    : attentionKeys;
...
export function buildTurnRecords({
  ...
  // Explicit preconditions, because this is now PUBLIC API with a different
  // caller shape. `shouldSpendTurn` guarantees both are non-null by the time it
  // gets here — its refusal branches return first — but the whole point of
  // extracting this is that a boot turn calls it DIRECTLY, skipping those
  // branches. There, a null census or companion doc is a `TypeError` deep in a
  // property read rather than a legible refusal.
  if (currentCensusDoc == null) {
    throw new Error('flight-companion-gate: buildTurnRecords requires a census doc (a boot caller must handle no-census itself)');
  }
  if (companionDoc == null) {
    throw new Error('flight-companion-gate: buildTurnRecords requires a companion doc (seed it before reserving)');
  }
