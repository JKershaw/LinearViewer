...
/**
 * Resolve whether a task's grounding is provably CURRENT (LIN-1240) — i.e. nothing
 * observable about the task or the code has changed since the last captured snapshot,
 * so a worker can trust its prior grounding instead of re-reading/re-grounding.
 *
 * This is the read-side of the LIN-1238 "suppress redundant re-grounding" chain. It is
 * async because it reads the store; the prompt formatters that consume its result stay
 * pure/synchronous — the proxy route calls this once and threads the plain result in as
 * `context.groundingFreshness`.
 *
 * `fresh` is true ONLY when EVERY conservative check passes:
 *   1. `diffLatest` reports `from && to && !changed` — there are at least TWO snapshots
 *      and the two newest are identical. This is the load-bearing guard: a bare
 *      `changed:false` is NEVER sufficient because `diffLatest` also returns it with
 *      fewer than two snapshots (the LIN-735 false-fresh collapse), so we require
 *      `from && to` explicitly rather than trusting `changed` alone.
 *   2. The latest snapshot's slice-hash equals `currentInputHash`. This reuses the SAME
 *      `hashContext` digest the capture seam stores on `snapshot.inputHash` — no second
 *      hashing scheme is invented here; the caller passes the digest it already computed.
 *   3. The latest snapshot's stored worker HEAD (`snapshot.headSha`, LIN-1239) equals the
 *      reported `currentHeadSha`. An absent reported HEAD, or an absent stored HEAD, is
 *      never fresh — a HEAD move (code changed under an unchanged ticket slice) must fall
 *      back to a full re-ground.
 *
 * Falls back to `{ fresh: false }` (full re-ground) for every other case: store
 * absent/error, missing identifiers, no reported HEAD, no current hash, fewer than two
 * snapshots, a changed slice, a HEAD move, or a missing stored HEAD. Never throws.
 *
 * @param {Object} params
 * @param {{ diffLatest: Function }|null} params.store - a TaskSnapshotStore (or null).
 * @param {string} params.urlKey
 * @param {string} params.taskIdentifier - human identifier, e.g. "LIN-1240"
 * @param {string|null} params.currentHeadSha - worker-reported git HEAD for this read.
 * @param {string|null} params.currentInputHash - `hashContext(context)` for this read.
 * @returns {Promise<{fresh: boolean, capturedAt: (string|null), snapshotRef: (string|null)}>}
 */
export async function resolveGroundingFreshness({ store, urlKey, taskIdentifier, currentHeadSha, currentInputHash } = {}) {
  const notFresh = { fresh: false, capturedAt: null, snapshotRef: null };

  // Conservative preconditions: without a store, a task, a current hash, AND a reported
  // HEAD there is nothing to prove freshness against — re-ground fully.
  if (!store || typeof store.diffLatest !== 'function') return notFresh;
  if (!urlKey || !taskIdentifier || !currentInputHash || !currentHeadSha) return notFresh;

  let diff;
  try {
    diff = await store.diffLatest(urlKey, taskIdentifier);
  } catch (err) {
    console.error('resolveGroundingFreshness diff error:', err?.message || err);
    return notFresh;
  }

  // Require two identical snapshots (from && to && !changed). Asserting `from && to`
  // explicitly is what rejects the <2-snapshot false-fresh case — see the doc above.
  if (!diff || !diff.from || !diff.to || diff.changed) return notFresh;

  const latest = diff.to;
  // Slice-hash match — the current task slice equals what was last captured.
  if (latest.inputHash !== currentInputHash) return notFresh;
  // HEAD match — the worker's code HEAD equals the HEAD stored on that snapshot.
  const storedHead = latest.snapshot?.headSha || null;
  if (!storedHead || storedHead !== currentHeadSha) return notFresh;
