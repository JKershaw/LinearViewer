...
/**
 * True for any of the three decision-lifecycle stamp kinds (LIN-2891/LIN-3037):
 * `decision-answer`, `decision-withdrawn` and `decision-withdrawal-reversed`.
 * These are answer/withdrawal metadata written by `dispatch-store.js`
 * (`markDecisionAnswered` and the withdrawal/reversal writers), not chat
 * turns or free-text session content — every metadata renderer and free-text
 * telemetry reader in that class excludes all three, not just the answer
 * stamp `isDecisionAnswerEntry` alone covered (LIN-1728 Phase 1/2).
 *
 * @param {{kind?: string}} entry
 * @returns {boolean}
 */
export function isDecisionLifecycleStampEntry(entry) {
  return entry?.kind === 'decision-answer'
    || entry?.kind === 'decision-withdrawn'
    || entry?.kind === 'decision-withdrawal-reversed';
}
...
/**
 * Decision-aware live-withdrawal lookup for ONE `decision_id`. Collect
 * `reversedIds` from every valid `kind: 'decision-withdrawal-reversed'` entry
 * (any order — order-independence is what makes reversal terminal regardless
 * of whether the reversal or the withdrawal it cancels comes first in
 * `feedback`), then scan `feedback` backwards for the last
 * `kind: 'decision-withdrawn'` entry whose message parses to
 * `{decision_id: decisionId, reason}` (decision_id equal to the requested id,
 * reason a non-empty string) and whose `decision_id` is not in `reversedIds`.
 * A malformed or incomplete entry — either kind — is skipped and the scan
 * continues, mirroring `_findDecisionAnswer`'s fail-closed discipline. Returns
 * `null` when no live withdrawal remains for `decisionId`.
 *
 ...
 *
 * LIN-2891 F1 (LIN-3034 follow-up): the second `decisionId` argument is
 * REQUIRED and makes this read decision-aware. A row can carry several
 * decision ids (a session that raised `d1` then `d2` on the same item records
 * both), so a plain "last live withdrawal on the row" scalar let a later
 * withdrawal of a MOOT `d1` replace `d2`'s effective withdrawal and reopen a
 * live withdrawn ruling. Scoping the backward scan to the row's CURRENT
 * decision (`deriveLoopFacingFacts` passes `_findLastDecision`'s
 * `decision_id`) keeps the one scalar `withdrawal` field truthful for the
 * decision it is compared against, with no parallel source of truth. Passing
 * a non-string / empty id returns `null` (there is no decision to be withdrawn
 * for). The store's terminal-reversal pre-check passes the caller's target
 * `decisionId`, so a live withdrawal of a non-current (moot) id stays
 * independently reversible.
 *
 * @param {Array<{kind?: string, message?: string, timestamp?: string}>} feedback
 * @param {string} decisionId - the `decision_id` the withdrawal must name
 * @returns {{decisionId: string, reason: string, timestamp: string|null}|null}
 */
export function _findDecisionWithdrawal(feedback, decisionId) {
  if (!Array.isArray(feedback)) return null;
  ...
  // A reversal is terminal for the pair — even a later re-emit of the same
  // withdrawal must not read as live.
  if (reversedIds.has(decisionId)) return null;
  ...
  for (let i = feedback.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      if (
        parsed &&
        typeof parsed.decision_id === 'string' && parsed.decision_id === decisionId &&
        typeof parsed.reason === 'string' && parsed.reason.length > 0
      ) {
        return { decisionId, reason: parsed.reason, timestamp: entry.timestamp ?? null };
      }
    } catch {
      // Malformed stamp — treat as absent and keep scanning backward for an
...
export function deriveLoopFacingFacts(feedback, isoDispatchedAt) {
  ...
  // LIN-2891 F1: decision-aware — the row's OWN current decision id, so a
  // withdrawal of a moot/older decision cannot masquerade as this row's.
  const withdrawal = _findDecisionWithdrawal(feedback, decision?.decision_id);
