...
/**
 * Render the staleness threshold in the compact form the Flight Companion
 * brief uses ("7d", "6h", "30m"). Deliberately MIRRORED from
 * `lib/prompts/flight-companion-brief.js`'s `formatFossilThreshold` rather
 * than imported: this module's static-import allowlist
 * (`tests/unit/observer-pass.test.js`) pins it to three specifiers so a
 * prompt module cannot become a dependency of the pass, and the count is the
 * load-bearing agreement between surfaces — the threshold only has to read
 * sensibly. Falls back to "the staleness threshold" so a malformed value never
 * renders "older than undefined" inside a block the prompt calls ground truth.
 *
 * @param {number} thresholdMs
 * @returns {string}
 */
function formatStaleThreshold(thresholdMs) {
...
export function buildPassMessages({ censusDoc, priorSummary }) {
  ...
  // LIN-2645 (LIN-2619 release companion). Since LIN-2619 the census folds
  // rows past FOSSIL_AGE_MS into `staleAttentionCount` instead of enumerating
  // them, and `truncated` reports ONLY cap truncation of the FRESH population.
  // A fossil-dominated fleet therefore lands under the cap, and without this
  // line the model is told a small attention list with `truncated: false` and
  // no fossil count anywhere — the ticket's motivating failure (a real backlog
  // reading as quiet). State both the count and its threshold, and say
  // explicitly that `truncated` does not cover them.
  const staleAttentionCount = Number.isFinite(state.staleAttentionCount) ? state.staleAttentionCount : 0;
  if (staleAttentionCount > 0) {
    lines.push('');
    lines.push(`Older rows folded out of the list above (NOT included in the rows, and NOT reflected in "truncated"): ${staleAttentionCount} silent/blocked rows older than ${formatStaleThreshold(state.staleAttentionThresholdMs)}.`);
    lines.push('A short attention list can mean a calm fleet or a fossilised one — if the folded count is large, say so; do not read the short list as a clean fleet.');
  }
...
export async function runObserverPass(urlKey, deps) {
  ...
  // LIN-2645 (LIN-2619 release companion): carry the census's fossil fields
  // into `report.*`. The panel renders from `report.*` alone, so without this
  // lift a fossil-dominated fleet's summary line is unreachable (the ticket's
  // item 3). Absent on a pre-LIN-2619 census doc — default to 0/null so the
  // panel renders no line rather than a fabricated zero.
  const staleAttentionCount = Number.isFinite(censusDoc?.state?.staleAttentionCount) ? censusDoc.state.staleAttentionCount : 0;
  const staleAttentionThresholdMs = Number.isFinite(censusDoc?.state?.staleAttentionThresholdMs) ? censusDoc.state.staleAttentionThresholdMs : null;
