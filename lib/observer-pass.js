 ...
 */
export function assessQuietPath({ censusDoc, lastCensusStateHash }) {
 ...
 * @param {Function} [deps.getPaidEnvKey] - `() => string|undefined`
...
export async function runObserverPass(urlKey, deps) {
  ...
  const { quiet, reason: quietReason } = assessQuietPath({ censusDoc, lastCensusStateHash: priorState?.lastCensusStateHash || null });
  ...
  const apiKey = typeof getPaidEnvKey === 'function' ? getPaidEnvKey() : null;
  ...
  const nextState = {
    v: 1,
    summary: clampText(narrative || '', SUMMARY_MAX_CHARS),
    report,
    authority,
 ...
 * @param {Function} [deps.getPaidEnvKey]
