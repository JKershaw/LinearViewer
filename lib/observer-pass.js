...
export function assessQuietPath({ censusDoc, lastCensusStateHash, priorDegraded = false }) {
  ...
  if (lastCensusStateHash && censusDoc.stateHash === lastCensusStateHash) {
    ...
    // restate — and LIN-2405's carry-forward (below in `run`) then republishes
    ...
    //
    // COST, stated rather than waved at. The dominant case — `llm-unavailable`
    // — costs NOTHING to retry: that branch never reaches the model, it just
    // re-checks whether a key resolves now. A model-side degradation
    // (`unparseable`/`missing-narrative`) does re-spend one call per tick while
    // the model keeps failing, which is bounded by the pass's own 15-minute
    // cadence and stops the moment a tick succeeds. That is the right trade:
    // a persistently broken model is a state you want the system recovering
    // from as soon as it can, not one you want frozen into the panel.
...
export async function runObserverPass(urlKey, deps) {
  ...
  const { quiet, reason: quietReason } = assessQuietPath({
    ...
    // LIN-2408. Read off the stored report, which is the only durable record
    // that the last conclusion was not a real one.
    priorDegraded: priorState?.report?.degraded != null
  });
  ...
  if (quiet) {
    ...
    const messages = buildPassMessages({ censusDoc, priorSummary: priorState?.summary || null });
    let buffer = '';
