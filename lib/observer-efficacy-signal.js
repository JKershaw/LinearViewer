 ...
 *    than taking the positionally-next entry. It ALSO skips `kind:
 *    'assistant-text'`/`'evidence'` — the agent's own relayed turn text and
 *    evidence postings (`hook.js`'s `postTurnText`) — on the same structural
 *    argument: the runner and the agent are never an external party, so
 *    their own output is never a response, regardless of its prose (review
 *    R1′, verified live against dispatch `001164ac`: the agent's own
 *    self-check answer at `kind: 'assistant-text'` was the last thing
 *    measured as "the response" before this fix). It ALSO skips the
 *    runner's own `kind: 'status'` self-talk — the stall-failsafe re-ask /
 *    in-place resume prose (`[working …]`) and a repeated `[blocked]`
 *    re-declaring the same state — since a `status` entry can carry either
 *    genuine wake markers or that self-talk under the same `kind` (review
 *    R1, verified live against dispatch `5ac0084e`). Still a proxy for
 *    "when did something respond", not a verified causal link to any
 *    specific actor, but no longer mistaking the runner's or agent's own
 *    bookkeeping/self-talk/relayed-turn-text for a response. `resolved`
 *    reuses
...
// LIN-2263 (F1): feedback-entry `kind`s that are ALWAYS runner/agent
// bookkeeping or self-generated content, never a response, regardless of
// message text — `hook.js`'s `postToolActivityDelta` (`kind: 'tool'`,
// TOOL_ACTIVITY_RELAY) and `postUsageSnapshot` (`kind: 'usage'`,
// WORKER_USAGE_RELAY) both fire right after the `[blocked]` status entry at
// the same Stop boundary; `reapers.js`'s `runHeartbeats` (`kind:
// 'heartbeat'`) and its resource sampler (`kind: 'resources'`,
// RESOURCE_RELAY) post their own periodic telemetry; `hook.js`'s
// `postTurnText` posts the agent's own relayed turn text/evidence (`kind:
// 'assistant-text'`/`'evidence'`) — the session talking to itself, never an
// external party (review R1′: live evidence showed the incumbent arm's last
// remaining non-null measurements were all landing on `assistant-text`).
// Kept as a real skip set — not merely "safe because ACTIVE_PHASES excludes
// BLOCKED" (review R2) — because that phase invariant lives in a different
// repository (simple-dispatcher) this module cannot see or assert against;
// a hand-enumerated allowlist a comment promises is complete is exactly the
// class of bug this ticket exists to fix, so every one of these kinds is
// handled the same structural way — never argued safe by omission. Genuine
// external-response kinds (`decision-answer`, a kindless entry, a real wake
// marker) are untouched by this set.
const RUNNER_BOOKKEEPING_KINDS = new Set(['usage', 'tool', 'heartbeat', 'resources', 'assistant-text', 'evidence']);
 ...
 * data; likewise a `decision`/`decision-answer`/etc. entry always counts.
 *
 * KNOWN-INCOMPLETE BY CONSTRUCTION — LIN-2310 owns the real fix. This predicate
 * infers provenance from `kind`, but `kind` does not encode provenance:
 * `decision`, `assistant-text` and `status` are each written by the runner or
 * the agent on some paths and by an external party on others. A `decision`
 * entry in particular is frequently parsed by `hook.js`'s own
 * `scanDecisionBlock` out of the agent's turn text, so it is NOT the "genuine
 * external content, never runner/agent-emitted" an earlier revision of this
 * comment asserted — that claim was false, and is the reason this note exists.
 * The denylist cannot be completed by adding members: three successive rounds
 * of additions each surfaced another kind the previous round's corpus did not
 * contain. LIN-2310 owns the bounding shape (record provenance at write time,
 * or invert to a positive allowlist so an unclassified kind fails closed).
 * Until that lands, this skips every runner/agent-emitted kind observed on live
 * data — a measured improvement, never a guarantee.
 *
...
export function compareArms(newHarnessSignal, incumbentSignal) {
  return {
    ...
    caveats: [
      ...
      'incumbent.timeToRespondMs measures time to the next REAL feedback entry after a [blocked] marker (LIN-2263/F1: runner-emitted bookkeeping and agent-emitted self-content — kind "usage"/"tool"/"heartbeat"/"resources"/"assistant-text"/"evidence" — are skipped, and a kind "status" entry only counts as a response when it carries a genuine state-change wake marker, not runner self-talk prose or a repeated [blocked] re-declaration), not a verified causal response.',
      'incumbent.timeToRespondMs residue (LIN-2263/R1′): a loop whose only post-[blocked] activity carries kind usage/tool/heartbeat/resources/assistant-text/evidence, or kind "status" self-talk/a repeated [blocked], reports timeToRespondMs: null rather than a fabricated measurement — this covers every runner/agent-emitted kind observed on live data as of this fix, not a guarantee against a future kind this module has not yet been told about.',
      'incumbent.resolved and incumbent.timeToRespondMs can disagree on a [skipped] force-close (LIN-2263/N1): dispatcher.js\'s [skipped] marker is terminal for deriveLifecycleStatus (resolved: true) but is not a wake-marker state change for findNextResponse, so a force-closed loop can report resolved: true alongside timeToRespondMs: null — a force-close is not a response, not a coverage gap.',
