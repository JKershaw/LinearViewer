 ...
 *    this codebase's ledger-item discipline (docs/architecture/prompt-system.md's
 *    close-out section)
 *    warns against.
 ...
 *    `kind: 'decision-answer'` feedback entry — LIN-2310: a POSITIVE
 *    ALLOWLIST of the one kind that is structurally always and only an
 *    external response (`findNextResponse`'s own doc comment has the full
 *    provenance argument), not a denylist of runner/agent-emitted kinds to
 *    skip. Three successive denylist rounds (LIN-2263) each found another
 *    kind the previous round's corpus did not contain — `kind` does not
 *    encode provenance, so no enumeration of "bad" kinds could ever be
 *    complete. `kind: 'decision-answer'` fails closed instead: every
 *    kindless entry, every runner/agent-emitted kind, and every OTHER
 *    genuine wake marker (`[done]`/`[complete]`/`[failed]`/`[aborted]`/
 *    `[pending]`, on any kind) now reads as no-response too, not just
 *    runner/agent self-talk. In practice this makes `timeToRespondMs` `null`
 *    for nearly every loop, because a human's reply to a `[blocked]` loop
 *    normally lands as a NEW dispatch row (`followUpTo`) rather than a
 *    `decision-answer` entry on this one — see `compareArms.caveats[]` for
 *    the full statement of that residue. `resolved` reuses
...
/**
 * Scan forward from `fromIndex` (exclusive) for the first entry stamped
 * `kind: 'decision-answer'`, or null if none exists (or there are no more
 * entries) — i.e. no real response has landed yet.
 *
 * LIN-2310: a POSITIVE ALLOWLIST, not the denylist this predicate used to be.
 * Three successive denylist rounds (LIN-2263) each surfaced another
 * runner/agent-emitted `kind` the previous round's corpus did not contain,
 * because `kind` does not encode provenance — `decision`, `assistant-text`
 * and `status` are each written by the runner or the agent on some paths and
 * by an external party on others, so no enumeration of "bad" kinds can ever
 * be complete. `kind: 'decision-answer'` is different: `grep 'feedback:
 * feedbackEntry'` (`lib/dispatch-store.js`) turns up exactly three writers —
 * `addFeedback` (the runner's Bearer-token path, whose sanitizer's
 * `FEEDBACK_ENTRY_KINDS` allowlist deliberately holds `'decision-answer'`
 * out), `markDecisionAnswered` (session auth, the only path that ever writes
 * it), and `markDecisionWithdrawalReversed` (LIN-2891/LIN-3035, added
 * alongside the other two; session auth, terminal). Provenance is therefore
 * already total and structural at this one kind, with nothing left to infer:
 * `decision-answer` iff a non-runner path could have written it, and every
 * kindless entry and every other kind — including a genuine `[done]`/
 * `[complete]`/`[failed]`/`[aborted]`/`[pending]` wake marker on any kind,
 * not just runner self-talk — now fails closed as "no response yet" rather
 * than being counted. See `compareArms.caveats[]` below for what that means
 * for `timeToRespondMs` in practice.
 *
 * The other two `$push` writers introduced alongside `markDecisionAnswered`
 * (LIN-2891/LIN-3035) are neither a "response" in this module's sense, and
 * `findNextResponse` below is unchanged by them — it still matches only
 * `kind: 'decision-answer'`. Their own provenance is the opposite shape from
 * each other: `'decision-withdrawn'` is runner-writable — it stays IN
 * `FEEDBACK_ENTRY_KINDS` so `addFeedback`'s sanitizer accepts it, bounded by
 * its required `reason` and the item-scoped read rather than by which
 * credential posted it (see `FEEDBACK_ENTRY_KINDS`'s own docblock in
 * `lib/dispatch-store.js` for the class audit) — while
 * `'decision-withdrawal-reversed'` is off-list and exclusive to
 * `markDecisionWithdrawalReversed`, session-auth only, exactly like
 * `decision-answer`. So a withdrawal or its reversal never starts or stops
 * the clock this signal measures.
 *
...
function findNextResponse(feedback, fromIndex) {
  for (let i = fromIndex + 1; i < feedback.length; i++) {
    if (feedback[i]?.kind === 'decision-answer') return feedback[i];
  }
  return null;
}
 ...
 *
 * LIN-2647 (fossil cap, producer-side, recorded as intended): since
 * LIN-2619 the sweep folds any attention-eligible row older than
 * `FOSSIL_AGE_MS` into `staleAttentionCount` instead of enumerating it
 * (lib/observer-sweep.js), so the shadow log stops gaining entries for a
 * loop once it passes that boundary (lib/observer-shadow-log.js's
 * `computeWouldBeActions` — the ruling and its producer-side pin live in
 * that docblock). The consequence for THIS function: for any loop blocked
 * longer than 7 days, `lastLoggedAt` freezes at ≈ the fossil boundary, so
 * `stillBlockedObservedMs` is silently capped at ≈ `FOSSIL_AGE_MS` (minus
 * detection lag) and `relayCount` stops growing, even though the loop is
 * still blocked. Read both fields as "observed while fresh enough to
 * enumerate", never as the full blocked span. `truncated` does NOT flag
 * this bias — it is keyed on the retention cap above, an unrelated bound —
 * so a caller must take this caveat from the docblock, not from the result
 * shape. Pinned by the LIN-2647 cross-module test in
 * tests/unit/observer-efficacy-signal.test.js.
 *
...
export function compareArms(newHarnessSignal, incumbentSignal) {
  return {
    ...
    caveats: [
      ...
      'incumbent.timeToRespondMs measures time to the next kind: "decision-answer" feedback entry after a [blocked] marker (LIN-2310: a positive allowlist, not a denylist — markDecisionAnswered is the ONLY writer of that kind, session-auth only, so no runner/agent-emitted or unclassified kind can ever forge one), not a verified causal response.',
      'incumbent.timeToRespondMs is null for nearly every loop (LIN-2310): a human\'s reply to a [blocked] loop normally lands as a NEW dispatch row (followUpTo), not a decision-answer entry on this row — only markDecisionAnswered\'s own write path (an explicit decision reply) ever produces one. This is the honest, measured result of fail-closed provenance, not a regression or a coverage gap to fix here.',
      'incumbent.resolved and incumbent.timeToRespondMs now diverge for most answered loops, not only a [skipped] force-close (LIN-2310, broadening LIN-2263/N1): deriveLifecycleStatus can call a loop resolved by many routes (a real terminal marker of any kind, a [skipped] force-close, a later wake event on a different row) while findNextResponse only ever counts a decision-answer entry on THIS row — resolved: true alongside timeToRespondMs: null is the common case now, not a narrow disagreement.',
      'findFirstBlockedMarker (this module) is a pure message-prefix regex scan with no kind/source distinction, so it also picks up any [blocked]-prefixed entry from other producers (e.g. Simple Dispatcher\'s own stall-failsafe diagnostics, a sibling track of LIN-2310) exactly like an agent\'s own self-declared BLOCKED. This module\'s [blocked]-wake population is therefore not guaranteed homogeneous in cause — a future consumer of this signal should not assume every wake in it means the same thing.',
