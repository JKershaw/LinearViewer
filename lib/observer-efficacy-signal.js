 ...
 * feedbackEntry'` (`lib/dispatch-store.js`) turns up exactly two writers —
 ...
 * out) and `markDecisionAnswered` (session auth, the only path that ever
 * writes it). Provenance is therefore already total and structural at this
 * one kind, with nothing left to infer: `decision-answer` iff a non-runner
 * path could have written it, and every kindless entry and every other kind
 * — including a genuine `[done]`/`[complete]`/`[failed]`/`[aborted]`/
 * `[pending]` wake marker on any kind, not just runner self-talk — now fails
 * closed as "no response yet" rather than being counted. See
 * `compareArms.caveats[]` below for what that means for `timeToRespondMs` in
 * practice.
 *
