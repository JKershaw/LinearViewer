 ...
 *    feedback entry (a proxy for "when did something respond", not a
 *    verified causal link to any specific actor). `resolved` reuses
 ...
 */
export function computeNewHarnessSignal(shadowLogEntries) {
...
export function computeIncumbentSignal(loops) {
  ...
  for (const loop of loops || []) {
    ...
    const next = feedback[first.index + 1] || null;
...
export function compareArms(newHarnessSignal, incumbentSignal) {
  return {
    ...
    caveats: [
      ...
      'incumbent.timeToRespondMs measures time to the NEXT feedback entry after a [blocked] marker, not a verified causal response.',
 ...
 * only) — never calls recordActions or cleanup.
 *
 * @param {string} urlKey
 * @param {Object} deps
...
export async function collectNewHarnessSignal(urlKey, { observerShadowLogStore } = {}) {
  ...
  return computeNewHarnessSignal(items);
}
