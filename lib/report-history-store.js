...
export const MAX_REPORTS_PER_WORKSPACE = 20;
...
export class ReportHistoryStore {
  ...
  /**
   * Fetch the newest full report that actually carries orientation bearings,
   * skipping any newer report(s) saved with an empty `orientation` (no north
   * star / free-tier limit / stream failure — see LIN-324). Falls back to
   * null when no report for the workspace has ever produced bearings.
   *
   * Backs the Ship view's orientation mode (LIN-1228): unlike getLatest(),
   * which every other consumer relies on for the newest narrative regardless
   * of orientation, this is scoped to orientation availability only so a
   * degraded newest report doesn't hide the last known-good bearings.
   *
   * @returns {Promise<ReportRecord|null>}
   */
  async getLatestWithOrientation(urlKey) {
    if (!this.collection || !urlKey) return null;
    try {
      const docs = await this._docsSorted(urlKey);
      const withOrientation = docs.find(d => Array.isArray(d.orientation) && d.orientation.length > 0);
      return withOrientation ? toRecord(withOrientation) : null;
    } catch (err) {
      console.error('Error getting latest report with orientation:', err);
      return null;
    }
  }

  /**
   * List ALL full report records for a workspace, newest-first.
   *
   * Unlike list() which projects to lightweight summaries (dropping narrative
   * and orientation), this returns toRecord(doc) for every stored doc —
   * including runs saved with orientation: [] (empty bearings are legitimate:
   * no north star / free-tier limit / stream failure).
   *
   * Unlike getLatestWithOrientation() which skips past empty-orientation runs,
   * this includes them. The entire history is returned unfiltered.
   *
   * Additive seam for LIN-1675 P1 (LIN-1683). No route wired to it yet.
   *
   * @param {string} urlKey
   * @returns {Promise<ReportRecord[]>}
   */
  async listFull(urlKey) {
    if (!this.collection || !urlKey) return [];
    try {
      const docs = await this._docsSorted(urlKey);
      return docs.map(toRecord);
    } catch (err) {
      console.error('Error listing full report history:', err);
      return [];
    }
  }
