 ...
 *   subscribe: boolean,       // Push-comms edge declaration (default false). When true this child's terminal events wake its dispatching parent with a follow-up. Stored + forwarded blindly; Harbour reads it only when building the wake follow-up. See LIN-826.
 *   expiresAt: Date           // TTL-based expiration
 * }
 */

import crypto from 'crypto';
import { buildWakeFollowUp } from './dispatch-wake.js';
...
export class DispatchQueueStore {
   ...
   * @param {boolean} [item.subscribe] - Push-comms edge declaration (default false); this child's terminal events wake its dispatching parent; stored + forwarded blindly (LIN-826)
  ...
  async addItem(urlKey, item) {
    ...
    const doc = {
      ...
      // Push-based inter-session comms (LIN-826). Both stored + forwarded
  ...
  async addFeedback(itemId, urlKey, { message, url, urlLabel }, tokenLabel) {
    ...
    try {
      ...
      // Up-chain wake auto-enqueue (LIN-826/LIN-843). If this feedback is a wake
      // event for a SUBSCRIBED child that hasn't already woken its parent, build
      // the parent-addressed follow-up. A wake event is terminal (done/failed/…)
      // OR a `[pending]` *pause* (LIN-843) — the latter labelled "paused (pending),
      // not done" so the parent never reads it as completion. `wakeEnqueued` is a
      // DURABLE once-only guard set in the SAME write as the feedback append, so
      // repeated wake feedback on this child enqueues at most one wake (it is NOT
      // an in-process cache — proxy-dedupe.js is the wrong, best-effort seam). The
      // loop guard is structural: the follow-up is subscribe:false, so it can never
      // itself produce a wake (see lib/dispatch-wake.js).
      const wakeFollowUp = doc.wakeEnqueued ? null : buildWakeFollowUp(doc, updated);

      const setFields = { feedback: updated };
      if (wakeFollowUp) setFields.wakeEnqueued = true;
      ...
      // Enqueue AFTER the durable mark lands (mark-then-enqueue): a wake that
      // fails to enqueue is covered by the runner's --resume net, but a double
      // enqueue is not acceptable. addItem is the existing dispatch seam — the
  ...
  _formatItem(doc) {
    return {
      ...
      // (LIN-827). subscribe: edge declaration that this child's terminal events
      // wake its parent — Harbour reads it when building the wake follow-up.
      queueIfBusy: doc.queueIfBusy === true,
      subscribe: doc.subscribe === true,
