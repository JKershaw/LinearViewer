...
export class DispatchQueueStore {
   ...
   */
  async listItems(urlKey, { issueIdentifier, sessionId, followUpTo, projection } = {}) {
    if (!urlKey) {
      return [];
    }
   ...
   */
  async listHistory(urlKey, { limit, offset = 0, issueIdentifier, since, projection, sessionId, followUpTo } = {}) {
   ...
   */
  async addFeedback(itemId, urlKey, { message, url, urlLabel, kind, rootItemId }, tokenLabel) {
    ...
    try {
      ...
      if (newWake) {
        ...
        if (!isSelfWakeTermination && !(isTerminalWake && edgeDoc.terminalWakeEnqueued)) {
      ...
      if (markTerminalOnEdge) {
        const { matchedCount } = await this.historyCollection.updateOne(
          { _id: edgeDoc._id, terminalWakeEnqueued: { $ne: true } },
          { $set: { terminalWakeEnqueued: true } }
        );
        if (matchedCount !== 1) wakeFollowUp = null;
      }
      if (wakeFollowUp) {
