...
export class DispatchQueueStore {
   ...
   * @returns {Promise<{count: number, alreadyCounted: boolean}|null>} `count`
   *   is the number of distinct `issueIdentifier`s among qualifying rows
   *   BEFORE this candidate; `alreadyCounted` is whether the candidate's own
   *   `issueIdentifier` is already one of them. `null` on a read error.
   */
  async countDistinctTasksForSession(urlKey, sessionId, issueIdentifier) {
    ...
    const findOpts = { projection: { issueIdentifier: 1 } };
    ...
    try {
      ...
      return { count: tasks.size, alreadyCounted: !!issueIdentifier && tasks.has(issueIdentifier) };
    } catch (err) {
   ...
   * @param {number} maxTasks - the new, tighter bound (positive integer)
   * @param {Object} [meta]
   ...
   */
  async trimSessionBudget(urlKey, sessionId, maxTasks, { by = null } = {}) {
    if (!urlKey || !sessionId || !Number.isInteger(maxTasks) || maxTasks < 1) {
    ...
    try {
      ...
      // A previously-unbounded run (maxTasks: null) is always trimmable —
      // turning on a bound for the first time is a downward move from
      // "unbounded". Otherwise the new bound must be strictly tighter.
      if (typeof existing.maxTasks === 'number' && maxTasks >= existing.maxTasks) {
        return { ok: false, reason: 'not-downward' };
      }

      const auditEntry = { at: new Date(), maxTasks, by: by || null };
      const filter = { _id: sessionId, urlKey };
      const update = { $set: { maxTasks }, $push: { trimHistory: auditEntry } };
