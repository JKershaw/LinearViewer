 ...
 *   producingItemId: string,  // Durable wake witness (LIN-1698): on a `kind:'wake'` row ONLY, the `_id` of the producing item whose terminal/blocked feedback minted this wake — the reverse handle `wakeWitnessMeta` (on the edge doc) lacks. Set once at mint time in `_mintWake`; threaded through both `addItem`'s field list and `_archiveItem`'s allowlist so it survives the queue→history archive hop. null for every non-wake row.
...
export class DispatchQueueStore {
   ...
   * seeds `wakeWitnessMeta.<producingItemId>` = `{feedbackIndex, attempt: 0,
   * mintedWakeId: null, lastAttemptAt}` in the SAME atomic `updateOne` as the
   * `terminalWakeItems` CAS — one write, not two. `attempt: 0` is the implicit
   * live-path slot, stamped once at seed time. If the subsequent `addItem`
   ...
   * @private
   */
  async _mintWake(urlKey, edgeDoc, doc, feedback, wakeFollowUp, marker) {
    ...
    const { matchedCount } = await this.historyCollection.updateOne(
      { _id: edgeDoc._id, terminalWakeItems: { $ne: producingItemId } },
      {
        $addToSet: { terminalWakeItems: producingItemId },
        $set: {
          [`wakeWitnessMeta.${producingItemId}`]: {
            feedbackIndex,
            attempt: 0,
            mintedWakeId: null,
            lastAttemptAt: now
    ...
    try {
      await this.historyCollection.updateOne(
        ...
        {
          $set: {
            [`wakeWitnessMeta.${producingItemId}.mintedWakeId`]: minted._id,
            [`wakeWitnessMeta.${producingItemId}.lastAttemptAt`]: new Date()
          }
        }
      );
    } catch (err) {
  ...
  async addFeedback(itemId, urlKey, { message, url, urlLabel, kind, rootItemId }, tokenLabel, provisionWakeCredential) {
    ...
    try {
      ...
      if (markTerminalOnEdge) {
        const mintResult = await this._mintWake(urlKey, edgeDoc, doc, updated, wakeFollowUp, newWake.marker);
