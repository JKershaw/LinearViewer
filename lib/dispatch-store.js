 ...
 */
export const FEEDBACK_ENTRY_KINDS = ['status', 'recap', 'heartbeat', 'evidence', 'assistant-text', 'tool'];
...
/**
 * Milliseconds-since-epoch for a feedback entry's `timestamp` (a `Date` or an
 * ISO string), or `NaN` when missing/unparseable. Shared by
 * `_collectGroupFeedback`'s merge sort so "no valid timestamp" is handled
 * once, explicitly, instead of feeding NaN into a numeric comparator.
 *
 * @param {Date|string|undefined} timestamp
 * @returns {number}
 */
function feedbackTimestampMs(timestamp) {
...
export class DispatchQueueStore {
   ...
   * follow-up's session carries the SAME original rootItemId throughout). So
   * this keeps the cheap indexed `{urlKey, sessionGroupId}` query (index-
   * backed, lib/db-indexes.js) to fetch CANDIDATE siblings, then filters the
   * merge down to entries matching this item's own rootItemId lineage.
   *
   * The lineage anchor is derived from THIS doc's own feedback (the first
   * entry that carries a `rootItemId`), falling back to the doc's own `_id`
   * when it has none yet — correct for the common case where this doc IS the
   * lineage root and just hasn't had a tagged entry posted yet. A sibling
   * entry with no `rootItemId` at all (pre-LIN-1289 legacy data) never
   * matches and is conservatively excluded — the queried item must NOT
   * absorb feedback it can't verify belongs to its own lineage.
   *
   ...
   * sibling query itself fails. See LIN-1461.
   *
   * @param {string} urlKey
  ...
  async _collectGroupFeedback(urlKey, doc) {
    ...
    try {
      siblings = await this.historyCollection
        .find(
          { urlKey, sessionGroupId: groupId, _id: { $ne: doc._id } },
    ...
    // This item's own lineage anchor — see the rootItemId discussion above.
    const anchor = ownFeedback.find(f => f.rootItemId)?.rootItemId || doc._id;

    const siblingFeedback = siblings
      .flatMap(s => s.feedback || [])
      .filter(f => f.rootItemId === anchor);

    // Deterministic sort: timestamp ascending, with entries whose timestamp is
    // missing/unparseable (NaN) sorted after every valid one instead of feeding
    // NaN into the comparator (which — per Array#sort — leaves relative order
    // among the NaN-affected pairs undefined). A tie (equal valid timestamps,
    // or two invalid ones) falls through to `0`, which Array#sort's guaranteed
    // stability (ES2019+) resolves by preserving this array's OWN construction
    // order — own feedback first, then each sibling's own entries in query
    // order — so repeated reads of the same underlying data always return
    // entries in the same order.
    const merged = [...ownFeedback, ...siblingFeedback].sort((a, b) => {
      const at = feedbackTimestampMs(a.timestamp);
      const bt = feedbackTimestampMs(b.timestamp);
    ...
    return this._formatFeedbackEntries(merged);
  }
   ...
   */
  async listItems(urlKey, { issueIdentifier, sessionId, sessionGroupId, followUpTo, projection } = {}) {
    if (!urlKey) {
      return [];
    }
   ...
   */
  async listHistory(urlKey, { limit, offset = 0, issueIdentifier, since, projection, sessionId, sessionGroupId, followUpTo } = {}) {
