...
export class DispatchQueueStore {
   ...
   *   callback, injected by the route so this store stays a mechanism (donor
   *   lookup) while the route owns provisioning policy (mint args, fail-closed
   *   catch). Called ONLY when a wake follow-up is actually about to enqueue.
   *   `reason: null` means no token was wanted (enqueue normally, possibly with
   *   a null bootstrapToken); a non-null `reason` means one was required and
   *   could not be provided, which suppresses the wake for retryability. Absent
  ...
  async addFeedback(itemId, urlKey, { message, url, urlLabel, kind, rootItemId }, tokenLabel, provisionWakeCredential) {
    ...
    try {
      ...
      if (wakeFollowUp && provisionWakeCredential) {
        ...
        const { token, reason } = await provisionWakeCredential(parentStatus?.harness ?? null);
        if (reason) {
          // A credential was required and could not be provided. Withdraw the
          // wake (retryable — a re-report of the same terminal re-enters and
          // provisions again) rather than enqueue a follow-up with no credential
          // a claude-code consumer would need. Do NOT mark the CAS witness.
          wakeFollowUp = null;
          markTerminalOnEdge = false;
          nullWakeReason = reason;
        } else {
          // token may legitimately be null (prose harness) — that is not a
          // failure, so bootstrapToken stays null on the item, as before S2.
