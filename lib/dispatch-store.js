...
export class DispatchQueueStore {
  ...
  async addFeedback(itemId, urlKey, { message, url, urlLabel }, tokenLabel) {
    ...
    try {
      ...
      // loop guard is structural: the follow-up is kind:'wake', so it can never
      // itself produce a wake (see lib/dispatch-wake.js, LIN-901 trap #1). Note the
      // set of bubbling edges is now LARGER — under §5 terminals bubble regardless
      // of subscription, so a previously-silent (non-subscribed) child now wakes on
      // `[done]`/`[failed]`/`[blocked]`; `wakeEnqueued` still bounds it to one.
