...
export class DispatchQueueStore {
  ...
  async addFeedback(itemId, urlKey, { message, url, urlLabel }, tokenLabel) {
    ...
    try {
      ...
      // Up-chain wake auto-enqueue (LIN-826). If this feedback is a terminal wake
      // event for a SUBSCRIBED child that hasn't already woken its parent, build
      // the parent-addressed follow-up. `wakeEnqueued` is a DURABLE once-only
      // guard set in the SAME write as the feedback append, so repeated or
      // post-terminal feedback on this child enqueues at most one wake (it is NOT
      // an in-process cache — proxy-dedupe.js is the wrong, best-effort seam). The
      // loop guard is structural: the follow-up is subscribe:false + carries
      // followUpTo, so it can never itself produce a wake (see lib/dispatch-wake.js).
