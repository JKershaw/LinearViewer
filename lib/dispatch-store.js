...
export class DispatchQueueStore {
    ...
    try {
      ...
      if (this.historyCollection) {
        ...
        if (hist) {
          ...
          formatted.feedback = await this._collectGroupFeedback(urlKey, hist);
          return formatted;
        }
      }
   ...
   * own history doc, not the original's). A follow-up inherits the SAME
   * `sessionGroupId` from its anchor (dispatch-factory.js's inheritance
   * seam), all the way back to the root — so this is one indexed query
   * against the group, not a `followUpTo` chain walk. Without this, the
   * original item's `feedback[]` stays frozen at repoint time and
   * `deriveTerminalStatus` (routes/proxy.js) never sees a terminal marker
   * that only ever landed on a later item — the "dark" symptom in LIN-1461.
   *
   ...
   * it has no `sessionGroupId` (a pre-LIN-1341 legacy row). See LIN-1461.
   *
   * @param {string} urlKey
   * @param {Object} doc - the resolved history doc for the queried item
   * @returns {Promise<Array>} formatted feedback entries across the group,
   *   timestamp-ascending
   * @private
   */
  async _collectGroupFeedback(urlKey, doc) {
    ...
    try {
      siblings = await this.historyCollection
        .find({ urlKey, sessionGroupId: groupId, _id: { $ne: doc._id } })
        .toArray();
    } catch (err) {
    ...
    const merged = [...ownFeedback, ...siblings.flatMap(s => s.feedback || [])];
    merged.sort((a, b) => {
