...
export class DispatchQueueStore {
   ...
   * @returns {Promise<{id: string, dispatchedAt: Date, feedback?: Array|null}|null>}
   *   The newest matching fresh dispatch, or null when there is none (or the
   *   read failed). `feedback` is the prior's OWN stored feedback entries (the
   *   runner's `[working]`/`[failed]`/… markers), added for the duplicate
   *   guard's launch-time-failure exemption (LIN-2872); it is `null` when the
   *   row carries none (a still-`queued` prior, or a test fake).
   */
      ...
      for (const doc of results.flat()) {
        ...
        if (!newest || dispatchedAt > newest.dispatchedAt) {
          newest = {
            id: doc._id,
            dispatchedAt,
            // LIN-2872: the prior's own feedback so the duplicate guard can
            // exempt a launch-time `[failed]` (a row that never started work
            // is not a duplicate risk). `null` when the row carries none.
            feedback: Array.isArray(doc?.feedback) ? doc.feedback : null
          };
        }
      }
      return newest;
