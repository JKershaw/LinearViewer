...
export class DispatchQueueStore {
   ...
   * (lib/digest-feedback.js) confirms `decisionId` names the CURRENT live
   * withdrawal on this item before anything is appended. A never-withdrawn
   * or already-reversed pair returns `null` with NO append — the property
  ...
  async markDecisionWithdrawalReversed(itemId, urlKey, decisionId) {
    ...
    try {
      ...
      const liveWithdrawal = _findDecisionWithdrawal(doc.feedback);
      if (!liveWithdrawal || liveWithdrawal.decisionId !== decisionId) {
        // Never-withdrawn, or already reversed (and thus no longer "live") —
        // both collapse to this one terminal `null`, no append either way.
        return null;
      }
