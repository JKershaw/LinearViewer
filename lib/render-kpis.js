...
function renderChartBox(id, title, { wide = false } = {}) {
  return `      <section class="kpi-chart-box${wide ? ' kpi-chart-wide' : ''}">
        <h3><span class="kpi-tree-glyph">├─</span> ${escapeHtml(title)}</h3>
...
${renderChartBox('chart-activity', 'activity · last 30 days', { wide: true })}

    <div class="kpi-chart-grid">
${renderChartBox('chart-dispatch-outcomes', 'dispatch outcomes')}
${renderChartBox('chart-proxy-status', 'proxy responses')}
${renderChartBox('chart-top-endpoints', 'top proxy endpoints · 30d')}
${renderChartBox('chart-foreman-actions', 'foreman actions · 30d')}
    </div>

${renderChartBox('chart-free-tier', 'free tier prompts · last 7 days', { wide: true })}
