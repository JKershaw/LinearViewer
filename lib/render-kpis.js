...
function renderTerminalMarkedTaskCostCard(cost = {}, planFeeConfig = {}) {
  ...
  return renderCard({
    ...
    body: [
      ...
      `<span class="kpi-cost-sample">${escapeHtml(formatNumber(issueCount))} terminal-marked issues · ${escapeHtml(formatNumber(unpriced))} unpriced (excluded) · ${escapeHtml(formatNumber(noLineageCount))} never observed (no lineage)</span>`,
...
function renderChartBox(id, title, { wide = false, ranges = null, anchorId = null, caption = null } = {}) {
  ...
  // (LIN-2325).
  const captionHtml = caption ? `<span class="kpi-chart-caption">${escapeHtml(caption)}</span>` : '';
...
${renderChartBox('chart-top-endpoints', 'top proxy endpoints · 30d', { ranges: ['30d', '24h'], caption: stats.topEndpointsOtherCount ? `+${formatNumber(stats.topEndpointsOtherCount)} more` : null })}
