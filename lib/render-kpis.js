...
export function renderKpisPage(stats, { deployInfo } = {}) {
  ...
  const cards = [
    ...
    renderStatCard(totals.linearCalls24h, 'linear api calls · 24h'),
    renderStatCard(totals.linearFailures24h, 'linear errors · 24h')
  ].join('\n');
  ...
  return renderPage({
    title: 'KPIs - Linear Projects Viewer',
...
${renderChartBox('chart-linear-calls', 'linear api calls · 24h')}
    </div>
