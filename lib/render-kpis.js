...
import { DEFAULT_USD_PER_POINT_PROVENANCE } from './weekly-budget.js';
 ...
 *
 * `markerOccupiedLineages`/`markerOccupancyShare` (LIN-2423) render directly beside
 * `laneLandedCount` in the sample line: `laneLandedCount === 0` alone cannot distinguish "no
 * lane-landed tickets this window" from "the marker channel itself is empty" — both read as
 * the same zero. The occupancy count/share resolve that ambiguity for a reader without
 * changing what `laneLandedCount` itself means.
 */
function renderTerminalMarkedTaskCostCard(cost = {}, planFeeConfig = {}) {
  const {
    ...
    issueCount = 0,
    unpriced = 0,
    // LIN-2253 review (non-blocking, F2): read the canonical `laneLandedCount`
    // — the rename this ticket itself shipped — never the deprecated
    // `noLineageCount` alias. Migrating this call site is the point of the
    // one-cycle alias: leaving it on the old name means a NEXT-cycle removal
    // of `noLineageCount` fails this card silently (the destructuring
    // default kicks in, both rendered figures below read `0`), not loudly.
    laneLandedCount = 0,
    pricedTicketCount = 0,
    ticketsPerPricedLane = null,
    costUsd = null,
    // LIN-2253 review (blocking, F1): the headline rate's own numerator,
    // matched to `pricedTicketCount`'s inclusion population — see
    // lib/terminal-marked-task-cost.js's module doc comment. Falls back to
    // `costUsd` only when a caller/fixture omits the field entirely (never
    // when the compute layer explicitly emits `null`), so pre-existing
    // stats shapes that predate this field keep degrading sanely.
    pricedTicketCostUsd = costUsd,
    inFlightUsd = null,
    overheadUsd = null,
    ...
    markerOccupiedLineages = 0,
    markerOccupancyShare = null
  } = cost || {};
  ...
  const perTaskUsd = (pricedTicketCostUsd !== null && pricedTicketCount > 0)
    ? pricedTicketCostUsd / pricedTicketCount
    : null;
  ...
  // lineage and are computed over `lineageBearingCount` there. Splitting
  // into a goodness group (still implicitly over issueCount, already named
  // by the sample line above) and an ignorance group with its OWN basis
  // span naming the excluded population is what makes that denominator
  // visible rather than assumed — a disclosure whose basis is unstated is
  // the bug this ticket fixes. `lineageBearingCount` is derived locally
  // from the two fields already on the wire (issueCount, laneLandedCount) —
  // no new field, mirroring the compute layer's own derivation.
  //
  // LIN-2419: the two shares stopped sharing ONE denominator once
  // `opencodeSummed`/`unknownHarness` became three-valued
  // (lib/terminal-marked-task-cost.js) — `unknownHarnessShare`'s known
  // population is still exactly `lineageBearingCount`, but
  // `opencodeSummedShare`'s is narrower (a lineage-bearing issue whose only
  // lineage declared no harness is unknown for that share too, not a known
  // `false`). This basis span can still name `lineageBearingCount` exactly
  // (unknownHarnessShare's true basis) but can only qualify the opencode
  // share's narrower one in words, not a number — publishing that exact
  // count is a new `__KPI_DATA__` field, deliberately deferred to a
  // follow-up rather than landed here.
  const lineageBearingCount = issueCount - laneLandedCount;
  ...
  return renderCard({
    ...
    body: [
      ...
      `<span class="kpi-cost-sample">${escapeHtml(formatNumber(issueCount))} terminal-marked issues · ${escapeHtml(formatNumber(unpriced))} unpriced (excluded), of which ${escapeHtml(formatNumber(laneLandedCount))} never observed (no lineage) · headline ÷ ${escapeHtml(formatNumber(pricedTicketCount))} priced tickets</span>`,
      `<span class="kpi-cost-marker-occupancy">marker channel: ${escapeHtml(formatNumber(markerOccupiedLineages))} lineages occupied (${escapeHtml(formatShare(markerOccupancyShare))}) — distinguishes no lane-landed tickets from an empty channel</span>`,
      ...
      '</span>',
      `<span class="kpi-cost-shares-ignorance-basis">of ${escapeHtml(formatNumber(lineageBearingCount))} with a lineage (${escapeHtml(formatNumber(laneLandedCount))} no-lineage excluded) · opencode share also excludes issues with no declared harness</span>`,
...
function renderWeeklyBudgetGaugeCard(gauge = {}) {
  ...
  // Calibration provenance (LIN-2404): sourced from the imported constant,
  // never from `gauge` — it describes the DEFAULT $/point factor's own
  // derivation (window/sample/date), which stays true regardless of whether
  // an operator override (WEEKLY_BUDGET_USD_PER_POINT) is active for this
  // instance, so it is worded as "default", never as a claim about the
  // figure currently in effect. A caption only, per the LIN-2404 scope
  // sketch — no wire-shape change to the `weeklyBudgetGauge` stats payload.
  const calibrationLabel = `default $/point calibrated ${escapeHtml(DEFAULT_USD_PER_POINT_PROVENANCE.calibratedAt)} `
    + `from ${escapeHtml(formatNumber(DEFAULT_USD_PER_POINT_PROVENANCE.windowPoints))} weekly-window points `
    + `against $${escapeHtml(formatNumber(DEFAULT_USD_PER_POINT_PROVENANCE.spendUsd))} spend `
    + `(${escapeHtml(DEFAULT_USD_PER_POINT_PROVENANCE.source)})`;
  ...
  return renderCard({
    ...
    body: [
      ...
      `<span class="kpi-budget-calibration" title="${calibrationLabel}">${calibrationLabel}</span>`
    ].join('')
  });
}
