...
export class Scheduler {
   ...
   * absent — a duplicate-key throw means a prior boot already seeded it,
   * tolerated as the expected case on every deploy after the first; the
   * seed write must never reset an already-held lease). Does not arm the
   * job's timer — call `start()` for that, once the process is reachable.
   *
   * @param {Object} job
  ...
  async register({ name, intervalMs, leaseMs, run }) {
    ...
    try {
      await this.collection.insertOne({ _id: lockId, lockedUntil: 0 });
    } catch (err) {
