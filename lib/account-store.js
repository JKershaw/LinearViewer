...
export class AccountStore {
   ...
   *     Permanent and one-way — never reassigned once set. A merge where
   *     EITHER side is already merged is refused here — the merged side
   *     re-pointing elsewhere, and (LIN-2265) the canonical side itself
   *     already being merged away, which would otherwise write a two-node
   *     `mergedInto` CYCLE (chain resolution through `mergedInto` is Ticket
   *     B's `resolveCanonicalAccountId`, not a write concern of this method —
   *     but a cycle is a write-path defect, not something resolution can be
  ...
  async mergeAccounts(canonicalId, mergedId, { accountWorkspaceStore, mergeLogStore } = {}) {
    ...
    if (canonical.mergedInto) {
      // LIN-2265: refuse merging INTO an account that is itself already
      // merged away. Without this, a later merge in the opposite direction
      // (this `canonicalId` was the `merged` side of an earlier merge) writes
      // `mergedInto` onto the ORIGINAL canonical account too, producing a
      // two-node cycle — both accounts permanently unresolvable
      // (`resolveCanonicalAccountId` throws for both, forever; no unmerge
      // exists to recover). Symmetric to the `merged.mergedInto` guard below.
      return { ok: false, reason: 'canonical-already-merged', mergedInto: canonical.mergedInto };
    }
