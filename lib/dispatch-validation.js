...
// Known `effort` levels (LIN-2615), used ONLY for a server-side console.warn on
// an out-of-set non-empty value — never to reject the request. Claude Code
// itself warns and runs on an unknown `--effort`, so Harbour must not be
// stricter than the thing it forwards to.
const DISPATCH_EFFORT_LEVELS = ['low', 'medium', 'high', 'xhigh', 'max'];
 ...
 * @param {{maxLength?: number, reportReceivedLength?: boolean}} [options] -
 *   `reportReceivedLength` (default `false`) appends the received length to
 *   the length-cause message, e.g. `goal exceeds maximum length of 1000 (got
 *   1247)`. Off by default so the existing call sites' messages stay
 *   byte-identical (LIN-2075).
 * @returns {{error: string}|null} An error object, or null when valid
 */
function validateOpaqueDispatchField(
  value,
  fieldName,
  { maxLength = MAX_NAME_LENGTH, reportReceivedLength = false } = {}
) {
  ...
  if (value.length > maxLength) {
    const suffix = reportReceivedLength ? ` (got ${value.length})` : '';
    return { error: `${fieldName} exceeds maximum length of ${maxLength}${suffix}` };
  }
...
function validateDispatchPayload(body = {}) {
  const {
    ...
    model, harness, terminal, effort, issueId, followUpTo, target, force, cascade, abort, sessionId
  } = body;
  ...
  // Terminal driver (LIN-2452): the runner's terminal-emulator driver name,
  // same opaque rule — never checked against a driver registry (runner-owned).
  const terminalValidationError = validateOpaqueDispatchField(terminal, 'terminal', { maxLength: MAX_NAME_LENGTH });
  if (terminalValidationError) return terminalValidationError;
  // Effort (LIN-2615): same opaque rule as model/harness/terminal — type/length/
  // dangerous-chars only. An out-of-set level is NEVER rejected (Claude Code
  // itself warns and runs on an unknown `--effort`); it only gets a server-side
  // console.warn, never a 400 and never a `warnings[]` response field.
  const effortValidationError = validateOpaqueDispatchField(effort, 'effort', { maxLength: MAX_NAME_LENGTH });
  if (effortValidationError) return effortValidationError;
  ...
  // meaningful only alongside a verb that HAS a guard for it to override.
  //
  // The verbs that qualify were `followUpTo` (runner-side active-session gate) and
  // `abort` (runner-side human-continued skip). LIN-1656 added a THIRD, on this
  // side of the wire: the duplicate-dispatch guard in `createDispatchItem` reads
  // `force` to let an operator push a deliberate re-dispatch past a same-issue+kind
  // refusal. So a bare `force: true` on a fresh ISSUE-SCOPED dispatch is now
  // accepted — the rule's premise ("the flag would be stored inert") no longer
  // holds for that shape, and it was the premise, not the shape, that was
  // load-bearing.
  //
  // Relaxed exactly that far and no further. With no `issueIdentifier` there is
  // nothing for the duplicate guard to key on, so its gate skips before reading
  // `force` and the flag really would be stored inert — still rejected. And
  // `force`/`cascade` stay mutually exclusive: that rule is about a cascade
  // emitting its own unforced aborts, which LIN-1656 does not touch.
  ...
  if (force === true && !isAbort && (followUpTo === undefined || followUpTo === null) && !issueIdentifier) {
    return { error: 'force requires followUpTo, abort, or an issueIdentifier' };
  }
