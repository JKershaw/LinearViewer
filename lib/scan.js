...
export function parseScanResponse(raw, { issueId, inputHash } = {}) {
  ...
  // on_answer is INJECTED here, never surfaced in the LLM schema above (LIN-2773
  // Area 2) — a scan-produced decision always resolves to `record` on answer,
  // regardless of anything the model returned, so it is spread AFTER `payload`
  // to win over a stray/hallucinated key rather than merge with one.
  const decision = parseDecision(
    JSON.stringify({ ...payload, decision_id: decisionId, on_answer: { effect: 'record' } })
  );
