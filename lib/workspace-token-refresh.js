...
async function doOwnerRefresh({ ownerAccountId, urlKey, provider, refreshAccessToken, store, lifecycleEventStore }) {
  ...
  if (won) {
    // LIN-2329: `isSameCredential` is computed above for the expiry freeze and
    // was then never consulted again, so this recorded `via: 'rotated'`
    // unconditionally — asserting a rotation for an exchange that returned the
    // bytes we already had. The predicate is deliberately `isSameCredential`
    // and not a bytes-only comparison: it also requires a finite stored expiry
    // (M3's guard), and a same-bytes exchange that does NOT freeze behaves like
    // a rotation, so a bytes-only label would contradict the write it describes.
    //
    // `via` on `refresh_success`: 'rotated' (the provider returned new bytes),
    // 'byte-identical' (same bytes AND the stored expiry was frozen),
    // 'converged-race-loser' / 'converged-cas-loser' (this caller lost to
    // another writer and adopted what was durably stored).
    await recordLifecycleEvent(lifecycleEventStore, {
      accountId: ownerAccountId, urlKey, provider,
      kind: CREDENTIAL_LIFECYCLE_EVENT_KINDS.REFRESH_SUCCESS,
