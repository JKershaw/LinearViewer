...
async function doOwnerRefresh({ ownerAccountId, urlKey, provider, refreshAccessToken, store, lifecycleEventStore }) {
  ...
  if (won) {
    // LIN-2329: say which of the two things actually happened. `isSameCredential`
    // is computed above for the expiry freeze (LIN-2097's guard) and was then
    // never consulted again, so this write recorded `via: 'rotated'`
    // UNCONDITIONALLY — asserting a rotation, and by implication a recovery,
    // for an exchange that handed back the bytes we already had and whose
    // expiry we deliberately froze.
    //
    // Not hypothetical: LIN-2327's research recorded 19 byte-identical no-op
    // refreshes on one fingerprint producing 19 × `refresh_success{via:'rotated'}`
    // — an event stream asserting recovery for a credential stuck in an
    // unrecoverable reject → refresh → same-bytes → reject loop. Counting
    // `refresh_success` is the natural first query for "is this recovering?",
    // and on this path it returned the wrong answer with full confidence.
    //
    // A DISTINCT `via` rather than an extra flag alongside `via: 'rotated'`:
    // a flag leaves the natural query still wrong for anyone who does not know
    // to also read it, which is the whole defect. With this, LIN-2327's
    // `refresh_skip{branch:'byte-identical-after-rejection'}` and this write can
    // both be read literally instead of one having to be discounted.
    //
    // The `via` vocabulary on `refresh_success` is now: 'rotated' (the provider
    // genuinely returned new bytes), 'byte-identical' (it returned the same
    // bytes and the stored expiry was frozen), 'converged-race-loser' and
    // 'converged-cas-loser' (this caller lost to another writer and adopted
    // what was durably stored). Each names a distinct thing that happened.
    //
    // This changes only what the event SAYS. The freeze itself is LIN-2097's
    // deliberate behaviour, re-confirmed by LIN-2327's research, and is
    // untouched — as is `refreshToken`, still written unconditionally.
    await recordLifecycleEvent(lifecycleEventStore, {
      accountId: ownerAccountId, urlKey, provider,
      kind: CREDENTIAL_LIFECYCLE_EVENT_KINDS.REFRESH_SUCCESS,
      detail: { via: isSameCredential ? 'byte-identical' : 'rotated' },
    });
