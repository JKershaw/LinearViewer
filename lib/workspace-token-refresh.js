 ...
 */
import { selectExpiredOwnerRow, selectOwnerSessionRow, TOKEN_REFRESH_BUFFER_MS } from './workspace-token-resolver.js';
...
async function doRefresh({ sessions, urlKey, ownerAccountId, refreshAccessToken, resolveExchange, persistSession, resolveProvider, store, fetchImpl, now }) {
  ...
  if (ownerRow) {
    ownerWorkspace = ownerRow.session.workspaces[ownerRow.workspaceIndex];
...
export function refreshOwnerCredential({ ownerAccountId, urlKey, provider, refreshAccessToken, store }) {
  ...
  // LIN-2097: a successful refresh whose resulting expiry is not (comfortably)
  // in the future is not usable — Step 1's freeze-on-byte-identical-exchange
  // means a dead credential's `doOwnerRefresh` result can carry an expiry that
  // has already aged past `now`, or will within the same buffer every selector
  // in workspace-token-resolver.js already requires. Serving THAT as success
  // would cache a dead token as `ok`. This is the ONE choke point all three of
  // doOwnerRefresh's success returns flow through (the `won` branch, and both
  // `convergeOnStored` re-reads — CAS-loss and invalid_grant), so this check
  // applies uniformly without enumerating branches. `result &&` short-circuits
  // a `null` (nothing refreshable) through unchanged.
  return promise.then((result) => (result && !isRefreshResultLive(result)) ? null : result);
}
...
async function doOwnerRefresh({ ownerAccountId, urlKey, provider, refreshAccessToken, store }) {
  ...
  const isSameCredential = tokenData.access_token === record.token;
