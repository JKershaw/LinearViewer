 ...
 *   When the owner DOES still have session row(s) (the `session_expired`
 *   case), the fresh accessToken/tokenExpiresAt are mirrored into EVERY one
 *   of them (LIN-2235 L4.2) as a pure cache via the separate pure sibling
 *   `selectOwnerSessionRows` (no refreshability filter — unlike
 *   `selectExpiredOwnerRow`, it only answers "which rows exist to mirror
 *   into") — never the refreshToken, and never required for the refresh
 *   itself to succeed.
 *
 ...
 */
import { selectExpiredOwnerRow, selectOwnerSessionRow, selectOwnerSessionRows, TOKEN_REFRESH_BUFFER_MS } from './workspace-token-resolver.js';
...
// L4.1 (LIN-2235, amendment A3): Linear's documented refresh-token reuse
// grace — how long a just-rotated refresh token stays acceptable for reuse.
// This is what makes the spend-intent journal's recovery safe: replaying a
// spent-but-unpersisted token inside this window is expected to succeed;
// past it, Linear's own side of that rotation is unrecoverable (see
// `doOwnerRefresh`'s pending-spend check below).
const SPEND_INTENT_GRACE_MS = 30 * 60 * 1000;
...
async function doRefresh({ sessions, urlKey, ownerAccountId, refreshAccessToken, resolveExchange, persistSession, resolveProvider, store, fetchImpl, now }) {
  ...
  const refreshed = await refreshOwnerCredential({ ownerAccountId, urlKey, provider, refreshAccessToken: exchange, store });
  ...
  // EVERY live session row for this owner+workspace (L4.2, LIN-2235) — the
  // same mirroring `ensureValidToken` does on its own session-resident path,
  // preserved here even though this refresh was triggered off-session (e.g. a
  // proxy token resolving while the human's browser session sits idle).
  // `sessions` was already loaded by the caller for the (now largely
  // dead-for-Linear) `selectExpiredOwnerRow` scan above, so this reuses it —
  // no extra read. Looping every row (not just the single latest-expiring
  // one) retires the OTHER rows' stale mirrored copies on the spot instead of
  // leaving them to serve a dead credential until their own next refresh —
  // the shape behind the transient-401 burst on an otherwise-healthy
  // credential (comment `18f2f69d`). No row at all for a `not_connected`
  // owner (e.g. post-logout): nothing to mirror into, which is fine — the
  // durable record alone is authoritative.
  const ownerRows = selectOwnerSessionRows(sessions, urlKey, ownerAccountId);
  // The FIRST row (latest-expiring — same tie-break `selectOwnerSessionRow`
  // used) still supplies the binding a structured (Jira) call scope is built
  // from below; every row gets the same accessToken/tokenExpiresAt mirror.
  let ownerWorkspace = null;
  for (const row of ownerRows) {
    const ws = row.session.workspaces[row.workspaceIndex];
    ws.accessToken = refreshed.token;
    ws.tokenExpiresAt = refreshed.expiresAt;
    if (!ownerWorkspace) ownerWorkspace = ws;
    await persistSession(row.sid, row.session);
  }
 ...
 */
export function refreshOwnerCredential({ ownerAccountId, urlKey, provider, refreshAccessToken, store }) {
  const partition = normalizeProviderName(provider);
  const key = `${ownerAccountId}::${urlKey}::${partition}`;
  let promise = inflight.get(key);
  if (!promise) {
    promise = doOwnerRefresh({ ownerAccountId, urlKey, provider: partition, refreshAccessToken, store });
...
async function doOwnerRefresh({ ownerAccountId, urlKey, provider, refreshAccessToken, store }) {
  ...
  // L4.1 spend-intent journal (LIN-2235, amendment A3): `record.pendingSpend`
  // is a marker a PRIOR attempt left before spending `attempted` — present
  // and still naming the SAME token means that prior attempt died (process
  // exit, crash) somewhere between here and the durable CAS write below, so
  // its outcome at Linear was never recorded. Within Linear's documented
  // reuse grace, replaying `attempted` below is safe (Linear tolerates the
  // reuse, so the exchange just below succeeds normally). Past grace, that
  // prior attempt's fate is unrecoverable — the SAME `invalid_grant` →
  // re-read → unchanged-record → EXPIRED path every subsequent resolve would
  // independently rediscover, at the cost of a real network round-trip each
  // time. Short-circuit it here instead: one loud, distinguishable failure
  // now rather than retrying a token already known to be dead.
  const pending = record.pendingSpend;
  if (pending?.refreshToken === attempted && Date.now() - pending.attemptedAt > SPEND_INTENT_GRACE_MS) {
    throw new TokenRefreshError(
      `Owner credential spend-intent for ${urlKey} unresolved past Linear's reuse grace ` +
      `(attempted ${new Date(pending.attemptedAt).toISOString()}) — presumed credential-dead`,
      'UNKNOWN'
    );
  }

  // Written BEFORE spending: a crash anywhere after this line (the exchange
  // itself, or after it but before the CAS write below) leaves this durable,
  // so the check above can tell "already tried this" from "first attempt" on
  // the next resolve. Any write that resolves this record's credential fields
  // afterward — the CAS win below, or a fresh `put` from re-authentication —
  // clears it as a side effect (see owner-credential-store.js), so it only
  // ever survives past this call when nothing ran to clear it.
  await store.markPendingSpend(ownerAccountId, urlKey, provider, attempted, Date.now());
 ...
 */
export async function refreshOwnerWorkspaceToken({ sessions, urlKey, ownerAccountId, refreshAccessToken, resolveExchange, persistSession, resolveProvider, store, fetchImpl, now }) {
  ...
  return doRefresh({ sessions, urlKey, ownerAccountId, refreshAccessToken, resolveExchange, persistSession, resolveProvider, store, fetchImpl, now });
}
