...
import { updateWorkspaceTokens, remintActiveCredential } from './workspace.js';
...
async function doRefresh({ sessions, urlKey, ownerAccountId, refreshAccessToken, persistSession, resolveProvider, fetchImpl, now }) {
  ...
  // byte-identical. updateWorkspaceTokens is Linear-wire-shaped
  // (access_token/refresh_token/expires_in) — a GitHub patch must never reach
  // it, so the minting arm goes through remintActiveCredential -> linkProvider
  // exclusively, which mutates `workspace` (== session.workspaces[workspaceIndex])
  // in place so the shared persistSession below captures it either way.
 ...
 * @param {Function} [deps.now] - test seam forwarded to the provider's refreshCredential (deterministic clock)
 * @returns {Promise<{token: string, expiresAt: number, provider: string}|null>}
 */
export async function refreshOwnerWorkspaceToken({ sessions, urlKey, ownerAccountId, refreshAccessToken, persistSession, resolveProvider, fetchImpl, now }) {
  const key = `${ownerAccountId}::${urlKey}`;
  let promise = inflight.get(key);
  if (!promise) {
    promise = doRefresh({ sessions, urlKey, ownerAccountId, refreshAccessToken, persistSession, resolveProvider, fetchImpl, now });
