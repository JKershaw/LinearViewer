...
/**
 * Shape a durable record as a successful refresh result.
 * Exported (LIN-2473) so server.js's `attemptSuspectCredentialRefresh` can
 * reuse this exact shaping when adopting a durable credential that has
 * already superseded a rejected one, without spending an OAuth exchange.
 */
export function convergeOnStored(record) {
