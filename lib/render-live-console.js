...
export function renderLiveConsolePage(options = {}) {
  ...
  // Status banner: one live line the client refreshes each poll. Tempo sparkline
  // sits alongside it as the system's "rhythm".
  const bannerHtml = `<div class="lc-banner" data-testid="live-console-banner">
        ...
        <span class="lc-status-line" id="live-console-status">connecting to the feed…</span>
        <span class="lc-tempo" aria-hidden="true"><canvas id="live-console-tempo" width="160" height="28"></canvas></span>
      </div>`;
  ...
  const lanesHtml = `<section class="lc-lanes-section" aria-label="Working now">
        ...
        ${renderEmptyState({ tag: 'p', className: 'lc-lanes-empty', id: 'live-console-lanes-empty', text: '○ no agents working right now' })}
      </section>`;
  ...
  const streamHtml = `<section class="lc-stream-section" aria-label="Activity stream">
        ...
        <ol class="lc-stream" id="live-console-stream" data-testid="live-console-stream" aria-live="polite"></ol>
        ${renderEmptyState({ tag: 'p', className: 'lc-stream-empty', id: 'live-console-stream-empty', text: '○ waiting for the first update…' })}
      </section>`;
