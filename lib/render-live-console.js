...
export function renderLiveConsolePage(options = {}) {
  ...
  //
  // LIN-1505 Phase C: strip-only zoom controls, wrapping (not replacing) the
  // pre-existing `.lc-pulse` div — its class/id/position are unchanged so the
  // mount-order pin (tests/unit/live-console-wiring.test.js:119) and the
  // `.lc-pulse` geometry pin (LIN-1741) both still hold; only new siblings are
  // added around it. Reuses the timeline's `.lc-timeline-preset*` classes
  // verbatim (same idiom, zero new button chrome) rather than inventing a
  // second zoom control style — the strip's own zoom is independent of the
  // timeline's (never coupled, never shares state), but they look and behave
  // like the same control. 3m carries the initial `aria-pressed="true"`
  // (matches the always-3min-on-load default) since, unlike the timeline's
  // `fit`, the strip's default never depends on data.
  const pulseHtml = `<section class="lc-pulse-section" aria-label="Live activity strip">
        <div class="lc-timeline-presets lc-pulse-presets" role="group" aria-label="Strip range" data-testid="live-console-pulse-presets">
          <button type="button" class="lc-timeline-preset" data-testid="live-console-pulse-preset-3m" data-range="3m" aria-pressed="true">3m</button>
          <button type="button" class="lc-timeline-preset" data-testid="live-console-pulse-preset-15m" data-range="15m" aria-pressed="false">15m</button>
          <button type="button" class="lc-timeline-preset" data-testid="live-console-pulse-preset-1h" data-range="1h" aria-pressed="false">1h</button>
          <button type="button" class="lc-timeline-preset" data-testid="live-console-pulse-preset-6h" data-range="6h" aria-pressed="false">6h</button>
          <span class="lc-pulse-span-text" id="live-console-pulse-span-text" data-testid="live-console-pulse-span-text">last 3 minutes</span>
        </div>
        <div class="lc-pulse" title="live activity — heartbeats and events, newest on the right">
  ...
  // panel of recent dispatch runs below the ambient strip. All layout/paint/
  // gesture wiring is client-side (public/live-console.js's paintTimeline +
  // the zoom/pan gesture handlers, using lib/timeline-zoom.js's shared pure
  // helper) — this is only the shell + mount points. The `fit`/1h/24h preset
  // buttons are wired client-side by public/live-console.js's
  // wireTimelineGestures; `fit` is the default-on-load window (LIN-1928, a
  // first-paint latch derived from the initial feed's runs), so it — not
  // `24h` — carries the initial `aria-pressed="true"` here, and the section
  // label/aria-label/empty-state text below are placeholders the client
  // overwrites with the actual active window as soon as it's known (see
  // updateTimelineWindowText). The `<svg>` inside the bars viewport is the
  // run-to-run connector overlay (paintTimelineConnectors) — a static, empty
  // mount point; it must come before any bar node so bars (appended later via
  ...
  const timelineHtml = `<section class="lc-timeline-section" id="live-console-timeline-section" aria-label="Last 24 hours">
        <div class="lc-section-label"><span class="lc-section-mark" aria-hidden="true">▤</span> <span id="live-console-timeline-label-text">last 24 hours</span></div>
        <div class="lc-timeline-presets" role="group" aria-label="Timeline range">
          <button type="button" class="lc-timeline-preset" data-testid="live-console-timeline-preset-fit" data-range="fit" aria-pressed="true">fit</button>
          ...
          <button type="button" class="lc-timeline-preset" data-testid="live-console-timeline-preset-24h" data-range="24h" aria-pressed="false">24h</button>
        </div>
        ...
        ${renderEmptyState({ tag: 'p', className: 'lc-timeline-empty', id: 'live-console-timeline-empty', attrs: 'hidden', text: '○ no runs yet' })}
      </section>`;
