...
export function renderLiveConsolePage(options = {}) {
  ...
  // Swimlane timeline (LIN-1742, Phase 1 of LIN-1720): a static (non-zoomable)
  // last-24h panel below the ambient strip. All layout/paint is client-side
  // (public/live-console.js's paintTimeline) — this is only the shell + mount
  // points. The 1h/24h preset buttons are inert here (no click wiring): they
  // exist so Phase 2 (zoom/pan, lib/timeline-zoom.js) can wire behavior onto
  // them without a markup change.
  //
  // Only the axis+bars sub-wrapper (`.lc-timeline-breakout`) goes full-bleed —
  // the label/presets stay in `.lc-page`'s normal centered column, aligned
  // with every other section label on this page. A full-section breakout was
  // tried and reverted: see the CSS comment on `.lc-timeline-breakout` in
  // public/live-console.css for why breaking out the WHOLE section (label
  // included) silently clipped it to invisibility on desktop.
  const timelineHtml = `<section class="lc-timeline-section" aria-label="Last 24 hours">
        ...
        <div class="lc-timeline-breakout">
          <div class="lc-timeline-axis" id="live-console-timeline-axis" aria-hidden="true"></div>
          <div class="lc-timeline-viewport" id="live-console-timeline" data-testid="live-console-timeline"></div>
        </div>
