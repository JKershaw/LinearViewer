...
//   { model, harness, byKind: { <PROMPT_TEMPLATES key>: { model, harness } } }
// Both fields stay opaque strings (no registry/validation), matching the
// dispatch item's own model/harness fields. `byKind` is scoped to the 14
// PROMPT_TEMPLATES keys only — meta/periodical pass-through kinds like
// autopilot/defer/custom are not user-configurable "types" and never look up
// a per-kind override. Writing dispatchDefaults uses the plain read-merge-
...
export async function resolveDispatchDefaults({ urlKey, kind, store }) {
  ...
  const byKind = (kind && kind in PROMPT_TEMPLATES && defaults.byKind?.[kind]) || {};
  return {
