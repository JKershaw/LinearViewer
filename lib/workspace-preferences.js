 ...
 * overrides. Scoped to the 7 v1 kinds (LIN-2197 Phase 5 added `scan`, closing
 * Phase 4 close-out ledger item L1 — the scan route already reads
 * `byKind.scan` via `resolveAiOperationModel({ opKind: 'scan' })`, but the
 * override was unreachable through the product since `scan` was absent from
 * this list); experimental operations and the trivial `feedback-title` call
 * are deliberately excluded.
 */
export const AI_OPERATION_KINDS = ['recommend', 'recap', 'brief', 'scan', 'run-summary', 'session-summary', 'next-run'];
