 ...
 *
 * LIN-1694: `harnessInForce` (optional 3rd arg) is the harness that will
 * ACTUALLY run this dispatch, resolved by the CALLER (`dispatch-factory.js`'s
 * two-pass harness-then-model resolution) — never re-derived here. Both
 * `byKind[kind]` and the top-level `cfg` are each their own ROW (a
 * `{model, harness}` pair saved together); a row's `model` is eligible only
 * if that row's own `harness` is blank or equal to `harnessInForce`. This
 * closes the class this ticket is about: an explicit incoming `harness`
 * (or an anchor/preset harness) must not import a `model` from a row that
 * was configured for a DIFFERENT harness. Omitted ⇒ `!harnessInForce` ⇒
 * every row eligible ⇒ byte-identical to the pre-LIN-1694 2-arg behaviour;
 * `harnessInForce === null` (e.g. an all-blank-harness workspace) means the
 * same "nothing to conflict with yet" case. A row whose own harness is
 * blank stays eligible regardless — blank still means inherit. An
 * ineligible `byKind.model` falls through to `cfg.model`, not straight to
 * null, so a lower-precedence eligible row still applies.
 *
 ...
 * @param {string|null} [harnessInForce] - The harness this dispatch will actually run on
 * @returns {{model: string|null, harness: string|null}}
 */
export function resolveRoutingFromConfig(config, kind, harnessInForce) {
  const cfg = config || {};
  const byKind = (kind && DISPATCH_DEFAULT_KINDS.includes(kind) && cfg.byKind?.[kind]) || {};
  const eligible = (row) => !harnessInForce || !row.harness || row.harness === harnessInForce;
  const byKindModel = eligible(byKind) ? byKind.model : undefined;
  const cfgModel = eligible(cfg) ? cfg.model : undefined;
  return {
    model: byKindModel ?? cfgModel ?? null,
    harness: byKind.harness ?? cfg.harness ?? null
  };
}
 ...
 * @param {WorkspacePreferencesStore} options.store - The workspace preferences store
 * @param {string|null} [options.harnessInForce] - LIN-1694: the harness this dispatch will
 *   actually run on, passed straight through to `resolveRoutingFromConfig`'s row-atomic
 *   eligibility guard. Omitted ⇒ byte-identical to the pre-LIN-1694 behaviour.
 * @returns {Promise<{model: string|null, harness: string|null}>}
 */
export async function resolveDispatchDefaults({ urlKey, kind, store, harnessInForce }) {
  if (!urlKey || !store) {
