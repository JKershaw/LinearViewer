...
/**
 * Validates that a string is safe to embed inside a double-quoted shell word.
 * Double quotes are weaker than single quotes: `$`, backtick, backslash and
 * `"` still have special meaning inside them.
 *
 * @param {string} value
 * @returns {boolean}
 */
 ...
 *
 * Quoting strategy:
 *  - Values that are pure UUIDs/URLs/base64url are interpolated inside
 *    double quotes so `$HOME` expansion works for the workspace path.
 *  - The repo and staging file path are single-quote wrapped (after the
 *    isSafeShellLiteral check, which rejects `'` and control chars).
 *  - `$HOME` — not `~` — is used because tilde inside quotes does NOT
 *    expand in POSIX shells.
 *
 * @param {Object} params
 * @param {string} params.repo - Repo URL or shorthand (e.g. github.com/user/repo)
 * @param {string} params.dispatchId - UUID used as the workspace dir name
 * @param {string} params.stagingFilePath - Absolute path to the prompt staging file
 * @param {string} params.feedbackUrl
 * @param {string} params.token
 * @returns {string}
 */
function buildHarbourCommand({ repo, dispatchId, stagingFilePath, feedbackUrl, token }) {
  const sq = (v) => `'${v}'`;
  // Workspace path: $HOME (shell-expanded) + "/harbour-workspaces/" + <uuid>.
  // dispatchId is a UUID, so it's safe inside double quotes without further escape.
  const workspacePath = `"$HOME/harbour-workspaces/${dispatchId}"`;
  return [
    `export HARBOUR_DISPATCH_ID="${dispatchId}" HARBOUR_DISPATCH_FEEDBACK_URL="${feedbackUrl}" HARBOUR_DISPATCH_TOKEN="${token}"`,
    `git clone ${sq(repo)} ${workspacePath}`,
    `cd ${workspacePath}`,
    `exec claude "$(cat ${sq(stagingFilePath)})"`
  ].join(' && ');
}
