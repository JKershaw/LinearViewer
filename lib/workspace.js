 ...
 */
export async function remintActiveCredential(workspace, provider) {
  ...
  const refreshed = await provider.refreshCredential(active);
...
export function unlinkProvider(workspace, provider, scope) {
  ...
  if (workspace.provider === removed.provider) {
    ...
    if (next) {
      workspace.provider = next.provider;
      workspace.credentials = { ...next.credentials };
      workspace.accessToken = next.credentials?.token;
      workspace.refreshToken = next.credentials?.refreshToken;
      workspace.tokenExpiresAt = next.credentials?.tokenExpiresAt;
    } else {
...
export function getWorkspaceCallScope(workspace) {
  ...
  if (workspace?.provider !== 'github') return token;
  const bindings = getBindingsForWorkspace(workspace);
  const active =
    bindings.find(b => b.provider === 'github' && b.credentials?.token === token) ||
    bindings.find(b => b.provider === 'github');
  return { token, repo: active?.scope };
}
