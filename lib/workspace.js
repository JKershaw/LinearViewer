 ...
 * `workspace.urlKey`. Shared by {@link rotateOwnerCredential} (the rotation
 * seam) and the OAuth acquisition sites (`routes/auth.js`), which reach this
 * directly since `workspace` there is already fully populated by
 * `linkProvider` — there is no `updateWorkspaceTokens` call to wrap.
 *
 * @param {string} accountId - identity the durable record is keyed on
 * @param {import('./workspace.js').Workspace} workspace - already-mutated workspace to read the credential off
 * @param {import('./owner-credential-store.js').OwnerCredentialStore} store
 */
export async function persistOwnerCredential(accountId, workspace, store) {
  const active = workspace.bindings?.find(b => b.provider === workspace.provider);
...
/**
 * Async seam ABOVE `updateWorkspaceTokens` (LIN-1523, Session 1 of LIN-1501)
 * — never inside it. `updateWorkspaceTokens` stays pure and synchronous, so
 * its existing unit tests stay green untouched; every Linear rotation call
 * site is re-pointed through this seam instead, so every rotation emits
 * exactly ONE durable write from ONE writer (the single-ownership test the
 * plan requires) without ever making the mutator itself async.
 *
 * Deliberately NOT `linkProvider`: `linkProvider` always creates/upserts a
 * binding, while `updateWorkspaceTokens` never does and guarantees legacy
 * un-migrated workspaces stay byte-identical — delegating would silently
 * synthesize `bindings[]` on every legacy workspace's next refresh.
 *
 * Mirroring asymmetry (do not "tidy" in either direction): `accessToken` and
 * `tokenExpiresAt` stay mirrored into the session via `updateWorkspaceTokens`
 * as a pure cache (dropping it forces a full refresh round-trip on every
 * post-logout resolve, ~30.3s worst case). `refreshToken` is the durable,
 * rotate-on-every-use field and is written ONLY to `store` here — it must
 * never be mirrored back into the session, or a second rotating copy bricks
 * itself via `invalid_grant` (rejected approach E).
 *
 * LIN-1524 close-out Finding #4 (legacy no-`accountId` session, decided —
 * NOT backfilled here): `accountId` reaches this seam unguarded from
 * `req.session.accountId`. `store.put` already fails safe on a missing one
 * (warns, returns `false`, never throws — see owner-credential-store.test.js's
 * direct guard test and this module's own composed test), so the session-side
 * rotation above is never blocked by it. This is genuinely REACHABLE, not
 * theoretical: LIN-1329 (which routes every sign-in path through
 * `establishAccount` to set `session.accountId`) landed 2026-07-15, and
 * sessions carry a 30-day rolling TTL from last activity, so a session that
 * predates LIN-1329 and keeps getting used can still be alive with no
 * `accountId` today. A live backfill would need to re-derive the session's
 * provider-side identity (e.g. a Linear `fetchViewer` call) inside this hot
 * refresh path purely to mint/attach an account it has no other reason to
 * touch — a bigger, riskier seam than this rotation call warrants. Accepted
 * consequence: such a session keeps rotating its session-side token exactly
 * as it does today; it simply gains no durable fallback, so it degrades to
 * pre-LIN-1524 behaviour (no durable-read survival across logout/session-loss)
 * rather than failing outright. Not a regression — a bounded, self-expiring
 * gap (worst case 30 days from LIN-1329's landing) that a follow-up can close
 * with a dedicated identity-backfill design if it proves worth doing.
 *
 * @param {Object} params
 * @param {string} params.accountId - identity the durable record is keyed on
 * @param {import('./workspace.js').Workspace} params.workspace - mutated in place by updateWorkspaceTokens
 * @param {Object} params.tokenData - OAuth token response, same shape `updateWorkspaceTokens` takes
 * @param {import('./owner-credential-store.js').OwnerCredentialStore} params.store
 */
export async function rotateOwnerCredential({ accountId, workspace, tokenData, store }) {
  updateWorkspaceTokens(workspace, tokenData);
  await persistOwnerCredential(accountId, workspace, store);
}
