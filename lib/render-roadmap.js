...
/**
 * Render a single shipped item as a two-line, hierarchical entry (LIN-1222):
 * the title wraps on its own line as a human label (sans), with `project · date`
 * as a secondary machine-fact meta line beneath it (mono, muted). Nothing is
 * truncated at any width — this replaces the former dense log row whose
 * right-rail `project · date` column + nowrap/ellipsis title read as raw log
 * output and clipped on mobile.
 */
function renderShipItem(item) {
  ...
  // box-less inline glyph (no pill chrome per list row); it aligns to the first
  // line of the (now wrapping) title and preserves the green ✓ through the
  // `done` state colour.
...
export function renderRoadmapPage(data, options = {}) {
  ...
  const { deployInfo = {}, urlKey = null, openRouterSource = null, workspaces = [], featureFlags = {}, availableModels = [], teams = [], selectedTeamId = null } = options;
  ...
  const navBarHtml = renderNavBar({ workspaces, teams, selectedTeamId, urlKey, currentPage: 'roadmap', featureFlags });
  ...
  // Each fact is its own atomic flex item; the `·` separators between them are
  // CSS pseudo-elements (see .roadmap-header-meta), NOT literal text — so at 390
  // the row wraps fact-by-fact and never orphans a dangling separator (LIN-1222).
  const metaFacts = [
    workspaceName ? `<span class="roadmap-header-workspace">${escapeHtml(workspaceName)}</span>` : '',
  ...
  // Pricing (LIN-993) rides as a data-attribute, not option text; the hint line
  // below the selector shows the selected model's rate (Workspace default has none).
  const modelOptions = [
  ...
  const modelSelectorHtml = hasAI
    ? `<label class="roadmap-model-select-label">
          ...
          <span class="roadmap-model-price" id="roadmap-model-price" data-model-price></span>
        </label>`
    : '';
