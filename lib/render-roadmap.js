...
/**
 * Render the ship log (Recently shipped) section.
 */
function renderShipLogSection(milestones) {
  const { total, buckets } = buildShipLog(milestones);

  if (total === 0) {
    return `<section class="roadmap-section roadmap-ship-log">
      <h2 class="roadmap-section-heading">│ Recently shipped</h2>
      <p class="roadmap-empty">No completions in the last 90 days.</p>
    </section>`;
  }
  ...
  return `<section class="roadmap-section roadmap-ship-log">
    <h2 class="roadmap-section-heading">│ Recently shipped <span class="roadmap-section-total">(${total} in 90 days)</span></h2>
    ${bucketsHtml}
  </section>`;
}
...
function renderProjectCard(milestone, criticalPaths, executionQueue, risks) {
  ...
  if (milestoneRisks.length > 0) {
    const riskItems = milestoneRisks.map(risk => {
      ...
      return `<div class="roadmap-risk roadmap-risk--${severity}"><span class="roadmap-risk-sev">${severity}</span> ${description}</div>`;
    }).join('');
  ...
  return `<div class="roadmap-milestone-card" data-milestone-name="${escapeHtml(milestoneName)}">
    <div class="roadmap-milestone-header">
      <h3 class="roadmap-milestone-name">${name}</h3>
      <span class="roadmap-milestone-progress-text">${pct}% · ${done}/${total} done</span>
    </div>
    ${descriptionHtml}
    <div class="roadmap-milestone-progress">
      <code class="roadmap-progress-bar">${progressBar}</code>
    </div>
...
export function renderRoadmapPage(data, options = {}) {
  ...
  const headerHtml = `<header class="roadmap-header">
  ...
  const shipLogHtml = renderShipLogSection(milestones);
  ...
  const projectsSectionHtml = milestones.length > 0
    ? `<section class="roadmap-section roadmap-milestones">
        <h2 class="roadmap-section-heading">│ By project</h2>
  ...
  // Server renders the static skeleton — north star input, generate button,
  // and five layer sections — so layout doesn't reflow as streaming begins.
  ...
  const layerSection = (layerId, heading) => `
        <div class="roadmap-layer" data-layer="${layerId}" data-state="idle">
...
<body>
  ...
  <main class="roadmap-page" data-url-key="${encodedUrlKey}">
    ...
    <section class="${chatClass}" id="roadmap-chat">
