...
export function renderRoadmapPage(data, options = {}) {
  ...
  const milestoneCardsHtml = milestones.map(milestone => {
    const name = escapeHtml(milestone.projectName || milestone.name || 'Untitled');
    ...
    const projectedWeeks = milestone.projectedWeeks ?? 0;
    const confidenceLow = milestone.confidenceRange?.low ?? projectedWeeks;
    const confidenceHigh = milestone.confidenceRange?.high ?? projectedWeeks;
    ...
    const milestoneId = milestone.id || milestone.projectId || '';
    const pathKey = milestoneId;
    const criticalPath = criticalPaths instanceof Map
      ? (criticalPaths.get(pathKey) || [])
      : (criticalPaths[pathKey] || []);

    let criticalPathHtml = '';
    if (criticalPath.length > 0) {
      const pathItems = criticalPath.map((item, idx) => {
        const prefix = idx < criticalPath.length - 1 ? '├─' : '└─';
        const statusIcon = item.stateType === 'completed' ? '✓'
      ...
      criticalPathHtml = `
          <div class="roadmap-critical-path">
            <div class="roadmap-path-heading">│ critical path</div>
            ${pathItems}
          </div>`;
    }

    // Risk indicators for this milestone
    const milestoneRisks = risks.filter(r => r.milestoneId === milestoneId || r.projectId === milestoneId);
    let risksHtml = '';
    ...
    return `
        <div class="roadmap-milestone-card" data-milestone-id="${escapeHtml(milestoneId)}">
          ...
          <div class="roadmap-milestone-stats">
            ...
            <span>projected: ~${escapeHtml(String(projectedWeeks))} weeks</span>
            <span>confidence: ${escapeHtml(String(confidenceLow))}–${escapeHtml(String(confidenceHigh))} weeks</span>
