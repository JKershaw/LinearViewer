...
export function renderRoadmapPage(data, options = {}) {
  ...
  const milestoneCardsHtml = milestones.map(milestone => {
    ...
    // Project description (truncated for display)
    ...
    // Subtask tree — show tasks with their children
    const tasksInQueue = milestone.tasksInQueue || [];
    let taskTreeHtml = '';
    if (tasksInQueue.length > 0) {
      const taskItems = tasksInQueue.slice(0, 8).map(task => {
        ...
        if (task.subtasks && task.subtasks.length > 0) {
          const r = task.rollup || {};
          const rollupText = `(${r.subtaskDone || 0}/${r.subtaskTotal || task.subtasks.length} subtasks)`;
          const childItems = task.subtasks.map((sub, idx) => {
            ...
            return `<div class="roadmap-task-child">${prefix} ${subIcon} ${subId} ${subTitle}</div>`;
          }).join('\n              ');

          return `<div class="roadmap-task-parent">${statusIcon} ${taskId} ${taskTitle} ${rollupText}
              <div class="roadmap-task-children">
              ${childItems}
              </div>
            </div>`;
        }
        return `<div class="roadmap-task-item">${statusIcon} ${taskId} ${taskTitle}</div>`;
      }).join('\n            ');

      taskTreeHtml = `
          <div class="roadmap-task-tree">
            <div class="roadmap-tree-heading">│ tasks</div>
            ${taskItems}
          </div>`;
    }
    ...
    return `
        <div class="roadmap-milestone-card" data-milestone-name="${escapeHtml(milestoneName)}">
          ...
          </div>${taskTreeHtml}${criticalPathHtml}${risksHtml}
        </div>`;
  }).join('\n');
...
<body>
  ...
  <main class="roadmap-page" data-url-key="${encodedUrlKey}">
    ...
    <div class="roadmap-milestones">
      <h2 class="roadmap-section-heading">│ Projects</h2>
