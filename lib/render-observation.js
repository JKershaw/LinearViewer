...
function renderBulkScanDisclaimer(isFreeTier) {
  const quotaNote = isFreeTier
    ? `<p class="obs-bulk-scan-quota-note" id="obs-due-bulk-quota-note">You&rsquo;re on the free tier: each scan you send also draws against your daily/hourly free quota, a separate gate from the dollar estimate above &mdash; a scan can be refused there even when the estimate looks affordable.</p>`
    : '';
  return `<p class="obs-bulk-scan-limits" id="obs-due-bulk-disclaimer">The estimate above is a 30-day historical mean, not a quote &mdash; actual per-scan cost can vary. Abort stops the queue and releases the browser; scans already sent will finish, bill, and may still raise a ruling. Scans this queue runs are judged behind the same Principle&nbsp;0 gate as any other scan &mdash; a backstop against missed risk, not a promise nothing gets missed.</p>
      ${quotaNote}`;
}
