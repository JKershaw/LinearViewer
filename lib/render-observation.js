...
export function renderObservationPage(data, options = {}) {
  ...
  // nav's workspace selector; `← projects` is emitted by the nav for
  // non-projects pages; the decorative health dot (deferred design Q6, no real
  // wiring) is dropped. (The obs-appbar's sticky/translucent feel is not carried
  // over: a global sticky header intercepts clicks on scrolled content, so the
  // shared nav stays in normal flow — see the note in style.css `.nav-bar`.)
