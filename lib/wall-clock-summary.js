...
export const BUCKET_OF_KIND = {
  ...
  review: 'after', 'close-out': 'after', retro: 'after', 'retrospective-audit': 'after',
...
// LIN-3037 (Surface 11): inline copy of the 3 decision-lifecycle stamp kinds
// (lib/digest-feedback.js's isDecisionLifecycleStampEntry), not imported —
// consistent with the same inline-literal choice in session-telemetry.js,
// which this module already sources parseHeartbeats from.
...
export function decomposeEffort(step = {}) {
  ...
  // LIN-2641: this `wallMs` is STEP-SCOPED (this row's own feedback only,
  // preferring a caller-supplied `completedAt`) — a DIFFERENT derivation from
  // `lib/task-cost.js`'s `buildTaskCost`, which merges a whole lineage's
  // feedback first (anchor-scoped, one figure per lineage). The two diverge
  ...
  // `reason`) can incidentally contain a CI/test word and flip `touchedCi`
  // on prose that was never a CI/test signal.
  const touchedCi = feedback.some((f) =>
    !DECISION_LIFECYCLE_STAMP_KINDS.has(f?.kind) && CI_TEST_SIGNATURE.test(f?.message || ''));
