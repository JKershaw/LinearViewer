...
// Attachment relay byte cap (LIN-679 Stage 3a / LIN-2536): a plain export
// rather than an injected dep, matching GITHUB_UPLOAD_HOSTS above — this is
// the exact existing precedent for a two-consumer (group D relay, group E
// upload) constant living here with no injection.
export const MAX_ATTACHMENT_BYTES = 10 * 1024 * 1024; // 10 MB — matches /api/image
...
export function flattenIssue(issue) {
  ...
  // priorityLevel (LIN-2239): derived, ADDITIVE canonical ascending priority
  // (0=unknown,1=lowest…4=highest), alongside the untouched Linear-native
  // `priority`/`priorityLabel`. One mapping, no per-provider branch needed:
  // every provider without Linear's priority scale already encodes "no
  // signal" as native `priority: 0` (see each provider's `_toCanonicalIssue`),
  // and `linearPriorityToCanonical(0) === 0` regardless of source — so the
  // same formula is simultaneously "Linear's inverse" and "unknown passthrough"
  // for every other provider. Gated on `priority` being present so a shape
  // that never selected it (none exist today) stays untouched.
  if (issue.priority !== undefined) {
    issue.priorityLevel = linearPriorityToCanonical(issue.priority);
  }
