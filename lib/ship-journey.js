...
// Display-oriented cap, distinct from the persistence-layer cap
// (MAX_ORIENTATION_FIELD_CHARS = 2000, lib/report-history-store.js:57). A
// waypoint's `reason` is embedded into every page load's __SHIP_JOURNEY_DATA__
// blob (lib/render-ship-journey.js), so it needs its own much smaller,
// caption/tooltip-sized budget — see LIN-2066 plan for the sizing rationale.
export const MAX_WAYPOINT_REASON_CHARS = 240;
...
/**
 * Truncate a waypoint's display reason to MAX_WAYPOINT_REASON_CHARS,
 * ellipsised on truncation. Falsy input (including '') normalizes to null.
 * @param {string|null} reason
 * @returns {string|null}
 */
function truncateWaypointReason(reason) {
  if (!reason) return null;
  return reason.length > MAX_WAYPOINT_REASON_CHARS
    ? `${reason.slice(0, MAX_WAYPOINT_REASON_CHARS)}…`
    : reason;
}
 ...
 *
 *   candidateReadings  Map(identifier → { bearing, reason, sourceRunId,
 *                      sourceRunGeneratedAt }) — overwritten on every
 ...
 *                      bookkeeping needed. `reason` is the raw persisted string
 *                      (possibly ''); `sourceRunId`/`sourceRunGeneratedAt` are
 *                      the `id`/`generatedAt` of the report that supplied this
 *                      reading.
    ...
    if (report.orientation && report.orientation.length > 0) {
      for (const entry of report.orientation) {
        if (!entry.archived && entry.identifier && entry.bearing) {
          candidateReadings.set(entry.identifier, {
            bearing: entry.bearing,
            reason: typeof entry.reason === 'string' ? entry.reason : '',
            sourceRunId: report.id,
            sourceRunGeneratedAt: report.generatedAt
          });
        }
      }
    }
 ...
 *
 * @param {Map<string, {bearing: string, reason: string, sourceRunId: string, sourceRunGeneratedAt: string}>} candidateReadings
 * @param {Map<string, Object>} issueIndex
 * @returns {Array<{identifier: string, bearing: string, angle: number, completedAt: string, title: string|null, topic: string|null, reason: string|null, source: {id: string, generatedAt: string}}>}
 */
function deriveWaypoints(candidateReadings, issueIndex) {
  const waypoints = [];

  for (const [identifier, reading] of candidateReadings) {
    ...
    waypoints.push({
      ...
      angle: BEARING_TO_ANGLE[reading.bearing],
      completedAt: issue.completedAt,
      title: issue.title || null,
      topic: issue.project?.name || null,
      reason: truncateWaypointReason(reading.reason || null),
      source: { id: reading.sourceRunId, generatedAt: reading.sourceRunGeneratedAt }
    });
  }
 ...
 * An exact reversal (180° apart) always resolves to -180, in either
 * direction.
 * @param {number} target
 ...
 * waypoint's own bearing angle (no clamp on step one); every later waypoint,
 * north-star break or not, takes the same bounded-turn step from wherever the
 * walk already is (John's 2026-09-20 ruling on LIN-2956: a north-star change
 * continues the walk rather than resetting it to a fresh berth at the
 * origin). A break is purely a *rendering* concept — public/ship-journey.js
 * still draws no connecting line across it and marks it with a ★ — invisible
 * to this derivation, which is why `starChanges` is no longer a parameter
 * here; the client computes its own `breakBefore` from `starChanges` and
 * `completedAt` directly (there is no server-side twin of that computation
 * to keep in sync with any more).
 *
 * An out-of-vocabulary bearing (`angle` not finite — `BEARING_TO_ANGLE` has
 * no entry for it) holds the current heading steady for that step instead of
 * turning toward an undefined target (review F1, LIN-2065): the old `toXY`
 * guarded the same lookup with `(wp.angle || 0)`, and an unguarded carry
 * forward here turns one bad reading into `NaN` for every waypoint behind it.
 * A waypoint still gets a placed `x`/`y` either way — every `completedAt`-
 * bearing waypoint reaching render.js's `placeableWaypoints` filter must
 * carry a position, since that filter does not also check for one. At the
 * very first step (no prior heading to hold), an unknown bearing falls back
 * to angle 0, the same default `toXY` used.
 *
 ...
 */
function derivePositions(waypoints) {
  ...
  for (let i = 0; i < placeable.length; i++) {
    const rawAngle = placeable[i].angle;
    const hasKnownBearing = Number.isFinite(rawAngle);
    if (i === 0) {
      heading = hasKnownBearing ? rawAngle : 0;
    } else if (hasKnownBearing) {
      heading = normalizeAngle(heading + clampTurn(shortestAngleDelta(rawAngle, heading)));
    }
    // else: unknown bearing — hold heading steady, do not turn.
 ...
 *
 *         waypoints[]     { identifier, bearing, angle, completedAt, title,
 *                          topic, reason, source: {id, generatedAt}, x?, y? }
 *                          ascending by completedAt. `title` is the issue
 *                          title; `topic` is `issue.project.name`; `reason` is
 *                          the retained reading's rationale, display-truncated
 *                          to `MAX_WAYPOINT_REASON_CHARS`; `source` is the
 *                          `id`/`generatedAt` of the report that supplied the
 *                          retained bearing. All four are `null` when absent,
 *                          never invented. `x`/`y` are the heading-inertia
 *                          walk position (see `derivePositions`) and are
 *                          present only on placeable waypoints (a truthy
 *                          `completedAt`).
 *
...
export function deriveJourney({ reports, issues }) {
  ...
  const positions = derivePositions(waypoints);
