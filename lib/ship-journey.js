 ...
 * in the segment. A waypoint still gets a placed `x`/`y` either way — every
 * `completedAt`-bearing waypoint reaching render.js's `placeableWaypoints`
 * filter must carry a position, since that filter does not also check for
 * one. At the very first step of a segment (no prior heading to hold), an
 * unknown bearing falls back to angle 0, the same default `toXY` used.
 *
...
function derivePositions(waypoints, starChanges) {
  ...
  for (let i = 0; i < placeable.length; i++) {
    ...
    // else: unknown bearing mid-segment — hold heading steady, do not turn.
