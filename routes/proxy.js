...
POST ${baseUrl}/api/proxy/recommend-and-dispatch
  ...
  → "repo" (optional) overrides the project's "repo=" inheritance for the dispatched item. An opaque string: max 1000 characters (UTF-16 code units), no control characters — violating either returns a 400 naming the constraint and the received length. Omitted or explicit "null" are both accepted as absent, so the project-derived "repo=" (or none) is used instead. See LIN-2075.
...
POST ${baseUrl}/api/proxy/autopilot/kickoff
  ...
  → "goal" (optional, GENERAL runs only — a SCOPED run pins the goal to "issueIdentifier" and otherwise ignores this) is free text steering the stack walk. An opaque string: max 1000 characters (UTF-16 code units), no control characters — violating either returns a 400 naming the constraint and the received length. Omitted or explicit "null" are both accepted as absent, walking the stack under the default precedence policy. See LIN-2075.
  → "repo" (optional) overrides the project's "repo=" inheritance for a SCOPED run. An opaque string, same validation as "goal": max 1000 characters (UTF-16 code units), no control characters, 400 on violation naming the constraint and the received length. Omitted or explicit "null" are both accepted as absent, so the project-derived "repo=" (or none) is used instead. See LIN-2075.
