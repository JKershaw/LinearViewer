...
routes/
  ...
  jira-auth.js         Jira Cloud API-token Basic-auth link routes (LIN-1885, Phase 1 of LIN-275): GET /auth/jira?workspace=<urlKey> (renders the {email,apiToken,site} link form) + POST /auth/jira/link (validate-then-link, synchronous — no OAuth round-trip, no session-carried `mode`; closer to the Local provider's non-OAuth create than to GitHub's picker). Validates via a lightweight read probe (GET /rest/api/3/myself) before writing the binding; add-source only this phase, no top-level "Continue with Jira" entry point
