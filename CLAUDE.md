...
lib/
  ...
  providers/           Provider abstraction (decouples views from Linear specifics)
    ...
    interface.js       Provider interface contract. Also carries the **declared identity surface** (LIN-2010): `landingCatalogue` (`{blurb, order}`), `addProvider` (`{authShapes, addHint, blockedBy, configPredicate}`) and `entryCta` (`{href, isConfigured()}`), each defaulting to `null` on the base class — `null` means "absent from that surface", the fail-safe (e.g. `local` declares no `addProvider`, `github-projects` no `entryCta`). Extends the `refresh-strategy.js` declared-table idiom to identity so render surfaces read the registry instead of a parallel list. `ui.displayName` stays the single source of truth for the name (deliberately not duplicated); CTA copy/classes/testids stay surface-owned
