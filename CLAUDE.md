...
public/
  ...
  live-console.css / live-console.js  Live Console view (experimental, liveConsole flag): polls the events endpoint every 5s (pauses on tab-hidden) and paints the ambient surfaces — status banner + canvas tempo sparkline, breathing pulse-lanes for working agents, and a newest-first activity stream where genuinely-new events animate in; semantic-token-only (dark-safe), reduced-motion aware; LIN-1436
