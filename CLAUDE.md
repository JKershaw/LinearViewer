...
public/
  ...
  ship-journey.css / ship-journey.js  Ship Journey view (experimental, shipJourney flag, LIN-1675 P3): plays back the embedded, server-filtered waypoint list as a spiralling SVG trail (bearing → angle per lib/ship-layout.js's BEARING_TO_ANGLE convention, sequence position → radius, so the trail grows outward over time) with play/pause/step/scrub controls; a north-star change breaks the trail into a new segment (no connecting line across the change) and marks the new segment's first waypoint with a ★; `window.computeFitZoom` (common.js) recomputes on every frame from the bounding box of the currently revealed points, so the view zooms out as playback advances. No-ops entirely when the map mount is absent (the thin-data empty state path)
