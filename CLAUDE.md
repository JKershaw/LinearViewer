...
lib/
  ...
  components/
    navbar.js          Nav bar with workspace/team selectors, queue badge. The `isLanding` bar (sole sign-in path on the swipe/swim/ship previews) CALLS `isJiraOAuthConfigured()` directly rather than taking a threaded option — there is no threading path here, and ~7 page renderers would each have to remember to pass one (LIN-1890 E4). Its GitHub CTA deliberately still reads `process.env.GITHUB_CLIENT_ID` inline: swapping it for `isGitHubConfigured()` requires all five GITHUB_* vars and so is a real behaviour change owned by GitHub's lane, recorded not taken. Each CTA carries its own testid (`nav-login-linear`/`-github`/`-jira`) because the shared `nav a.login` class is ambiguous once more than one renders
    footer.js          Footer with deploy info, AI status
    landing-hero.js    Landing brand hero (anchor mark + wordmark + Linear/GitHub/Jira CTAs; GitHub gated on GITHUB_CLIENT_ID, Jira on the threaded `jiraEnabled` = `isJiraOAuthConfigured()`; LIN-726/LIN-1890)
content/
