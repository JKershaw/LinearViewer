...
routes/
  ...
  live-console.js      Live Console view (experimental, liveConsole flag): ambient "watch the swarm" page + generation-free events endpoint (GET /api/live-console/events) that merges the agent-status feed across every connected workspace and shapes it via lib/live-console.js; poll spends no LLM call (LIN-1436)
...
lib/
  ...
  live-console.js      Live Console data layer (experimental): pure, generation-free transform from workspace-tagged agent-status entries → { events (newest-first stream), lanes (currently-working agents), tempo (event-rate buckets for the sparkline), summary (fleet totals) }; tolerant/deterministic (`now` injected), same discipline as session-telemetry.js (LIN-1436)
